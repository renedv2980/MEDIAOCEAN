*          DATA SET CTREP9102S AT LEVEL 111 AS OF 05/01/02                      
*PHASE CT9102A,+0                                                               
...
REPPMKT6 ZIC   R6,1(R4)                                                         
         ...
         SR    R7,R7                                                            
         LA    R4,3(R4)                                                         
         CH    R6,=H'15'           IS MORE THEN 1 LINE NEEDED                   
...
REPPMKT8 BCT   R6,REPPMKT7                                                      
         ...
         B     EXIT                                                             
*-- PRINT ROUTINE FOR INVALID MARKET                                            
...
STARPT2A L     R4,FILEC                                                         
         ...
         L     RF,SVCALLCT         NUMBER OF ENTRIES                            
         MH    RF,=H'12'                                                        
         LA    RF,STACALLT(RF)     POINT TO NEXT SLOT                           
STARPT3  CLI   0(RE),0                                                          
         BE    STARPT2                                                          
...
STARPT4  ZIC   R1,1(RE)            NEXT ELEMENT                                 
         AR    RE,R1                                                            
         B     STARPT3                                                          
STARPT5  MVC   0(5,RF),2(RE)       SAVE OLD/BOOK/NEW                            
         MVC   5(2,RF),CONBOOK                                                  
         MVC   7(5,RF),7(RE)                                                    
         L     R1,SVCALLCT         BUMP ENTRY COUNTER                           
         LA    R1,1(R1)                                                         
         ST    R1,SVCALLCT                                                      
         LA    RF,12(RF)           NEXT SLOT                                    
         B     STARPT4                                                          
         EJECT                                                                  
STARPT6  OC    SVCALLCT,SVCALLCT   ANY ENTRIES                                  
         BZ    STARPT13            GET NEXT RECORD                              
         L     RF,SVCALLCT                                                      
         GOTO1 XSORT,DMCB,(0,STACALLT),(RF),12,7,0                              
         L     RF,SVCALLCT         TABLE IS SORTED                              
         SR    RE,RE               SET UP PRINT COLUMNS                         
         D     RE,=F'45'           RF=NUMBER OF COLUMNS                         
         LTR   RE,RE               ANY REMAINDER                                
         BZ    *+8                                                              
         LA    RF,1(RF)            BUMP NUMBER OF COLUMNS                       
         LA    R1,STATLINK         SET UP FOR COLUMN PRINTING                   
         XC    STATLINK,STATLINK                                                
         LA    R5,STACALLT                                                      
STARPT7  ST    R5,0(R1)            SAVE ADDRESS AND NUMBER OF ITEMS             
         MVI   0(R1),45                                                         
         LA    R1,4(R1)                                                         
         AH    R5,=H'540'          SET ADDRESS TO NEXT ITEM                     
*                                  (45 * 12)=540                                
         BCT   RF,STARPT7                                                       
         SH    R1,=H'4'                                                         
         LTR   RE,RE                                                            
         BZ    *+8                                                              
         STC   RE,0(R1)            SAVE NUMBER OF ENTRIES IN                    
*                                  LAST SLOT                                    
         SPACE 2                                                                
         MVI   FORCEHED,C'Y'                                                    
         MVC   HEAD3+7(L'PRTSRC),PRTSRC                                         
         MVC   HEAD4+7(L'PRTMED),PRTMED                                         
         LA    R1,6                SET UP FOR HEADLINES                         
         LA    R5,STATLINK                                                      
         LA    R6,HEAD7                                                         
STARPT8  CLI   0(R5),0                                                          
         BE    STARPT9                                                          
         MVC   0(17,R6),=C'            START'                                   
         MVC   132(17,R6),=C' OLD   NEW   BOOK'                                 
         MVC   264(17,R6),=C' ---   ---   ----'                                 
         LA    R6,20(R6)                                                        
         LA    R5,4(R5)                                                         
         BCT   R1,STARPT8                                                       
         SPACE 2                                                                
STARPT9  LA    R5,STATLINK         PRINT REPORT                                 
STARPT9A CLI   0(R5),X'FF'         ANY DATA IN FIRST COL                        
         BE    STARPT9B                                                         
         CLI   0(R5),0             ANY DATA IN FIRST COL                        
         BNE   STARPT9C             YES - PRINT IT                              
         MVI   0(R5),255                                                        
         MVI   FORCEHED,C'Y'                                                    
         SPACE 2                                                                
         MVC   HEAD3+7(L'PRTSRC),PRTSRC                                         
         ...
         LA    R1,6                SET UP FOR HEADLINES                         
         LA    R6,HEAD7                                                         
STARLP   MVC   0(17,R6),=C'            START'                                   
         MVC   132(17,R6),=C' OLD   NEW   BOOK'                                 
         MVC   264(17,R6),=C' ---   ---   ----'                                 
         LA    R6,20(R6)                                                        
         BCT   R1,STARLP                                                        
         SPACE 2                                                                
STARPT9B LA    R5,24(R5)           NO - BUMP A FULL PAGE AND TRY AGAIN          
         LA    RF,STATLINK+92                                                   
         CR    R5,RF               NEED TO STOP AT END                          
         BNL   STARPT13                                                         
         B     STARPT9A                                                         
         SPACE 1                                                                
STARPT9C LA    R6,P                                                             
...
STARPT11 L     R7,0(R5)                                                         
         ...
         MVC   6(5,R6),7(R7)                                                    
         ST    R1,FULL                                                          
         GOTO1 DATCON,DMCB,(3,5(R7)),(6,12(R6))                                 
         L     R1,FULL                                                          
         MVI   ANYDATA,1                                                        
         ZIC   R8,0(R5)                                                         
         BCTR  R8,0                                                             
...
PDNAM7   DS    CL7                 7 CHAR NAME                                  
         DS    CL4                                                              
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'111CTREP9102S05/01/02'                                      
         END                                                                    
