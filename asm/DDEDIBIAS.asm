*          DATA SET DDEDIBIAS  AT LEVEL 012 AS OF 11/06/08                      
*PHASE EDIBIASA                                                                 
*INCLUDE BINSR31                                                                
*INCLUDE CARDS                                                                  
*INCLUDE DATCON                                                                 
...
***********************************************************************         
*                                                                     *         
*  TITLE:        EDIBIAS -- TRANSMIT PQ REPORTS VIA BIAS (APPC/MVS)   *         
*                                                                     *         
...
*                                                                     *         
***********************************************************************         
         TITLE 'DDEDIBIAS -- PERFORM BIAS TRANSMISSIONS USING APPC/MVS'         
         MACRO                                                                  
&NAME    PRNT  &A,&PRINT=                                                       
&NAME    DS    0H                                                               
         AIF   (T'&A EQ 'O').NOMOVE                                             
...
*                                                                               
         ...
         MVC   DCBDDNAM-IHADCB(8,RF),=C'BITRACE '  DDNAME=BITRACE               
*                                                                               
         MVC   TITLE(27),=C'EDICT: BIAS SENDING SUBTASK'                        
...
*                                                                               
         ...
         ATTACH EPLOC=(R2),ECB=BIRECB,PARAM=BIRPARMS,SZERO=NO                   
         ST    R1,BIRTCB                                                        
         OC    BIRTCB,BIRTCB                                                    
         BNZ   *+6                                                              
...
BADSHAKE PRNT  BADHANDSHAKE,PRINT=ALWAYS                                        
         GOTO1 =V(LOGIO),DMCB,X'FF000001',=C'EDICT01 *JDS/BIAS: ERROR D+        
               URING HANDSHAKE WITH SENDER'                                     
         B     STOP                HANDSHAKE FAILED                             
*                                                                               
SHAKEOK  GOTO1 =V(LOGIO),DMCB,X'FF000001',=C'EDICT01 *JDS/BIAS: HANDSHA+        
               KE WITH SENDER OK'                                               
         EJECT                                                                  
...
*                                                                               
         ...
         XC    0(4,RF),0(RF)                                                    
*                                                                               
STOP     PRNT  STOPPINGBIASRRCV,PRINT=ALWAYS                                    
         LA    R2,BIRPARMS         PARAMETERS TO RECEIVER                       
         USING BIRPARMD,R2                                                      
         POST  BIRSTOP             TELL EDIBIAR TO STOP                         
         DROP  R2                                                               
*                                                                               
...
DONTSTOP L     RF,ALOOKECB                                                      
         ...
         XC    0(4,RF),0(RF)                                                    
*                                                                               
         GOTO1 =A(XFERREPS),(RC)   TRANSMIT ALL BIAS REPORTS IN TABLE           
         B     LOOP                YES                                          
*                                                                               
CHKRCVR  TM    BIRECB,X'40'        IS BIAS RECEIVER STILL RUNNING?              
         BO    *+6                                                              
         DC    H'0'                HOW DID WE GET OUT OF THE WAIT?              
*                                                                               
         OC    BIRECB+1(3),BIRECB+1                                             
         BZ    DETACH              RECEIVER ENDED CLEANLY                       
*                                                                               
         GOTO1 =V(LOGIO),DMCB,X'FF000001',=C'EDICT01 *BIAS RECEIVER DIE+        
               D'                                                               
         ABEND 701,DUMP                                                         
         SPACE 3                                                                
         LTORG                                                                  
...
*                                                                               
         LA    R2,BIRPARMS           PARAMETERS TO RECEIVER                     
         USING BIRPARMD,R2                                                      
         MVC   BIRXMTTB,SXMTTBLE     A(REPORT TABLE)                            
         MVC   BIRMAJNM,SMAJORNM     A(MAJOR NAME FOR ENQ)                      
         MVC   BIRADMGR,SADTAMGR     A(DATAMGR)                                 
         MVC   BIREDADD,SEDCTADD     A(ROUTINE TO ADD EDICT RECORDS)            
         MVC   BIRTRACE,TRACEFLG     C'Y' = PRODUCE DETAILED TRACE              
         MVC   BIRWRITE,WRTSTAMP     C'Y' = WRITE TIMESTAMPS TO EDCTFIL         
         MVC   BIRLUNAM,=A(LOCAL_LU_NAME)   A(LOCAL_LU_NAME)                    
         MVC   BIRTPNAM,=A(TP_NAME)         A(TP_NAME)                          
         DROP  R2                                                               
*                                                                               
...
RC10     GOTO1 =V(CARDS),DMCB,CARD,=C'RE00'                                     
         ...
         BNE   *+6                                                              
         DC    H'0'                                                             
         CLC   =C'++BIAS',CARD     LOOK FOR START OF PARAMETERS                 
         BNE   RC10                                                             
*                                                                               
RC20     GOTO1 =V(CARDS),DMCB,CARD,=C'RE00'                                     
         ...
         BNE   *+6                                                              
         DC    H'0'                                                             
         CLC   =C'++BIASEND',CARD  LOOK FOR END OF PARAMETERS                   
         BE    RCX                                                              
*                                                                               
...
XFER20   MVC31 XMTENTRY,0(R6)                                                   
         ...
         BE    XFER50              YES, NO MORE                                 
         CLI   XMTSOURC,XMTSRCPQ   PRINT QUEUE ENTRY?                           
         BNE   XFER30                                                           
         CLI   XMTMETH,C'B'        YES -- BIAS METHOD?                          
         BNE   XFER30                                                           
...
XFER30   AHI   R6,XMTTBLQ          BUMP TO NEXT REPORT                          
         ...
         DROP  R3                                                               
*                                                                               
XFER50   OC    WORK,WORK           ANYTHING FOUND TO SEND?                      
         BZ    XFER90              NO                                           
...
*                                                                               
         ...
         GOTO1 =V(POSTSENT)                                                     
         BNE   XFER60              CAN'T POST EDICT FILE                        
         PRNT  RPTMARKEDJUNK,PRINT=ALWAYS                                       
         DROP  R1                                                               
XFER60   EQU   *                   BUILD MQ STATUS MESSAGE                      
         BRAS  RE,BLDMQS                                                        
         B     XFER80                                                           
*                                                                               
XFER70   LA    R3,TABLNTRY                                                      
         USING XMTTABLD,R3                                                      
...
*                                                                               
         GOTO1 =A(SENDBIAS),(RC)                                                
*                                                                               
XFER80   CLI   OPERSTOP,C'Y'       OPERATOR WANTS TO STOP?                      
         BNE   XFER10              NO                                           
         B     XFERX                                                            
*                                                                               
XFER90   L     R2,SMAJORNM         MAJOR RESOURCE NAME                          
         LA    R6,=C'XMTTABLE'     MINOR RESOURCE NAME                          
         MVC   P+30(8),0(R2)                                                    
         MVC   P+40(8),0(R6)                                                    
         PRNT  DEQUEUE                                                          
...
XFERX    XIT1                                                                   
         ...
         LTORG                                                                  
         EJECT                                                                  
SENDBIAS NMOD1 0,SENDBIAS                                                       
         LR    RC,R1               A(COMMON STORAGE AREA)                       
*                                                                               
* TRANSFER A REPORT VIA BIAS                                                    
*                                                                               
...
SNDBI40  XC    BUFFER,BUFFER                                                    
         ...
         BE    SNDBI60                                                          
*                                                                               
SNDBI50  MVC   P+30(14),=C'BAD BIAS WRITE'                                      
         B     CANTSEND            SKIP IT                                      
*                                                                               
SNDBI60  PRNT  SENTVIABIAS,PRINT=ALWAYS                                         
*                                                                               
         LA    R1,DMCB                                                          
         USING EDFPARMD,R1                                                      
         XC    0(EDFPRMLQ,R1),0(R1)                                             
         ...
         MVC   EADTAMGR,SADTAMGR                                                
         GOTO1 =V(POSTSENT)                                                     
         BNE   SNDBI80             CAN'T POST EDICT FILE                        
         DROP  R1                                                               
         PRNT  RPTMARKEDSENTDLVD,PRINT=ALWAYS                                   
*                                                                               
SNDBI80  EQU   *                   BUILD MQ STATUS MESSAGE                      
         BRAS  RE,BLDMQS                                                        
*                                                                               
SNDBIXIT XIT1                                                                   
         SPACE 2                                                                
...
APPCM2Q  EQU   *-APPCM2                                                         
         SPACE 2                                                                
APPCM3   DC    C'+++BIAS SEND TIME=HHMMSS'                                      
APPCM3Q  EQU   *-APPCM3                                                         
         SPACE 2                                                                
...
WAITSECS DC    F'12000'            2 MINS. FOR APPC CALLS TO COMPLETE           
         SPACE 2                                                                
RCVAMSG  DC    C'EDICT01 *JDS/BIAS: UNDEFINED LOCAL LUNAME XXXXXXXX. RE+        
               PLY RETRY OR CANCEL'                                             
RCVAMSGQ EQU   *-RCVAMSG                                                        
         SPACE 2                                                                
ECBLST   DS    0F                                                               
ALOOKECB DC    X'00',AL3(0)        A(LOOKECB)                                   
         DC    X'00',AL3(BIRECB)   A(RECEIVER ECB)                              
...
ASTPECB2 DC    X'00',AL3(0)        A(STOPECB)                                   
         ...
         SPACE 2                                                                
BIRPARMS DC    XL(BIRPRMLQ)'00'    PARAMETERS TO EDIBIAR (VIA R1)               
BIRTCB   DS    F                   TCB OF ATTACHED SUBTASK EDIBIAR              
BIRECB   DC    F'0'                ECB OF ATTACHED SUBTASK EDIBIAR              
*                                                                               
SUBTASK  DC    C'EDIBIAR '         RECEIVING SUBTASK NAME                       
...
* DDEDICTFIL                                                                    
       ...
       ++INCLUDE DDEDICTFIL                                                     
         ...
         SPACE 3                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'012DDEDIBIAS 11/06/08'                                      
         END                                                                    
