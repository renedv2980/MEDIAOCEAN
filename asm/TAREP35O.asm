*          DATA SET TAREP35O   AT LEVEL 243 AS OF 07/30/03                      
*PHASE T70335A,*                                                                
*                                                                               
*******  HANDLES SOAPS & SOPRES OPTION - DO NOT DELETE                          
*                                                                               
       ++INCLUDE TAREP35TXT                                                     
         TITLE 'T70335 - RETROACTIVE PAYMENT GENERATOR'                         
T70335   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 EPITABLQ,T70335,RA,R5                                            
         LR    R3,RC                                                            
         L     RC,0(R1)                                                         
...
*              MODE CONTROLLED ROUTINES                                         
         ...
         ST    R3,AEPITAB          A(EPISODE AIR DATE TABLE)                    
         SPACE 1                                                                
...
VOPT10   CLC   =C'TRACE',SCDATA1   TRACE                                        
         ...
         B     VOPT90                                                           
         SPACE 1                                                                
         CLC   =C'SOPRES',SCDATA1  PROCESS SOAP RESIDUAL RETROS ONLY            
         BNE   *+12                                                             
         OI    TROPTS2,TRSOPRES                                                 
         B     VOPT90                                                           
         SPACE 1                                                                
         CLC   =C'BOTH',SCDATA1    PROCESS PNH INCREASE & PYMT INCREASE         
         BNE   *+12                                                             
         OI    TROPTS2,TRBOTH                                                   
         B     VOPT90                                                           
         SPACE 1                                                                
...
******** MVC   TIFUN,=C'AFT'       ** USE AFT UNION FOR NOW**                   
         ...
         B     INIT20                                                           
*                                                                               
INIT5    TM    TROPTS2,TRSOPRES    PROCESS SOAP RESIDUAL RETROS ONLY            
         BZ    INIT10                                                           
         MVC   TIFUSE,=C'SO1'      ONLY INTERESTED IN SOR, SDR, SWR             
         NI    TIFUSE,X'7F'        TURN OFF X'80' BIT TO INDICATE FLIST         
         MVI   TIFCTYPE,CTYSOAP                                                 
         MVI   TIQDTYPE,TIQDPAY    SET FILTERING ON PAY DATE                    
         GOTO1 =A(BLDAIR),DMCB,(RC) BUILD EPI AIR DATE TABLE                    
         B     INIT20                                                           
*                                                                               
...
IOH60    CLI   TIMODE,PROCREC      PROCESS CHECK                                
         BNE   IOHX                                                             
         SPACE 1                                                                
IOH65    MVC   TRACHK,TIAREC       SET A(CHECK RECORD)                          
...
CALCINV  NTR1                                                                   
         ...
         XC    TRUNAMTS(TRUNALNQ),TRUNAMTS           UNION CHECK AMTS           
         SPACE 1                                                                
...
*              ROUTINE TO CALCULATE NEW CHECK                                   
         SPACE 1                                                                
CALCCHK  NTR1                                                                   
         LA    RE,SORTCHK          INITIALIZE CHECK-RELATED FLDS                
         LA    RF,SORTCHKL                                                      
         XCEFL                                                                  
         ...
         SPACE 1                                                                
         TM    TROPTS2,TRSOPRES    IF SOAP RESIDUALS                            
         BZ    CCHK10                                                           
         CLC   TISSN,=C'000002010' AND WGA OR DGA UNION CHECKS                  
         BE    CCHK5                                                            
         CLC   TISSN,=C'000002021'                                              
         BE    CCHK5                                                            
         CLC   TISSN,=C'000005051'                                              
         BNE   CCHK10                                                           
CCHK5    BAS   RE,SETUACCS         SET ACCUMULATORS WITH COMBINED TOTS          
         B     CCHKX                                                            
         SPACE 1                                                                
CCHK10   L     R4,TRACHK                                                        
         MVI   ELCODE,TACDELQ                                                   
...
CCHK30   TM    TGUSSTA2,NORATES    IF NO RATES DEFINED IN TABLE                 
         ...
         BNE   NO                                                               
         SPACE 1                                                                
         TM    TROPTS2,TRSOPRES    IF NOT SOAP RESIDUALS                        
         BO    *+12                                                             
...
CCHK50   MVI   SORTERR,ERR6        PAYMENT IS ZERO                              
         ...
         SPACE 1                                                                
CCHK60   TM    TROPTS,TRSOAPS      AND NOT SOAP RETROS                          
         BO    CCHK70                                                           
         TM    TROPTS2,TRSOPRES    AND NOT SOAP RESIDUAL RETROS                 
         BO    CCHK70                                                           
         ...
         B     CCHKX                                                            
         SPACE 1                                                                
CCHK70   TM    TROPTS2,TRSOPRES    AND NOT SOAP RESIDUAL RETROS                 
         BO    *+12                                                             
         BAS   RE,SETACCS          SET ACCUMS IN SORT REC (PASS R1=PMT)         
         B     *+12                                                             
         BAS   RE,SETEACCS         SET ACCUMS & EPI ACCUMS(PASS R1=PMT)         
         B     CCHKX                                                            
         SPACE 1                                                                
         TM    TROPTS2,TRBOTH       IF BOTH OPTION                              
         BZ    CCHKX                                                            
         CLC   TIBIDATE,=X'990101'                                              
         BL    CCHKX                                                            
         LA    R3,SORTACCS                                                      
         USING ACCD,R3                                                          
         ICM   R1,15,TAPDSPNH                                                   
...
SETTC    NTR1                                                                   
         ...
         SPACE 1                                                                
**NO-OP**TM    TGCATYPE,EXTRA      EXTRAS AREN'T ELIGIBLE FOR RETROS            
**NO-OP**BO    NO                                                               
         TM    TROPTS,TRSOAPS      IF SOAP RETROS                               
         BZ    SETC20                                                           
         CLI   TGCAEQU,CTP         PROCESS PART-TIME                            
         BE    SETC20                                                           
**NO-OP**CLI   TGCAEQU,CTANN       PROCESS ANNOUNCERS                           
**NO-OP**BE    SETC20                                                           
         CLI   TGCAEQU,CTX         EXTRAS                                       
         BE    SETC20                                                           
         CLI   TGCAEQU,CTX2                                                     
         BE    SETC20                                                           
...
SETC20   L     R4,TRACHK                                                        
         ...
         BZ    NO                  THEN IGNORE                                  
         SPACE 1                                                                
STC60    TM    TROPTS2,TRSOPRES    IF SOAP RESIDUALS                            
         BZ    *+12                                                             
         BAS   RE,SETEPI           SET EPISODE DETAILS                          
         BNE   NO                                                               
         SPACE 1                                                                
         L     R4,TRACHK                                                        
...
STCX     B     YES                                                              
         EJECT                                                                  
*              ROUTINE TO SET EPISODE DETAILS                                   
         SPACE 1                                                                
SETEPI   NTR1                                                                   
         MVC   TCAEPISA,AEPITAB    A(EPISODE AIR DATE TABLE)                    
*                                                                               
         LA    RE,BLOCK                                                         
         LA    RF,480                                                           
         XCEFL ,                                                                
         LA    R3,BLOCK                                                         
         ST    R3,TCATSAR          A(ECAST RECORD)                              
         LA    R3,ECSTEPIS-ECASTABD(R3)                                         
         USING EPISD,R3                                                         
*                                                                               
         L     R4,TRACHK                                                        
         MVI   ELCODE,TASOELQ                                                   
...
*                                                                               
         ...
         ZIC   RE,TCNEPIS                                                       
         AR    RE,R1                                                            
         STC   RE,TCNEPIS          # OF EPISODES                                
*                                                                               
         LR    R0,R4               SAVE R4                                      
         LA    R4,TASOSEPI                                                      
         USING TASOSEPI,R4                                                      
*                                                                               
SETEPI5  MVC   HALF,TASOEPI        GET AIR DATE INFO                            
         BAS   RE,CHKARNG          CHECK AIR DATE WITHIN RANGE                  
         BE    SETEPI8                                                          
         MVI   SORTERR,ERR11       SET ERROR                                    
         B     NO                                                               
*                                                                               
SETEPI8  MVC   EPINUM,TASOEPI      SET UP ECAST EPI INFO                        
         TM    TASOSTAT,TASOSH                                                  
         BZ    *+8                                                              
         OI    EPISTAT,EPISTHW                                                  
         TM    TASOSTAT,TASOSB                                                  
         ...
         BZ    *+12                                                             
         BAS   RE,SETWPCT          SET WRITER PERCENTAGE                        
         BNE   NO                                                               
         LA    R3,EPINUMLN(R3)                                                  
*                                                                               
SETEPI20 LA    R4,L'TASOSEPI(R4)                                                
         BCT   R1,SETEPI5                                                       
         LR    R4,R0               RESTORE R4                                   
         B     SETEPI2                                                          
         SPACE 2                                                                
*              ROUTINE CHECKS EPISODE IN TABLE                                  
         SPACE 2                                                                
CHKARNG  NTR1                                                                   
         L     R2,TCAEPISA                                                      
CHKARNG5 CLI   0(R2),X'FF'                                                      
         BE    NO                                                               
         CLC   0(2,R2),HALF                                                     
         BE    YES                                                              
         LA    R2,7(R2)                                                         
         B     CHKARNG5                                                         
         EJECT                                                                  
*              ROUTINE CALCULATES WRITER ROLE PCT                               
         SPACE 1                                                                
SETWPCT  NTR1                                                                   
         MVC   WORK(1),TASOSTAT    SAVE WRITER ROLES                            
         LH    RE,TASOEPI                                                       
         CVD   RE,DUB                                                           
         UNPK  TGEPI,DUB+5(3)                                                   
         OI    TGEPI+4,X'F0'                                                    
*                                                                               
         L     R2,=A(PCTWRTAB)                                                  
SETWPCT5 CLI   0(R2),X'FF'                                                      
         BE    SETWPCTX                                                         
         MVC   BYTE,WORK                                                        
         NC    BYTE,0(R2)          IF WRITER IS THIS ROLE                       
         BZ    SETWPCT8                                                         
*                                                                               
         BAS   RE,ECREAD           GET ECAST RECORD                             
         BNE   NO                                                               
*                                                                               
         L     R4,AIO              GET WRITER ROLE PCT FROM ECAST               
         MVI   ELCODE,TACAELQ                                                   
         BAS   RE,GETEL                                                         
         BE    *+6                                                              
         DC    H'0'                                                             
         ...
         L     R1,=F'100000'                                                    
*                                                                               
         ZIC   R0,1(R2)            TABLE PCT FOR ROLE                           
         MR    R0,R0                                                            
         D     R0,=F'50'                                                        
         LTR   R1,R1                                                            
         BM    *+8                                                              
...
SETWPCT8 LA    R2,2(R2)                                                         
         B     SETWPCT5                                                         
*                                                                               
SETWPCTX MVI   ELCODE,TASOELQ      RESET THE ELEMENT CODE                       
         B     YES                                                              
         SPACE 2                                                                
ECREAD   NTR1                                                                   
         MVC   TRCAT,TGCAT         SAVE CATEGORY                                
*                                                                               
         XC    TGINV,TGINV                                                      
         TM    TGCATYPE,WRITER                                                  
         BO    ECREAD10                                                         
         GOTO1 RECVAL,DMCB,TLECCCDQ,(X'20',0)                                   
         BE    ECREADY                                                          
         B     ECREADN                                                          
*                                                                               
...
ECREAD15 CLC   KEY(TLECCCAT-TLECPKEY),KEYSAVE                                   
         ...
         CLC   =CL3'W',TRCAT       CHECK COMPATABLE WRITER CATEGORY             
         BNE   ECREAD18                                                         
         CLI   TGCAEQU,CTWH                                                     
         BE    ECREAD20                                                         
         CLI   TGCAEQU,CTWS                                                     
         ...
         B     ECREAD12                                                         
*                                                                               
ECREAD18 CLC   =CL3'W2',TRCAT      CHECK COMPATABLE WRITER CATEGORY             
         BNE   ECREAD19                                                         
         CLI   TGCAEQU,CTW2H                                                    
         BE    ECREAD20                                                         
         CLI   TGCAEQU,CTW2S                                                    
         ...
         B     ECREAD12                                                         
*                                                                               
ECREAD19 CLC   =CL3'W3',TRCAT      CHECK COMPATABLE WRITER CATEGORY             
         BNE   ECREADN                                                          
         CLI   TGCAEQU,CTW3H                                                    
         BE    ECREAD20                                                         
         CLI   TGCAEQU,CTW3S                                                    
...
ECREAD20 GOTO1 GETREC                                                           
         ...
         USING TASOD,R4                                                         
         MVC   BYTE,TASOSTAT                                                    
         NC    BYTE,0(R2)          CHECK MATCH ON CURRENT WRITER ROLE           
         BZ    ECREAD12                                                         
         B     ECREADY                                                          
*                                                                               
ECREADN  MVI   SORTERR,ERR10                                                    
         LA    R0,TRECMAX                                                       
         LA    R4,TRECTAB                                                       
ECREADN2 OC    0(9,R4),0(R4)                                                    
         BZ    ECREADN4                                                         
         LA    R4,9(R4)                                                         
         BCT   R0,ECREADN2                                                      
         DC    H'0'                                                             
ECREADN4 MVC   0(9,R4),TGSSN                                                    
         GOTO1 CATVAL,DMCB,TRCAT   RESET CATEGORY                               
         B     NO                                                               
*                                                                               
ECREADY  GOTO1 CATVAL,DMCB,TRCAT   RESET CATEGORY                               
         B     YES                                                              
         EJECT                                                                  
...
SETEACCS NTR1                                                                   
         ...
         SPACE 2                                                                
*&&UK                                                                           
*              ROUTINE TO SET RETRO PAYMENT BY EPISODES                         
*              HANDLES APPLIED CREDITS AND TOTALS PAY, PNH, & MDED              
         SPACE 1                                                                
SETEACCS NTR1                                                                   
         LA    R3,SORTEACC                                                      
         USING ACCED,R3            R3=A(SORT RECORD ACCUMS)                     
*                                                                               
         ZIC   R0,TCNEPIS                                                       
         L     R2,TCATSAR                                                       
         LA    R2,ECSTEPIS-ECASTABD(R2)                                         
         USING EPISD,R2                                                         
*                                                                               
         MVI   SORTACDE,APPLOTH    DEFAULT TO OTHER APPLY CODE                  
*                                                                               
SETEACC5 XC    TCPENS,TCPENS                                                    
         XC    TCHLTH,TCHLTH                                                    
         XC    TCMDED,TCMDED                                                    
         MVC   ACCEEPI,EPINUM      EPISODE NUMBER                               
         LH    RE,EPINUM                                                        
         CVD   RE,DUB                                                           
         UNPK  TGEPI,DUB+5(3)                                                   
         OI    TGEPI+4,X'F0'                                                    
         BAS   RE,GETEOLD          GET OLD PAYMENT & APPL BY EPISODE            
*                                                                               
         BAS   RE,GETEAIR          IF AIR DATE FOR EPI ON/AFTER 3/1/95          
         BNE   SETEACC6            NO APPLIED CREDITS                           
*        TM    TGUNEQU,DGA         IF DIRECTORS                                 
         GOTO1 UNITEST,DMCB,TGUNEQUS,DGA,0,0,0                                  
         BO    SETEACC6            NO APPLIED CREDITS                           
*        TM    TGUNEQU,WGA         IF WRITERS                                   
         GOTO1 UNITEST,DMCB,TGUNEQUS,WGA,0,0,0                                  
         BO    SETEACC6            NO APPLIED CREDITS                           
*                                                                               
         BAS   RE,ECREAD           READ ECAST RECORD                            
         BNE   NO                                                               
         BAS   RE,GETEBAL          RETURN CREDIT BALANCE IN FULL                
         L     RE,FULL             IF CREDIT BALANCE                            
         LTR   RE,RE                                                            
         BZ    SETEACC6                                                         
*                                                                               
         XR    R1,R1                                                            
         ICM   R1,7,EPIPAY                                                      
         L     RF,TREOLD                                                        
         SR    R1,RF               R1=RETRO PAYMENT AMOUNT                      
*                                                                               
         CR    R1,RE               IF PAYMENT AMOUNT < CREDIT BALANCE           
         BNL   *+6                                                              
         LR    RE,R1               ONLY APPLY PAYMENT AMOUNT                    
         SR    R1,RE                                                            
         ST    R1,ACCEPAY          RETRO PAYMENT                                
         ST    RE,ACCEAPPL         APPLIED AMOUNT                               
         MVI   SORTACDE,APPLSESS   AND NEW APPLY CODE                           
         B     SETEACC8                                                         
*                                                                               
SETEACC6 MVC   ACCEAPPL,TREOLD     SET ORIG. PMT AS APPLIED BY EPISODE          
         XR    R1,R1                                                            
         ICM   R1,7,EPIPAY                                                      
         L     RF,TREOLD                                                        
         A     RF,TREOAPPL                                                      
         SR    R1,RF                                                            
         ST    R1,ACCEPAY          SET RETRO PAYMENT BY EPISODE                 
*                                                                               
SETEACC8 BAS   RE,PNHECALC         SET PNH AMT (ACCEPNH,TCPENS,TCHLTH)          
         BAS   RE,RUNTOTS          ADD TO TOTALS FOR THIS CHK & UNIONS          
         LA    R2,EPINUMLN(R2)     PROCESS NEXT EPISODE                         
         LA    R3,ACCELNQ(R3)                                                   
         BCT   R0,SETEACC5                                                      
         B     YES                 CC EQUAL                                     
         EJECT                                                                  
*              ROUTINE TO GET OLD PAYMENT & APPLIED AMOUNT BY EPISODE           
         SPACE 1                                                                
GETEOLD  NTR1                                                                   
         L     R4,TRACHK                                                        
         MVI   ELCODE,TASOELQ                                                   
...
GETEOLD5 CLC   TASOEPI,EPINUM                                                   
         ...
         B     GETEOLD2                                                         
*                                                                               
GETEOLD8 XC    TREOLD,TREOLD       SET ORIGINAL PAYMENT AMT BY EPISODE          
         MVC   TREOLD+1(3),TASOPAY                                              
         XC    TREOAPPL,TREOAPPL   SET ORIGINAL APPLIED AMT BY EPISODE          
         MVC   TREOAPPL+1(3),TASOAPPL                                           
         B     XIT                                                              
         EJECT                                                                  
*        ROUTINE CHECKS AIR DATE ON OR AFTER 3/1/95                             
         SPACE 1                                                                
GETEAIR  NTR1                                                                   
         L     R4,TCAEPISA                                                      
GETEAIR5 CLI   0(R4),X'FF'                                                      
         BE    NO                                                               
         CLC   0(2,R4),EPINUM      MATCH ON EPISODE NUMBER                      
         BE    *+12                                                             
         LA    R4,7(R4)                                                         
         B     GETEAIR5                                                         
*                                                                               
         CLC   2(3,R4),=X'950301'  IF AIR DATE ON OR AFTER 3/1/95               
         BNL   NO                  NO APPLIED CREDITS                           
         B     YES                                                              
         DROP  R2                                                               
         SPACE 2                                                                
*              ROUTINE RETURNS REDIT BALANCE IN FULL FOR EPISODE                
*                                  XIT - FULL=BALANCE                           
         SPACE 1                                                                
GETEBAL  NTR1                                                                   
         XC    FULL,FULL                                                        
         L     R4,AIO                                                           
         USING TLECD,R4                                                         
         TM    TLECSTAT,TLECSBAL   IF CREDIT BALANCE                            
         BZ    XIT                                                              
         MVI   ELCODE,TACRELQ                                                   
         BAS   RE,GETEL                                                         
         BE    *+6                                                              
         ...
         B     XIT                                                              
         EJECT                                                                  
         USING ACCED,R3            R3=A(SORT ACCUMS BY EPISODE)                 
PNHECALC NTR1                                                                   
         L     R1,ACCEPAY          AMOUNT SUBJECT TO P&H                        
*                                                                               
*        TM    TGUNEQU,DGA         FOR DIRECTORS                                
         GOTO1 UNITEST,DMCB,TGUNEQUS,DGA,0,0,0                                  
         BZ    PNHECAL2                                                         
         LR    R4,R1               SAVE AMOUNT SUBJECT TO P&H                   
         LA    R0,250              PNH FOR EMPLOYEE CONTRIB.                    
         BAS   RE,MULT                                                          
         ST    R1,TCMDED                                                        
         LR    R1,R4               RESTORE AMOUNT SUBJECT TO P&H                
         B     PNHECAL5                                                         
*                                                                               
*NHECAL2 TM    TGUNEQU,WGA         FOR WRITERS                                  
PNHECAL2 GOTO1 UNITEST,DMCB,TGUNEQUS,WGA,0,0,0                                  
         BZ    PNHECAL5                                                         
         LR    R4,R1               SAVE AMOUNT SUBJECT TO P&H                   
         LH    R0,=AL2(600)        PENSION RATE                                 
         BAS   RE,MULT                                                          
         L     RE,TCPENS           KEEP SEPERATE PENSION TOTAL                  
         AR    RE,R1                                                            
         ST    RE,TCPENS                                                        
*                                                                               
         LR    R2,R1               SAVE PENSION AMOUNT                          
         LH    R0,=AL2(650)        HEALTH RATE                                  
         LR    R1,R4               RESTORE AMOUNT SUBJECT TO P&H                
         BAS   RE,MULT                                                          
         L     RE,TCHLTH           KEEP SEPERATE HEALTH TOTAL                   
         AR    RE,R1                                                            
         ST    RE,TCHLTH                                                        
         AR    R2,R1               ADD HEALTH AMOUN TO PENSION AMOUNT           
         STH   R2,ACCEPNH          SAVE COMBINED P&H                            
         B     XIT                                                              
*                                                                               
PNHECAL5 LH    R0,TCPNHR           SET RATE                                     
         BAS   RE,MULT             MULTIPLY                                     
         STH   R1,ACCEPNH          SAVE P&H                                     
         B     XIT                                                              
         EJECT                                                                  
         USING TAPDD,R4            R4=A(PAYMENT DETAILS ELEMENT)                
RUNTOTS  NTR1                                                                   
         ...
         CLI   TAPDW4TY,TAW4TYES                                                
         BNE   RUNTOTS2                                                         
         L     R1,SORTACCS+ACCPAYI-ACCD                                         
         L     RE,ACCEPAY                                                       
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCPAYI-ACCD                                         
         B     RUNTOTS4                                                         
*                                                                               
RUNTOTS2 L     R1,SORTACCS+ACCPAYC-ACCD                                         
         L     RE,ACCEPAY                                                       
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCPAYC-ACCD                                         
*                                                                               
RUNTOTS4 L     R1,SORTACCS+ACCSPNH-ACCD                                         
         L     RE,ACCEPAY                                                       
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCSPNH-ACCD                                         
*                                                                               
         L     R1,SORTACCS+ACCAPPL-ACCD                                         
         L     RE,ACCEAPPL                                                      
         LCR   RE,RE               REVERSE SIGN                                 
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCAPPL-ACCD                                         
*                                                                               
         L     R1,SORTACCS+ACCPNH-ACCD                                          
         LH    RE,ACCEPNH                                                       
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCPNH-ACCD                                          
*                                                                               
*        CLI   TGUNEQU,DGA         IF DIRECTORS                                 
         GOTO1 UNITEST,DMCB,TGUNEQUS,DGA,0,0,0                                  
         BNE   RUNTOTS5                                                         
         L     R1,SORTACCS+ACCMDED-ACCD                                         
         L     RE,TCMDED                                                        
         AR    R1,RE                                                            
         ST    R1,SORTACCS+ACCMDED-ACCD                                         
*                                                                               
         L     R1,TRUNPNH          ADD TO RUNNING TOTAL FOR UNION               
         LH    RE,ACCEPNH                                                       
         AR    R1,RE                                                            
         ST    R1,TRUNPNH                                                       
         L     R1,TRUNMDED                                                      
         ...
         B     RUNTOTSX                                                         
*                                                                               
RUNTOTS5 CLI   TGUNEQU,WGA         IF WRITERS                                   
         BNE   RUNTOTSX                                                         
         L     R1,TRUNPEN          ADD TO SEP RUNNING TOTAL FOR UNIONS          
         A     R1,TCPENS                                                        
         ST    R1,TRUNPEN                                                       
         L     R1,TRUNHLTH                                                      
         A     R1,TCHLTH                                                        
...
*&&                                                                             
         EJECT                                                                  
*              ROUTINE TO SET ACCUMS IN SORT RECORD FOR UNION CHECK             
         SPACE 1                                                                
SETUACCS NTR1                                                                   
         LA    R3,SORTACCS                                                      
         USING ACCD,R3             R3=A(SORT RECORD ACCUMS)                     
...
*                                                                               
         ...
         L     R1,TRUNHLTH                                                      
*                                                                               
SETUACC5 ST    R1,ACCPAYC          SET PAYMENT AMOUNT                           
         L     R1,TRUNMDED                                                      
         LCR   R1,R1                                                            
         ST    R1,ACCMDED          SET MISC. DED                                
         B     XIT                                                              
         EJECT                                                                  
*                                  R1=AMOUNT, R0=RATE                           
*                                  RETURNS R1=RESULT                            
...
REPORT   NTR1                                                                   
         ...
         BZ    REPX                                                             
         LA    RE,TRSRTREC         PRE-CLEAR CURRENT SORT RECORD                
         LA    RF,SORTLNQ                                                       
         XCEFL ,                                                                
...
REPX     BAS   RE,REQTOTS          PRINT REQUEST TOTALS                         
         ...
         TM    TROPTS2,TRSOPRES                                                 
         BZ    XIT                                                              
         BAS   RE,ECERRS           PRINT ECAST SSN ERRORS                       
         B     XIT                                                              
         EJECT                                                                  
...
NEWAGY   NTR1                                                                   
         ...
         SPACE 1                                                                
         GOTO1 =A(GETINV),DMCB,(RC) GET NEXT INVOICE NUMBER                     
         B     XIT                                                              
         SPACE 3                                                                
...
WRIAGY   NTR1                                                                   
         ...
         BAS   RE,UPDAGY           UPDATE AGENCY RECORD                         
         GOTO1 MYTRACE,DMCB,(RC),=C'UPD MAGY'                                   
...
**NO-OP**MVC   LINEST,SORTEST      ESTIMATE NUMBER                              
         SPACE 1                                                                
...
NCHK20   MVC   AIO,=A(CHKIO)       SET I/O AREA TO A(CHECK RECORD)              
         ...
         GOTO1 EXTRACT             EXTRACT GLOBAL VALUES FROM RECORD            
         SPACE 1                                                                
         GOTO1 =A(ELPROC),DMCB,(RC) PROCESS ELEMENTS                            
         GOTO1 MYTRACE,DMCB,(RC),=C'RETRO CHK'                                  
         SPACE 1                                                                
         MVC   SYSFIL,=CL8'CHKFIL'                                              
         MVC   SYSDIR,=CL8'CHKDIR'                                              
         GOTO1 =A(ADDIT),DMCB,(RC) ADD THE RECORD                               
         ...
         BNE   NCHK70                                                           
NCHK60   TM    TROPTS2,TRSOPRES    IF SOAP RESIDUALS                            
         BZ    *+12                                                             
         BAS   RE,ETRACK           HANDLE ECAST/ETRACK RECORD MAINT.            
         B     NCHK70                                                           
         BAS   RE,FTRACK           HANDLE CAST/FTRACK RECORD MAINT.             
         SPACE 1                                                                
NCHK70   LA    R2,P                R2=A(PRINT LINE)                             
         ...
         TM    TROPTS2,TRSOPRES    IF SOAP RESIDUALS                            
         BZ    NCHK80                                                           
         GOTO1 =A(FMTEPI),DMCB,(RC) PRINT ACCUSM BY EPI                         
         SPACE 1                                                                
NCHK80   TM    TRAYSTAT,TAAYSCOD   IF THIS IS A COD AGENCY                      
         BZ    *+14                                                             
...
NEWINV   NTR1                                                                   
         ...
         XC    TLININV,HEXFFS      (COMPLEMENTED)                               
         SPACE 1                                                                
         GOTO1 =A(ELPROC),DMCB,(RC) PROCESS ELEMENTS                            
...
HISTCMNT NTR1                                                                   
         ...
         L     R1,AIO                                                           
         CLI   0(R1),TLFTCDQ       IF THIS IS FTRACK/ECTRACK RECORD             
         BE    HISTC10                                                          
         CLI   0(R1),TLETCDQ                                                    
         BNE   *+8                                                              
HISTC10  MVI   TACMTYPE,TACMTYPG   SET GENERAL TYPE                             
         SPACE 1                                                                
...
TRACK    NTR1                                                                   
         ...
         B     XIT                                                              
         EJECT                                                                  
*              ROUTINE HANDLES ECAST/ETRACK RECORD MAINTENANCE                  
         SPACE 1                                                                
ETRACK   NTR1                                                                   
         TM    TROPTS,TRPHONLY     GET OUT IF RUNNING P&H ONLY OPTION           
         BO    ETX                                                              
         ...
         SPACE 1                                                                
         LA    R0,MAXEPIS                                                       
         LA    R3,SORTEACC         R3=A(SORT RECORD ACCUMS BY EPISODE           
         USING ACCED,R3                                                         
*                                                                               
ET2      OC    0(ACCELNQ,R3),0(R3)                                              
         BZ    ETX                                                              
         OC    ACCEAPPL,ACCEAPPL   IF APPLIED AMOUNT                            
         BZ    ET50                                                             
         LH    RE,ACCEEPI                                                       
         CVD   RE,DUB                                                           
         UNPK  TGEPI,DUB+5(3)                                                   
         OI    TGEPI+4,X'F0'                                                    
         XC    TGINV,TGINV                                                      
         GOTO1 RECVAL,DMCB,TLECCCDQ,(X'20',0)  GET ECAST RECORD                 
         BE    *+6                                                              
         DC    H'0'                                                             
         SPACE 1                                                                
         L     R4,AIO                                                           
         ...
         BZ    ET50                                                             
*                                                                               
         MVI   ELCODE,TACRELQ      GET APPLIED CREDIT ELEMENT                   
         BAS   RE,GETEL                                                         
         BE    *+6                                                              
         DC    H'0'                                                             
         USING TACRD,R4                                                         
         SPACE 1                                                                
         L     R2,TACRBAL          R2=REMAINING BALANCE                         
         L     R1,ACCEAPPL         R1=APPLIED AMOUNT                            
         LCR   R1,R1                                                            
         ST    R1,TCAPPLCR         SAVE COMPLEMENTED APPLIED AMOUNT             
         LTR   R2,R2               TEST FOR REMAINING BALANCE                   
         BNP   ET25                                                             
         AR    R2,R1               ADD APPLIED CREDITS TO BALANCE               
         BNM   *+6                                                              
         XR    R2,R2               INSURE BALANCE DOESN'T GO NEGATIVE           
         ST    R2,TACRBAL                                                       
         MVC   TRBAL,TACRBAL       SAVE NEW BALANCE                             
         B     ET30                                                             
         SPACE 1                                                                
ET25     MVC   TRBAL,TACRBAL       SAVE CURRENT BALANCE                         
ET30     BAS   RE,WRICAST          WRITE BACK CAST RECORD                       
         BAS   RE,UPDTLET          * ALWAYS TRACK RETRO CHANGES *               
*                                                                               
ET50     LA    R3,ACCELNQ(R3)                                                   
         BCT   R0,ET2                                                           
*                                                                               
ETX      B     XIT                                                              
         EJECT                                                                  
*              ROUTINE TO ADD ETRACK RECORD                                     
         SPACE 1                                                                
UPDTLET  NTR1                                                                   
         MVC   AIO,AIO2            SET SECONDARY I/O AREA                       
         XC    KEY,KEY                                                          
         ...
         MVC   TLETCAT,TGCAT       CATEGORY CODE                                
         ...
         MVC   TAPDAPPL,TGDUB                                                   
         XC    TAPDPNH,TAPDPNH                                                  
         MVC   TAPDPNH(2),SORTEACC+ACCEPNH-ACCED                                
         MVC   TAPDPAYI,SORTEACC+ACCEPAY-ACCED                                  
         ...
         SPACE 1                                                                
         L     R3,AIO              R3=A(RECORD)                                 
         MVC   TLETINV,TRINV       ADD INVOICE NUMBER TO KEY                    
         SPACE 1                                                                
         BAS   RE,HISTCMNT         ADD HISTORY COMMENT TO RECORD                
         SPACE 1                                                                
         GOTO1 MYTRACE,DMCB,(RC),=C'ETRK'                                       
         SPACE 1                                                                
         GOTO1 =A(ADDIT),DMCB,(RC) ADD THE RECORD                               
         MVC   AIO,AIO1            RESTORE DEFAULT I/O AREA                     
         B     XIT                                                              
...
*&&UK                                                                           
ECERRS   NTR1                                                                   
         OC    TRECTAB,TRECTAB     IF ANY ERROR ECAST RECORDS                   
         BZ    YES                                                              
         MVC   P(40),=CL40'ERROR 10 - ECAST RECORD NOT FOUND'                   
         MVI   SPACING,2                                                        
         BAS   RE,PRNTIT                                                        
         SPACE 1                                                                
         LA    R0,TRECMAX                                                       
         LA    R3,TRECTAB                                                       
ECERRS5  OC    0(9,R3),0(R3)                                                    
         BZ    YES                                                              
         MVC   P(9),0(R3)                                                       
         BAS   RE,PRNTIT                                                        
         LA    R3,9(R3)                                                         
...
FORMAT   NTR1                                                                   
         ...
         BZ    FMT10                                                            
         MVC   LINUSE(8),0(RF)     MOVE TO PRINT LINE                           
         SPACE 1                                                                
         CLI   RCSUBPRG,0          IF HIGHER THAN DETAIL                        
         BE    FMT10                                                            
         MVC   LINCID(8),LINUSE    SLIDE LITERAL OVER                           
         MVC   LINUSE(8),SPACES                                                 
...
FMT10    L     RE,ACCAPPL          ALWAYS DISPLAY APPLIED AMT AS POS.           
         ...
         EDIT  (4,ACCMDED),(11,LINMDED),2,FLOAT=-  MISC DED                     
         SPACE 1                                                                
...
PUTS10   GOTO1 SORTER,DMCB,=C'PUT',TRSRTREC                                     
         SPACE 1                                                                
**NO-OP**GOTO1 MYTRACE2,DMCB,(RC),=C'PUTSRT',TRSRTREC,SORTLNQ                   
         B     XIT                                                              
         SPACE 3                                                                
GETSORT  NTR1                                                                   
         LA    RE,TRLSTREC         SAVE CURRENT SORT RECORD                     
         LA    R0,TRSRTREC                                                      
         LA    RF,SORTLNQ                                                       
         LR    R1,RF                                                            
         MVCL  RE,R0                                                            
         ...
         LA    RF,SORTLNQ                                                       
         LR    R1,RF                                                            
         MVCL  RE,R0               MOVE CURRENT RECORD TO LOCAL W/S             
         SPACE 1                                                                
**NO-OP**GOTO1 MYTRACE2,DMCB,(RC),=C'GETSRT',TRSRTREC,SORTLNQ                   
         B     YES                                                              
         SPACE 3                                                                
...
*                                                                               
         ...
         DC    CL40'ERROR 10 - ECAST RECORD NOT FOUND'                          
         DC    CL40'ERROR 11 - EPISODE NOT WITHIN RANGE'                        
         DC    X'FF'                                                            
         SPACE 2                                                                
PCTWRTAB DS    0C                  TABLE OF PERCENTAGES FOR WRI ROLES           
         DC    AL1(TASOSH,35)                                                   
         DC    AL1(TASOSS,50)                                                   
         DC    AL1(TASOSB,15)                                                   
         DC    X'FF'                                                            
...
MYSPECS  DS    0H                                                               
         ...
         SSPEC H9,03,C'    ID'                                                  
         SPACE 1                                                                
         SSPEC H8,15,C' OLD    NEW     USE'                                     
         SSPEC H9,15,C' INV    INV'                                             
         SPACE 1                                                                
         SSPEC H8,39,C' S/S     CAT CAM A'                                      
         SSPEC H9,39,C'NUMBER'                                                  
         SPACE 1                                                                
         SPROG 0,1,2,3                                                          
         SSPEC H8,60,C'APPLIED    INDIVIDUAL  CORPORATE'                        
         SSPEC H9,60,C'AMOUNT       PAYMENT    PAYMENT'                         
         SPACE 1                                                                
         SSPEC H8,96,C'SUBJ TO    PENSION    MISC'                              
         SSPEC H9,96,CL33' P && H     && HEALTH   DEDUCTION'                    
         DC    X'00'                                                            
         EJECT                                                                  
*              ROUTINE TO BUILD AIR DATE TABLE FOR TASYSCALC                    
         SPACE 1                                                                
         DS    0D                                                               
BLDAIR   NMOD1 0,*BLDAIR*                                                       
         L     RC,0(R1)                                                         
         USING GEND,RC             GENCON DSECT                                 
*                                                                               
         MVC   AIO,AIO1                                                         
         LA    R3,EPIMAX           R3=MAX N'EPISODE ENTRIES                     
         L     R2,AEPITAB          R2=A(EPI AIR DATE TABLE)                     
*                                                                               
         XC    KEY,KEY                                                          
         LA    R6,KEY                                                           
         USING TLEPD,R6                                                         
         ...
         B     BLDAIR5                                                          
BLDAIR2  GOTO1 SEQ                                                              
BLDAIR5  CLC   KEY(TLEPEPI-TLEPD),KEYSAVE                                       
         BE    BLDAIR8                                                          
         MVI   0(R2),X'FF'         MARK END OF TABLE                            
         XIT1                                                                   
*                                                                               
BLDAIR8  GOTO1 GETREC                                                           
         L     R6,AIO                                                           
         LR    R4,R6                                                            
         MVI   ELCODE,TAEIELQ                                                   
         BAS   RE,GETEL2                                                        
         ...
         DC    H'0'                                                             
         USING TAEID,R4                                                         
         CLC   TAEIADT,=X'930101'                                               
         BL    BLDAIR2                                                          
         CLC   TAEIADT,=X'990331'                                               
         BH    BLDAIR2                                                          
         MVC   WORK(L'TLEPEPI),TLEPEPI                                          
         XC    WORK(L'TLEPEPI),HEXFFS                                           
         PACK  DUB,WORK(L'TLEPEPI)                                              
         CVB   R0,DUB                                                           
         STH   R0,0(R2)                                                         
         MVC   2(3,R2),TAEIADT                                                  
         LA    R2,7(R2)                                                         
         BCT   R3,BLDAIR2                                                       
         DC    H'0'                MAKE AIR TABLE BIGGER                        
         DROP  R6,R4                                                            
         SPACE 2                                                                
...
*                                                                               
         ...
         BE    *+6                                                              
         DC    H'0'                                                             
         GOTO1 MYTRACE,DMCB,(RC),=C'MAGY - GETINV'                              
         SPACE 1                                                                
...
*                                                                               
         ...
         DC    H'0'                                                             
         SPACE 1                                                                
ELP10    LA    R2,ELTAB            R2=A(ELEMENT PROCESSING TABLE)               
         SPACE 1                                                                
...
ELP30    XR    RF,RF                                                            
         ...
         BR    RF                                                               
         SPACE 1                                                                
ELP40    BAS   RE,NEXTEL4          GET NEXT ELEMENT                             
         BE    ELP10                                                            
         SPACE 1                                                                
ELPROCX  XIT1                                                                   
         EJECT                                                                  
...
PDPROC   DS    0H                                                               
         ...
         L     R1,ACCPAYI          PAYI+PAYC=GRS (APPL EXCLUDED)                
         ST    R1,TAPDPAYI                                                      
         L     R0,ACCPAYC                                                       
         ST    R0,TAPDPAYC                                                      
         AR    R1,R0                                                            
         ...
         SPACE 1                                                                
         CLC   TAPDCOM,=X'00045EDB' FOR WRITER OR DIRECTOR COMMLS               
         BE    PDP20                                                            
         CLC   TAPDCOM,=X'00045EC9'                                             
         BE    PDP20                                                            
PDP10    MVC   TAPDPNH,ACCPNH      DON'T SET PNH                                
         SPACE 1                                                                
PDP20    TM    TRSTAT,TRPROCIN     FOR CHECK RECORD ONLY                        
         BO    PDP30                                                            
         MVC   TAPDMDED,ACCMDED    SET MISC DEDUCTIONS                          
         SPACE 1                                                                
PDP30    TM    TROPTS,TRCURRNT     IF WE NEED CURRENT COMML VALUES              
         BZ    PDPX                                                             
...
*              ELEMENT PROCESSING ROUTINES                                      
         SPACE 1                                                                
         USING TASOD,R4            R4=A(INVOICE STATUS EL.)                     
SOPROC   DS    0H                                                               
         ZIC   R0,TASONUM                                                       
         LA    R4,TASOSEPI                                                      
         USING TASOSEPI,R4                                                      
*                                                                               
SOPROC2  LA    RE,MAXEPIS                                                       
         LA    R3,SORTEACC         R3=A(SORT RECORD ACCUMS BY EPI)              
         USING ACCED,R3                                                         
SOPROC5  OC    0(ACCELNQ,R3),0(R3)                                              
         BZ    SOPROC7                                                          
         CLC   TASOEPI,ACCEEPI                                                  
         BE    SOPROC8                                                          
         LA    R3,ACCELNQ(R3)                                                   
         BCT   RE,SOPROC5                                                       
*                                                                               
SOPROC7  XC    TASOPAY,TASOPAY     NO RETRO PAYMENT FOR THIS EPISODE            
         XC    TASOAPPL,TASOAPPL                                                
         XC    TASOPNH,TASOPNH                                                  
         B     SOPROC10                                                         
*                                                                               
SOPROC8  L     RE,ACCEPAY                                                       
         STCM  RE,7,TASOPAY                                                     
         L     RE,ACCEAPPL                                                      
         STCM  RE,7,TASOAPPL                                                    
         MVC   TASOPNH,ACCEPNH                                                  
...
ELTAB    DS    0F                  * INVOICE RECORD ELEMENTS                    
         ...
         DC    AL1(TAPAELQ),AL3(0)       PAY DETAILS                            
         DC    AL1(TASOELQ),AL3(SOPROC)  SOAP DETAILS                           
...
*                                  * COMMON ELEMENTS                            
         ...
         EJECT                                                                  
         SPACE 3                                                                
*              ROUTINE TO PRNT ACCUMS BY EPISODE                                
         SPACE 1                                                                
         DS    0D                                                               
FMTEPI   NMOD1 0,*FMTEPI*                                                       
         L     RC,0(R1)                                                         
         USING GEND,RC             GENCON DSECT                                 
*                                                                               
         LA    R0,MAXEPIS                                                       
         LA    R3,SORTEACC         R3=A(ACCUMS BY EPISODE)                      
         USING ACCED,R3                                                         
         LA    R2,P                R2=A(PRINT LINE)                             
         USING LINED,R2                                                         
*                                                                               
FMTE10   OC    0(ACCELNQ,R3),0(R3)                                              
         BZ    FMTE20                                                           
         LH    R1,ACCEEPI                                                       
         CVD   R1,DUB                                                           
         UNPK  LINSSN+4(5),DUB+5(3)                                             
         OI    LINSSN+8,X'F0'                                                   
         L     RE,ACCEAPPL         ALWAYS DISPLAY APPLIED AMT AS POS.           
         LPR   RE,RE                                                            
         EDIT  (RE),(12,LINAPPL),2,FLOAT=-     APPLIED AMOUNT                   
         EDIT  ACCEPAY,(12,LINPAYI),2,FLOAT=-  PAYMENT                          
         EDIT  ACCEPNH,(11,LINPNH),2,FLOAT=-   PNH                              
         BAS   RE,PRNTIT                                                        
         LA    R3,ACCELNQ(R3)                                                   
         BCT   R0,FMTE10                                                        
*                                                                               
...
FMTE21   BAS   RE,NEXTEL                                                        
         ...
         USING TASOSEPI,R4                                                      
*                                                                               
FMTE22   LA    RE,MAXEPIS                                                       
         LA    R3,SORTEACC         R3=A(SORT RECORD ACCUMS BY EPI)              
         USING ACCED,R3                                                         
FMTE25   OC    0(ACCELNQ,R3),0(R3)                                              
         BZ    FMTE30                                                           
         CLC   TASOEPI,ACCEEPI                                                  
         BE    FMTE40                                                           
         LA    R3,ACCELNQ(R3)                                                   
         BCT   RE,FMTE25                                                        
*                                                                               
FMTE30   LH    RE,TASOEPI          NO RETRO PAYMENT FOR THIS EPISODE            
         CVD   RE,DUB                                                           
         UNPK  LINSSN+4(5),DUB+5(3)                                             
         OI    LINSSN+8,X'F0'                                                   
         MVC   LINAPPL(12),=CL12'AIR DATE NOT'                                  
         MVC   LINPAYI(12),=CL12' IN RANGE'                                     
         BAS   RE,PRNTIT                                                        
*                                                                               
FMTE40   LA    R4,L'TASOSEPI(R4)                                                
         BCT   R0,FMTE22                                                        
         LR    R4,R1               RESTORE R4                                   
         B     FMTE21                                                           
*                                                                               
FMTEX    LA    RE,SORTEACC         CLEAR THIS LEVEL ACCUMS                      
         LA    RF,L'SORTEACC                                                    
         XCEFL ,                                                                
*                                                                               
         XIT1                                                                   
...
*                                                                               
         ...
         AP    TALCOUNT,=P'1'                                                   
         SPACE 1                                                                
         CLI   TLRCCD,TLFTCDQ      IF ADDING ETRACK/FTRACK RECORD               
         BE    ADDI10                                                           
         CLI   TLRCCD,TLETCDQ                                                   
         BE    ADDI10                                                           
         TM    TROPTS,TRWRIDSK     OR IF WRITING ALL RECORDS TO DISK            
         BZ    ADDI20                                                           
ADDI10   GOTO1 ADDREC              DO ADDREC                                    
         XC    ELEM,ELEM                                                        
         XR    RF,RF                                                            
...
*                                                                               
         ...
         CLC   TIBIDATE,=X'990101' UNTIL 1/1/99                                 
         BL    *+10                                                             
         MVC   TCPNHR,=H'1210'                                                  
*                                                                               
         L     R4,TRACHK                                                        
...
SOPTAB   DS    0CL13               * SOAP TABLE INCREASES                       
         DC    AL1(CTP),XL3'981123',XL3'990212',AL4(66200),AL2(2000)            
         DC    AL1(CTP),XL3'981123',XL3'990212',AL4(72820),AL2(2200)            
         DC    AL1(CTU5),XL3'981123',XL3'990212',AL4(28800),AL2(900)            
         DC    AL1(CTU5),XL3'981123',XL3'990212',AL4(31680),AL2(990)            
         DC    AL1(CTU52),XL3'981123',XL3'990212',AL4(28800),AL2(900)           
         DC    AL1(CTU52),XL3'981123',XL3'990212',AL4(31680),AL2(990)           
         DC    AL1(CTX),XL3'981123',XL3'990212',AL4(12800),AL2(400)             
         DC    AL1(CTX),XL3'981123',XL3'990212',AL4(14080),AL2(440)             
         DC    AL1(CTX2),XL3'981123',XL3'990212',AL4(12800),AL2(400)            
         DC    AL1(CTX2),XL3'981123',XL3'990212',AL4(14080),AL2(440)            
         DC    AL1(CTX),XL3'981123',XL3'990212',AL4(10240),AL2(320)             
         DC    AL1(CTX),XL3'981123',XL3'990212',AL4(11264),AL2(352)             
         DC    AL1(CTX2),XL3'981123',XL3'990212',AL4(10240),AL2(320)            
         DC    AL1(CTX2),XL3'981123',XL3'990212',AL4(11264),AL2(352)            
         DC    X'FF'                                                            
*                                                                               
...
AEPITAB  DS    A                   A(EPISODE AIR DATE TABLE)                    
EPIMAX   EQU   2000                                                             
EPITABLQ EQU   (EPIMAX*7)                                                       
*                                                                               
...
*                                                                               
TROPTS2  DS    XL1                 OPTIONS                                      
TRSOPRES EQU   X'80'               PROCESS SOAP RESIDUAL RETROS ONLY            
TRBOTH   EQU   X'40'               PNH INCREASE AND PAYMENT INCREASE            
*                                                                               
...
TREOLD   DS    F                   RATE UNDER OLD CONTRACT BY EPISODE           
TREOAPPL DS    F                   OLD APPLIED AMOUNT BY EPISODE                
*                                                                               
...
TRSVMAJ  DS    XL1                                                              
*                                                                               
TRUNAMTS DS    0X                  UNION CHECK AMOUNTS                          
TRUNPEN  DS    F                                                                
TRUNHLTH DS    F                                                                
TRUNPNH  DS    F                                                                
TRUNMDED DS    F                                                                
TRUNALNQ EQU   *-TRUNAMTS                                                       
*                                                                               
TRECMAX  EQU   20                                                               
TRECTAB  DS    CL(TRECMAX*9)                                                    
*                                                                               
TRCAT    DS    CL3                 SAVED CATEGORY CODE                          
...
ERR10    EQU   10                  NO ECAST RECORD FOUND                        
ERR11    EQU   11                  NO EPISODES IN RANGE ON CHECK                
*                                                                               
SORTCHK  DS    0F                  * CHECK RECORD FIELDS                        
SORTACCS DS    CL(ACCLNQ)          ACCUMS                                       
SORTEACC DS    CL(MAXEPIS*ACCELNQ) ACCUMS BY EPISODE                            
...
ACCPNH   DS    F                   P&H                                          
ACCMDED  DS    F                   MISC. DED FOR DIRECTORS ONLY                 
ACCLNQ   EQU   *-ACCD                                                           
NACCS    EQU   ACCLNQ/4                                                         
         SPACE 2                                                                
*              DSECT TO COVER ACCUMULATORS AT EPISODE LEVEL                     
         SPACE 1                                                                
ACCED    DSECT                                                                  
ACCEAPPL DS    F                   APPLIED AMOUNT                               
ACCEPAY  DS    F                   PAYMENT AMOUNT                               
ACCEPNH  DS    H                   P&H                                          
ACCEEPI  DS    H                   EPISODE NUMBER                               
ACCELNQ  EQU   *-ACCED                                                          
         SPACE 3                                                                
...
LINPNH   DS    CL11                P&H                                          
BC14     DS    0CL1                                                             
LINMDED  DS    CL11                MISC. DEDUCTION                              
BR       DS    CL1                                                              
         EJECT                                                                  
       ++INCLUDE TAREPFFD                                                       
         EJECT                                                                  
...
* TAREPWORKD                                                                    
       ...
       ++INCLUDE TAREPWORKD                                                     
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'243TAREP35O  07/30/03'                                      
         END                                                                    
