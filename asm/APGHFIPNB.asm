*          DATA SET APGHFIPNB  AT LEVEL 006 AS OF 05/01/02                      
*PHASE ACHFIPNB,+0                                                              
         TITLE 'APG HOOK FOR PORTER NOVELLI BRANCH REPORT'                      
ACHFIPNB CSECT                                                                  
         PRINT NOGEN                                                            
...
BUD      CLI   MODE,PROCBUD                                                     
         ...
         L     R4,APGIO                                                         
         LA    R4,56(R4)           ADD LENGHT ON NEW KEY                        
         MVI   ELCODE,X'1D'                                                     
         BAS   RE,FIRSTEL                                                       
         BNE   XIT                                                              
...
GETREC   DS    0H                                                               
         ...
         CLC   LSTINC(1),CMPKEY    IS SAVED INCOME SAME OFFICE                  
         BNE   GETNO                                                            
         CLC   LSTREV(1),CMPKEY    IS SAVED REVENUE SAME OFFICE                 
         BNE   GETNO                                                            
                                                                                
         LA    R1,LSTINCC          R1=CURRENT INCOME                            
         LA    R2,LSTREVC          R2=CURRENT REVENUE                           
         LA    R4,LSTICOC          R4=CURRENT INTERCO MANPWR                    
         CLI   CMPKEY+4,CURR                                                    
         BE    GETR07                                                           
         LA    R1,LSTINCB          R1=BUDGET INCOME                             
         LA    R2,LSTREVB          R2=BUDGET REVENUE                            
         LA    R4,LSTICOB          R4=BUDGET INTERCO MANPWR                     
         CLI   CMPKEY+4,BUDG                                                    
         BE    GETR07                                                           
         LA    R1,LSTINCP          R1=PRIOR INCOME                              
         LA    R2,LSTREVP          R2=PRIOR REVENUE                             
         LA    R4,LSTICOP          R4=PRIOR INTERCO MANPWR                      
                                                                                
GETR07   LA    R0,14               COMPUTE RATIO FOR 14 COLUMNS                 
         LA    R3,SRAMT            R3=OPERATING RATIO RECORD                    
GETR09   ZAP   0(8,R3),=P'0'       INITIALIZE RATIO                             
         ZAP   WORK(16),0(8,R1)    INCOME                                       
         MP    WORK(16),=P'10000'                                               
         ZAP   WORK+16(8),0(8,R2)  REVENUE                                      
         CLC   LSTICO(1),CMPKEY    IS SAVED INTERCO SAME OFFICE                 
         BNE   *+10                NO SO DON'T SUPTRACT INTERCOMPANY            
         SP    WORK+16(8),0(8,R4)  SUBTRACT INTERCOMPANY MANPOWER               
         CP    WORK+16(8),=P'0'                                                 
         BE    GETR11              DON'T DIVID BY ZERO                          
         DP    WORK(16),WORK+16(8) DIVIDED BY (REVENUE-INTERCO MANPWR)          
         CP    WORK(8),=P'10000'   SKIP RATIOS OVER 100.00%                     
         BH    GETR11              NO NEGATIVES                                 
         CP    WORK(8),=P'-10000'  SKIP RATIOS UNDER -100.00%                   
         BL    GETR11              NO NEGATIVES                                 
...
GETR32   CLC   CMPKEY+1(3),=C'2C4' IS IT INTERCOMP MANPOWER                     
         BNE   GETR33                                                           
         MVC   LSTICO,CMPKEY       SAVE KEY                                     
         LA    R1,LSTICOC          R1=INTERCOMP MANPOWER                        
         B     GETR37                                                           
                                                                                
GETR33   CLC   CMPKEY+1(3),=C'41X' IS IT OPERATING INCOME                       
         BNE   GETR35                                                           
         MVC   LSTINC,CMPKEY       SAVE INCOME KEY                              
         LA    R1,LSTINCC          R1=CURRENT INCOME                            
         B     GETR37                                                           
...
LSTINC   DS    CL5                 LAST OPERATING INCOME LINE KEY               
LSTREV   DS    CL5                 LAST COMMISION AND FEE LINE KEY              
LSTCUR   DS    CL5                 LAST CURRENT(ANY LINE) KEY                   
LSTICO   DS    CL5                 LAST INTERCOMPANY MANPOWER                   
                                                                                
LSTINCC  DS    14PL8               LAST OPER. INC. LINE (CURRENT)               
LSTLNQ   EQU   *-LSTINCC                                                        
LSTINCB  DS    14PL8               LAST OPER. INC. LINE (BUDGET)                
LSTINCP  DS    14PL8               LAST OPER. INC. LINE (PRIOR)                 
...
LSTICOC  DS    14PL8               LAST INTERCOMP MANPWR LINE (CURRENT)         
LSTICOB  DS    14PL8               LAST INTERCOMP MANPWR LINE (BUDGET)          
LSTICOP  DS    14PL8               LAST INTERCOMP MANPWR LINE (PRIOR)           
...
OFBNM    DC    AL1(OFSRV),CL36'DPN CORP.'                                       
...
GRPPN    DC    CL36'PORTER NOVELLI'                                             
         DC    C'B,H,L,P,S,T,W,Y',X'00'                                         
                                                                                
GRPDP    DC    CL36'PN-NEW YORK COMBINED'                                       
         DC    C'S,W',X'00'                                                     
         EJECT                                                                  
*------------------------------------------------------------------*            
...
*        ACGENBOTH                                                              
       ...
       ++INCLUDE ACGENBOTH                                                      
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'006APGHFIPNB 05/01/02'                                      
         END                                                                    
