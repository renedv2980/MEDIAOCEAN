*          DATA SET RELFM01    AT LEVEL 021 AS OF 01/14/05                      
*PHASE T80401A,*                                                                
...
PRFTBL   EQU   *                                                                
         ...
         DC    AL1(RREPQEZP),CL8'EZPOST'                                        
         DC    AL1(RREPQSOM),CL8'SOM'                                           
...
*                                                                               
         ...
         ST    RF,FULL             SAVE RF FOR EOR TEST                         
*                                                                               
*   DON'T STEP ON 'FULL', PLEASE, UNTIL END OF RECORD TEST IS DONE.             
*                                                                               
...
***      MVC   DIETEST(2),X'0000'  PLACE FOR A DUMP                             
...
PTPF100  DS    0H                                                               
         LA    R2,FPPIUSEH                                                      
         MVC   8(30,R2),SPACES     CLEAR FIELD                                  
         FOUT  (R2)                                                             
         LA    R2,FPPIUBYH                                                      
         OI    1(R2),X'0C'         TURN ON 'LOW INTENSITY'                      
         MVC   8(16,R2),SPACES     CLEAR FIELD                                  
         FOUT  (R2)                                                             
         LA    R2,FPPTOF1H         SET A(TAKEOVER OFFICE ADDRESS)               
         LA    R0,4                                                             
PTPF120  EQU   *                                                                
         MVC   8(2,R2),SPACES                                                   
         FOUT  (R2)                                                             
         ...
         AR    R2,RF                                                            
         BCT   R0,PTPF120          GO BACK FOR ALL FOUR FIELDS                  
*                                                                               
         LA    R2,FPPTOF1H         SET A(TAKEOVER OFFICE ADDRESS)               
*                                                                               
         GOTO1 VGETEL,DMCB,(X'22',RPTPREC),DMCB+8                               
         CLI   DMCB,X'FF'                                                       
         BE    PTPF160             NO ELEMENTS                                  
*                                                                               
         USING RPTPIBEM,R6         INBOX ADDRESS                                
         L     R6,DMCB+8           A(1ST ELEMENT)                               
*                                                                               
PTPF140  DS    0H                                                               
         MVC   8(2,R2),RPTPIBOF    MOVE INBOX OFFICE TO SCREEN                  
*                                                                               
         DROP  R6                                                               
*                                                                               
         ZIC   RF,1(R6)            BUMP TO NEXT X'22' ELEMENT                   
         AR    R6,RF                                                            
         CLI   0(R6),X'22'         ANOTHER TAKEOVER OFFICE ADDR?                
         BNE   PTPF160             NO  - FINISHED                               
         ZIC   RF,0(R2)            YES - BUMP TO NEXT SCREEN POSITION           
         AR    R2,RF                                                            
         B     PTPF140             GO BACK AND LOAD NEXT SCRN FIELD             
*                                                                               
PTPF160  DS    0H                                                               
         B     EXXMOD              EXIT                                         
*                                                                               
...
PTPE0010 EQU   *                                                                
         ...
         XC    OLDOFFS,OLDOFFS     CLEAR TAKENET OFFICES                        
         XC    FPPIUSE,FPPIUSE     CLEAR 'IN USE BY' FIELDS'                    
         FOUT  FPPIUSEH                                                         
*                                                                               
         CLI   BACT,C'A'           ADD?                                         
         BE    PTPE0020            YES                                          
...
*                                                                               
         GOTO1 =A(NETOFFS),DMCB,REC2,RR=Y                                       
*                                                                               
*- BUILD FIXED PART OF RECORD                                                   
PTPE0020 EQU   *                                                                
...
PTPE0100 EQU   *                                                                
*                                                                               
         GOTO1 =A(CHKNTOFF),RR=Y                                                
         BZ    PTPE0120            TAKENET OFFICES OKAY                         
         MVC   LFMMSG(L'NETOFFER),NETOFFER                                      
*                                  TAKENET OFFICE ERROR                         
         LA    R2,FPPIUBYH                                                      
         NI    1(R2),X'FF'-X'0C'   TURN FIELD TO NORMAL INTENSITY               
         MVC   8(16,R2),=C'In Use By/Error>'                                    
         FOUT  (R2)                OUTPUT FIELD                                 
         LA    R2,FPPTOF1H         SET CURSOR POSITION                          
         B     MYERR                                                            
*                                                                               
*                                                                               
*- BUILD TAKENET OFFICE ELEMENTS, IF ANY                                        
*                                                                               
PTPE0120 EQU   *                                                                
         LA    R2,NEWOFFS          SET A(NEW OFFICES)                           
         LA    R0,4                SET LOOP CONTROL                             
PTPE0140 EQU   *                                                                
         CLC   0(2,R2),SPACES      ANYTHING IN FIELD?                           
         BNH   PTPE0160            NO  - FINISHED                               
         XC    WORK2(20),WORK2                                                  
         MVI   WORK2,X'22'         SET ELEMENT CODE                             
         MVI   WORK2+1,8           SET ELEMENT LENGTH                           
         MVC   WORK2+2(2),0(R2)    INSERT OFFICE CODE                           
         GOTO1 VADDELEM,DMCB,RPTPREC,WORK2     ADD ACTIVITY ELEMENT             
         LA    R2,2(R2)            BUMP TO NEXT NEW OFFICE                      
         B     PTPE0140            GO BACK FOR NEXT                             
PTPE0160 EQU   *                                                                
*                                                                               
*                                                                               
*- BUILD ACTIVITY DATE ELEMENT                                                  
...
*                                                                               
FLFLEXT2 EQU   *                                                                
         CLI   BREC,X'31'          CONTRACT TYPE                                
         BNE   FLFLEXT3                                                         
         BAS   RE,PTPPASS          SET UP PASSIVE KEYS                          
*                                                                               
FLFLEXT3 EQU   *                                                                
         B     EXXMOD                                                           
         EJECT                                                                  
***************************************************************                 
* PTPPASS:  SET UP PASSIVE KEYS FOR THIS POINT PERSON RECORD  *                 
*           DELETE ALL PASSIVES FOR OFFICES IN OLDOFFS        *                 
*           ADD    ALL PASSIVES FOR OFFICES IN NEWOFFS        *                 
***************************************************************                 
PTPPASS  NTR1                                                                   
         LA    R1,OLDOFFS          A(OLD OFFICE LIST)                           
         LA    R0,4                                                             
PTPP0020 EQU   *                                                                
         OC    0(2,R1),0(R1)       ANY OFFICE IN SLOT?                          
         BZ    PTPP0060            NO  - FINISHED WITH OLD OFFICES              
         XC    KEY,KEY                                                          
         MVI   KEY,X'84'           SET KEY TYPE                                 
         MVC   KEY+20(2),REPALPHA  INSERT REP CODE                              
         MVC   KEY+22(2),0(R1)     INSERT OFFICE CODE                           
         MVC   KEY+24(3),THISPPN   INSERT POINT PERSON OF RECORD                
         GOTO1 HIGH                READ FOR PASSIVE                             
         CLC   KEY(27),KEYSAVE     KEY FOUND?                                   
         BNE   PTPP0040            NO  - THAT'S ACCEPTABLE                      
         OI    KEY+27,X'80'        YES - SET DELETE FLAG                        
         GOTO1 WRITE               REWRITE KEY AS DELETED                       
PTPP0040 EQU   *                                                                
         LA    R1,2(R1)            BUMP TO NEXT OLD OFFICE                      
         BCT   R0,PTPP0020         GO BACK FOR NEXT OFFICE                      
PTPP0060 EQU   *                                                                
***>>>>                                                                         
         LA    R1,NEWOFFS          A(NEW OFFICE LIST)                           
         LA    R0,4                                                             
PTPP0120 EQU   *                                                                
         OC    0(2,R1),0(R1)       ANY OFFICE IN SLOT?                          
         BZ    PTPP0160            NO  - FINISHED WITH NEW OFFICES              
         XC    KEY,KEY                                                          
         MVI   KEY,X'84'           SET KEY TYPE                                 
         MVC   KEY+20(2),REPALPHA  INSERT REP CODE                              
         MVC   KEY+22(2),0(R1)     INSERT OFFICE CODE                           
         ...
         GOTO1 HIGH                READ FOR PASSIVE                             
         CLC   KEY(27),KEYSAVE     KEY FOUND?                                   
         BNE   PTPP0140            NO  - MUST BE ADDED                          
         TM    KEY+27,X'80'        YES - IS KEY DELETED?                        
         BO    *+6                 YES - PROCEED                                
         DC    H'0'                UNDELETED?  SHOULDN'T BE!                    
         NI    KEY+27,X'FF'-X'80'  YES - SET DELETE FLAG                        
         MVC   KEY+28(4),BSVDA     INSERT DISK ADDRESS                          
         GOTO1 WRITE               REWRITE KEY AS ACTIVE                        
         B     PTPP0160                                                         
PTPP0140 EQU   *                                                                
         MVC   KEY(27),KEYSAVE     RESET KEY TO BE ADDED                        
         MVC   KEY+28(4),BSVDA     INSERT DISK ADDRESS                          
         GOTO1 ADD                 ADD KEY AS ACTIVE                            
PTPP0160 EQU   *                                                                
         LA    R1,2(R1)            BUMP TO NEXT OLD OFFICE                      
         BCT   R0,PTPP0120         GO BACK FOR NEXT OFFICE                      
PTPP0180 EQU   *                                                                
         XIT1                                                                   
         EJECT                                                                  
***>>>>                                                                         
...
*                                                                               
NETOFFER DC    C'TAKENET OFFICE DECLARATION REJECTED.'                          
*                                                                               
...
SPSAL    DSECT                                                                  
       ++INCLUDE GEGENSPSAL                                                     
         EJECT                                                                  
T80401   CSECT                                                                  
CHKNTOFF NTR1  LABEL=*,BASE=*                                                   
         MVC   THISPPN,RPTPKREC    SAVE P/P OF RECORD                           
         LA    R2,FPPTOF1H         SET A(FIRST TAKENET OFFICE CODE)             
         LA    R3,FPPIUSE          SET A(POSSIBLE ERROR INDICATOR)              
         LA    R4,NEWOFFS          SET A(NEW OFFICE LIST)                       
         LA    R0,4                LOOP FOUR FIELDS                             
         MVI   ERRORBYT,0          CLEAR A FLAG                                 
CNTO0020 EQU   *                                                                
DIE0020  EQU   *                                                                
...
CNTO0030 EQU   *                                                                
         ...
         AR    R2,RF                                                            
         LA    R3,5(R3)            BUMP A(POSSIBLE ERROR INDICATOR)             
         BCT   R0,CNTO0020         GO BACK FOR NEXT FIELD                       
         B     CNTO0100            FINISHED - CHECK FOR DUPES                   
CNTO0040 EQU   *                                                                
*- USE SUBSIDIARY REP IF GIVEN, ELSE USE SIGN-ON REP                            
         XC    KEY,KEY                                                          
         MVI   KEY,X'04'           REC ID                                       
         ...
         BZ    CNTO0060                                                         
         MVC   KEY+23(2),RPTPREP   USE SUB REP                                  
CNTO0060 MVC   KEY+25(2),8(R2)     INSERT OFFICE INTO KEY                       
         MVC   0(2,R4),8(R2)       SAVE NEW OFFICE                              
         LA    R4,2(R4)            BUMP NEW OFFICE LIST                         
         GOTO1 HIGH                                                             
         CLC   KEY(27),KEYSAVE     OFFICE ON FILE?                              
         BE    CNTO0080            YES - CONTINUE CHECK                         
         MVC   0(2,R3),=C'**'      NO  - SET ERROR INDICATOR                    
         MVI   ERRORBYT,1          TURN ON ERROR INDICATOR                      
         B     CNTO0030            CHECK NEXT OFFICE                            
CNTO0080 EQU   *                                                                
*                                                                               
*   NOW CHECK POINT PERSON TAKENET PASSIVE KEY X'84' (MASTER LEVEL)             
*                                                                               
         XC    KEY,KEY                                                          
         MVI   KEY,X'84'           REC ID                                       
         MVC   KEY+20(2),REPALPHA                                               
         MVC   KEY+22(2),8(R2)     INSERT OFFICE INTO KEY                       
         GOTO1 HIGH                                                             
         CLC   KEY(24),KEYSAVE     TAKENET PASSIVE ON FILE?                     
         BNE   CNTO0030            NO  - CAN BE USED - GO BACK                  
*                                     FOR NEXT TAKENET OFFICE                   
         CLC   KEY+24(3),RPTPKREC  PASSIVE P/P IN KEY FOUND                     
*                                     SAME AS P/P ON SCREEN?                    
         BE    CNTO0030            YES - ACCEPTED -                             
         MVC   0(3,R3),KEY+24      NO  - INDICATE P/P USING OFFICE              
         MVI   ERRORBYT,1          TURN ON ERROR INDICATOR                      
***      MVC   DIE0020(2),=X'0000'                                              
         B     CNTO0030            CHECK NEXT OFFICE                            
CNTO0100 EQU   *                                                                
*                                                                               
*   ROUTINE WILL CHECK EACH OFFICE IN TABLE WITH ALL OFFICES AFTER              
*        IT IN THE TABLE.  THEREBY, IF THERE ARE THREE OFFICES, THE             
*        FIRST WILL BE CHECKED AGAINST THE SECOND AND THIRD, THE                
*        SECOND WILL BE CHECKED AGAINST THE THIRD, AND THE THIRD NEED           
*        NOT BE CHECKED AT ALL.                                                 
*                                                                               
         LA    R4,NEWOFFS          SET A(NEW OFFICE LIST)                       
         LA    R3,FPPIUSE          SET A(POSSIBLE ERROR INDICATOR)              
         LA    R0,3                SET NEW OFFICE LOOP                          
         LA    R1,3                SAVE TO SET NEXT LOOP                        
*                                     DON'T NEED TO CHECK LAST OFFC             
CNTO0120 EQU   *                                                                
         CLC   0(2,R4),SPACES      ANY ENTRY IN TABLE?                          
         BNH   CNTO0180            NO  - FINISHED                               
         LR    R2,R4                                                            
         LA    R2,2(R2)            BUMP TO NEXT OFFICE SLOT                     
CNTO0140 EQU   *                                                                
         CLC   0(2,R4),0(R2)                                                    
         BNE   CNTO0160            NOT SAME                                     
         MVC   0(2,R3),=C'EQ'      SAME: SET INDICATOR                          
         MVI   ERRORBYT,1          TURN ON ERROR INDICATOR                      
CNTO0160 EQU   *                                                                
         LA    R2,2(R2)            BUMP TO NEXT SLOT                            
         BCT   R0,CNTO0140         GO BACK FOR NEXT SLOT                        
         CH    R1,=H'2'            PENULTIMATE ITEM DONE?                       
         BE    CNTO0180            YES - FINISHED                               
*                                                                               
         LR    R0,R1               RESET LOOP                                   
         BCTR  R0,0                DECREMENT COUNT                              
         BCTR  R1,0                DECREMENT CONTROL                            
         LA    R4,2(R4)            BUMP TO NEXT OFFICE                          
         LA    R3,5(R3)            BUMP TO NEXT POSSIBLE ERROR IND              
         B     CNTO0120            GO BACK FOR NEXT                             
CNTO0180 EQU   *                                                                
                                                                                
CNTO0900 EQU   *                                                                
         ZIC   RF,ERRORBYT         SET ERROR ON EXIT                            
         LTR   RF,RF               SET CONDITION CODE                           
         XIT1                                                                   
         LTORG                                                                  
         EJECT                                                                  
                                                                                
NETOFFS  NTR1  LABEL=*,BASE=*                                                   
         L     R4,0(R1)            SET A(POINT PERSON RECORD)                   
         USING RPTPREC,R4                                                       
         LA    R1,OLDOFFS          SET A(OLD OFFICES)                           
         LA    RF,RPTPELEM         SET A(1ST ELEMENT)                           
NOFF0020 EQU   *                                                                
         CLI   0(RF),0             END OF RECORD?                               
         BE    NOFF0800            YES - FINISHED                               
         CLI   0(RF),X'22'         INBOX BY OFFICE ELEMENT?                     
         BNE   NOFF0040            NO                                           
         MVC   0(2,R1),2(RF)       YES - MOVE OFFICE CODE TO TABLE              
         LA    R1,2(R1)            BUMP TO NEXT OFFICE CODE SLOT                
NOFF0040 EQU   *                                                                
         ZIC   RE,1(RF)            BUMP TO NEXT RECORD ELEMENT                  
         AR    RF,RE                                                            
         B     NOFF0020                                                         
NOFF0800 EQU   *                                                                
         XIT1                                                                   
...
ASPS0100 EQU   *                                                                
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'021RELFM01   01/14/05'                                      
         END                                                                    
