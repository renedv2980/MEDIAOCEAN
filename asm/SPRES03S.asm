*          DATA SET SPRES03S   AT LEVEL 114 AS OF 05/01/02                      
*PHASE T20F03A,+0                                                               
...
*                                                                               
         ...
         GOTO1 VSUBR08,DMCB,('EDITOPTE',(RC))                                   
         BNE   SPERR                                                            
*                                                                               
         MVI   NMARKET,0                                                        
...
*---->   BAS   RE,EDITDPT                                                       
         GOTO1 VSUBR08,DMCB,('SETPHASE',(RC))                                   
         GOTO1 VSUBR08,DMCB,('EDITDPTE',(RC))                                   
         MVI   NFLDS,1                                                          
         MVC   PRPNDEMS,NDEMOS                                                  
         MVC   TMPSEPBF,NDEMOS                                                  
         MVI   DEMSTYLE,C'D'                                                    
...
STAT4    CLI   8(R2),C'+'                                                       
         ...
         SPACE 1                                                                
         MVI   STASTYLE,C'C'                                                    
         GOTO1 VSUBR08,DMCB,('VALDSTAE',(RC))                                   
         MVC   0(5,R4),ACTSTAT                                                  
         GOTO1 VSUBR07,DMCB,('EDITSBE',(RC))                                    
         BNE   BADBOOKS                                                         
         STC   R3,NSTATS                                                        
         BAS   RE,BUMP                                                          
         CLI   5(R2),0                                                          
...
FILLCMB  NTR1  WORK=(R7,50)                                                     
         GOTO1 VSUBR08,DMCB,('SETUNIVE',(RC))                                   
*                                                                               
         LA    R1,4                                                             
         LA    RE,DEMOS                                                         
         LA    RF,COPDEMOS                                                      
...
PC0A     STC   RF,MAXLINES                                                      
         CLI   CALCOPT,C'Y'                                                     
         BNE   PC1                                                              
         GOTO1 VSUBR08,DMCB,('SETNEWUE',(RC))                                   
...
COMPR6   CLC   LASTVAL,BUFFDEMS                                                 
         ...
         STC   R4,0(R2)                                                         
         CLC   BUFFDEMS(2),BUFFDEMS+L'BUFFREC                                   
         BNE   COMPR10                                                          
...
VIO1  DC      C'#=VIOLATION: SPECIAL STATION ACTIVITIES. DETAILS: P.5B X        
               ARB MKT. REPORT'                                                 
VIO2     DC    C'A = STATION CITED  Z = STATION LISTED BELOW THE LINE'          
*NDHDR   DC    C'**CONDENSED MARKET - EXERCISE DISCRETION WHEN USING THX        
*              ESE ESTIMATES**'                                                 
...
SHUFFLE  NTR1                                                                   
         ...
         BZ    XIT                      JUST GET OUT                            
         OC    1(2,R4),1(R4)       IF NO WORST JUST ACCEPT IT                   
         BZ    *+14                                                             
         CLC   1(2,R2),1(R4)       COMPARE AGAINST THE WORST                    
         BNH   XIT                 IF BETTER,                                   
...
SHUFFLE4 MVC   0(L'BUFFREC,RE),0(R2)       MOVE IN A NEW LINE                   
         ZIC   R5,MAXRANK          AND RERANK THIS DAYPART                      
SHUFFLE5 GOTO1 XSORT,DMCB,(1,(R3)),(R5),L'BUFFREC,2,1   (DESCENDING)            
         B     XIT                                                              
         DROP  R2                                                               
         DROP  R3                                                               
SHUF1ST  DC    C'Y'                                                             
...
*                                  FORMAT UNIVERSES FOR HEADLINES               
         ...
         CLI   DBSELSRC,C'M'                                                    
         BNE   DEMHOOK0                                                         
         GOTO1 VSUBR08,DMCB,('SETUNIBE',(RC))                                   
...
*        MVC   THISNOA(2),MARDATE+2                                             
         CLI   MARDATE+3,C' '                                                   
         BNH   DMHKT                                                            
         MVC   20(2,R2),MARDATE+2                                               
DMHKT    GOTO1 DEMOUT,DMCB,(C'L',DEMOS),DBLOCK,THISLINE+4                       
         SPACE 1                                                                
...
FORM18   LA    R3,BUFFDEMS         AND SET UP FOR THE DEMOS                     
         ...
         SPACE 1                                                                
FORM20   SR    R1,R1                                                            
         ICM   R1,3,0(R3)          USE AS AN UNSIGNED NUMBER                    
         CLI   WORSTOPT,C'Y'                                                    
         BNE   *+6                                                              
         LCR   R1,R1                                                            
         N     R1,=F'65535'                                                     
...
DIVTOTS2 CLI   WORSTOPT,C'Y'                                                    
         BNE   *+6                                                              
         LCR   RF,RF                                                            
         STCM  RF,3,0(R3)          SAVE SHARE RESULT                            
...
COMPC8   NTR1                      ADJUST COLUMN FOR THIS DEMO                  
         ...
         SR    R1,R1                                                            
         SPACE 1                                                                
COMPC10  AH    R1,0(R3)            PICK UP SHARE FOR THIS STATION               
         STH   R1,0(R3)            ADD TO PREVIOUS AND RESTORE                  
...
SETDPTS  NMOD1 0,**SETDPTS**                                                    
         ...
         L     RC,4(R1)                                                         
*                                                                               
         GOTO1 VSUBR08,DMCB,('EDITDPTE',(RC))     EDIT DAYPARTS AGAIN           
...
EDDT11   CLI   12(R4),C'('         IS THIS A COMBO DAYPART?                     
         ...
         BE    NEQXIT2             ERROR, NO NESTED COMBOS                      
         MVI   CMBDPT,C'Y'                                                      
...
EDDT24   MVC   0(1,R8),BYTE        MOVE DAY TO LIST                             
         ...
         CLI   0(R4),0                                                          
         BNE   EDDT26                                                           
         B     NEQXIT2             CAN'T HAVE JUST DAYS, ERROR                  
         SPACE 1                                                                
...
BADDPT   MVC   CONHEAD(L'INVDAYPT),INVDAYPT                                     
         ...
         DROP  R4                                                               
         SPACE 1                                                                
DPTDYT   GOTO1 VSUBR07,DMCB,('EDDYTME',(RC))                                    
         BNE   BADDPT                                                           
         B     EQXIT3                                                           
         SPACE 2                                                                
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'114SPRES03S  05/01/02'                                      
         END                                                                    
