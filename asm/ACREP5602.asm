*          DATA SET ACREP5602  AT LEVEL 067 AS OF 06/12/18                      
*PHASE AC5602A,*                                                                
...
RNF10    XC    0(L'COMLIN1,R2),0(R2)   REPORT                                   
         LA    R2,L'COMLIN1(R2)                                                 
         BCT   R3,RNF10                                                         
*                                                                               
...
RNF20    GOTOR HEXOUT,DMCB,0(R3),MCDUB+4,1                                      
...
*                                                                               
         ...
         XC    0(L'CHAKEY,R2),0(R2)                                             
         MVI   CHAKTYP,CHAKTYPQ                                                 
         MVC   CHAKCULA,0(R4)                                                   
         MVC   KEYSAVE,0(R2)                                                    
         DROP  R2                                                               
         GOTO1 DATAMGR,DMCB,=C'DMRDHI',=C'ACCOUNT',(R2),(R2)                    
         CLC   KEYSAVE(CHAKLDG-CHAKEY+1),0(R2)                                  
         BE    *+6                                                              
         DC    H'0'                                                             
*                                                                               
INT14    L     R4,AIO2                                                          
...
INT16    CLI   0(R4),0             EOF?                                         
         BNE   INT16B                                                           
         L     R2,AIO2                                                          
         GOTO1 DATAMGR,DMCB,=C'DMRSEQ',=C'ACCOUNT',(R2),(R2)                    
         CLC   KEYSAVE(CHAKLDG-CHAKEY+1),0(R2)                                  
         BE    INT14                                                            
         B     ERR9                                                             
*                                                                               
INT16B   CLI   0(R4),OCNELQ        X'54' CHECK ELEMENT                          
         BNE   INT15                                                            
...
*                                                                               
         CLI   OCNLASR,OCNWSP      IS THIS WEB SERIES CHECKS                    
         BNE   *+8                                                              
         OI    LSRSW,LSRWSP        SET WEBSERIES INDICATOR                      
*                                                                               
*MN SPEC-9590                                                                   
         CLI   OCNLASR,OCNWSL      IS THIS WEB SERIES CHECKS                    
         BNE   *+8                                                              
         OI    LSRSW,LSRWSP        SET WEBSERIES INDICATOR                      
*MN SPEC-9590                                                                   
*                                                                               
INT16C   TM    OCNSTAT2,OCNSFTP    IF FILE TRANSFER                             
         BZ    *+8                                                              
         OI    LGSW,LGFTP                                                       
         TM    OCNSTAT2,OCNS820          820 FILE TRANSMISSION                  
         BZ    *+8                                                              
         OI    LGSW,LGFTP+LGUPD                                                 
*                                                                               
...
VALVOID3 LA    R6,VOID1            R6 = START VOID                              
         ...
         MH    R3,=Y(L'VTAB)                                                    
         AR    R4,R3               R2 = NEXT AVAILABLE ENTRY                    
         L     R0,MXVOIDX          MAXIMUM NUMBER OF VOIDS                      
         L     R3,NVOD             R3 = NUMBER OF VOIDS                         
*                                                                               
...
*                                                                               
         TM    LSRSW,LSRWSP        WEBSERIES CHECKS RUN SOON AND                
         BO    REG01               COULD HAVE DUPLICATE POSTING FILES           
*                                                                               
         CLI   RCPOSTNG,C'N'                                                    
         BE    REG01                                                            
         BAS   RE,CKDUPWK                                                       
...
REG04    BAS   RE,WRKREAD          READ THE CHECK FILE                          
         ...
         BNE   REG04A              CHECK FOR STARTING CHECK #                   
         TM    RNSW,SOON                                                        
         BZ    REG04                                                            
         TM    LGSW,LGMIC                                                       
         BZ    REG04                                                            
         ...
         CLC   CRDDATA,SPACES                                                   
         BE    REG04                                                            
         PACK  FIRSTC,CRDDATA(6)                                                
         PACK  FIRSTR,CRDDATA(6)                                                
         ZAP   CURRENT,FIRSTC                                                   
         B     REG04                                                            
         DROP  R2                                                               
*                                                                               
REG04A   TM    RNSW,FIRST          IS IT FIRST TIME                             
         BNO   REG07                                                            
         NI    RNSW,ALL-FIRST                                                   
...
REG20C   CLI   0(R4),0             EOF?                                         
         ...
         B     REG22                                                            
*                                                                               
REG21    CLI   0(R4),OCNELQ        X'54'-CHECK ELEMENT                          
         BNE   REG20B                                                           
...
*                                                                               
         ...
         GOTO1 POSTWRK,DMCB,(RA),(C'R',WKID)    UPDATE THE FILE                 
         B     XIT                 THE END                                      
         EJECT                                                                  
***********************************************************************         
...
RQT      NTR1  ,                                                                
         ...
         LA    R4,1+L'TXT9(R4)                                                  
         EDIT  (P6,REQTOT),(15,0(R4)),2,MINUS=YES                               
...
LSRWSP   EQU   X'40'               WEB SERIES CHECKS                            
*                                                                               
...
* DDCOREQUS                                                                     
       ...
       ++INCLUDE DDCOREQUS                                                      
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'067ACREP5602 06/12/18'                                      
         END                                                                    
