*          DATA SET REREP3702  AT LEVEL 165 AS OF 07/11/02                      
*PHASE RE3702A,*                                                                
         TITLE 'RE3702 - REREP3702 - INTEREP INTERFACE TAPE'                    
**********************************************************************          
...
*                                                                   *           
* SEP/89 (MRR) --- DETAIL CLEAN-UP                                  *           
*                  ADD OUTPUT OF DOLLARS ON TAPE                    *           
*                                                                   *           
* OCT26/89 (MRR) --- ADD QOPTION2 CONTROL TO REPORT ONLY            *           
*                                                                   *           
* MAR12/90 (MRR) --- ADD FIELDS TO OUTPUT RECORD                    *           
*                                                                   *           
* APR10/90 (MRR) --- FIX FROM MAR12 - CATEGORY WAS EXPANDED TO 5    *           
*                     BUT THE CONTRACT STILL ONLY HOLDS 2 AND TYPE  *           
*                     WAS EXPANDED TO 2 BUT IS STILL ONLY 1         *           
*                                                                   *           
* AUG01/90 (MRR) --- > ADD 'M' OR 'W' TO 1ST UNUSED POSITION TO     *           
*                      INDICATE MONTHLY OR WEEKLY TAPE              *           
*                    > STOP BUCKET SCAN IF > CONTRACT END DATE      *           
*                                                                   *           
* JUN19/91 (MRR) --- > CALL DEM0CON TO CONVERT DEMO NUMS TO TEXT    *           
*                                                                   *           
* NOV18/91 (BU ) --- MAKE APPROPRIATE CHANGES FOR VALUENEW FACILITY *           
*                                                                   *           
* DEC24/91 (BU ) --- FIX TABLE-SEARCH START IN WEEKLY BUCKETING     *           
*                                                                   *           
* JAN07/92 (MRR) --- FIX ASAT DATE TO PREVIOUS MONDAY DATE          *           
*                                                                   *           
* MAR12/92 (MRR) --- >FIX CHECK FOR ORDERED AND INVOICED FOR SAME   *           
*                      MONTH IN SAME WEEK WHEN > 1 INVOICE EL       *           
*                                                                   *           
* MAR27/92 (BU ) --- MAKE COMPATIBLE WITH VALU2NEW                  *           
*                    REREPRGEQU ---> REREPRGEQA                     *           
*                                                                   *           
* MAY05/92 (BU ) --- INSERT CHANGED COMM RATE INTO ABSOLUTE WHEN    *           
*                    ABS GROSS=0, ABS COMMD=0, AND ABS COMR=0 IN    *           
*                    GETAMT30 ROUTINE.  THIS WILL CORRECT NO COMM   *           
*                    RATE IN OUTPUT RECORD AFTER FIELDS SWAPPED.    *           
*                                                                   *           
* MAY06/92 (BU ) --- ADD PRINTOUT OF TAPE OUTPUT IF REQUESTOR =     *           
*                    'SPEC PRINT'                                   *           
*                                                                   *           
* MAY20/92 (BU ) --- REFIX CHECK FOR ORD/INV FOR SAME MONTH IN SAME *           
*                    WEEK WHEN > 1 INV ELEMENT                      *           
*                                                                   *           
* JUN12/92 (BU ) --- INSERT COMMISSION RATE IF ABSOLUTE COMM $ NOT  *           
*                    = $0, AND NO COMMISSION RATE IS SET.           *           
*                                                                   *           
* SEP17/92 (BU ) --- LARGE NEGATIVE NUMBERS O/P TO TAPE WRONG. FIX. *           
*                                                                   *           
* DEC11/95 (BG ) --- 2K CONTRACTS REGENALL REGENALL1                *           
*                                                                   *           
* DEC04/96 (BU ) --- CORRECT MONTHLY BEST$ PROCESSING               *           
*                                                                   *           
* MAY21/97 (BU ) --- UPGRADE FOR YR 2000                            *           
*                                                                   *           
* JAN25/98 (JRD) --- 4K CONTRACTS                                   *           
*                                                                   *           
* JUN24/98 (BU ) --- ABC/CON TYPE Z/ ZERO COMMISSION RATE           *           
*                                                                   *           
* AST06/99 (AST) --- FIX FOR DEMOCON                                *           
*                                                                   *           
* MAY17/99 (BU ) --- CUMULUS/CON TYPE Z/ ZERO COMMISSION RATE       *           
*                    USES ABCROUTINE                                *           
*                                                                   *           
* SEP27/99 (BU ) --- EXPAND TWO DATE FIELDS TO INCLUDE CENTURY      *           
*                                                                   *           
* JAN11/00 (BU ) --- FIX Y2K DATE BUG                               *           
*                                                                   *           
* NOV29/01 (BU ) --- DAILY PACING                                   *           
*                                                                   *           
*                                                                   *           
*                                                                   *           
*                    *** END TOMBSTONE ***                          *           
...
****                                                                            
* DEMOCON FIX                                                                   
         GOTO1 LOADER,DMCB,=CL8'T00AE0',0                                       
         MVC   VDEMOCON,4(R1)                                                   
...
*                                                                               
         GOTO1 DATCON,DMCB,(0,QASAT),(X'20',QASAT2)                             
         MVC   TTRANDT+2(6),QASAT2 TRANSACTION DATE                             
         MVC   TTRANDT(2),=C'19'   SET CENTURY TO 20TH                          
         CLC   QASAT2(2),=C'80'    1980 ARBITRARY CUTOFF                        
         BH    POST10              1980 - 1999 RECOGNIZED AS 20TH               
         MVC   TTRANDT(2),=C'20'   SET CENTURY TO 21TH                          
POST10   EQU   *                                                                
         MVC   TTYPE(2),SPACES                                                  
...
POST130  SR    R0,R0                                                            
         ICM   R0,1,1(R3)                                                       
         BZ    POST150            END OF LOOP: CHECK 04'S                       
         AR    R3,R0                                                            
         CLI   0(R3),0                                                          
         BE    POST150            END OF LOOP: CHECK 04'S                       
         CLI   0(R3),3             03 ELEMENT FOUND?                            
         BNE   POST130             NO  - GO BACK FOR NEXT                       
*                                                                               
         MVI   RECALC,0            YES -                                        
         MVI   TTRANST,C'E'        X'03' ARE ESTIMATES                          
         CLI   DAILYPAC,C'Y'       DAILY PACING?                                
         BNE   POST146             NO  - CHECK FOR EXACT DATE MATCH             
*                                  YES - CHECK FOR WEEK RANGE                   
         CLC   RCONBKWK-RCONBKEL(2,R3),COMPSTDT                                 
         BL    POST130                                                          
         CLC   RCONBKWK-RCONBKEL(2,R3),COMPDT                                   
         BH    POST130                                                          
         B     POST148                                                          
POST146  EQU   *                                                                
...
*                                                                               
*   TEST                                                                        
**       MVC   P+1(10),=C'CHKINV/148'                                           
**       GOTO1 REPORT                                                           
*   TEST                                                                        
*                                                                               
         BAS   RE,CHKINVEL         SEE IF A '04' EL EXISTS                      
         BZ    POST149             NO X'04' EXISTS                              
*                                                                               
*   TEST:                                                                       
*        MVC   P+1(18),=C'04 EXISTS: SKIP IT'                                   
*        GOTO1 REPORT                                                           
*   TEST:                                                                       
*                                                                               
         B     POST130             AN X'04' EXISTS: SKIP IT?                    
POST149  EQU   *                                                                
*                                                                               
*   TEST:                                                                       
*        MVC   P+1(18),=C'NO 04: OUTPUT 03$$'                                   
*        GOTO1 REPORT                                                           
*   TEST:                                                                       
*                                                                               
         MVC   GROSSDT(2),RCONBKYR-RCONBKEL(R3)                                 
         MVI   GROSSDT+2,1                                                      
         GOTO1 DATCON,DMCB,(3,GROSSDT),(0,GROSSDT2)                             
...
*                                                                               
         ...
         CLI   DAILYPAC,C'Y'       DAILY PACING?                                
         BNE   POST156             NO  - CHECK FOR EXACT DATE MATCH             
*                                  YES - CHECK FOR WEEK RANGE                   
*                                                                               
*   TEST                                                                        
*        MVC   P+1(05),=C'RANGE'                                                
*        MVC   P+8(2),RCONBKWK-RCONBKEL(R3)                                     
*        MVI   P+10,C'/'                                                        
*        MVC   P+11(2),COMPSTDT                                                 
*        MVI   P+13,C'/'                                                        
*        MVC   P+14(2),COMPDT                                                   
...
*        MVC   P+8(2),ENDDT                                                     
*        MVI   P+10,C'/'                                                        
*        MVC   P+11(2),RCONSTYR-RCONSTEL(R3)                                    
*        GOTO1 REPORT                                                           
*   TEST END                                                                    
*                                                                               
...
*                                                                               
         ...
         GOTO1 DATCON,DMCB,(0,BRDDTS+6),(X'20',TSERVDT+2)                       
***>>>   MVC   TSERVDT+2(6),BRDDTS+6                                            
*                                  END DATE = MONTH OF SERVICE                  
         MVC   TSERVDT(2),=C'19'   SET CENTURY TO 20TH                          
         CLC   TSERVDT+2(2),=C'80' 1980 ARBITRARY CUTOFF                        
         BH    POST260             1980 - 1999 RECOGNIZED AS 20TH               
         MVC   TSERVDT(2),=C'20'   SET CENTURY TO 21TH                          
POST260  EQU   *                                                                
*                                                                               
         ST    R5,ABSGROS                                                       
...
*                                                                               
         GOTO1 =A(ABCRZERO),DMCB,(RC),ABSCOMD,ABSCOMR                           
*                                                                               
...
*                                  END DATE = MONTH OF SERVICE                  
         MVC   TSERVDT(2),=C'19'   SET CENTURY TO 20TH                          
         CLC   TSERVDT+2(2),=C'80' 1980 ARBITRARY CUTOFF                        
         BH    GETAMT15            1980 - 1999 RECOGNIZED AS 20TH               
         MVC   TSERVDT(2),=C'20'   SET CENTURY TO 21TH                          
GETAMT15 EQU   *                                                                
*                                                                               
         L     R5,CHAGROS                                                       
...
*                                                                               
*                                                                               
         GOTO1 =A(ABCRZERO),DMCB,(RC),CHACOMD,CHACOMR                           
*                                                                               
...
GETRANST NTR1                                                                   
         ...
         GOTO1 DATCON,DMCB,(0,0(R2)),(2,COMPDT)                                 
*                                                                               
         LA    R1,RCONELEM                                                      
         SR    R0,R0                                                            
GETRAN10 ICM   R0,1,1(R1)                                                       
         ...
         BNE   GETRAN10                                                         
         CLC   COMPDT,RCONSTYR-RCONSTEL(R1)   YEAR&MONTH OF SERVICE             
         BNE   GETRAN10                                                         
*                                                                               
...
*                                                                               
*        R3  ->  CURRENT ELEMENT, EITHER X'03' OR X'04'                         
*                                                                               
CHKINVEL NTR1                                                                   
*                                                                               
         LA    R4,RCONELEM         SET A(01 ELEMENT): START SEARCH              
         ZIC   R5,0(R3)            GET CURR ELT CODE (X'03' OR X'04')           
         CLI   0(R3),3             X'03'?                                       
         BE    CIEL10              YES - MUST LOOK FOR X'04'                    
         BCTR  R5,0                NO  - X'04':  SET TO LOOK FOR X'03'          
         B     CIEL11                                                           
CIEL10   EQU   *                                                                
         LA    R5,1(R5)            X'03': SET TO LOOK FOR X'04' ELT             
CIEL11   EQU   *                                                                
         SR    R6,R6               USE R6 AS ORDERED EL COUNT                   
         SR    R7,R7               USE R7 AS ORDERED DOL ACCUM                  
*                                                                               
CIEL50   EQU   *                                                                
         ZICM  R0,1(R4),1          L(CURRENT ELEMENT)                           
         BZ    CIEL70              END OF RECORD REACHED IF ZERO                
         AR    R4,R0               BUMP TO NEXT ELEMENT                         
         CLI   0(R4),0             END OF RECORD?                               
         BE    CIEL70              YES                                          
         ZIC   R0,0(R4)            LOOK FOR OPPOSING ELT: X'03'/X'04'           
         CR    R0,R5               ELEMENT FOUND?                               
         BNE   CIEL50              NO  - GO BACK FOR NEXT ELEMENT               
*                                                                               
         CLC   2(2,R4),2(R3)       SAME MONTH OF SERVICE?                       
         BNE   CIEL50              NO  - GO BACK FOR NEXT                       
*                                                                               
         CLI   DAILYPAC,C'Y'       DAILY PACING?                                
         BNE   CIEL52              NO                                           
*                                                                               
...
*   TEST END                                                                    
*                                                                               
         CLC   4(2,R4),COMPSTDT    YES - RANGE: USE START AS CUTOFF             
         BNL   CIEL60                                                           
         B     CIEL54                                                           
CIEL52   EQU   *                                                                
...
CIEL54   EQU   *                                                                
         ...
         CLI   0(R4),X'03'         X'03' ELEMENT/ORDERED $$  ?                  
         BNE   CIEL50              NO  - DON'T ACCUMULATE                       
         A     R7,6(R4)            YES - ACCUM DOLLARS                          
         B     CIEL50                                                           
CIEL60   EQU   *                                                                
...
*   TEST                                                                        
*        MVC   P+1(05),=C'CIEL '                                                
*        MVC   P+8(2),RCONBKWK-RCONBKEL(R4)                                     
*        MVI   P+10,C'/'                                                        
*        MVC   P+11(2),COMPSTDT                                                 
*        MVI   P+13,C'/'                                                        
*        MVC   P+14(2),COMPDT                                                   
...
*   TEST END                                                                    
*                                                                               
         CLC   4(2,R4),COMPSTDT    YES - RANGE:  PRE-MONDAY?                    
         BL    CIEL70              YES                                          
***      BL    CIELGOOD            YES                                          
         CLC   4(2,R4),COMPDT      NO  - RANGE:  AFTER AS-AT DATE?              
         BH    CIEL70                                                           
***      BH    CIELGOOD                                                         
*                                                                               
*   TESTS MAKE NO SENSE.  A MATCH OF COMPDT (THIS WEEK'S AS AT)                 
*        TRANSFERS TO 'BAD'???                                                  
*                                                                               
         B     CIELBAD             NO  - PROCESS                                
***      B     CIEL70              NO  - PROCESS                                
CIEL66   EQU   *                                                                
         CLC   4(2,R4),COMPDT      COMPARE ACTIVITY DATE TO MONDAY DATE         
         BE    CIELBAD             SAME: CONSIDER FOUND                         
CIEL70   EQU   *                                                                
*                                                                               
*   TEST END                                                                    
*        MVC   P+01(06),=C'CIEL70'                                              
*        GOTO1 REPORT                                                           
*   TEST END                                                                    
*                                                                               
         LTR   R6,R6                                                            
...
TSERVDT  DS    CL08           +045 MONTH OF SVC DATE CCYYMMDD                   
TTRANDT  DS    CL08           +053 TRANSACTION DATE  CCYYMMDD                   
TCHGROSS DS    CL09           +061 CHANGED GROSS AMT                            
TCHCOMAT DS    CL09           +070 CHANGED COMMISSION AMT                       
TGROSS   DS    CL09           +079 ABSOLUTE GROSS                               
TCOMMAT  DS    CL09           +088 ABSOLUTE COMMISSION AMOUNT                   
TADVC    DS    CL04           +097 ADVERTISER CODE                              
TADVNM   DS    CL20           +101 ADVERTISER NAME                              
TAGYC    DS    CL04           +121 AGENCY CODE                                  
TAGYCTY  DS    CL02           +125 AGENCY CITY CODE                             
TAGYNM   DS    CL20           +127 AGENCY NAME                                  
TTYPE    DS    CL02           +147 TYPE                                         
TCON     DS    CL08           +149 CONTRACT NUMBER                              
TTRANST  DS    CL01           +157 TRANSACTION STATUS                           
TSALC    DS    CL03           +158 SALESPERSON CODE                             
TSALNM   DS    CL20           +161 SALESPERSON NAME                             
TTEAMC   DS    CL02           +181 TEAM CODE                                    
TTEAMNM  DS    CL10           +183 TEAM NAME                                    
TTEAMDN  DS    CL10           +193 TEAM DIVISION NAME                           
TCATCD   DS    CL05           +203 CATEGORY CODE                                
TPRODCD  DS    CL03           +208 PRODUCT CODE                                 
TPRODDS  DS    CL20           +211 PRODUCT DESCRIPTION                          
TDEMO    DS    CL20           +231 DEMOGRAPHICS                                 
TNETCON  DS    CL08           +251 NETWORK CONTRACT NUMBER                      
TPOINT   DS    CL03           +259 POINTPERSON CODE                             
TMARKET  DS    CL20           +262 STATION MARKET NAME                          
TSRANK   DS    CL01           +282 STATION RANK                                 
TOFFREG  DS    CL02           +283 OFFICE REGION                                
TOPTION1 DS    CL01           +285 REQUEST OPTION1, 'W' OR 'M'                  
         DS    CL14           +286 **** SPARE ****                              
*                             +300                                              
         EJECT                                                                  
*                                                                               
*   ABCRZERO:  IF ABCRADIO (INTEREP = IB), AND TAKEOVER ORDER                   
*              IF CUMULUS  (INTEREP = UO), AND TAKEOVER ORDER                   
*        (CONTRACT TYPE = Z), SET COMMISSION RATE/AMOUNT TO ZERO                
*                                                                               
ZEROCOMM CSECT                                                                  
ABCRZERO NMOD1 0,*ZERO*                                                         
         L     RC,0(R1)            RESET A(WORSPACE)                            
         L     R2,4(R1)            A(COMMISSION AMOUNT)                         
         L     R3,8(R1)            A(COMMISSION RATE)                           
         CLC   RCONKREP,=C'UO'     CUMULUS  (INTEREP)                           
         BE    AZER0040            NO  - OUT                                    
         CLC   RCONKREP,=C'IB'     ABCRADIO (INTEREP)                           
         BNE   AZER0080            NO  - OUT                                    
AZER0040 EQU   *                                                                
         CLI   RCONTYPE,C'Z'       YES - CONTRACT TYPE Z?                       
         BNE   AZER0080            NO  - OUT                                    
***      CLI   RCONSAL,C'T'        TAKEOVER S/P?                                
***      BNE   AZER0080            NO  - OUT                                    
         XC    0(4,R2),0(R2)       YES - ZERO OUT AMOUNT                        
         XC    0(4,R3),0(R3)       ZERO OUT RATE                                
AZER0080 EQU   *                                                                
         XIT1                                                                   
         LTORG                                                                  
         EJECT                                                                  
GETANMOD NMOD1 0,*GETA*                                                         
         L     RC,0(R1)            RESET A(WORSPACE)                            
...
GETAMT42 EQU   *                                                                
         GOTO1 EDITOUT2,DMCB,ABSGROS,TGROSS,9,0                                 
         GOTO1 EDITOUT2,DMCB,ABSCOMD,TCOMMAT,9,0                                
         GOTO1 EDITOUT2,DMCB,CHAGROS,TCHGROSS,9,0                               
         GOTO1 EDITOUT2,DMCB,CHACOMD,TCHCOMAT,9,0                               
         GOTO1 EDITOUT2,DMCB,CHACOMR,TCOMMRT,6,1                                
         XIT1                                                                   
         EJECT                                                                  
*                                                                               
*        LOCAL EDIT ROUTINES TO PRODUCED ZERO FILLED SIGN OVERPUNCHED           
...
EOUT230  EQU   *                                                                
         LR    R1,R4               COPY INPUT LEN FOR ZERO FILLING              
         LTR   R8,R8               CHECK OVERPUCH FLAG                          
         BNZ   EOUT250             NON-ZERO IS DON'T DO                         
         AR    R4,R3               POINT TO THE LAST CHARACTER                  
         BCTR  R4,0                                                             
         LTR   R5,R5                                                            
         BZ    EOUT250                                                          
...
*   END TEST 2B                                                                 
*                                                                               
         MVC   DAILYPAC,RREPPROF+27 SAVE DAILY PACING FLAG                      
*                                                                               
         OPEN  (INTFILE,OUTPUT)                                                 
*                                                                               
         MVC   TEMPDATE,QASAT                                                   
         MVC   SAVQASAT,QASAT      SAVE ORIGINAL DATE                           
...
*                                                                               
         ...
         MVC   COMPSTDT,COMPDT     SAVE START OF RANGE TEST                     
         CLI   DAILYPAC,C'Y'       DAILY PACING IN USE?                         
         BNE   INIT20              NO  - USE ADJUSTED DAY TO MONDAY             
         GOTO1 DATCON,DMCB,(0,SAVQASAT),(2,COMPDT)                              
*                                  YES - USE ORIGINAL DATE                      
*                                                                               
INIT20   EQU   *                                                                
         CLC   =C'SPEC PRINT',QUESTOR                                           
         BNE   INIT0025                                                         
         MVC   P+1(16),=C'DAILY=  DATE IS '                                     
         MVC   P+7(1),DAILYPAC                                                  
         GOTO1 DATCON,DMCB,(2,COMPSTDT),(5,P+18)                                
         GOTO1 DATCON,DMCB,(2,COMPDT),(5,P+28)                                  
         GOTO1 REPORT                                                           
INIT0025 EQU   *                                                                
         L     R6,AMONFORC                                                      
         MVC   0(2,R6),MONACT+2                                                 
...
*                                                                               
         SPACE                                                                  
DMCHK201 TM    DMCB+8,X'10'        TEST FOR RECORD NOT FOUND                    
         BZ    DM2010                                                           
         TM    29(R2),X'80'        IS RECORD MARKED FOR DELETION                
         BZ    DM2020              NO - ERROR                                   
         LTR   RB,RB               YES - RETURN WITH CC NE 0                    
         B     GBEXT2                                                           
*                                                                               
DM2010   TM    DMCB+8,X'EF'        TEST FOR OTHER ERRORS                        
         BZ    GBEXT2                                                           
*                                                                               
DM2020   MVC   WORK(41),=C'**********DATA MANAGER ERROR*************'           
         GOTO1 LOGIO,WORK+48,1,(41,WORK)                                        
         MVC   WORK(41),SPACES                                                  
         BASR  RE,RF                                                            
         BAS   RE,DMTRAC2                                                       
...
*                                                                               
DMTRAC24 DS   0H                                                                
         GOTO1 HEXOUT,DMCB,MTRA2DM8,P+130,1                                     
         MVI   SKIPSPEC,C'Y'                                                    
         GOTO1 REPORT                                                           
GBEXT2   EQU   *                                                                
         XIT1                                                                   
...
MTRA2DM8 DS    C                                                                
         ...
         LTORG                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'165REREP3702 07/11/02'                                      
         END                                                                    
