*          DATA SET SPNWS21    AT LEVEL 039 AS OF 02/27/07                      
*PHASE T20721C,*                                                                
...
READ16   CLI   RNKOPT,RNKSTA       TEST SORT IN STA/DAY/TIME SEQ                
         ...
         MVC   APBYTE,4(R1)        SAVE OFF THE 2D/OVERRIDE BITS                
READ16E  CLI   RNKOPT,RNKDEM       TEST RANK IN DEMO VALUE SEQ                  
...
REP26    CLC   1(2,R1),DEMOS+1                                                  
         BNE   REP26E                                                           
***  2 DECIMAL                                                                  
         LR    R4,RE               SAVE OFF RE AND RF                           
         LR    R0,RF                                                            
         ST    R1,APPARM                                                        
         GOTO1 AADJPREC,APPARM                                                  
         L     R1,APPARM                                                        
         LR    RE,R4               RESTORE RE AND RF                            
         LR    RF,R0                                                            
***  2 DECIMAL                                                                  
*                                                                               
         MVC   APDUB(4),4(R1)      APDUB = 1ST/2ND RATINGS                      
REP26E   CLC   1(2,R1),DEMOS+4                                                  
         BNE   *+10                                                             
         MVC   APDUB+4(4),4(R1)                                                 
         BXLE  R1,RE,REP26                                                      
*                                                                               
...
REP27    MVI   EBFLOAT,0                                                        
         ...
         MVC   APFULL,0(R4)                                                     
         NI    APFULL,255-DMODEMOV                                              
         MVC   APBYTE,0(R4)        SAVE OFF THE 2D/OVERRIDE BITS                
         MVI   EBDECS,1                                                         
************************************************************                    
* NOTE:  APDUB = RATING WITH 2D/OVERRIDE BITS              *                    
*        APFULL = RATING WITHOUT THE BITS                  *                    
*        APBYTE = 2D/OVERRIDE BITS IF IT EXISTS IN APDUB   *                    
************************************************************                    
***  2 DECIMAL                                                                  
         TM    APFULL,DMODEM2D     X'40' 2 DECIMAL PRECISION?                   
         BZ    REP27C               - NOPE                                      
         MVI   EBDECS,2             - YUP                                       
         NI    APFULL,255-DMODEM2D                                              
***  2 DECIMAL                                                                  
REP27C   MVI   EBSCIN,0                                                         
...
***      OC    APDUB(4),APDUB      TEST FOR NON-ZERO RATING                     
         MVC   APBYTE,APDUB        SAVE OFF 2D/OVERRIDE BITS                    
         MVC   APFULL,APDUB        SAVE OFF RATING WITHOUT BITS                 
         NI    APFULL,X'FF'-X'C0'   TAKE OFF 2D/OVERRIDE BITS                   
         OC    APFULL,APFULL       TEST FOR NON-ZERO RATING                     
         BZ    REP28                                                            
...
**       BE    REP27G               - YUP                                       
**       CLI   ESTDEMS+1,C'E'      MAIN DEMO IS E-RATING?                       
**       BNE   REP27K               - NOPE, IT'S IMPRESSION                     
REP27G   M     R0,=F'10'                                                        
***  2 DECIMAL                                                                  
REP27K   M     R0,=F'20'                                                        
***      D     R0,APDUB                                                         
         D     R0,APFULL           APFULL HAS RATING AT THIS POINT              
         LA    R1,1(R1)                                                         
         SRA   R1,1                                                             
         ST    R1,APFULL           APFULL IS NOT RATING ANYMORE                 
         MVI   EBDECS,2                                                         
         MVI   EBSCIN,0                                                         
         LA    R1,PCPP                                                          
         ST    R1,EBAOUT                                                        
...
*                                                                               
         MVC   APBYTE,APDUB+4      SAVE OFF 2D/OVERRIDE BITS                    
         MVC   APFULL,APDUB+4      SAVE OFF RATING WITHOUT BITS                 
         NI    APFULL,X'FF'-X'C0'   TAKE OFF 2D/OVERRIDE BITS                   
         OC    APFULL,APFULL       CPM                                          
...
**       BE    REP27P               - YUP                                       
**       CLI   ESTDEMS+1,C'E'      MAIN DEMO IS E-RATING?                       
**       BNE   REP27R               - NOPE, IT'S IMPRESSION                     
REP27P   M     R0,=F'10'                                                        
***  2 DECIMAL                                                                  
REP27R   M     R0,=F'20'                                                        
**       D     R0,APDUB+4                                                       
         D     R0,APFULL           APFULL HAS THE RATING STILL                  
         LA    R1,1(R1)                                                         
         SRA   R1,1                                                             
         ST    R1,APFULL           NOT ANYMORE NOW                              
         LA    R1,PCPM                                                          
         ST    R1,EBAOUT                                                        
         MVI   EBLOUT,L'PCPM                                                    
         MVI   EBFLOAT,C'$'                                                     
...
*                                                                               
         ...
         MVC   REPH4+109(22),=C'(C) 20XX NIELSEN MEDIA'                         
         GOTO1 VDATCON,APPARM,(5,0),(20,APDUB)                                  
         MVC   REPH4+113(4),APDUB   UPDATE THE YEAR!                            
         CLI   CUDMED,C'C'        IF CANADIAN?                                  
         BNE   *+10                - NOPE, DON'T REPLACE WITH CSI               
         MVC   REPH4+115(22),=C'CSI'                                            
*                                                                               
         MVC   REPH5+99(L'CMPUPIN),CMPUPIN                                      
...
*                                                                               
         ...
         CLI   DEMOS+1,C'R'                                                     
         BE    *+8                                                              
         CLI   DEMOS+1,C'E'        EWM2554 IS ALSO A RATING...                  
         BE    *+8                 ...NOT AN IMPRESSION!!  MHC 05/12/03         
         MVI   REPM2+23,C'M'                                                    
         MVI   REPM2+83,C'M'                                                    
         CLI   DEMOS+4,C'R'                                                     
         BNE   *+8                                                              
...
GETCAM10 GOTO1 AGETPRD,CMPPRDN     GET PRODUCT                                  
         ...
         BE    GETCAM15                                                         
         MVC   FVMSGNO,=AL2(FVIEST)   INVALID ESTIMATE                          
GETCAM15 LR    RE,R0                                                            
         BR    RE                                                               
         EJECT                                                                  
***********************************************************************         
...
GETNAMES NTR1                                                                   
         ...
         BNZ   *+8                                                              
         MVI   APPARM,1                                                         
         GOTO1 VDEMOCON,APPARM,,(6,DEMNAMES),(C'S',DBLOCK)                      
...
DBLOCKD  DSECT                                                                  
       ++INCLUDE DEDBLOCK                                                       
         ...
         SPACE 1                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'039SPNWS21   02/27/07'                                      
         END                                                                    
