*          DATA SET DECAVCO    AT LEVEL 015 AS OF 02/28/20                      
...
*INCLUDE SMTP                                                                   
*---------------------------------------------------------------------*         
* WEEKLY CABLE COMMERCIAL AVERAGE CONVERSION OUTPUT PHASE             *         
* IPHASE: DECAVCI                                                     *         
* OPHASE: DECAVCO                                                     *         
*                                                                     *         
* NOTE: THIS CONVERSION HAS BEEN COPIED FROM THE WKLY CABLE CONVERSION*         
*       (DECNVCB2) WITH THE NECESSARY CHANGES. ALTHOUGH THERE IS NO   *         
...
DECAVCO  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 CBLWRKX-CBLWRKD,*DECAVCO,RA,R4                                   
         USING CBLWRKD,RC          RC=A(TEMP W/S)                               
...
QCNV12   MVC   PRGKEY,INTKEY       SAVE  RECDS PRG KEY                          
         MVC   PRGMEDIA,INTMEDIA                                                
         XC    THISKEY,THISKEY                                                  
         LA    R6,THISKEY                                                       
         USING PMKEY,R6            BUILD -Q- RECD NEW PRG KEY                   
         MVI   PMCODE,PMCODEQU                                                  
         MVC   PMMEDIA,INTMEDIA                                                 
         MVC   PMSRC,INTKEY+QKSRC-QPRGKD  SOURCE/VIEWING TYPE                   
         MVC   PMBOOK,INTBOOK                                                   
         MVC   PMSTAT,INTSTA                                                    
...
PCNV     DS    0H                                                               
*                                                                               
         OC    PREVKEY,PREVKEY     CK IF NEED TO RELEASE PREV RECORD            
         BZ    PCNV10                                                           
         CLI   PREVKEY,PMCODEQU    'Q' RECORD? (ORIGINAL OR CORRECTION)         
         BNE   PCNV10                                                           
         CLI   PRGKEY,C'R'         CORRECTION RECD?                             
         BNE   *+8                                                              
         BAS   RE,RELS             RELEASE REST OF CORRECTION RECD              
         BAS   RE,FINFF            OUTPUT '0F-FF' ON PRG RECDS                  
         OC    CNTKEY,CNTKEY       RELEASE PREV Q-RECD'S CNTL RECD              
         BZ    *+8                                                              
         BAS   RE,CNTREC                                                        
*                                                                               
PCNV10   XC    THISKEY,THISKEY                                                  
         LA    R6,THISKEY                                                       
         USING PRKEY,R6                                                         
         MVI   PRCODE,PRCODEQU     BUILD -P- RECD KEY                           
         MVC   PRMEDIA,INTMEDIA    MEDIA FROM INTKEY                            
         MVC   PRSRC,INTKSRC                                                    
         MVC   PRSTAT,INTSTA                                                    
         MVC   PRBOOK,INTBOOK                                                   
         MVC   PRBTYP,INTBTYP                                                   
         ...
         CLI   PREVKEY,PRCODEQU    'P' RECORD                                   
         BNE   *+8                                                              
         BAS   RE,PUTTAPE          PUT OUT PREV RECD                            
*                                                                               
PCNV20   MVI   CORRFLAG,NO         NOT A CORRECTION RECORD                      
...
*                                                                               
RCNV2    OC    PREVKEY,PREVKEY     NO, NEW PRGM.  RELEASE LAST RECD?            
...
RCNV12   MVI   COPY,C'Y'                                                        
         ...
         DROP  RE                                                               
RCNV14   OC    DDSNUM,DDSNUM       DO WE HAVE AN INTERNAL PRG# ?                
         BNZ   *+8                                                              
...
*                                                                               
RCNV33   CLI   0(R6),DTCODEQ       DATA TYPE ELEMENT? (VIEWING TYPE)            
         BNE   RCNV35                                                           
         ZIC   RE,1(R6)                                                         
         BCTR  RE,0                                                             
         EXMVC RE,TEMP,0(R6)       COPY DATA TYPE ELEMENT                       
         GOTO1 APUTEL,TEMP         PUT IT ON RECD                               
         L     R6,IPTR                                                          
         ZIC   RE,1(R6)            BUMP TO NEXT ELEMENT                         
         AR    R6,RE                                                            
         ST    R6,IPTR             PT TO 1ST DATA ELEMENT                       
...
ADD10    CLI   0(R1),0                                                          
         ...
         BE    ADD15                                                            
         CLI   0(R1),DTCODEQ       ALREADY CREATED                              
         BE    ADD15                                                            
*M       CLI   COPYMAR,C'Y'                                                     
...
CPY30    CLI   0(R6),X'00'         END OF RECORD?                               
         ...
         BE    CPY35                                                            
         CLI   0(R6),DTCODEQ       DON'T COPY PREV DATA TYPE ELEMS              
         BE    CPY35                                                            
...
*                                                                               
         ...
         LA    R6,4(R6)                                                         
         MVC   DATADISP,=H'23'                                                  
         MVI   ELCODE,TIMCODEQ     CONTROL RECORDS HAS TO HAVE TIME EL          
         BAS   RE,GETEL                                                         
         BNE   GETUNVX             NOT A CONTROL RECORD                         
*                                                                               
         L     R6,AIREC            PT TO RECD IN AIREC                          
         LA    R6,4(R6)                                                         
         MVC   DATADISP,=H'23'                                                  
...
DDSFIL   NTR1                                                                   
         ...
         CLI   INTMEDIA,C'A'       LOWER CASE MEDIA IS COMMERCIAL DATA          
         BNL   *+8                                                              
         L     R1,=A(DDSTBLC)      PT TO COMMERCIAL NTI-DDS TABLE               
         USING DDSTBLD,R1                                                       
         MVC   DDSBK,INTBOOK                                                    
         MVC   DDSBKTY,INTBTYP                                                  
...
*        BE    DDSF12                                                           
         ...
         CLC   LSTMED,INTMEDIA                                                  
         BE    *+8                                                              
DDSF11   BAS   RE,RDRECS           GET LATEST FILE# FROM FILE                   
         MVC   FILENUM,LSTFNUM                                                  
...
DDSF30   DS    0H             BUMP FILE NUMBER BASED ON BK IN THISKEY           
         L     R1,=A(DDSTBL)                                                    
         LA    RE,THISKEY                                                       
         CLI   PMMEDIA-PMKEY(RE),C'A'   LOWER CASE IS COMMERCIAL DATA           
         BNL   *+8                                                              
         L     R1,=A(DDSTBLC) USE DIFFERENT TABLE FOR COMMERCIAL AVG            
...
*                                                                               
         CLI   INTKEY,C'P'                                                      
         BNE   *+14                                                             
         MVC   SVKSRC,INTKSRC        VIEWING TYPE FOR P-RECORDS                 
         B     BLDEL07                                                          
         CLI   INTKEY,C'Q'         VIEWING TYPE FOR Q AND R-RECORDS             
         BE    *+14                                                             
         CLI   INTKEY,C'R'                                                      
         BE    *+6                                                              
         DC    H'0'                OTHER RECORD TYPES?                          
         MVC   SVKSRC,INTKEY+QKSRC-QPRGKD                                       
BLDEL07  BAS   RE,BLDDT            BUILD DEMO DATA TYPE ELEMENT                 
...
BLDEL30  XC    0(NTLENEQ3,R6),0(R6) CLEAR ELEMENT AREA                          
         MVI   NTCODE,NTCODEQU                                                  
         MVI   NTLEN,NTLENEQ3                                                   
         MVC   NTSQH,INTSQH                                                     
         MVC   NTEQH,INTEQH                                                     
...
*                                                                               
         ...
         MVC   NTEOA,INTEOA                                                     
         MVC   NTGAP,INTGAP                                                     
         MVC   NTCMSEC,INTCOMSC    COMMERCIAL SECONDS                           
         MVC   NTCMTEL,INTCOMTC    COMMERCIAL TELECASTS                         
         GOTO1 APUTEL,NTELEM                                                    
*                                                                               
...
GENN2    LA    R6,PTRKEY           BUILD 'N' RECORD KEY                         
         USING PNKEY,R6                                                         
         MVI   PNCODE,PNCODEQU                                                  
         MVC   PNMEDIA,INTMEDIA    MEDIA FROM INTKEY                            
         MVC   PNSRC,INTKEY+QKSRC-QPRGKD                                        
...
LST15    OC    CNTKEY,CNTKEY       RELEASE LAST CNTL RECD                       
         BZ    LST20                                                            
         BAS   RE,CNTREC                                                        
LST20    BAS   RE,SVTBLS           RELEASE J-RECDS (NTI-DDS-FILE#'S)            
         MVI   SORTSW,0                                                         
*                                                                               
         TM    FLAGS1,CREATE_NETCTREC   NETCTREC=Y                              
         BNO   LST30                                                            
...
SVTB15   LA    R5,L'NTITBL(R5)                                                  
         B     SVTB10                                                           
*                                                                               
SVTB20   DS    0H                  RELEASE 'J' RECDS FOR PROGRAM DATA           
         L     R5,=A(DDSTBL)                                                    
         USING DDSTBLD,R5                                                       
SVTB25   OC    0(5,R5),0(R5)       IS TABLE EMPTY?                              
         ...
         MVI   PJCODE,PJCODEQU                                                  
         MVI   PJMEDIA,C'C'                                                     
         MVC   PJSRC,SVKSRC        REPLACE WITH PROG AVG VIEWING TYPES          
*                                                                               
         CLC   =C'OOH',IPHSPARM    OOH MIT TAPE (SPEC-30665)                    
         BNE   SVTB27                                                           
         CLI   SVKSRC,SRCLOCLQ     YES - LIVE?                                  
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOLQ                                                    
         CLI   SVKSRC,SRCLOCSQ     LIVE+SD?                                     
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOSQ                                                    
         CLI   SVKSRC,SRCLOC3Q     LIVE+3?                                      
         JNE   *+8                                                              
         MVI   PJSRC,SRCLO3Q                                                    
         CLI   SVKSRC,SRCLOC7Q     LIVE+7?                                      
         JNE   *+8                                                              
         MVI   PJSRC,SRCLO7Q                                                    
         J     SVTB28                                                           
*                                                                               
SVTB27   CLI   SVKSRC,SRCLIVE                                                   
         BNE   *+8                                                              
         MVI   PJSRC,SRCALV                                                     
         CLI   SVKSRC,SRCLIVSD                                                  
         BNE   *+8                                                              
         ...
         MVI   PJSRC,SRCAL7                                                     
*                                                                               
SVTB28   MVC   PJBOOK,DDSBOOK      SET BOOK                                     
...
SVTB30   LA    R5,L'DDSTBL(R5)                                                  
         ...
         DROP  R5,R6                                                            
*                                                                               
SVTB40   L     R5,=A(DDSTBLC)      RELEASE 'J' RECORDS FOR COMM DATA            
         USING DDSTBLD,R5                                                       
SVTB45   OC    0(5,R5),0(R5)       IS TABLE EMPTY?                              
         BZ    SVTBLX                                                           
         LA    R6,KEY              LOOK UP NTI PGR CODE PASSIVE RECD            
         XC    KEY,KEY                                                          
         USING PJKEY,R6                                                         
         MVI   PJCODE,PJCODEQU                                                  
         MVI   PJMEDIA,X'83'                                                    
         MVC   PJSRC,SVKSRC        REPLACE WITH COMM VIEWING TYPES              
*                                                                               
         CLC   =C'OOH',IPHSPARM    OOH MIT TAPE (SPEC-30665)                    
         BNE   SVTB47                                                           
         CLI   SVKSRC,SRCLOLQ      YES - LIVE?                                  
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOCLQ                                                   
         CLI   SVKSRC,SRCLOSQ      LIVE+SD?                                     
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOCSQ                                                   
         CLI   SVKSRC,SRCLO3Q      LIVE+3?                                      
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOC3Q                                                   
         CLI   SVKSRC,SRCLO7Q      LIVE+7?                                      
         JNE   *+8                                                              
         MVI   PJSRC,SRCLOC7Q                                                   
         J     SVTB48                                                           
*                                                                               
SVTB47   CLI   SVKSRC,SRCALV                                                    
         BNE   *+8                                                              
         MVI   PJSRC,SRCLIVE                                                    
         CLI   SVKSRC,SRCALS                                                    
         BNE   *+8                                                              
         ...
         MVI   PJSRC,SRCLIVE7                                                   
*                                                                               
SVTB48   MVC   PJBOOK,DDSBOOK      SET BOOK                                     
         MVC   PJBTYPE,DDSBKTY     BOOK TYPE                                    
         MVC   PJSTAT(4),DDSNET    NETWORK                                      
         MVI   PJSTAT+4,C'C'       CABLE                                        
         CLC   DDSPRG,=X'FFFF'     OUTPUT MAX FILE# AS DDSPRG#=0                
...
CKNTIX   B     XIT                                                              
**********************************************************************          
* USING SVKSRC, X'08' ELEMENT PASSED BACK IN TEMP                               
**********************************************************************          
BLDDT    NTR1                                                                   
*                                                                               
         LA    R6,TEMP                                                          
...
*                                                                               
SUBJECT  DC    C'UNKNOWN STATIONS IN THE ACM CABLE CONVERSION, '                
SUBJJBID DS    CL8                                                              
         DC    C' '                                                             
SUBJJBNM DS    CL8                                                              
...
NTITBL   DC    XL12'00'            NTI-DDS: NET(5) , NTI(5) ,  DDS(2)           
         DC    2500XL12'00'                                                     
         DC    X'FFFFFFFFFF'                                                    
*TABLE FOR PROGRAM AVERAGE PROGRAM NO/FILE NO                                   
DDSTBL   DC    XL(DDSQ)'00'        DDS-FILE: NET(5) , DDS(2), FILE(2)           
         DC    10000XL(DDSQ)'00'                                                
         DC    X'FFFFFFFFFF'                                                    
*TABLE FOR COMMERCIAL AVERAGE PROGRAM NO/FILE NO                                
DDSTBLC  DC    XL(DDSQ)'00'        DDS-FILE: NET(5) , DDS(2), FILE(2)           
         DC    10000XL(DDSQ)'00'                                                
         DC    X'FFFFFFFFFF'                                                    
         EJECT                                                                  
**********************************************************************          
...
SVKSRC   DS    C                   SAVED SOURCE/VIEWING TYPE                    
*                                                                               
COMMAND  DS    CL7                                                              
ELCODE   DS    X                                                                
DATADISP DS    H                                                                
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'015DECAVCO   02/28/20'                                      
         END                                                                    
