*          DATA SET REREPFA02  AT LEVEL 043 AS OF 05/17/12                      
*PHASE REFA02B                                                                  
*INCLUDE HEXIN                                                                  
*INCLUDE HEXOUT                                                                 
*INCLUDE BINSRCH2                                                               
...
*INCLUDE CASHVAL                                                                
*INCLUDE DATVAL                                                                 
         TITLE 'REREPFA02  (REFA02A) --- FOX TAPE ACTUALIZATION'                
*                                                                               
********************************************************************            
*                                                                  *            
*                                                                  *            
*        REREPFA02  -- FOX ACTUALIZATION:  INVOICE INFORMATION     *            
*                      FROM TAPE.                                  *            
*                      ACCEPT FOX TAPE, UPDATE EXISTING CONTRACTS  *            
*                      AND ADD 'NEW' CONTRACTS (ADDLINES)          *            
*                                                                  *            
*           04/25/12 - 'NEW' CONTRACTS ROUTINE COMMENTED OUT       *            
*                                                                  *            
*  *******  03/15/12 - COPIED FROM REREPKA02 *******               *            
*                                                                  *            
* ---------------------------------------------------------------- *            
* UPDATE HISTORY:                                                  *            
...
*                                                                  *            
* DEC21/11 (KUI) --- ADD B3 AND FB                                 *            
*                                                                  *            
*  MAR/12  (SMY) --- COPY FROM REREPKA02 AND MODIFY FOR FOX        *            
*                    WITH ADDITIONAL EDITING INCLUDING AN          *            
*                    "ALTERNATE CALENDAR" VERSION BASED ON         *            
*                    AN ENTRY IN THE UPLOAD RECORD                 *            
*                    COMMENTED OUT NEWORDER TO PROVIDE             *            
*                    SPACE FOR THE "ALTERNATE CALENDAR" PROCESS    *            
*                                                                  *            
*                                                                  *            
*                    **  END TOMBSTONE  **                         *            
********************************************************************            
...
**       BNH   MAIN0040                                                         
**       C     RF,=F'200'          END AFTER N RECORDS                          
**       BH    MAIN0200                                                         
*                                                                               
         CLI   ACLERR,C'E'         NO ALTERNATE CALENDAR RECORD ERROR           
         BE    MAIN0200            END RUN                                      
*                                                                               
         L     RF,DISPCTR                                                       
         LA    RF,1(RF)                                                         
...
MAIN0120 EQU   *                                                                
         ...
         B     MAIN0140                                                         
MAIN0130 EQU   *                                                                
         CLI   QOPTION1,C'E'       TEST RUN - EJOR/B3?                          
         BNE   MAIN0133            NO                                           
         MVC   RUNID,=C'EJOR '     SAVE RUN ID                                  
         MVC   POWERCDE,=C'B3'     SET POWER CODE                               
         LA    R3,TESTDIV          YES                                          
         B     MAIN0140                                                         
MAIN0133 EQU   *                                                                
         CLI   QOPTION1,C'F'       FSSNY       RUN?                             
         BNE   MAIN0135            NO                                           
*                                                                               
         MVI   QUESTOR+6,C'Y'      FORCE "SHORT" ERROR PRINT FOR FOX            
*                                                                               
         MVC   RUNID,=C'FSSNY'     SAVE RUN ID                                  
         MVC   POWERCDE,=C'FB'     SET POWER CODE                               
         LA    R3,FOXDIV           YES                                          
         B     MAIN0140                                                         
MAIN0135 EQU   *                                                                
...
*                                                                               
         XC    ACLREC,ACLREC       CLEAR ALT CALENDAR RECORD AREA               
         XC    ACLERR,ACLERR       AND ALT CALENDAR ERROR INDICATOR             
*                                                                               
...
*                                                                               
ALTC003  EQU   *                                                                
         MVI   NEW04ELT,X'04'      CONTRACT STATION AMOUNT ELEM CODE            
*                                                                               
         CLI   KCONALTC,C'A'       USE ALTERNATE CALENDAR ?                     
         BNE   OLDR0003            NO                                           
*                                                                               
         MVI   NEW04ELT,X'54'      ALTERNATE CON STATION AMT ELEM CODE          
*                                                                               
         CLI   ACLREC,X'20'        ALREADY HAVE ALTERNATE CALENDAR REC?         
         BE    OLDR0003            YES                                          
*                                                                               
*        ****** GET AND SAVE ALTERNATE CALENDAR RECORD DATA ******              
*                                                                               
...
*                                  CONVERT DATE TO BINARY FORMAT                
         MVC   MNTHNUM,WORK+7      SAVE MONTH FROM DATE FIELD                   
*                                                                               
         MVC   KEYWORK(27),KEY     SAVE CONTRACT KEY                            
         XC    KEY,KEY                                                          
         MVI   KEY,X'20'           INSERT ALTERNATE CALENDAR REC TYPE           
         MVC   KEY+1(2),REPUSE     INSERT REP CODE                              
         MVC   KEY+8(1),WORK+6     INSERT YEAR                                  
*                                  GET THE ALT CALENDAR RECORD                  
         GOTO1 HIGH                                                             
         CLC   KEY(27),KEYSAVE     ALT CALENDAR RECORD FOUND?                   
         BNE   ALTC003X            NO - PRINT ERROR AND END RUN                 
*                                                                               
         MVC   FULL,AIOAREA        SAVE CURRENT A(IOAREA)                       
         LA    RF,ACLREC           SET ALTERNATE IO AREA                        
         ST    RF,AIOAREA                                                       
*                                                                               
         GOTO1 GREC                GET ALT CALENDAR RECORD (TO ACLREC)          
*                                                                               
         MVC   AIOAREA,FULL        RESTORE ORIGINAL A(IOAREA)                   
*                                                                               
         XC    KEY,KEY                                                          
         MVC   KEY(27),KEYWORK     "RESTORE" CONTRACT KEY                       
         GOTO1 HIGH                  AND SEQUENCE                               
         CLC   KEY(27),KEYSAVE     CONTRACT RECORD FOUND?                       
         BE    *+6                 YES                                          
         DC    H'0'                MUST BE FOUND                                
*                                                                               
         LA    R6,ACLREC           ALTERNATE CALENDAR REC                       
         MVI   ELCODE,X'10'     ** SAVE CALENDAR START AND END DATES            
         BAS   RE,GETEL         **   (ACLSTART & ACLEND)                        
         BNZ   ALTC003X            NO ELEM - PRINT ERROR AND END RUN            
*                                  R6==> REP ELEMENT OF ALT CAL REC             
         ZIC   RF,MNTHNUM          MONTH NUMBER                                 
         MHI   RF,2                * 2 TO DISPLACE TO ELEMENT MONTH             
         AR    R6,RF               DISPLACE TO MONTH'S DATA                     
         ZICM  RF,0(R6),2          MOVE COMPRESSED DATE TO REG                  
         LA    RF,1(RF)            START DATE FOR THIS CALENDAR MONTH           
         STH   RF,WORK                                                          
         GOTO1 DATCON,DMCB,(2,WORK),(3,ACLSTART)  ALT BCAST CAL START           
         MVC   WORK(2),2(R6)       END DATE FOR THIS CALENDAR MONTH             
         GOTO1 DATCON,DMCB,(2,WORK),(3,ACLEND)    ALT BCAST CAL END             
         B     OLDR0003                                                         
*                                                                               
ALTC003X EQU   *                   NO ALTERNATE CALENDAR RECORD FOUND           
*          PRINT ERROR MESSAGE AND END THE RUN                                  
         GOTO1 REPORT                                                           
         GOTO1 REPORT                                                           
         MVC   P+15(21),=C'**  JOB CANCELLED  **'                               
         GOTO1 REPORT                                                           
         MVC   P+10(31),=C'* NO ALTERNATE CALENDAR FOUND *'                     
         GOTO1 REPORT                                                           
         MVC   P+15(21),=C'**  JOB CANCELLED  **'                               
         GOTO1 REPORT                                                           
         GOTO1 REPORT                                                           
*                                                                               
         MVI   ACLERR,C'E'         FOR END THE RUN                              
         B     OLDR0900                                                         
*                                                                               
OLDR0003 EQU   *                                                                
*                                  **** CONTRACT # EDIT                         
         MVC   WORK2(9),SPACES     MUST BE FULL 8 NUMERIC CHARACTERS            
         MVC   WORK2(8),KCONKCON                                                
         LA    R1,WORK2                                                         
         LA    R0,8                COUNTER                                      
CNUMTOP  DS    0H                                                               
         CLI   0(R1),X'F0'                                                      
         ...
         BNH   CNUMBK                                                           
CNUMERR  DS    0H                                                               
         MVC   P+01(16),=C'INVALID CONTRACT'                                    
         L     RF,INVCON           INCREMENT COUNTER                            
         LA    RF,1(RF)                                                         
         ST    RF,INVCON                                                        
         B     OLDERRX             ERROR                                        
CNUMBK   DS    0H                                                               
         LA    R1,1(R1)            BUMP TO NEXT CHARACTER IN #                  
         BCT   R0,CNUMTOP          CHECK NEXT                                   
*                                  **** END OF CONTRACT # EDIT                  
*                                                                               
         XC    KEY,KEY                                                          
...
*                                  COMPLEMENT CONTRACT NUMBER                   
         ...
         BE    OLDR0040            YES                                          
         MVC   P+1(18),=C'CONTRACT NOT FOUND'                                   
         L     RF,NOCONT           INCREMENT COUNTER                            
         LA    RF,1(RF)                                                         
         ST    RF,NOCONT                                                        
OLDERRX  EQU   *                                                                
...
*                                                                               
         CLC   KCONRSTA(4),RCONREC+6    UPLOAD STATION=CONREC STATION?          
         BE    OLDR0050            YES                                          
         MVC   P+1(16),=C'STATION MISMATCH'                                     
         L     RF,ODDSTATN         INCREMENT COUNTER                            
         LA    RF,1(RF)                                                         
         ST    RF,ODDSTATN                                                      
         B     OLDERRX             ERROR                                        
*                                                                               
OLDR0050 EQU   *      UPLOAD MONTH/YEAR MUST BE WITHIN CONTRACT FLIGHTS         
         CLI   KCONALTC,C'A'       USE ALTERNATE CALENDAR ?                     
         BE    OLDR0060            YES                                          
*TESTING                                                                        
*****    B     OLDR0060                                                         
*TESTING                                                                        
         MVC   WORK+6(2),KCONRFLT+4  SET YEAR FIRST                             
         MVC   WORK+8(2),KCONRFLT  SET MONTH                                    
         MVC   WORK+10(2),=C'15'    SET DAY TO 15                               
*                                                                               
         GOTO1 DATCON,DMCB,(0,WORK+6),(0,WORK)      YMD BROADCAST START         
*                                                                               
         GOTO1 GETBROAD,DMCB,WORK,WORK+6        GET BROADCAST START/END         
*                                                                               
         GOTO1 DATCON,DMCB,(0,WORK+6),(3,FLTSTART)  YMD BROADCAST START         
         GOTO1 DATCON,DMCB,(0,WORK+12),(3,FLTEND)   YMD BROADCAST END           
*&&DO                                                                           
         MVC   P(5),=C'GETB:'                                                   
         MVC   P+10(6),WORK                                                     
         MVC   P+20(6),WORK+6                                                   
...
*&&                                                                             
         LA    RF,RCONREC+34                                                    
         USING RCONELEM,RF                                                      
         CLC   FLTSTART,RCONDATE+3                                              
         BH    OLDR005E            ERROR - NOT WITHIN CONTRACT FLIGHT           
         CLC   FLTEND,RCONDATE                                                  
         BL    OLDR005E            ERROR - NOT WITHIN CONTRACT FLIGHT           
         B     OLDR0070                                                         
         DROP  RF                                                               
OLDR005E EQU   *                                                                
         MVC   P+1(21),=C'INVALID INVOICE MONTH'                                
         L     RF,BADDATES         INCREMENT COUNTER                            
         LA    RF,1(RF)                                                         
         ST    RF,BADDATES                                                      
         B     OLDERRX             ERROR                                        
*                                                                               
OLDR0060 EQU   *                   USE ALTERNATE CALENDAR                       
*                                                                               
         CLI   ACLREC,X'20'        DO WE HAVE AN ALT CALENDAR RECORD?           
         BNE   ALTC003X            NO - PRINT ERROR AND END RUN                 
*                                                                               
OLDR006C EQU   *                                                                
         LA    RF,RCONREC+34                                                    
         USING RCONELEM,RF                                                      
         CLC   ACLSTART,RCONDATE+3                                              
         BH    OLDR005E            ERROR - NOT WITHIN CONTRACT FLIGHT           
         CLC   ACLEND,RCONDATE                                                  
         BL    OLDR005E            ERROR - NOT WITHIN CONTRACT FLIGHT           
         B     OLDR0070                                                         
...
*                                  INSERT ACTIVITY DATE                         
         ...
         MVC   P+1(19),=C'MONTHLY $$$ INVALID'                                  
         L     RF,BADDOLS          INCREMENT COUNTER                            
         LA    RF,1(RF)                                                         
         ST    RF,BADDOLS                                                       
         B     OLDERRX             ERROR                                        
*                                                                               
...
*                                                                               
         ...
         MOVE  ((RF),(R1)),(RE)                                                 
OLDR0120 EQU   *                                                                
         LA    R6,RCONREC          DELETE OLD 04 OR 54 ELTS FOR MONTH           
*                                                                               
         MVI   ELCODE,X'04'        CONTRACT STATION AMOUNT ELEM                 
         CLI   KCONALTC,C'A'       ALTERNATE CALENDAR?                          
         BNE   OLDR0130            NO                                           
*                                                                               
         MVI   ELCODE,X'54'        ALTERNATE CONT STATION AMOUNT ELEM           
*                                                                               
OLDR0130 EQU   *                                                                
         BAS   RE,GETEL                                                         
...
***      GOTO1 REPORT                                                           
         ...
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C' INVALID CONTRACT NO.  :'                             
         EDIT  INVCON,(12,P+30),COMMAS=YES                                      
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C' CONTRACT NOT FOUND    :'                             
         EDIT  NOCONT,(12,P+30),COMMAS=YES                                      
         GOTO1 REPORT                                                           
***      MVC   P+1(24),=C' PRODUCT NOT ON FILE   :'                             
***      EDIT  NOPROD,(12,P+30),COMMAS=YES                                      
***      GOTO1 REPORT                                                           
         MVC   P+1(24),=C' STATION MISMATCH      :'                             
         EDIT  ODDSTATN,(12,P+30),COMMAS=YES                                    
         GOTO1 REPORT                                                           
***      MVC   P+1(24),=C' SALESPERSON UNKNOWN   :'                             
***      EDIT  NOSALPER,(12,P+30),COMMAS=YES                                    
***      GOTO1 REPORT                                                           
         MVC   P+1(24),=C' INVALID INVOICE MONTH :'                             
         EDIT  BADDATES,(12,P+30),COMMAS=YES                                    
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C' INVALID INVOICE $$    :'                             
         EDIT  BADDOLS,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
...
*                                                                               
MNTHNUM  DS    XL1                                                              
ACLSTART DS    XL3                 ALT CALENDAR START DATE                      
ACLEND   DS    XL3                 ALT CALENDAR   END DATE                      
ACLREC   DS    CL150                                                            
ACLERR   DS    CL1                                                              
*                                                                               
ELCODE   DS    CL1                                                              
...
FILOUTB  DCB   DDNAME=FILOUTB,DSORG=PS,RECFM=VB,MACRF=PM,              X        
               LRECL=4004,BLKSIZE=32760,BUFNO=2                                 
INTAPE   DCB   DDNAME=INTAPE,DSORG=PS,RECFM=FB,LRECL=400,              X        
               BLKSIZE=4000,MACRF=GM,EODAD=MAIN0200                             
*                                                                               
         SPACE 3                                                                
...
*                                                                               
         ...
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'043REREPFA02 05/17/12'                                      
         END                                                                    
