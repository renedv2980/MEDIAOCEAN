*          DATA SET DMFLASHER  AT LEVEL 001 AS OF 01/12/09                      
*PHASE DMFLASHA                                                                 
*INCLUDE DMDMGRL                                                                
*INCLUDE CARDS                                                                  
*INCLUDE HEXOUT                                                                 
...
*INCLUDE PRINTER                                                                
*INCLUDE LOGIO                                                                  
         TITLE 'DMFLASHER - SET FILE FLASHABLE IN DATA SPACE'                   
         PRINT NOGEN                                                            
FLASHER  CSECT                                                                  
         ENTRY SSB                                                              
         ENTRY UTL                                                              
         NBASE 0,DMFLASH,WORK=A(FLASHWRK)                                       
*                                                                               
INIT     L     RA,=V(CPRINT)                                                    
         USING DPRINT,RA                                                        
         MVC   TITLE(40),=C'XXX - SET DATASPACE FLAG FOR FLASH COPY '           
...
INPUT4X  EQU   *                                                                
*                                                                               
INPUT5   CLC   =C'FLASH=',C       FLASH=Y/N TO TURN OFF/ON                      
         BNE   INPUT5X                                                          
         MVC   P(30),C                                                          
         CLI   C+6,C'Y'                                                         
         BE    *+12                                                             
         CLI   C+6,C'N'                                                         
         BNE   INPUT5E                                                          
         CLI   FLASH,C' '         ONLY ONE FLASH= CARD ALLOWED                  
         BNE   INPUT5E                                                          
         MVC   FLASH,C+6                                                        
         MVC   P(30),C                                                          
         GOTO1 =V(PRINTER)                                                      
         OI    PRINTF,X'01'                                                     
         B     INPUT                                                            
INPUT5E  MVC   P+30(32),=CL32'**ERROR** INVALID FLASH= CARD'                    
         GOTO1 =V(LOGIO),DUB,1,(62,P)                                           
         GOTO1 =V(PRINTER)                                                      
         B     FLASHERX                                                         
INPUT5X  EQU   *                                                                
...
*                                                                               
INPUTB   CLI   DSPID,C' '          MUST HAVE DSPACE= INPUT                      
         BE    INPUTBE                                                          
...
INPUTBX  EQU   *                                                                
*                                                                               
INPUTC   CLI   FLASH,C' '          MUST HAVE FLASH= INPUT                       
         BNE   INPUTCX                                                          
INPUTCE  MVC   P+30(32),=CL32'**ERROR** MISSING FLASH= CARD'                    
         GOTO1 =V(LOGIO),DUB,1,(32,P+30)                                        
         GOTO1 =V(PRINTER)                                                      
         B     FLASHERX                                                         
INPUTCX  EQU   *                                                                
...
MAIN1    L     RE,DMCB+8           GET A(FILE INFO DATA)                        
         ...
         TM    DTFFLAG-DTF(R4),DTFFLSH                                          
         BO    MAIN2                                                            
         CLI   FORCE,C'Y'                                                       
         BE    MAIN2                                                            
         MVC   P+30(32),=CL32'**ERROR** FILE IS NOT FLASHABLE'                  
         GOTO1 =V(LOGIO),DUB,1,(62,P)                                           
         GOTO1 =V(PRINTER)                                                      
         B     MAINX                                                            
*                                                                               
MAIN2    BAS   RE,FLASHY           SINGLE FILE                                  
         MVC   P+00(9),C                                                        
         MVC   P+10(5),SENAME                                                   
         MVC   P+16(8),DDNAME                                                   
         MVC   P+25(1),FLASH                                                    
...
*                                                                               
         LA    R0,21               SET UP SVC CALL FOR DATA SPACE               
         LNR   R0,R0                                                            
         LA    R1,WORK                                                          
         XC    WORK(28),WORK                                                    
         MVC   WORK(4),=C'GETA'                                                 
...
SETDSPX  XIT1                                                                   
         EJECT                                                                  
***********************************************************************         
*ROUTINE TO FIND FILE DATA IN DATASPACE AND TURN ON/OFF FLASH BIT     *         
***********************************************************************         
         SPACE 1                                                                
FLASHY   NTR1                                                                   
         ...
         DROP  RE                                                               
*                                                                               
FLASHY1  SR    RF,RF               LOCATE DATASPACE INFO FOR SENUM              
         IC    RF,SENUM                                                         
         SLL   RF,6                                                             
...
*                                                                               
FLASHY2  MVI   RESULT,C'2'         TEST THE SE NUM MATCHES                      
         ...
         LA    R2,DSYDATA                                                       
         USING DSFILHDR,R2                                                      
FLASHY3  CLC   DSFILNUM,FILNUM     FIND THE CORRECT FILE ENTRY                  
         BE    FLASHY4                                                          
         LA    R2,32(,R2)                                                       
         BCT   R0,FLASHY3                                                       
         B     FLASHYX                                                          
*                                                                               
FLASHY4  MVC   WFILHDR,0(R2)       COPY THE FILE HEADER INTO W/S                
         MVI   RESULT,C'N'         SET FILE WAS NOT FLASHED                     
         TM    DSFILFLG,X'80'                                                   
         BZ    *+8                                                              
         MVI   RESULT,C'Y'         SET FILE WAS FLASHED                         
*                                                                               
FLASHY5  NI    DSFILFLG,255-X'80'  TURN OFF FILE IS FLASHED                     
         CLI   FLASH,C'Y'                                                       
         BNE   *+8                                                              
         OI    DSFILFLG,X'80'      SET FILE IS FLASHED                          
*                                                                               
FLASHYX  CLI   RESULT,C'0'         EXIT WITH CC LOW IF OK                       
         SAC   0                                                                
         XIT1                                                                   
         EJECT                                                                  
...
*DDDPRINT                                                                       
       ++INCLUDE DDDPRINT                                                       
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'001DMFLASHER 01/12/09'                                      
         END                                                                    
