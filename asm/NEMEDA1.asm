*          DATA SET NEMEDA1    AT LEVEL 179 AS OF 05/01/02                      
*************************************                                           
...
*INCLUDE CENTER                                                                 
*INCLUDE NETCOM                                                                 
         TITLE 'T31EA1 - NETWORK ACCOUNTING REPORT'                             
         PRINT NOGEN                                                            
T31EA1   CSECT                                                                  
         NMOD1 0,**ACPR**,RR=R2                                                 
         L     RC,0(R1)                                                         
         ...
         LA    RA,2048(RB)                                                      
         LA    RA,2048(RA)                                                      
         USING T31EA1+4096,RA                                                   
         L     R8,ASPOOLD                                                       
         USING SPOOLD,R8                                                        
         L     R9,ASYSD                                                         
         USING NETSYSD,R9                                                       
         L     R7,ANETWS2          USE W/S AREA 2 FOR W/S                       
         LA    R7,400(R7)          CLIENT REC IN ANETWS1=1280 NOW               
         USING ACCTD,R7                                                         
         ST    R2,RELO                                                          
*                                                                               
         LA    R2,HOOK                                                          
...
*                                                                               
         ...
         BAS   RE,TOTTOTS                                                       
*                                                                               
TX4      BAS   RE,ENDCOMM           PRINT END COMMENTS                          
*                                                                               
TXMOD    XMOD1                                                                  
*                                                                               
PROCERR  DC    F'0'                                                             
...
*                                                                               
         L     R3,ATWA                                                          
         USING T31EFFD,R3                                                       
         CLI   SPLSKP,C'N'         IF WANT A NEW PAGE FOR EACH NETWORK          
         BE    NET6                                                             
         MVI   FORCEHED,C'Y'       FORCE NEW PAGE                               
NET6     GOTO1 SPOOL,DMCB,(R8)     BLANK LINE OR NEW PAGE                       
         DROP  R3                                                               
*                                                                               
...
*                                                                               
         ...
         MVC   PHPRD,SPLPRO                                                     
         MVC   PHPRNM,SPLPRON                                                   
         MVC   PHEST(8),SPLEST      IF EST FILT OR RANGE DON'T                  
         CLI   PHEST+3,0            PRINT EST NAME                              
         BNE   HH1                                                              
         MVC   PHESNM,SPLESTN                                                   
HH1      MVC   PHNET,=C'ALL '                                                   
...
HH6      EQU   *                                                                
         ...
         CLI   NBSELFLT,1                                                       
         BNE   HH7                                                              
         MVC   PHFILT,=CL11'(PAID)'                                             
HH7      CLI   NBSELFLT,2                                                       
         BNE   HH8                                                              
         MVC   PHFILT,=CL11'(UNPAID)'                                           
HH8      CLI   NBSELFLT,3                                                       
         BNE   HH9                                                              
...
HH30     EQU   *                                                                
         CLI   SPLNOPT,C'Y'        NET OPTION                                   
         BNE   HH32                                                             
         MVC   0(10,R4),=C'NET OPTION'                                          
         LA    R4,132(R4)                                                       
HH32     EQU   *                                                                
         LA    R1,DFL              OFFSET WHERE WE ARE NOW POINTING             
         SR    R4,R1               RESET TO BEGINNING OF LINE                   
*                                                                               
         OC    NACPCT,NACPCT       CHECK IF A PERCENTAGE USED                   
         BZ    HH34                IF SO, PUT IT IN HEADLINE                    
...
*                                                                               
HH34     ST    R4,CURCOMM                                                       
         BAS   RE,COMENT           R4 IS CURRENT PRINT LINE                     
         DROP  R5                                                               
*                                                                               
* COLUMN HEADERS                                                                
*                                                                               
         USING RWINBLOK,R5                                                      
         L     R5,ADRIVE           GET A(DRIVE TABLE)                           
         L     R4,CURCOMM          GET NEXT COMMENT LINE                        
         LA    R4,132(R4)          1 BLANK LINE AFTER LAST COMMENT              
HK2      CLI   RWINTYPE,0                                                       
         BE    HKXIT                                                            
         MVI   RWHEDILN,HEDLEN                                                  
         ST    R4,RWAHEDOU                                                      
...
HK3      GOTO1 VNEWRW,DMCB,NETSYSD,(R5)                                         
         ZIC   R2,RWPRLEN                                                       
         CLI   RWHEDTYP,NRHGAP     DONT UNDERLINE GAPS                          
         BE    HK4                                                              
...
*********************************************************                       
         EJECT                                                                  
*********************************************************                       
* COMENT - PRINT A COMMENT IN THE HEADLINES ( MAX 4 LINES OF COMMENT)           
*    INPUT: R4 - CURRENT PRINT LINE                                             
*    OUTPUT: CURCOMM - CURRENT COMMENT LINE                                     
************************                                                        
COMENT   NTR1                                                                   
         L     R6,ATWA                                                          
         USING T31EFFD,R6                                                       
...
PHDP     DS    CL8                 DAYPART                                      
*                                                                               
DFL      EQU   40                  OFFSET TO DATE FILTER LINE                   
         ORG   PHEAD+6*PHLENGTH+DFL                                             
PHBPLINE DS    CL30                BILLED OR PAID DATE FILTER LINE              
         ORG   PHEAD+7*PHLENGTH+DFL                                             
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'179NEMEDA1   05/01/02'                                      
         END                                                                    
