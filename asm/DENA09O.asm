*          DATA SET DENA09O    AT LEVEL 030 AS OF 04/16/09                      
*PHASE DENA09OA                                                                 
*INCLUDE DEBKUPDT                                                               
**********************************************************************          
* NAD - MONTHLY NAD CONVERSION WITH SUPPORT FOR WEEKLY DATA                     
* IPHASE: DENA09I                                                               
**********************************************************************          
         TITLE 'DEMCON - NHTI CONVERSION - OUTPUT PHASE'                        
DENA09O  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 NADWRKX-NADWRKD,DENA09O                                          
         USING NADWRKD,RC          RC=A(TEMP W/S)                               
         LA    R7,2048(RB)                                                      
         LA    R7,2048(R7)                                                      
         USING DENA09O+4096,R7                                                  
...
QCNV5    MVI   PMCODE,PMCODEQU                                                  
         ...
         MVC   PMBTYP,INTBTYP                                                   
         BAS   RE,BLDLDKY          BUILD KEY FOR LOAD INDICATOR RECD            
*                                                                               
QCNV6    OC    PREVKEY,PREVKEY                                                  
         ...
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PW1HUT N'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PW7HUT N'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PNNUUUUN'   SAVE HUT UNIVERSE                      
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PN1UUUUN'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PN7UUUUN'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PNNHUT N'                                          
         BE    QCNV6A0                                                          
         CLC   PREVKEY(8),=C'PN1HUT N'                                          
         BE    QCNV6A0                                                          
         CLC   PREVKEY(8),=C'PN7HUT N'                                          
         BNE   QCNV7                                                            
...
*                                                                               
         CLI   THISKEY,PMCODEQU    ---  Q RECORDS ONLY -----------              
         BNE   BLDEL25                                                          
*                                                                               
BLDEL3   DS    0H                                                               
...
*                                                                               
         ...
         CLC   PREVKEY(8),=C'PN7UUUUN'                                          
         BNE   SVHUT11                                                          
SVHUT10A MVC   UNIVHUT,DUB+4       SAVE AWAY UNIV HUT                           
         B     SVHUTX                                                           
*                                                                               
*CALC HUT RATING                                                                
SVHUT11  OC    UNIVHUT,UNIVHUT                                                  
...
CNV30    L     R1,ACOMWRK          ADDRESS OF COMMON WORK AREA                  
         ...
         BE    CNV45                                                            
         CLC   PREVKEY(8),=C'PW1HUT N'  SAVE WEEKLY HUTS IN BUFFER              
         BE    CNV45                                                            
         CLC   PREVKEY(8),=C'PW7HUT N'  SAVE WEEKLY HUTS IN BUFFER              
         BE    CNV45                                                            
         CLC   PREVKEY(8),=C'PNNHUT N'                                          
         BE    CNV40                                                            
         CLC   PREVKEY(8),=C'PN1HUT N'                                          
...
CNV390   DS    0H                  RELEASE NEW BITMAP FOR NETWORK               
         ...
         MVI   SORTSW,0                                                         
*                                                                               
         BAS   RE,RELLDR           RELEASE LOAD INDICATOR RECORDS               
*                                                                               
...
LLRECQ   EQU   23+4                L'RECD + 4BYTES FOR LENGTH                   
         EJECT                                                                  
* ********************************************************************          
...
GETSUM4  LA    R6,THISKEY          BUILD A KEY-INITIALIZE RECORD                
         ...
         CLI   PRBOOK,YR_2007      HAVE ALL STREAMS AFTER 2007                  
         BL    *+10                                                             
         MVC   PRSRC,KEYSRC                                                     
         ZIC   R1,HUTHALF                                                       
         SLL   R1,1                                                             
...
*                                                                               
         ...
         BNE   PUTTP10                                                          
         CLC   =C'UUUU',PRSTAT                                                  
         BE    PUTTAPEX            EXCLUDE UNIVERSES                            
         MVC   DBUBOOK,PRBOOK      BOOK FOR P RECORDS                           
         CLI   PRSTAT+4,C'M'                                                    
         BNE   *+8                                                              
         MVI   DBUPSTYP,DBUPSYNQ   POSTING TYPE FOR SYNDICATION                 
         CLI   PRMEDIA,C'N'        MONTHLY BOOK                                 
         BNE   *+12                                                             
         MVI   DBUBKFLG,DBUBKMQ                                                 
         B     PUTTP20                                                          
         CLI   PRMEDIA,C'W'        WEEKLY BOOK                                  
         BE    *+6                                                              
...
PUTTP10  CLI   PMCODE,PMCODEQU     Q RECORDS                                    
         ...
         CLI   PMSTAT+3,C'T'                                                    
         BE    PUTTAPEX            SKIP SUMMARY RECORDS                         
         CLI   PMSTAT+4,C'M'                                                    
         BNE   *+8                                                              
         MVI   DBUPSTYP,DBUPSYNQ   POSTING TYPE FOR SYNDICATION                 
         CLI   PMMEDIA,C'N'        MONTHLY BOOK                                 
         BNE   *+12                                                             
         MVI   DBUBKFLG,DBUBKMQ                                                 
         B     PUTTP20                                                          
         CLI   PMMEDIA,C'W'        WEEKLY BOOK                                  
         BE    *+6                                                              
         DC    H'0'                INVALID MEDIA                                
         MVI   DBUBKFLG,DBUBKWQ                                                 
         B     PUTTP20                                                          
         DROP  RE                                                               
*                                                                               
PUTTP20  CLI   DBUBOOK+1,0         SKIP IT FOR YEARLY HUTS                      
         BE    PUTTAPEX                                                         
         MVC   DBUACOMS,ACOMFACS                                                
         MVI   DBURTYP,DBURNETQ                                                 
         DROP  R3                                                               
...
DAYS     EQU   (*-DAYTAB)/L'DAYTAB                                              
         SPACE 2                                                                
*                                                                               
LDRTAB   DS    25XL(L'PLKMAJOR)'00'  TABLE OF LOAD INDICATOR RECD KEYS          
LDRTABX  DS    0X                                                               
*                                                                               
* EQUATES                                                                       
*                                                                               
...
* DEDEMTABD                                                                     
       ...
       ++INCLUDE DBUPARMSD                                                      
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'030DENA09O   04/16/09'                                      
         END                                                                    
