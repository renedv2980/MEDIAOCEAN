*          DATA SET SEACS15    AT LEVEL 004 AS OF 10/26/15                      
*PHASE TA0D15A                                                                  
*INCLUDE SCINKEY                                                                
...
VQ030    GOTO1 AFVAL,REPAGRH       VALIDATE ACCESS GROUP FILTER                 
         ...
         MVC   SELAGR,FVIFLD                                                    
*                                                                               
VQ035    GOTO1 AFVAL,REPAPRH       VALIDATE APPROVER GROUP FILTER               
         BNE   VQ040                                                            
         LA    R2,IOKEY                                                         
         USING SAAPREC,R2                                                       
...
***********************************************************************         
         SPACE 1                                                                
FR08     MVC   RPTWORK,SPACES                                                   
*                                                                               
         L     R2,AIOAREA1         CHECK PERSON RECORD KEY TYPE                 
...
FR12     MVC   APRECKEY(L'SAPEKEY),SAPEKEY  SAVE LAST RECORD KEY                
         ...
         XC    APHALF,SAPEDEF                                                   
         XC    APWORK,APWORK                                                    
         GOTO1 VDATCON,APPARM,(2,APHALF),(X'11',APWORK)                         
...
PRAGC    MVI   SELAGRF,1           FLAG ACCESS GROUP FOUND                      
         ...
         SPACE 2                                                                
***********************************************************************         
* APPROVER GROUP CODE FILTERING                                       *         
***********************************************************************         
         USING SAAPCD,R3           APPROVER GROUP ELEMENT                       
PRAPC    CLI   SAAPCLN,SAAPCLNQ                                                 
         BL    FR02                                                             
         MVI   SELAPCF,1                                                        
         OC    SELAPC,SELAPC       APPROVER GROUP CODE FILTER?                  
         BZ    FR16                NO                                           
         CLC   SELAPC,SAAPCCOD     COMPARE ON APPROVER GROUP CODE               
         BNE   FR02                SKIP                                         
         B     FR16                                                             
         SPACE 2                                                                
***********************************************************************         
...
GINF06   L     R2,AIOAREA3                                                      
         ...
         SPACE 2                                                                
***********************************************************************         
* ACCESS GROUP CODE ELEMENT                                           *         
***********************************************************************         
         SPACE 1                                                                
         USING SAAGCD,R3                                                        
GRAGC    MVC   RPTAGC,SAAGCCOD                                                  
         B     GINF04                                                           
         SPACE 2                                                                
***********************************************************************         
* ACTIVITY ELEMENT                                                    *         
...
GRNAM    XR    RF,RF                                                            
         ...
         XR    R1,R1                                                            
*                                                                               
NAM10    TM    SANAMIND,SANAMIFN                                                
         BZ    NAM14                                                            
         IC    R1,0(,RE)                                                        
         BCTR  R1,0                                                             
*&&UK*&& CLI   RPTFMT,0            TEST STANDARD REPORT REQUIRED                
*&&UK*&& BE    NAM12               YES, LAST NAME ONLY                          
         EX    R1,*+8                                                           
         B     *+10                                                             
         MVC   0(0,R8),1(RE)                                                    
         LA    R8,2(R1,R8)                                                      
NAM12    LA    RE,2(R1,RE)                                                      
NAM14    TM    SANAMIND,SANAMIMN                                                
         BZ    NAM18                                                            
         IC    R1,0(,RE)                                                        
         BCTR  R1,0                                                             
*&&UK*&& CLI   RPTFMT,0            TEST STANDARD REPORT REQUIRED                
*&&UK*&& BE    NAM16               YES, LAST NAME ONLY                          
         EX    R1,*+8                                                           
         B     *+10                                                             
         MVC   0(0,R8),1(RE)                                                    
         LA    R8,2(R1,R8)                                                      
NAM16    LA    RE,2(R1,RE)                                                      
NAM18    TM    SANAMIND,SANAMILN                                                
         BZ    GINF04                                                           
         IC    R1,0(,RE)                                                        
         BCTR  R1,0                                                             
         EX    R1,*+8                                                           
...
P        USING REPAD,BOXCOLS                                                    
         ...
         BNE   *+12                YES                                          
         CLI   PIDREQD,C'Y'        PPS ON?                                      
         BNE   PRS04               NO - OKAY TO SHOW PASSWORD                   
*                                                                               
         MVI   P.PRTC1,C'L'        COLUMNS FOR NO PASSWORD                      
*&&UK*&& MVI   P.PRTC1N,C'C'       IN SHORT REPORT                              
         MVI   P.PRTC2,C'C'                                                     
         MVI   P.PRTC3,C'C'                                                     
         MVI   P.PRTC4,C'C'                                                     
         MVI   P.PRTC5,C'C'                                                     
...
PRS06    CLI   OPTPWS,C'Y'         PASSWORD?                                    
         BNE   PRS10               NO                                           
         CLI   PIDREQD,C'Y'        PPS ON?                                      
         BE    PRS10               YES - DON'T SHOW PASSWORD                    
         ...
         MVC   PRPAGC,RPTAGC       ACCESS GROUP CODE                            
         MVC   REPM1+67(L'CT@VALS),CT@VALS                                      
...
*                                                                               
         ...
         B     PRS14                                                            
*                                                                               
PRS08    MVC   REPM1+67(L'CT@PRGAC),CT@PRGAC                                    
         MVC   PRPSYS(L'RPTSPG),RPTSPG                                          
         B     PRS14                                                            
*                                                                               
PRS10    MVC   PRTPID,RPTPID       MOVE FIELDS INTO LIST LINE                   
*&&UK*&& MVC   PRTLNM,RPTNAME                                                   
         CLI   EXPFLAG,C'N'        CHECK EXPIRED FLAG                           
         BE    *+8                                                              
         MVI   PRTEXPF,C'*'                                                     
         MVC   PRTDEF1,RPTDEF1                                                  
         MVC   PRTDATE,RPTDATE                                                  
         MVC   PRTAGC,RPTAGC       ACCESS GROUP CODE                            
         MVC   REPM1+70(L'CT@VALS),CT@VALS                                      
         OC    SELPGM,SELPGM                                                    
         BZ    PRS12                                                            
         MVC   REPM1+70(L'CT@PRGAC),CT@PRGAC                                    
...
PRX12    MVC   PRLPIDD(L'CT@PID),CT@PID                                         
         MVC   PRLPID,RPTPID                                                    
         MVC   PRLNAME,RPTNAME                                                  
         MVC   PRLAGCD(L'CT@SECGR),CT@SECGR    ACCESS GROUP CODE                
         MVC   PRLAGC,RPTAGC                                                    
...
PRX14    CLI   OPTPWS,C'Y'         PASSWORD WANTED?                             
         BNE   PRX16               NO                                           
         CLI   PIDREQD,C'Y'        PPS ON?                                      
         BE    PRX16               YES - DON'T SHOW PASSWORD                    
...
***********************************************************************         
*    READ-ONLY USERID - DEFAULT IS NONE                               *         
***********************************************************************         
PRX21    LA    R8,L'REPP1(R8)      NEXT FREE PRINT LINE                         
         MVC   PRLUSRD(13),=CL13'READ-ONLY IDS'                                 
*                                                                               
         BAS   RE,DISRIDS          GET USERID DETAILS INTO BLOCK                
         LA    RE,BLOCK            WHERE DETAILS ARE CURRENTLY                  
         LA    RF,PRLUSR           WHERE THEY ARE TO BE PRINTED                 
         XR    R1,R1                                                            
         ICM   R1,1,BLKCNT         COUNT OF USERIDS                             
...
*                                                                               
PRX21C   LA    R0,PRLUSR+L'PRLUSR-10                                            
         CR    RF,R0               WILL NEXT USERID FIT?                        
         BNH   PRX21E              YES                                          
         BCTR  RF,0                                                             
         MVI   0(RF),C' '          REMOVE COMMA FROM END OF THIS LINE           
         ...
         LA    R0,PRLUSR+L'PRLUSR-10                                            
*                                                                               
PRX21E   MVC   0(10,RF),0(RE)      INSERT USERID INTO NEXT GAP ON LINE          
         LA    RF,9(RF)                                                         
         CLI   0(RF),C' '          SQUASH OUT SPACES                            
         BNE   *+10                                                             
         BCTR  RF,0                                                             
...
*              VALID SYSTEM LIST                                      *         
***********************************************************************         
         LA    R8,L'REPP1(R8)      NEXT FREE PRINT LINE                         
PRX22    CLC   RPTSYS,SPACES       ANY VALID SYSTEMS?                           
         BNH   PRX30               NO                                           
*                                                                               
...
***********************************************************************         
*                                                                               
         XC    CLSTACC,CLSTACC     CLEAR CLIENT LIST LIMIT ACCESS               
         BAS   RE,PRTCLA                                                        
         OC    CLSTACC,CLSTACC     CLIENT LIST LIMIT ACCESS SET                 
         BZ    *+14                                                             
*&&US*&& MVC   PRLLIMD(L'CT@LIDAC),CT@LIDAC                                     
*&&UK*&& MVC   PRLLIMD(L'CT@LIMAC),CT@LIMAC                                     
         B     PRX32                                                            
*                                                                               
...
*&&US*&& MVC   PRLLIMD(L'CT@LIDAC),CT@LIDAC                                     
*&&UK*&& MVC   PRLLIMD(L'CT@LIMAC),CT@LIMAC                                     
         MVC   PRLLIM,APWORK       MOVE IN LIMIT ACCESS VALUES                  
*                                                                               
...
PRNTSYS  NTR1                                                                   
*                                                                               
PPRL01   MVI   BYTE,C'N'           SET NOT DDS TERM AND DDS AGENCY              
         TM    CUSTAT,CUSDDS       TEST DDS TERMINAL                            
         BNO   PPRL01X                                                          
         CLC   AGENCYID,DDSAGYA1   TEST DDS AGENCY                              
         BE    *+14                                                             
         CLC   AGENCYID,DDSAGYA2                                                
         BNE   PPRL01X                                                          
         MVI   BYTE,C'Y'           SET DDS TERM AND DDS AGENCY                  
PPRL01X  EQU   *                                                                
*                                                                               
         ...
         XC    PROGRAM,PROGRAM                                                  
         SPACE 1                                                                
PPRL02   CLI   BYTE,C'Y'           TEST DDS TERMINAL AND DDS AGENCY             
         BE    PPRL02X                                                          
         TM    PGMIND,PGMIACC      IGNORE RESTRICTED ACCESS PROGRAM             
         BO    PPRL12                                                           
         TM    PGMIND,PGMINOP      IGNORE NOP PROGRAM                           
         BO    PPRL12                                                           
         TM    PGMIND4,PGMINAUT    IGNORE NO AUTH REQUIRED PROGRAMS             
         BO    PPRL12                                                           
PPRL02X  EQU   *                                                                
*&&UK                                                                           
...
PPRL20   L     RE,ASYSEL           CHECK AUTH TAKEN FROM DEFAULT                
         USING SASYSD,RE                                                        
         CLC   SASYSALL,NAUTH                                                   
         BE    PPRL60              IF AUTH<>N DISPLAY PROGRAM                   
         B     PPRL22                                                           
*                                                                               
...
PPRL30   MVI   ACCASF,C'N'                                                      
         ...
         B     PPRL50                                                           
*                                                                               
PPRL40   BAS   RE,DISPAVAL                                                      
         MVC   PRTA,APWORK                                                      
*                                                                               
PPRL50   LA    R2,L'PGMNAME+4(R2)                                               
         ZIC   RF,COUNT1                                                        
         LA    RF,1(RF)                                                         
         STC   RF,COUNT1                                                        
         SPACE 1                                                                
PPRL60   LA    R3,L'PGMNAME+L'PGMNUM(R3)                                        
         BCT   R8,PPRL14                                                        
*                                                                               
PPRLX    XIT1                                                                   
         EJECT                                                                  
...
*                                                                               
         ...
         EJECT                                                                  
***********************************************************************         
*  BUILD ID LIST INTO BLOCK (UPDATIVE)                                *         
*  R2 =  RECORD                                                       *         
***********************************************************************         
         SPACE 1                                                                
         USING SA0REC,R2                                                        
...
DISID02  EQU   *                                                                
         ...
         STC   R1,BLKCNT                                                        
DISID03  BAS   RE,NEXTEL           GET NEXT ID                                  
         BE    DISID01                                                          
         DROP  R3                                                               
*                                                                               
DISIDX   B     EXIT                                                             
         EJECT                                                                  
***********************************************************************         
*  BUILD ID LIST INTO BLOCK (READ ONLY)                               *         
*  R2 =  RECORD                                                       *         
***********************************************************************         
         SPACE 1                                                                
         USING SA0REC,R2                                                        
DISRIDS  NTR1                                                                   
         ...
         GOTO1 AGETELS,SA0REC                                                   
         ICM   R3,15,APPARM        WAS EL FOUND                                 
         BZ    DISRIDX             NOT FOUND END                                
         LA    R4,BLOCK                                                         
*                                                                               
DISRID10 EQU   *                                                                
         USING SAIDD,R3                                                         
         TM    SAIDTYP,SAIDNEQ     NEW TYPE?                                    
         BNZ   DISRID50            NO - SKIP IT                                 
         TM    SAIDTYP,SAIDROQ     READ-ONLY USERID?                            
         BNO   DISRID50            NO - SKIP IT                                 
*                                                                               
         MVC   0(10,R4),SPACES                                                  
         LR    RE,R4                                                            
...
*                                                                               
         ...
         LA    R1,1(R1)            BUMP COUNTER                                 
         STC   R1,BLKCNT                                                        
DISRID50 BAS   RE,NEXTEL           GET NEXT ID                                  
         BE    DISRID10                                                         
         DROP  R3                                                               
*                                                                               
DISRIDX  B     EXIT                                                             
...
GASFX    MVC   FVMSGNO,=AL2(FVFOK) IGNORE IO ERROR MESSAGE                      
         ...
         EJECT                                                                  
***********************************************************************         
* ROUTINE TO PRINT CLIENT CODE LIST IN SYSTEM LIMIT ACCESS FIELD                
***********************************************************************         
         USING SA0REC,R2           R2=A(RECORD KEY)                             
PRTCLA   NTR1  ,                                                                
         LA    R4,PRLLIM                                                        
...
DCLIST   DS    0D                  DATA DICT CONSTANT LIST                      
         ...
         DCDDL CT#USRIS,15,L                                                    
         DCDDL CT#PLIST,15,L                                                    
*&&US*&& DCDDL CT#LIDAC,14,L                                                    
*&&UK*&& DCDDL CT#LIMAC,14,L                                                    
         DCDDL CT#PROG,8,L                                                      
         DCDDL CT#VALUE,5,L                                                     
         DCDDL CT#PID,11,L                                                      
...
REPSPEC0 DS    0X                  REPORT HEADINGS SPECIFICATIONS               
         ...
         SPEC  H3,120,PAGE                                                      
         SPEC  H4,100,REPORT                                                    
         SPEC  M1,4,CT#PID,11,L                                                 
         SPEC  M1,18,CT#PSWD,10,L                                               
         SPEC  M1,31,CT#EFFD,8,L                                                
         SPEC  M1,42,CT#LCHNG,12,L                                              
         SPEC  M1,57,CT#SECGR,8,L                                               
         SPEC  END                                                              
         SPACE 2                                                                
REPSPEC1 DS    0X                  REPORT HEADINGS SPECIFICATIONS               
         ...
         SPEC  H3,120,PAGE                                                      
         SPEC  H4,100,REPORT                                                    
         SPEC  M1,3,CT#PID,11,L                                                 
*&&US                                                                           
         SPEC  M1,16,CT#EFFD,8,L                                                
         SPEC  M1,27,CT#LCHNG,12,L                                              
         SPEC  M1,42,CT#SECGR,8,L                                               
*&&                                                                             
*&&UK                                                                           
         SPEC  M1,16,CT#LASTN,15,L                                              
         SPEC  M1,34,CT#EFFD,8,L                                                
         SPEC  M1,45,CT#LCHNG,12,L                                              
         SPEC  M1,60,CT#SECGR,8,L                                               
*&&                                                                             
         SPEC  END                                                              
         SPACE 2                                                                
REPSPEC2 DS    0X                                                               
...
SELAPC   DS    CL(L'SAAPCCOD)      APPROVER GROUP FILTER                        
...
RPTAGC   DS    CL(L'SAAGCCOD)      ACCESS GROUP CODE                            
...
PRTPID   DS    CL8                 PERSONAL ID                                  
         DS    XL3                                                              
*&&UK                                                                           
PRTC1N   DS    XL1                 COL 1 A                                      
         DS    XL1                                                              
PRTLNM   DS    CL15                LAST NAME                                    
         DS    XL1                                                              
*&&                                                                             
PRTC2    DS    XL1                 COL 2                                        
...
PRTC4    DS    CL1                 COL 4                                        
         DS    CL1                                                              
PRTAGC   DS    CL8                 SECURITY GROUP CODE                          
         DS    CL1                                                              
PRTC5    DS    CL1                 COL 5                                        
         DS    CL1                                                              
...
PRPC5    DS    CL1                 COL 5                                        
         DS    CL1                                                              
PRPAGC   DS    CL8                 SECURITY GROUP CODE                          
         DS    CL1                                                              
PRPC6    DS    CL1                 COL 6                                        
         DS    CL1                                                              
...
PRLNAME  DS    CL60                PERSON NAME                                  
         DS    CL3                                                              
PRLAGCD  DS    CL7                 ACCESS GROUP CODE DESCRIPTOR                 
         DS    CL3                                                              
PRLAGC   DS    CL8                 ACCESS GROUP CODE                            
         ORG   PRLPIDD                                                          
...
PRLDATE  DS    CL8                 EFFECTIVE DATE                               
         ORG   PRLPIDD                                                          
PRLUSRD  DS    CL20                USER ID DESCRIPTOR (UPDATIVE)                
PRLUSR   DS    CL109               USER ID LIST (UPDATIVE)                      
         ORG   PRLPIDD                                                          
PRLURRD  DS    CL20                USER ID DESCRIPTOR (READ ONLY)               
PRLURR   DS    CL109               USER ID LIST (READ ONLY)                     
         ORG   PRLPIDD                                                          
...
PRTBLEN  EQU   *-PRTBLKD                                                        
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'004SEACS15   10/26/15'                                      
         END                                                                    
