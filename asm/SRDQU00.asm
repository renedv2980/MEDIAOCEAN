*          DATA SET SRDQU00    AT LEVEL 003 AS OF 04/02/19                      
*PHASE T15900A                                                                  
*INCLUDE SIXPACK                                                                
...
*                                                                               
         ...
         BNH   M10                                                              
         LLC   R1,PFKEY                                                         
         AHI   R1,-12              EQUATE HIGH PF KEYS TO LOWER VALUES          
         STC   R1,PFKEY                                                         
*                                                                               
M10      CLI   PFKEY,PFHELP        CHECK HELP PFKEY PRESSED                     
         ...
         CLI   PFKEY,PFPQR                                                      
         BNE   M11                                                              
         L     RF,=A(PQRET)        SET UP TO RETURN VIA PF4                     
         A     RF,RELO                                                          
         BASR  RE,RF                                                            
         B     EXIT                                                             
*                                                                               
M11      L     RF,=A(READSTR)      READ S/R SAVE STORAGE PAGE INTO TWA          
         A     RF,RELO                                                          
         BASR  RE,RF                                                            
         ...
         MVC   REPPIN,OREPPIN      SET CURRENT PIN                              
*                                                                               
M12      BAS   RE,VALIDATE                                                      
         BAS   RE,PROCESS                                                       
         BAS   RE,DISPLAY                                                       
...
VALIDATE NTR1                                                                   
         ...
         TM    TSTAT6,TST6STRO                                                  
         BZ    *+8                                                              
         OI    DDS,X'40'           SET FRONT ENDED WITH PC APPLICATION          
*                                                                               
...
VX       B     DQXIT                                                            
         EJECT                                                                  
*************************************************************                   
* VALIDATE SPOOL ID PIN=1234,U=XXXXXXXX,ABC,12345           *                   
*************************************************************                   
VALSPID  NTR1                                                                   
VSP1     LA    R2,DQUSPIDH         R2=A(REPORT ID FIELD HEADER)                 
         XC    REPIFLDS(4),REPIFLDS                                             
         CLI   5(R2),0                                                          
         BNE   VSP1A                                                            
         ...
         STC   RF,5(R2)                                                         
*                                                                               
VSP1A    CLI   8(R2),C'?'          ? FOR HELP                                   
         BNE   VSP1B                                                            
         MVI   HELPNUM,1                                                        
         ST    R2,QHDR                                                          
         B     HELPOUT                                                          
*                                                                               
VSP1B    GOTO1 CSCANNER,DMCB,(R2),(4,SCANOUT)                                   
         LA    R3,SCANOUT                                                       
         MVC   REPIFLDS,4(R1)                                                   
         CLI   REPIFLDS,0                                                       
         BE    ERR2                                                             
*                                                                               
VSP2     CLI   1(R3),0             TEST KEYWORD=VALUE FIELD                     
         BE    VSP2X                                                            
         CLC   12(8,R3),DC@PIN     FIRST MAY BE PIN=                            
         BE    VSP2A                                                            
         CLC   12(2,R3),=C'N '     OR N=                                        
         BE    VSP2A                                                            
         CLC   12(2,R3),=C'P '     OR P= FOR SCAMBLED PIN                       
         BNE   VSP2X                                                            
         TM    DDS,X'40'           MUST BE FRONT ENDED BY PC APP                
         BZ    VSP2A                                                            
         CLI   1(R3),4                                                          
         BNE   VSP2A                                                            
         MVC   PININ,22(R3)        UNSCRAMBLE PIN                               
         MVI   PINACT,2                                                         
         L     RF,=A(PINMIX)                                                    
         A     RF,RELO                                                          
         BASR  RE,RF                                                            
         MVC   22(4,R3),PINOUT                                                  
VSP2A    CLI   1(R3),1             PIN IS 1 OR 4 CHRS                           
         BL    ERR2                                                             
         BH    VSP2D                                                            
VSP2B    CLI   22(R3),C'0'         PIN=0 TO CLEAR THE PIN                       
         BNE   *+14                                                             
         XC    REPPIN,REPPIN                                                    
         B     VSP2E                                                            
         CLI   22(R3),C'*'         PIN=* TO USED SAVED PIN                      
         BNE   ERR2                                                             
         OI    REPIFLAG,X'81'      SET PIN=* INPUT FLAG                         
         B     VSP2W                                                            
VSP2D    CLI   1(R3),4             PIN=XXXX                                     
         BNE   ERR2                                                             
         CLC   22(4,R3),=C'0000'   PIN=0000 TO CLEAR THE PIN                    
         BNE   *+14                                                             
         XC    REPPIN,REPPIN                                                    
         B     VSP2E                                                            
         CLC   22(4,R3),=C'****'   PIN=**** TO USED SAVED PIN                   
         BNE   *+12                                                             
         OI    REPIFLAG,X'84'      SET PIN=**** INPUT FLAG                      
         B     VSP2W                                                            
         MVC   REPPIN,22(R3)                                                    
         TM    DDS,X'01'           IF DDS TERMINAL                              
         BNO   VSP2E                                                            
         CLC   REPPIN,DDSPIN       DDSPIN MATCHES ALL                           
         BNE   VSP2E                                                            
         MVC   REPPIN,FFS                                                       
VSP2E    LA    RE,8(R2)            FIND LENGTH OF PIN FIELD                     
         SR    R0,R0                                                            
         IC    R0,5(R2)                                                         
VSP2F    CLI   0(RE),C','                                                       
         BE    VSP2G                                                            
         ...
         BCT   R0,VSP2F                                                         
         B     ERR2                                                             
VSP2G    LA    RE,1(RE)            POINT TO REST OF REPID                       
         MVC   WORK1,0(RE)         SAVE IT FOR LATER IN WORK1                   
         SR    RE,R2                                                            
         AHI   RE,-8               RE=LEN OF INPUT PIN FIELD                    
         STC   RE,REPIPINL                                                      
VSP2W    LA    R3,32(R3)                                                        
         SR    R0,R0               DECR NUMBER OF FIELDS                        
         IC    R0,REPIFLDS                                                      
         AHI   R0,-1                                                            
         STC   R0,REPIFLDS                                                      
         BZ    ERR2                                                             
VSP2X    EQU   *                                                                
*                                                                               
VSP3     CLI   1(R3),0             TEST KEYWORD=VALUE FIELD                     
         BE    VSP8                                                             
         CLC   12(2,R3),=C'U '     U=USERID                                     
         BNE   ERR2                                                             
         TM    DDS,X'03'           MUST BE DDS/PRIVILEGED USER                  
         BZ    ERR2                                                             
         MVC   REPCUSR,22(R3)                                                   
...
VSP5     TM    DDS,X'01'           DDS USER CAN INPUT ANY VALID ID              
         ...
         CLI   8(R1),X'00'                                                      
         BNE   ERR0                                                             
*&&UK*&& GOTO1 CCALLOV,DMCB,0,X'D9000AF9',0                                     
*&&US*&& GOTO1 CCALLOV,DMCB,0,X'D9000AFA',0                                     
         CLI   4(R1),X'FF'                                                      
         JE    *+2                                                              
...
VSP7     LA    R3,32(R3)                                                        
         ...
         BZ    ERR2                                                             
*                                                                               
VSP8     CLI   REPIFLDS,2          MUST BE LEFT WITH SUBID,SEQNUM               
         BNE   ERR2                                                             
         CLI   0(R3),3             FIRST IS SUBID                               
         BH    ERR2                                                             
         CLI   0(R3),2                                                          
         BL    ERR2                                                             
         MVC   REPSUBID,12(R3)                                                  
*                                                                               
         ...
         LA    R3,32(R3)                                                        
*                                                                               
         CLI   1(R3),0             SECOND IS REPORT SEQUENCE NUM                
         BNE   ERR2                                                             
         TM    2(R3),X'80'                                                      
         BZ    ERR2                                                             
         OC    4(4,R3),4(R3)                                                    
...
*                                                                               
VSP30    CLI   REPIPINL,0          TEST IF PIN=DATA INPUT                       
         BE    VSPX                                                             
         TM    REPIFLAG,X'80'      TEST IF PIN=**** INPUT                       
         BO    VSPX                                                             
VSP30A   SR    R1,R1               CLEAR INPUT FIELD                            
         IC    R1,5(R2)                                                         
         BCTR  R1,0                                                             
         EX    R1,*+8                                                           
         B     *+10                                                             
         XC    8(0,R2),8(R2)                                                    
VSP30B   SR    RE,RE               RE=OLD LEN OF PIN FIELD                      
         IC    RE,REPIPINL                                                      
         SR    R0,R0               R0=NEW LEN OF PIN FIELD                      
         OC    REPPIN,REPPIN                                                    
         BZ    VSP30D                                                           
         LA    R0,7                                                             
         MVC   8(7,R2),=C'N=****,'                                              
VSP30C   TM    DDS,X'40'           DISPLAY SCRAMBLED PIN IF PC APP              
         BZ    VSP30D                                                           
         MVI   8(R2),C'P'                                                       
         MVC   10(4,R2),REPPIN                                                  
         CLC   REPPIN,FFS                                                       
...
VSP30D   SR    RF,RF                                                            
         IC    RF,5(R2)                                                         
         SR    RF,RE               RF=LEN OF REPID FIELD                        
         BZ    VSP30E                                                           
         LA    RE,8(R2)                                                         
         AR    RE,R0                                                            
         BCTR  RF,0                                                             
         EX    RF,*+8                                                           
         B     *+10                                                             
         MVC   0(0,RE),WORK1       REPID WAS SAVED IN WORK1                     
         LA    RF,1(RF)                                                         
         AR    RF,R0                                                            
VSP30E   STC   RF,5(R2)            SET NEW FIELD LEN AND TRANSMIT               
         OI    6(R2),X'80'                                                      
*                                                                               
VSPX     B     DQXIT                                                            
         EJECT                                                                  
...
VALREP   NTR1                      INITIALIZE BUFFER                            
         ...
         CLI   8(R1),0                                                          
         JNE   *+2                 DIE ON INIT BUFFER DISK ERROR                
*                                                                               
         LA    R5,KEY              SEARCH PRTQUE INDEX FOR REPORT               
         USING UKRECD,R5                                                        
         XC    UKINDEX,UKINDEX                                                  
         MVI   UKFLAG,UKFLNUM      SET TO LOCATE DIRECLY VIA UKREPNO            
         MVC   UKSRCID,REPUSER                                                  
...
*                                                                               
         ...
         MVC   ATTRIBS,QLATTB                                                   
*                                                                               
VR02     TM    QLATTB,QLATPW       TEST PASSWORD PROTECT                        
         BNO   VR05                NO                                           
         MVC   GSECVAL,QLSECINF    YES EXTRACT SECURITY INFO                    
         L     RF,=A(SECVAL)                                                    
         A     RF,RELO                                                          
         BASR  RE,RF                                                            
         BNE   ERRSEC              SECURITY ERROR                               
*                                                                               
...
*                                                                               
VR20     EQU   *                                                                
VR30     SR    RF,RF               STORE NUMBER OF FINAL PAGE                   
         ICM   RF,3,QLPAGES                                                     
         ST    RF,LASTPAGE                                                      
VRX      B     DQXIT                                                            
         EJECT                                                                  
...
VP11     TM    4(R2),X'08'         CHECK FOR VALID NUMERIC INPUT                
         BZ    ERR2                                                             
         LLC   R1,5(R2)            EXTRACT NUMBER FROM FIELD                    
         BCTR  R1,0                                                             
         EX    R1,*+8                                                           
         B     *+10                                                             
         PACK  DUB,8(0,R2)                                                      
...
DS3      BAS   RE,DISPHDS                                                       
*                                                                               
         LA    RF,19               SET LINES TO SCROLL FOR NEXT SCREEN          
         LLC   RE,PROFILE+(PROFNHDS-PROFD)                                      
         SR    RF,RE                                                            
         ST    RF,SCROLL                                                        
         EDIT  (RF),(4,DQUSCR),ALIGN=LEFT                                       
*                                                                               
         LA    RF,PROFILE          ENSURE (PROFHDST+PROFNHDS),=SLINE            
         USING PROFD,RF                                                         
         LLC   RE,PROFHDST                                                      
         LLC   R0,PROFNHDS                                                      
         DROP  RF                                                               
...
*                                                                               
         ...
         LLC   RE,PROFILE+(PROFHDST-PROFD)  WHERE 'FLOATING' HEADS END          
         CR    RF,RE                                                            
         BNL   DS20                                                             
...
DS30     A     R3,SCOL             R3 POINTS TO CORRECT COLUMN                  
         ...
         BNL   *+6                                                              
         LR    R3,RF               DONT ALLOW IT TO HAPPEN                      
...
*                                                                               
         ...
         DROP  R5                                                               
*                                                                               
DS50     LLC   R0,0(R2)            BUMP R2 TO NEXT FIELD                        
         AR    R2,R0                                                            
...
DISPTOP  NTR1                                                                   
         ...
         OI    DQUOPTH+6,X'80'                                                  
         OI    DQUSCRH+6,X'80'                                                  
         EDIT  SPAGE,(5,DQUPAGE),ALIGN=LEFT                                     
...
*************************************************************                   
DISPHELP NTR1                                                                   
*NOP*    MVC   DQUMSG(27),=C'PRINT QUEUE DISPLAY PROGRAM'                       
*NOP*    OI    DQUMSGH+6,X'80'                                                  
...
DHLOOP   MVC   8(79,R2),0(R3)      MOVE LINE IN                                 
         ...
         LLC   R0,0(R2)            BUMP TO NEXT DISPLAY LINE                    
         AR    R2,R0                                                            
...
*                                                                               
NL20     L     RF,AOUTALLC         TRANSLATE TO SCREEN DISPLAYABLE              
...
NL25     MVI   PAGEWAIT,1                                                       
*                                                                               
NL30     CLC   PLINE+1(L'PLINE-1),SPACES SKIP BLANK LINES                       
         BE    NL5                                                              
*                                                                               
NL40     CLI   PLINE+1,C'<'        TEST IF POSSIBLE XML COMMAND                 
         BNE   NL50                                                             
         LA    RF,PLINE+1                                                       
         LA    R0,L'PLINE-1                                                     
NL42     CLI   0(RF),C'>'                                                       
         ...
         BCT   R0,NL42                                                          
         B     NL50                                                             
NL44     TM    SCRFLAG,SHOWXML     SKIP XML LINES UNLESS REQUESTED              
         BZ    NL5                                                              
*                                                                               
NL50     AF    QLINE,=F'1'         SET NEXT LINE                                
         B     NLX                                                              
*                                                                               
...
* EXPECTS: R2=DISPLAY LINE, R3=PLINE                        *                   
*************************************************************                   
VARCOLS  NTR1                                                                   
         LA    R4,PROFILE                                                       
         USING PROFD,R4                                                         
         LLC   RF,PROFSTCL         GET KEEP START COL                           
         LLC   RE,PROFEDCL         GET KEEP END COL                             
         DROP  R4                                                               
...
*                                                                               
VCL20    DS    0H                                                               
         LLC   R5,0(RF)            R5=FLOAT END COL                             
         CHI   R0,4                FIRST CHUNK?                                 
         BNE   VCL30                                                            
         LLC   R1,PROFILE+(PROFEDCL-PROFD)   R1=PROFEDCL                        
         B     VCL40                                                            
...
*                                                                               
         ...
         B     VCL110                                                           
*                                                                               
VCL100   LLC   R5,PROFILE+(PROFEDCL-PROFD)   R5=PROFEDCL                        
         C     R5,SCOL                                                          
         BL    *+14                                                             
...
* EXPECTS: R1=LINE COUNTER, R2=FIRST DISPLAY FIELD                    *         
* RETURNS: R1=UPDATED LINE COUNTER, R2=NEX AVAIL DISPLAY FIELD        *         
***********************************************************************         
DISPHDS  NTR1                                                                   
         LA    R4,PROFILE                                                       
         USING PROFD,R4                                                         
         ...
         BAS   RE,FIRSTLIN                                                      
*                                                                               
         LLC   R5,PROFNHDS         GET NUMBER OF HEADLINES                      
         SR    R0,R0                                                            
DH10     DS    0H                                                               
         LA    R3,PLINE                                                         
         TM    SCRFLAG,KEEPSDS                                                  
...
ERR6     LA    RE,SREICN           INVALID COLUMN NUMBER                        
         B     ERRX                                                             
ERR7     LA    RE,80               REPORT CONTAINS SECURE INFORMATION           
         B     ERRX                                                             
...
DDSPIN   DC    CL6'DDPQ  '         WAS DDSPQ - MAX IS NOW 4 CHRS                
...
VO30     CLI   5(R2),1                                                          
         ...
         B     VOX                                                              
*                                                                               
VO40     CLC   8(2,R2),=C'NF'      OVERRIDE SCROLL PROFILE?                     
         BNE   VO45                                                             
         OI    SCRFLAG,SCROVR                                                   
         B     VOX                                                              
VO45     CLC   8(2,R2),=C'UF'      USE SCROLL PROFILE?                          
         BNE   VO50                                                             
         NI    SCRFLAG,X'FF'-SCROVR                                             
         B     VOX                                                              
*                                                                               
VO50     CLC   8(3,R2),=C'XML'     SHOW XML TAGS?                               
         BNE   VO55                                                             
         OI    SCRFLAG,SHOWXML                                                  
         B     VOX                                                              
VO55     CLC   8(5,R2),=C'NOXML'                                                
         BNE   VO60                                                             
         NI    SCRFLAG,X'FF'-SHOWXML                                            
         B     VOX                                                              
*                                                                               
...
*************************************************************                   
SECVAL   NTR1  BASE=*,LABEL=*                                                   
         MVI   GSECRES,0           SET OK RETURN CODE                           
         MVC   FULL,REPPIN         FULL HAS THE INPUT PASSWORD TEXT             
         CLC   FULL,FFS                                                         
         BE    SECVALX             DDS SPECIAL PQ PASSWORD                      
*                                                                               
SECVAL1  TM    GSECF1,QLSINONO     TEST IF VALID SECURITY FLAG                  
         BO    SECVALX             YES                                          
*                                                                               
SECVAL2  TM    GSECF1,QLSIPIN      TEST IF PIN ATTACHED TO REPORT               
         BZ    SECVAL3                                                          
         OC    FULL,FULL           TEST IF PIN IS DEFINED                       
         BNZ   *+12                                                             
         MVI   GSECRES,197         PIN REQUIRED FOR THIS REPORT                 
         B     SECVALX                                                          
         CLC   GSECPSWD,FULL       TEST PIN WITH INPUT                          
         BE    SECVALX                                                          
         MVI   GSECRES,198         PIN INCORRECT FOR THIS REPORT                
         B     SECVALX                                                          
*                                                                               
SECVAL3  TM    GSECF1,QLSIPID      TEST IF PID ATTACHED TO REPORT               
         BZ    SECVAL4                                                          
         CLC   GSECPSWD+0(2),TAGYSEC                                            
         BNE   *+14                                                             
         CLC   GSECPSWD+2(2),TPERSON                                            
         BE    SECVALX                                                          
...
* PROCESS PF4 REQUEST                                       *                   
*************************************************************                   
PQRET    NTR1  BASE=*,LABEL=*                                                   
         L     R1,TBUFF            LOCATE TERMINAL BUFFER                       
...
SAVERE   DS    F                   VALUE OF RD AFTER FIRST NMOD1                
RELO     DS    F                                                                
MSG      DS    CL60                MESSAGE TO GO INTO MESSAGE FIELD             
*                                                                               
GSECVAL  DS    0CL6                WORK AREA FOR SECVAL                         
GSECPSWD DS    CL4                                                              
GSECF1   DS    X                                                                
GSECF2   DS    X                                                                
         DS    CL2                                                              
GSECRES  DS    XL1                                                              
*                                                                               
PINMIXD  DS    0CL10               WORK AREA FOR PINMIX                         
PININ    DS    CL4                 INPUT PIN                                    
PINOUT   DS    CL4                 OUTPUT PIN                                   
PINACT   DS    X                   X'01'=SCRAMBLE,X'02'=UNSCRAMBLE              
PINFLG   DS    X                   N/D                                          
*                                                                               
...
SHOWXML  EQU   X'02'               SHOW XML COMMANDS                            
*                                                                               
REPPIN   DS    CL4                 INPUT PIN                                    
*                                                                               
...
*                                                                               
REPIFLDS DS    XL1                 NUMBER OF FIELDS IN REPID                    
REPIPINL DS    XL1                 LEN OF PIN DATA                              
REPIRIDL DS    XL1                 LEN OF RID DATA                              
REPIFLAG DS    XL1                 FLAGS FOR REPID FIELD                        
*                                                                               
...
* CTGENFILE                                                                     
       ...
       ++INCLUDE CTGENFILE                                                      
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'003SRDQU00   04/02/19'                                      
         END                                                                    
