*          DATA SET NEWRI85A   AT LEVEL 006 AS OF 05/01/02                      
*          DATA SET NEWRI85    AT LEVEL 023 AS OF 10/21/98                      
*PHASE T32085A ,+0                                                              
*INCLUDE PRINT                                                                  
*INCLUDE PRNTBL                                                                 
*                                                                               
...
NET10    CLI   RERUNTST,C'Y'       ARE WE DOING A RERUN?                        
         BNE   NET13                                                            
         B     NET13                                                            
* IN THIS CASE COPY/CHG DOES NOT HAVE HISTORY REC                               
* START WITH CHANGE REC AND TREAT IT AS AN ADD                                  
*                                                                               
         CLC   RECVHDR(12),=X'2A02B68B00004F230031823C'                         
         BNE   GET                 NO MATCH GET                                 
         CLC   RVCHR(12),=X'1B281E002B1165060BE022F9'                           
         BNE   GET                 NO MATCH GET                                 
         CLC   0(20,R6),=X'04E3998FCA5920C6C1D440E2D5D7F0F0F10802C3'            
         BNE   GET                 NO MATCH GET                                 
         MVI   RERUNTST,0          YES, SO HANDLE FROM NOW                      
         L     R6,AUNITREC         AND TREAT THIS AS AN ADD                     
         USING NURECD,R6                                                        
         MVC   RECDAT,RDATE        SAVE RECOVERY DATE                           
         MVC   RECTIME,RTIME       SAVE RECOVERY TIME                           
         MVC   RECFILTY,RECVHDR                                                 
         B     NET30B                                                           
*  NORMALLY DROPPPED IT DOWN                                                    
         B     NET13                                                            
...
NET15    DS    0H                                                               
         ...
         B     GET                    ON AUDIT BIT AND SKIP THIS REC            
...
NET20A   CLI   HISTFLG,C'Y'        IS IT ALREADY IN HIST STREAM?                
         ...
         TM    NUPACKST,X'02'               IS HIST FLAG TURNED ON?             
         BNO   GET                          NO -SKIP IT                         
...
NET20D   BAS   RE,PROCDEL                YES/PROCESS DELETED UNIT REC           
         ...
         MVI   PREVREC,0           KEY CHANGED/NEW REC ADDED IN PROCDEL         
         B     GET                                                              
...
**       DC    H'0'                                                             
**       USING NUACTD,R6                                                        
**       GOTO1 DATCON,DMCB,(3,NUACTADT),WORK                                    
**       L     R6,NBAIO                                                         
**       USING NURECD,R6                                                        
**       GOTO1 GETDAY,DMCB,WORK,WORK+10                                         
**       CLC   =C'SAT',WORK+10                                                  
**       BE    NET30B              IF MISSING HIST ADDED ON SAT                 
****     DC    H'0'                OK/ELSE TAKE A HIT                           
*                                                                               
         L     R6,NBAIO                                                         
         USING NURECD,R6                                                        
...
*                                                                               
NET60    DS    0H                                                               
**       CLC   =X'B31C',NBACTDAT   *** TEST                                     
**       BNE   GET                                                              
*                                                                               
         BAS   RE,ACTIVDAT          SET ACTIVITY DATE                           
         BAS   RE,MYADD             ADD RECORD                                  
*                                                                               
         B     GET                                                              
...
RDHIST   NTR1                                                                   
         ...
         MVI   UPDTBIT,X'80'        READ FOR UPDATE                             
         BAS   RE,MYGET                                                         
         XIT1                                                                   
         DROP  R4,R5                                                            
*                                                                               
...
*                                                                               
       ...
       ++INCLUDE DDGENTWA                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'006NEWRI85A  05/01/02'                                      
         END                                                                    
