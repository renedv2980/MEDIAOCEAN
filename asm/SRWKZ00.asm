*          DATA SET SRWKZ00    AT LEVEL 005 AS OF 03/04/16                      
*PHASE T13B00A                                                                  
*INCLUDE SQUASHER                                                               
*INCLUDE TIMBER                                                                 
*INCLUDE WKZSCAN                                                                
*INCLUDE NUMVAL                                                                 
*INCLUDE DECODE                                                                 
         TITLE '$WKZ - ROOT CONTROLLER'                                         
         PRINT NOGEN                                                            
WRKZ000  CSECT                                                                  
         NMODL WKZWKX-WKZWKD,**$WK0**,RA,R9,RR=R4,CLEAR=YES                     
         USING WKZWKD,RC                                                        
         ST    RB,ABASE                                                         
...
*                                                                               
         ...
         L     RF,ACOMFACS         SAVE COMFACS ROUTINES                        
         USING COMFACSD,RF                                                      
         ...
         DROP  RF                                                               
*                                                                               
         MVC   AUTL1,VUTL          SAVE SYSFACS ADDRESSES                       
         MVC   ASSB,VSSB                                                        
*                                                                               
         ...
         MVC   AGENIDS,0(R1)                                                    
*                                                                               
         LA    RF,GETUSER          COMMON SUBROUTINES                           
         ST    RF,AGETUSER                                                      
         LA    RF,LOADSCR                                                       
         ST    RF,ALOADSCR                                                      
         LA    RF,SELTABL                                                       
         ...
         ST    RF,AWKUPDT                                                       
*                                                                               
         L     RF,=A(CTREC-WKZWKD) SAVE OUT OF RANGE WORK                       
         LA    RF,WKZWKD(RF)                                                    
         ST    RF,ACTREC                                                        
         L     RF,=A(CXREC-WKZWKD)                                              
         LA    RF,WKZWKD(RF)                                                    
...
*                                                                               
         ...
         B     WRKZ150                                                          
*                                                                               
WRKZ050  TM    TSTAT1,X'60'        TEST DDS TERMINAL                            
...
*----------------------------------------------------------------------         
*        CREATE LIST OF AVAILABLE WRKZ FILES                                    
*----------------------------------------------------------------------         
WRKZ150  LA    RF,L'W_INDEX        SAVE WRKZL DATA                              
         STH   RF,CINDXLN                                                       
         L     RF,VENQDEQ                                                       
         ST    RF,CIENQDEQ                                                      
*                                                                               
         L     R5,ACIREC                                                        
         GOTO1 VDATAMGR,WKDMCB,(X'00',GLIST),WRKZID,NDX,ACTREC,(R5)             
         ICM   RE,15,NXUSRINF                                                   
...
*----------------------------------------------------------------------         
WRKZ170  LA    R4,SRVIDH           SET R4 FOR ERR EXIT                          
         OC    USERID,USERID                                                    
         BNZ   WRKZ175             GET WKFILE FOR USERID                        
         TM    DDS,DDSTRM                                                       
         BZ    ERR0                NON DDS TRMS MUST BE CONNECTED               
*                                                                               
         MVC   WRKZCHR,WRKZNTRY+1  DEFAULT TO FIRST WRKZ FILE                   
         B     WRKZ176                                                          
*                                                                               
WRKZ175  MVC   NXSRCID,USERID      GET WRKZ ID FROM USER ID                     
         GOTO1 VDATAMGR,WKDMCB,(X'00',GFILE)                                    
         MVC   WRKZID,NXUSRINF                                                  
         MVC   WKPASS,WRKZCHR                                                   
*                                                                               
WRKZ176  BAS   RE,SETBUFF          INIT BUFFER                                  
                                                                                
*----------------------------------------------------------------------         
*        SET UP DATES (TODAYS)                                                  
*----------------------------------------------------------------------         
WRKZ180  GOTO1 ADATCON,DMCB,(5,0),(0,DATE)                                      
         GOTO1 (RF),(R1),,(1,DATE1)                                             
         GOTO1 (RF),(R1),,(2,DATE2)                                             
...
*----------------------------------------------------------------------         
WRKZ190  L     R5,ATIA             READ AND LOCK TWA11 INTO TIA                 
         USING SRSD,R5                                                          
         LA    R4,SRPAGENO                                                      
         SLL   R4,32-8                                                          
         ICM   R4,3,TRM                                                         
...
*                                                                               
         ...
         DROP  RF                                                               
*                                                                               
EXIT1A   CLI   LOCKT,0             UNLOCK WRKZ                                  
         BE    EXIT2                                                            
         MVC   CFWKID,=C'WRKZX'                                                 
         L     R1,ATCB             USE TCB TO GET WRKZ CHR                      
         CLI   TCBWFCHR-TCBD(R1),0                                              
         BE    EXIT2                                                            
         MVC   CFWKID+4(1),TCBWFCHR-TCBD(R1)                                    
*                                                                               
         GOTO1 CIENQDEQ,DMCB,(C'D',CFWKID)                                      
         MVI   LOCKT,0                                                          
*                                                                               
EXIT2    L     R1,ATCB             DOUBLE CHECK WF UNLOCKED                     
         TM    TCBFLAG2-TCBD(R1),TCBENQWF                                       
         BNO   EXITX                                                            
         DC    H'0'                                                             
*                                                                               
EXITX    XMOD1                                                                  
...
GETFILT  NTR1                                                                   
         ...
         DC    H'0'                                                             
*                                                                               
         MVC   CIADDR(L'CISADR),CISADR                                          
         MVI   CIADDR+3,1                                                       
*                                                                               
         GOTO1 VDATAMGR,DMCB,(X'00',BUFFER),WRKZID,NDX,ACTREC,(R6)              
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'0'                                                             
*                                                                               
...
P1VAL    NTR1                                                                   
         ...
         BNO   P1V1                                                             
*                                                                               
P1VPQ    CLI   1(R6),0             FIRST PARM CAN BE Q=WRKZID                   
         BE    P1V1                                                             
         CLI   12(R6),C'Q'                                                      
         BNE   ERR2                                                             
         CLI   1(R6),1             WRKZID IS 1 CHR FIELD                        
         BNE   ERR2                                                             
         CLI   22(R6),C'U'         Q=U MEANS THIS USERS QUEUE                   
         BE    P1V015                                                           
         LA    RE,WRKZLST+8        MATCH CHR AGAINST WRKZ LIST                  
P1V010   CLI   0(RE),0                                                          
         BE    ERR10                                                            
         CLC   22(1,R6),1(RE)                                                   
         BE    P1V015                                                           
...
P2V030   TM    2(R6),X'80'         TEST NUMERIC                                 
         BZ    P2V040                                                           
         MVC   WKFILEN,4(R6)       EXTRACT FILENO                               
*                                                                               
...
*************************************************************                   
*        INIT BUFFER FOR WRKZID (ALSO SAVE WRKZID)          *                   
*************************************************************                   
SETBUFF  ST    RE,SAVERE                                                        
         MVC   WRKZIDSV,WRKZID     SAVE FILEID & INIT BUFFER                    
         L     R5,ACIREC                                                        
         GOTO1 VDATAMGR,WKDMCB,(X'00',BUFFER),WRKZID                            
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R5,ACIREC                                                        
         MVC   BUFFDATA,0(R5)         SAVE V(W_TAB)/V(W_FILE)                   
         MVC   CIDATA,12(R5)          SAVE FILE DATA FOR THIS WRKZ              
         L     RE,SAVERE                                                        
         BR    RE                  RETURN                                       
         EJECT                                                                  
...
WKUPDT   NTR1  BASE=ABASE                                                       
         LM    R9,RB,ABASES                                                     
         L     R8,ASYSFAC                                                       
         GOTO1 ADATAMGR,WKDMCB,(LOCKT,STAACT),WRKZID                            
         CLI   8(R1),0                                                          
         BE    WKUPDTX             OK EXIT                                      
         BAS   RE,W_UNLK                                                        
...
*************************************************************                   
*        LOCK WRKZ IF NOT LOCKED ALREADY                    *                   
*************************************************************                   
WKLOCK   NTR1  BASE=ABASE                                                       
         LM    R9,RB,ABASES                                                     
         L     R8,ASYSFAC                                                       
         TM    LOCKT,X'40'         TEST ALREADY LOCKED                          
         BO    WKLOCKX                                                          
*                                                                               
         BAS   RE,W_LOCK           ENQ WRKZ                                     
         TM    4(R1),X'40'                                                      
         BNO   *+6                                                              
         DC    H'0'                                                             
*                                                                               
...
*************************************************************                   
         SPACE 1                                                                
SETUSR   NTR1                                                                   
         CLI   WKQUEUE,0           TEST IF SPECIFIC WRKZ NAMED BY Q=N           
         BE    SETUSR2             NO                                           
         CLI   WKQUEUE,C'U'                                                     
         BE    SETUSR2                                                          
         MVC   WRKZID,=CL8'WRKZ'   SET SPECIFIC QUEUE NAME                      
         MVC   WRKZCHR,WKQUEUE                                                  
...
*                                                                               
SETUSR3  MVC   NXSRCID,USERID      GET WRKZ ID FROM U=... VALUE                 
         L     R5,ACIREC                                                        
         GOTO1 VDATAMGR,WKDMCB,(X'00',GFILE),WRKZL                              
         MVC   WRKZID,NXUSRINF                                                  
         MVC   WKPASS,WRKZCHR                                                   
*                                                                               
...
*                                                                               
       ++INCLUDE DMWRKZR           WORKER ROUTINES                              
         ...
         EJECT                                                                  
*                                                                               
       ++INCLUDE SRWKZDD           DICTIONARY REFS                              
         DS    0H                                                               
         EJECT                                                                  
       ++INCLUDE SRWKZWK                                                        
         EJECT                                                                  
...
*DDGLOBEQUS                                                                     
       ...
       ++INCLUDE DDGLOBEQUS                                                     
         PRINT ON                                                               
**NOP  ++INCLUDE SRWKZTWAB                                                      
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'005SRWKZ00   03/04/16'                                      
         END                                                                    
