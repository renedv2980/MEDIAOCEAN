*          DATA SET RESFM3DA   AT LEVEL 090 AS OF 05/01/02                      
*PHASE T8183DA,*                                                                
*INCLUDE PRINT                                                                  
*INCLUDE PRNTBL                                                                 
*INCLUDE LOADER                                                                 
         TITLE 'T8183D - RESFM3D - KATZ WEEKLY STATION ACTV REPORT'             
*                                                                               
**********************************************************************          
*                                                                    *          
*        RESFM3D (T8183D) --- KATZ WEEKLY STATION ACTIVITY REPORT    *          
*                                                                    *          
* 17JAN00 (MLB) SENDING REPORTS BY EMAIL                             *          
*                                                                    *          
*                                                                    *          
*                                                                    *          
* ------------------------------------------------------------------ *          
...
T8183D   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,**183D**,RA,R7   ** NOTE BASE REGISTERS **                     
         L     RC,0(R1)                                                         
         USING GEND,RC                                                          
         L     R9,ASYSD                                                         
         USING SYSD,R9                                                          
...
RDR22    DS    0H                                                               
         OI    DMINBTS,X'08'     PASS DELETES - X'AC' KEYS NOT DELETED          
         GOTO1 GETREC                                                           
         L     R6,AIO                                                           
         USING RCONREC,R6                                                       
         TM    RCONCNTL,X'80'      IF DELETED                                   
         BO    NXTREC              SKIP                                         
         NI    DMINBTS,X'FF'-X'08' OK/TURN OF DELETE                            
*************************************************************                   
* CONTRACT MUST BE GRAPHNET AND                                                 
* CONTRACT  MUST BE LAST SENT BY REP WITHIN REQ DATES OR                        
* CONTRACT  MUST BE CONFIRMED WITHIN REQ DATES OR                               
* CONTRACT  MUST HAVE BUY LINES AND LAST SENT BY REP (ON ANY DATES)             
                                                                                
         MVI   CONTYPE,0           CLEAR FLAG                                   
         TM    RCONMODR+1,X'40'    ...MUST BE GRAPHNET                          
         BNO   NXTREC                                                           
                                                                                
         MVI   ELCODE,X'20'        ...LAST SENT BY REP ?                        
         BAS   RE,GETEL                                                         
         BNE   RDR25                                                            
         USING RCONSEND,R6                                                      
         TM    RCONSENF,X'80'      LAST SENT BY REP?                            
         BZ    RDR25                                                            
         CLC   RCONSRDT,MONDAYDT     SENT BY REP WITHIN DATES?                  
         BL    RDR25                                                            
         CLC   RCONSRDT,TODAY2       SENT BY REP WITHIN DATES?                  
         BNH   RDR28                 YES                                        
         EJECT                                                                  
*                                                                               
RDR25    L     R6,AIO              ...CONFIRMED WITHIN DATES?                   
         MVI   ELCODE,X'1F'                                                     
         BAS   RE,GETEL                                                         
         BNE   NXTREC                                                           
         USING RCONXEL,R6                                                       
         ...
         DROP  R6                                                               
*                                  NO                                           
         L     R6,AIO              IF LAST SENT BY REP AND BUY LINES ?          
         USING RCONREC,R6                                                       
         TM    RCONMODR,X'40'+X'20'+X'10'      BUY LINES ?                      
         BZ    NXTREC              NO BUY LINES - SKIP REC                      
         MVI   ELCODE,X'20'        ...LAST SENT BY REP ?                        
         BAS   RE,GETEL                                                         
         BNE   NXTREC                                                           
         USING RCONSEND,R6                                                      
         TM    RCONSENF,X'80'      LAST SENT BY REP?                            
         BZ    NXTREC                                                           
         MVI   CONTYPE,C'O'        YES - SET TYPE TO 'OPEN'                     
         B     RDR28                                                            
         DROP  R6                                                               
                                                                                
RDR26    MVI   ELCODE,X'22'        MOD DATE/TIME ELEM                           
         L     R6,AIO                                                           
         BAS   RE,GETEL                                                         
         BNE   NXTREC                                                           
         USING RMODELEM,R6                                                      
         CLC   RMODEL1D,MONDAYDT   WITHIN REQ DATES?                            
         BL    NXTREC                                                           
         CLC   RMODEL1D,TODAY2     WITHIN REQ DATES?                            
         BH    NXTREC                                                           
...
* SORTREC KEY FOR STATION VERSION ***********************                       
...
*                                                                               
         BAS   RE,CHKMKTNM         CHECK MARKET NAME                            
         MVC   PNDMKTNM,WORK                                                    
*                                                                               
RDR38    DS    0H                                                               
...
RDR50    MVC   PNDDOLS,FULL                                                     
         ...
         CLI   CONTYPE,C'O'        OPEN CONTRACT ?   (UNCONFIRMED NOW           
*                                      AND NOT SENT WITHIN REQ DATES)           
         BNE   RDR50A                                                           
         MVC   PNDMOD,CONTYPE       YES-OPEN CONTRACT                           
         B     RDR52                                                            
*                                                                               
RDR50A   L     R6,AIO                                                           
...
* MODIFICATIO/VERION NUMBER                                                     
         ...
         USING RCONREC,R6                                                       
         CLI   PNDMOD,C'O'         OPEN/NOT CONFIRMED NOW                       
         BE    RDR62                                                            
...
*                                                                               
*&&UK                                                                           
GETFLG1  OC    PREVIOUS,PREVIOUS    FIRST TIME?                                 
         BNZ   GETFLG2                                                          
...
* CHECKFLG                                                                      
         ...
         MVC   REPHEAD,PNDKREP      SAVE REP FOR HEADLINE                       
*                                                                               
         XC    KEY,KEY             GET THE STATION REC                          
         LA    R6,KEY                                                           
         USING RSTAREC,R6                                                       
*                                                                               
         MVI   RSTAKTYP,X'02'                                                   
...
GETFLG12 GOTO1 HIGH                                                             
         ...
         GOTO1 GETREC                                                           
*                                                                               
         MVI   ELCODE,X'25'        GET EMAIL ELEMENT                            
         BAS   RE,GETEL                                                         
         BNE   SKIPFLG                                                          
         USING RSTAEML,R6                                                       
*                                                                               
...
*                                                                               
         ...
         CLI   OFFICVER,C'Y'               OFFICE VERSION OF REPORT ?           
         BNE   PRT30A                                                           
...
PRT45    DS    0H                                                               
         MVC   PLTYPE+132(4),=C'OPEN'      OPEN?                                
         CLI   PNDMOD,C'O'                                                      
         BE    PRT50                                                            
         MVC   PLTYPE+132(4),=C'NEW '      NEVER CONFIRMED?                     
         CLI   PNDMOD,C'N'                                                      
         BE    PRT50                                                            
...
HOOK10   MVC   H4+1(7),=C'STATION'                                              
         ...
         B     HOOK20                                                           
*                                                                               
HOOK20   GOTO1 DATCON,DMCB,(2,MONDAYDT),(5,H3+52)                               
         MVI   H3+60,C'-'                                                       
         GOTO1 DATCON,DMCB,(2,TODAY2),(5,H3+61)                                 
HOXX     B     XIT                                                              
...
*                                                                               
         ...
         USING RSTAEML,R6                                                       
*                                                                               
         ZIC   R5,RSTAEMLN                 ELEMENT LENGTH                       
         SHI   R5,4                        MINUS CODE+LENGTH+FLAG+EX            
         EX    R5,*+8                                                           
         B     *+10                                                             
...
*                                                                               
         MVC   P(5),=C'++DDS'                                                   
         MVC   P+11(3),=C'SUB'                                                  
         MVC   P+15(27),=C'WKLY STA REPORT FOR STATION'                         
         MVC   P+44(5),STAHEAD                 STATION                          
*                                                                               
         MVI   SKIPSPEC,C'Y'                                                    
         MVI   FORCEHED,C'N'                                                    
         MVI   FORCEMID,C'N'                                                    
...
REGSPECS DS    0H                                                               
         ...
         PSPEC H9,1,C'                    '                                     
         SPROG 0                                                                
         PSPEC H1,50,C'WKLY STA FAX ACTIVITY'                                   
         PSPEC H2,50,C'---- --- --- --------'                                   
         ...
         PSPEC H10,21,C'-----------------------------------------------X        
               --------------------------------------------------'              
         SPROG 1                                                                
         PSPEC H1,50,C'WKLY OFF FAX ACTIVITY'                                   
         PSPEC H2,50,C'---- --- --- --------'                                   
...
XRX      XIT1                                                                   
         EJECT                                                                  
*                                                                               
* REPIOTBL INITIALIZED IN RESFM3E                                               
*                                                                               
XINIT    DS    0H                                                               
         CLI   MONDAYDT,0          DID USER INPUT DATES?                        
         BNE   INIT5                              YES                           
...
INIT4    GOTO1 GETDAY,DMCB,WORK,WORK+6            RETURNS DAY OF WEEK           
         ...
         GOTO1 DATCON,DMCB,(0,WORK+6),(2,MONDAYDT)                              
*                                                                               
                                                                                
* - INITIALIZE TSAROFF                                                          
INIT5    GOTO1 =V(LOADER),DMCB,=CL8'T00A7D',0    TSAROFF                        
         MVC   ATSAROFF,DMCB+4                                                  
...
*                                                                               
         ...
         LTORG                                                                  
*                                                                               
NONGRAPH DS    CL5000              FOR 1000 STATIONS                            
         DC    X'FF'                                                            
         EJECT                                                                  
       ++INCLUDE DDTSARD                                                        
         EJECT                                                                  
...
SLSPRSN  DS    CL3                 SALESPERSON FILTER                           
...
REPHEAD  DS    CL2                                                              
OPENFAX  DS    CL1                                                              
MONDAYDT DS    CL2                 PREVIOUS MONDAY'S DATE                       
CONTYPE  DS    CL1                                                              
MYKEYSV  DS    CL40                                                             
...
EDICTD   DSECT                                                                  
       ...
       ++INCLUDE EDILINKD                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'090RESFM3DA  05/01/02'                                      
         END                                                                    
