*          DATA SET ACCLB0FB   AT LEVEL 055 AS OF 12/22/99                      
*PHASE T6210FB                                                                  
...
*&&UK*&& LH    RE,=Y(LC@CLIBG-TWAD)                                             
         ...
         MVC   CLIBG2,0(RE)                                                     
*                                                                               
         L     R1,BCAUTL           TEST AGENCY LANG = CONNECTED LANG            
         MVC   BOBYTE1,TAGCTRY-UTLD(R1)                                         
         L     R1,ACTRY                                                         
         LA    R1,6(R1)                                                         
         USING CTRYTABD,R1                                                      
GFRST02  CLC   CTRYCODE,BOBYTE1                                                 
         BE    GFRST04                                                          
         CLI   CTRYCODE,FF                                                      
         BE    GFRST10                                                          
         LA    R1,CTRYTABL(R1)                                                  
         B     GFRST02                                                          
GFRST04  CLC   CTRYLANG,CULANG                                                  
         BE    GFRST10                                                          
         MVC   CLIBG2,BCSPACES      NO - GET TEXT IN AGENCY LANGUAGE            
         MVI   CLIBG2,AC#ESCL                                                   
         MVC   CLIBG2+1(2),=AL2(AC#CLIBG)                                       
         MVI   CLIBG2+3,L'CLIBG2                                                
         MVI   BOFULL1+0,C'S'                                                   
*&&UK*&& ICM   RF,14,=C'SL '                                                    
*&&US*&& ICM   RF,14,=C'SU '                                                    
         MVI   BOFULL1+2,C' '                                                   
         ICM   RF,1,CTRYLANG                                                    
         DROP  R1                                                               
         GOTO1 VDICTAT,BOPARM,(RF),CLIBG2                                       
GFRST10  DS    0H                                                               
*                                                                               
...
*                                                                               
         ...
         BM    GETNEXT                                                          
         EX    RF,*+8                                                           
         BE    GNEXT02             BILL DID NOT ORIGINATE FROM CBIL             
         CLC   TRNNARR(0),CLIBG1                                                
         EX    RF,*+8                                                           
         BE    GNEXT02                                                          
...
*&&UK                                                                           
         ...
         L     R3,AIO1                                                          
         LA    R3,TRNRFST-TRNRECD(R3)                                           
         USING TRSELD,R3           FIND ACTIVITY DATE OF BILL                   
         XR    RF,RF                                                            
GNEXT04  CLI   TRSEL,0                                                          
         BE    GNEXT10                                                          
         CLI   TRSEL,TRSELQ                                                     
         ...
         IC    RF,TRSLN                                                         
         BXH   R3,RF,GNEXT04                                                    
         GOTO1 VDATCON,BOPARM,(2,TRSDATE),(1,BOWORK1)                           
         CLC   BCCPYSCM,BOWORK1    TEST PRECEDES CONVERSION                     
         BNH   GNEXT10                                                          
         MVC   TLPPCOD,BCCPYSEC    YES - SET TO SECONDARY                       
*&&                                                                             
GNEXT10  DS    0H                                                               
         L     R3,AIO1                                                          
         LA    R3,TRNRFST-TRNRECD(R3)                                           
...
GNEXT14  IC    RF,AFCLN                                                         
         BXH   R3,RF,GNEXT12                                                    
GNEXT16  MVC   TLPPCOD,AFCCURR                                                  
         MVC   TLPPEXC,AFCX                                                     
         DROP  R3                                                               
         B     GNEXT14                                                          
         USING RATELD,R3                                                        
GNEXT18  CLC   RATRATE,=X'8000'    THIS MEANS FULLY REVERSED & UPDATED          
         BE    GETNEXT                                                          
         B     GNEXT14                                                          
         DROP  R3                                                               
*                                                                               
GNEXT20  GOTO1 CURTOTAB,BOPARM,TLPPCOD,TLPPTAB                                  
         BE    GETNEXTX                                                         
...
*                                                                               
         CLC   TLPPCOD,CSCPYCUR    TEST IN BILLING CURRENCY                     
         BE    DISBILX                                                          
*&&UK                                                                           
         CLC   TLPPCOD,BCCPYSEC    TEST IN SECOND CURRENCY                      
         BNE   DISBILX                                                          
         MVC   BCWORK(L'CSCPYCUR),CSCPYCUR                                      
         MVC   BCWORK+L'CSCPYCUR(L'CSBILCUR),TLPPCOD                            
         L     R1,ACOM                                                          
         L     RF,CTOBACCO-COMFACSD(R1)                                         
         GOTO1 (RF),BCPARM,('TOBAACVS',BCWORK),AIO1,ACOM,0,0                    
         OI    LSINDS2,X'20'                                                    
*&&                                                                             
DISBILX  B     EXIT                                                             
         SPACE 1                                                                
...
VALS04   CLI   AFCEL,0                                                          
         ...
         BE    VALS05                                                           
         CLC   TLPPCOD,CSCPYCUR    TEST IF DEFAULT CURRENCY                     
         BNE   *+14                NO - GIVE ERROR                              
         CLC   AFCCURR,BCCPYSEC    TEST PENDING CURRENCY IS SECOND              
         BE    VALS06              ALLOW SELECT (REVERSE WILL FILTER)           
         MVC   FVMSGNO,=AL2(AE$BCDMA)                                           
         MVC   FVXTRA(L'TLPPCOD),TLPPCOD                                        
         B     EXITN                                                            
VALS05   CLC   TLPPCOD,CSCPYCUR    IF AGENCY CURRENCY DONT TEST RATE            
         BE    VALS06                                                           
         CLC   AFCXRATE,TLPPEXC+(AFCXRATE-AFCX) TEST MATCHING X-RATE            
         BE    VALS06                                                           
         MVC   FVMSGNO,=AL2(AE$BRDMA)                                           
         GOTO1 AEDTRAT,BOPARM,(L'FVXTRA,FVXTRA),                       *        
               TLPPEXC+(AFCXRATE-AFCX),0                                        
         B     EXITN                                                            
         DROP  R3                                                               
*                                                                               
...
BILREV02 MVC   CSBILNUM,TLPPBIL    SET SELECTED BILL NUMBER                     
         GOTO1 ASETUP,BOPARM,(X'18',0),TLPPCOD,TLPPEXC                          
         B     EXITY                                                            
         EJECT                                                                  
***********************************************************************         
* AUTO REVERSE                                                        *         
...
AUTREV06 MVC   CSBILNUM,TLPPBIL    SET SELECTED BILL NUMBER                     
         GOTO1 ASETUP,BOPARM,(X'18',0),TLPPCOD,TLPPEXC                          
*                                                                               
         ICM   R0,3,TLNUM          SAVE NUMBER FOR XITSES RETURN                
*                                                                               
...
*                                                                               
CLIBG1   DS    CL(L'UC@CLIBG)      "CLIENT BILLING"                             
CLIBG2   DS    CL(L'UC@CLIBG)      "CLIENT BILLING"                             
*                                                                               
...
CODTABL  EQU   *-CODTABD                                                        
         SPACE 1                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'055ACCLB0FB  12/22/99'                                      
         END                                                                    
