*          DATA SET PQNDXF     AT LEVEL 002 AS OF 04/13/20                      
*PHASE PQNDXFA                                                                  
*INCLUDE CARDS                                                                  
*INCLUDE DMDMGRL                                                                
*INCLUDE DATCON                                                                 
...
*INCLUDE HEXIN                                                                  
*INCLUDE SORTER                                                                 
         TITLE 'PQNDXF - FILE INDEX FIX AND REBUILD'                            
         PRINT NOGEN                                                            
PQNDXF   CSECT                                                                  
*                                                                               
         ENTRY UTL                                                              
         ENTRY SSB                                                              
*                                                                               
         NBASE WORKX-WORKD,**PQXF**,=A(WORKAREA),RA,R9                          
         USING WORKD,RC                                                         
         ST    RD,SAVERD                                                        
*                                                                               
...
***********************************************************************         
*        READ DATA LINES AND WRITE TO PRTQ                                      
***********************************************************************         
MAIN     NTR1                                                                   
*                                                                               
         BAS   RE,GETLIST          GET LIST OF PRTQ FLES                        
*                                                                               
MAIN010  BAS   RE,INITPQ           GET FILE CI DETAIL                           
*                                                                               
         BAS   RE,NDX                                                           
         BNE   MAINERR                                                          
*                                                                               
...
*                                                                               
         ...
         GOTO1 VDATAMGR,DMCB,(0,GLIST),PQFILE,UKKEY,CIADDR,APQBUFF              
         ICM   RE,15,UKUSRINF                                                   
         BZ    GETLISD                                                          
*                                                                               
...
*                                                                               
         LA    RF,CIADDR                                                        
         MVC   CIADDR,=X'00010100'                                              
         GOTO1 VDATAMGR,DMCB,BUFFER,PQFILE,UKKEY,(RF),APQBUFF                   
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R1,APQBUFF                                                       
...
***********************************************************************         
* READ THROUGH PRTQ FILE AND UPDATE INDEXES ON THE FILE                         
***********************************************************************         
         USING PQRECD,R6                                                        
         USING SKBUFFD,R7                                                       
NDX      NTR1                                                                   
...
*                                                                               
         MVI   FERI,1                                                           
         GOTO1 VDATAMGR,DMCB,DMREAD,PQFILE,CXADDR,(R4)                          
         CLI   8(R1),0                                                          
         JNE   *+2                                                              
*                                                                               
...
*                                                                               
         MVI   FERI,2                                                           
         GOTO1 VDATAMGR,DMCB,DMREAD,PQFILE,CIADDR,(R6)                          
         CLI   DMCB+8,0                                                         
         JNE   *+2                                                              
*                                                                               
         L     R1,NDXTOTAL                                                      
         ...
         ST    R1,NDXTOTAL                                                      
*                                                                               
         CLC   0(L'PQINDEX,R5),PQINDEX                                          
         BE    NDX8                                                             
*                                                                               
*        CLI   W_STAT-PQINDEX(R5),W_STPU                                        
*        BE    *+18                                                             
*        MVC   QLINDEX,0(R5)                                                    
*        MVI   BYTE,C'D'                                                        
*        BRAS  RE,PUTSORT                                                       
...
*                                                                               
*                                                                               
         MVC   PLINE(10),=C'FROM FILE '                                         
         GOTO1 =V(HEXOUT),PLIST,(R6),PLINE+10,30                                
         BRAS  RE,PRINTL                                                        
         MVC   PLINE(10),=C'TO INDEX  '                                         
         GOTO1 =V(HEXOUT),PLIST,(R5),PLINE+10,30                                
         BRAS  RE,PRINTL                                                        
*                                                                               
         MVC   0(L'PQINDEX,R5),PQINDEX                                          
*                                                                               
...
NDX10    CLI   RCWRITE,C'Y'                                                     
         BNE   NDX4                                                             
         MVI   FERI,3                                                           
         GOTO1 VDATAMGR,DMCB,DMWRT,PQFILE,CXADDR,(R4)                           
         CLI   DMCB+8,0                                                         
         JNE   *+2                                                              
*                                                                               
         L     R1,NDXWRITE                                                      
...
*        CLI   REPFLG,C'N'               TEST FOR REPORT=NO                     
*        BE    PUTSORTX                                                         
*        MVC   QLINDEX-2(2),QLREPNO                                             
*        MVC   QLINDEX-4(2),QLSRCID                                             
*        MVC   QLINDEX-5(1),PQFILE+4                                            
*        MVC   QLINDEX-6(1),BYTE                                                
*                                                                               
*        GOTO1 =V(SORTER),DMCB,=C'PUT',QLINDEX-6                                
*                                                                               
...
REP020   BAS   RE,COUNT            KEEP A COUNT OF TOTALS                       
         MVC   PLIUSER,GUSERN                                                   
         MVC   PLPRTQ(1),QLINDEX-5 PRTQ FILE NUMBER                             
         MVC   PLPRTQ+1(1),QLINDEX-6 ADD OR DELETE                              
         MVC   DUB,QLKEY                                                        
...
REP061   MVC   PLRETAD,DOTS                                                     
         MVC   HALF1,QLAGERD       SET HALF1 TO CMPRSD NEW DATE                 
         TM    QLTYP1,QLTYNCD                                                   
         BO    *+8                                                              
         NI    HALF1,X'7F'                                                      
...
*                                                                               
         ...
         MVC   CUSERF,QLINDEX-5    SAVE NAFILE NUMBER                           
*                                                                               
         CLI   QLINDEX-6,C'D'                                                   
         BE    COUNTX                                                           
*                                                                               
...
*                                                                               
         ...
         BAS   RE,COUNTR                                                        
*                                                                               
         TM    QLSTAT,X'80'        TEST FOR ACVTIVE                             
         BZ    COUNTX                                                           
*                                                                               
         MVC   HALF1,TODAYC                                                     
         TM    QLTYP1,QLTYNCD                                                   
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'002PQNDXF    04/13/20'                                      
         END                                                                    
