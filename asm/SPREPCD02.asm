*          DATA SET SPREPCD02  AT LEVEL 079 AS OF 05/01/02                      
*PHASE SPCD02A                                                                  
*INCLUDE BINSRCH2                                                               
*INCLUDE COVAIL                                                                 
*INCLUDE MEDMOVER                                                               
...
*INCLUDE ADDAY                                                                  
*INCLUDE GETDAY                                                                 
         TITLE 'SPREPCD02-COKE PERFORMANCE'                                     
         SPACE 2                                                                
***********************************************************************         
*                                                                               
*         THIS PROGRAM IS A CLONE OF SPREPCC02.                                 
*         IT COMPARES BUY VS GOAL DATA AND SETS THE RESULTS                     
*         IN THE INCH STATUS RECORDS. IT RUNS AS AN AUTOMATIC REQUEST           
*         OUT OF THE Z5.                                                        
*                                                                               
*         QOPT1=W = REQUESTED BY GW                                             
*         THE CD IS NOW AUTO REQUESTED AS A STEP AFTER THE GX WHICH             
*         LOADS GOALS ONTO THE FILE - THE CD WILL RESET THE STATUS              
*         OF THE INCH RECORD TO UNEVALUATED IN THIS CASE                        
*                                                                               
*     *** THE REQUESTS ARE FOR ALL STATIONS FOR THE GIVEN MARKET                
*     *** FOR THE WHOLE ESTIMATE PERIOD. (AS OF APR28/98)                       
*     *** NOTE- REQUEST IS ALTERED AT REQF.                                     
*                                                                               
*         THE CC USED (AMONG OTHER THINGS) TO DO A                              
*         BUY/LOCKIN CHECK AND SET THE RESULTS IN THE INCH STATUS               
*         RECORDS. IT NO LONGER DOES THIS, BUT IS AUTO-REQUESTED BY             
*         THE MM+ TYPE PROGRAMS.                                                
*                                                                               
***********************************************************************         
*                                                                               
...
* --------------                                                      *         
*                                                                     *         
* ???96     COKE STATUS VERSION CREATED                                         
* SEP97     MAINTAIN COKE GOAL STATUS IN MSR RECORDS, NOT BGR'S.      *         
*                                                                     *         
***********************************************************************         
         EJECT                                                                  
* SPROG SETTINGS                                                                
...
SPCD02   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,SPCD02,RR=R5                                                   
         LA    R2,2048(RB)                                                      
         LA    R2,2048(R2)                                                      
         USING SPCD02+4096,R2                                                   
         L     RA,0(R1)                                                         
         LA    RC,2048(RA)                                                      
         LA    RC,2048(RC)                                                      
         USING SPWORKD,RA,RC                                                    
...
REQF     CLI   MODE,REQFRST                                                     
         BNE   M20                                                              
*                                                                               
         CLI   QOPT1,C'W'          REQUEST AFTER THE SGW?                       
         BE    REQF10                                                           
*                             FIX REQUEST (UNTIL Z5 GETS IT RIGHT)              
         MVC   DUB(5),QSTA                                                      
         MVC   DUB+5(3),QCBLNET                                                 
         GOTO1 MSPACK,DMCB,QMKT,DUB,SVRMKSTA   SAVE REQUESTED STATION           
*                                               AND MONTH OF SERVICE            
REQF10   CLC   QEND(4),SPACES      VALID                                        
         BH    REQF12                                                           
         GOTO1 AENDREQ             NO SKIP REQUEST                              
*                                                                               
REQF12   MVC   DUB(4),QEND         QSTART/END WILL BE BRDMON                    
         MVC   SVRMOS2,QEND                                                     
         MVC   DUB+4(2),=C'01'     GET DATE 01 OF BRDMON                        
         GOTO1 DATCON,DMCB,DUB,(2,SVRMOS)                                       
         XC    SVRMOS,=X'FFFF'     COMPLEMENT                                   
*                                                                               
         MVC   QSTA(5),=C'ALL  '   ALL STATIONS                                 
         MVC   QSTAUTO,=C'ES '     WHOLE ESTIMATE                               
*                                                                               
         L     RE,AFLTRTAB                                                      
         L     RF,=F'500'                                                       
         XCEF                                                                   
...
CHKSLTOT CLI   PROGPROF+4,C'Y'                                                  
         ...
         BAS   R9,SUPRPTS                                                       
*                                                                               
M21EX    GOTO1 =A(SETBUF),DMCB,(RA),LCODE,LVCNTRL,RR=RELO                       
         MVC   HIGROUP,LCODE                                                    
         L     RF,BUFFBUFF                                                      
...
*        MARKET LAST                                                            
***********************************************************************         
M5       CLI   MODE,MKTLAST     DO ALL STATIONS IN MARKET                       
         BNE   M6                                                               
         CLI   QCOMPARE,C'S'                                                    
         BE    M6                                                               
*                                                                               
...
*        MVI   FORCEHED,C'Y'                                                    
         L     R4,ACTAREA                                                       
         GOTO1 =V(VMDAPRNT),DMCB,(RA),(R4),1,RR=RELO                            
         SPACE 2                                                                
...
M53      L     R3,=A(WKTABLE)                                                   
         A     R3,RELO                                                          
*        GOTO1 SPCDBG                                                           
         GOTO1 =A(SPCDBG),DMCB,(R3),RR=RELO                                     
...
DOSUM    L     R4,BUFFIO           DO SUMMARY REPORTS                           
         CLC   QPROG,=C'CD'        NO SUMMARY FOR CD                            
         BER   R9                                                               
...
***********************************************************************         
GDTE     NTR1                                                                   
         GOTO1 =A(VGDTE),DMCB,(RA),HCAP1,RR=RELO                                
         XIT1                                                                   
         LTORG                                                                  
         DROP  RF                                                               
EXIT     XMOD1 1                                                                
...
POSTER   NTR1                                                                   
         MVI   BUFFACT,1                                                        
         GOTO1 =A(POSTR),DMCB,PSLIST,RR=RELO                                    
         XIT1                                                                   
***********************************************************************         
*        SUP RPTS                                                               
***********************************************************************         
...
*                                                                               
         ...
         CLC   QPROG,=C'CD'        IS IT AN M3 REQUEST                          
...
*        USING WKTD,R4                                                          
         L     R5,=A(LKTABLE)                                                   
         USING LKTD,R5                                                          
         XC    WKTOTS,WKTOTS       CLEAR CERTAIN WEEK TOTALS                    
         XC    MOS,MOS                                                          
*                                                                               
STAT10   DS    0H                                                               
         CLI   QCOMPARE,C'S'       COMPARE LOCKIN DATA?                         
         BE    STATX               *** NO-OP LOCKIN LOGIC **                    
*                                                                               
*        THE CD ONLY SETS MSRBYHI(OVER BUDGET) AND MSRNOBD(NO BUDGET)           
*                                                                               
STAT30   OC    WKTWEEK,WKTWEEK                                                  
         BNZ   STAT31                                                           
*                                                                               
*        NEW TEST FOR OVERBUDGET - BUY DOLLARS MUST BE 10% OR $1000             
*                                  GREATER THAN THE GOALS                       
*                                                                               
         L     R1,WKTOTBDL                                                      
         L     R3,WKTOTGDL                                                      
         SR    R1,R3               BUYS-GOALS                                   
         C     R1,=F'100000'       $1000 + PENNIES                              
         BL    *+8                                                              
         OI    STATUS,BGRMBYHI     BUY HIGHER THAN BUDGET LIMIT                 
         MHI   R1,10               IF THE DIFFERENCE*10 > GOAL$                 
         CR    R1,R3               (SAME AS OVERBUDGET 10%)                     
         BL    *+8                                                              
         OI    STATUS,BGRMBYHI     BUY HIGHER THAN BUDGET LIMIT                 
*                                                                               
...
*                                                                               
STAT30B  DS    0H                                                               
         OC    MOS,MOS             THIS TST IS ANY DATA OR NOT                  
         BZ    STATX               NONE, SKIP                                   
         CLI   QOPT1,C'W'          REQUEST FROM GW                              
         BNE   STAT30C                                                          
         BAS   RE,MSRPROCW         SPECIAL LOOP TO GET STATIONS                 
         B     *+8                                                              
STAT30C  BAS   RE,MSRPROC          SET STATUS IN MSR REC                        
         XC    WKTOTS,WKTOTS       CLEAR TOTALS                                 
         B     STATX                                                            
*                                                                               
STAT31   DS    0H                                                               
         OC    MOS,MOS             HAVE WE ALREADY GOT MONTH                    
         BNE   STAT32                                                           
         GOTO1 DATCON,DMCB,(2,WKTWEEK),DUB                                      
         CLC   DUB+4(2),=C'01'                                                  
         BE    STAT31B                                                          
         GOTO1 ADDAY,DMCB,DUB,DUB,7    NEXT MONTH                               
         MVC   DUB+4(2),=C'01'         RESET TO FIRST OF MONTH                  
*                                                                               
STAT31B  DS    0H                                                               
         GOTO1 DATCON,DMCB,DUB,(2,MOS)                                          
         XC    MOS,=X'FFFF'        COMPLEMENT                                   
*                                                                               
STAT32   DS    0H                                                               
         LM    R0,R1,WKTOTBDL      TOTAL BUY/GOAL DOLLARS                       
         A     R0,WKTDL                                                         
         A     R1,WKTGLD                                                        
         STM   R0,R1,WKTOTBDL                                                   
*                                                                               
...
WKTOTGDL DS    F                                                                
         EJECT                                                                  
***********************************************************************         
*        LOOP TO GET ALL STATIONS WITH INCH RECS WHEN REQUEST FROM GW           
***********************************************************************         
MSRPROCW NTR1                                                                   
         USING MSRKEYD,R2                                                       
         LA    R2,XKEY                                                          
...
*                                                                               
         MVC   XKEY2,XKEY                                                       
MSRW10   GOTO1 DATAMGR,DMCB,=C'DMRDHI',=C'XSPDIR',XKEY,XKEY                     
         CLC   XKEY(MSRKSTA-MSRKEY),XKEY2     SAME THRU MKT?                    
         BNE   MSRWX                                                            
         CLC   MSRKMOS,SVRMOS      RIGHT MONTH                                  
         BE    MSRW20                                                           
         ZIC   R1,MSRKMOS+1        FORCE NEXT MONTH                             
         LA    R1,1(R1)                                                         
         STC   R1,MSRKMOS+1                                                     
         B     MSRW10                                                           
*                                                                               
MSRW20   MVC   SVRMKSTA+2(3),MSRKSTA                                            
         BAS   RE,MSRPROC          PROCESS THIS STATION                         
         MVI   MSRKMOS,X'FF'       FORCE NEXT STATION                           
         B     MSRW10                                                           
*                                                                               
MSRWX    XIT1                                                                   
         DROP  R2                                                               
         EJECT                                                                  
***********************************************************************         
*        GET INCH STATUS RECORD AND POST BUY/GOAL STATUS              *         
*                                                                               
*              NOTE- BUY/GOAL STATUS USED TO BE POSTED TO BUCH RECS AND         
*                    LOCKIN  TO INCH RECS.                                      
*                    NOW ONLY GOALS TO INCH RECS.                               
*                                                                               
*        *** THIS IS THE ONLY STATUS-SETTING MODULE THAT IS  ***                
*        *** NOW ACTUALLY USED IN THIS PROGRAM.              ***                
*        *** SEE SPREPCC02 FOR BUCH STATUS POSTING.          ***                
*                                                                               
***********************************************************************         
MSRPROC  NTR1                                                                   
         OI    STATUS,MSRBGUSD     SET DOING B/G STATUS, NOT LOCKIN             
*                                  NOTE, WATCH STATUS=0 TESTS                   
         MVI   FLAG1,0                                                          
*                                                                               
         L     R5,=A(MINBLK)                                                    
         USING MINBLKD,R5                                                       
         NI    RECFLAG,X'FF'-NOSTATEL                                           
         XC    MINMKEY,MINMKEY                                                  
         LA    R2,MINMKEY                                                       
         USING MSRKEYD,R2                                                       
         MVI   MSRKTYPE,MSRKTYPQ                                                
...
MGR20    DS    0H                                                               
         ...
         BNE   MGR30                                                            
         CLI   MSRSTEL,MSRSTELQ      MATCHING STATUS ELEMENT?                   
         BE    MGR40                                                            
...
MGR30    DS    0H                                                               
         ...
         XC    0(L'MELEM,R4),0(R4)   CLEAR MINELEM AREA                         
         MVI   MSRSTEL,MSRSTELQ      ELEMENT CODE                               
         MVI   MSRSTLEN,MSRSTLNQ     ELEMENT LENGTH                             
         MVC   MSRSTSTA(2),SPACES                                               
         MVC   MSRSTERS,=C'NNNNNN'                                              
         MVC   MSRSTBGS,STATUS       NEW MIS STATUS                             
         MVC   X,0(R4)               SAVE ELEMENT                               
         GOTO1 VMINIO,DMCB,('MINADD',MINBLKD)                                   
         CLI   MINERR,0                                                         
         BE    MGR50                                                            
         DC    H'0'                                                             
...
MGR50    DS    0H                                                               
         ...
         LA    R4,MINEKEY                                                       
         USING MSRICELD,R4                                                      
         MVI   MSRICEL,MSRICELQ                                                 
         GOTO1 VMINIO,DMCB,('MINHI',MINBLKD)                                    
*                                                                               
         L     R4,=A(MELEM)          A(AREA FOR ELEM OR CLUSTER)                
         CLI   MINERR,0              TREAT ALL ERRS HERE AS EOF                 
         BNE   MGR60                                                            
         CLI   MSRICEL,MSRICELQ      MATCHING STATUS ELEMENT?                   
         BNE   MGR60                                                            
*                                                                               
         CLI   MSRICST,C'A'        ALREADY APPROVED?                            
         BE    MGR60               THEN DON'T CHANGE                            
*                                                                               
         OI    FLAG1,FLCHANGE      CHANGE IN INCH STATUS                        
         MVI   MSRICST,C'U'          RESET TO UNEXAMINED                        
         OI    MSRICST2,MSRICUGQ     BECAUSE OF SUBSEQUENT GOALS                
         MVC   SVINCH,0(R4)          SAVE FOR TRACE                             
         GOTO1 VMINIO,DMCB,('MINWRT',MINBLKD)                                   
         CLI   MINERR,0                                                         
         BE    *+6                                                              
         DC    H'0'                                                             
*                                                                               
MGR60    GOTO1 VMINIO,DMCB,('MINCLS',MINBLKD)                                   
         TM    FLAG1,FLCHANGE      CHANGE IN INCH STATUS                        
         BNO   MGR60A                                                           
         MVC   P(36),=C'** CHANGE IN GOALS - STATUS UPDATED '                   
         MVC   DUB(4),SVRMOS2                                                   
         MVC   DUB+4(2),=C'01'                                                  
         GOTO1 DATCON,DMCB,DUB,(6,P+36)                                         
         GOTO1 REPORT                                                           
*                                                                               
MGR60A   CLI   QOPT3,C'T'          TEST TO TRACE                                
         BNE   MGR62                                                            
         CLI   QOPT1,C'W'          IF FROM THE GW                               
         BNE   MGR61                                                            
         TM    FLAG1,FLCHANGE      CHANGE IN INCH STATUS                        
         BNO   MGR62               THEN DON'T BOTHER                            
MGR61    RELOC RELO2                                                            
         GOTO1 HEXOUT,DMCB,MINMKEY,P,40,=C'N',RR=RELO2                          
         GOTO1 REPORT                                                           
         GOTO1 HEXOUT,DMCB,X,P,90,=C'N',RR=RELO2                                
         GOTO1 REPORT                                                           
         CLI   QOPT1,C'W'                                                       
         BNE   MGR62                                                            
         GOTO1 HEXOUT,DMCB,SVINCH,P,MSRICLNQ,=C'N',RR=RELO2                     
         GOTO1 REPORT                                                           
*                                                                               
MGR62    DS    0H                                                               
         TM    STATUS,MSRNOBD                                                   
         ...
         GOTO1 REPORT                                                           
MGR70    DS    0H                                                               
         TM    STATUS,MSRNOBY    *** NOTE - BIT NOT SET                         
         BNO   MGR80                                                            
...
MGR80    DS    0H                                                               
         ...
         GOTO1 REPORT                                                           
MGR90    DS    0H                                                               
         TM    STATUS,MSRBPLO          *** NOTE- BIT NOT SET                    
         BNO   MGRX                                                             
         MVC   P(43),=C'BUY POINTS < GOAL - 10%'                                
         GOTO1 REPORT                                                           
*                                                                               
MGRX     GOTO1 REPORT                                                           
         XIT1                                                                   
...
****     GOTO1 =V(HEXOUT),DMCB,LKTWEEK,P,LKTLN,0,0,RR=RELO2                     
****     GOTO1 =V(HEXOUT),DMCB,WKTWEEK,P2,WKTLN,0,0,RR=RELO2                    
****     GOTO1 REPORT                                                           
PRNTRPX  XIT1                                                                   
         DROP  R4,R5                                                            
         EJECT                                                                  
...
***********************************************************************         
         ...
         MVC   SLKEY,KEY           SAVE KEYS AND REC AREA                       
         MVC   SLKEYSAV,KEYSAVE                                                 
         MVC   SLAREC,AREC                                                      
         LA    R4,KEY                                                           
         XC    KEY,KEY                                                          
         USING SLHRECD,R4                                                       
         ...
         MVC   SLHKAGMD,BAGYMD                                                  
         MVC   SLHKCLT,BCLT                                                     
         MVC   SLHKMKT(5),BMKT     MARKET/STATION                               
         LA    R2,SLREC                                                         
         ST    R2,AREC                                                          
         GOTO1 HIGH                                                             
         CLC   KEY(SLHKSEQ-SLHKEY),KEYSAVE                                      
         BNE   SPCDSLX                                                          
         LA    R2,KEYSAVE                                                       
         USING SLKRECD,R2                                                       
         XC    KEYSAVE,KEYSAVE                                                  
         ...
         MVC   SLKKAGMD,BAGYMD                                                  
         MVC   SLKKSEQ,SLHKSEQ                                                  
         MVC   SLKKPRD,BPRD        CAREFUL NO PIGGY SUPPORT                     
         MVC   KEY,KEYSAVE                                                      
         GOTO1 HIGH                                                             
         B     SLK1                                                             
SLK0     GOTO1 SEQ                                                              
SLK1     CLC   KEY(SLKKPRD2-SLKKEY),KEYSAVE                                     
         BNE   SPCDSLX                                                          
         LA    R2,KEY                                                           
         CLC   SLKKEST,BEST        ONLY SINGLE EST                              
         BNE   SLK0                                                             
         GOTO1 GET                                                              
*                                                                               
...
*                                                                               
         ...
         LA    R2,SSQEL                                                         
         DROP  R2                                                               
SLK4     CLI   0(R2),0             FIN THE ELEMENT                              
         BE    SLK0                                                             
         CLI   0(R2),LOKELCDQ                                                   
         BE    SLK10                                                            
SLK5     ZIC   RE,1(R2)                                                         
...
SLK11    MVC   LKTWEEK,LOKWEEK     SET OR FIND WEEK SLOT                        
...
*                                                                               
         ORG   SPACEND                                                          
SVRMOS   DS    XL2                 REQUESTED MOS                                
SVRMKSTA DS    XL5                 AND MKT/STA                                  
SVRMOS2  DS    CL4                 REQUESTED MOS                                
*                                                                               
         PRINT OFF                                                              
       ++INCLUDE SPREPPTBUF                                                     
         EJECT                                                                  
...
* DDDICTATED                                                                    
       ++INCLUDE DDDICTATED                                                     
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'079SPREPCD02 05/01/02'                                      
         END                                                                    
