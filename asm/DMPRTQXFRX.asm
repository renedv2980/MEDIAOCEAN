*          DATA SET DMPRTQXFRX AT LEVEL 005 AS OF 05/01/02                      
*PHASE PRTQXFRA                                                                 
*INCLUDE DMDMGRL                                                                
*INCLUDE CARDS                                                                  
*INCLUDE DATCON                                                                 
...
*                                                                               
MVPQ1    ST    R1,ACOMRG           SAVE MVS SUPV INFO                           
         L     R1,0(R1)                                                         
         LH    R2,0(R1)                                                         
...
MVPQ1B   LA    R1,1(R1)                                                         
         LA    RF,1(RF)                                                         
         BCT   R2,MVPQ1A                                                        
MVPQ1C   MVC   UPSIINP,UPSIVAL     SAVE UPSI                                    
         B     MVPQ1X                                                           
UPSITAB  DC    X'8040201008040201'                                              
MVPQ1X   EQU   *                                                                
         EJECT                                                                  
MVPQ2    XC    ALOADPT,ALOADPT     CLEAR EXTERNAL LOAD POINT ADDR               
         MVI   XFLAG,0                                                          
         ...
         ST    RE,EXCB+20                                                       
*                                                                               
MVPQ3    CLI   MVSDUMP,C' '        TEST IF DUMP=MVS INPUT                       
         BNE   MVPQ3X                                                           
         ST    RB,STXITLST         SET UP AREA TO DUMP                          
         L     RF,=A(PRTQXFRX)                                                  
         ST    RF,STXITLST+4                                                    
         MVI   STXITLST+4,X'80'                                                 
...
GP4X     EQU   *                                                                
*                                                                               
GP5      TM    MODE,COPY           COPY - SET DEFAULT OUTPUT=                   
         BZ    GP5X                                                             
         CLI   OUTPUT,0                                                         
         BNE   GP5X                                                             
         TM    INPUT,MVS                                                        
...
GP5X     EQU   *                                                                
*                                                                               
GP6      CLI   OUTPUT,0            OUTPUT ONLY FOR MODE=COPY                    
         BE    GP6B                                                             
         CLI   MODE,COPY                                                        
         BE    GP6B                                                             
...
GP8B     TM    INPUT,PRTQ          IF INPUT=PRTQ MUST HAVE XXX,NNNN             
         ...
         BZ    GP8A                                                             
         B     GP8X                                                             
GP8C     TM    REPORT,B'00001110'  IF INPUT=MVS MUST HAVE REPORT=XXX            
         BNZ   GP8A                                                             
GP8X     OC    ALOADPT,ALOADPT     SET LOAD TO NOP IF NOT USED                  
         ...
         ST    R1,ALOADPT                                                       
*                                                                               
         B     OPEN                ALL INPUT PARAMS ARE CONSISTANT              
         SPACE 2                                                                
ANOP     BR    RE                                                               
         SPACE 2                                                                
...
OPEN4    GOTO1 =V(PRINTER)                                                      
         ...
         MVC   TITLE+35(7),DUB2                                                 
*                                                                               
OPEN5    CLI   MODE,2              GO TO ROUTINE FOR MODE                       
         BE    PRTPQ                                                            
         CLI   MODE,4                                                           
         BE    CPYPQ                                                            
         DC    H'0'                                                             
...
CPYPQ0   TM    INPUT,PRTQ          BUMP TO NEXT INPUT PRTQ FILE                 
         ...
         ST    RF,COPYINDX                                                      
         BAS   RE,PQLOCK           ENQUEUE PRTQUE IF COPY FROM PRTQ             
         XC    INDEX,INDEX                                                      
*                                                                               
...
CPYPQ5A  CLI   XFLAG,C'E'          END REC RETURN                               
         ...
         B     CPYPQ6A                                                          
*                                                                               
CPYPQ5B  LA    R7,=C'CLOSE'        SET NORMAL EOF                               
         CLC   5(3,R3),2(R3)                                                    
         BE    *+8                                                              
...
RIMVS3   TM    36(R1),X'02'        NO MACHINE CHR ON INPUT RECORD               
         ...
         B     RIMVS5                                                           
*                                                                               
RIMVS4   CLI   TYPE+1,C'C'         CC CHR AT START OF INPUT RECORD              
         BE    RIMVS4A                                                          
...
VPARM1A1 MVC   DATEIPL,C+5         OVERRIDE SYSTEM IPL DATE                     
         ...
         B     VPARM1P                                                          
*                                                                               
VPARM1B  CLC   C(6),=CL6'DDSIO='   DDSIO=XXXXXXXX TO SET WHICH DMDMGR           
         BNE   VPARM1C                                                          
...
VUSER8   L     RF,AREPTAB          SET END OF TABLE                             
         ...
         MVC   0(2,RF),=X'FFFF'                                                 
*                                                                               
VUSER9   TM    MODE,COPY           USER MUST BE SPECIFIC FOR MODE=COPY          
         BZ    VUSER9A                                                          
         TM    USER,X'02'                                                       
         BZ    VUSERERR                                                         
         B     VUSERX                                                           
...
OPNPQ5   L     R2,ACXREC           READ FIRST INDEX RECORD                      
         ...
         TM    0(RE),X'80'                                                      
         BZ    *+14                                                             
         MVC   20(20,RE),24(R2)    EXTRACT PART 2 INDEX DATA CUNT HEAD          
         NI    0(RE),X'7F'                                                      
...
*DDBIGBOX                                                                       
       ++INCLUDE DDBIGBOX                                                       
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'005DMPRTQXFRX05/01/02'                                      
         END                                                                    
