*          DATA SET REVIEW24   AT LEVEL 009 AS OF 09/27/05                      
*          DATA SET REVIEW24   AT LEVEL 007 AS OF 11/19/99                      
*PHASE T81724A                                                                  
VIEW24   TITLE 'WH/6/9 REPORTS FOR VIEWER'                                      
VIEW24   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,REVIEW24,R7,RR=RE                                              
         USING TWAD,RA                                                          
         USING WORKD,R9                                                         
         L     R8,AGWORK                                                        
         USING GWORKD,R8                                                        
...
***********************************************************************         
         SPACE 1                                                                
DISGRP   MVC   FVIFLD(L'REWHGRP),FLTIFLD                                        
         B     EXITOK                                                           
         SPACE 1                                                                
***********************************************************************         
...
***********************************************************************         
         SPACE 1                                                                
DISTEM   CLC   FLTIFLD(L'REWHTEAM),BCEFFS                                       
         BNE   *+14                                                             
         MVC   FVIFLD(3),=C'ALL'                                                
         B     EXITOK                                                           
*                                                                               
         MVC   FVIFLD(L'REWHTEAM),REWHTEAM                                      
         B     EXITOK                                                           
         SPACE 1                                                                
***********************************************************************         
...
VALTEM   CLI   FVILEN,3                                                         
         ...
         CLC   =C'ALL',FVIFLD                                                   
         BNE   VTEMN                                                            
         MVC   FLTIFLD(L'REWHTEAM),BCEFFS                                       
         B     EXITOK                                                           
*                                                                               
VTEM02   CLI   FVILEN,L'REWHTEAM                                                
         BNE   VTEMN                                                            
*                                                                               
...
***********************************************************************         
         SPACE 1                                                                
DOFQTR   OC    REWHTOT,REWHTOT                                                  
         BZ    FLTXX               NOT A TOTAL LINE                             
*                                                                               
         CLC   REWHPRD+1(1),FLTIFLD                                             
         BL    FLTXL                                                            
         BE    FLTXE                                                            
         BH    FLTXH                                                            
         DROP  R2                                                               
...
*                                                                               
         ...
         MVCL  R0,RE                                                            
*                                                                               
         MVC   IOKEY,REWHKEY       SET CURRENT RECORD INTO IO1                  
...
*                                                                               
         MVC   IOKEY,0(R4)         SET KEY IN IOKEY                             
         CLC   REWHKEY(REWHPRD-REWHKEY),REC.REWHKEY                             
...
*                               ** PERIOD IS YYMM BINARY                        
         ...
         CLI   REWHPRD+1,X'0D'      IF MM IS 'OD' THIS IS A YRLY TOTAL          
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         MVC   L1PED(L'ALL),ALL                                                 
         CLI   REWHPRD+1,FF         IF MM IS 'FF' PUT OUT 'ALL'                 
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
...
SCRL02   MVC   0(L'GROUP,R4),GROUP                                              
         MVI   L'GROUP(R4),C'='                                                 
         LA    R4,L'GROUP+1(R4)                                                 
         MVC   0(L'REWHGRP,R4),REWHGRP                                          
         CLC   REWHGRP,BCEFFS     IS THIS FOR ALL GROUPS?                       
         BNE   SCRL04             NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
         LA    R4,L'ALL(R4)                                                     
         B     *+8                                                              
SCRL04   LA    R4,L'REWHGRP(R4)                                                 
         MVI   0(R4),C','                                                       
         LA    R4,1(R4)                                                         
*                               ** OFFICE FILTER                                
         ...
         MVI   L'OFFICE(R4),C'='                                                
         LA    R4,L'OFFICE+1(R4)                                                
         MVC   0(L'REWHOFF,R4),REWHOFF                                          
         CLC   REWHOFF,BCEFFS     IS THIS FOR ALL OFFICES?                      
         BNE   *+10               NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
         LA    R4,L'ALL(R4)                                                     
         CLI   0(R4),C' '                                                       
...
*                               ** TEAM FILTER                                  
         ...
         MVI   L'TEAM(R4),C'='                                                  
         LA    R4,L'TEAM+1(R4)                                                  
         MVC   0(L'REWHTEAM,R4),REWHTEAM                                        
         CLC   REWHTEAM,BCEFFS    IS THIS FOR ALL TEAMS?                        
         BNE   SCRL06             NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
*                                                                               
SCRL06   LH    R1,LSHEDDSP         REDISPLAY HEADLINE FIELD                     
         A     R1,ATWA                                                          
         USING FHD,R1                                                           
         XR    RE,RE                                                            
         IC    RE,FHLN             SET INDEX IN RE                              
...
NLST02   L     RF,AIOREC           I/S RETURNS INTO RECORD BUFFER               
         CLC   0(REWHPAD-REWHKEY,RF),LSINIKEY                                   
         BNE   EXITL               MAKE SURE SAME REPORT/REP                    
*                                                                               
         CLC   =C'DATES=',REWHOFF-REWHKEY(RF)                                   
         BE    NLST                                                             
*                                                                               
...
FF       EQU   X'FF'                                                            
         SPACE 1                                                                
FILTTAB  DC    AL2(00001),AL1(REWHGRP-REWHDSCT,L'REWHGRP)                       
         DC    AL2(00003),AL1(REWHOFF-REWHDSCT,L'REWHOFF)                       
         DC    AL2(00008),AL1(REWHTEAM-REWHDSCT,L'REWHTEAM)                     
         DC    AL2(EOT)                                                         
         SPACE 1                                                                
...
*        DDFLDHDR                                                               
       ...
       ++INCLUDE DDFLDHDR                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'009REVIEW24  09/27/05'                                      
         END                                                                    
