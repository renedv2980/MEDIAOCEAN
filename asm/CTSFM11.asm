*          DATA SET CTSFM11    AT LEVEL 004 AS OF 05/01/02                      
*PHASE TA0A11A                                                                  
*                                                                               
***********************************************************************         
*                                                                     *         
...
*          SCREENS CTSFMFE6 (TA0AE6) -- LIST                          *         
*          SCREENS CTSFMFE9 (TA0AE9) -- REPORT                        *         
*                                                                     *         
...
*                                                                               
         ...
         BNE   *+8                                                              
         BAS   RE,LR                                                            
         CLI   MODE,PRINTREP       ONLINE PRINT REPORT                          
         BNE   *+8                                                              
         BAS   RE,PR                                                            
*                                                                               
         XIT1                                                                   
...
*                                                                               
         ...
         BNE   VKLIST                                                           
         LA    R2,SFRSYSH          DSECT FOR REPORT SCREEN                      
         CLI   5(R2),0             ANY DATA?                                    
         BNE   VKSYS               YES - SEE IF VALID SYSTEM.                   
         MVI   SYSTYPE,X'00'       NO SYSTEM INPUT                              
         B     VK10                SEE IF USER ENTERED MODULE NAME              
VKLIST   CLI   ACTNUM,ACTLIST      ACTION LIST?                                 
         BNE   *+8                                                              
...
*                                                                               
         ...
         B     SFMERROR            REQUIRED                                     
*                                                                               
VKSYS    MVC   SYSTYPE,8(R2)       REQUEST SYSTEM                               
         L     R5,ASYSLST                                                       
         USING SYSLSTD,R5                                                       
...
VK10     CLI   ACTNUM,ACTLIST      IS ACTION A LIST                             
         BE    VK15                                                             
         CLI   ACTNUM,ACTREP       IS ACTION A REPORT?                          
         BNE   VKPRGM      IF A MAINT. ACTION PROGRAM IS REQUIRED.              
         BE    VK20                PROGRAM IS NOT INPUT FOR A REPORT            
VK15     LA    R2,SFLPGMH                                                       
         MVC   PROGRAM,8(R2)                                                    
         LA    R2,SFLMODH                                                       
         ZIC   R5,5(R2)                                                         
...
* THEN GET DESCRIPTION ELEMENT, NUMBER 10                                       
DR       NTR1                                                                   
         LA    R7,SFMDESCH                                                      
         TWAXC (R7)                                                             
         LA    R7,SFMSEQH                                                       
...
*                                                                               
         ...
         B     *+10                                                             
LR40     MVC   LSTDESCR,SNDSCTXT       TRUNCATED MOVE, 40 CHARS                 
*                                                                               
*  ATTRIBUTES ELEMENT                                                           
         LA    R3,SOON1STL                                                      
         USING SNATTD,R3                                                        
...
* BUILD THE KEY                                                                 
         ...
         MVC   SOONKSYS,SYSTYPE    SYSTEM TYPE                                  
         MVC   SAVEKEY,KEY                                                      
         MVC   SAVESYS,SOONKSYS  SAVE SYSTEM TYPE FOR COMPARISON                
*                                                                               
* READ FIRST RECORD, PULL OUT SYSTEM NAME FROM TABLE.                           
PR10     GOTO1 HIGH                FIRST RECORD                                 
         ...
         L     R7,2(R5)            A(END OF TABLE)                              
         LA    R5,6(R5)            A(FIRST ENTRY)                               
         CLC   SYSLRPLT,SOONKSYS   VALID SYSTEM MATCH?                          
         BE    *+8                                                              
         BXLE  R5,R6,*-10          TRY AGAIN FOR MATCH                          
         MVC   SYSNAME,SYSLNAME                                                 
*                                                                               
PR20     CLI   SYSTYPE,X'00'     USER WANTS ALL THE SYSTEMS                     
         BNE   PR25                                                             
         CLC   KEY(2),SAVEKEY                                                   
         BNE   PREND                                                            
         BE    *+14                                                             
PR25     CLC   KEY(10),SAVEKEY   ANY MORE RECORDS OF SAME TYPE + SYS            
         BNE   PREND                                                            
         CLC   SAVESYS,SOONKSYS  SAME SYSTEM?                                   
         BE    PR28              YES, THEN DON'T NEED TO LOOK UP NAME.          
         MVI   FORCEHED,C'Y'     EJECT PAGE ON A SYSTEM CHANGE                  
         MVC   SAVESYS,SOONKSYS  SAVE THE NEW SYSTEM                            
*                                                                               
* LOOK UP NEW SYSTEM NAME FROM TABLE                                            
         L     R5,ASYSLST                                                       
         USING SYSLSTD,R5                                                       
         LH    R6,0(R5)            LENGTH OF TABLE ENTRY                        
         L     R7,2(R5)            A(END OF TABLE)                              
         ...
         MVC   SYSNAME,SYSLNAME                                                 
PR28     GOTO1 GETREC                                                           
         B     PRALLOW           FIGURE OUT NO. OF LINES FOR OUTPUT             
PR29     L     R4,AIO            SOON RECORD                                    
*                                                                               
         MVC   PRTPGM,SOONKPRG                                                  
         LA    R3,SOON1STL       ADDR OF FIRST ELEMENT                          
         USING SNDSCCD,R3        DESCRIPTION  ELEMENT                           
         ...
         EX    R1,*+8                                                           
         B     *+10                                                             
         MVC   PRTDESCR(0),SNDSCTXT     DESCRIPTION                             
*                                                                               
*  ATTRIBUTES ELEMENT                                                           
         LA    R3,SOON1STL                                                      
         USING SNATTD,R3                                                        
         ...
         BE    *+6                                                              
         DC    H'0'                                                             
         MVC   PRTMOD,SNATTLM      MODULE NAME TO OUPUT                         
         L     R1,MODCOUNT                                                      
         C     R1,=F'0'            DID USER ENTER MODULE INPUT?                 
         BE    PR30                                                             
         BCTR  R1,0                                                             
         ...
         B     *+10                                                             
         CLC   SNATTLM(0),MODULE    IS THIS THE MODULE THE USER WANTS?          
         BNE   PR200             DID NOT PASS FILTER TEST                       
PR30     MVC   PRTPRTY,SNATTPRT  MOVE PRIORITY TO OUTPUT                        
         EDIT  SNATTTIM,(5,PRTTIME),ALIGN=LEFT     SECONDS                      
         MVC   PRTMODH,=C'MODULE='                                              
         MVC   PRTTIMEH,=C',TIME='                                              
         MVC   PRTPRTYH,=C',PRIORITY='                                          
         GOTO1 SQUASHER,DMCB,P2,132                                             
         GOTO1 SPOOL,DMCB,(R8)   SEND OUPTUT TO REPORT                          
*                                                                               
* CONTROL CARD ELEMENTS                                                         
         ...
         DC    H'0'                                                             
*                                                                               
* FILL IN TEXT FROM CONTROL CARDS                                               
PR100    ZIC   R1,SNCRDLEN         LENGTH OF ELEMENT                            
         LA    R6,SNCRDOV          OVERHEAD LENGTH                              
         SR    R1,R6               LENGTH OF CONTROL CARD TEXT                  
         LTR   R1,R1               IS THERE CARD INPUT                          
         BZ    PREND               SHOULD BE CARD INPUT                         
         BCTR  R1,0                                                             
         EX    R1,*+8                                                           
         B     *+10                                                             
         MVC   P+5(0),SNCRDTXT    CARD TEXT LINE                                
         GOTO1 SPOOL,DMCB,(R8)   SEND OUPTUT TO REPORT                          
...
* GET NEXT TEXT FIELDS                                                          
         ...
         BE    PR100                                                            
*                                                                               
* GET ANOTHER RECORD, AND DO SPACING                                            
         ZIC   R5,LINE       SIZE OF LINE SO FAR                                
         AH    R5,=H'3'      NEED AT LEAST 3 SPACES LEFT ON PAPER               
         ZIC   R6,MAXLINES   MAX NO OF LINES ON PAPER                           
         SR    R6,R5         SHOULD WE DO SPACING?                              
         BNP   PR200                                                            
         MVC   P1,SPACES                                                        
         MVC   P2,SPACES                                                        
         MVI   SPACING,X'02'                                                    
         GOTO1 SPOOL,DMCB,(R8)   FOR SPACING                                    
PR200    GOTO1 SEQ               NEXT RECORD                                    
         LA    R4,KEY            POINT R4 BACK TO KEY                           
         B     PR20                                                             
         DROP  R4                                                               
* COUNT NUMBER OF LINES OF CARD INPUT, IN ORDER TO SET ALLOWLIN                 
*                                                                               
PRALLOW  SR    R6,R6              LINE COUNT                                    
         L     R3,AIO             A(FIRST ELEMENT)                              
         USING SNCRDD,R3                                                        
         MVI   ELCODE,SNCRDCDQ    ELEMENT CODE 30                               
         BAS   RE,GETEL                                                         
...
*                                                                               
* GET NEXT TEXT FIELDS                                                          
PRLOOP   LA    R6,1(R6)           ADD TO LINE COUNT                             
         BAS   RE,NEXTEL           GET NEXT CONTROL CARD                        
         BE    PRLOOP                                                           
         LA    R6,2(R6)           THE OTHER 2 LINES OF OUTPUT                   
         STC   R6,ALLOWLIN                                                      
         B     PR29                                                             
PREND    XIT1                                                                   
         EJECT                                                                  
HDHOOK   NTR1                                                                   
         MVC   H1+55(19),=C'SOON RECORD REPORT'                                 
         MVI   H2+55,X'BF'  UNDERLINE CHAR                                      
         MVC   H2+56(18),H2+55                                                  
         MVC   H4+8(1),SAVESYS       SYSTEM TYPE                                
         MVC   H4+10(7),SYSNAME      SYSTEM NAME                                
         XIT1                                                                   
         SPACE 3                                                                
...
SAVESYS  DS    X                   SAVED SYTEM TYPE                             
CARDCNT  DS    X                   LINE COUNT                                   
SYSNAME  DS    CL7                 SYSTEM NAME FROM TABLE                       
...
PRTPRTY  DS    CL1                                                              
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'004CTSFM11   05/01/02'                                      
         END                                                                    
