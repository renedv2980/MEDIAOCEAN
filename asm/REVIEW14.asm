*          DATA SET REVIEW14   AT LEVEL 006 AS OF 09/14/05                      
*          DATA SET REVIEW14   AT LEVEL 004 AS OF 11/19/99                      
*PHASE T81714A                                                                  
VIEW14   TITLE 'K4 REPORT FOR VIEWER'                                           
VIEW14   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,REVIEW14,R7,RR=RE                                              
         USING TWAD,RA                                                          
         USING WORKD,R9                                                         
         L     R8,AGWORK                                                        
         USING GWORKD,R8                                                        
...
***********************************************************************         
         SPACE 1                                                                
DISGRP   MVC   FVIFLD(L'REK4GRP),FLTIFLD                                        
         B     EXITOK                                                           
         SPACE 1                                                                
***********************************************************************         
...
***********************************************************************         
         SPACE 1                                                                
DOFQTR   OC    REK4TOT,REK4TOT                                                  
         BZ    FLTXX               NOT A TOTAL LINE                             
*                                                                               
         CLC   REK4PRD+1(1),FLTIFLD                                             
         BL    FLTXL                                                            
         BE    FLTXE                                                            
         BH    FLTXH                                                            
         DROP  R2                                                               
...
*                                                                               
         LA    R4,FVIFLD           FIRST LINE IS HEADLINES                      
         USING LINE1D,R4                                                        
...
*                                                                               
         ...
         MVCL  R0,RE                                                            
*                                                                               
         MVC   IOKEY,REK4KEY       SET CURRENT RECORD INTO IO2                  
...
*                                                                               
         MVC   IOKEY,0(R4)         SET KEY IN IOKEY                             
         CLC   REK4DSCT(REK4RNK-REK4DSCT),REC.REK4DSCT                          
...
*                                                                               
         ...
         CLC   REK4ADAG-REK4KEY(L'REK4ADAG,RF),REC.REK4ADAG                     
         BNE   DLIN04              YES                                          
*                                                                               
...
**       OI    FLD.FHOI,FHOITR                                                  
         ...
         CLC   REC.REK4ADV,BCEFFS  ALL/ALL?                                     
         BNE   *+14                NO                                           
         CLC   REC.REK4AGY,BCEFFS  DON'T SHOW SECOND 'ALL'                      
         BE    DLIN06                                                           
         GOTOX DOAGY,BOPARM,AIO1                                                
         MVC   L1.L2AGY,BOWORK1                                                 
...
*                                                                               
DLIN08   DS    0H                                                               
**       CH    R5,=H'2'            MUST BE 2 LINES FOR NEXT DISPLAY             
**       BL    EXITOK                                                           
*                                                                               
         L     R0,AIO1                                                          
         L     RE,AIO2                                                          
...
*                                                                               
         L     R4,AIO1                                                          
         CLC   REC.REK4AGY,BCEFFS  RANK - NO DISPLAY FOR 'ALL'                  
         BNE   *+14                                                             
         CLC   REC.REK4ADV,BCEFFS                                               
         BE    DLIN10                                                           
*                                                                               
         XR    RF,RF                                                            
         ICM   RF,15,REC.REK4RNK                                                
...
**       DROP  FLD,R2,REC                                                       
         ...
         EJECT                                                                  
***********************************************************************         
* ROUTINE TO GET DATES FOR THIS PARTICULAR REPORT                     *         
***********************************************************************         
         SPACE 1                                                                
GETDATE  NTR1  ,                                                                
         XC    IOKEY,IOKEY                                                      
X        USING REK4KEY,IOKEY                                                    
         L     RF,AIOREC                                                        
         USING REK4KEY,RF                                                       
         MVC   X.REK4RPT,=AL2(REK4RPTQ)                                         
         MVC   X.REK4REP,CUAALF                                                 
...
X        USING RAGYRECD,IOKEY                                                   
         MVI   X.RAGYKTYP,X'0A'                                                 
         MVC   X.RAGYKAGY,REK4AGY                                               
         MVC   X.RAGYKAOF,REK4AGY+L'RAGYKAGY                                    
...
*                               ** PERIOD IS YYMM BINARY                        
         ...
         CLI   REK4PRD+1,X'0D'      IF MM IS 'OD' THIS IS A YRLY TOTAL          
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         MVC   L1PED(L'ALL),ALL                                                 
         CLI   REK4PRD+1,FF         IF MM IS 'FF' PUT OUT 'ALL'                 
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         ...
         MVI   L1PED+2,C'4'                                                     
*                                                                               
DOL00    OC    REK4TOT,REK4TOT      IF SET THIS IS SUB-TOTAL LINE               
         BNZ   DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         XR    RF,RF                                                            
...
SCRL02   MVC   0(L'GROUP,R4),GROUP                                              
         MVI   L'GROUP(R4),C'='                                                 
         LA    R4,L'GROUP+1(R4)                                                 
         MVC   0(L'REK4GRP,R4),REK4GRP                                          
         CLC   REK4GRP,BCSPACES   IS THIS FOR ALL GROUPS?                       
         BH    SCRL04             NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
         LA    R4,L'ALL(R4)                                                     
         B     *+8                                                              
SCRL04   LA    R4,L'REK4GRP(R4)                                                 
         MVI   0(R4),C','                                                       
         LA    R4,1(R4)                                                         
*                               ** STATION FILTER                               
...
NLST02   L     RF,AIOREC           I/S RETURNS INTO RECORD BUFFER               
         CLC   0(REK4GRP-REK4KEY,RF),LSINIKEY                                   
         BNE   EXITL               MAKE SURE SAME REPORT/REP                    
*                                                                               
         CLC   =C'DATES=',REK4STN-REK4KEY(RF)                                   
         BE    NLST                                                             
*                                                                               
...
FF       EQU   X'FF'                                                            
         SPACE 1                                                                
FILTTAB  DC    AL2(00001),AL1(REK4GRP-REK4DSCT,L'REK4GRP)                       
         DC    AL2(00002),AL1(REK4STN-REK4DSCT,L'REK4STN)                       
         DC    AL2(00005),AL1(REK4RNK-REK4DSCT,L'REK4RNK)                       
         DC    AL2(EOT)                                                         
         SPACE 1                                                                
...
*        DDFLDHDR                                                               
       ...
       ++INCLUDE DDFLDHDR                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'006REVIEW14  09/14/05'                                      
         END                                                                    
