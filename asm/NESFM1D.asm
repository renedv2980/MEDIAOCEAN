*          DATA SET NESFM1D    AT LEVEL 002 AS OF 10/31/05                      
*          DATA SET NESFM1C    AT LEVEL 087 AS OF 05/01/02                      
*PHASE T31C1DA                                                                  
         TITLE 'T31C1D  -PGSPB- SPLIT BILLING RECORDS FOR PG'                   
T31C1D   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,T31C1D                                                         
         L     RC,0(R1)                                                         
...
VK5      LA    R2,SLMOPTH          OPTIONS                                      
         ...
         CLI   5(R2),8                                                          
         BH    EINV                                                             
         MVC   OPTPROD,13(R2)       SAVE PRODUCT FILTER                         
         OI    OPTPROD+2,X'40'                                                  
*                                                                               
...
*                                                                               
         ...
         CLC   AGENCY,=CL2'DU'     CHECK FOR MEDIAVEST                          
         BNE   *+8                                                              
         LA    R0,NPRODSDU         R0=MAX N'PRODUCTS FOR MEDIAVEST              
         ...
         B     VR3                                                              
*                                                                               
VR2      XC    ELEM,ELEM           BUILD PRODUCT ELEMENTS                       
         MVI   NECELCD,NECELCDQ                                                 
         MVI   NECELLN,NECELLNQ                                                 
         CLI   5(R4),0             TEST END OF PRODUCTS                         
         BE    VR12                                                             
*                                                                               
...
*                                                                               
         XC    ELEM,ELEM           BUILD PRODUCT EXTRA ELEMENT                  
         USING NEXEL,R6                                                         
         MVI   NEXELCD,NEXELCDQ                                                 
         MVI   NEXELLN,NEXELLNQ                                                 
         STC   R3,NEXSEQ           SET SEQUENCE NUMBER                          
*                                                                               
         LA    R2,SFMPGB1H         VALIDATE PG BRAND                            
         MVC   NEXPGPR,8(R2)                                                    
         LA    R2,SFMSUF1H         VALIDATE BRAND SUFFIX                        
         CLI   5(R2),2             MUST HAVE A LENGTH OF 2                      
         BNE   EINV                                                             
         TM    4(R2),X'04'         MUST BE ALPHA                                
         BZ    EINV                                                             
         MVC   NEXSUFF,8(R2)                                                    
...
DR1C     LA    R2,SFMPRD1H                                                      
         ...
         CLC   AGENCY,=CL2'DU'     CHECK FOR MEDIAVEST                          
         BNE   *+8                                                              
         LA    R7,NPRODSDU         R0=MAX N'PRODUCTS FOR MEDIAVEST              
...
DR5      OI    SFMRATEH+6,X'80'                                                 
         ...
         DROP  R2                                                               
*                                                                               
*  DISPLAY PG BRAND/BRAND SUUFIX FROM 06 ELEMENT                                
*                                                                               
DR7      LA    R2,SFMPRD1H                                                      
         USING SFMPRD1H,R2                                                      
         MVI   ELCODE,X'06'                                                     
         ...
         USING NEXEL,R6                                                         
*                                                                               
DR8      MVC   SFMPGB1,NEXPGPR     PG BRAND                                     
         OI    SFMPGB1H+6,X'80'                                                 
*                                                                               
         MVC   SFMSUF1,NEXSUFF     PG SUFFIX                                    
         OI    SFMSUF1H+6,X'80'                                                 
*                                                                               
         LA    R2,SFMPRD2H-SFMPRD1H(R2)                                         
         BAS   RE,NEXTEL                                                        
         BE    DR8                                                              
*                                                                               
DR10     CLI   SFMTYP,C'X'         IF SHOWING DOLLARS AS PCT                    
         BNE   DRX                                                              
...
LR5      EX    R3,*+8                                                           
         ...
         GOTO1 GETREC                                                           
*                                                                               
* PG SPB MUST HAVE AN 06 ELEMENT TO LIST                                        
         L     R6,AIO                                                           
         MVI   ELCODE,X'06'                                                     
         BAS   RE,GETEL                                                         
         BNE   LR4                                                              
*                                                                               
* SEE IF PRODUCT FILTER IS SET                                                  
         OC    OPTPROD,OPTPROD                                                  
         BZ    LR7                                                              
         L     R6,AIO                                                           
         USING NECEL,R6                                                         
...
*                                                                               
         ...
         BE    *+6                                                              
         DC    H'0'                                                             
LR8      OC    OPTPROD,OPTPROD                                                  
         BZ    *+8                                                              
         ICM   R6,15,HOLDELEM      USE SAVED ELEMENT ADDRESS                    
         MVC   0(3,R5),NECPRD                                                   
         LA    R5,2(R5)                                                         
         CLI   0(R5),X'40'                                                      
         BNH   *+8                                                              
...
LR10     MVC   WORK(9),=X'402021204B20202020'         PERCENT                   
         ...
         MVC   0(7,R5),WORK+2                                                   
         LA    R5,7(R5)                                                         
LR10D    OC    OPTPROD,OPTPROD     CHECK PROD FILTER OPTION                     
         BNZ   LR15                                                             
         C     R7,=F'4'            4=MAX PRODS FOR DISPLAY                      
         BE    LR10X                                                            
         BAS   RE,NEXTEL                                                        
         BNE   LR10X                                                            
         MVI   0(R5),C','                                                       
...
LR10X    GOTO1 LISTMON                                                          
         B     LR4                                                              
*                                                                               
*  MOVE OUT PG BRAND AND SUUFIX OUT WHEN PRODUCT FILTER REQUESTED               
*                                                                               
LR15     MVC   BYTE,NECSEQ                                                      
         MVI   0(R5),C','                                                       
         LA    R5,1(R5)            BUMP LISTAR                                  
*                                                                               
*  FIND 06 ELEM THAT HAS THE SAME SEQUENCE NUMBER                               
*  AS THE ABOVE 04 ELEMENT                                                      
*                                                                               
         L     R6,AIO                                                           
         MVI   ELCODE,X'06'                                                     
         USING NEXEL,R6                                                         
         ...
         BE    *+6                                                              
         DC    H'0'                                                             
LR16     CLC   NEXSEQ,BYTE         FIND CORRECT SEQUENCE NUMBER                 
         BE    LR17                                                             
         BAS   RE,NEXTEL                                                        
         BE    LR16                                                             
         DC    H'0'                                                             
*                                                                               
LR17     MVC   0(4,R5),NEXPGPR     PG BRAND                                     
LR18     CLI   0(R5),X'40'                                                      
         BNH   LR19                                                             
         LA    R5,1(R5)                                                         
         B     LR18                                                             
LR19     MVI   0(R5),C','                                                       
         MVC   1(2,R5),NEXSUFF     BRAND SUFFIX                                 
*                                                                               
         GOTO1 LISTMON                                                          
*                                                                               
LR20     DS    0H                                                               
...
MSGERR   MVI   ERROR,0                                                          
         ...
         LTORG                                                                  
         EJECT                                                                  
NPRODS   EQU   5                   MAX PRODUCTS PER SCREEN                      
...
PRDHDRD  DSECT                                                                  
       ...
       ++INCLUDE SPGENBILL                                                      
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'002NESFM1D   10/31/05'                                      
         END                                                                    
