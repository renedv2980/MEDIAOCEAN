*          DATA SET REROM00    AT LEVEL 065 AS OF 02/03/05                      
*PHASE T83F00C,*                                                                
*INCLUDE RECUP                                                                  
*INCLUDE OUTDAY                                                                 
*INCLUDE REGENDAB                                                               
...
*INCLUDE REDARTKO                                                               
*INCLUDE KHDUMMY                                                                
         TITLE 'T83F00 - REROM00 - REP DARE'                                    
*                                                                               
*******************************************************************             
*                                                                 *             
*    REROM00 --- REP DOCUMENT AUTOMATIC RETRIEVAL EXCHANGE        *             
*                                                                 *             
* --------------------------------------------------------------- *             
* UPDATE HISTORY:                                                 *             
*                                                                 *             
* FEB02/05 HQ  FIX MASTER DUMP WHEN VIEWING SUBREP CONTRACT IN CON*             
* FEB13/03 BU  NEW VERSION: SINGLE LINE REDI USING REDAREA SCRN   *             
* FEB13/03 BU  NEW VERSION: SINGLE LINE REDI USING REDAREA SCRN   *             
* NOV18/02 HQ  HANDLE GLOBBER CONTRACT CONFIRM ERROR RETURN       *             
* OCT17/02 BU  REDIRECT TO RADIO IN-BOX ON OPTION                 *             
*          SKU ADD AMEND FUNCTIONALITY AND SPECIAL OPEN PROCESSING*             
...
T83F00   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 LENWORK,**T83F00,R7,R5,RR=R2,CLEAR=YES                           
         USING GEND,RC                                                          
...
*                                                                               
* IF USER SIGNED IN AS MASTER, REPLACE AGENCY (REP CODE) WITH THE               
* SUBSIDIARY REP CODE IF RECORD/ACTION IS ANYTHING OTHER THAN ORD/LIS           
*                                                                               
         LR    R4,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R4,DARPROFS-CONHEADH                                             
         USING SVDSECT,R4                                                       
         NI    DMISFLGX,X'FF'-X'40'-X'20'                                       
*                                                                               
         CLI   TWAMODE,1           ONLY VALIDATE THIS FOR ONLINE                
         BE    DAMN0003                                                         
         CLC   =C'UND',CONREC       UNDO IS DDS ONLY                            
         BNE   DAMN0003                                                         
         CLI   DDS,C'Y'                                                         
         BE    DAMN0003                                                         
         LA    R3,5                                                             
         L     R6,ACOMFACS                                                      
         USING COMFACSD,R6                                                      
         GOTO1 CGETTXT,DMCB+12,(R3),0,(C'E',DMCB),0,0,0                         
         L     RD,SAVERD           BACK OUT ALL THE WAY                         
         DROP  R6                                                               
         XIT1                                                                   
*                                                                               
DAMN0003 DS    0H                                                               
         GOTO1 =A(ISMAST),RR=RELO                                               
         BNE   DAMN0010            MASTER REP SIGNED ON?                        
         OI    DMISFLGX,X'40'      FLAG I AM MASTER                             
*                                                                               
         CLC   =C'ORD',CONREC                                                   
         BNE   DAMN0005                                                         
         CLC   =C'LIS',CONACT                                                   
...
*                                                                               
DAMN0005 DS    0H                                                               
         OC    SELECTKY,SELECTKY   DO WE HAVE THE SUBSIDIARY ORDER KEY?         
         BZ    DAMN0010                                                         
*                                                                               
DAMN0008 DS    0H                                                               
DARKEYD  USING RDARKEY,SELECTKY    FAKE OUT THE REST OF THE PROGRAM             
         MVC   TWAAGY,DARKEYD.RDARKREP                                          
         OI    DMISFLGX,X'20'      WITH THE SUB REP'S CODE                      
         DROP  DARKEYD,R4                                                       
*                                                                               
DAMN0010 DS    0H                                                               
...
DAMN0040 DS    0H                  EXPECTING REC/ACT ORDER/LIST                 
         ZIC   R1,CONRECH+5        EXCEPT FOR MAKEGOOD                          
         LTR   R1,R1               NO INPUT, FORCE ORDER/LIST                   
         BZ    DAMN0280                                                         
         BCTR  R1,0                                                             
         EX    R1,DAMN0060         LOOK FOR MGGROUP                             
         BE    DAMN0200                                                         
         ...
         BE    DAMN0200                                                         
         EX    R1,DAMN0065         LOOK FOR TODO                                
         BE    DAMN0200                                                         
         EX    R1,DAMN0066         LOOK FOR TODO                                
         BE    DAMN0200                                                         
         B     DAMN0080                                                         
DAMN0060 CLC   CONREC(0),=C'MGGROUP'                                            
DAMN0061 CLC   CONREC(0),=C'CAMPAIGN'                                           
DAMN0062 CLC   CONREC(0),=C'BUYER'                                              
DAMN0063 CLC   CONREC(0),=C'FLIGHT'                                             
DAMN0064 CLC   CONREC(0),=C'SENTDATE'                                           
DAMN0065 CLC   CONREC(0),=C'TODO    '                                           
DAMN0066 CLC   CONREC(0),=C'UNDO    '                                           
                                                                                
DAMN0080 DS    0H                  SOFT MATCH ON ORDER?                         
         EX    R1,DAMN0100                                                      
         BNE   DAMN0120                                                         
         B     DAMN0200                                                         
DAMN0100 CLC   CONREC(0),=C'ORDER'                                              
                                                                                
DAMN0120 DS    0H                  SOFT MATCH ON ORDER?                         
         EX    R1,DAMN0140                                                      
         BE    DAMN0160                                                         
*                                                                               
*  CHANGING THIS CODING IS DANGEROUS, BECAUSE IT MAY HAVE SOME                  
*        IMPACT ON DARE, UNTIL THIS IS RUNNING AS A SEPARATE                    
*        PROGRAM (ROM).                                                         
*                                                                               
*                                                                               
         LA    R3,956                                                           
*                                                                               
         ...
         XIT1                                                                   
         DROP  R6                                                               
DAMN0140 CLC   CONREC(0),=C'REVISION'                                           
                                                                                
DAMN0160 DS    0H                  SOFT MATCH ON REPORT?                        
         ZIC   R1,CONACTH+5                                                     
         LTR   R1,R1               NO INPUT, FORCE ORDER/LIST                   
         BZ    DAMN0280                                                         
         BCTR  R1,0                                                             
         ...
         BNE   DAMN0280                                                         
*                                                                               
         CLI   TWALREC,MYACTORD    ONLY ON ORDER/SELECT SCREEN                  
         BNE   DAMN0280                                                         
         CLI   TWALACT,MYACTSEL                                                 
         BNE   DAMN0280                                                         
         MVI   GOAGAIN,C'Y'        SET GO AGAIN SO THAT PROGRAM                 
         MVI   LISTNUM,0           WILL RUN AFTER GENCON SET SCREEN             
         B     AGAIN10                                                          
*                                                                               
DAMN0180 CLC   CONACT(0),=C'REPORT'                                             
DAMN0190 CLC   CONACT(0),=C'AMEND'                                              
DAMN0195 CLC   CONACT(0),=C'#OPEN'                                              
DAMN0198 CLC   CONACT(0),=C'DETAILS'                                            
                                                                                
DAMN0200 DS    0H                  SOFT MATCH ON LIST?                          
         ZIC   R1,CONACTH+5                                                     
         LTR   R1,R1               NO INPUT, FORCE ORDER/LIST                   
         BZ    DAMN0280                                                         
         BCTR  R1,0                                                             
         ...
         B     DAMN0280                                                         
*                                                                               
DAMN0220 CLC   CONACT(0),=C'LIST'                                               
DAMN0230 CLC   CONACT(0),=C'REPORT'                                             
DAMN0240 CLC   CONACT(0),=C'DISPLAY'                                            
*                                                                               
DAMN0260 DS    0H                                                               
         MVI   LISTNUM,0           IN CASE WE CAME FROM SELECT SCREEN           
         B     DAMN0360                                                         
                                                                                
DAMN0280 DS    0H                  IF NOT, DEFAULT ORDER/LIST                   
         XC    CONREC,CONREC                                                    
         MVC   CONREC,=C'ORDER   '                                              
         MVI   CONRECH+5,8                                                      
...
* WHEN SIGNONID IS 4 CHARS OR LESS                                              
         ...
         BE    DAMN0520                                                         
*                                                                               
DAMN0380 DS    0H                  SAME FOR MAKEGOOD                            
...
*                                                                               
         ...
         BE    DAMN0520                                                         
*                                                                               
DAMN0400 DS    0H                  SAME FOR MAKEGOOD                            
*                                                                               
*   THIS SECTION USES R3 AS A POINTER TO THE PROPER OFFICE FIELD ON             
*        EITHER THE ORIGINAL OR THE RADIO IN-BOX SCREEN.  DO NOT                
*        STEP ON IT, PLEASE.                                                    
*                                                                               
         LA    R3,DRFOFFH          SET A(ORIG INBOX OFFICE)                     
*                                                                               
         LR    R4,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R4,DARPROFS+12-CONHEADH                                          
*                                  SET TO A(DARE PROFILE)                       
         USING SVDSECT,R4                                                       
         TM    SVPGPBIT+DARINBXB,DARINBXA                                       
*                                  USE RADIO IN-BOX?                            
         BZ    DAMN0420            NO  - USE ORIGINAL SCREEN FIELDS             
         DROP  R4                                                               
         LA    R3,DE1OFFH          SET A(RADIO INBOX OFFICE)                    
*                                                                               
         CLC   =C'ORD',CONREC                                                   
         BNE   DAMN0420                                                         
         CLC   =C'LIS',CONACT                                                   
...
*                                                                               
*                                                                               
*   TURN ON 'UNWIRED' IF MASTER SIGNON                                          
*                                                                               
         LR    R4,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R4,DARPROFS-CONHEADH                                             
         USING SVDSECT,R4                                                       
         ...
         DROP  R4                                                               
         LA    RF,DE1UNWH                                                       
         MVI   8(RF),C'Y'          SET TO UNWIRED                               
         OI    6(RF),X'80'         TURN ON 'TRANSMIT NOW'                       
         B     DAMN0420                                                         
*                                                                               
DAMN0405 EQU   *                                                                
         CLC   =C'K3',AGENCY       MASTER SIGNON: KATZ RADIO?                   
         BE    DAMN0420            YES                                          
         CLC   =C'IR',AGENCY       MASTER SIGNON: INTEREP?                      
         BE    DAMN0420            YES                                          
         CLC   =C'MR',AGENCY       MASTER SIGNON: KATZ TV?                      
         BE    DAMN0420            YES                                          
         CLC   =C'MS',AGENCY       MASTER SIGNON: TEST?                         
         BE    DAMN0420            YES                                          
*                                                                               
*   NOT A MASTER: TURN OFF UNWIRED INDICATOR                                    
*                                                                               
         LA    RF,DE1UNWH                                                       
         TM    DE1UNWH+4,X'80'     ENTERED THIS TIME?                           
         BO    DAMN0410            YES - LEAVE AS ENTERED                       
         TM    DE1UNWH+4,X'20'     PREVIOUSLY VALID?                            
         BO    DAMN0410            YES - LEAVE AS ENTERED                       
         MVI   8(RF),C' '          SET TO NOT UNWIRED                           
DAMN0410 EQU   *                                                                
*                                                                               
         LA    RF,DE1REPH          SET A(REP/COMPANY FIELD)                     
*        OI    1(RF),X'20'         TURN ON 'PROTECTED' BIT                      
         MVC   8(2,RF),AGENCY      FORCE SIGNON AGENCY CODE                     
         OI    6(RF),X'80'         TURN ON 'TRANSMIT NOW'                       
DAMN0420 EQU   *                                                                
         CLC   =C'ORD',CONREC                                                   
         BNE   DAMN0460                                                         
         CLC   =C'LIS',CONACT                                                   
         BNE   DAMN0600                                                         
         CLI   5(R3),0             AND FIRST TIME                               
         BNE   DAMN0500                                                         
*        TM    4(R3),X'20'         FORCE OFFICE LIMIT ACCESS                    
*        BO    DAMN0500                                                         
         CLC   =C'O=',TWAACCS                                                   
         BNE   DAMN0500                                                         
         MVC   8(2,R3),TWAACCS+2   IN THE OFFICE SCRN FIELD                     
*                                                                               
...
*                                                                               
         ...
         CLC   =C'PV',AGENCY                                                    
         BE    DAMN0500                                                         
DAMN0440 DS    0H                  SAME FOR MAKEGOOD                            
         CLC   =C'DV',8(R3)                                                     
         BNE   *+10                                                             
         MVC   8(2,R3),=C'DN'                                                   
         MVI   5(R3),2                                                          
         B     DAMN0500                                                         
*                                                                               
*   END OF SECTION USING R3 AS POINTER.                                         
*                                                                               
                                                                                
DAMN0460 DS    0H                  SAME FOR MAKEGOOD                            
         ...
         CLC   =C'DIS',CONACT                                                   
         BNE   DAMN0500                                                         
DAMN0480 CLI   MKGOFFH+5,0         AND FIRST TIME                               
         BNE   DAMN0500                                                         
...
*                                  SET A(DARE PROFILE)                          
         USING SVDSECT,R4                                                       
         LA    R3,DRFSTATH         SET A(ORIGINAL INBOX FIELD)                  
*                                                                               
         TM    SVPGPBIT+DARINBXB,DARINBXA                                       
         BZ    DAMN0540                                                         
         LA    R3,DE1STATH         SET A(RADIO    INBOX FIELD)                  
         DROP  R4                                                               
DAMN0540 DS    0H                                                               
         CLI   5(R3),0             AND FIRST TIME                               
         BNE   DAMN0600                                                         
...
*        MVI   MAXIOS,NIOS         USES 3 I/O AREAS                             
         ...
         MVC   SYSPHASE,=X'D9083F00'    PRESET FOR SYSTEM CALLOVS               
...
*                                                                               
         GOTO1 CALLOV,DMCB,0,X'D9083F24',0                                      
         MVC   VREDAR24,0(R1)      ESTABLISH REROM24 OVERLAY                    
*                                                                               
SYSXIT   DS    0H                                                               
         B     XIT                                                              
         EJECT                                                                  
...
*                                                                               
         ...
         USING GLCONNUM,R6                                                      
         XC    ELEM,ELEM                                                        
         GOTO1 CGLOBBER,DMCB,=C'GETD',ELEM,GLCONLNQ,GLRKACT                     
         TM    DMCB+8,X'10'        DON'T HAVE CONTRACT ACTION ELEMENT           
         BO    CKGLOB05                                                         
*                                                                               
         ...
         BE    *+6                                                              
         DC    H'0'                                                             
         CLC   =C'CF ',GLCONCA       CONTRACT CONFIRM?                          
         BNE   CKGLOB05                                                         
*                                                                               
         SR    R3,R3                                                            
         OC    GLCONERR,GLCONERR    IS THERE AN ERROR?                          
         BZ    CKG03                NO                                          
*                                                                               
         ICM   R3,3,GLCONERR                                                    
         GOTO1 CGETTXT,DMCB+12,(R3),0,(C'E',DMCB),0,0,0                         
...
*                                                                               
CKG03    DS    0H                                                               
         OC    GLCONINF,GLCONINF    IS THERE AN INFO?                           
         BZ    CKGLOB05             NO                                          
         ICM   R3,3,GLCONINF                                                    
         GOTO1 CGETTXT,DMCB+12,(R3),0,(C'I',DMCB),0,0,0                         
*                                                                               
CKG04    DS    0H                                                               
         OI    CONHEADH+6,X'80'     TRANSMIT HEADER                             
         L     RD,SAVERD           BACK OUT ALL THE WAY                         
...
*                                                                               
CKGLOB10 DS    0H                                                               
         CLC   =C'*A',FULL         AMEND ACTION TRIGGER?                        
         BE    CKGLOB20                                                         
         CLC   =C'*O',FULL         SPECIAL OPEN ACTION TRIGGER?                 
         BE    CKGLOB20                                                         
*                                                                               
         ZIC   R1,CONKEYH+5                                                     
         AHI   R1,1                                                             
...
*                                                                               
         ...
         MVI   CONACTH+5,4                                                      
*                                                                               
         CLI   FULL,C'*'           SPECIAL TRIGGERS?                            
         BNE   CKGLZ                                                            
         XC    CONREC,CONREC                                                    
         XC    CONACT,CONACT                                                    
         MVC   CONREC(8),=C'REVISION'                                           
         MVI   CONRECH+5,8                                                      
*                                                                               
         CLC   =C'*A',FULL         SPECIAL AMEND TRIGGER?                       
         BNE   CKGLOB30                                                         
         MVC   CONACT(5),=C'AMEND'                                              
         MVI   CONACTH+5,5                                                      
         B     CKGLOB40                                                         
*                                                                               
CKGLOB30 DS    0H                                                               
         CLC   =C'*O',FULL         SPECIAL OPEN TRIGGER?                        
         BE    *+6                                                              
         DC    H'0'                                                             
         MVC   CONACT(5),=C'#OPEN'                                              
         MVI   CONACTH+5,5                                                      
*                                                                               
CKGLOB40 DS    0H                                                               
         LR    R3,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R3,DARPROFS-CONHEADH                                             
         USING SVDSECT,R3                                                       
         TM    DMISFLGX,X'40'      AM I MASTER?                                 
         BZ    CKGLZ                                                            
         MVC   TWAAGY,FULL+2       USE CALLER'S REP CODE                        
         OI    DMISFLGX,X'20'      SET I'M USING SUB REP'S CODE                 
         B     CKGLZ                                                            
         DROP  R3                                                               
*                                                                               
*        CKGLOB EXIT                                                            
...
*                                                                               
SETP005  EQU   *                                                                
         XC    SVPGENTY(24),SVPGENTY                                            
*                                  CLEAR BOTH PROFILE SAVE AREAS                
         CLI   TWAOFFC,C'*'        DDS TERMINAL?                                
         BNE   SETP010                                                          
         MVC   SVPGREP(2),AGENCY   INITIALIZE CONTRACT PROFILES                 
         MVI   SVPGP#,RREPQCNT                                                  
****>>   MVC   SVPGPBIT,=8X'FF'                                                 
*                                                                               
*   DON'T SET ALL BITS FOR DDS TERMINAL!!                                       
*                                                                               
         MVC   SVPGREP+12(12),SVPGREP                                           
*                                  INITIALIZE DARE PROFILES                     
         MVI   SVPGP#+12,RREPQDAR  SET TO DARE PROFILE VALUE                    
SETP010  EQU   *                                                                
         XC    KEY,KEY                                                          
         MVI   KEY,X'01'                                                        
         MVC   KEY+25(2),AGENCY                                                 
...
SETP050  EQU   *                                                                
         ...
         BE    SETP060                                                          
         CLI   0(RE),RREPQDAR      LOOKING FOR DARE                             
         BE    SETP080                                                          
SETP055  EQU   *                                                                
...
SETP060  MVC   SVPGPBIT(8),2(RE)           SAVE UNIT IN TWA.                    
         B     SETP055             GO BACK FOR DARE PROFILE                     
*                                                                               
SETP080  MVC   SVPGPBIT+12(8),2(RE)        SAVE UNIT IN TWA.                    
*                                  BOTH PROFILES SAVED                          
SETP100  EQU   *                                                                
         LA    R2,12(R2)           BUMP TO SECOND PROFILE IN AREA               
*                                     R2 STILL COVERING DSECT                   
         TM    SVPGPBIT+DARINBXB,DARINBXA                                       
*                                  USE RADIO IN-BOX?                            
         BZ    SETP120             NO                                           
*                                                                               
*   TURN OFF TV ORDER LIST SELECTIONS                                           
*                                                                               
         LA    RF,RECACT2          YES - SET A(ORDER LIST ENTRY)                
         LA    R0,RECACT2#         SET LOOP CONTROL                             
*                                                                               
SETP110  EQU   *                                                                
         MVI   1(RF),X'FF'         OVERWRITE RECORD NUMBER                      
         LA    RF,RECACTL(RF)      BUMP TO NEXT ENTRY                           
         BCT   R0,SETP110          OVERWRITE RECORD NUMBER                      
*                                                                               
*   NOW TURN OFF EITHER DOUBLE OR SINGLE LINE LIST SELECTIONS                   
         TM    SVPGPBIT+DAR1LINB,DAR1LINA                                       
*                                  SINGLE LINE SCREEN?                          
*                                                                               
*   FORCE SINGLE LINE INBOX IN ALL CASES.                                       
*                                                                               
*        BZ    SETP112             NO                                           
*                                                                               
*   TURN OFF DOUBLE LINE LIST OPTIONS                                           
*                                                                               
         LA    RF,RECACT3          YES - TURN OFF DOUBLE LINE ENTRIES           
         LA    R0,RECACT3#         SET LOOP CONTROL                             
         B     SETP114                                                          
SETP112  EQU   *                                                                
*                                                                               
*   TURN OFF SINGLE LINE LIST OPTIONS                                           
*                                                                               
         LA    RF,RECACT4          TURN OFF SINGLE LINE ENTRIES                 
         LA    R0,RECACT4#         SET LOOP CONTROL                             
*                                                                               
SETP114  EQU   *                                                                
         MVI   1(RF),X'FF'         OVERWRITE RECORD NUMBER                      
         LA    RF,RECACTL(RF)      BUMP TO NEXT ENTRY                           
         BCT   R0,SETP140          OVERWRITE RECORD NUMBER                      
         B     SETP160                                                          
*                                                                               
SETP120  EQU   *                                                                
         LA    RF,RECACT3          SET RADIO OPTIONS OFF                        
         LA    R0,RECACTB#         SET LOOP CONTROL                             
*                                  COVER BOTH DOUBLE AND SINGLE                 
*                                     LINE DISPLAY VERSIONS                     
SETP140  EQU   *                                                                
         MVI   1(RF),X'FF'         OVERWRITE RECORD NUMBER                      
         LA    RF,RECACTL(RF)      BUMP TO NEXT ENTRY                           
         BCT   R0,SETP140          GO BACK FOR NEXT                             
*                                                                               
SETP160  EQU   *                                                                
...
*                                                                               
         NI    STEREOFG,X'FF'-STFINUSE INIT STEREO FLAG                         
*                                                                               
         L     R1,0(R1)                                                         
...
*                                                                               
         LR    R1,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R1,DARPROFS-CONHEADH                                             
         USING SVDSECT,R1                                                       
         TM    DMISFLGX,X'40'      AM I MASTER ?                                
         BO    CON10               BYPASS OFFICE CHECK FOR CONTRACT             
         DROP  R1                                                               
*                                                                               
         ...
         SPACE 1                                                                
CON10    DS    0H                                                               
         MVC   CCONKSTA,RCONKSTA   STATION CALL LETTERS                         
         MVC   CCONKREP,RCONKREP                                                
         MVC   ESTATION,SPACES                                                  
...
CON19B   DS    0H                                                               
         ...
         BNE   CON20                                                            
         USING RCONSEND,R6                                                      
         MVC   CCONVER#,RCONSRV    SAVE OFF CONTRACT VERSION                    
*                                                                               
         TM    RCONSENF,X'10'                                                   
         BO    CON20                                                            
...
INIT20   DS    0H                                                               
         LR    R2,RA                                                            
         AHI   R2,DARPROFS-CONHEADH                                             
         MVC   BLOCK(SVDSECTQ),0(R2) SAVE OFF PROFILE INFO                      
*                                                                               
...
*                                                                               
         TM    CTLRFLG1,CF1ISREV+CF1PDING                                       
         BZ    PFIX                REVISION APPROVE/REJECT OVERRIDE             
         CLC   =C'OPEN',CONACT                                                  
         BNE   PFI90                                                            
...
***      LH    RF,=Y(TWAMXLEN)                                                  
         LH    RF,=Y(TWAMAX)       LATEST RECORD LENGTH                         
         L     R0,ATWA                                                          
         LR    R1,RF                                                            
         MVCL  RE,R0                                                            
*                                                                               
...
RELO     DS    A                                                                
SAVERD   DS    A                                                                
CCONKREP DS    CL2                 SAVED REP FIELD IN CONTRACT REC              
         SPACE 1                                                                
SYSVCON  DS    0F                                                               
         DC    V(RECUP)                                                         
         DC    V(DUMMY)                                                         
...
*                                  CL1 PHASE NUMBER FOR HELP SCREEN             
         ...
         DC    X'01',C'CAMPAIGN',AL1(08),X'0000'                                
         DC    X'01',C'BUYER   ',AL1(09),X'0000'                                
         DC    X'01',C'FLIGHT  ',AL1(10),X'0000'                                
         DC    X'01',C'SENTDATE',AL1(11),X'0000'                                
         DC    X'01',C'TODO    ',AL1(12),X'0000'                                
         DC    X'01',C'UNDO    ',AL1(12),X'0000'                                
...
*                                  CL2 CODE FOR EOD HANDLING                    
         SPACE 1                                                                
RECACT2  DC    X'03',AL1(01,10),X'F001000080',C'    '  ORDER LIST               
RECACTL  EQU   *-RECACT2                                                        
...
RECACT2# EQU   (*-RECACT2)/RECACTL                                              
*                                                                               
*                                                                               
*   IF RADIO ORDER LIST IS OPTIONED, FIRST 'ORDER LIST' ENTRY IS                
*        INVALIDATED BY CHANGING RECORD NUMBER TO X'FF'                         
*                                                                               
*        DC    X'03',AL1(01,12),X'00300030C0',C'    '  ORDER     REPORT         
         ...
         DC    X'03',AL1(06,12),X'F612003120',C'RMRM'  REVISION  REPORT         
         DC    X'03',AL1(06,15),X'F824000081',C'    '  REVISION  RJ/AMD         
         DC    X'03',AL1(06,17),X'EA24000081',C'    '  REVISION  DIFF           
         DC    X'03',AL1(06,18),X'E823000081',C'    '  REVISION  DETAIL         
...
*                                                                               
* RADIO INBOX BULK PRINT OPTION -                                               
*                                                                               
         DC    X'03',AL1(12,12),X'E703000368',C'RBRB'  DAO PRINT REPORT         
*                                                                               
* RADIO INBOX OPTIONS - TURN OFF IF DARE INBOX NOT SET TO RADIO                 
*                                                                               
*   DOUBLE LINE DISPLAY SECTION FOR REDI                                        
*                                                                               
RECACT3  DC    X'03',AL1(01,10),X'E102000080',C'    '  ORDER LIST RADIO         
         DC    X'03',AL1(01,13),X'E102000081',C'    '  ORDER SEL  RADIO         
         DC    X'03',AL1(01,12),X'E102000260',C'RLRL'  ROM LIST  REPORT         
         DC    X'03',AL1(08,10),X'E102000080',C'    '  CAMPAIGN LIST            
         DC    X'03',AL1(08,13),X'E102000081',C'    '  CAMPAIGN SEL             
         DC    X'03',AL1(08,12),X'E102000260',C'RLRL'  ROM LIST  REPORT         
         DC    X'03',AL1(09,10),X'E102000080',C'    '  BUYER LIST               
         DC    X'03',AL1(09,13),X'E102000081',C'    '  BUYER SEL                
         DC    X'03',AL1(09,12),X'E102000260',C'RLRL'  ROM LIST  REPORT         
         DC    X'03',AL1(10,10),X'E102000080',C'    '  FLIGHT LIST              
         DC    X'03',AL1(10,13),X'E102000081',C'    '  FLIGHT SEL               
         DC    X'03',AL1(10,12),X'E102000260',C'RLRL'  ROM LIST  REPORT         
         DC    X'03',AL1(11,10),X'E102000080',C'    '  SENTDATE LIST            
         DC    X'03',AL1(11,13),X'E102000081',C'    '  SENTDATE SEL             
         DC    X'03',AL1(11,12),X'E102000260',C'RLRL'  ROM LIST  REPORT         
RECACT3# EQU   (*-RECACT3)/RECACTL                                              
*                                                                               
*   SINGLE LINE DISPLAY SECTION FOR REDI                                        
*                                                                               
RECACT4  DC    X'03',AL1(01,10),X'E104000080',C'    '  ORDER LIST RADIO         
         DC    X'03',AL1(01,13),X'E104000081',C'    '  ORDER SEL  RADIO         
         DC    X'03',AL1(01,12),X'E104000460',C'RSRS'  ROM LIST  REPORT         
         DC    X'03',AL1(08,10),X'E104000080',C'    '  CAMPAIGN LIST            
         DC    X'03',AL1(08,13),X'E104000081',C'    '  CAMPAIGN SEL             
         DC    X'03',AL1(08,12),X'E104000460',C'RSRS'  ROM LIST  REPORT         
         DC    X'03',AL1(09,10),X'E104000080',C'    '  BUYER LIST               
         DC    X'03',AL1(09,13),X'E104000081',C'    '  BUYER SEL                
         DC    X'03',AL1(09,12),X'E104000460',C'RSRS'  ROM LIST  REPORT         
         DC    X'03',AL1(10,10),X'E104000080',C'    '  FLIGHT LIST              
         DC    X'03',AL1(10,13),X'E104000081',C'    '  FLIGHT SEL               
         DC    X'03',AL1(10,12),X'E104000460',C'RSRS'  ROM LIST  REPORT         
         DC    X'03',AL1(11,10),X'E104000080',C'    '  SENTDATE LIST            
         DC    X'03',AL1(11,13),X'E104000081',C'    '  SENTDATE SEL             
         DC    X'03',AL1(11,12),X'E104000460',C'RSRS'  ROM LIST  REPORT         
RECACT4# EQU   (*-RECACT4)/RECACTL                                              
RECACTB# EQU   (*-RECACT3)/RECACTL            L(DOUBLE + SINGLE LINES)          
*                                                                               
         DC    X'FF'                                                            
MYACTORD EQU   1                                                                
         EJECT                                                                  
         ...
         EJECT                                                                  
***********************************************************************         
* ISMAST: IS THIS A MASTER SIGN ON?                                             
***********************************************************************         
ISMAST   NTR1  BASE=*,WORK=(R4,IMWORKQ),LABEL=*                                 
         USING IMWORKD,R4                                                       
         XC    IMKEY,IMKEY                                                      
K        USING CTIKEY,IMKEY                                                     
         MVI   K.CTIKTYP,C'I'                                                   
         MVC   K.CTIKID+8(2),TWAORIG                                            
         DROP  K                                                                
*                                                                               
         MVC   IMKEYSAV,IMKEY                                                   
         GOTO1 DATAMGR,DMCB,(0,=C'DMREAD'),=C'CTFILE',IMKEY,IMIO                
*                                                                               
         SPACE 1                                                                
         LA    R5,IMIO                                                          
         USING CTIKEY,R5                                                        
         ...
         USING CTAGYD,R6                                                        
*                                                                               
         LR    R5,RA               USE R2 TO COVER THE ENTRY                    
         AHI   R5,DARPROFS-CONHEADH                                             
         USING SVDSECT,R5                                                       
         MVC   MSTREPCX,CTAGYID                                                 
*                                                                               
IM20     DS    0H                                                               
         XC    IMKEY,IMKEY                                                      
K        USING RREPKEY,IMKEY                                                    
         MVI   K.RREPKTYP,X'01'                                                 
         MVC   K.RREPKREP,CTAGYID                                               
         DROP  K,R5,R6                                                          
*                                                                               
         MVC   IMKEYSAV,IMKEY                                                   
         XC    DMCB(24),DMCB                                                    
         GOTO1 DATAMGR,DMCB,=C'DMREAD',=C'REPDIR',IMKEYSAV,IMKEY,0,0            
         CLI   DMCB+8,X'10'                                                     
         BNE   *+6                                                              
         DC    H'0'                REP RECORD NOT ON FILE?  HOW?                
*                                                                               
         XC    DMCB(24),DMCB                                                    
         GOTO1 DATAMGR,DMCB,=C'GETREC',=C'REPFILE',IMKEY+28,IMIO,      X        
               IMDMWORK,0                                                       
*                                                                               
         LA    R6,IMIO                                                          
         MVI   ELCODE,X'01'                                                     
...
************                                                                    
       ...
       ++INCLUDE REROME1D                                                       
         ORG   CONTAGH                                                          
       ++INCLUDE REROMF2D          MAKEGOOD LIST SCREEN                         
         ORG   CONTAGH                                                          
       ++INCLUDE REROME7D          TODO REPORT SCREEN                           
**********************************************                                  
...
****************************                                                    
       ...
       ++INCLUDE REDARPROF                                                      
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'065REROM00   02/03/05'                                      
         END                                                                    
