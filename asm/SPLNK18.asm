*          DATA SET SPLNK18    AT LEVEL 039 AS OF 02/26/21                      
*PROCESS USING(WARN(15))           ENABLE ALL USING STATEMENT WARNINGS          
*PHASE T21E18B                                                                  
*INCLUDE GETBROAD                                                               
SPLNK18  TITLE '- SPOT PRISMA DEMO DOWNLOADS FOR DESKTOP'                       
         PRINT NOGEN                                                            
SVRDEF   CSECT                                                                  
         LKSVR TYPE=D,IDF=Y,REQUEST=*,CODE=CODE,WORKERKEY=CVLK,        *        
               SYSTEM=SPTSYSQ,APPEND=Y,SERVERTYPE=TSTDEMC,             *        
               AUTOCLEAR=Y,BLOCKS=(B#WORKD,WORKD,B#SAVED,SAVED),       *        
               FILES=FILES                                                      
                                                                                
CODE     NMOD1 0,**SL18**,RR=RE                                                 
         BASR  RA,0                                                             
         AHI   RA,GLOBALS-*                                                     
...
* RADIO COUNTY COVERAGE DONT HAVE MARKETS                                       
         ...
         OC    MARKET,MARKET       IF MARKET IS ALREADY FILLED IN VIA           
...
NXTBLD58 L     RF,ACOMFACS                                                      
         ICM   RF,15,CDEMTABS-COMFACSD(RF)                                      
         GOTO1 (RF),DMCB,LIVEBKTY       PARAM1=A(LIVE BOOK TYPE TABLE)          
         ICM   RE,15,0(R1)                                                      
         JNZ   *+6                                                              
         DC    H'0'                THIS SHOULD NEVER HAPPEN                     
...
*                                                                               
         L     RF,=V(GETBROAD)                                                  
         A     RF,SRVRRELO                                                      
         GOTO1 (RF),DMCB,(X'01',WORK),TEMPBRO,VGETDAY,VADDAY                    
         MVC   TEMPBRO(6),WORK                                                  
...
VBOWEEKN DS    X                   WEEK NUM (1,2,3,4,W) USED FOR LATBN          
VBOUTL   EQU   *-VBOUTD                                                         
...
TRNSBKT  LM    R2,R4,LP_AINP                                                    
         ...
         MVI   0(R4),C'~'                                                       
         J     EXITY                                                            
         GOTOR (#TRNSBT,ATRNSBT),DMCB,(R2),(R3),(R4)                            
         CHI   R3,0                                                             
         JE    EXITY                                                            
         CLI   0(R4),X'FF'                                                      
         JE    EXITN                                                            
...
***********************************************************************         
* REQUEST MAP FOR PRISMA REPORT DOWNLOAD - X'0019'                    *         
***********************************************************************         
                                                                                
REQCGR   LKREQ H,I#PRPRGD,OUTDUM                                                
...
LatBt    LKREQ F,17,(I,B#SAVED,SAVED),(R,TRNSBKT),                     *        
               OLEN=L'VBOLATBT,TEXT=(*,LBKTLIT),COL=*                           
WeekNum  LKREQ F,26,,CHAR,OLEN=1,TEXT=SP#WEEK,COL=*,                   *        
               ARRAY=E                                                          
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'039SPLNK18   02/26/21'                                      
         END                                                                    
