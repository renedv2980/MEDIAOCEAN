*          DATA SET SPSFM19    AT LEVEL 016 AS OF 03/14/02                      
*PHASE T21719A                                                                  
...
DK       DS    0H                                                               
         ...
         SP    WORK1,WORK2                                                      
         EDIT  (P4,WORK1),(5,CTMCNUM),FILL=0   CONTRACT NUMBER                  
         SRP   WORK1,1,0           SHIFT LEFT 1 DIGIT (UNCOMPLIMENTED)          
         MVC   CNUMPWOS,WORK1      SAVE CONTRACT NUMBER                         
...
*************************  TOTAL CASH  *****************************            
         ...
         BO    VR280                                                            
         OI    RECFLAG3,RECCHNGD   CONTRACT RECORD CHANGED                      
         XC    CTDSTCSH,CTDSTCSH   SET TO ZERO IN CASE EMPTY                    
         ZICM  RE,5(R2)            LENGTH OF FIELD                              
         BZ    VR280               EMPTY FIELD                                  
                                                                                
         ST    RE,DMCB+4                                                        
         GOTO1 CASHVAL,DMCB,(2,CTMTCSH)                                         
         CLI   0(R1),0             CHECK RETURN CODE                            
         BNE   INVALDGN            INVALID INPUT                                
                                                                                
         ZICM  RE,4(R1),4                                                       
         BM    INVALDGN            NEGATIVE NUMBER                              
         STCM  RE,15,CTDSTCSH      SAVE TOTAL CASH                              
...
VR295    LA    R2,CTMCONSH         START DATE                                   
         ...
         ST    RE,DMCB                                                          
         STC   R3,DMCB                                                          
         OI    DMCB,X'40'          FORMAT MM/DD FOR NEW PERVAL                  
...
DR63     OC    CTDSTCSH,CTDSTCSH                                                
         BZ    DR65                                                             
         EDIT  (4,CTDSTCSH),(9,CTMTCSH),2  DISPLAY TOTAL CASH                   
         OI    CTMTCSHH+6,X'80'                                                 
         OI    CTMTCSHH+4,X'20'                                                 
                                                                                
DR65     OC    CTDSCGCI,CTDSCGCI                                                
         BZ    DR70                                                             
         MVC   CTMTCTR,CTMGCI      DISPLAY GCI IN TOTAL CONTRACT FIELD          
         OI    CTMTCTRH+6,X'80'                                                 
         OI    CTMTCTRH+4,X'20'                                                 
...
DR280    L     R6,AIO              A(FIRST ELEMENT)                             
         ...
         CLI   CALLSP,1            CHECK IF USED PFKEY TO GET HERE              
         BNE   DR290                                                            
...
LR70     CLC   KEY(L'CTAPTYP+L'CTAPSUB+L'CTAPAGMD),SAVEKEY                      
         ...
         EDIT  (P4,WORK1),(5,LCNUM),FILL=0      CONTRACT NUMBER                 
...
DU       DS    0H                                                               
         TM    RECFLAG1,KEYCHNGD   KEY CHANGED                                  
         BO    DU03                                                             
         CLI   MYLSTACT,20         FIRST CALL TO SUBROUTINE?                    
         BNE   DU03                                                             
         TM    USGFLAG1,CONTUSGE   CONTINUE DISPLAY FROM ANOTHER PAGE?          
         BNO   DU05                                                             
         LA    R3,BUFF                                                          
         LR    R6,R3                                                            
         A     R3,SAVEDSP1                                                      
...
DU42     DS    0H                                                               
         ...
         LA    R6,BUFF                                                          
         MVI   ELCODE,CTAXFELQ     TRANSFER ELMENT                              
         USING CTAXFELD,R6                                                      
         BAS   RE,GETEL                                                         
         B     *+8                                                              
DU43     BAS   RE,NEXTEL                                                        
...
DU44     XC    STAB,STAB           LOOK FOR USAGE ELEMENTS W/O A MKSTA          
...
DU100    CLI   0(R6),CTAXFELQ      TRANSFER ELEMENT                             
         BNE   DU110                                                            
         USING CTAXFELD,R6                                                      
         ZAP   WORK1,=P'999999'                                                 
         ZAP   WORK2,=P'0'                                                      
         MVO   WORK2,CTAXFCON              CONVERT TO PACKED                    
         SP    WORK1,WORK2                                                      
         EDIT  (P4,WORK1),(5,UCLT),FILL=0  CONTRACT NUMBER                      
                                                                                
         ICM   R1,15,CTAXFOGR      GROSS DOLLARS                                
         LR    RF,R1               GROSS ORDERED DOLLARS                        
         NI    USGFLAG1,X'FF'-XFROUT                                            
         MVC   USTATN(7),=C'XFR IN '                                            
         TM    CTAXSTAT,CTAXSTOU                                                
         BNO   DU135                                                            
         MVC   USTATN(7),=C'XFR OUT'                                            
         LNR   R1,R1                                                            
         OI    USGFLAG1,XFROUT                                                  
         B     DU135                                                            
         DROP  R6                                                               
                                                                                
DU110    TM    USGFLAG1,NOMRUSGE                                                
         BO    DU50                                                             
         USING CTAUSELD,R6         USAGE ELEMENT                                
         MVC   USTATN,STATN                                                     
         CLC   =C'BF ',STATN                                                    
...
DU135    L     RE,BALANCE                                                       
         ...
         D     RE,=F'100'          LOOSE TWO DECIMAL PLACES                     
         ST    RF,WORK1                                                         
         EDIT  (B4,WORK1),(7,UORDERED),ZERO=NOBLANK  ORDERED                    
         CLI   0(R6),CTAXFELQ                                                   
         BNE   DU140                                                            
         TM    USGFLAG1,XFROUT                                                  
         BNO   DU145                                                            
...
DU140    DS    0H                                                               
         ...
         D     RE,=F'100'          LOOSE TWO DECIMAL PLACES                     
         ST    RF,WORK1                                                         
DU145    EDIT  (B4,WORK1),(7,UPAID),ZERO=NOBLANK       GROSS PAID               
...
SETUP20  CLI   ACTEQU,16                        ACTION MAINT                    
         BNE   SETUP40                                                          
         MVC   CTMTDRN,=C'RDR Trader'           SPECIAL HEADING FOR TBS         
         CLC   AGENCY,=C'TY'       TBSNY                                        
         BE    *+10                                                             
         MVC   CTMTDRN,=C'Trader    '                                           
         OI    CTMTDRNH+6,X'80'                                                 
...
*---------------------------- OTHER DSECTS ---------------------------*         
       ...
       ++INCLUDE SPSFMFFD                                                       
         ...
         EJECT                                                                  
         ORG   CONTAGH                                                          
       ++INCLUDE SCSFM80D         LIST SCREEN                                   
         SPACE 2                                                                
         ORG   CONTAGH                                                          
       ++INCLUDE SCSFM86D         MAINTENANCE SCREEN                            
         ORG   CONTAGH                                                          
       ++INCLUDE SCSFMD9D         USAGE SCREEN                                  
         EJECT                                                                  
...
XFROUT   EQU   X'08'               TRANSFER FROM ANOTHER                        
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'016SPSFM19   03/14/02'                                      
         END                                                                    
