*          DATA SET RERIS05    AT LEVEL 056 AS OF 07/11/12                      
...
*                                                                     *         
* AUG23/10 (SMY) --- RCU (SCRIPT CONTRACT UPLOAD FILTERING)           *         
*                                                                     *         
*                                                                     *         
*                    **  END TOMBSTONE  **                            *         
***********************************************************************         
...
*                                                                               
FIRSTLIN DS    0H                                                               
         TM    VIEWS,X'02'+X'10'   IF NOT LISTD OR LISTE                        
         BNZ   FL10                                                             
...
*                                                                               
* - VIEWS=X'01'= LIST$, X'02'=LISTD, X'04'=BUY$, X'10'=LISTE                    
...
* FOR LISTD                                                                     
         TM    VIEWS,X'02'+X'10'   IF LISTD OR LISTE VIEW                       
         BZ    FOUT20                                                           
         XC    RISBOTM,RISBOTM                                                  
         LA    R2,RISBOTM                                                       
...
* - COMMON TO LIST$ AND LISTD AND LISTE                                         
...
DATA23   EQU   *                                                                
         ...
         TM    VIEWS,X'02'+X'10'   IS IT LISTD OR LISTE VIEW?                   
         BNZ   DATANEW             YES                                          
*                                                                               
         ...
         UNPK  WORK(9),DUB(5)                                                   
***      MVC   L2CON,WORK+1                                                     
         MVC   L2CON,WORK          EXPANDED FOR 8 CHAR ORDER #S                 
*                                                                               
DATA26   ZIC   RE,1(R4)                                                         
         AR    R4,RE                                                            
         CLI   0(R4),X'17'         COMBO?                                       
...
*                                                                               
         ...
         BR    RE                                                               
         SPACE 1                                                                
DATA40   MVC   L2NAME,=CL18'NO PRODUCT GIVEN'                                   
...
*                                                                               
         ...
         MVC   L3PRD,WORK                                                       
         BAS   RE,GETSLSNM         RETURNS SALESPERSON'S NAME IN WORK           
         MVC   L3SLS,WORK                                                       
         ...
         GOTO1 =V(HEXOUT),DMCB,RCONKCON,L3CNT,4,=C'TOG',RR=YES                  
...
DATAN08  MVI   L3CNT+8,X'83'        SET LOWER CASE 'C'                          
         DROP  R4                                                               
DATAN08A EQU   *                                                                
         TM    VIEWS,X'02'+X'10'   LISTD OR LISTE OPTION?                       
         BZ    DATAN08B            NO                                           
         TM    VIEWS3,X'10'        RCU CONTRACTS ONLY ?                         
         BO    DATAN14             YES                                          
         TM    VIEWS3,X'08'        RCM CONTRACTS ONLY ?                         
         BO    DATAN14             YES                                          
DATAN08B EQU   *                                                                
...
*        PRINT CPE FOR LISTE                                                    
*                                                                               
         TM    VIEWS,X'10'         IF LISTE VIEW                                
         BNO   *+18                                                             
         BRAS  RE,GETCPE              PRINT CPE ON LINE 2                       
         MVC   L3CPE,WORK                                                       
         B     DATAN17                SKIP DEMO AND SHARE                       
*                                                                               
* GET DEMO                                                                      
...
DATAN12B MVC   WORK(3),0(RE)                                                    
         ...
         B     DATAN13                                                          
         DROP  R1                                                               
DATAN13  NI    WORK,X'FF'-X'40'    CLEAR PRIMARY DEMO FLAG (X'40')              
         BAS   RE,GETDEMNM         RETURNS DEMO NAME IN WORK                    
         MVC   L3DEMO,WORK                                                      
         B     DATAN17                                                          
         DROP  R4                                                               
DATAN14  EQU   *                   ONLY PROCESSING RCU CONTRACTS                
         LA    R4,RCONELEM         FIND                                         
DATAN14D EQU   *                                                                
         CLI   0(R4),0             END OF RECORD?                               
         BNE   *+6                                                              
         DC    H'0'          SHOULD BE ONLY PROCESSING RCU'S OR RCM'S           
         CLI   0(R4),X'3C'         SCRIPT CONTRACT UPLOAD ELEMENT ?             
         BE    DATAN14H            YES -                                        
         ZIC   RF,1(R4)                                                         
         AR    R4,RF               BUMP TO NEXT ELEMENT                         
         B     DATAN14D            GO BACK FOR NEXT                             
DATAN14H EQU   *                                                                
         USING RCONCUEL,R4                                                      
         MVC   L3TRAF,RCONCUW#     TRAFFIC ORDER NUMBER                         
*****    B     DATAN17                                                          
         DROP  R4                                                               
*                                                                               
* FUDGE RISTATUS AND USE LSTDLR TO GET $                                        
...
PTKOV10  ZIC   R0,1(R1)                                                         
         ...
         B     PATAN17              SKIP GET DEMO IN TAKEOVER MODE              
****************************************************************                
...
*        GET CPE FOR LISTE                                                      
*                                                                               
         TM    VIEWS,X'10'         IF LISTE                                     
         BNO   *+18                                                             
         BRAS  RE,GETCPE              SHOW CPE                                  
         MVC   P3CPE,WORK                                                       
         B     PATAN17                SKIP DEMO AND SHARE                       
...
PATAN16  DS    0H                  ONLY PROCESSING RCU CONTRACTS                
         LA    R4,RCONELEM         FIND                                         
PATAN16D EQU   *                                                                
         CLI   0(R4),0             END OF RECORD?                               
         BNE   *+6                                                              
         DC    H'0'                SHOULD BE ONLY PROCESSING RCU'S              
         CLI   0(R4),X'3C'         SCRIPT CONTRACT UPLOAD ELEMENT ?             
         BE    PATAN16H            YES -                                        
         ZIC   RF,1(R4)                                                         
         AR    R4,RF               BUMP TO NEXT ELEMENT                         
         B     PATAN16D            GO BACK FOR NEXT                             
PATAN16H EQU   *                                                                
         USING RCONCUEL,R4                                                      
         MVC   P3TRAF,RCONCUW#     TRAFFIC ORDER NUMBER                         
*****    B     PATAN17                                                          
         DROP  R4                                                               
...
*                                                                               
         ...
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTDIV4                                                          
         MVC   L2NAME,=CL18'TEAM REC NOT FOUND'                                 
         B     FOUTLINE                                                         
*                                                                               
...
*                                                                               
         ...
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTSAL4                                                          
         MVC   L2NAME,=CL18'SALESPERSON UNKNOWN'                                
         B     FOUTLINE                                                         
*                                                                               
...
LSTAGY3  BAS   RE,HIGH                                                          
         ...
         MVC   KEY+25(2),=C'ZZ'                                                 
         B     LSTAGY3                                                          
LSTAGY3N MVC   L2NAME,=CL18'AGENCY NOT FOUND'                                   
         B     FOUTLINE                                                         
*                                                                               
...
LSTADV2  BAS   RE,HIGH                                                          
         ...
         MVC   KEY+25(2),=C'ZZ'                                                 
         B     LSTADV2                                                          
LSTADV3  MVC   L2NAME,=CL18'ADVERTISER UNKNOWN'                                 
         B     FOUTLINE                                                         
*                                                                               
...
LSTSTA2  BAS   RE,HIGH                                                          
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTSTA4                                                          
         MVC   L2NAME,=CL18'STATION NOT FOUND'                                  
         B     FOUTLINE                                                         
*                                                                               
...
*                                                                               
         ...
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTOFF4                                                          
         MVC   L2NAME,=CL18'OFFICE NOT FOUND'                                   
         B     FOUTLINE                                                         
*                                                                               
...
*                                                                               
         ...
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTCTG4                                                          
         MVC   L2NAME,=CL18'CATEGORY NOT FOUND'                                 
         B     FOUTLINE                                                         
*                                                                               
...
DISPRD   BAS   RE,GETREC                                                        
         MVC   L2NAME+6(12),RPRDNAME                                            
         B     FOUTLINE                                                         
         SPACE 1                                                                
NOPRD    MVC   L2NAME+6(12),=CL12'CODE MISSING'                                 
         B     FOUTLINE                                                         
         SPACE 2                                                                
...
*                                                                               
         ...
         CLC   KEY(27),KEYSAVE                                                  
         BE    LSTGRP4                                                          
         MVC   L2NAME,=CL18'GROUP NOT FOUND'                                    
         B     FOUTLINE                                                         
*                                                                               
...
*                                                                               
         ...
         BAS   RE,GETEL                                                         
         BE    LTRAF10                                                          
         MVC   L2CTG(22),=C'NO TRAFFIC INSTRUCTIONS'                            
         B     LTRAF90                                                          
LTRAF10  EQU   *                                                                
...
MYPLHEAD DC    C'      STA/ADV   AGENCY/ADVERTISER      PRODUCT/SALES  X        
                       FLIGHT     CONT/DEMO      DOLLARS     CRD/ACTV  X        
               VER/SNT'                                                         
MYPLHD2  DC    C'      STA/ADV  AGENCY/ADVERTISER      PRODUCT/SALES   X        
                      FLIGHT    NEW/OLD #     DOLLARS      CRD/TKO    VX        
               ER/SNT'                                                          
MYPLHD3  DC    C'      STA/ADV   AGENCY/ADVERTISER      PRODUCT/SALES  X        
                       FLIGHT     CONT/TRAF      DOLLARS     CRD/ACTV  X        
               VER/SNT'                                                         
...
***********************************************************************         
*                                                                     *         
*        CONSTRUCT CLT/PRD/EST IN WORK                                *         
*                                                                     *         
***********************************************************************         
         SPACE 2                                                                
         DS    0D                  ALIGNMENT                                    
GETCPE   NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         XC    WORK,WORK           INIT RETURN AREA                             
*                                                                               
         LA    R8,RCONREC                                                       
         MVI   ELCODE,X'A2'        GET EASI CODE ELEMENT                        
         BRAS  RE,GETEL                                                         
         BNE   GETCPE90               NONE FOND                                 
*                                                                               
         USING RCONIEL,R8          ESTABLISH EASI CODE ELEMENT                  
*                                                                               
         MVC   WORK(L'RCONIADV),RCONIADV DISPLAY ADVERTISER                     
*                                                                               
         LA    R0,L'RCONIADV       COUNTER                                      
         LA    R1,WORK+L'RCONIADV-1    FIND END OF ADV CODE                     
*                                                                               
         CLI   0(R1),C' '                                                       
         BH    *+12                                                             
         SHI   R1,1                BACKUP A BYTE                                
         BCT   R0,*-12                                                          
*                                                                               
         MVI   1(R1),C'/'          SET SEPARATOR                                
*                                                                               
         AHI   R1,2                BUMP POINTER                                 
*                                                                               
         MVC   0(L'RCONIPRD,R1),RCONIPRD DISPLAY PRODUCT CODE                   
*                                                                               
         LA    R0,L'RCONIPRD       COUNTER                                      
         LA    R1,L'RCONIPRD-1(R1)    FIND END OF PRD CODE                      
*                                                                               
         CLI   0(R1),C' '                                                       
         BH    *+12                                                             
         SHI   R1,1                BACKUP A BYTE                                
         BCT   R0,*-12                                                          
*                                                                               
         CLC   RCONIPR2,=CL4' '    IF PIGGY BACK PRODUCT                        
         BNH   *+12                                                             
         MVI   1(R1),C'*'             SET INDICATOR                             
...
*                                                                               
         AHI   R1,2                BUMP POINTER                                 
*                                                                               
         MVC   0(L'RCONIEST,R1),RCONIEST DISPLAY ESTIMATE CODE                  
*                                                                               
         CLC   RCONIEST,=CL3' '    IF NO ESTIMATE CODE                          
         BH    *+10                                                             
         MVC   0(L'RCONXEST,R1),RCONXEST    DISPLAY EXTENDED ESTIMATE           
*                                                                               
         B     GETCPEX                                                          
*                                                                               
GETCPE90 DS    0H                                                               
         MVC   WORK(2),=C'//'      SET NO DATA INDICATOR                        
*                                                                               
GETCPEX  DS    0H                                                               
         XIT1                                                                   
*                                                                               
...
SETHEADS CSECT                                                                  
         ...
         TM    VIEWS,X'02'+X'10'   LISTDOR LISTE VIEW                           
         BNZ   SHED0020                                                         
         SR    R6,R6               NO                                           
         IC    R6,LISTBYTE                                                      
         LA    R6,ATITLE(R6)                                                    
...
SHED0020 EQU   *                                                                
         ...
         BO    SHED0040            YES                                          
         TM    VIEWS3,X'10'        DISPLAY RCU CONTRACTS ONLY?                  
         BO    SHED0060            YES                                          
         TM    VIEWS3,X'08'        DISPLAY RCM CONTRACTS ONLY?                  
         BO    SHED0060            YES                                          
*                                                                               
         TM    VIEWS,X'10'         IF LISTE                                     
         BNO   *+14                                                             
         MVC   RISTITL(L'TITLED6),TITLED6                                       
         B     *+10                                                             
...
TITLED2  DC    C'OFF   ADVERTISER   SALES                 DEMO        SX        
               HARE      ACTIVITY VER  '                                        
TITLED3  DC    C'ST/OF  AGY/ADV      PROD/SALES   FLIGHT   CON/DEMO    X        
                 AMT/SHR  CRD/ACT  VR/ST'                                       
TITLED4  DC    C'ST/OF  AGY/ADV     PROD/SALES   FLIGHT   NEW/OLD CON  X        
                 AMT/SHR  CRD/TKO  VR/ST'                                       
TITLED5  DC    C'ST/OF  AGY/ADV      PROD/SALES   FLIGHT   CON/TRAF    X        
                 AMT/SHR  CRD/ACT  VR/ST'                                       
TITLED6  DC    C'ST/OF  AGY/ADV      PROD/SALES   FLIGHT   CON/CPE     X        
                 AMT      CRD/ACT  VR/ST'                                       
         LTORG                                                                  
         EJECT                                                                  
*--------------------------------------------------------------|                
...
CLCX     XIT1                                                                   
         ...
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'056RERIS05   07/11/12'                                      
         END                                                                    
