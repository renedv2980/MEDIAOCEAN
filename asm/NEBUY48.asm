*          DATA SET NEBUY48    AT LEVEL 027 AS OF 12/05/12                      
*PHASE T31148A,+0                                                               
         TITLE 'NETPAK BUY PROGRAM - COPY UNITS STEWARD - T31148'               
T31148   CSECT                                                                  
         PRINT NOGEN                                                            
         ...
         L     RE,AIOAREA4                                                      
         LA    R1,CPURECLN                                                      
*        IF LARGER THEN 200 BYTES STEWREC MUST BE MADE LARGER                   
         MOVE  ((RF),(R1)),(RE)                                                 
         SPACE                                                                  
...
******   BZ    IC100               NO                                           
         BAS   RE,BLDSCRN          MOVE STEWARD INFO TO SCREEN                  
...
*                                                                               
         MVC   PPROG,CPUPCODE                                                   
         MVC   NBSELPRG,CPUPCODE   PROGRAM CODE                                 
         DROP  RE                                                               
         MVC   NBSELDP,NBACTDP                                                  
...
CP530    CLI   NBPRD,0             IF UNIT UNNALLOCATED                         
         ...
         BNE   CP400               NO-NEXT RECORD                               
*                                                                               
CP540    TM    CPYSTAT1,X'30'       ARE WE DOING DOLLAR FILTERING               
         BZ    CP580                NO                                          
         MVI   BYTE,C'N'            DEFAULT TO NOT ZERO DOLLAR UNIT             
         OC    NBACTUAL,NBACTUAL    IS ACTUAL COST ZERO                         
         BNZ   CP550                                                            
         TM    NBUNITST,X'20'       HAS ACTUAL COST BEEN INPUTTED               
         BZ    CP550                                                            
         MVI   BYTE,C'Y'            THIS IS A ZERO DOLLAR UNIT                  
CP550    TM    CPYSTAT1,X'20'       ONLY ZERO DOLLAR UNITS                      
         BZ    *+12                                                             
         CLI   BYTE,C'Y'                                                        
         BNE   CP400                                                            
         TM    CPYSTAT1,X'10'       BYPASS ZERO DOLLAR UNITS                    
         BZ    CP580                                                            
         CLI   BYTE,C'Y'                                                        
         BE    CP400                                                            
*                                                                               
CP580    CLC   NBACTDAT,STADATE    FILTER ON FIRST DATE                         
...
BD200    GOTO1 VHELLO,DMCB,(C'D',UNTFILE),(X'99',(R4)),0                        
         ...
         SPACE                                                                  
         BRAS  RE,SETAUDIT         SET THE AUDIT INFO IN THE BUY                
*                                                                               
         L     R4,NBAIO                                                         
...
SUBPRTX  B     EXXMOD                                                           
         DROP  R4                                                               
         EJECT                                                                  
* BYUILD SCREEN FOR STEWARD RECORD                                              
*                                                                               
BLDSCRN  NTR1                                                                   
         LA    R4,STEWREC                                                       
         USING CPYUPLDD,R4         R4 INPUT RECORD                              
*                                                                               
         MVC   CPYSTAT1,CPUSTAT1                                                
*                                                                               
*   FROM CLIENT                                                                 
         LA    R2,COPFCLTH                                                      
         MVC   COPFCLT,CPUCLI                                                   
...
*                                                                               
*   START DATE FILTER                                                           
         OC    CPUSTDAT,CPUSTDAT                                                
         BZ    BLDSC200                                                         
         LA    R2,COPFSTRH                                                      
         MVI   5(R2),8                                                          
         GOTO1 VDATVAL,DMCB,(0,CPUSTDAT),DUB                                    
         GOTO1 VDATCON,DMCB,(0,DUB),(8,COPFSTR)    MAKE COMPRESSED              
         BAS   RE,FINDLEN                                                       
*                                                                               
*   START DATE FILTER                                                           
BLDSC200 OC    CPUENDAT,CPUENDAT                                                
         BZ    BLDSCEX                                                          
         LA    R2,COPFENDH                                                      
         MVI   5(R2),8                                                          
         GOTO1 VDATVAL,DMCB,(0,CPUENDAT),DUB                                    
         GOTO1 VDATCON,DMCB,(0,DUB),(8,COPFEND)    MAKE COMPRESSED              
         BAS   RE,FINDLEN                                                       
*                                                                               
BLDSCEX  B     EXXMOD                                                           
         DROP  R4                                                               
...
CPYSTAT1 DS    XL1                                                              
*                                  X'80' = COPY TRAFFIC INFO                    
*                                  X'40' = COPY PRODUCT INFO                    
*                                  X'20' = ONLY ZERO DOLLAR UNITS               
*                                  X'10' = EXCLUDE ZERO DOLLAR UNITS            
*                                                                               
STEWREC  DS    CL200               STEWARD RECORD HOLD AREA                     
*                                                                               
         DS    0D                                                               
BLOCK    DS    CL256                                                            
MKGSV    DS    CL560                                                            
...
*  DDOFFICED                                                                    
       ...
       ++INCLUDE DDGLOBEQUS                                                     
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'027NEBUY48   12/05/12'                                      
         END                                                                    
