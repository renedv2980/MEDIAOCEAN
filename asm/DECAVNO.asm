*          DATA SET DECAVNO    AT LEVEL 003 AS OF 02/01/08                      
*PHASE DECAVNOA                                                                 
*INCLUDE DEBKUPDT                                                               
*---------------------------------------------------------------------*         
* NTI/NSS COMMERCIAL AVERAGE CONVERSION OUTPUT PHASE                  *         
* IPHASE: DECAVNI                                                     *         
* OPHASE: DECAVNO                                                     *         
*                                                                     *         
* NOTE: THIS CONVERSION HAS BEEN COPIED FROM THE NTI/NSS CONVERSION   *         
*       (DECNVNTI01) WITH THE NECESSARY CHANGES.                      *         
*---------------------------------------------------------------------*         
         TITLE 'DEMO CONVERSION: NTI/NSS COMMERCIAL AVERAGE'                    
DECAVNO  CSECT                                                                  
NUMCATS  EQU   38                                                               
         PRINT NOGEN                                                            
         NMOD1 NTIWRKX-NTIWRKD,**NTICNV                                         
         USING NTIWRKD,RC          RC=A(TEMP W/S)                               
         LA    R7,2048(RB)                                                      
         LA    R7,2048(R7)                                                      
         USING DECAVNO+4096,R7     R7=2ND BASE REGISTER                         
...
*                                                                               
         CLC   INTBOOK,=X'5B33'    STUPID BLACK WEEK SHIT                       
         BNE   *+10                    <                                        
         MVC   INTIBOOK,=X'5B21'       <                                        
*                                                                               
...
*                                                                               
         ...
         BZ    CNV36                                                            
         CLC   PREVMED,INTMEDIA    NEW MEDIA                                    
         BNE   CNV34               GO RELEASE PREV CORRECTION BOOK              
         CLI   INTKEY+2,C'P'                                                    
         BNE   *+14                                                             
         ...
         BE    CNV40                                                            
CNV34    CLI   EOFSW,1             EOF ON FILE ALREADY                          
...
CNV60    MVI   PMCODE,PMCODEQU                                                  
         ...
         MVC   PMSRC,INTKSRC                                                    
         MVC   KEYSRC,PMSRC        SAVE FILE TYPE FOR HUT SUMMARIES             
         MVC   MED,PMMEDIA         SAVE MEDIA TYPE FOR HUT SUMMARIES            
         MVC   PMBOOK,INTBOOK                                                   
         MVC   PMBTYP,BOOKTYPE                                                  
         MVC   PMSTAT,INTSTA                                                    
         MVC   PMSTYP,INTSTYP                                                   
...
*                                                                               
         ...
         ICM   R1,15,DMCB          A(TABLE) RETURNED IN P1                      
         BNZ   *+6                                                              
         DC    H'0'                BAD TABLEID PASSED                           
CNV72    CLC   INTKSRC,0(R1)                                                    
         BE    CNV75                                                            
         CLI   0(R1),X'FF'                                                      
         BNE   *+6                                                              
         DC    H'0'                                                             
...
CNV76    XC    TEMP,TEMP           CLEAR ELEMENT AREA                           
         ...
         MVI   PHTLEN,PHTLNEQ                                                   
         MVC   PHTDTYP,INTDTYP                                                  
         MVC   PHTPTYP(2),INTPTYP4                                              
*                                                                               
         CLC   INTSPTYP,=C'    '   INSET SUBPROGRAM TYPE IF PRESENT             
         BE    *+16                                                             
         MVC   PHTPTYP4,INTPTYP4                                                
         MVC   PHTSPTYP,INTSPTYP                                                
*                                                                               
...
*                                  BUILD NETWORK PROG RUN/TIME ELEM             
         USING NTELEM,R6                                                        
         XC    0(NTLENEQ3,R6),0(R6) CLEAR ELEMENT AREA                          
         MVI   NTCODE,NTCODEQU                                                  
         MVI   NTLEN,NTLENEQ3                                                   
         MVC   NTSQH,INTSQH                                                     
         MVC   NTEQH,INTEQH                                                     
...
CNV80    CLI   0(R5),0                                                          
         ...
         MVC   NTCMTEL,INTCOMTC    COMMERCIAL TELECASTS                         
         GOTO1 APUTEL,NTELEM                                                    
CNV90    LA    R5,7(R5)            NEXT DAY IN INTVAR                           
...
** ONLY PROGRAM DATA, NO TP ON FILE                                             
...
*                                  ************* -P- ONLY ***********           
         ...
         CLI   INTMEDIA,C'A'       LOWER CASE IS COMMERCIAL DATA                
         BNL   *+8                                                              
         AHI   R1,1002             USE PRGAVG2 FOR COMMERCIAL DATA              
...
CNV212   MVC   0(2,RF),PNPNUM      SAVE PROGRAM NUMBER                          
         ...
         MVI   PKCODE,PKCODEQU     'K'                                          
         MVC   PKMEDIA,PNMEDIA     CAPITAL OR LOWER CASE 'N'                    
         MVC   PKSRC,PNSRC                                                      
...
ADDBUFF  NTR1                                                                   
         ...
         BNE   ADDBUFF1                                                         
*        L     RE,VPUTBUFF         YES-CLEAR BUFFER                             
*        L     RF,HUTBUFFL                                                      
*        XCEF                                                                   
         MVI   FRSTSW,NO                                                        
*                                                                               
...
*                                                                               
ADDBUFF4 B     ADDBUFFX                                                         
*DDBUFF4 MH    R2,BOOKLEN          INDEX TO BOOK DATA IN HUT BUFFER             
         A     R2,VPUTBUFF                                                      
         ZIC   R1,INTDAYWK         GET DAY CODE                                 
         SRL   R1,4                SHIFT TO LOW ORDER NIBBLE                    
         MH    R1,DAYLEN           *L'ONE DAY'S DATA                            
         LA    R2,0(R1,R2)                                                      
         ZIC   R1,INTSQH                                                        
...
*            KEYSRC  =KEY SOURCE                                                
*            MED     =INTERNAL MEDIA (LOWER CASE FOR COMMMERCIAL DATA)          
*                                    (UPPER CASE FOR PROGRAM DATA)              
GETSUM   NTR1                                                                   
         DC    H'0'                  PATCH TO MAKE SURE ROUTINE                 
         LA    R5,DBLOCKA            DOESN'T GET CALLED                         
         XC    DBLOCK,DBLOCK                                                    
         MVC   DBFILE,=C'NTI'                                                   
         L     RE,AOREC                                                         
         LA    RE,4(RE)                                                         
         ...
         MVC   DBSELSRC,OUTSRC                                                  
*        MVC   DBSELAGY,=C'11'                                                  
         MVC   DBSELAGY,=C'SJ'     TEMPORARY FIX                                
...
*                                                                               
         L     R4,=A(LIVEEXT)     CREATE EXTENT 'LIVE'                          
         USING DBXLIVD,R4                                                       
         MVI   DBXLIVE,DBXLLQ                                                   
         LA    RE,SRCVTAB                                                       
         USING SRCVTABD,RE                                                      
GETSUM05 CLI   0(RE),X'FF'                                                      
         BNE   *+6                                                              
         DC    H'0'                UNKNOWN KEY SOURCE                           
         CLC   KEYSRC,SCVSRC                                                    
         BE    GETSUM07                                                         
         LA    RE,SRCVTABL(RE)                                                  
         B     GETSUM05                                                         
GETSUM07 MVC   DBXLIVE,SCVVTYP    VIEWING TYPE INDICATOR IN EXTENTION           
         DROP  RE                                                               
*                                                                               
GETSUM1  ICM   RE,15,DBEXTEND     OLD START OF THE LIST                         
         ...
         DROP  R4                                                               
*                                                                               
         CLI   MED,C'A'           LOWER CASE MEDIA IS COMMERCIAL DATA           
         BNL   GETSUM1A                                                         
         L     R4,=A(CAVGEXT)     CREATE EXTENT 'LIVE' IF COMMERCIAL            
         USING DBXCAVD,R4                                                       
         MVI   DBXCAV,DBXCVYQ                                                   
         ICM   RE,15,DBEXTEND     ADD COMMERCIAL AVERAGE EXTENSION              
         MVC   DBEXTEND,=A(CAVGEXT)                                             
         ICM   RF,15,DBEXTEND     NEW START OF THE LIST                         
         STCM  RE,15,DBXCAVNX     LINK THEM                                     
         DROP  R4                                                               
*                                                                               
GETSUM1A GOTO1 CDEMAND,DMCB,DBLOCK,0                                            
...
PKEY     NTR1                                                                   
         ...
         BE    PKEY30              NO                                           
         XC    KEYP,KEYP                                                        
         MVI   PRCODE,PRCODEQU     YES - SET UP KEY                             
         MVC   PRMEDIA,INTMEDIA                                                 
         MVC   PRSRC,INTKSRC                                                    
         ...
         MVC   PRMEDIA,SVMEDIA     RESET MEDIA                                  
*                                                                               
PKEY10   MVC   KEY,KEYP                                                         
...
PKEY95   XC    KEY,KEY             DONE RELSG RECDS IN BOOK.RESET KEYS          
         ...
         XC    SVSTYP,SVSTYP       RESET SAVED STATION TYPE                     
         XC    SVMEDIA,SVMEDIA     RESET SAVED MEDIA                            
...
QKEY     NTR1                                                                   
         ...
         BE    QKEY30              NO                                           
         XC    KEYQ,KEYQ                                                        
         MVI   PMCODE,PMCODEQU     YES - SET UP KEY                             
         MVC   PMMEDIA,INTMEDIA                                                 
         MVC   PMSRC,INTKSRC                                                    
         ...
         MVC   PMMEDIA,SVMEDIA     YES, RESET MEDIA                             
*                                                                               
QKEY10   MVC   KEY,KEYQ                                                         
         MVI   RDHIQSW,1                                                        
         BAS   RE,HIGH                                                          
...
PREVDW   DC    X'00'                                                            
*                                                                               
PRVPKSTA DC    CL5' '              PREVIOUS 'K' VALUES                          
PRVPKBK  DC    XL2'00'                                                          
PRVPKBTYP DC   C' '                                                             
PRVMEDIA DS    C' '                                                             
*                                                                               
...
DAYS     EQU   (*-DAYTAB)/L'DAYTAB                                              
         SPACE 2                                                                
SRCVTAB  DS    0X                  TABLE OF EXTENT INDICATORS BY SOURCE         
         DC    C'N',AL1(DBXLLQ)    LIVE                                         
         DC    C'1',AL1(DBXLL1Q)   LIVE+SD                                      
         DC    C'7',AL1(DBXLL7Q)   LIVE+7                                       
         DC    C'0',AL1(DBXLLF1Q)  LIVE+1                                       
         DC    C'2',AL1(DBXLL2Q)   LIVE+2                                       
         DC    C'3',AL1(DBXLL3Q)   LIVE+3                                       
         DC    X'FF'                                                            
* EQUATES                                                                       
*                                                                               
YES      EQU   C'Y'                                                             
...
SVSTYP   DS    X'00'               CORR RECD'S STATION TYPE                     
SVMEDIA  DS    X'00'               CORR RECD'S MEDIA                            
PREVMED  DC    C' '                PREVIOUS MEDIA                               
*                                                                               
...
CAVGEXT  DC    C'CAVG',AL4(0),X'00'   COMMERCIAL AVERAGE EXTENT DBXCAVD         
*                                                                               
KEYSRC   DS    C                                                                
MED      DS    C                                                                
*                                                                               
...
NTIWRKX  EQU   *                                                                
       ...
       ++INCLUDE DEINTNC1D                                                      
         SPACE 2                                                                
*                                                                               
SRCVTABD DSECT                     TABLE OF SOURCES BY VIEWING TYPE             
SCVSRC   DS    C                   SOURCE IN DEMO RECORD KEY                    
SCVVTYP  DS    X                   VIEWING TYPE REQUESTED IN DBXLIVE            
SRCVTABL EQU   *-SRCVTABD                                                       
*                                                                               
* DEDBLOCK                                                                      
         PRINT OFF                                                              
DBLOCKD  DSECT                                                                  
...
* DEDBEXTRAD                                                                    
       ...
       ++INCLUDE DBUPARMSD                                                      
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'003DECAVNO   02/01/08'                                      
         END                                                                    
