*          DATA SET RELDLOCAL  AT LEVEL 255 AS OF 05/01/02                      
*PHASE RELDLOC,*                                                                
*INCLUDE PRNTBL                                                                 
*INCLUDE PRINT                                                                  
*INCLUDE HEXOUT                                                                 
         TITLE 'RELDXNK - LOAD/DUMP MODEL EXTERNAL ROUTINE'                     
* PARAMETER LIST                                                                
*                                                                               
...
*******************************************************************             
*                                                                 *             
*   FLAG ORDERS FOR REPCODE NB, OFFICE 04 OR 27 AS LOCAL                        
*                                                                 *             
*******************************************************************             
*                                                                               
         PRINT NOGEN                                                            
...
DMXREC   DS    0H                                                               
         ...
         USING RCONREC,R5                                                       
         CLI   RCONKTYP,X'0C'                                                   
         BNE   DMXKEEP             NOT A CONTRACT - KEEP                        
         CLC   RCONKREP,=C'NB'                                                  
         BNE   DMXKEEP             NOT NBC - KEEP                               
         CLC   RCONKOFF,=C'04'                                                  
         BE    DMX10               OFFICE 04 - PROCESS                          
         CLC   RCONKOFF,=C'27'                                                  
         BNE   DMXKEEP             NOT OFFICE 27 - KEEP                         
*                                                                               
DMX10    DS    0H                                                               
         L     R5,AREC             POINT TO RECORD                              
         USING RCONRFEL,R5                                                      
         ...
         BE    DMX20                                                            
*                                                                               
         XC    WORK,WORK               ELEMENT NOT FOUND ADD IT                 
E        USING RCONRFEL,WORK                                                    
         MVI   E.RCONRFCD,X'1E'                                                 
         MVI   E.RCONRFLN,RCONRFLQ     <- MUST BE LESS THAN 64 BYTES            
         DROP  E                                                                
*                                                                               
         L     R5,AREC                                                          
         LA    R5,RCONELEM-RCONREC(R5)                                          
DMX12    DS    0H                                                               
         CLI   0(R5),X'1E'                                                      
         BH    DMX14                                                            
         CLI   0(R5),X'00'                                                      
...
*                                                                               
DMX14    DS    0H                                                               
         GOTO1 RECUP,DMCB,(C'R',AREC),WORK,(R5)                                 
*                                                                               
         AP    GROWN,=P'1'                                                      
*                                                                               
DMX20    DS    0H                                                               
         OI    RCONRF1,X'20'       SET NBC LOCAL ORDER                          
*                                                                               
         AP    CHANGE,=P'1'                                                     
*                                                                               
         LR    R6,R5                                                            
         ...
         MVC   P(2),RCONKREP                                                    
         MVC   P+4(2),RCONKOFF                                                  
         GOTO1 =V(HEXOUT),DMCB,RCONKCON,P+10,4                                  
         GOTO1 =V(HEXOUT),DMCB,(R6),P+30,RCONRFLQ                               
         GOTO1 VPRINTER                                                         
*                                                                               
DMX100   DS    0H                                                               
         B     DMXKEEP             NOT IN PURGE LIST, KEEP REC                  
         DROP  R5                                                               
         EJECT                                                                  
*******************************************************************             
* END-OF-FILE LOGIC                                               *             
...
WORKX    EQU   *                                                                
       ...
       ++INCLUDE REGENSDD                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'255RELDLOCAL 05/01/02'                                      
         END                                                                    
