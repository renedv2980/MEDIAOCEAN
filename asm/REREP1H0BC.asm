*          DATA SET REREP1H0BC AT LEVEL 010 AS OF 05/01/02                      
*PHASE RE1H02B,*                                                                
*INCLUDE GETBROAD                                                               
*INCLUDE ADDAY                                                                  
*INCLUDE GETDAY                                                                 
...
* UPDATE HISTORY                                                    *           
*                                                                   *           
* SEP23/99 (BU ) --- INITIAL ENTRY:  CONTRACT COUNTER FOR SELECT    *           
*                    LIST OF STATIONS AND REPS                      *           
*                                                                   *           
*                                                                   *           
*                    ***  END TOMBSTONE  ***                        *           
*********************************************************************           
...
INITIAL  NTR1                                                                   
         ...
         LA    R2,CONTABLE         SET A(KATZ CONTINENTAL TABLE)                
         CLC   =C'CQ',RCREPFL      KATZ CONTINENTAL RUN?                        
         BE    INIT0005            YES -                                        
         LA    R2,SELTABLE         SET A(SELTEL TABLE TABLE)                    
         CLC   =C'SZ',RCREPFL      SELTEL RUN?                                  
         BE    INIT0005            YES -                                        
         LA    R2,BLRTABLE         SET A(BLAIR TABLE)                           
         CLC   =C'BL',RCREPFL      BLAIR RUN?                                   
         BE    INIT0005            YES -                                        
         LA    R2,PETTABLE         SET A(PETRY TABLE)                           
         CLC   =C'PV',RCREPFL      PETRY RUN?                                   
         BE    INIT0005            YES -                                        
         DC    H'0'                UNKNOWN REP                                  
INIT0005 EQU   *                                                                
         MVC   KEY+1(2),RCREPFL    LOAD REP                                     
INIT0007 EQU   *                                                                
         CLI   0(R2),0             END OF TABLE REACHED?                        
         BE    INIT0080            YES - FINISHED                               
         XC    KEY+3(24),KEY+3     CLEAR LOW VALUE OF KEY                       
         MVC   KEY+3(5),0(R2)      LOAD STATION INTO KEY                        
INIT0010 EQU   *                                                                
         GOTO1 HIGH                                                             
         B     INIT0040                                                         
INIT0020 EQU   *                                                                
...
*                                                                               
         ...
         BNE   INIT0045            NO  - BREAK                                  
         CLC   KEY+3(5),0(R2)      SAME STATION?                                
         BNE   INIT0045            NO  - BREAK                                  
         ...
         CLC   PULSECTR,=F'5000'                                                
*                                                                               
         B     INIT0050            SKIP PULSECTR DIPLAY                         
*                                                                               
         BNE   INIT0050                                                         
...
INIT0045 EQU   *                                                                
         ...
         EDIT  CTR1998,(8,P+10)                                                 
         GOTO1 REPORT                                                           
         XC    CTR1998,CTR1998     CLEAR COUNTER                                
         LA    R2,5(R2)            BUMP TO NEXT STATION                         
         B     INIT0007            GO BACK FOR NEXT                             
**>>>                                                                           
INIT0050 EQU   *                                                                
         CLC   KEY+10(2),=X'C752'  FLIGHT END DATE: 10/18/99                    
***      CLC   KEY+10(2),=X'C421'  FLIGHT END DATE: 01/01/98                    
         BL    INIT0053            BEFORE DATE - SKIP IT                        
...
INIT0080 EQU   *                                                                
         ...
         B     MODEEXIT                                                         
         EJECT                                                                  
CONTABLE DC    C'WLKY '            KATZ CONTINENTAL TABLE                       
         DC    X'0000'                                                          
*                                                                               
SELTABLE DC    C'WNNE '            SELTEL TABLE                                 
         DC    X'0000'                                                          
*                                                                               
BLRTABLE DC    C'KOAT '            BLAIR  TABLE                                 
         DC    C'WBAL '                                                         
         DC    C'KHBS '                                                         
         DC    C'KHOG '                                                         
         DC    C'WDSU '                                                         
         ...
         DC    X'0000'                                                          
*                                                                               
PETTABLE DC    C'KCCI '            PETRY  TABLE                                 
         DC    C'WESH '                                                         
         DC    C'WPTZ '                                                         
         DC    C'KCRA '                                                         
         DC    X'0000'                                                          
...
BRDLEN   EQU   *-BRDSTART                                                       
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'010REREP1H0BC05/01/02'                                      
         END                                                                    
