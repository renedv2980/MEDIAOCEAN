*          DATA SET DMWRKRMX   AT LEVEL 011 AS OF 05/01/02                      
*PHASE WKMAINTA                                                                 
*INCLUDE DMDMGRL                                                                
*INCLUDE ADDAY                                                                  
*INCLUDE CARDS                                                                  
...
*                                                                               
INIT0    MVC   DTFFID,FILEID                                                    
         GOTO1 VDATAMGR,DMCB,=C'DMOPEN',=C'SER',DUB                             
INIT1    MVC   DNEXT,=X'00010000'                                               
         MVC   P1,=A(RDID)                                                      
         GOTO1 =V(DADDS),P1                                                     
         DROP  R2                                                               
*                                                                               
...
*                                                                               
         ...
         GOTO1 =V(PRINTER)                                                      
*                                                                               
INIT2    CLI   MODE,1              READ FIRST INDEX RECORD                      
         BE    INITWK                                                           
         L     R2,ACXREC                                                        
...
*                                                                               
         ...
         STH   R5,CXENTRY                                                       
*                                                                               
INIT4    TM    INPUT,TAPE          OPEN INPUT TAPE IF SPECIFIED                 
         BZ    INIT6                                                            
         BAS   RE,OPNIN                                                         
*                                                                               
INIT6    TM    OUTPUT,TAPE         OPEN OUTPUT TAPE IF SPECIFIED                
         BZ    INIT8                                                            
         BAS   RE,OPNOUT                                                        
*                                                                               
INIT8    CLI   MODE,2                                                           
...
COPYWK4  MVC   SAVE1(36),12(R3)    WORK FILE FOUND FOR COPYING                  
         TM    INPUT,TAPE                                                       
         BO    COPYWK6                                                          
         L     RF,ACIREC           COPY CXREC DISK BUFFER TO CIREC              
         L     RE,ACXREC                                                        
         LA    R1,4000                                                          
         MOVE  ((RF),(R1)),(RE)                                                 
...
*                                                                               
REPTWKI  TM    WKSTAT,WKSTAC+WKSTHO                                             
         BZ    REPTWKJ                                                          
...
RITAPE   ST    RE,RWSAVRE                                                       
         ...
         B     RITAPEXX                                                         
*                                                                               
RITAPEX  GOTO1 =V(LOGIO),DMCB,1,=C'ANY MORE INPUT TAPES'                        
         MVC   WORK,SPACES                                                      
...
ERRFATTB DC    X'800101',X'1200',X'0001',X'00',XL8'00'                          
         DC    CL16'**ERROR**ERROR**'                                           
*                                                                               
HLINE1   DC    CL120'USER-ID T FILE-KEY  SEQN STATUS CREATED  TIME RETAX        
               INED RECS RECL NCI PCT COMMENT          OLDSTA OLDS'             
HLINE2   DC    CL120'------- - --------- ---- ------ ------------- ----X        
               ---- ---- ---- --- --- ---------------- ------ ----'             
HLINE3   DC    CL120' '                                                         
ZEROS    DC    20C'0'                                                           
...
LOCKLX   DC    X'FF'                                                            
         EJECT                                                                  
*&&DO                                                                           
TAPEIN   DTFMT DEVADDR=SYS008,IOAREA1=TIIOA,FILABL=NO,RECFORM=VARBLK,  X        
               REWIND=UNLOAD,TYPEFLE=INPUT,BLKSIZE=8200,WORKA=YES,     X        
               IOAREA2=TIIOA2,EOFADDR=RITAPEX                                   
*                                                                               
TAPEOUT  DTFMT DEVADDR=SYS007,IOAREA1=TOIOA,FILABL=NO,RECFORM=VARBLK,  X        
...
*&&                                                                             
         SPACE 2                                                                
SSB      DC    F'0',F'0',XL24'00'                                               
UTL      DC    F'0',X'01',XL3'00',XL24'00'                                      
         EJECT                                                                  
...
*                                                                               
         ...
         MVC   VWKFILE,DMCB+12                                                  
         L     RE,VWKFILE                                                       
         MVC   FIDWKFI,22(RE)      EXTRACT ORIGINAL WKFILE ID                   
*                                                                               
         GOTO1 VDATAMGR,DMCB,=C'DTFADR',=C'FACWRK'                              
         MVC   VFACWRK,DMCB+12                                                  
         L     RE,VFACWRK                                                       
         MVC   FIDFACW,22(RE)      EXTRACT ORIGINAL FACWRK ID                   
*&&US                                                                           
         GOTO1 VDATAMGR,DMCB,=C'DTFADR',=C'EASIWK'                              
         MVC   VEASIWK,DMCB+12                                                  
         L     RE,VEASIWK                                                       
         MVC   FIDEASI,22(RE)      EXTRACT ORIGINAL EASIWK ID                   
*&&                                                                             
...
VBLKS    MVI   BLKSIZE,1           BLOCK SIZE IN BYTES                          
         ...
         BZ    VBLKSERR                                                         
         L     RF,8(R2)                                                         
         C     RF,=F'3660'         MAXIMUM VALUE                                
         BH    VBLKSERR                                                         
         C     RF,=F'3660'         MINIMUM VALUE                                
         BL    VBLKSERR                                                         
         STH   RF,CIBLKLN                                                       
         B     *+8                                                              
...
VUSERX   XIT1                                                                   
         SPACE 2                                                                
VFILE    NTR1                      FILE ID = ALL OR SPPXDDC,LLLL,HHHH           
         ST    R2,FULL                                                          
         CLI   MODE,2                                                           
         BNL   *+12                                                             
         MVI   ERRNUM,3                                                         
         ...
         ZIC   RF,1(R2)            MUST BE 1 THRU 7 CHRS                        
         CLI   1(R2),1                                                          
         BL    VFILEERR                                                         
         CLI   1(R2),7                                                          
         BH    VFILEERR                                                         
...
*DDDPRINT                                                                       
       ++INCLUDE DDDPRINT                                                       
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'011DMWRKRMX  05/01/02'                                      
         END                                                                    
