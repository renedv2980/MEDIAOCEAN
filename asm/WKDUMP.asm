*          DATA SET WKDUMP     AT LEVEL 005 AS OF 03/20/20                      
*PHASE WKDUMPA                                                                  
*INCLUDE CARDS                                                                  
*INCLUDE DMDMGRL                                                                
*INCLUDE DATCON                                                                 
...
*INCLUDE HEXIN                                                                  
*INCLUDE SORTER                                                                 
         TITLE 'WKDUMP - TRANSFER WRKF FILES FROM DISK/TAPE'                    
         PRINT NOGEN                                                            
WKDUMP   CSECT                                                                  
*                                                                               
         ENTRY UTL                                                              
         ENTRY SSB                                                              
*                                                                               
         NBASE WORKX-WORKD,**WKDU**,=A(WORKAREA),RA,R9                          
         USING WORKD,RC                                                         
         ST    RD,SAVERD                                                        
*                                                                               
...
INIT     NTR1                                                                   
*                                                                               
         L     RF,=A(SSB)          SET SERVICE FILES AS READ-ONLY               
...
*                                                                               
MAIN005  BAS   RE,INITWK           GET FILE CI DETAIL                           
*                                                                               
...
*                                                                               
         CLC   WLSOFLAB,=C'*SOFSOF*'                                            
         BNE   GETF500                                                          
         MVC   CSOFNDX,WLINDEX     SAVE FIRST INDEX                             
         MVI   WLFLAG,0            ENSURE FLAGS ARE ZERO                        
         MVC   IOAREAF,=X'00980000'                                             
*                                                                               
GETF900  OI    WLFLAG,WLFLRCOP     COPY FLAG                                    
*                                                                               
GETFILX  B     EXITEQ                                                           
         EJECT                                                                  
...
*                                                                               
         ...
         BE    GETLISX                                                          
*                                                                               
GL010    L     R5,AWKBUFF          GET LIST OF DISK FILES                       
         LA    RF,IOAREA                                                        
...
*                                                                               
FILT060  EQU   *                   RID# (report ID #)                           
         LLC   RF,RID#             Branch condition                             
...
*                                                                               
         ...
         GOTO1 =V(DATAMGR),DMCB,FFOPEN,WRKFILE,WKKEY,(RF),AWKBUFF               
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R1,AWKBUFF                                                       
...
*                                                                               
         ...
         BNM   *+6                                                              
         DC    H'0'                . BAD REPORT NUMBER IN CARDS                 
...
*                                                                               
         ...
         BNL   RDWKEOF             END OF WORKER FILE TO PROCESS                
*                                                                               
         OC    RID#HI,RID#HI                                                    
         BZ    RDFST010                                                         
         CLC   SVFILENO,RID#HI                                                  
         BH    RDWKEOF             END OF WORKER FILE TO PROCESS                
*                                                                               
RDFST010 GOTO1 =V(DATAMGR),DMCB,DMREAD,WRKFILE,CIADDR,(R5)                      
         CLI   8(R1),0                                                          
         BE    RDFST020                                                         
         TM    8(R1),X'80'                                                      
         BO    RDWKEOF                                                          
...
*                                                                               
         USING W_RECD,R5                                                        
RDFST020 OC    W_FILENO,W_FILENO                                                
         BZ    RDNXTRPT                                                         
*                                                                               
         XC    WLHEAD(28),WLHEAD                                                
         MVC   WLSOFLAB,=C'*SOFSOF*'                                            
         MVC   WLREPRNO,W_FILENO                                                
         MVC   WLREPFNO,WRKFILE+4                                               
         MVC   WLINDEX,W_INDEX                                                  
*                                                                               
         ...
         XC    SKRECS,SKRECS                                                    
         MVC   SKKEY,W_KEY                                                      
         MVC   SKDISP,=AL2(W_DATA1-W_INDEX)                                     
         XC    SKLEN,SKLEN                                                      
*                                                                               
...
*----------------------------------------------------------------------         
* READ NEXT RECORD ON WORKER FILE                                               
*----------------------------------------------------------------------         
RDNXT    LR    R6,R5                                                            
...
RDNXT10  GOTO1 =V(DATAMGR),DMCB,DMREAD,WRKFILE,CIADDR,(R5)                      
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'00'                                                            
...
RDREC    LR    RF,R1               Length of record                             
         ...
         B     RDOKAY                                                           
*                                                                               
RDEOR    XC    WLHEAD(28),WLHEAD                                                
         MVC   WLSOFLAB,=C'*EOFEOF*'                                            
         MVC   IOAREAF,=X'000C0000'                                             
         B     RDNXTRPT                                                         
*                                                                               
RDWKERR  XC    WLHEAD(28),WLHEAD                                                
         MVC   WLSOFLAB,=C'*EOFERR*'                                            
         MVC   IOAREAF,=X'000C0000'                                             
*                                                                               
...
RDOKAY   SR    RE,RE                                                            
         J     EXIT                                                             
*                                                                               
RDWKEOF  LTR   RE,RE               Set not equal CC                             
         J     EXIT                                                             
*                                                                               
...
*                                                                               
         ...
         ST    R1,RCWRKFC                                                       
*                                                                               
         CLC   RCWRKF,WRKFILE+4    Same WRKF                                    
...
TODAYO   DS    H                   TODAY COMP BASE 1964                         
...
SVFILENO DS    H                   SAVED REPORT NUMBER                          
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'005WKDUMP    03/20/20'                                      
         END                                                                    
