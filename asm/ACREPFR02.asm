*          DATA SET ACREPFR02  AT LEVEL 051 AS OF 02/26/04                      
*PHASE ACFR02A                                                                  
*INCLUDE UNDERLIN                                                               
*INCLUDE ACSLRY                                                                 
*INCLUDE ACSALHST                                                               
...
*                                                                               
         CLI   QOPT7,C'Y'                                                       
         BNE   REQF10                                                           
         MVI   RCSUBPRG,10         SET PRINT RTE AS INVALID                     
...
*                                                                               
         ...
         MVC   SVSUBPRG,RCSUBPRG   SAVE OFF PRINTING NUMBER                     
         GOTO1 ACREPORT                                                         
...
*                                                                               
         ...
         BE    REQF120                                                          
         OI    FLAG2,FLGPRD        MARK AS RUNNING PROD REPORT                  
*                                                                               
         USING ACTRECD,R2                                                       
         LA    R2,DKEY                                                          
         MVC   ACTKEY,XSPACES                                                   
...
PSUB60   CLC   PRDCP,0(RF)         IS THIS ONE OF THE CLI/PRD                   
         BNE   PSUB80                                                           
         MVC   SVGRP,PRDGRP        SAVE GROUP FROM TABLE                        
         LA    RE,=C'50'           CHECK IF LDA                                 
         TM    FLAG2,FLGFH         ARE WE READING FH                            
         BO    *+8                                                              
         LA    RE,=C'52'           CHECK IF LDA                                 
         CLC   PRDGRP,0(RE)        LDA NOT INCLUDED IN PROD HOURS               
         BNE   PSUB70                                                           
...
*        GOTO1 ADUMP,DMCB,(RC),(R4),16                                          
         ...
         AP    SDYCHRS,BUKCR                                                    
         ZAP   GRPYCHRS,BUKCR                                                   
         TM    FLAG2,FLGPRD        ARE WE RUNNING PRD LEV RPT?                  
         BNO   PHST35                                                           
         BAS   RE,PSTPER           POST TO PERSON TABLE                         
         TM    FLAG2,FLGPST        SHOULD WE POST TO PRDS?                      
         BNO   PHST35                                                           
         AP    SDPHRA,BUKCR        PRODUCT HOURS FOR ALL PRODS                  
         AP    GRPPHRA,BUKCR       ALL PROD HRS FOR GROUP RPT                   
         AP    SDYPHRSA,BUKCR      YTD PROD HRS FOR ALL PRDS                    
         AP    GRPYPHRA,BUKCR      ALL PROD HRS YTD FOR GROUP RPT               
*                                                                               
PHST35   TM    ACCSTUS,REQPRD      REQUESTED PRODUCT?                           
         BZ    PHST40                                                           
...
PHST110  DS    0H                                                               
         ...
         BZ    PHSTX                                                            
*                                                                               
         BAS   RE,PSTGRP           POST TO GROUP TABLE                          
*                                                                               
PHSTX    B     EXIT                                                             
         DROP  R3,R4,R5,R6                                                      
         EJECT                                                                  
...
ACCL40   BAS   RE,SALGRP           ADD ONLY SALARY TO GROUP                     
         TM    FLAG2,FLGPRD        ARE WE DOING PRODUCT REPORT?                 
         BNO   *+8                                                              
         BAS   RE,SALPER           POST TO PERSON TABLE                         
...
**********************************************************************          
         SPACE 1                                                                
RUNL     DS    0H                                                               
         ZAP   PACK8,=P'0'         CLEAR ACCUMULATOR                            
*                                                                               
         USING BIND,RE                                                          
         L     RE,ATOTBUFF         RE=TOTAL TABLE                               
         ICM   R0,15,BININ                                                      
         BZ    RUNLX                                                            
         USING TOTD,R2                                                          
         LA    R2,BINTABLE                                                      
...
RUNL10   DS    0H                                                               
         ...
         DROP  RE                                                               
*                                                                               
RUNL20   CLC   TOTPER,=X'FFFFFFFFFFFF'   TOTAL LINE?                            
         BNE   *+14                                                             
         ZAP   TOTSAL,PACK8                                                     
         B     RUNL40                                                           
         CLC   TOTPER,PERCDE       MATCH ON PERSON CODE                         
         BE    RUNL30                                                           
*                                                                               
         LA    R3,PERLEN(R3)                                                    
         BCT   R1,RUNL20                                                        
         B     RUNL50              MUST BE THERE                                
*                                                                               
RUNL30   ZAP   TOTSAL,PERSAL                                                    
         AP    PACK8,TOTSAL        KEEP SALARY ACCUMULATOR                      
RUNL40   ZAP   TOTDIFF,TOTPST      FIND DIFFERENCE                              
         ZAP   PACK16,TOTSAL                                                    
         SP    TOTDIFF,PACK16                                                   
*                                                                               
         ZAP   PACK16,TOTDIFF      MONTHLY X FACTOR A                           
         MP    PACK16,=PL8'22500'                                               
         SRP   PACK16,64-4,5       SHIFT AND ROUND                              
         ZAP   TOTDIFF,PACK16+(L'PACK16-8)(8)                                   
         CP    TOTDIFF,=P'5000'    PRINT OUT ANYTHING 50 AND UP                 
         BL    *+14                                                             
         MVC   MSG1,=CL10'**ERROR**'                                            
         B     *+10                                                             
*                                                                               
RUNL50   MVC   MSG1,=CL10'TOTAL PER'                                            
         GOTO1 ADUMP,DMCB,(RC),(R2),TOTLEN                                      
*                                                                               
         LA    R2,TOTLEN(R2)                                                    
         BCT   R0,RUNL10                                                        
*                                                                               
RUNLX    GOTO1 WRAP            RETURN ACQUIRED STORAGE                          
         B     EXIT                                                             
         EJECT                                                                  
**********************************************************************          
...
GETF40   DS    0H                                                               
         ...
         ZAP   FRLHOURS,NFFLOVER   MOVE NEW FREELANCER HOURS OVERRIDE           
*                                                                               
*        MVC   START,FISCSTR       ADJUST START DATE                            
*        MVC   WORK(2),START                                                    
*        MVI   WORK+2,X'01'                                                     
...
PSTGX    B     EXIT                                                             
         ...
         EJECT                                                                  
**********************************************************************          
* POST SORT RECORD TO PERSON TABLE                                   *          
* AND MARK FLAG TO POST TO SORTER BUFFER FOR ALL PRODUCT HRS         *          
*      R4 - A(BUCKET ELEMENT)                                        *          
*      R5 - A(SORTER DSECT)                                          *          
*      R6 - A(CONTRA HISTORY RECORD)                                 *          
**********************************************************************          
         SPACE 1                                                                
         USING BUKELD,R4                                                        
         USING SRTD,R5                                                          
         USING CACRECD,R6                                                       
PSTPER   NTR1                                                                   
         NI    FLAG2,X'FF'-FLGPST  TURN OFF POSTING FLAG                        
         CLC   SRDEPT,=C'00'       DON DO FOR UNDECLARED                        
         BE    PSTPX                                                            
*                                                                               
         LA    RF,CACKCACT         RF = A(OFFICE)                               
         SR    R1,R1                                                            
...
*                                                                               
         ...
         USING PRDTBD,R3                                                        
         LA    R3,BINTABLE                                                      
PSTP10   CLC   RQCLIENT,0(RF)      MAKE SURE THAT WE ARE ATLEAST DOING          
         BNE   PSTP20              THE RIGHT CLIENT                             
         CLC   PRDCP,0(RF)         IS THIS ONE OF THE CLI/PRD                   
         BE    *+16                                                             
*                                                                               
PSTP20   LA    R3,PRDLNQ(R3)                                                    
         BCT   R0,PSTP10                                                        
         B     PSTPX               NOT A MATCH - SKIP                           
*                                                                               
         CLC   PRDCLI,=C'LEX'      ARE WE DOING LEXUS?                          
         BNE   *+14                                                             
         CLC   PRDPRD,=C'LDA'      DON'T COUNT LDA                              
         BE    PSTPX                                                            
         DROP  R3,RE                                                            
*                                                                               
         OI    FLAG2,FLGPST        TURN ON POSTING FLAG                         
*                                                                               
         USING PERD,R2                                                          
         LA    R2,PERWRK           PERSON RECORD                                
         MVC   PERWRK,SPACES       CLEAR WORK AREA                              
         LA    RE,PERBUK                                                        
         LA    R0,PERNUMBK                                                      
         ZAP   0(SDBUKLN,RE),=P'0'                                              
         LA    RE,SDBUKLN(RE)                                                   
         BCT   R0,*-10                                                          
*                                                                               
         MVC   PERCDE,SRPERSON     PERSON CODE                                  
         ZAP   PERHRS,BUKCR        ADD THE HOURS                                
*                                                                               
*        MVC   MSG1,=CL10'PERSON'                                               
*        GOTO1 ADUMP,DMCB,(RC),SRPERSON,L'SRPERSON                              
*                                                                               
*        MVC   MSG1,=CL10'CACKCACT'                                             
*        GOTO1 ADUMP,DMCB,(RC),CACKCACT,L'CACKCACT                              
*                                                                               
*        MVC   MSG1,=CL10'PRDCP'                                                
*        GOTO1 ADUMP,DMCB,(RC),PRDCP,L'PRDCP                                    
*                                                                               
*        MVC   MSG1,=CL10'PER BUFF'                                             
*        GOTO1 ADUMP,DMCB,(RC),(R2),L'PERWRK                                    
*                                                                               
         GOTO1 BINADD,DMCB,PERWRK,APERBUFF                                      
PSTPX    B     EXIT                                                             
         DROP  R2,R4,R5,R6                                                      
         EJECT                                                                  
**********************************************************************          
...
*                                                                               
         ...
         L     R5,ASRTREC          SORT RECORD                                  
*                                                                               
SALP10   CLC   SRPERSON,PERCDE     MATCH ON PERSON CODE                         
         BE    SALP20                                                           
*                                                                               
         LA    R2,PERLEN(R2)                                                    
         BCT   R0,SALP10                                                        
         B     SALPX               MUST BE LDA AS CLIENT!!!!                    
*                                                                               
         USING SDTD,R6                                                          
SALP20   DS    0H                                                               
         LA    R6,SRDTL                                                         
         LA    R0,MAXMTHS                                                       
*                                                                               
         AP    PERSAL,SDSAL        ACCUMULATE SALARY TOTALS                     
         LA    R6,SDLEQ(R6)                                                     
         BCT   R0,*-10                                                          
*                                                                               
SALPX    B     EXIT                                                             
...
********************************************************************            
NFISTAB  DS    0F                                                               
         DC    XL2'0010',XL2'A009',PL6'180000',PL6'154000',PL6'000000',*        
               CL1'H'                                                           
         DC    XL2'A010',XL2'A103',PL6'090000',PL6'077000',PL6'000000',*        
               CL1'H'                                                           
         DC    XL2'A104',XL2'A203',PL6'180000',PL6'154000',PL6'000000',*        
               CL1'H'                                                           
         DC    XL2'A204',XL2'FF03',PL6'200000',PL6'161000',PL6'021250',*        
               CL1'R'                                                           
********************************************************************            
         EJECT                                                                  
**********************************************************************          
...
*                                                                               
         ...
         BO    CLR20                                                            
*                                                                               
         L     RE,AADJTAB          CLEAR MONTHLY ADJUSTMENT TABLE               
         LA    RF,MAXMTHS*ADJLNQ                                                
         SR    R1,R1                                                            
         MVCL  RE,R0                                                            
*                                                                               
         USING ADJD,R1                                                          
CLR20    L     R1,AADJTAB          MONTHLY ADJUSTMENT TABLE                     
         LA    RF,SRDTL                                                         
...
RPT20    DS    0H                                                               
         ...
         MOVE  ((RF),(R1)),(RE)                                                 
RPT30    L     R5,ASRTREC                                                       
         TM    FLAG2,FLGPRD        REQUESTED PRODUCT?                           
         BNO   *+8                                                              
         BRAS  RE,ADJPER           FIX PERSAL TO MONEY EARNED                   
         BRAS  RE,CHKPER           CHECK IF PERSON HIT 1800 PROD HRS            
...
DUP10    MVI   FORCEHED,C'Y'                                                    
         ...
         NI    FLAG2,FLGFH+FLGPRD                                               
         OI    FLAG2,FLGDUP        DUP REPORT                                   
         MVC   SVKEY,XSPACES                                                    
         MVC   SVPERCD,XSPACES                                                  
...
DUP110   LA    R6,SDLEQ(R6)        NEXT AVAILABLE MONTH SLOT                    
         ...
         BO    DUP120                                                           
         MVI   RCSUBPRG,4          ALWAYS SET BEFORE GOING TO CHKPER            
         TM    FLAG2,FLGPRD        REQUESTED PRODUCT?                           
         BNO   *+8                                                              
         BRAS  RE,ADJPER           FIX PERSAL TO MONEY EARNED                   
         BRAS  RE,CHKPER           CHECK IF PERSON HIT 1800 PROD HRS            
*        BRAS  RE,PCT              FIGURE OUT PCT OF TIME                       
         ...
         BASR  RE,RF               FIGURE OUT PCT OF TIME                       
*                                                                               
DUP120   TM    FLAG2,FLGPRD        REQUESTED PRODUCT?                           
         BNO   *+8                                                              
         BRAS  RE,ADJPER           FIX PERSAL TO MONEY EARNED                   
         BRAS  RE,CHKPER           CHECK IF PERSON HIT 1800 PROD HRS            
         TM    FLAG2,FLGPRD        REQUESTED PRODUCT?                           
         BZ    *+8                                                              
         BRAS  RE,PCTADJ           YES-USE PCTADJ TO FIGURE PCT                 
         BRAS  RE,REFRESH          REFRESH LEVEL NAMES                          
         BRAS  RE,DETAIL           PROCESS THE DETAIL RECORD                    
         B     DUP20                                                            
*                                                                               
...
*                                                                               
         LA    RE,=C'3A1B'        IF PERSONS IN 3A1B/4A1B LEAVE *               
         CLC   =C'TMS',RQCLIENT    ARE WE DOING TOYOTA TMS                      
         BE    *+8                                                              
         LA    RE,=C'4A1B'         IF LEXUS LEAVE * IN FRONT OF NAME            
         CLC   TCQOFF,0(RE)                                                     
         BE    *+12                                                             
         MVI   WORK,C' '            CLEAR OFF '*' IN FRONT OF NAME              
         B     *+8                                                              
         MVI   WORK,C'*'            ADD '*' IN FRONT OF NAME                    
*                                                                               
         MVC   XP+7(36),WORK                                                    
...
**********************************************************************          
         ...
         EJECT                                                                  
**********************************************************************          
* CHECK PERSON TABLE TO SEE IF PERSONS PROD HOURS > 1800             *          
*       R5 = A(SORT RECORD)                                          *          
**********************************************************************          
         SPACE 1                                                                
         USING SRTD,R5                                                          
CHKPER   NTR1  BASE=*,LABEL=*                                                   
         USING BIND,RE                                                          
         L     RE,APERBUFF         R1=PERSON TABLE                              
         ICM   R0,15,BININ                                                      
         BZ    CHKPERX                                                          
         ...
         DROP  RE                                                               
*                                                                               
CHKPER10 CLC   SRPERSON,PERCDE     MATCH ON PERSON CODE                         
         BE    CHKPER20                                                         
*                                                                               
         LA    R2,PERLEN(R2)                                                    
         BCT   R0,CHKPER10                                                      
         B     CHKPERX             MUST LDA AS CLIENT!!!!!                      
*                                                                               
CHKPER20 DS    0H                                                               
         CP    PERHRS,ADJHOURS     DID PERSON GO OVER 1800?                     
         BL    *+8                                                              
         OI    SRSTAT,SRPRDHRS     SET FLAG TO SHOW PER EXCEEDED 1800           
         ZAP   SRYTPHR,PERHRS      YTD PRODUCT HOURS                            
*        MVC   MSG1,=CL10'BINTABLE'                                             
*        GOTO1 ADUMP,DMCB,(RC),(R2),L'PERWRK                                    
*                                                                               
         USING SDTD,R6                                                          
         LA    R6,SRDTL                                                         
         LA    R0,MAXMTHS                                                       
*                                                                               
         ZAP   SDTPST,PERSAL       YTD TOTAL POSTED                             
         LA    R6,SDLEQ(R6)                                                     
         BCT   R0,*-10                                                          
*                                                                               
CHKPERX  DS    0H                                                               
...
ADJPER   NTR1  BASE=*,LABEL=*                                                   
         ...
         DROP  RE                                                               
*                                                                               
ADJPER10 CLC   SRPERSON,PERCDE     MATCH ON PERSON CODE                         
         BE    ADJPER20                                                         
*                                                                               
         LA    R3,PERLEN(R3)                                                    
         BCT   R0,ADJPER10                                                      
         B     ADJPERX             MUST LDA AS CLIENT!!!!!                      
*                                                                               
ADJPER20 DS    0H                                                               
         CP    PERHRS,ADJHOURS     DID PERSON GO OVER 1800?                     
         BNL   ADJPERX                                                          
...
*                                                                               
         LA    R2,MAXMTHS          ALL MONTHS                                   
ADJPER30 OC    SDMNTH,SDMNTH                                                    
         BZ    ADJPER60            GET NEXT MONTH                               
*                                                                               
         CLC   RQCLIENT,=C'TMS'    ARE WE RUNNING TOYOTA?                       
         BNE   ADJPER40            MUST BE LEXUS                                
         ...
         B     ADJPER50                                                         
*                                                                               
ADJPER40 LA    RE,SDYCHRS          ASSUME IT'S YTD CLIENT HRS                   
...
*                                                                               
         ...
         ZAP   DUB,0(SDBUKLN,R1)                                                
*                                                                               
ADJPER50 CP    SDYPHRSA,ZEROS      YTD WORK ON PRODUCT THIS MONTH?              
         BE    ADJPER60            NO- SKIP THIS MONTH                          
         ZAP   PACK16,SDYPHRSA     ACTUAL TOYOTA HOURS YTD                      
         SRP   PACK16,6,0          SHIFT 6 POSITIONS TO LEFT                    
         ZAP   PKYPCT,ZEROS                                                     
         CP    DUB,ZEROS           FIND ANY HOURS?                              
         BZ    *+16                                                             
         DP    PACK16,DUB          CLIENT BY TOTAL YTD                          
         ZAP   PKYPCT,PACK16(L'PACK16-L'DUB)                                    
         SRP   PKYPCT,64-2,5                                                    
*                                                                               
...
*                                                                               
         ...
         MP    PACK16,PKYPCT       X  YTD PERCENT OF TIME ON TOYOTA             
         SRP   PACK16,64-4,5       SHIFT AND ROUND FOR NON GROUP                
*                                                                               
         ZAP   PKYPST,PACK16+(L'PACK16-8)(8) YTD SALARY COST                    
*                                                                               
ADJPER60 DS    0H                                                               
         LA    R6,SDLEQ(R6)                 NEXT MONTH                          
         BCT   R2,ADJPER30                                                      
         ZAP   PERSAL,PKYPST       REPLACE 100% SALARY WITH SAL EARNED          
*                                                                               
ADJPERX  DS    0H                                                               
         J     COM2XIT                                                          
         EJECT                                                                  
...
ADJUST   NTR1  BASE=*,LABEL=*                                                   
         L     R5,ASRTREC                                                       
         LA    R6,SRDTL                                                         
         TM    FLAG2,FLGPRD        ARE WE RUNNING A PRD REPORT?                 
         BO    ADJXX                                                            
...
PCTA10   ZAP   SDYPST,ZEROS                                                     
         ...
         ZAP   PACK16,SDYPHRS      ACTUAL TOYOTA HOURS YTD                      
         SRP   PACK16,9,0          SHIFT 6 POSITIONS TO LEFT                    
         ZAP   SDYPCT,ZEROS                                                     
         CP    SRYTPHR,ZEROS       FIND ANY HOURS?                              
         BZ    *+16                                                             
         DP    PACK16,SRYTPHR      CLIENT BY TOTAL YTD                          
         ZAP   SDYPCT,PACK16(L'PACK16-L'SRYTPHR)                                
*                                                                               
* BECAUSE TOYOTA YEAR IS DIFFERENT THAN SAATCHI YEAR WE COULD NOT USE           
* REAL POSTED DOLLARS AS INTENDED SO WE NEED THIS CALC                          
*                                                                               
         ...
         MP    PACK16,SDYPCT       X  YTD PERCENT OF TIME ON TOYOTA             
         SRP   PACK16,64-9,5       SHIFT AND ROUND                              
         ZAP   SDYPST,PACK16+(L'PACK16-8)(8) YTD SALARY COST                    
         SRP   SDYPCT,64-5,5                                                    
         ZAP   SDPST,SDYPST        MONTHLY = YTD  - (YTD-1)                     
         SP    SDPST,MINUS1                                                     
...
DET10    OC    SDMNTH,SDMNTH                                                    
         ...
         MVC   PMNTH(5),=C'TOTAL'                                               
*                                                                               
         TM    FLAG2,FLGPRD        ARE WE RUNNING PROD RPT?                     
         BO    DET30                                                            
...
DET20    LA    R7,XPTHIRD                                                       
         ...
         B     DET170                                                           
*                                                                               
DET30    TM    SRSTAT,SRPRDHRS                                                  
         BO    DET70                                                            
         LA    R7,XPTHIRD                                                       
         MVC   PMNTH(27),=C'NOTE:FINAL SALARY IS A % OF'                        
         MVC   PMNTH+28(33),=C'TOTAL $ FROM CLI LEV(           )'               
*                                                                               
         USING BIND,RE                                                          
         L     RE,APERBUFF         R1=PERSON TABLE                              
         ICM   R0,15,BININ                                                      
         ...
         DROP  RE                                                               
*                                                                               
DET40    CLC   SRPERSON,PERCDE     MATCH ON PERSON CODE                         
         BNE   DET50                                                            
         EDIT  PERSAL,(11,PMNTH+49),2,ZERO=NOBLANK,FLOAT=$,MINUS=YES            
         B     DET60                                                            
*                                                                               
DET50    LA    R3,PERLEN(R3)                                                    
         BCT   R0,DET40                                                         
         DROP  R3                                                               
*                                                                               
DET60    LA    R7,XPFOURTH                                                      
         MVC   PMNTH(18),=C'PCT=(IND PROD HRS('                                 
         MVC   PMNTH+18(L'RQPROD),RQPROD                                        
         MVC   PMNTH+21(34),=C')/TOTAL PROD HRS FROM CONSOLIDATED'              
         MVC   PMNTH+56(7),=C'REPORT)'                                          
         LA    R7,XP               RESTORE XP LINE                              
         B     DET80                                                            
*                                                                               
DET70    LA    R7,XPTHIRD                                                       
         MVC   PMNTH(29),=C'NOTE:PERSON HAS EXCEEDED 1800'                      
         MVC   PMNTH+30(19),=C'TOTAL PRODUCT HOURS'                             
         LA    R7,XPFOURTH                                                      
         MVC   PMNTH(18),=C'PCT=(IND PROD HRS('                                 
         MVC   PMNTH+18(L'RQPROD),RQPROD                                        
         MVC   PMNTH+21(34),=C')/TOTAL PROD HRS FROM CONSOLIDATED'              
...
*        BE    DET110                                                           
         ...
         B     DET170                                                           
*                                                                               
DET90    CP    SDCLIHR,ZEROS       WORK ON TOYOTA THIS MONTH?                   
         BNE   DET100              YES-CONTINUE                                 
         CP    SDSAL,ZEROS         SALARY DOLLARS?                              
         BNE   DET100              YES-                                         
         CP    SDPST,ZEROS         DOLLARS ON TOYOTA THIS MONTH?                
         BE    DET230              NO- SKIP THIS MONTH                          
*                                                                               
DET100   TM    FLAG,FLGGRP         ARE WE DOING GROUP REPORT?                   
         BNO   DET110                                                           
...
*                                                                               
         ...
         BNZ   DET140              SKIP ADITIONAL FIGURING                      
*                                                                               
DET110   TM    FLAG2,FLGPRD                                                     
         BNO   DET120                                                           
         C     R5,ASRTREC          DO PCT FOR TOTAL RECORDS HERE                
         BE    DET140                                                           
*                                                                               
         ZAP   SDYPCT,ZEROS                                                     
         CP    SDYPHRS,ZEROS       YTD WORK ON PRODUCT THIS MONTH?              
         ...
         B     DET140                                                           
*                                                                               
DET120   LA    RE,SDYTHRS          YTD TOTAL 1C HOURS                           
...
*                                                                               
         ...
         SRP   SDYPCT,64-2,5                                                    
*                                                                               
DET130   ZAP   PACK16,SDSALY       YTD SALARY                                   
         SRP   PACK16,6,0          SHIFT 6 POSITIONS TO LEFT                    
         ZAP   SDHRRAT,ZEROS                                                    
         ZAP   DUB,SDYTHRS                                                      
...
*                                                                               
         ...
         SRP   SDHRRAT,64-4,5                                                   
*                                                                               
DET140   C     R5,ASRTREC          DON'T DO FACTOR FOR TOTAL RECS               
         BNE   DET160                                                           
         ZAP   PACK16,SDPST        MONTHLY X FACTOR A                           
         ZAP   PACK8,=PL6'22500'   CHECK FOR OVERRIDE?                          
         CP    FRLHOURS,=P'0'      DO WE HAVE AN OVERRIDE?                      
         BE    DET150                                                           
         LA    RE,=C'3A1B'      MARK PERSONS IN 3A1B/4A1B AS FREELNCERS         
         CLC   =C'TMS',RQCLIENT    ARE WE DOING TOYOTA TMS                      
         BE    *+8                                                              
...
*                                                                               
         ...
         TM    FLAG2,FLGPRD        CHECK FOR UNWANTED POSTING AMTS              
         BNO   DET180                                                           
         CLC   SRPERSON,=C'Z02587'                                              
         BNE   DET180                                                           
         CP    SDYPHRS,=P'0'       DO WE HAVE ANY YTD PROD HOURS?               
         BNE   DET180              YES-CONTINUE AS NORMAL                       
         ZAP   SDYPST,=P'0'        CLEAR OUT ALL THESE NUMBERS                  
         ZAP   SDPST,=P'0'                                                      
         ZAP   SDTOTAL,=P'0'                                                    
DET180   EDIT  SDYPST,PYCOST,2,MINUS=YES,COMMAS=YES,ZERO=BLANK                  
...
*                                                                               
         ...
         MVI   RCSUBPRG,8                                                       
         B     *+12                                                             
DET190   TM    FLAG2,FLGPRD        DONT PRINT WHEN RUNNING PRD RPT              
         BNO   DET200                                                           
         EDIT  SRYTPHR,PYTPHR,2,MINUS=YES,COMMAS=YES,ZERO=BLANK                 
         EDIT  SDYPCT,PYGRPPCT,2,MINUS=YES,TRAIL=C'%'                           
*                                                                               
DET200   CLI   SDMNTH,X'FF'        TOTAL LINE?                                  
         BE    DET220                                                           
...
DET220   DS    0H                                                               
         ...
         BE    DET230                                                           
         BRAS  RE,OUTREC           UPDATE HEADLINES AND PRINT                   
DET230   LA    R6,SDLEQ(R6)        NEXT MONTH                                   
         BCT   R2,DET10                                                         
         J     COM2XIT                                                          
         EJECT                                                                  
**********************************************************************          
...
**********************************************************************          
         ...
         EJECT                                                                  
**********************************************************************          
* POST TO TOTAL BUFFER                                               *          
*         R5 SET TO ADDRESS OF SORT RECORD COMING IN                 *          
**********************************************************************          
         SPACE 1                                                                
         USING SRTD,R5                                                          
PSTTOT   NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         USING TOTD,R3                                                          
         LA    R3,TOTWRK           TOTAL RECORD                                 
         MVC   TOTWRK,SPACES       CLEAR WORK AREA                              
         LA    RE,TOTBUK                                                        
         LA    R0,TOTNUMBK                                                      
         ZAP   0(TOTBKQ,RE),=P'0'                                               
         LA    RE,TOTBKQ(RE)                                                    
...
*                                                                               
         USING SDTD,R6                                                          
         LA    R6,SRTOTAL          CHECK TOTAL LINE FOR INCORRECT $             
         CLI   SDMNTH,X'FF'        TOTAL LINE?                                  
         BNE   PSTT20                                                           
         CP    SDYPHRS,=P'0'       DO WE HAVE ANY YTD PROD HOURS?               
         BNE   PSTT20              YES-CONTINUE AS NORMAL                       
*                                                                               
* SINCE TOTAL LINE SAYS THAT YTD PRODUCTS ARE ZERO - ALL TOTS ARE ZERO          
*                                                                               
         LA    R6,SRDTL            MONTHLY DETAIL                               
         LA    R0,MAXMTHS                                                       
PSTT10   DS    0H                                                               
...
*                                                                               
         ...
         BCT   R0,PSTT30                                                        
*                                                                               
         MVC   MSG1,=CL10'TOTBUF'                                               
         GOTO1 ADUMP,DMCB,(RC),TOTWRK,L'TOTWRK                                  
*                                                                               
         GOTO1 BINADD,DMCB,TOTWRK,ATOTBUFF                                      
         MVC   TOTPER,=X'FFFFFFFFFFFF'                                          
         GOTO1 (RF),DMCB,TOTWRK,ATOTBUFF                                        
         J     COM2XIT                                                          
**********************************************************************          
* LITERALS                                                           *          
**********************************************************************          
...
*                                                                               
         ...
         BE    *+8                                                              
         LA    RE,=C'4A1B'         IF LEXUS LEAVE * IN FRONT OF NAME            
         CLC   SROFFICE,0(RE)      ONLY ADD LINE FOR OFF/DPT IF FRLNCR          
         BNE   *+10                                                             
         MVC   GRPHRS.TCQOFF,SROFFICE           OFFICE DPT OF PERSON            
*                                                                               
...
HEADUP   NTR1  BASE=*,LABEL=*                                                   
         ...
         BNL   *+12                RECAP AND ANALYSIS REPORTS                   
...
*                                                                               
HDUP20   CLI   RCSUBPRG,8          GROUP REPORT FOR ADJ PERSON                  
         BNL   HDUP30                                                           
         ...
         BE    HDUP40                                                           
*                                                                               
HDUP30   TM    FLAG2,FLGDUP        ARE WE RUNNING DUP REPORT?                   
         BO    HDUP40                                                           
*                                                                               
         CLC   SVGRP,XSPACES       ANY GROUP?                                   
         BNH   *+22                                                             
...
*                                                                               
         ...
         MVC   XHEAD8+106(21),=C'CHARGE FACTOR = 2.25 '                         
         CLI   PRSNAME,C'*'        IS THIS A FREELANCER                         
         BNE   *+10                                                             
         MVC   XHEAD8+106(21),=C'CHARGE FACTOR = 2.125'                         
         B     HDUP60                                                           
*                                                                               
HDUP40   MVC   XHEAD5(10),LLEVDNAM         DUP LOCATION PAGE                    
...
*                                                                               
HDUP50   MVC   XHEAD4+101(21),=C'*********************'                         
         MVC   XHEAD5+101(21),=C'** MULTI LOCATIONS **'                         
         MVC   XHEAD6+101(21),=C'*********************'                         
         B     HDUP70                                                           
*                                                                               
HDUP60   TM    FLAG,FLGGRP         ADDITIONAL GROUP REPORT-ADJ PERSON           
         BNO   HDUP70                                                           
         MVC   XHEAD4+101(21),=C'*********************'                         
         MVC   XHEAD5+101(21),=C'** REPORT BY GROUP **'                         
         MVC   XHEAD6+101(21),=C'*********************'                         
         MVC   XHEAD8+70(L'MSGS),MSGS                                           
*                                                                               
HDUP70   MVC   XHEAD1+46(35),=C'  T O Y O T A  F E E  R E P O R T  '            
         CLC   =C'LEX',RQCLIENT                                                 
         BNE   *+10                                                             
...
*                                                                               
         ...
         CLI   RCSUBPRG,3                                                       
         BNE   HDUPX                                                            
HDUP80   MVC   XHEAD5+70(L'MSGS),MSGS                                           
         MVC   XHEAD5+106(26),=C'CHARGE FACTOR = 2.25/2.125'                    
         MVC   XPTHIRD,XSPACES                                                  
...
HDUP90   CLI   RCSUBPRG,6          RECAP REPORT                                 
         ...
         MVC   XHEAD4+22(L'SVGRPNM),SVGRPNM                                     
*                                                                               
HDUP100  CLI   RCSUBPRG,6          RECAP REPORT                                 
         BE    HDUP110                                                          
         MVC   XHEAD5(10),LLEVBNAM                                              
...
*                                                                               
*                                                                               
HDUP110  MVC   XHEAD1+53(24),=C'TOYOTA MOTOR SALES, INC.'                       
         CLC   =C'LEX',RQCLIENT                                                 
         BNE   *+10                                                             
...
HDUPX    DS    0H                                                               
         ...
         CLI   RCSUBPRG,7          TIME/COST REPORT                             
         BNH   HDUPX10                                                          
         TM    FLAG2,FLGPRD        ARE WE ON PROD REPT                          
         BNO   HDUPX10                                                          
         MVI   RCSUBPRG,9                                                       
HDUPX10  GOTO1 ACREPORT                                                         
         MVC   RCSUBPRG,SVSUBPRG   RESET PRINTING                               
         J     COM2XIT                                                          
         EJECT                                                                  
**********************************************************************          
* LITERALS                                                           *          
...
PSTADJ   NMOD1 0,**PSTA**                                                       
         ...
         MVC   SVDPT,XSPACES                                                    
*                                                                               
*        TM    FLAG2,FLGPRD        ARE WE DOING THE PRD RPT?                    
*        BO    PSTA05              YES - ALWAYS POST ADJUSTMENTS IF ANY         
*        TM    FLAG2,FLGNEG        DID SALARY GET ADJUSTED BACKWARDS?           
*        BO    PSTA05                                                           
*        TM    SRSTAT,SRHRS        DID PERSON REACH 1800 HRS (CLI TIME)         
*        BNO   PSTAX               NO - EXIT                                    
*        TM    SRSTAT,SRPRDHRS     WAS PERSON ALEADY ADJUSTED?                  
*        BO    PSTAX               YES - EXIT                                   
*                                                                               
PSTA05   LA    RE,SVSRTTOT         TOTAL DETAIL ACCUMS BEFORE ADJUST            
...
*                                                                               
         BRAS  RE,PSTTOT           INCREMEMNT TOTALS                            
*                                                                               
*        TM    FLAG2,FLGNEG        DID SALARY GET ADJUSTED BACKWARDS?           
...
TOTLINE  USING SDTD,R4                                                          
         L     R5,ASRTREC          SORT RECORD                                  
         LA    R6,SRDTL            MONTHLY DETAIL                               
         ZAP   PACK16,SRYTPHR      SAVE OFF PERSON NUMBERS                      
         NI    FLAG2,X'FF'-FLGREQ  TURNOFF REQUEST TOT REC FLAG                 
*        MVC   MSG1,=CL10'TOTIT SRT'                                            
*        GOTO1 ADUMP,DMCB,(RC),(R5),SRDTL-SRTD                                  
...
TOT10    TM    FLAG,FLGLDA         DON'T ADD TO REQUEST TO FOR LDA              
         ...
         BE    TOTITX              DON'T ADD UNKNOWN TO REQ TOTAL               
         LA    R6,SRDTL            MONTHLY DETAIL                               
         OI    FLAG2,FLGREQ        REQUEST RECORD TOTALING                      
...
*                                                                               
TOT20    ST    RE,SAVERE                                                        
         TM    FLAG2,FLGDUP        ARE WE DOING A DUP REPORT?                   
         BNO   *+12                                                             
         TM    FLAG2,FLGREQ        ARE WE DOING THE REQUEST TOTALS?             
         BO    *+10                                                             
         AP    SRYTPHR-SRTD(L'SRYTPHR,R5),PACK16                                
*        MVC   MSG1,=CL10'TOTIT #2'                                             
*        GOTO1 ADUMP,DMCB,(RC),(R5),SRDTL-SRTD                                  
...
DWNR10   STC   R0,SVCNT                                                         
         ...
         MVC   SVSUBPRG,RCSUBPRG   SAVE OFF REPORT NUMBER                       
         MVI   RCSUBPRG,10         INVALID REPORT NUM FOR DOWNLOAD              
...
CODAPER  DC    AL2(APGDPT-ACFRD),AL1(2)                                         
         DC    AL2(DPTCODE-ACFRD),AL1(4)                                        
         DC    AL2(SUBCODE-ACFRD),AL1(2)                                        
         DC    AL2(PRSCODE-ACFRD),AL1(L'PRSCODE)                                
CODNUMP  EQU   (*-CODADRP)/CODLNQ                                               
CODAEND  DC    X'FF'                                                            
*                                                                               
CODAAPG  DC    AL2(APGDPT-ACFRD),AL1(2)                                         
         DC    X'FF'                                                            
*                                                                               
* GROUP REPORT TABLE                                                            
*                                                                               
CODADRPG DS    0CL3                ADDRESS TABLE OF PER/GRP LEVELS              
CODAPERG DC    AL2(SVGRP-ACFRD),AL1(2)                                          
         DC    AL2(APGDPT-ACFRD),AL1(2)                                         
         DC    AL2(DPTCODE-ACFRD),AL1(4)                                        
         DC    AL2(SUBCODE-ACFRD),AL1(2)                                        
         DC    AL2(PRSCODE-ACFRD),AL1(L'PRSCODE)                                
...
CODAENDG DC    X'FF'                                                            
*                                                                               
CODAAPGG DC    AL2(SVGRP-ACFRD),AL1(2)                                          
         DC    AL2(APGDPT-ACFRD),AL1(2)                                         
         DC    X'FF'                                                            
*                                                                               
CODAGRPG DC    AL2(SVGRP-ACFRD),AL1(2)                                          
         DC    X'FF'                                                            
         EJECT                                                                  
***********************************************************************         
* LITERALS                                                            *         
...
*        MVC   MSG1,=CL10'BUF REC'                                              
*        GOTO1 ADUMP,DMCB,(RC),BUFKEY,BLEN                                      
*                                                                               
         GOTO1 BUFFALO,DMCB,(L'PUTC,PUTC),ADBUFF,(R4)                           
...
*                                                                               
         LA    RE,=C'3A1B'        IS PERSONS IN 3A1B/4A1B                       
         CLC   =C'TMS',RQCLIENT    ARE WE DOING TOYOTA TMS                      
         BE    *+8                                                              
         LA    RE,=C'4A1B'         IF LEXUS CHECK 4A1B                          
         CLC   SROFFICE,0(RE)      ONLY ADD LINE FOR OFF/DPT IF FRLNCR          
         BNE   *+10                                                             
         MVC   HRS.TCQOFF,SROFFICE     OFFICE/DEPT OF PERSON                    
*                                                                               
...
*                                  COST RECORD                                  
         ...
         MVC   COST.TCQOFF,XSPACES      OFFICE/DEPT OF PERSON                   
...
CCOMTAB  DS    0D                  TBLE FOR COMMON WORK NMOD ENTRIES            
         DC    AL2(COMMON-ACFRD),AL2(COM1-ACFRD),AL1(COMNUM1)                   
         DC    AL2(COMMON2-ACFRD),AL2(COM2-ACFRD),AL1(COMNUM2)                  
         DC    AL1(EOT)                                                         
*                                                                               
*                                                                               
...
CSUMPST  DS    0D                                                               
         ...
         DC    Y(SDPHRA-SDTD),AL1(SUMPADD),AL1(0),Y(BUFPHRA-BUFD)               
         ...
         DC    Y(SDYPHRSA-SDTD),AL1(SUMPZAP),AL1(0),Y(BUFYPHRA-BUFD)            
...
AMAINTAB DS    0F                                                               
         DC    CL8'**PROD** '                                                   
         DC    AL2(APRDBUFF-ACFRD) STORED ADDR OF THE TABLE (APGD)              
...
*                                                                               
         DC    CL8'**APGN** '                                                   
         DC    AL2(APGBUFF-ACFRD)  STORED ADDR OF THE TABLE (APGD)              
...
*                                                                               
         DC    CL8'**APGG** '                                                   
         DC    AL2(APGGBUF-ACFRD)  STORED ADDR OF THE TABLE (APGGD)             
...
*                                                                               
         DC    CL8'**NAME** '                                                   
         DC    AL2(NMEBUFF-ACFRD)  STORED ADDR OF THE TABLE (NMED)              
...
*                                                                               
         DC    CL8'**DUPL** '                                                   
         DC    AL2(DUPBUFF-ACFRD)  STORED ADDR OF THE TABLE (DUPD)              
...
*                                                                               
         DC    CL8'**TIT*** '                                                   
         DC    AL2(TIBUFF-ACFRD)   STORED ADDR OF THE TABLE (TITD)              
...
*                                                                               
         DC    CL8'**GRP*** '                                                   
         DC    AL2(AGRPBUFF-ACFRD) STORED ADDR OF THE TABLE (GRPD)              
...
*                                                                               
         DC    CL8'**PER*** '                                                   
         DC    AL2(APERBUFF-ACFRD) STORED ADDR OF THE TABLE (PERD)              
         DC    AL2(PERLEN)         RECORD LENGTH                                
         DC    AL2(PERKLEN)        DISP IN REC/KEY LENGTH                       
         DC    AL2(PERMAX)         MAX NUMBER OF TAB ENTRIES                    
         DC    AL1(PERNUMBK)       NUMBER OF BUCKETS                            
         DC    AL1(PERBKDSP)       DISP TO BUCKETS                              
         DC    AL4(PERSIZE)        SIZE OF THE TABLE                            
*                                                                               
         DC    CL8'**TOT*** '                                                   
         DC    AL2(ATOTBUFF-ACFRD) STORED ADDR OF THE TABLE (PERD)              
         DC    AL2(TOTLEN)         RECORD LENGTH                                
         DC    AL2(TOTKLEN)        DISP IN REC/KEY LENGTH                       
         DC    AL2(TOTMAX)         MAX NUMBER OF TAB ENTRIES                    
         DC    AL1(TOTNUMBK)       NUMBER OF BUCKETS                            
         DC    AL1(TOTBKDSP)       DISP TO BUCKETS                              
         DC    AL4(TOTSIZE)        SIZE OF THE TABLE                            
*                                                                               
...
*                                     **NOTE: UPDATE LENSIZE!!!**               
AWORKTAB DS    0F                                                               
         DC    AL2(AAPGWK-ACFRD),AL2(APGLEN)                                    
         DC    AL2(ASRTREC-ACFRD),AL2(SRTLEN)                                   
         DC    AL2(AGRPREC-ACFRD),AL2(SRTLEN)                                   
         DC    AL2(ADPTREC-ACFRD),AL2(SRTLEN)                                   
         DC    AL2(ADUPREC-ACFRD),AL2(SRTLEN*MAXDPTS)                           
         DC    AL2(AREQREC-ACFRD),AL2(SRTLEN)                                   
         DC    AL2(AWRKREC-ACFRD),AL2(SRTLEN)                                   
         DC    AL2(ABUFWRK-ACFRD),AL2(BLEN)                                     
         DC    AL2(ATCAPWK-ACFRD),AL2(TCAPLNQ)                                  
         DC    AL2(ARCAPWK-ACFRD),AL2(TCAPLNQ)                                  
         DC    AL2(ARCAPTOT-ACFRD),AL2(TCAPLNQ)                                 
         DC    AL2(ATCQWK-ACFRD),AL2(TCLNQ)                                     
         DC    AL2(ATCQWK2-ACFRD),AL2(TCLNQ)                                    
...
PERMAX   EQU   2000                                                             
PERSIZE  EQU   L'MAINEYE+BINLENQ+(PERMAX*PERLEN)                                
TOTMAX   EQU   2000                                                             
TOTSIZE  EQU   L'MAINEYE+BINLENQ+(TOTMAX*TOTLEN)                                
GRPMAX   EQU   4000                                                             
...
APERBUFF DS    A                   ADDR OF PERSON TABLE                         
ATOTBUFF DS    A                   ADDR OF PERSON TABLE                         
*                                                                               
...
*                                                                               
PKYPCT   DS    PL(SDBUKLN)         PERCENT TOYOTA TO TOTAL                      
PKYPST   DS    PL(SDBUKLN)         YTD POSTED TO TOYOTA CLIENT                  
*                                                                               
...
FLGPRD   EQU   X'20'               CURRENTLY RUNNING PROD REPORT                
FLGDUP   EQU   X'10'               CURRENTLY RUNNING DUP REPORT                 
FLGPST   EQU   X'08'               POST PRD HOURS TO SORT RECORD                
FLGREQ   EQU   X'04'               CURRENTLY ON REQUEST TOTAL RECORD            
*                                                                               
...
PERWRK   DS    CL(PERLEN)          WORK AREA FOR PERSON TABLE                   
TOTWRK   DS    CL(TOTLEN)          WORK AREA FOR TOTAL TABLE                    
*                                                                               
...
MANHOURS DS    PL6                 P'154000',P'077000',P'161000' MANYR          
FRLHOURS DS    PL6                 P'212500' FREELANCER OVERRIDE/ZERO           
*                                                                               
SVQTR    DS    CL1                                                              
TOTSW    DS    CL1                                                              
*                                                                               
...
NFFLOVER DS    PL6                 P'212500' - FREELANCER OVERRIDE              
...
SRPRDHRS EQU   X'10'               ***PROD REPORT ONLY*** PRD HRS>1800          
*                                                                               
...
SDPRDHR  DS    PL(SDBUKLN)         HOURS WORKED ON PRODUCT (REQUESTED)          
SDPHRA   DS    PL(SDBUKLN)         HOURS WORKED ON PRODUCT (ALL PRODS)          
SDCLIHR  DS    PL(SDBUKLN)         HOURS WORKED ON CLIENT(TOYOTA/LEX)           
SDTHRS   DS    PL(SDBUKLN)         TOTAL HOURS WORKED                           
SDYPHRS  DS    PL(SDBUKLN)         YTD HOURS WORKED ON PRODUCT (REQ)            
SDYPHRSA DS    PL(SDBUKLN)         YTD HOURS WORKED ON PRODUCT (ALL)            
...
GRPPRDHR DS    PL(SDBUKLN)         HOURS WORKED ON PRODUCT (REQUESTED)          
GRPPHRA  DS    PL(SDBUKLN)         HOURS WORKED ON PRODUCT (ALL PRODS)          
GRPCLIHR DS    PL(SDBUKLN)         HOURS WORKED ON CLIENT(TOYOTA/LEX)           
GRPTHRS  DS    PL(SDBUKLN)         TOTAL HOURS WORKED                           
GRPYPHRS DS    PL(SDBUKLN)         YTD HOURS WORKED ON PRODUCT (REQ)            
GRPYPHRA DS    PL(SDBUKLN)         YTD HOURS WORKED ON PRODUCT (ALL)            
...
GRPYTCHR DS    PL(SDBUKLN)         YTD ACTUAL CLIENT HOURS                      
GRPDTLQ  EQU   *-GRPDTD                                                         
         EJECT                                                                  
**********************************************************************          
* DSECT FOR A PERSON TABLE                                           *          
**********************************************************************          
         SPACE 1                                                                
PERD     DSECT                                                                  
PERCDE   DS    CL6                 PERSON CODE                                  
PERKLEN  EQU   *-PERD                                                           
PERBKDSP EQU   *-PERD                                                           
PERBUK   DS    0C                                                               
PERHRS   DS    PL(SDBUKLN)         MONTHLY BUCKET - TOTAL HOURS                 
PERSAL   DS    PL(SDBUKLN)         MONTHLY BUCKET - TOTAL SALARY                
PERNUMBK EQU   (*-PERBUK)/SDBUKLN                                               
PERLEN   EQU   *-PERD              RECORD LENGTH                                
         EJECT                                                                  
**********************************************************************          
* DSECT FOR A PERSON TOTAL TABLE                                     *          
**********************************************************************          
         SPACE 1                                                                
TOTD     DSECT                                                                  
TOTPER   DS    CL6                 PERSON CODE                                  
TOTKLEN  EQU   *-TOTD                                                           
TOTBKDSP EQU   *-TOTD                                                           
TOTBUK   DS    0C                                                               
TOTSAL   DS    PL6                 MNTHLY BUCKET-TOT SAL EARNED (CLI)           
TOTBKQ   EQU   *-TOTBUK                                                         
TOTHRS   DS    PL6                 TOTAL HRS                                    
TOTDIFF  DS    PL6                 DIFF BTW SAL EARNED (CLI) & (PRD)            
TOTPST   DS    PL6                 MONTHLY POSTED TO TOYOTA CLIENT              
TOTTOTAL DS    PL6                 TOTAL CHARGE                                 
TOTNUMBK EQU   (*-TOTBUK)/TOTBKQ                                                
TOTLEN   EQU   *-TOTD              RECORD LENGTH                                
         EJECT                                                                  
**********************************************************************          
...
BUFMNTH  DS    CL2                 YEAR/MONTH PACKED                            
         DS    CL4                 ADDED FOR OFFICE/DEPT OF PERSON              
         DS    CL24                SPARE                                        
BUFKLEN  EQU   *-BUFD                                                           
*                                                                               
BUFBK    DS    PL8                                                              
...
BUFPRDHR DS    PL(BUFBKLN)         HOURS WORKED ON PRODUCT (REQUESTED)          
BUFPHRA  DS    PL(BUFBKLN)         HOURS WORKED ON PRODUCT (ALL PRODS)          
BUFCLIHR DS    PL(BUFBKLN)         HOURS WORKED CLIENT (TOYOTA/LEXUS)           
BUFTHRS  DS    PL(BUFBKLN)         TOTAL HOURS WORKED                           
BUFYPHRS DS    PL(BUFBKLN)         YTD HOURS WORKED ON PRODUCT (REQ)            
BUFYPHRA DS    PL(BUFBKLN)         YTD HOURS WORKED ON PRODUCT (ALL)            
...
TCQOFF   DS    CL4                 OFFICE DEPARTMENT OF PERSON                  
TCQKLEN  EQU   *-TCQD                                                           
TCQBK    DS    PL8                                                              
TCQBKLN  EQU   *-TCQBK                                                          
         ORG   TCQBK                                                            
...
PYPHRS   DS    CL11                YTD PRODUCT HOURS 99999.99-                  
         DS    CL1                                                              
PYPCT    DS    CL11                YTD ACTUAL PCT                               
         DS    CL1                                                              
PYRATE   DS    CL7                 HOURLY RATE 999.99-                          
         DS    CL1                                                              
         ORG   PYPCT                                                            
PYTPHR   DS    CL11                YTD TOTAL PROD HRS FOR GRP REP               
         DS    CL1                                                              
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'051ACREPFR02 02/26/04'                                      
         END                                                                    
