*          DATA SET REVIEW1A   AT LEVEL 007 AS OF 09/21/05                      
*          DATA SET REVIEW1A   AT LEVEL 005 AS OF 11/19/99                      
*PHASE T8171AA                                                                  
VIEW1A   TITLE 'KA REPORT FOR VIEWER'                                           
VIEW1A   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,REVIEW1A,R7,RR=RE                                              
         USING TWAD,RA                                                          
         USING WORKD,R9                                                         
         L     R8,AGWORK                                                        
         USING GWORKD,R8                                                        
...
***********************************************************************         
         SPACE 1                                                                
DISGRP   MVC   FVIFLD(L'REKAGRP),FLTIFLD                                        
         B     EXITOK                                                           
         SPACE 1                                                                
***********************************************************************         
...
DLIN00   LH    R1,LSROWREP                                                      
         ...
         MVC   IOKEY,REKAKEY       SET CURRENT RECORD INTO IO2                  
...
*                                                                               
         MVC   IOKEY,0(R4)         SET KEY IN IOKEY                             
         CLC   REKAKEY(REKARNK-REKAKEY),REC.REKAKEY                             
...
*                                                                               
         ...
         CLC   REKAADAG-REKAKEY(L'REKAADAG,RF),REC.REKAADAG                     
         BNE   DLIN04              YES                                          
*                                                                               
...
DLIN04   CLI   FRSTLINE,C'1'       FIRST LINE ONLY DISPLAYED?                   
         ...
         CLC   REC.REKAADV,BCEFFS  ALL/ALL?                                     
         BNE   *+14                NO                                           
         CLC   REC.REKAAGY,BCEFFS  DON'T SHOW SECOND 'ALL'                      
         BE    DLIN06                                                           
         GOTOX DOAGY,BOPARM,AIO1                                                
         MVC   L1.L2AGY,BOWORK1                                                 
...
*                                                                               
         L     R4,AIO1                                                          
         CLC   REC.REKAAGY,BCEFFS  RANK                                         
         BNE   *+14                                                             
         CLC   REC.REKAADV,BCEFFS                                               
         BE    DLIN10                                                           
         ICM   RF,15,REC.REKARNK                                                
         CURED (RF),(4,L1.L1RANK),0,DMCB=BOPARM,ALIGN=LEFT,ZERO=NOBLANK         
*                                                                               
...
GETDATE  NTR1  ,                                                                
         XC    IOKEY,IOKEY                                                      
X        USING REKAKEY,IOKEY                                                    
         L     RF,AIOREC                                                        
         USING REKAKEY,RF                                                       
         MVC   X.REKARPT,=AL2(REKARPTQ)                                         
         MVC   X.REKAREP,CUAALF                                                 
...
X        USING RAGYRECD,IOKEY                                                   
         MVI   X.RAGYKTYP,X'0A'                                                 
         MVC   X.RAGYKAGY,REKAAGY                                               
         MVC   X.RAGYKAOF,REKAAGY+L'RAGYKAGY                                    
...
*                               ** PERIOD IS YYMM BINARY                        
         ...
         CLI   REKAPRD+1,X'0D'      IF MM IS 'OD' THIS IS A YRLY TOTAL          
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         MVC   L1PED(L'ALL),ALL                                                 
         CLI   REKAPRD+1,FF         IF MM IS 'FF' PUT OUT 'ALL'                 
         BE    DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         ...
         MVI   L1PED+2,C'4'                                                     
*                                                                               
DOL00    OC    REKATOT,REKATOT      IF SET THIS IS SUB-TOTAL LINE               
         BNZ   DOL01                                                            
*                                                                               
         MVC   L1PED,BCSPACES                                                   
         XR    RF,RF                                                            
...
SCRL02   MVC   0(L'GROUP,R4),GROUP                                              
         MVI   L'GROUP(R4),C'='                                                 
         LA    R4,L'GROUP+1(R4)                                                 
         MVC   0(L'REKAGRP,R4),REKAGRP                                          
         CLC   REKAGRP,BCSPACES   IS THIS FOR ALL GROUPS?                       
         BH    SCRL04             NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
         LA    R4,L'ALL(R4)                                                     
         B     *+8                                                              
SCRL04   LA    R4,L'REKAGRP(R4)                                                 
         MVI   0(R4),C','                                                       
         LA    R4,1(R4)                                                         
*                               ** STATION FILTER                               
...
*                               ** OFFICE FILTER                                
         ...
         MVI   L'OFFICE(R4),C'='                                                
         LA    R4,L'OFFICE+1(R4)                                                
         MVC   0(L'REKAOFF,R4),REKAOFF                                          
         CLC   REKAOFF,BCEFFS     IS THIS FOR ALL OFFICES?                      
         BNE   *+10               NO                                            
         MVC   0(L'ALL,R4),ALL                                                  
*                                                                               
...
NLST02   L     RF,AIOREC           I/S RETURNS INTO RECORD BUFFER               
         CLC   0(REKAGRP-REKAKEY,RF),LSINIKEY                                   
         BNE   EXITL               MAKE SURE SAME REPORT/REP                    
*                                                                               
         CLC   =C'DATES',REKASTN-REKAKEY(RF)                                    
         BE    NLST                                                             
*                                                                               
...
***********************************************************************         
         SPACE 1                                                                
FILTTAB  DC    AL2(00001),AL1(REKAGRP-REKADSCT,L'REKAGRP)                       
         DC    AL2(00002),AL1(REKASTN-REKADSCT,L'REKASTN)                       
         DC    AL2(00003),AL1(REKAOFF-REKADSCT,L'REKAOFF)                       
         DC    AL2(00005),AL1(REKARNK-REKADSCT,L'REKARNK)                       
         DC    AL2(EOT)                                                         
*                                                                               
FF       EQU   X'FF'                                                            
         SPACE 1                                                                
...
*        DDFLDHDR                                                               
       ...
       ++INCLUDE DDFLDHDR                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'007REVIEW1A  09/21/05'                                      
         END                                                                    
