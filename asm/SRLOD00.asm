*          DATA SET SRLOD00    AT LEVEL 006 AS OF 05/01/14                      
*PHASE T11700A                                                                  
...
MAINX    MVC   OPERPARM,SPACES     CLEAR PARMS                                  
         MVC   OPERPID,PID         SET PID AND REASON                           
         MVC   OPERWHY,REASON                                                   
         XC    LOGID,LOGID         CLEAR LOGREC                                 
         CLI   OPERPID,C' '        ANYTHING WORTH LOGGING                       
         BE    *+8                                                              
...
VLIN02   CLI   ACTN,ADEL           WANT TO DO A DELETE?                         
         ...
         B     VLIN08                                                           
*                                                                               
VLIN04   CLI   ACTN,ALOAD          TYPED LOAD INTO ACTION FIELD?                
         BE    VLIN06              YES                                          
         CLI   PRODSYS,C'N'        FORCE ONLY FOR PRODUCTION SYSTEM             
         BE    VLIN06                                                           
         CLI   DS.PSLVL,C' '       WITH LIVE PHASE LOADS                        
         BH    VLIN06                                                           
         CLI   LL.PSLVL,C' '       BUT WITH TEST LOADLIB ENTRIES                
         BNH   VLIN06                                                           
         MVI   FERN,35                                                          
         B     EXITL                                                            
*                                                                               
VLIN06   BRAS  RE,LOADIT           THEN YOU MUST BE LOADING                     
         BNE   EXITL                                                            
         MVI   FERN,0                                                           
         MVI   HDRN,2                                                           
*                                                                               
...
*                                                                               
VLIN08   GOTO1 ACALLOV,DMCB,0,(C'C',0),0                                        
         CLI   ACTN,ADEL                                                        
         BE    VLIN10                                                           
*                                                                               
         TM    PSFLAG1,CTPHSCRQ                                                 
...
*                                                                               
         DROP  R2                                                               
*                                                                               
         GOTO1 ACALLOV,DMCB,0,(C'D',0),0                                        
*                                                                               
VLIN10   MVC   LOGID,=C'$LD.'      SET UP LOGREC                                
...
*                                                                               
         MVC   LL.PSLVL,FHDA+6                                                  
         CLI   FHDA+6,C'A'         LEVEL A THRU F AND S OK FOR LOADLIB          
         BL    VPH03                                                            
         CLI   FHDA+6,C'F'                                                      
         BNH   VPH04                                                            
         CLI   FHDA+6,C'S'                                                      
         BE    VPH04                                                            
*                                                                               
VPH03    MVI   FERRDSP,6           INVALID LEVEL STAMP                          
         MVI   FERN,32                                                          
         B     EXITL                                                            
*                                                                               
VPH04    LA    R1,NOLOAD           IS PHASE ON THE RESTRICTED LIST              
VPH04A   CLI   0(R1),X'FF'                                                      
         BE    VPH05               END OF LIST, SAFE TO LOAD                    
         CLC   FHDA(6),0(R1)       ON THE LIST?                                 
         BE    VPH04B              YES                                          
         LA    R1,L'NOLOAD(,R1)                                                 
         B     VPH04A                                                           
         LA    R1,SRVP2H           R2=A(PHASE NAME INPUT FIELD)                 
VPH04B   CLI   FHILD(R1),1         ONLY ONE CHAR ALLOWED FOR LEVEL              
         BNE   VPH04C                                                           
         CLI   FHDAD(R1),C'A'      LEVEL MUST BE A/B/C                          
         BL    VPH04C                                                           
         CLI   FHDAD(R1),C'C'                                                   
         BNH   VPH05               LOADED AS A,B, OR C IS ACCEPTABLE            
VPH04C   MVI   FERN,36             SORRY NOT SAFE TO LOAD LIVE                  
         B     EXITL                                                            
*                                                                               
VPH05    MVC   LL.PSLANG,FHDA      SAVE FIRST LETTER OF PHASE NAME              
         MVC   DS.PSLANG,FHDA                                                   
*                                                                               
         L     R1,ALANGTAB                                                      
         LH    RE,0(R1)                                                         
...
*                                                                               
         ...
         BH    VAC06                                                            
         CLC   =C'FORCE',SRVP2     UNLESS YOU KNOW THE KEYWORD                  
         BE    VAC06                                                            
*                                                                               
...
VNODES   NTR1  ,                                                                
         ...
         B     EXITL                                                            
*                                                                               
VNO02    CLC   =C'SCREEN',FHDA                                                  
         BNE   VNO04                                                            
         MVI   SCREEN,C'Y'                                                      
         XC    NODES,NODES                                                      
         B     EXITOK                                                           
*                                                                               
VNO04    CLI   FHIL,2              INPUT MUST BE LENGTH 2                       
         BE    *+12                                                             
         MVI   FERN,2                                                           
         B     EXITL                                                            
...
*                                                                               
         MVI   TTEST1,TTESTURT     SET CALLOV FLAG TO USE REAL TTEST            
*NOPAHYD MVI   TTEST,TTESTSRA      *TEMP*                                       
...
*                                                                               
         ...
         B     EXITL                                                            
*                                                                               
ADD01    CLI   DS.PSLVL,0          ADDING 'LIVE' PHASE TO LIST?                 
         BNE   *+10                NO                                           
...
*                                                                               
         CLI   NODESET,C'Y'        SET NODES?                                   
         BNE   *+10                NO                                           
         MVC   PSNODES,NODES       SET NODES                                    
*                                                                               
         CLI   CORERES,C'Y'        ADD CORE-RESIDENT?                           
         BE    ADD02                                                            
         CLI   CORERES,C'D'        ADD NON CORE-RESIDENT?                       
         BE    ADD02                                                            
*                                                                               
         TM    PSFLAG1,CTPHSCRQ    CORE RESIDENT?                               
         BZ    ADD02               NO                                           
         CLI   PSNODES,0                                                        
         BE    ADD02                                                            
         MVI   FERN,34                                                          
         B     EXITL                                                            
*                                                                               
ADD02    MVC   NODES,PSNODES                                                    
*                                                                               
         CLI   PSNODES,0           ZERO NODE PROGRAMS ARE CORE-RESIDENT         
         BNE   ADD03               AND MUST BE LOADED TO CORE                   
         CLI   DS.PSOVR,SCRNQ      SCREEN?                                      
         BNL   ADD03                                                            
         CLI   SCREEN,C'Y'         FORCE SCREEN NON-CORERES                     
         BE    ADD03                                                            
*                                                                               
         CLI   CORERES,C'D'        FORCE NON CORE-RESIDENT?                     
         BE    ADD03                                                            
         OI    PSFLAG1,CTPHSCRQ    FORCE LOAD TO CORE                           
*                                                                               
ADD03    CLI   CORERES,C'Y'        ADD CORE-RESIDENT?                           
         BNE   ADD04                                                            
         ...
         BL    *+8                                                              
         OI    PSFLAG1,CTPHSCSQ                                                 
         CLI   SCREEN,C'Y'         FORCE SCREEN                                 
         BNE   *+8                                                              
         OI    PSFLAG1,CTPHSCSQ                                                 
         B     ADD06                                                            
*                                                                               
ADD04    CLI   SCREEN,C'Y'         FORCE SCREEN NON-CORERES                     
         BNE   *+8                                                              
         OI    PSFLAG1,CTPHSSCQ                                                 
*                                                                               
         CLI   CORERES,C'Q'        DUMMY PHASE                                  
         BNE   ADD06                                                            
         ...
         B     EXITL                                                            
*                                                                               
ADD06    MVC   PSADR,=AL4(PSADRL)  SET WANT TO DO LOAD                          
         XC    PSLENH,PSLENH       RESET ALL LENGTHS                            
         XC    PSLEN,PSLEN                                                      
*                                                                               
ADD08    ICM   R0,7,PSNAME                                                      
         ICM   R0,8,=C'W'                                                       
         GOTO1 ACALLOV,DMCB,PSREC,(R0),(C'D',0)                                 
         CLI   4(R1),255                                                        
         BNE   *+6                                                              
...
*                                                                               
         ...
         B     EXITL                                                            
*                                                                               
LIT04    CLI   CORERES,C'Y'        ADD CORE-RESIDENT?                           
         BNE   EXITOK              NO                                           
         B     EXITOK                                                           
*                                                                               
         GOTO1 ACALLOV,DMCB,0,(C'R',DKEY),0                                     
         DROP  R2                                                               
         EJECT                                                                  
***********************************************************************         
...
*                                                                               
         ...
         B     EXITL                                                            
*                                                                               
         MVC   PID,SC1STFLD        SAVE ID VALUE INPUT                          
*                                                                               
...
XMOD     L     RD,SAVERD                                                        
         ...
         MVC   TTEST,SAVETEST                                                   
         MVC   TTEST1,SAVETST1                                                  
         MVC   TLANG,SAVELANG      SAVE USER LANGUAGE CODE                      
         NI    TSTATC,255-TSTCLOAD                                              
         XMOD1 ,                                                                
         DROP  RF                                                               
         EJECT                                                                  
***********************************************************************         
...
*&&                                                                             
*&&US                                                                           
SCRNQ    EQU   X'A0'               ASSUME SCREEN IF GT X'AA'                    
*&&                                                                             
         LTORG                                                                  
         EJECT                                                                  
***********************************************************************         
...
*                                                                               
* A LIVE LOAD OF THESE PHASES IS UNSAFE AND NOT ALLOWED                         
NOLOAD   DC    0CL6                ** DO NOT =LOAD THESE PHASES LIVE **         
         DC    CL6'T00A7A'         DDSTAPACK                                    
         DC    CL6'T00A9E'         SPCBLLIST                                    
         DC    X'FF'                                                            
         EJECT                                                                  
...
OK6      DC    CL(IMSGL)'Attempting Rebuild'                                    
         SPACE 1                                                                
***********************************************************************         
* ERROR MESSAGES                                                      *         
***********************************************************************         
...
ERR13    DC    CL(EMSGL)'Cannot delete a production phase w/o "FORCE"'          
...
ERR20    DC    CL(EMSGL)'Pid and Reason required for production load'           
...
*&&US*&& DC    CL(EMSGL)'Level must be <live>, A, B, C or S'                    
*&&UK*&& DC    CL(EMSGL)'Level must be <live>, A, B, C or X'                    
ERR33    DC    CL(EMSGL)'Core space is full - can''t load phase'                
ERR34    DC    CL(EMSGL)'Nodes/Coreres don''t match - specify, please'          
ERR35    DC    CL(EMSGL)'Loading test phase live requires "LOAD" here'          
ERR36    DC    CL(EMSGL)'You are not allowed to load this phase live!'          
         EJECT                                                                  
***********************************************************************         
...
*CTGENPHASE                                                                     
       ...
       ++INCLUDE CTGENPHASE                                                     
         ...
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'006SRLOD00   05/01/14'                                      
         END                                                                    
