*          DATA SET PRSFM11    AT LEVEL 075 AS OF 04/13/11                      
*PHASE T41C11A,*                                                    L01         
         TITLE 'T41C11  CHANGE LOG'                                 L01         
***********************************************************************         
*                                                                               
* BPLA 05/10     ADD MAIN PST CODE FOR USE FOR MOS JUL/10 AND AFTER             
*                                                                               
* SMYE 07/02     IN VK IF REPORTING AND LIMIT ACCESS ACTIVE, REQUIRE            
*                CLIENT CODE ENTRY                                              
*                                                                               
* SMYE 11/06/00  ADDED CLIENT VALIDATION (SECURITY) TO DK                       
*                                                                               
* KWAN 1/6/99    CK FOR CLT BEFORE VALIPRD AND CK FOR CLT & PRD                 
*                BEFORE VALIEST. ERROR MSG, IF INPUT IS MISSING.                
*                                                                               
* SMYE 9/97      ACCEPT ALL IN PRODUCT FOR LIST OR REPORT                       
*                      (TREAT AS NO INPUT)                                      
*                                                                               
* BPLA 7/97      IF NO INPUT IN ACC ACCOUNTS - SKIP CHECK OF ACCFILE            
*                                                                               
* SMYE 3/97      DISALLOW MORE THAN 1 PST CODE ENTRY (AT VALPST)                
*                                                                               
* SMYE 7/96      ADD LOGIC FOR STANDARD COMMENT                                 
*                                                                               
...
T41C11   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,T41C11,R7         NOTE USE OF R7 AS SECOND BASE                
         L     RC,0(R1)                                                         
         USING GEND,RC                                                          
         L     RA,ATWA                                                          
...
*                                                                               
         ...
         BE    VK1                                                              
*NOP*    CLI   ACTNUM,X'11'        OK IF NOW                       L03          
*NOP*    BE    VK3                                                 L03          
         B     VK2                                                              
*                                                                               
VK1      DS    0H                                                               
...
VK3      XC    SVPRD,SVPRD                                                      
         ...
         CLC   8(3,R2),=C'ALL'  TREAT ALL LIKE NO INPUT             L01         
         BE    VK3D                                                 L01         
         CLI   5(R2),0             TEST ANY INPUT                               
         BNE   VK4                                                              
...
VK4      DS    0H                                                               
*                                                                               
         CLI   AGRCLIH+5,0         ANYTHING IN CLI FIELD?                       
         BNE   *+16                                                             
         LA    R2,AGRCLIH                                                       
         MVI   ERROR,MISSING       CLI MUST EXIST FOR VALIPRD                   
         B     TRAPERR                                                          
*                                                                               
         GOTO1 VALIPRD                                                          
...
*                                                                   L01         
         ...
         BNE   *+16                                                             
         LA    R2,AGRCLIH                                                       
         MVI   ERROR,MISSING       CLI MUST EXIST FOR VALIEST                   
         B     TRAPERR                                                          
         CLI   AGRPROH+5,0         ANYTHING IN PRD FIELD?                       
         BNE   *+16                                                             
         LA    R2,AGRPROH                                                       
         MVI   ERROR,MISSING       PRD MUST EXIST FOR VALIEST                   
         B     TRAPERR                                                          
*                                                                   L01         
         GOTO1 VALIEST                                              L01         
...
*                                  RESTORE KEY AND RECORD                       
         MVC   KEY(L'AORKEY),SVKEY                                              
         GOTO1 HIGH                                                             
         CLC   KEY(L'AORKEY),SVKEY       SAME RECORD ?                          
         BE    *+6                 YES - OK                                     
...
*                                                                               
         ...
         DROP  R6                                                               
*                                                                               
VR2      MVI   ELCODE,X'02'        REMOVE ADDRESS ELEM                          
         GOTO1 REMELEM                                                          
*                                                                               
VR2C     XC    ELEM,ELEM           BUILD NEW ELEMENTS                           
         LA    R6,ELEM                                                          
         USING AORADREL,R6                                                      
...
VR9C     DS    0H                                                               
         CLI   5(R2),0            ANY INPUT                                     
         BE    VR9E                NO SKIP ACCOUNT FILE READ                    
         LA    R3,AORRCVBL                                                      
...
*GEN                                                                            
         PRINT NOGEN                                                            
*GEN                                                                            
         DS    0H                      STANDARD COMMENT                         
         MVI   ELCODE,X'66'                                                     
         GOTO1 REMELEM                 REMOVE STANDARD COMMENT ELEM             
         LA    R2,AGRCMTH                                                       
*                                                                               
         CLI   5(R2),0                 CHK FOR INPUT                            
         BE    VR900                   NO - DONE                                
         CLI   5(R2),6                                                          
         BH    VR99                    INVALID (LENGTH)                         
         XC    KEY+10(10),KEY+10                                                
         MVC   KEY+4(6),SPACES                                                  
         SR    R1,R1                                                            
         IC    R1,5(R2)                                                         
...
*                                                                               
         ...
         CLC   KEY(20),KEYSAVE                                                  
         BE    VR10                                                             
         B     VR99                INVALID - COMMENT REC NOT FOUND              
*                                                                               
VR10     XC    ELEM,ELEM                                                        
         LA    R6,ELEM                                                          
         MVC   ELEM(2),=X'6609'    ELEM CODE AND LENGTH                         
         MVC   ELEM+2(6),KEY+4                                                  
*                                                                               
         GOTO1 ADDELEM                                                          
*                                                                               
         MVC   KEY(31),SVKEY           RESTORE AOR HDR KEY                      
         B     VR900                                                            
*                                                                               
VR99     DS    0H                                                               
         MVI   ERROR,INVALID                                                    
...
*                                                                               
         BAS   RE,DISPMPST        DISPLAY MAIN PST                              
         OC    PSTOUT,PSTOUT                                                    
         BZ    DR17E                                                            
         MVC   AGRMPST,PSTOUT       OUTPUT                                      
         OI    AGRMPSTH+6,X'80'                                                 
*                                                                               
DR17E    BAS   RE,DISPPST         DISPLAY PST VALUES                            
         OC    PSTOUT,PSTOUT                                                    
         BZ    DR18                                                             
...
*                              TEST PSTOUT FOR MORE THAN ONE ENTRY              
         ...
         LA    R0,10               LOOP COUNTER                                 
         LH    RF,=H'1'                                                         
VPMLUP   CLI   0(R1),0             ANY ENTRY ?                                  
         BE    VPM5                NO                                           
         LA    RE,1(RE)            YES - ADD TO ENTRY COUNT                     
         MVC   AORMPSTC,0(R1)      SAVE CODE                                    
         STC   RF,AORMPSTP         SAVE PROVINCE #                              
VPM5     LA    RF,1(RF)                                                         
         LA    R1,1(R1)            BUMP TO NEXT BYTE                            
         BCT   R0,VPMLUP                                                        
         CH    RE,=H'1'            ONE ENTRY ?                                  
...
DPX      B     EXIT                                                             
*                                                                               
*        DISPLAY MAIN PST CODE                                                  
*                                                                               
DISPMPST NTR1                                                                   
***      LA    R6,ELEM                                                          
         USING AORELEM,R6                                                       
         XC    PSTOUT,PSTOUT                                                    
         OC    AORMPSTP(2),AORMPSTP  IS THERE ANYTHING TO DISPLAY               
         BZ    DPMX                                                             
*                                                                               
         LA    R4,BLOCK                                                         
         USING PSTBLKD,R4                                                       
         ...
         XC    WORK(10),WORK                                                    
         LA    R1,WORK                                                          
         ZIC   RF,AORMPSTP         PROVINCE #                                   
         SH    RF,=H'1'                                                         
         AR    R1,RF                                                            
         MVC   0(1,R1),AORMPSTC    SET CODE IN PST LIST                         
         LA    R1,WORK                                                          
         ST    R1,PSTADIN          INPUT ADDRESS                                
         LA    R1,PSTOUT                                                        
         ST    R1,PSTADOUT         OUTPUT ADDRESS                               
...
*                                                                               
CHKA8    DS    0H                                                               
         L     RF,ACOMFACS         SWITCH BACK TO PRINT                         
         L     RF,CSWITCH-COMFACSD(RF)                                          
...
*                                                                               
         OC    AORKEST,AORKEST                                                  
         BZ    PRADDR                                                           
         EDIT  (B2,AORKEST),(4,P1+9)                                            
         USING AORADREL,R6                                                      
PRADDR   MVI   ELCODE,02                                                        
         BAS   RE,GETEL            GET THE AOR ADDRESS ELEMENT                  
         BE    *+6                                                              
         DC    H'0'                IF NO AOR ADDRESS ELEMENT, DIE               
*                                                                               
         MVI   SPACING,2                                                        
*                                                                               
         MVC   P1+16(L'AORLIN1),AORLIN1                                         
         CLC   AORLIN2(L'AORLIN2),SPACES                                        
         BNH   *+10                                                             
         MVC   P2+16(L'AORLIN2),AORLIN2                                         
*                                                                               
         CLC   AORLIN3(L'AORLIN3),SPACES                                        
         BNH   *+10                                                             
         MVC   P3+16(L'AORLIN3),AORLIN3                                         
*                                                                               
         CLC   AORLIN4(L'AORLIN4),SPACES                                        
         BNH   *+10                                                             
         MVC   P4+16(L'AORLIN4),AORLIN4                                         
*                                                                               
         L     R6,AIO                                                           
         LA    R6,33(R6)                                                        
         MVI   ELCODE,3                                                         
         BAS   RE,FIRSTEL          GET THE AOR INFO ELEMENT                     
         BE    *+8                                                              
         B     PR80                                                             
         USING AORELEM,R6                                                       
*                                                                               
         EDIT  (B4,AORPCT),(10,P1+44),4,FLOAT=-                                 
...
*                                                                               
PR60     MVC   P1+55(12),WORK                                                   
         OC    AOREFF,AOREFF        CHK FOR DATE                                
         BZ    PR65                                                             
         GOTO1 DATCON,DMCB,(3,AOREFF),(9,P1+71)                                 
*                                                                               
PR65     OC    AORKILL,AORKILL                                                  
         BZ    PR70                                                             
         GOTO1 DATCON,DMCB,(3,AORKILL),(9,P1+81)                                
*                                                                               
PR70     MVC   P1+90(1),AORGSTCD                                                
         BAS   RE,DISPPST                                                       
         OC    PSTOUT,PSTOUT                                                    
         BZ    PR72                                                             
         MVC   P2+90(L'PSTOUT),PSTOUT                                           
*                                                                               
PR72     BAS   RE,DISPMPST                                                      
         OC    PSTOUT,PSTOUT                                                    
         BZ    PR75                                                             
         CLC   P2+90(4),SPACES                                                  
         BNH   PR72B                                                            
         MVC   P3+90(08),=C'MAIN PST'                                           
         MVC   P3+99(4),PSTOUT                                                  
         B     PR75                                                             
*                                                                               
PR72B    DS    0H                                                               
         MVC   P2+90(08),=C'MAIN PST'                                           
         MVC   P2+99(4),PSTOUT                                                  
         B     PR75                                                             
*                                                                               
PR75     MVC   P1+94(L'AORRCVBL),AORRCVBL                                       
         MVC   P1+108(L'AORCOMM),AORCOMM                                        
*                                                                               
PR80     L     R6,AIO                                                           
         MVI   ELCODE,X'66'                                                     
         BAS   RE,GETEL            GET THE AOR COMMENT ELEMENT                  
         BNE   PR100                                                            
         USING AORCOMEL,R6                                                      
         MVC   P1+124(L'AORCOM1),AORCOM1                                        
*                                                                               
*                                                                               
*                                                                               
*                                                                               
...
*                                                                               
*                                                                               
         SSPEC H6,83,C'KILL   GST'                                              
         SSPEC H6,124,C'STANDARD'                                               
         SSPEC H7,48,C'COMM. PCT. BASIS        EFF-DATE   DATE   CODE PX        
               AY/REC ACCT'                                                     
         SSPEC H8,48,C'---------- -----        --------  ------  ---- -X        
               ------------'                                                    
...
*                                                                               
         SSPEC H7,109,C'INCOME ACCOUNT  COMMENT'                                
         SSPEC H8,109,C'--------------  -------'                                
         DC    X'00'                                                            
*                                                                               
*GEN                                                                            
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'075PRSFM11   04/13/11'                                      
         END                                                                    
