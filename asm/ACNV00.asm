*          DATA SET ACNV00     AT LEVEL 085 AS OF 09/17/14                      
*PHASE ACNV00A                                                                  
*INCLUDE ACRECTYP                                                               
*INCLUDE BUFFAHI                                                                
*INCLUDE BINSRCH2                                                               
...
PPERK3   CLC   RECTYP,0(R5)        MATCH RECORD TYPE                            
         BE    PPERK5                                                           
         CLI   0(R5),EOT                                                        
         BNE   *+6                                                              
...
*                                                                               
         LA    RE,DKEY                                                          
         TM    RDINDS,RECIPAS2         TEST 2 BYTE RECORD TYPE                  
         BNO   SKIP3                                                            
         LA    RE,1(RE)                BUMP PAST FIRST BYTE OF REC TYP          
         XC    DKEY+2(L'ACCKEY-2),DKEY+2                                        
         B     *+10                                                             
SKIP3    XC    DKEY+1(L'ACCKEY-1),DKEY+1                                        
...
DUPCK5   SR    R1,R1                                                            
         ...
         ICM   RF,7,1(R7)                                                       
         MVC   FPRMS(FPLNQ),0(RF)                                               
         L     R1,FTAB                    FTAB=A(START OF TABLE)                
         LR    R5,R1                                                            
         SR    R0,R0                                                            
         IC    R0,FDPK                    FDPK=DISP TO PACKD COUNTR             
         AR    R5,R0               R5=A(PACKED FIELD COUNTER)                   
         L     R4,FNUM                    FNUM=NUMBER OF RECORDS                
         MH    R4,FRLN+2                  DATA LENGTH                           
         AR    R4,R1               R4=A(END OF TABLE +1)                        
         A     R1,FDSP             R1=A(KEY)                                    
         L     R6,FKLN                    FKLN=L'RECORD                         
         BCTR  R6,0                R6=LAST BYTE OF RECORD                       
*                                                                               
DUPCK20  LR    R2,R1                                                            
         ...
         B     DUPCK20                                                          
* - - - - -                                                                     
DUPCK30  CLI   ACTNSW,ACTNCHG      CHANGE ?                                     
         BNE   DUPCK60                                                          
*                                                                               
         L     R5,FTAB                    FTAB=A(START OF TABLE)                
         SR    R0,R0                                                            
         IC    R0,FDPK                    FDPK=DISP TO PACKD COUNTR             
         AR    R5,R0               R5=A(PACKED FIELD COUNTER)                   
         LR    R3,R5               R3=A(PACKED FIELD COUNTER)                   
*                                                                               
         L     R1,FTAB                    FTAB=A(START OF TABLE)                
         A     R1,FDSP                    FDSP=DISP TO KEY                      
         L     R0,FKLN             DEFAULT DISP TO NEW DATA                     
         CLI   FDND,0              TEST ANY OVERRIDE                            
         BE    *+8                                                              
         IC    R0,FDND                    FDND=DISP TO NEW DATA                 
         AR    R1,R0               R1=A(KEY) FOR NEW DATA                       
         LR    R2,R1               R2=A(KEY) FOR NEW DATA 2                     
*                                                                               
DUPCK40  CR    R2,R4               EOT FOR ALL RECORDS?                         
         BNL   DUPCK60             YES                                          
*                                                                               
         A     R1,FRLN                    FRLN=LENGTH OF RECORD                 
         A     R5,FRLN                                                          
         CR    R1,R4               EOT FOR CURRENT RECORD?                      
         BNL   DUPCK50             YES                                          
         EX    R6,*+8                                                           
         B     *+10                                                             
         CLC   0(0,R1),0(R2)       FOUND A DUPLICATE?                           
         BNE   DUPCK40              NO                                          
         OC    DUPERR,0(R7)        YES, SET ERROR BIT                           
         ZAP   0(5,R3),=P'1'       MARK COUNTER                                 
         B     DUPCK40                                                          
*                                                                               
DUPCK50  A     R2,FRLN             BUMP TO NEXT RECORD                          
         LR    R1,R2                                                            
         A     R3,FRLN             BUMP TO NEXT RECORD                          
         LR    R5,R3                                                            
         B     DUPCK40                                                          
*                                                                               
DUPCK60  LA    R7,4(R7)                                                         
...
PAR5     A     R2,PARINP           R2=INPUT DATA                                
         ...
         LA    R5,2(R5)            R5=START OF NEXT FIELD IN PARCTAB            
         BCT   R0,PAR3                                                          
*                                                                               
PAR7     CH    R7,=H'4'            MUST BE 4 LEVELS                             
         BNE   PARX                                                             
*                                                                               
         LA    RF,L'ACCO                                                        
         LA    R1,ACCO+(L'ACCO-1)                                               
...
PAR15    A     R2,PAROUT           R2=A(OUTPUT DATA)                            
         CLI   1(R5),0             LENGTH OF OUTPUT FIELD IN PARCTAB            
         BNH   PAR16                                                            
         ICM   R7,1,1(R5)          LENGTH OF KEY FIELD                          
         BCTR  R7,0                                                             
         EX    R7,*+4                                                           
         MVC   0(0,R2),SPACE                                                    
*                                                                               
PAR16    BCTR  R1,0                                                             
         CLI   0(RE),C' '          TEST ANY DATA                                
         BNH   *+14                                                             
         EX    R1,*+4                                                           
         MVC   0(0,R2),0(RE)                                                    
         LA    R1,1(R1)            R1=LENGTH OF FIELD                           
...
*                                                                               
         ...
         DC    AL1(RECIDIR+RECIPAS+RECIPAS2)                                    
         DC    AL4(0)                                                           
         DC    XL17'00'                                                         
*                                                                               
...
*                                                                               
         ...
         DC    XL17'00'                                                         
*                                                                               
         DC    AL1(ACRTPER),AL3(PERK),CL8'ACRTPER'                              
...
*                                                                               
         ENTRY PERT                                                             
PERT     DS    0XL9          SPLIT PERSON KEY                                   
         DC    AL1(ACRTPER)                  PERSON RECORD                      
         DC    AL1(0,0)                           OFFICE                        
         DC    AL1(0,0)                           DEPT                          
         DC    AL1(0,0)                           SUB-DEPT                      
         DC    AL1(PERKCODE-PERRECD,L'PERKCODE)   STAFF                         
*                                                                               
         DC    AL1(ACRTPCR)                  PRODUCTION CHARGE RATE             
         DC    AL1(PCHKDOF-PCHRECD,L'PCHKDOF)     OFFICE                        
         DC    AL1(PCHKDEP-PCHRECD,L'PCHKDEP)     DEPT                          
         DC    AL1(PCHKSUB-PCHRECD,L'PCHKSUB)     SUB-DEPT                      
         DC    AL1(PCHKSTF-PCHRECD,L'PCHKSTF)     STAFF                         
*                                                                               
         DC    AL1(ACRTPAR)                  PRODUCTION ADJ. RATE               
         DC    AL1(PAJKDOF-PAJRECD,L'PAJKDOF)     OFFICE                        
         DC    AL1(PAJKDEP-PAJRECD,L'PAJKDEP)     DEPT                          
         DC    AL1(PAJKSUB-PAJRECD,L'PAJKSUB)     SUB-DEPT                      
         DC    AL1(PAJKSTF-PAJRECD,L'PAJKSTF)     STAFF                         
*                                                                               
         DC    AL1(ACRTPHI)             PAYROLL HISTORY                         
         DC    AL1(PHIKOFC-PHIRECD,L'PHIKOFC)     OFFICE                        
         DC    AL1(PHIKDPT-PHIRECD,L'PHIKDPT)     DEPT                          
         DC    AL1(PHIKSBD-PHIRECD,L'PHIKSBD)     SUB-DEPT                      
         DC    AL1(PHIKPER-PHIRECD,L'PHIKPER)     STAFF                         
*                                                                               
         DC    AL1(ACRTCAP)             COST ALLOCATION PROFILE                 
         DC    AL1(CAPKOFC-CAPRECD,L'CAPKOFC)     OFFICE                        
         DC    AL1(CAPKDPT-CAPRECD,L'CAPKDPT)     DEPT                          
         DC    AL1(CAPKSDT-CAPRECD,L'CAPKSDT)     SUB-DEPT                      
         DC    AL1(CAPKPER-CAPRECD,L'CAPKPER)     STAFF                         
*                                                                               
         DC    AL1(ACRTSTD)             STANDARD HOURS                          
         DC    AL1(STDKOFC-STDRECD,L'STDKOFC)     OFFICE                        
         DC    AL1(STDKDPT-STDRECD,L'STDKDPT)     DEPT                          
         DC    AL1(STDKSBD-STDRECD,L'STDKSBD)     SUB-DEPT                      
         DC    AL1(STDKPER-STDRECD,L'STDKPER)     STAFF                         
*                                                                               
         DC    AL1(ACRTEDT)             EDIT HOURS                              
         DC    AL1(EDTKOFC-EDTRECD,L'EDTKOFC)     OFFICE                        
         DC    AL1(EDTKDPT-EDTRECD,L'EDTKDPT)     DEPT                          
         DC    AL1(EDTKSBD-EDTRECD,L'EDTKSBD)     SUB-DEPT                      
         DC    AL1(EDTKPER-EDTRECD,L'EDTKPER)     STAFF                         
*                                                                               
         DC    AL1(ACRTSSAV)            TIME SHEET SAVE                         
         DC    AL1(TSSKODS-TSSRECD,L'TSSKODS)     OFFICE                        
         DC    AL1(TSSKODS+2-TSSRECD,2)           DEPT                          
         DC    AL1(TSSKODS+4-TSSRECD,3)           SUB-DEPT                      
         DC    AL1(TSSKPER-TSSRECD,L'TSSKPER)     STAFF                         
*                                                                               
         DC    AL1(ACRTTPOX)            TEMP X-REF                              
         DC    AL1(TSXKODS-TSXRECD,L'TSXKODS)     OFFICE                        
         DC    AL1(TSXKODS+2-TSXRECD,2)           DEPT                          
         DC    AL1(TSXKODS+4-TSXRECD,3)           SUB-DEPT                      
         DC    AL1(TSXKPER-TSXRECD,L'TSXKPER)     STAFF                         
*                                                                               
         DC    AL1(EOT)                                                         
         EJECT                                                                  
***********************************************************************         
...
* DDDPRINT                                                                      
       ...
       ++INCLUDE DDDPRINT                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'085ACNV00    09/17/14'                                      
         END                                                                    
