*          DATA SET RESTX11    AT LEVEL 029 AS OF 05/03/11                      
*PHASE T83D11A                                                                  
         TITLE 'RESTX11  - CONTRACT WORKSHEET DOWNLOAD'                         
*INCLUDE REGENSTC                                                               
***********************************************************************         
*                                                                     *         
*         RESTX11 --- GENERATE CONTRACT ORDER WORKSHEET FOR DOWNLOAD  *         
*                                                                     *         
* ------------------------------------------------------------------- *         
* UPDATE HISTORY:                                                     *         
*                                                                     *         
...
RESTXWKS CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 MYWRKLQ,RESTX11,CLEAR=YES                                        
         LR    RA,RC                                                            
         USING MYWORKD,RA                                                       
         L     RC,4(R1)                                                         
         USING WORKD,RC                                                         
...
*                                                                               
FLTDT10  EQU   *                                                                
         GOTO1 VDATCON,DMCB,(3,0(R3)),(1,WORK)                                  
         GOTO1 AADDDATA,DMCB,AFABLK,WKSFLTST,WORK,0                             
         GOTO1 VDATCON,DMCB,(3,3(R3)),(1,WORK)                                  
...
*                                                                               
*                                                                               
* INSERT NON-DISCRIMINATION CLAUSE ON THE FLY VIA STORED COMMENTS               
*                                                                               
         XC    WORK,WORK                                                        
         MVC   WORK(2),=X'0207'                                                 
         MVC   WORK+2(5),=C'SC=* '                                              
...
PTPEMX   DS    0H                                                               
*                                                                               
*   ESTABLISH ORDER TYPE                                                        
*                                                                               
         GOTO1 =A(ORDTYP),RR=Y                                                  
*                                                                               
         GOTO1 =A(PENDSTUF),RR=Y                                                
*                                                                               
         GOTO1 =A(MKTL),RR=Y                                                    
*                                                                               
         B     EXIT                                                             
...
WKORDTYP DC    AL1(FLDLEN),AL2(0064),C'WORTY',AL1(MDTCHQ),AL1(001)              
WKPENLEN DC    AL1(FLDLEN),AL2(0065),C'PLEN ',AL1(MDTBIQ),AL1(002)              
WKPENDPT DC    AL1(FLDLEN),AL2(0066),C'PDYPT',AL1(MDTCHQ),AL1(001)              
WKPENCPP DC    AL1(FLDLEN),AL2(0067),C'PCPP ',AL1(MDTBIQ),AL1(004)              
         DC    X'00'                                                            
WKSDATAX EQU   *                                                                
         EJECT                                                                  
         PRINT OFF                                                              
...
*                                                                               
SCRATCH  DS    CL1                 MISCELLANEOUS ONE-BYTE                       
*                                                                               
...
CMPFULL  DS    F                                                                
*                                                                               
*   DOAGY:  MOVED OUT OF LINE FOR ADDRESSABILITY                                
*                                                                               
DOAGY    NTR1  LABEL=*,BASE=*                                                   
*                                                                               
         XC    WORK,WORK                                                        
...
*                                                                               
         ...
         EJECT                                                                  
*                                                                               
*   ORDTYP:  SET ORDER TYPE:  MANUAL, DARE, OR XML                              
*                                                                               
ORDTYP   NTR1  LABEL=*,BASE=*                                                   
*                                                                               
         MVI   SCRATCH,C'M'        SET ORDER TYPE TO 'MANUAL'                   
         LA    R6,RCONREC                                                       
         MVI   ELCODE,X'1D'        DARE ELEMENT                                 
         BAS   RE,GETEL                                                         
         JNE   ORDTYPX             NOT FOUND                                    
*                                                                               
         MVI   SCRATCH,C'D'        SET ORDER TYPE TO 'DARE'                     
         USING RCONDREL,R6                                                      
         TM    RCONDRF2,X'01'      LINKED TO XML ORDER?                         
         BNO   ORDTYPX             NO  - LEAVE AS 'DARE'                        
         MVI   SCRATCH,C'X'        SET ORDER TYPE TO 'XML'                      
         DROP  R6                                                               
ORDTYPX  EQU   *                                                                
         GOTO1 AADDDATA,DMCB,AFABLK,WKORDTYP,SCRATCH,0                          
         XIT1                                                                   
         LTORG                                                                  
         EJECT                                                                  
*                                                                               
*   PENDSTUF:  SET LENGTH, DAYPARTS, CPP FROM PENDING                           
*                                                                               
PENDSTUF NTR1  LABEL=*,BASE=*                                                   
*                                                                               
         LA    R6,RCONREC                                                       
         MVI   ELCODE,X'12'        PENDING ELEMENT                              
         BAS   RE,GETEL                                                         
         JNE   PSTF0040            NOT FOUND                                    
*                                                                               
         USING RSARXEL,R6                                                       
         LA    R4,RSARXRFR         SET A(LENGTHS)                               
         LA    R2,6                SET MAX COUNT                                
PSTF0020 EQU   *                                                                
         OC    0(2,R4),0(R4)       ANY LENGTH?                                  
         BZ    PSTF0040            NO  - FINISHED                               
*                                                                               
         MVC   WORK+20(2),0(R4)    EXTRACT LENGTH FROM ELEMENT                  
         GOTO1 AADDDATA,DMCB,AFABLK,WKPENLEN,WORK+20,0                          
         LA    R4,2(R4)            BUMP TO NEXT LENGTH                          
         BCT   R2,PSTF0020         GO BACK FOR NEXT                             
         DROP  R6                                                               
PSTF0040 EQU   *                                                                
         LA    R6,RCONREC                                                       
         MVI   ELCODE,X'41'        PENDING ELEMENT                              
         BAS   RE,GETEL                                                         
         JNE   PSTF0080            NOT FOUND                                    
*                                                                               
         USING RCPRDPEL,R6                                                      
         LA    R4,RCPRDPDP         SET A(DYPART/CPP)                            
*                                                                               
*   CALCULATE NUMBER OF DAYPARTS / CPPS IN ELEMENT                              
*                                                                               
         SR    RE,RE                                                            
         ZIC   RF,1(R6)            GET ELEMENT LENGTH                           
         SH    RF,=H'3'            SUBTRACT L(CONTROL)+ FLAGS BYTE              
         LTR   RF,RF                                                            
         BM    PSTF0080            NEGATIVE - FINISHED                          
         BZ    PSTF0080            ZERO     - FINISHED                          
         LA    R1,5                SET DIVISOR                                  
         DR    RE,R1                                                            
         LTR   RF,RF               TEST RESULT                                  
         BM    PSTF0080            NEGATIVE - FINISHED                          
         BZ    PSTF0080            ZERO     - FINISHED                          
         LR    R2,RF               POSITIVE VALUE - SET LOOP CONTROL            
PSTF0060 EQU   *                                                                
         MVC   SCRATCH,0(R4)       STRIP OUT DAYPART                            
         GOTO1 AADDDATA,DMCB,AFABLK,WKPENDPT,SCRATCH,0                          
*                                  ADD DAYPART DATA ITEM                        
         MVC   WORK+20(4),1(R4)    STRIP OUT CPP VALUE                          
         GOTO1 AADDDATA,DMCB,AFABLK,WKPENCPP,WORK+20,0                          
*                                                                               
         LA    R4,5(R4)            BUMP TO NEXT SLOT                            
         BCT   R2,PSTF0060         GO BACK FOR NEXT                             
         DROP  R6                                                               
PSTF0080 EQU   *                                                                
         XIT1                                                                   
         LTORG                                                                  
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'029RESTX11   05/03/11'                                      
         END                                                                    
