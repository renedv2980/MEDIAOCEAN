*          DATA SET RERMP31    AT LEVEL 053 AS OF 04/14/09                      
*PHASE T81031C                                                                  
*INCLUDE INVDAY                                                                 
*INCLUDE REBKLST                                                                
         TITLE 'T81031 - REPPAK FILE MAINT - INV /ADD/CHA/DIS/DEL/ '            
********************************************************************            
* HISTORY OF CHANGES                                               *            
*  JAN03/07 (BU ) --- MAKE SCREEN CHANGES                          *            
*  SEP25/08 (KUI) --- ADD PROTECTION TO PREVENT REP RECORD DELETION*            
*   MAR/09  (SMY) --- NEW INVENTORY KEY                            *            
*      (SOME "COMMENTED OUT" CODE NOT CHANGED - SEE *SMYE*)        *            
********************************************************************            
*                    ***  END TOMBSTONE  ***                       *            
********************************************************************            
T81031   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,T81031,RR=R5                                                   
         LA    R9,2048(RB)                                                      
         LA    R9,2048(R9)                                                      
         USING T81031+4096,R9                                                   
*                                                                               
         L     RC,0(R1)                                                         
         USING GEND,RC                                                          
         L     RA,ATWA                                                          
...
*                                                                               
         XC    KEY,KEY                                                          
         MVC   KEY(RINVKBK-RINVKEY),0(R6)                                       
         LA    R6,KEY                                                           
         XC    RINVKRSR,RINVKRSR                                                
...
*                                                                               
         ...
         GOTO1 HIGH                                                             
         NI    DMINBTS,X'F7'                                                    
         CLC   KEY(RINVKSPR-RINVKEY),KEYSAVE   CHECK THRU START DATE            
         BNE   ERREND                                                           
*                                                                               
RST100   DS    0H                                                               
         CLC   KEY(RINVKSPR-RINVKEY),KEYSAVE   CHECK THRU START DATE            
...
VR80     MVC   0(1,R5),0(R1)       MOVE DAYPART TO RECORD                       
         ...
         SPACE 1                                                                
*  EFFECTIVE DATES                                                              
         MVC   RINVKSTD(L'RINVKSTD),DTEHLD                                      
...
VR140    L     RE,AIO1                                                          
         MVC   RINVKSTD(L'RINVKSTD),RINVKSTD-RINVKEY(RE)                        
...
*                                                                               
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
*  *SMYE* 03/27/09 - BELOW NOT CHANGED FOR NEW INV KEY                          
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
*                                                                               
...
*                                                                               
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
*  *SMYE* 03/27/09 - ABOVE NOT CHANGED FOR NEW INV KEY                          
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
*                                                                               
...
*                                                                               
         ...
         SPACE 1                                                                
CHAOLDN  DS    0H                                                               
         CLC   KEYSAVE(RINVKSPR-RINVKEY),KEY     OLD BOOK OR TEXT ?             
...
*                                                                               
         ...
         BZ    LR225                                                            
         CLC   RINVKINV,TEMPKEY+RINVKINV-RINVKEY SAME INVENTORY NUMBER?         
         BNE   *+14                                                             
         MVC   SAVEKEY,KEY                                                      
         B     LR200                                                            
*                                                                               
...
*                                                                               
         GOTO1 GETINV                                                           
*                                                                               
         CLC   KEY(RINVKSPR-RINVKEY),SAVEKEY   MATCH UP TO EFF DATE?            
         BNE   OV2ERRND                                                         
         OC    RINVKRTP,RINVKRTP   IS THIS A HEADER?                            
         BNZ   OV2ERRND                                                         
*                                                                               
         MVC   AIO,AIO1                                                         
...
*                                                                               
         ...
         BE    OV2ERRND                                                         
         CLC   =C'C+',8(R2)        MARK ALL FROM THIS POINT ON?                 
         BE    OV2ERRND                                                         
         CLC   =C'D+',8(R2)        MARK ALL FROM THIS POINT ON?                 
         BE    OV2ERRND                                                         
         BNE   SELR25                                                           
*                                                                               
...
SELR27   DS    0H                                                               
         ...
         BE    OV2ERRND            NO                                           
         CLI   0(R5),C'C'          RECORD SELECTED BY 'C'                       
         BE    OV2ERRND            NO                                           
         CLI   0(R5),C'D'          RECORD SELECTED BY 'D'                       
         BE    OV2ERRND            NO                                           
*                                                                               
...
BYTE4    DS    CL1                                                              
*****************************************************                           
RELOKEY  DS    XL27                RELOAD KEY                                   
VALIDKEY DS    XL27                VALID KEY                                    
*****************************************************                           
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'053RERMP31   04/14/09'                                      
         END                                                                    
**PAN#1  CSECT                                                                  
         END                                                                    
