*          DATA SET DELMTPWI   AT LEVEL 033 AS OF 09/16/20                      
*PROCESS USING(WARN(15))                                                        
*PHASE DELMTWIA                                                                 
*INCLUDE LMPAIR                                                                 
         TITLE 'LOCAL MONTHLIES CONVERSION'                                     
*****************************************************************               
* AUG/2011: ADDED INTAB SUPPORT                                                 
*****************************************************************               
...
* SKIP IMPACT DATA                                                              
         CLI   W_KRSRVC,RSRVC_NSIX                                              
         BE    *+8                                                              
* SKIP PREVIEW DATA                                                             
         CLI   W_KRSRVC,RSRVC_NSIP                                              
         BNE   OPENWKLY                                                         
         OI    FLAGS1,EMPTY_OTAPE_OKAY  ITS OK TO PRODUCE EMPTY DATASET         
         B     OPENSKIP                                                         
*                                                                               
* ONLY PROCESS ALL WEEK AVERAGE RECORDS AND                                     
* THE TRUE INDIVIDUAL WEEK RECORDS                                              
*                                                                               
*   THE MONTHLY 4WK AVERAGE OR ALL WEEK AVERAGE LINE ?                          
OPENWKLY CLI   W_KFILETYPE,W_KFILETYPE_MONTHLY                                  
         BNE   OPENOK02                                                         
         CLI   W_JOIN_INDICATOR,W_ALL_WEEK_INDICATOR EQU                        
         BE    OPENOK02                                                         
OPENSKIP MVI   BYPREAD,0                                                        
         B     OPENOK                                                           
OPENOK02 DS    0C                                                               
*                                                                               
OPENOK10 CLC   W_KMKTC,=C'124'    ATLANTA HARDCODE EXCEPTION                    
         BNE   *+10                                                             
         MVC   W_KMKTC,=C'168'                                                  
*                                                                               
...
*                                                                               
         ...
         LA    RE,STALIST                                                       
         LHI   RF,STALSTLQ*L'STALIST  (#ENTRIES*L'ENTRY)                        
         XCEF  ,                                                                
...
*                                                                               
***********************************************************************         
LMHPT    CLI   W_KSAMTY,STYP_WIRED GIVE WIRED HUT A CALL LETTER                 
         BE    LMHP10                                                           
         XC    DUB,DUB                                                          
         PACK  DUB,W_KMKTC                                                      
         CVB   RE,DUB                                                           
...
***********************************************************************         
LMBOOK   DS    0H                                                               
         MVC   DUB(4),W_KRPYR      YYYY                                         
         MVC   DUB+4(2),W_KRPRD    MM                                           
         MVC   DUB+6(2),=C'01'     ANY DAY WILL SUFFICE                         
         GOTO1 VDATCON,DMCB,(9,DUB),(3,FULL)  (2027 LIMIT!)                     
         MVC   INTBOOK,FULL        BINARY Y/M                                   
*                                                                               
         CLI   FORCE,C'Y'                                                       
         BNE   *+10                                                             
...
*                                                                               
*****************************************************************               
* ONLY PROCESS ALL WEEK AVERAGE RECORDS IF BOOK IS FEB18 OR LATER               
         CLC   INTBOOK,=AL2(JAN_18)                                             
****     CLC   INTBOOK,=AL2(MAR_16)  TESTING PURPOSE                            
         BNL   LMBOOK10                                                         
         OI    FLAGS1,EMPTY_OTAPE_OKAY  ITS OK TO PRODUCE EMPTY DATASET         
         MVI   BYPREAD,0                                                        
         B     OPENOK                                                           
*****************************************************************               
LMBOOK10 CLI   W_RECTYPE,RECTYPE_UER GIVE UNIVERSE RECS A CALL LETTER           
         BNE   LMSTAT                                                           
         SR    RE,RE                                                            
         ICM   RE,3,INTMRKT                                                     
         CVD   RE,DUB                                                           
...
*                                                                               
* EFFECTIVE JUL18 SURVEY, DO NOT FORCE LIVE+1 TO STANDARD                       
...
****     CLC   INTBOOK,=AL2(MAY_16) TESTING PURPOSES                            
         BNL   LMBTYP10                                                         
*                                                                               
...
*                                                                               
*  -----------------------------------------------------------                  
LMBT10   CLI   W_KSAMTY,STYP_WIRED TOTAL DMA OR WIRED                           
         BNE   *+8                                                              
         MVI   LMBTKEY+3,C'W'                                                   
*                                                                               
*  THE FIELD TO SET THE HYBRID (EXTENDED) DATA TYPE                             
*                                                                               
         CLI   W_KRSRVC,C'7'       HYBRID (EXTENDED)                            
         BNE   LMBT16X                                                          
         MVI   LMBTKEY+1,C'E'                                                   
         CLC   W_KSUBSI,=C'HISP '                                               
         BNE   LMBT16X                                                          
...
LMMM10   CLC   0(L'INTSTA,R9),INTSTA DMA HOUSEHOLDS                             
         ...
         BE    LMMM30                                                           
LMMM20   LA    R9,L'STALIST(R9)     BUMP TO NEXT TABLE ENTRY                    
         CLI   0(R9),XFF                                                        
         BNE   LMMM01                                                           
         DC    H'0'                 STATION LIST BUFFER FILLED UP               
*                                   MUST INCREASE STALSTLQ                      
*                                                                               
...
*                                                                               
         ...
         B     LMDTX                                                            
*                                                                               
LMDT20   CLI   W_KSAMTY,STYP_WIRED                                              
         BE    LMDTX                                                            
         CLI   BYPREAD,0                                                        
         BNE   LMDTX                                                            
         MVI   AVGFLAG,0                                                        
...
UNIQBTY  DS    0XL10                                                            
         ...
         DC    X'FF'                                                            
*                                                                               
STALSTLQ EQU   350                 MAXIMUM # OF STATIONS IN STALIST             
STALIST  DS    (STALSTLQ)XL7                                                    
         DC    X'FF'               EOT MARKER                                   
*                                                                               
DIVIDEND DS    D                                                                
DIVISOR  DS    F                                                                
...
***      DC    C'P',C' ',C' ',C' ',C'P'                                         
         DC    C'O',C' ',C' ',C'W',AL1(BOOKTYPE_Z)                              
         DC    C'S',C' ',C' ',C'W',AL1(BOOKTYPE_WS)                             
         DC    C'3',C' ',C' ',C'W',AL1(BOOKTYPE_W3)                             
         DC    C'7',C' ',C' ',C'W',AL1(BOOKTYPE_W)                              
         DC    C'1',C' ',C' ',C'W',AL1(BOOKTYPE_W1)                             
***      DC    C'P',C' ',C' ',C'W',C'Y'                                         
*  CABLE                                                                        
         DC    C'O',C' ',C'C',C' ',AL1(BOOKTYPE_U)                              
         DC    C'S',C' ',C'C',C' ',AL1(BOOKTYPE_LS)                             
         DC    C'3',C' ',C'C',C' ',AL1(BOOKTYPE_C3)                             
         DC    C'7',C' ',C'C',C' ',AL1(BOOKTYPE_C)                              
         DC    C'1',C' ',C'C',C' ',AL1(BOOKTYPE_C1)                             
***      DC    C'P',C' ',C'C',C' ',C'C'                                         
         DC    C'O',C' ',C'C',C'W',AL1(BOOKTYPE_Z)                              
         DC    C'S',C' ',C'C',C'W',AL1(BOOKTYPE_WS)                             
         DC    C'3',C' ',C'C',C'W',AL1(BOOKTYPE_W3)                             
         DC    C'7',C' ',C'C',C'W',AL1(BOOKTYPE_W)                              
         DC    C'1',C' ',C'C',C'W',AL1(BOOKTYPE_W1)                             
***      DC    C'P',C' ',C'C',C'W',C'Y'                                         
*  HISPANIC                                                                     
         DC    C'O',C'H',C' ',C' ',AL1(BOOKTYPE_J)                              
         DC    C'O',C'H',C'C',C' ',AL1(BOOKTYPE_J)                              
         ...
         DC    C'7',C'H',C'C',C' ',AL1(BOOKTYPE_H)                              
         DC    C'1',C'H',C' ',C' ',AL1(BOOKTYPE_H1)                             
         DC    C'1',C'H',C'C',C' ',AL1(BOOKTYPE_H1)                             
***      DC    C'P',C'H',C' ',C' ',C'I'                                         
***      DC    C'P',C'H',C'C',C' ',C'I'                                         
*  BLACK                                                                        
         DC    C'7',C'B',C' ',C' ',AL1(BOOKTYPE_B)                              
         DC    C'7',C'B',C'C',C' ',AL1(BOOKTYPE_B)                              
*  PARENT ONLY                                                                  
         DC    C'O',C'A',C' ',C' ',AL1(BOOKTYPE_Q)                              
         DC    C'S',C'A',C' ',C' ',AL1(BOOKTYPE_QS)                             
         DC    C'3',C'A',C' ',C' ',AL1(BOOKTYPE_Q3)                             
         DC    C'7',C'A',C' ',C' ',AL1(BOOKTYPE_A)                              
...
***      DC    C'O',C'A',C' ',C' ',C'S'                                         
         ...
         DC    C'7',C'A',C' ',C'W',AL1(BOOKTYPE_R)                              
*  EXCLUSIONS                                                                   
*        DC    C'7',C'X',C' ',C' ',C'X'                                         
...
*  OLYMPIC EXCLUSIONS                                                           
         ...
         DC    C'7',C'O',C'C',C' ',AL1(BOOKTYPE_O)                              
         DC    C'1',C'O',C' ',C' ',AL1(BOOKTYPE_O1)                             
         DC    C'1',C'O',C'C',C' ',AL1(BOOKTYPE_O1)                             
*  TRADE                                                                        
...
*  HYBRID  (EXTENDED)                                                           
         ...
         DC    C'O',C'E',C' ',C'W',AL1(BOOKTYPE_YZ)                             
         DC    C'S',C'E',C' ',C'W',AL1(BOOKTYPE_YA)                             
         DC    C'3',C'E',C' ',C'W',AL1(BOOKTYPE_YB)                             
         DC    C'7',C'E',C' ',C'W',AL1(BOOKTYPE_YW)                             
         DC    C'1',C'E',C' ',C'W',AL1(BOOKTYPE_YM)                             
*  HYBRID CABLE   (EXTENDED)                                                    
         DC    C'O',C'E',C'C',C'W',AL1(BOOKTYPE_YZ)                             
         DC    C'S',C'E',C'C',C'W',AL1(BOOKTYPE_YA)                             
         DC    C'3',C'E',C'C',C'W',AL1(BOOKTYPE_YB)                             
         DC    C'7',C'E',C'C',C'W',AL1(BOOKTYPE_YW)                             
         DC    C'1',C'E',C'C',C'W',AL1(BOOKTYPE_YM)                             
         DC    C'O',C'E',C'C',C' ',AL1(BOOKTYPE_YU)                             
         DC    C'S',C'E',C'C',C' ',AL1(BOOKTYPE_YS)                             
         DC    C'3',C'E',C'C',C' ',AL1(BOOKTYPE_YD)                             
         DC    C'7',C'E',C'C',C' ',AL1(BOOKTYPE_YC)                             
         DC    C'1',C'E',C'C',C' ',AL1(BOOKTYPE_YK)                             
*  HYBRID HISPANIC                                                              
         ...
         DC    C'O',C'D',C'C',C' ',AL1(BOOKTYPE_YJ)                             
         DC    C'S',C'D',C'C',C' ',AL1(BOOKTYPE_YE)                             
         DC    C'3',C'D',C'C',C' ',AL1(BOOKTYPE_YF)                             
         DC    C'7',C'D',C'C',C' ',AL1(BOOKTYPE_YH)                             
         DC    C'1',C'D',C'C',C' ',AL1(BOOKTYPE_YN)                             
         DC    X'FF'                                                            
***********************************************************************         
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'033DELMTPWI  09/16/20'                                      
         END                                                                    
