*          DATA SET RERIS0A    AT LEVEL 025 AS OF 12/15/10                      
...
*                                                                     *         
* MAR18/10 (SKU) --- OMG! 12 YEARS LATER...SKIP FB CONTRACTS          *         
*                    WITH CONTRACT TYPE Y                             *         
*                                                                     *         
...
PNAMRFLT NTR1                                                                   
         L     R4,AIOAREA                                                       
         USING RCONREC,R4                                                       
         CLI   RIPGRP,X'40'         GROUP                                       
         BNH   *+14                                                             
         CLC   RIPGRP,RCONKGRP                                                  
         BNE   PNMX                                                             
         CLI   RIPAGY,X'40'         AGENCY                                      
         BNH   *+14                                                             
         CLC   RIPAGY(4),RCONKAGY                                               
         BNE   PNMX                                                             
         CLI   RIPAGOFF,X'40'         AGENCY/OFF                                
         BNH   *+14                                                             
...
*                                                                               
TST9C20  EQU   *                                                                
         L     RF,ASTASTOR         SET A(STATION STORAGE AREA)                  
         OC    0(32,RF),0(RF)      ANY MARKET STATIONS?                         
         BZ    TST9C28             NO                                           
*                                  YES - CHECK ORDER AGAINST STATIONS           
TST9C22  EQU   *                                                                
         OC    0(5,RF),0(RF)       END OF MARKET STATIONS?                      
         BZ    TST9C24             YES - NOT IN MARKET: SKIP IT                 
         CLC   0(5,RF),RCONQSTA    STATION IN TABLE?                            
         BE    TST9C30             YES - CHECK NEXT FILTERS                     
         LA    RF,5(RF)            BUMP TO NEXT SLOT                            
         B     TST9C22             GO BACK AND CHECK IT                         
TST9C24  EQU   *                                                                
         XC    KEY+13(14),KEY+13   CLEAR LOW-ORDER KEY                          
         ZIC   RF,KEY+12           INCREASE 'MEDIA' FOR SKIP-READ               
         LA    RF,1(RF)                                                         
         STC   RF,KEY+12                                                        
         B     RDKEY9C             GO BACK AND READ HIGH                        
TST9C28  EQU   *                                                                
         CLC   TBLSTA,ALL          TEST STATION                                 
         BE    TST9C30                                                          
         CLC   TBLSTA,RCONQSTA                                                  
...
FILTERS  NTR1                                                                   
*                                                                               
* SPECIAL FOR FOX STATION SALES (FB), SKIP CONTRACTS IF CONTRACT TYPE           
* IS C'Y'                                                                       
*                                                                               
         L     R4,AIOAREA                                                       
         USING RCONREC,R4                                                       
...
FIL65    EQU   *                                                                
         ...
         BZ    FIL70               NO                                           
*                                                                               
FIL65C   DS    0H                  SCRIPT CONTRACT UPLOAD ELEMENT ?             
         L     R4,AIOAREA                                                       
         LA    R4,RCONELEM-RCONREC(R4)                                          
FIL65F   DS    0H                  SCRIPT CONTRACT UPLOAD ELEMENT ?             
         ZIC   R0,1(R4)                                                         
         AR    R4,R0                                                            
         CLI   0(R4),0                                                          
         BE    FIL65X              NO SCRIPT CON UPLOAD ELEM FOUND              
         CLI   0(R4),X'3C'                                                      
         BNE   FIL65F                                                           
*                                                                               
         TM    VIEWS3,X'08'        RCM CONTRACTS ONLY ?                         
         BZ    FIL70               NO                                           
         CLI   1(R4),X'27'         DOES ELEMENT HAVE "MANUAL" DATA ?            
         BNH   FIL65X              NO - NOT LONG ENOUGH - SKIP                  
         USING RCONCUEL,R4                                                      
         OC    RCONCUMD,RCONCUMD   "MANUAL ENTRY" DATE ?                        
         BNZ   FIL70               YES - ENTERED MANUALLY - KEEP                
         DROP  R4                                                               
*                                                                               
FIL65X   DS    0H                                                               
         B     FLTNO               SKIP                                         
...
MAXOK    BR    R4                                                               
       ...
       ++INCLUDE FAFACTS                                                        
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'025RERIS0A   12/15/10'                                      
         END                                                                    
