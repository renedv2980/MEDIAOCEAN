*          DATA SET RESFM0E    AT LEVEL 064 AS OF 04/09/09                      
*PHASE T8180EC                                                                  
*INCLUDE NUMVAL                                                                 
...
*                                                                   *           
* 07APR09  (SMY) --- RELINK FOR NEW INVENTORY KEY                   *           
*                                                                   *           
*********************************************************************           
*                                                                               
T8180E   CSECT                                                                  
...
*SMY*    MVI   RIDPKTYP,X'92'                                                   
         ...
         SPACE 2                                                                
* COMPLETE THE KEY AND DETERMINE THE KEY COMPARE LENGTH                         
*                                                                               
         LA    R2,RIDPKDPT-RIDPKEY                                              
         MVC   RIDPKDPT,FILTDPT                                                 
         CLI   FILTDAY,0           STOP AFTER DAYPART IF NO DAY FILTER          
         BE    MA110               WAS GIVEN                                    
         TM    FILTCNTL,NODAYS                                                  
         BO    MA110               IGNORE DAY FILTER FOR DPT S,J & V            
         SPACE                                                                  
         LA    R2,RIDPKDAY-RIDPKEY MOVE IN DAY FILTER FOR REST OF DPTS          
         MVC   RIDPKDAY,FILTDAY                                                 
         SPACE 2                                                                
...
*-------------------------------------------------------------------*           
*                                                                               
MA150    OC    TIMRANGE,TIMRANGE   IF NO TIME FILTER ITS A HIT                  
         BZ    MA180                                                            
         MVC   PROGTIM,RINVPTIM                                                 
         OC    PROGEND,PROGEND                                                  
         BNZ   *+10                                                             
         MVC   PROGEND,PROGST                                                   
         CLC   TIMST,TIMEND                                                     
...
MA160    CLC   PROGST,PROGEND                                                   
         ...
         BL    MA120               FILTER, THEN IT MUST END BEFORE              
         B     MA180               FILTER END WHICH IS IN NEXT AM.              
         CLC   PROGEND,TIMEND                                                   
         BH    MA120               REJECT                                       
         B     MA180               OK.                                          
         SPACE                                                                  
...
*                                                                               
         DROP  R6                                                               
MA180    L     R6,AIO              RESET R6 TO RECORD START                     
         USING RINVREC,R6                                                       
         SPACE 1                                                                
...
MA210    ZIC   R1,INVCOUNT         OTHERWISE, ADD RECORD TO LIST                
         ...
         STC   RF,INVCOUNT                                                      
         MH    R1,=H'11'           DISPLACEMENT INTO LIST                       
         LA    RE,LIST(R1)         POINT TO NEXT POSITION IN LIST               
         MVC   0(3,RE),RINVKINV    SAVE INVENTORY NUMBER                        
         MVC   3(3,RE),RINVKSTD    INVENTORY EFFECTIVE START DATE               
* 23JAN90      *** START ***                                                    
         MVC   6(3,RE),FILTBK                                                   
         MVC   9(1,RE),FILTBKF                                                  
         MVC   10(1,RE),FILTTX                                                  
...
*                                                                               
         ...
         SPACE                                                                  
AD10     ZIC   R1,RAVLKDET         BUMP DETAIL NUMBER                           
         LA    R1,1(R1)                                                         
         STC   R1,RAVLKDET         AND MOVE IN THE NEXT INVENTORY               
         MVC   RAVLDINV,0(R2)      NUMBER IN LIST                               
         MVC   RAVLDATE,3(R2)      EFFECTIVE START DATE                         
         MVC   RAVLDBKS(3),6(R2)   BOOKS (FROM BK=)                             
         MVC   RAVLDBKF(1),9(R2)   C'+-' FILTER                                 
         CLI   TVSAT,C'T'                                                       
         BE    *+10                                                             
         MVC   RAVLDSAT,TVSAT      SATELLITE CODE                               
* 23JAN90      *** START ***                                                    
         CLI   10(R2),1            WAS TX=ALL ENTERED?                          
         BNE   AD15                                                             
...
AD15     DS    0H                                                               
         GOTO1 ADDREC                                                           
AD17     DS    0H                                                               
         LA    R2,L'LIST(R2)       POINT TO NEXT LIST ITEM                      
         BCT   R3,AD10                                                          
         SPACE 1                                                                
...
*AD30     DS    0H                                                              
         ...
         MVC   RPRPDINV,0(R2)      NUMBER IN LIST                               
         MVC   RPRPDATE,3(R2)      EFFECTIVE START DATE                         
...
* 23JAN90      *** START ***                                                    
         OC    6(3,R2),6(R2)       WAS A BOOK OVERRIDE ENTERED?                 
         BZ    AD35                                                             
         LA    R2,6(R2)                                                         
         XI    0(R2),X'80'         FLIP CPP/CPM BITS                            
         GOTO1 VUNBOOK,DMCB,(1,(R2)),DUMMYH,0,0                                 
         SH    R2,=H'6'                                                         
...
WAREAEND EQU   *                                                                
         SPACE 3                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'064RESFM0E   04/09/09'                                      
         END                                                                    
