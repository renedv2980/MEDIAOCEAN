*          DATA SET NEMED88N   AT LEVEL 195 AS OF 04/20/07                      
*PHASE T31E88A,*                                                                
*INCLUDE WGTLIST                                                                
...
*                                                                               
         ...
         MVI   ERROR,NBINVPRD                                                   
         LA    R2,SPLPRDH                                                       
         CLI   11(R2),C'*'             WE DON'T WANT AAA*BBB                    
         BE    TRAPERR                                                          
         MVI   FTERMFLG,0          SET REQUIRED FLAG                            
...
*****    MVC   REQPROD,NBSELPIG                                                 
         MVC   REQPRODA(3),NBEFFPN3         ALPHA PROD                          
         OC    NBSELPG3,NBSELPG3            ALPHA PRODS ?                       
         BZ    *+10                                                             
         MVC   REQPRODA,NBSELPG3           BOTH ALPHA PRODS                     
*                                                                               
         LA    R2,SPLLENH                                                       
         BAS   RE,VALLEN                                                        
...
**         BNE   TRAPERR                                                        
VK220    CLI   NBSELPG3,0          ONLY SINGLE PROD REQUEST                     
         BNE   TRAPERR                                                          
...
*-REMOVE THE PRODUCT FILTER TO NETIO.                                           
         ...
         XC    NBSELPG3,NBSELPG3                                                
         XC    NBSELPNM,NBSELPNM                                                
         XC    NBSELPN3,NBSELPN3                                                
         MVC   PIGPROD,NBEFFPNM                                                 
         MVC   PIGPRODA,NBEFFPN3          ALPHA PRODUCT                         
         MVC   SPLPPRD(3),=C'POL'                                               
         LA    R2,SPLPPRDH                                                      
         MVI   5(R2),3                                                          
         MVI   FTERMFLG,0          SET REQUIRED FLAG                            
...
RELOC20  CLI   5(R2),0             PRODUCT                                      
         ...
         MVI   NBSELLEN,0                                                       
         MVI   NBRETURN,X'03'      REVALIDATING PRODUCT                         
         CLI   11(R2),C'*'         AAA*BBB = ERROR                              
         BE    TRAPERR                                                          
         CLC   =C'ALL',8(R2)       ALL NOT ALLOWED/POL=UNALLOCATE               
         BE    TRAPERR                                                          
         NETGO NVPRDALL,DMCB,0,0                                                
         MVI   ERROR,NBINVPRD                                                   
         CLI   NBSELLEN,0                                                       
...
******   BZ    TRAPERR                                                          
         OC    NBSELPG3,NBSELPG3    MUST BE PIGGYBACK PRODS                     
         BZ    TRAPERR                                                          
*                                                                               
...
***      BE    RELL60              NO                                           
         CLI   REQPRODA+3,0        CHECK FOR PIGGYBACK                          
         BE    RELL100             NO                                           
         CLI   NBSELPG3,0          CHECK IF REALOCATE PROD A PIGGYBACK          
         BE    RELL60              NO                                           
...
**         BE    RELL160         NO                                             
*                                                                               
RELL100  CLI   NBSELPG3,0          CHECK IF REALOCATE PROD A PIGGYBACK          
         BE    RELL160             NO                                           
...
SORTCARD DC    CL80'SORT FIELDS=(6,5,A),FORMAT=BI,WORK=1'                       
...
*                                                                               
         ...
         BNE   LR235                                                            
         SPACE                                                                  
         TM    NBUNST3,X'40'       IS IT A COPY SPLIT ?                         
         BO    LR235               SKIP IT                                      
         CLI   NBPRDNO,0           IF MULTI PROD                                
         BNE   LR235               SKIP IT                                      
         SPACE                                                                  
...
**       BNE   LR235                                                            
         CLI   NBPR1CL3,0                                                       
         BNE   LR235                                                            
LR206    CLC   NBSELPRD,=CL3'POL'                                               
         BE    LR210                                                            
***      CLI   NBPRD2,0            CHECK PIGGY PROD                             
***      BE    LR210                                                            
***      CLC   NBSELPIG,NBPRD      MUST BE A MATCH                              
***      BNE   LR235                                                            
         CLI   NBPR2CL3,0           CHECK PIGGY PROD                            
         BE    LR210                                                            
         CLC   NBSELPG3,NBPR1CL3   MUST BE A MATCH                              
         BNE   LR235                                                            
*                                                                               
...
***      CLC   PIGPROD,NBPRD2                                                   
***      BNE   LR235                                                            
         CLC   PIGPRODA,NBPR1CL3                                                
         BE    LR220                                                            
         CLC   PIGPRODA,NBPR2CL3                                                
         BNE   LR235                                                            
*                                                                               
...
PUTSORT  NTR1                                                                   
         XC    RECWORK(18),RECWORK                                              
         MVC   SORTSTA,NBACTNET                                                 
         MVC   SORTPRG,NBACTPRG                                                 
         MVC   SORTPRD,0(R3)                                                    
         MVC   SORTPRDA,0(R3)       HUH ???     ALPHA PROD                      
...
ESPR050  DS    0H                 PASSING ALPHA PROD IN WORK                    
         GOTO1 =V(SPESTCHK),HKDMCB,NBAIO,ESTTAB,(RC)                            
         CLI   8(R1),0                                                          
         BNE   ESPR100             ESTIMATE NO OPEN ERROR                       
***      CLI   R4,1(R4)            CHECK PIGGYBACK                              
***      CLI   0(R4),0                                                          
***      BE    *+8                                                              
         CLI   8(R4),0             PIGGYBACK?                                   
         BE    *+14                                                             
         MVC   WORK(3),8(R4)                                                    
         BCT   R0,ESPR050                                                       
         B     EXIT                                                             
...
*                                                                               
         ...
         BAS   RE,NEXTEL                                                        
         BNE   *+12                                                             
         ST    R5,MYFULL           SAVE ADDR OF TRAFFIC ELEM                    
         B     CHGU050                                                          
*                                                                               
         XC    ELEMENT,ELEMENT                                                  
         MVI   ELEMENT,X'21'                                                    
...
* X'19' ELEMENT                                                                 
         L     R2,NBAIO                                                         
         LA    R5,NUMAINEL                                                      
         MVI   SRCHEL,X'19'         X'19' EXISTS                                
         BAS   RE,NEXTEL                                                        
         BNE   CHGU055                                                          
         USING NUPDEEL,R5                                                       
         MVC   NUPDEPR,5(R4)       EMTER NEW PROD                               
         CLC   =C'POL',NUPDEPR     ARE WE UNALLOCATING ???                      
         BE    CHGU054             YES                                          
         B     CHGU060             NO                                           
*                                  YES                                          
CHGU054  GOTO1 HELLO,DMCB,(C'D',UNTFILE),(X'19',(R2)),0,0                       
         B     CHGU117                                                          
         DROP  R5                                                               
*                                                                               
CHGU055  DS    0H                  X'19' DOES NOT EXIST                         
         XC    ELEM,ELEM              CREATE X'19' ELEMENT                      
         LA    R1,ELEM                                                          
         USING NUPDEEL,R1                                                       
         MVI   NUPDEEL,X'19'                                                    
         MVI   NUPDELEN,10                                                      
...
***      BNE   CHGU070                                                          
         ...
         BNE   CHGU070                                                          
         MVC   NUPRD2,0(R4)                                                     
         L     R5,MYFULL        GET COMMERCIAL ELEM                             
         USING NUCMLEL,R5                                                       
         OI    NUCMLFLG,X'40'   SET IT                                          
         DROP  R5                                                               
*                                                                               
         L     R2,NBAIO                                                         
         LA    R5,NUMAINEL                                                      
         MVI   SRCHEL,X'19'       X'19' EXISTS                                  
         BAS   RE,NEXTEL                                                        
***      BNE   CHGU065           DON'T KNOW WHY IT WOULD                        
***      BNE   CHGU065           NOT  BE HERE -                                 
         BE    *+6                                                              
         DC    H'0'                                                             
         USING NUPDEEL,R5                                                       
         LA    R5,NUPDEPR          BUMP TO 1ST PROD                             
         LA    R5,7(R5)            BUMP TO 2ND                                  
         MVC   0(3,R5),5(R4)       SET ALPHA                                    
***      OC    NUP1SHR,NUP1SHR                                                  
***      BZ    CHGU062                                                          
***      L     RE,=F'10000'        GET $ PERCENT SHARE                          
***      MVC   HALF,NUP1SHR                                                     
***      SR    RF,RF                                                            
***      LH    RF,HALF                                                          
***      SR    RE,RF                                                            
***      STCM  RE,3,3(R5)          SET $ PCT SHARE                              
*                                                                               
CHGU062  OC    NUFEED,NUFEED         NOW DO FEED %                              
***      BZ    CHGU070                                                          
***      MVC   HALF,NUFEED         NOW DO FEED %                                
***      SR    RF,RF                                                            
***      LH    RF,HALF                                                          
***      L     RE,=F'10000'        GET $ PERCENT SHARE                          
***      SR    RE,RF                                                            
***      STCM  RE,3,5(R5)          SET FEED PCT                                 
         B     CHGU070                                                          
         DROP  R5                                                               
*                                                                               
***********************************************************                     
*  THIS LABEL NOT CALLED AT THE MOMENT -                                        
CHGU065  DS    0H                  CREATE X'19' ELEMENT                         
         L     R2,NBAIO                                                         
         XC    ELEM,ELEM                                                        
         LA    R1,ELEM                                                          
         USING NUPDEEL,R1                                                       
         MVI   NUPDEEL,X'19'                                                    
         MVI   NUPDELEN,17                                                      
         MVC   NUPDEPR,5(R4)       SET 1ST PRODUCT                              
***      MVC   NUPDEPCT,NUP1SHR                                                 
***      MVC   NUPDEFD,NUFEED                                                   
         LA    R1,10(R1)           BUMP TO START OF 2ND PROD                    
         USING NUPDEPR,R1                                                       
         MVC   NUPDEPR,8(R4)       PROD CODE                                    
*                                                                               
***      L     RE,=F'10000'        GET $ PERCENT SHARE                          
***      OC    NUP1SHR,NUP1SHR                                                  
***      BZ    CHGU067                                                          
...
***      LH    RF,HALF                                                          
***      SR    RE,RF                                                            
***      STCM  RE,3,NUPDEPCT       SET $ PCT SHARE                              
*                                                                               
CHGU067  MVC   HALF,NUFEED         NOW DO FEED %                                
***      LH    RF,HALF                                                          
***      LTR   RF,RF                                                            
***      BZ    CHGU068                                                          
***      L     RE,=F'10000'        GET $ PERCENT SHARE                          
***      SR    RE,RF                                                            
***      STCM  RE,3,NUPDEFD        SET FEED PCT                                 
         DROP  R1                                                               
CHGU068  L     R5,NBAIO                                                         
         GOTO1 HELLO,DMCB,(C'P',UNTFILE),(0,(R5)),ELEMENT,0                     
*                                                                               
...
*                                   CREAT/CHANGE X'19'                          
         ...
         USING NURECD,R2                                                        
*                                       DELETE AND RECREATE                     
         GOTO1 HELLO,DMCB,(C'D',UNTFILE),(X'19',(R2)),0,0                       
         CLC   =C'POL',5(R4)       UNALLOCATE?                                  
         BE    CHGU120             YES                                          
         B     CHGU110                                                          
*                                                                               
CHGU110  DS    0H                  CREATE X'19'                                 
         L     R2,NBAIO                                                         
         XC    ELEM,ELEM                                                        
         LA    R5,ELEM                                                          
         USING NUPDEEL,R5                                                       
...
****     MVC   NUPDEPCT,NUP1SHR                                                 
****     MVC   NUPDEFD,NUFEED                                                   
         CLI   8(R4),X'40'         IS THERE 2ND PROD?                           
         BH    CHGU112                                                          
         MVI   NUPDELEN,10                                                      
         B     CHGU117                                                          
*                                                                               
CHGU112  LA    R5,10(R5)           BUMP TO START OF 2ND PROD                    
         USING NUPDEPR,R5                                                       
         MVC   NUPDEPR,8(R4)       2ND PROD CODE                                
*                                                                               
***      L     RE,=F'10000'            GET $ PERCENT SHARE                      
***      OC    NUP1SHR,NUP1SHR                                                  
***      BZ    CHGU113                                                          
...
***      STCM  RE,3,NUPDEPCT       SET $ PCT SHARE                              
*                                                                               
CHGU113  MVC   HALF,NUFEED         NOW DO FEED %                                
***      LH    RF,HALF                                                          
***      LTR   RF,RF                                                            
***      BZ    CHGU117                                                          
***      L     RE,=F'10000'            GET $ PERCENT SHARE                      
...
**ESTTAB   DS    1024C                                                          
*********************************************************                       
ESTTAB   DS    0CL864            (4+32) X 24 PRODS                              
ESTTPRD  DS    CL4                3 CHAR ALPHA PROD                             
ESTTNUMS DS    CL32               FOR 32 EST NUMBERS                            
ESTTABLN EQU   *-ESTTPRD                                                        
         DS    CL828                                                            
...
REQPRODA DS    CL6                 REQUESTED ALPHA PRODUCTS                     
...
PIGPRODA DS    CL3                 REQUESTED ALPHA PRODUCT                      
...
*                                                               *               
* *** NOTE *** NOTE *** NOTE ***                                *               
* WITH EXPANDED PODUCTS NOW TO 500 AND ALPHA PRODS THIS         *               
* ROUTINE IS NOW SIGNIFICANTLY CHANGED                          *               
* ESTTABLE IS 4 BYTE FOR ALPHA PROD + 32 HALF WORDS FOR EST #   *               
* RETAINS LIMIT OF 32 OPEN ESTIMATES PER PROD                   *               
* ESTTABLE LENGTH = 2 OPEN ESTIMATES PER PROD                   *               
*****************************************************************               
         SPACE 2                                                                
***SPESTCHK CSECT                                                               
***         NMOD1 0,SPESTCHK                                                    
...
*                                                                               
         ...
         L     R3,0(R1)            GET A(REC)                                   
* TEST EST NUM IN TABLE                                                         
         L     R1,4(R2)            POINT TO PROD/EST TABLE                      
ESTCHK2  CLI   0(R1),0             TEST E-O-L                                   
         BNE   ESTCHK3                                                          
         MVC   0(3,R1),WORK                                                     
         B     ESTCHK4                                                          
*                                                                               
ESTCHK3  CLC   0(3,R1),WORK        TEST MATCH                                   
         BE    ESTCHK4                                                          
         LA    R1,ESTTABLN(R1)                                                  
         CLI   0(R1),X'FF'                                                      
         BNE   ESTCHK2                                                          
         DC    H'0'                TABLE FULL                                   
*                                                                               
ESTCHK4  LA    RE,32               MAX # OF ESTIMATES PER PROD                  
         LA    R1,4(R1)            BUMP OVER ALPHA PROD                         
ESTCHK6  CLI   0(R1),0             EMPTY SLOT                                   
         BNE   *+14                                                             
         MVC   0(1,R1),17(R3)      SET EST #                                    
         B     ESTCHK10            AND READ TO CHECK PROD/EST OPEN              
         CLC   0(1,R1),17(R3)      A MATCH ?                                    
         BE    ESTCHKX                                                          
         LA    R1,1(R1)            BUMP TO NEXT EST                             
         BCT   RE,ESTCHK6                                                       
         DC    H'0'                EST TABLE FOR PRODS FULL                     
...
* MUST READ FOR EST KEY                                                         
         ...
         MVC   KEY+1(3),1(R3)      A-M/CLT                                      
         MVC   KEY+7(1),17(R3)     EST                                          
         MVC   KEY+4(3),WORK       PRD CODE                                     
         MVI   8(R2),X'FF'         PRESET EST NOT FOUND                         
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,=CL8'DMRDHI',=CL8'SPTDIR  ',KEY,KEY,0               
         CLC   KEY(13),KEYSAVE                                                  
         BNE   *+8                                                              
         MVI   8(R2),0             CLEAR ERROR FLAG                             
* RESTORE KEY FOR SEQ READING                                                   
         MVC   KEY,NBKEY                                                        
* BELIEVE NEXT INSTRUCTION IS NOT NECESSARY                                     
****     GOTO1 DATAMGR,DMCB,=CL8'DMRDHI',=CL8'UNTDIR  ',KEY,KEY,0               
*                                                                               
ESTCHKX  XMOD1 1                                                                
*                                                                               
...
ESTD     DSECT                                                                  
       ...
       ++INCLUDE DDMASTC                                                        
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'195NEMED88N  04/20/07'                                      
         END                                                                    
