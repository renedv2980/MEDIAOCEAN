*          DATA SET CTCONPSW   AT LEVEL 001 AS OF 07/05/17                      
*PHASE CONPSWA                                                                  
*INCLUDE REGSAVE                                                                
*INCLUDE CARDS                                                                  
*INCLUDE DMDMGRL                                                                
...
*INCLUDE ADDAY                                                                  
*INCLUDE QSORT                                                                  
         TITLE 'CTCONPSW - EXPIRE PASSWORDS'                                    
         PRINT NOGEN                                                            
CTPSW    CSECT                                                                  
*                                                                               
         ENTRY UTL                                                              
         ENTRY SSB                                                              
*                                                                               
         NBASE WORKX-WORKD,**CPSW**,=V(REGSAVE),RA,R9                           
         USING WORKD,RC                                                         
         ST    RD,SAVERD                                                        
*                                                                               
...
WPA020   CLI   0(R3),0                                                          
         BE    WPA040                                                           
         CLI   0(R3),SAPWHELQ      X'E4' PASSWORD HISTORY ELEMENTS              
         BNE   *+6                                                              
         LR    R4,R3               R4=A(LAST PASSWORD HISTORY ELEMENT)          
         LLC   R0,1(R3)                                                         
         AR    R3,R0                                                            
         B     WPA020                                                           
*                                                                               
         USING SAPWHD,R4                                                        
WPA040   LTR   R4,R4               DID WE FIND A HISTORY ELEMENT?               
         BNZ   WPA042              YES                                          
         OI    CUFLAG,PTFNOPW                                                   
         B     WPA054                                                           
*                                                                               
WPA042   GOTO1 VDATCON,DMCB,(2,SAPWHDTE),(0,DATE0)                              
*                                                                               
         LLC   RF,ACD.CTAADPTO     PASSWORD EXPIRES IN DAYS                     
         STCM  RF,15,DMCB+8                                                     
         GOTO1 VADDAY,DMCB,(C'D',DATE0),EXPDATE                                 
*                                                                               
         GOTO1 VDATCON,DMCB,(0,EXPDATE),(2,CUDATE)                              
*                                                                               
         TM    SAPWHFLG,SAPWHEXP   ALREADY MARKED EXPIRED?                      
         BZ    WPA044              YES                                          
         OI    CUFLAG,PTFAEXP                                                   
         B     WPA054                                                           
*                                                                               
WPA044   CLC   EXPDATE,TODAY0      ALREADY EXPIRED?                             
         BNL   WPA046              YES                                          
         OI    CUFLAG,PTFDEXP                                                   
         B     WPA054                                                           
*                                                                               
WPA046   OI    SAPWHFLG,SAPWHEXP   TURN ON EXPIRED BIT                          
*                                                                               
         CLI   RCWRITE,C'Y'                                                     
         BNE   WPA050                                                           
*                                                                               
...
*                                                                               
         ...
         DR    R0,RF                                                            
         C     R1,=A(MAXPIDS)                                                   
         JH    *+2                 TOO MANY PIDS, INCREASE TABLE                
         DROP  R1                                                               
*                                                                               
WPAEX    B     EXITEQ                                                           
         B     EXITNE                                                           
...
*                                                                               
         TM    PTFLAG,PTFAEXP                                                   
         BZ    *+10                                                             
         MVC   PLMESS(20),=CL20'EXPIRED BY ADMIN'                               
         TM    PTFLAG,PTFDEXP                                                   
         BZ    *+10                                                             
         MVC   PLMESS(20),=CL20'EXPIRED BY DATE'                                
         TM    PTFLAG,PTFNOPW                                                   
         BZ    *+10                                                             
         MVC   PLMESS(20),=CL20'NO PASSWORD FOUND'                              
*                                                                               
         GOTO1 VDATCON,DMCB,(2,PTDATE),(21,PLMESS+20)                           
*                                                                               
         BAS   RE,PRINTL                                                        
         LA    R5,PIDTABLQ(R5)                                                  
         B     PR010                                                            
...
PTDATE   DS    XL2                                                              
PTFLAG   DS    X                                                                
PTFAEXP  EQU   X'80'               ADMINISTRATOR EXPIRED                        
PTFDEXP  EQU   X'40'               DATE EXPIRED                                 
PTFNOPW  EQU   X'20'               NO PASSWORD FOUND                            
...
***********************************************************************         
* INCLUDES, DSECTS, AND DEFINITIONS                                             
***********************************************************************         
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'001CTCONPSW  07/05/17'                                      
         END                                                                    
