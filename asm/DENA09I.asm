*          DATA SET DENA09I    AT LEVEL 143 AS OF 05/22/17                      
*PROCESS USING(WARN(15))                                                        
*PHASE DENA09IA                                                                 
*********************************************************************           
* NAD  -  MONTHLY TAPE WITH WKLY DATA SUPPORTED                                 
* OPHASE: DENA09OA                                                              
*                                                                               
* 10/31/95 - SEPERATE PROGRAM NUMBERS FOR NETWORK: NET/SYN/NHTI                 
*        CALLS TO VNTIPRG PROGRAM HANDLE ASSIGNMENTS                            
*                                                                               
* 10/14/94 - FIXED VARS, BREAKOUTS, AND SPECIALS. INTSTYP, PMSTYP,              
*        PNSTYP, PNBSIND TO IDENTIFY BRKS AND SPECIALS ('B','S').               
*                                                                               
*********************************************************************           
         TITLE 'DEMO CONVERSION - NAD MIT FMT'                                  
         EJECT                                                                  
DENA09I  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,DENA09I,RA,R3,R4                                               
...
RDR      DS    0H                                                               
         ...
         BNE   RDR10                                                            
         MVI   TAPETYP,C'N'        REGULAR NAD                                  
         MVC   TAPEMKT,=H'200'     NAD MARKET                                   
         MVC   TAPWKMKT,=H'201'     WEEKLY NAD MARKET                           
         B     RDR20                                                            
*                                                                               
RDR10    CLC   =C'NSS NAD REPORT',RDRRPT                                        
         BE    *+6                 WRONG RECORD (MAYBE SYND)                    
         DC    H'0'                                                             
         MVI   TAPETYP,C'S'        NSS                                          
         MVC   TAPEMKT,=H'400'     NSS MARKET                                   
         MVC   TAPWKMKT,=H'401'     WEEKLY NSS MARKET                           
*                                                                               
RDR20    DS    0H                                                               
         XC    WORK,WORK                                                        
         GOTO1 VADDAY,DMCB,MITSTART+1,WORK,14      MIDDLE OF MONTH              
         GOTO1 VDATCON,DMCB,(0,WORK),(3,WORK+20) YYMM IN BINARY                 
         MVC   TAPEBK,WORK+20      YYMM ONLY                                    
...
RLUNVWK  DS    0H                                                               
         ...
         MVI   PRVUNV,0                                                         
*                                                                               
RLUNV5   BAS   RE,LDCRCO           GET START OF MARKET TABLE BY BOOK            
         L     RE,FULL                                                          
RLUNV6   CLI   0(RE),X'FF'         END OF MARKET LIST TABLE?                    
         BE    RLUNV7A             YES                                          
         CLI   0(RE),X'F0'         END OF SUB-TABLE?                            
         BNE   *+8                                                              
         LA    RE,7(RE)            YES.GO PAST HEADER OF NEXT SUB-TABLE         
         B     RLUNV8              AND CONTINUE                                 
*                                                                               
RLUNV7A  MVI   RELSLOT,0                                                        
         MVI   SUMSLOT,0                                                        
         CLI   BYPREAD,C'U'        DONE RELEASEING MONTHLY UNV RECD?            
         BNE   *+18                                                             
         MVC   WEEK(6),DSCSTDT     YES: INIT WKLY START BOOK LOOP               
...
**>> DON'T RELS REG USG RECDS, ONLY DO SUMMARIES                                
*                                                                               
...
RLHPROG1 MVI   BYPREAD,C'H'        THEN CREATE THE SUMMARY                      
         ...
         B     RLHSUM                                                           
*                                                                               
RLHPROG2 BAS   RE,LDCRCO           GET CORRECT CRCOTAB INTO FULL                
         L     RE,FULL                                                          
...
*                                                                               
         ...
         STC   RE,RELSLOT                                                       
         XC    INTPNTI,INTPNTI                                                  
         B     BLDKEY              OUTPUT THE USAGE RECDS                       
         EJECT                                                                  
* *********************************************************************         
...
*                                                                               
         ...
         DROP  R9                                                               
*                                  RELEASE THE DEMOS                            
RLHSUM4A BAS   RE,LDCRCO           GET CORRECT CRCOTAB INTO FULL                
         L     RE,FULL                                                          
...
D4RTN    DS    0H                                                               
         ...
         BNE   D4SKIP                                                           
         CLI   TAPETYP,C'S'        FOR SYN BYPASS INDIV DAYS EXCP B&S           
*                                                                               
         B     D4RTN1                                                           
*                                                                               
         BNE   D4RTN1              KEEP RECD                                    
         CLC   MITAVG,=C'01'                                                    
         BH    D4RTN1                                                           
         CLI   MITBREAK,C'0'       KEEP BREAKOUTS AND SPECIALS                  
         BNE   D4RTN1                                                           
         CLI   MITSPC,C'0'                                                      
         BNE   D4RTN1              ELSE SKIP DAILY RECD                         
*                                                                               
...
*                                                                               
         CLI   TAPETYP,C'S'                                                     
         BNE   D4R01X                                                           
         CLC   MITHOUR(4),ZEROS    AS OF JUN26/00 NO TIMES FOR SYN              
         BNE   D4R01X                                                           
         LA    R1,SYNSTIM          DUMMY A START TIME BASED ON DPT              
D4R01C   CLI   0(R1),X'FF'         SET UNKNOWN DPT TO 3:00PM                    
         BE    *+22                                                             
         CLC   MIDREPDP,0(R1)      COMPARE ON DAYPART                           
         BE    *+12                                                             
         LA    R1,L'SYNSTIM(R1)                                                 
         B     D4R01C                                                           
         MVC   MITHOUR(4),2(R1)    SET APPX TIME ON DAYPT                       
*                                                                               
D4R01X   LA    RE,MIDDAYS          SET UP SAVETIME TABLE FOR PDATA              
         MVC   PHUT,SVPROJ                                                      
...
D4R13    MVC   PNUM,0(R1)          SAVE PROGRAM NUMBER                          
         ...
         MVC   PSHORT,MIDSDUR                                                   
*                                                                               
         CLC   PNET,=C'UPN '       CALL LETTER CHANGES                          
         BNE   *+10                                                             
         MVC   PNET,=C'PAR '                                                    
         CLC   PNET,=C'ION '                                                    
         BNE   *+10                                                             
...
* *********************************************************************         
BLDKEY   CLI   INTRTYP,PMCODEQU    TEST FOR 'Q' RECORD                          
         BE    BLD70                                                            
*                                                                               
...
RELPRG7  MVI   BYPREAD,C'S'        CREATE SUMMARY                               
         B     RELSUM                                                           
*                                                                               
RELPRG10 BAS   RE,LDCRCO           GET CORRECT CRCOTAB INTO FULL                
         L     RE,FULL                                                          
...
*                                                                               
         ...
         A     RE,=A(PTYPTAB)                                                   
         B     *+8                                                              
         A     RE,=A(PTYPTS03)     NEW TABLE BEGINNING SEP/2003                 
*                                                                               
         CLI   INTSTYP,C'B'                                                     
...
*        BNE   INCRSUM                                                          
         BE    RELSUMD                                                          
*                                                                               
         CLC   TAPEBK,=X'6709'     RECORD DATE BEFORE SEP/2003?                 
         BL    INCRSUM                                                          
*                                                                               
...
RELSUM2B DS    0H                                                               
         ...
         B     *+8                                                              
         L     R9,=A(PTDPNAS03)    NEW TABLE BEGINNING SEP/2003                 
*                                                                               
         XC    INTPNAME,INTPNAME                                                
...
RELSUM3A MVC   INTPNAME(25),2(R9)                                               
         ...
         L     R9,=A(PTYTAB)                                                    
         B     *+8                                                              
         L     R9,=A(PTYTAS03)     NEW TABLE BEGINNING SEP/2003                 
*                                                                               
         USING PTYTABD,R9                                                       
...
CNVWR    DS    0H                                                               
         ...
         MVC   GAASW,INTKEY+29                                                  
         MVC   BYTE,INTDAYWK                                                    
         NI    BYTE,X'0F'          WEEKS BITS                                   
         MVC   TMP,INTWEEKS        EXTENDED WEEKS BITS                          
...
* SWAP SAVEKEY WITH INTKEY IN ORDER TO RELEASE PREVIOUS RECD                    
         ...
         XC    SAVEKEY,INTKEY                                                   
*                                                                               
         CLC   =C'QNN',INTKEY      ACCUMULATE WEEKS FOR PROG AVG                
         BNE   *+8                                                              
         CLI   INTSTA+3,C'P'                                                    
         BNE   *+16                                                             
         OC    INTDAYWK,AVGWEEKS                                                
         OC    INTWEEKS,AVGWEEK2   EXTENDED WEEK BITS                           
*                                                                               
         MVI   AVGWEEKS,0                                                       
         MVI   AVGWEEK2,0                                                       
         LA    RE,SAVEREC                                                       
...
FLAG     DS    X                                                                
AVGWEEKS DS    X                   AVERGA RECD WEEK BITS                        
AVGWEEK2 DS    X                   AVERG RECD EXTENDED WEEK BITS                
         DS    0F                                                               
...
CRCWPAY  EQU   71                  WIRED DIGITAL CABLE W/PAY (1/2005)           
CRCWNOPY EQU   72                  WIRED DIGITAL CABLE W/O PAY (1/2005)         
...
CRIO125  EQU   251                 125,000 PLUS   (1/2005)                      
...
CRVGO    EQU   85                  VIDEO GAME OWNER  (1/2005)                   
*                                                                               
CROGHSP  EQU   94                 HISPANIC ORIGIN                               
CROGNHSP EQU   95                 NON-HISPANIC ORIGIN                           
CROGHSPS EQU   96                 HISPANIC ORIGIN SPANISH LANGUAGE              
CROGHSPB EQU   97                 HISPANIC ORIGIN BILINGUAL                     
CROGHSPE EQU   98                 HISPANIC ORIGIN ENGLISH LANGUAGE              
*                                                                               
...
CRTV4O   EQU   123                 4+                                           
*                                                                               
CRRENTH  EQU   150                 HOME IS RENTED (1/2005)                      
CROWNH   EQU   151                 HOME IS OWNED  (1/2005)                      
*                                 AGE OF HOH                                    
...
CICWPAY  EQU   103                 WIRED DIGITAL CABLE W/PAY (1/2005)           
CICWNOPY EQU   104                 WIRED DIGITAL CABLE W/O PAY (1/2005)         
*                                 HOUSEHOLD SIZE                                
CIHHS1   EQU   16                  1                                            
CIHHS2   EQU   17                  2                                            
CIHHS3   EQU   18                  3                                            
...
CIIO125  EQU   100                 125,000 PLUS        (1/2005)                 
*                                 SELECTED UPPER DEMOS                          
...
CIVGO    EQU   99                  VIDEO GAME OWNER (1/2005)                    
*                                                                               
CIOGHSP  EQU   105                HISPANIC ORIGIN                               
CIOGNHSP EQU   106                NON-HISPANIC ORIGIN                           
CIOGHSPS EQU   107                HISPANIC ORIGIN SPANISH LANGUAGE              
CIOGHSPB EQU   108                HISPANIC ORIGIN BILINGUAL                     
CIOGHSPE EQU   109                HISPANIC ORIGIN EMGLISH LANGUAGE              
*                                                                               
...
CITV4O   EQU   41                  4+                                           
*                                                                               
CIRENTH  EQU   101                 HOME IS RENTED (1/2005)                      
CIOWNH   EQU   102                 HOME IS OWNED  (1/2005)                      
*                                 AGE OF HOH                                    
...
*                                                                               
CIDVR    EQU   110                DVR HOUSEHOLD                                 
*!! UPDATE NMKTS BELOW EVERY TIME YOU ADD A NEW MARKET BREAK                    
*                                                                               
NMKTS    EQU   111                 NO OF MARKET BREAKS,INCLUDING TOT US         
         EJECT                                                                  
**********************************************************************          
...
COCWPAY  EQU   29                  WIRED DIGITAL CABLE W/ PAY (1/2005)          
COCWNOPY EQU   30                  WIRED DIGITAL CABLE W/O PAY (1/2005)         
...
COIO125  EQU   60                  125,000 PLUS        (1/2005)                 
*                                                                               
*                                 SELECTED UPPER DEMOS                          
...
COVGO    EQU   107                 VIDEO GAME OWNER (1/2005)                    
*                                                                               
COOGHSP  EQU   89                 HISPANIC ORIGIN                               
COOGNHSP EQU   90                 NON-HISPANIC ORIGIN                           
COOGHSPS EQU   108                HISPANIC ORIGIN SPANISH LANGUAGE              
COOGHSPB EQU   109                HISPANIC ORIGIN BILINGUAL                     
COOGHSPE EQU   110                HISPANIC ORIGIN ENGLISH LANGUAGE              
*                                                                               
...
COTV4O   EQU   106                 4+                                           
*                                                                               
CORENTH  EQU   148                 HOME IS RENTED (1/2005)                      
COOWNH   EQU   149                 HOME IS OWNED (1/2005)                       
*                                 AGE OF HOH                                    
...
*                                 CABLE                                         
         ...
         DC    AL2(CRCWPAY,CICWPAY)   WIRED DIG CABLE W/ PAY (1/2005)           
         DC    AL2(CRCWNOPY,CICWNOPY) WIRED DIG CABLE W/O PAY (1/2005)          
...
*                                 INCOME                                        
         ...
         DC    AL2(CRIO125,CIIO125)  125,000 PLUS        (1/2005)               
*                                 SELECTED UPPER DEMOS                          
...
*                                                                               
         ...
         DC    AL2(CRVGO,CIVGO)    VIDEO GAME OWNER (1/2005)                    
*                                                                               
         DC    AL2(CROGHSP,CIOGHSP)    HISPANIC ORIGIN (9/2006)                 
         DC    AL2(CROGNHSP,CIOGNHSP)  NON-HISPANIC ORIGIN (9/2006)             
         DC    AL2(CROGHSPS,CIOGHSPS)  HISPANIC ORIGIN SPANISH (1/2007)         
         DC    AL2(CROGHSPB,CIOGHSPB)  HISPANIC ORIGIN BILINGL (1/2007)         
         DC    AL2(CROGHSPE,CIOGHSPE)  HISPANIC ORIGIN ENGLISH (1/2007)         
*                                                                               
...
*                                                                               
         DC    AL2(CRRENTH,CIRENTH)  HOME IS RENTED (1/2005)                    
         DC    AL2(CROWNH,CIOWNH)    HOME IS OWNED  (1/2005)                    
*                                 AGE OF HOH                                    
...
CRCOTAB  DS    0H                                                               
CRCO6B05 DC    X'6B05'             NEW MARKETS EFFECTIVE MAY/2007               
         DC    AL4(CRCO6B01-CRCO6B05)  LENGTH OF TABLE                          
                                                                                
         DC    AL2(CIDVR,CODVR),X'00'  DVR HOUSEHOLD                            
         DC    X'F0'                                                            
*                                                                               
*                                                                               
CRCO6B01 DC    X'6B01'             NEW MARKETS EFFECTIVE JAN/2007               
         DC    AL4(CRCO6A09-CRCO6B01)  LENGTH OF TABLE                          
                                                                                
         DC    AL2(CIOGHSPS,COOGHSPS),X'00' HISPANIC OG SPANISH                 
         DC    AL2(CIOGHSPB,COOGHSPB),X'00' HISPANIC OG BILINGUAL               
         DC    AL2(CIOGHSPE,COOGHSPE),X'00' HISPANIC OG ENGLISH                 
         DC    X'F0'                                                            
*                                                                               
CRCO6A09 DC    X'6A09'             NEW MARKETS EFFECTIVE SEP/2006               
         DC    AL4(CRCO6901-CRCO6A09)  LENGTH OF TABLE                          
                                                                                
         DC    AL2(CIOGHSP,COOGHSP),X'00'   HISPANIC ORIGIN                     
         DC    AL2(CIOGNHSP,COOGNHSP),X'00' NON-HISPANIC ORIGIN                 
         DC    X'F0'                                                            
*                                                                               
CRCO6901 DC    X'6901'             NEW MARKETS EFFECTIVE JAN/2005               
         DC    AL4(CRCO6709-CRCO6901) LENGTH OF TABLE                           
         ...
         DC    X'F0'                                                            
*                                                                               
CRCO6709 DC    X'6709'             NEW MARKETS EFFECTIVE SEP/2003               
         DC    AL4(CRCO0000-CRCO6709) LENGTH OF TABLE                           
...
PTYPTAB  DC    C'GD',AL2(000),C'12  ',AL1(RG),AL2(EVGD),C'     '                
         ...
         DC    C'CC',AL2(000),C'0   ',AL1(RG),AL2(EV6P7I),C'     '              
         DC    C'DO',AL2(000),C'0   ',AL1(RG),AL2(EV6P7I),C'     '              
         DC    C'DN',AL2(000),C'0   ',AL1(RG),AL2(EV6P7I),C'     '              
         DC    C'N ',AL2(000),C'0   ',AL1(RG),AL2(EV6P7I),C'     '              
         DC    C'IA',AL2(000),C'0   ',AL1(RG),AL2(EV6P7I),C'     '              
...
*        DEDEMCNVD                                                              
       ...
       ++INCLUDE DEDEMTABD                                                      
         ...
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'143DENA09I   05/22/17'                                      
         END                                                                    
**********************************************************************          
*              NAD DEMO GROUPS                                                  
**********************************************************************          
