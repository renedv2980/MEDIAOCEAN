*          DATA SET RESFM19    AT LEVEL 158 AS OF 09/29/04                      
*PHASE T81819A,*                                                                
*INCLUDE LOADER                                                                 
...
*                                                                    *          
*  SEP29/04 (BU ) --- CORRECT A TSAR DUPLICATE KEY SITUATION         *          
*                                                                    *          
*                                                                    *          
*                                                                    *          
* ------------------------------------------------------------------ *          
...
*                                                                               
RREC0020 LA    R2,KEY              RESET R2 -> KEY                              
         USING RCONRTYP,R2                                                      
         CLC   KEY(3),KEYSAVE                                                   
         BNE   XIT                                                              
         CLI   OFFICE,0                                                         
...
RREC0040 CLI   0(R3),X'FF'         END OF FILE                                  
         BE    RREC0060                                                         
         CLI   0(R3),0                                                          
         BE    RREC0060                                                         
         CLC   0(5,R3),RCONRSTA                                                 
...
RREC0060 MVC   MYKEYSV,KEY         SAVE PASSIVE KEY FOR SEQ READ                
         B     RREC0160                                                         
...
* CHECK FOR NONGRAPHNET STATION                                                 
RREC0120 L     R3,=A(NONGRAPH)     NONGRAPHNET STATION TABLE                    
RREC0140 CLI   0(R3),X'FF'         END OF FILE                                  
         BE    RREC0160                                                         
         CLI   0(R3),0                                                          
         BE    RREC0160                                                         
         CLC   0(5,R3),KEY+6                                                    
...
*************************************************************                   
* IF (STATION IS WEB CONFIRM                                                    
*      OR                                                                       
* CONTRACT IS BE GRAPHNET)                                                      
*     AND                                                                       
* CONTRACT  MUST BE LAST SENT BY REP TODAY OR CONFIRMED TODAY                   
                                                                                
         TM    RCONMODR+1,X'40'    GRAPHNET?                                    
         BO    RREC0200            YES - CHECK LAST SENT/CFRMD                  
         CLI   OFFICVER,C'Y'       OFFICE VERSION?                              
         BE    NXTREC              YES - NOT GRAPHNET - SKIP RECD               
*                                  NO  - CHECK FOR WEB STATION:                 
*                                        STATIONS BEING READ IN STA SEQ         
*                                           VIA X'0C' KEY                       
         MVC   KEYSAV2(27),RCONREC                                              
*                                  SAVE ACTIVE CONTRACT KEY                     
         CLC   RCONKSTA,LASTSTA    STATION ALREADY RETRIEVED?                   
         BE    RREC0180            YES - DON'T REREAD IT                        
         MVC   LASTSTA,RCONKSTA    NO  - RETRIEVE STATION                       
         MVC   SAVAIO,AIO          SAVE CURRENT A(IO BUFFER)                    
         MVC   AIO,AIO3            SET A(IO AREA 3)                             
         XC    KEY,KEY                                                          
         MVI   KEY,2               SET STATION RECORD TYPE                      
         MVC   KEY+20(2),RCONKREP  INSERT REP CODE INTO KEY                     
         MVC   KEY+22(5),RCONKSTA  INSERT STATION INTO KEY                      
         GOTO1 HIGH                                                             
         CLC   KEY(27),KEYSAVE     STATION FOUND?                               
         BE    *+6                 YES                                          
         DC    H'0'                SHOULDN'T HAPPEN                             
         GOTO1 GETREC              RETRIEVE STATION RECORD INTO IOAREA3         
*                                                                               
*   RESET CONTRACT READ SEQUENCE AFTER RETRIEVING STATION RECORD                
*        STATION RECORD IS IN IO AREA 3                                         
*        CONTRACT RECORD IO AREA HAS BEEN SAVED IN SAVAIO                       
*        CURRENT CONTRACT KEY IS IN KEYSAV2                                     
*                                                                               
         MVC   AIO,SAVAIO          RESET CONTRACT IO AREA                       
         MVC   KEY(27),KEYSAV2     RESET CONTRACT KEY                           
         GOTO1 HIGH                                                             
         CLC   KEY(27),KEYSAVE     KEY FOUND?  MUST BE THERE                    
         BE    *+6                                                              
         DC    H'0'                NO                                           
RREC0180 EQU   *                                                                
*                                                                               
...
*   TEST END                                                                    
*                                                                               
         MVI   WEBCON,C'N'         TURN OFF 'WEB CONFIRM' FLAG                  
         L     R6,AIO3             AIO = IO AREA3/STATION RECORD                
         MVI   ELCODE,X'08'        GET EXTRA DESCRIP ELT                        
         BAS   RE,GETEL                                                         
         BE    *+6                 MUST BE THERE                                
         DC    H'0'                                                             
DIE      EQU   *                                                                
         USING RSTAXXEL,R6                                                      
         TM    RSTAOPTB,X'08'      CONFIRM VIA WEB STATION?                     
         BNO   NXTREC              NO  - SKIP THIS RECORD                       
         MVI   WEBCON,C'Y'         TURN ON  'WEB CONFIRM' FLAG                  
*                                                                               
         DROP  R6                                                               
*                                                                               
RREC0200 EQU   *                                                                
         ...
         BNE   RREC0220                                                         
         USING RCONSEND,R6                                                      
         CLI   WEBCON,C'Y'         WEB CONFIRM STATION?                         
         BE    RREC0210            YES - JUST CHECK TODAY'S DATE                
...
RREC0220 L     R6,AIO              ...OR CONFIRMED TODAY                        
         MVI   ELCODE,X'1F'                                                     
         BAS   RE,GETEL                                                         
         BNE   NXTREC                                                           
         USING RCONXEL,R6                                                       
...
*                                                                               
RREC0260 LA    RE,SRTREC          CLEAR RECORD AREA                             
         LA    RF,SRTLENE                                                       
         XCEF                                                                   
         L     R6,AIO                                                           
         USING RCONREC,R6                                                       
...
RREC0280 MVC   PNDKOOF,RCONKOFF    OFFICE                                       
...
RREC0300 MVC   PNDKREP,RCONKREP    REP                                          
...
* SORTREC DATA ********************************************                     
RREC0320 UNPK  PNDCON(9),RCONKCON(5)          CONTRACT NUMBER (PWO)             
         MVI   PNDCON+8,0                     CLEAR SIGN BYTE                   
* GET RID OF LEADING ZEROS OF CONTRACT NUMBER                                   
         XC    WORK,WORK                                                        
         LA    R1,8                                                             
         LA    RE,PNDCON                                                        
RREC0340 CLI   0(RE),C'0'                                                       
         BNE   RREC0360                                                         
         LA    RE,1(RE)                                                         
         BCT   R1,RREC0340                                                      
...
*                                                                               
         ...
         MVI   ELCODE,X'20'                                                     
         BAS   RE,GETEL                                                         
         BNE   RREC0460            PER BILL UHR/IT JUST AIN'T THERE             
         USING RCONSEND,R6                                                      
         MVC   PNDCNFDT,RCONSSDT                                                
         MVC   PNDCNFTM,RCONSSTI                                                
         B     RREC0460                                                         
         USING RMODELEM,R6                                                      
RREC0440 MVC   PNDCNFDT,RMODEL1D   DATE                                         
...
RREC0540 DS    0H                                    AGE                        
         ...
         B     RREC0560            PASS REC TO TSAROFF                          
         EJECT                                                                  
...
*                         1.3.5.7.9.1.3.5.7.9.1.3.5.7.9                         
         MVC   P+1(28),=C'TSAR PRE  =       = XXXXXXXX'                         
         MVC   P+12(1),TSERRS                                                   
         MVC   P+20(36),TSARD                                                   
         BAS   RE,SPLAT                                                         
*&&                                                                             
...
*        TEST DISPLAY                                                           
         MVC   P,SPACES                                                         
*                         1.3.5.7.9.1.3.5.7.9.1.3.5.7.9                         
         MVC   P+1(28),=C'TSAR POST =       = XXXXXXXX'                         
         MVC   P+12(1),TSERRS                                                   
         MVC   P+20(36),TSARD                                                   
         BAS   RE,SPLAT                                                         
*&&                                                                             
...
*                                                                               
         CLI   TSERRS,0                                                         
         BE    RREC0580                                                         
         CLI   TSERRS,X'20'        DUPLICATE KEY ON ADD?                        
         BE    RREC0600            YES - JUST SKIP IT                           
         DC    H'0'                NO  - ABORT                                  
RREC0580 EQU   *                                                                
         L     R1,MYCOUNT                                                       
         LA    R1,1(R1)                                                         
...
PRT42    CLI   PNDVER,0                                                         
         BE    PRT45                                                            
         MVI   PLTYPE,C'R'                                                      
         TM    PNDVER,X'01'        R=ODD VERSION (REP)                          
         BO    *+8                 S=EVEN VERSION (STATION)                     
         MVI   PLTYPE,C'S'                                                      
...
TSARBUFL DC    A(SRTLENE*16000)    EXPANDED FROM 12000                          
*                                                                               
*                                                                               
TSAREA   DS    XL64                                                             
*                                                                               
...
NONGRAPH DS    CL10000             ROOM FOR 2000 STATIONS                       
         DC    X'FF'                                                            
         EJECT                                                                  
       ++INCLUDE DDTSARD                                                        
         EJECT                                                                  
...
MKTSLNE  EQU   *-MKTSAVE                                                        
KEYSAV2  DS    CL48                SECOND KEYSAVE AREA                          
LASTSTA  DS    CL5                                                              
SAVAIO   DS    A                                                                
WEBCON   DS    CL1                 Y = WEB CONFIRM STATION                      
MYWORKND EQU   *                                                                
*                                                                               
         EJECT                                                                  
*                                                                               
...
EDICTD   DSECT                                                                  
       ...
       ++INCLUDE EDILINKD                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'158RESFM19   09/29/04'                                      
         END                                                                    
