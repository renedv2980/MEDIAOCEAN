*          DATA SET DECAVCI    AT LEVEL 009 AS OF 02/28/20                      
*PROCESS USING(WARN(15))                                                        
*PHASE DECAVCIE                                                                 
*---------------------------------------------------------------------*         
* WEEKLY CABLE COMMERCIAL AVERAGE CONVERSION INPUT PHASE              *         
* IPHASE: DECAVCI                                                     *         
* OPHASE: DECAVCO                                                     *         
*                                                                     *         
* NOTE: THIS CONVERSION HAS BEEN COPIED FROM THE WKLY CABLE CONVERSION*         
*       (DECN0239) WITH THE NECESSARY CHANGES. ALTHOUGH THERE IS NO   *         
*       TIME PERIOD RECORD ON THIS FILE, I LEFT THE CODE IN JUST IN   *         
*       CASE WE MIGHT GET IT IN THE FUTURE                            *         
*---------------------------------------------------------------------*         
         TITLE 'DEMO CONVERSION: WEEKLY CABLE COMMERCIAL AVERAGE'               
DECAVCI  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,DECAVCI,R3,R4                                                  
...
READ41A  CLI   0(R5),X'FF'                                                      
         ...
         MVC   MITSRC,KEYSRC       SOURCE FOR THE MIT,BEFORE ADJUSTMENT         
*                                                                               
...
*                                                                               
         ...
         BNE   VTYPERR                                                          
*                                                                               
         CLC   MITSEQ,=C'04'       PROGRAM AVERAGE RECORDS?                     
         JNE   READ41B                                                          
         ...
         J     READ41C                                                          
*                                                                               
READ41B  CLC   MITSEQ,=C'06'       COMMERCIAL AVERAGE RECORDS?                  
         JNE   READ41C                                                          
         CLI   OOHVWTYP,OOHLQ      YES - LIVE?                                  
         JNE   *+8                                                              
         MVI   KEYSRC,SRCLOCLQ                                                  
...
*                                                                               
VTYPERR  CLC   MITSEQ,=C'04'       PROGRAM AVERAGE RECORDS                      
         BE    *+14                                                             
         CLC   MITSEQ,=C'06'                                                    
         BNE   VTYPERRX                                                         
         CLI   PVMITSRC,0          THE FILE SHOULD HAVE DATA FOR JUST           
         BNE   VTYPERR5            ONE VIEWING TYPE                             
         MVC   PVMITSRC,MITSRC                                                  
         B     VTYPERRX                                                         
VTYPERR5 CLC   PVMITSRC,MITSRC                                                  
         BE    *+6                                                              
         DC    H'0'                MULTIPLE VIEWING TYPES IN THE FILE           
VTYPERRX DS    0X                                                               
*                                                                               
         CLC   MITSEQ,=C'04'       PROGRAM AVERAGE RECORDS                      
         BNE   READ41C             FOR LIVE, LIVE+SD, LIVE+7                    
...
*                                                                               
READ41C  CLI   MITORIG,C'0'        BYPASS REPROCESSING FOR NOW                  
         BE    READ42                                                           
...
*                                                                               
RDR      DS    0H                                                               
         CLC   =C'OOH',IPHSPARM    OOH MIT TAPE?                                
         JNE   RDR4                                                             
         CLC   =C'CABLE OOH MIT',MI0FILE                                        
         JNE   RDR2                                                             
         CLC   =C'LIVE ',MI0FILE+14                                             
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHLQ                                                   
         B     RDR5                                                             
         CLC   =C'LIVE+SD',MI0FILE+14                                           
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHLSQ                                                  
         B     RDR5                                                             
         CLC   =C'LIVE+3',MI0FILE+14                                            
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHL3Q                                                  
         B     RDR5                                                             
         CLC   =C'LIVE+7',MI0FILE+14                                            
         JNE   *+2                                                              
         MVI   OOHVWTYP,OOHL7Q                                                  
         B     RDR5                                                             
*                                                                               
RDR2     CLC   =C'CABLE OOH PREVIEW',MI0FILE                                    
         JNE   *+2                                                              
         CLC   =C'LIVE ',MI0FILE+18                                             
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHLQ                                                   
         B     RDR5                                                             
         CLC   =C'LIVE+SD',MI0FILE+18                                           
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHLSQ                                                  
         B     RDR5                                                             
         CLC   =C'LIVE+3',MI0FILE+18                                            
         BNE   *+12                                                             
         MVI   OOHVWTYP,OOHL3Q                                                  
         B     RDR5                                                             
         CLC   =C'LIVE+7',MI0FILE+18                                            
         JNE   *+2                                                              
         MVI   OOHVWTYP,OOHL7Q                                                  
         B     RDR5                                                             
*                                                                               
RDR4     CLC   =C'CABLE COMMERCIAL MIT',MI0FILE                                 
         BE    *+6                                                              
...
*        BE    *+8                 DON'T OUTPUT A UNV RECD                      
         ...
         MVI   INTMEDIA,X'83'      COMM AVG HAS MEDIA LOWER CASE 'C'            
         MVC   INTKSRC,KEYSRC                                                   
         MVC   INTBOOK,BOOK                                                     
         MVC   INTIBOOK,IBOOK                                                   
...
*              RECORD SEQUENCE CODE = 3,5                                       
*              DATA TYPE CODE       = TVU,BAS                                   
*              RECORD TYPE          = H - P - P                                 
* *********************************************************************         
TVUREC   DS    0H                                                               
         CLI   MITREC,C'H'                                                      
         ...
         DC    H'0'                                                             
*                                                                               
TVU1     CLI   RELS,0              NOTHING TO RELEASE-JUST PROCESS              
         BE    TVU10                                                            
...
TVU20    L     R1,=A(DEMAREA)      SAVE IN DEMO AREA                            
         ...
         MVI   PUTSW,1             FLAG FOR PUT DEMOS                           
...
*                                                                               
         ...
         BNE   TVU30                                                            
         L     R1,=A(DEMAREA)                                                   
         AH    R1,PUTDISP          '05'-SAVE IN HUTS ONLY                       
         ST    R1,DMCB                  NO PUTS AVAILABLE                       
         LA    R1,MIXPDEM                                                       
         ST    R1,DMCB+4                                                        
         MVI   PUTSW,1                                                          
         BAS   RE,SLOTDEM          PROCESS DEMOS-SLOT IN BUFFER                 
*                                                                               
TVU30    L     R1,=A(DEMAREA)      NOW SLOT OTHER VARIOUS DEMOS                 
         AH    R1,OTHDISP                                                       
         ST    R1,DMCB             DMCB=A(SAVE AREA)                            
         BAS   RE,SLOTOTH          PROCESS DEMOS-SLOT IN BUFFER                 
*                                                                               
         MVC   SAVEINT(L'SAVEINT),INTVALS                                       
...
*                                                                               
         CLI   RELS,5                                                           
         BNE   USG40                                                            
         ZIC   RE,RELSLOT          NOW MOVE IN '05' HUTS                        
         MH    RE,MKTDISP                                                       
         A     RE,=A(DEMAREA)                                                   
         LA    R1,INTACCS                                                       
         AH    RE,PUTDISP                                                       
         ...
         LH    RF,=Y(NDEMS*4)                                                   
         BCTR  RF,0                                                             
         EXMVC RF,0(R1),0(RE)      SAVE HUTS IN INTACCS                         
*                                                                               
USG40    LA    R1,INTACCS                                                       
         AH    R1,=Y(RIUNVSQ)      R1=DESTN IN INTERIM REC FOR UNIVS            
         ZIC   RE,COVSMPD          GET UNIVS FOR COVG SAMPLE                    
...
*                                                                               
         ZIC   RE,RELSLOT          MOVE OTHER DEMOS TO INTERIM RECD             
         MH    RE,MKTDISP          MKT BRK DISPLACEMENT LENGTH                  
         A     RE,=A(DEMAREA)                                                   
         AH    RE,OTHDISP                                                       
         LA    R1,INTACCS                                                       
         AH    R1,=Y(RIOTHSQ)                                                   
         LH    RF,=Y(NDEMOQ*4)     RF=NUMBER BYTES TO MOVE                      
         BCTR  RF,0                                                             
         EXMVC RF,0(R1),0(RE)      SAVE DEMOS IN INTACCS                        
*                                                                               
         ZIC   RE,RELSLOT                                                       
         ...
         BNE   USG60                                                            
         MVC   INTSTA(4),=C'HUT '                                               
         CLI   INTFEED,C'L'        THIS HAPPENS FOR REG DATA ONLY               
         BNE   *+8                 FOR WEIGHTED DATA, RECDS WITH FEED           
         MVI   INTBTYP,C'L'        'L' ARE BYPASSED                             
*                                                                               
...
HDR      NTR1                      COMMON H-RECD PROC BETW RECD TYPES           
         ...
         CVB   R1,DUB                                                           
         STC   R1,INTAVG           HOMES FOR MKT BREAK                          
*                                                                               
         PACK  DUB,MIHPROJ         CONVERT HOMES PROJECTION                     
         ...
         CLC   MITSEQ,=C'03'                                                    
         BNE   HDR4                                                             
         CLI   MITMIN+1,C'0'       FIRST QTR HOUR--GRAB QTRHR HOMES             
         BNE   *+10                                                             
         PACK  DUB,MIHPROJ1        1ST QTR HOUR'S HOMES                         
         CLI   MITMIN+1,C'5'                                                    
         BNE   *+10                                                             
         PACK  DUB,MIHPROJ2        2ND QTR HOUR'S HOMES                         
         B     HDR5                                                             
HDR4     CLC   MITSEQ,=C'04'       FOR PRG RECD, HOMES IN DIFF LOCTN            
         BE    *+14                                                             
         CLC   MITSEQ,=C'06'       COMMERCIAL AVG PROGRAM RECORDS               
         BNE   HDR5                                                             
         PACK  DUB,MIDPROJ         HOMES PROJECTION FOR PRG RECD ONLY           
HDR5     CVB   R1,DUB                                                           
         ST    R1,MKTHOME          HOMES FOR MKT BREAK                          
         CLC   MITMKTBR,=C'000'    IF TOTAL SAMPLE SAVE AS USHOMES ALSO         
         BNE   *+8                                                              
         ST    R1,USHOME           USA HOMES                                    
*                                                                               
         PACK  DUB,MIHUSHUT        NOW SAVE THE HUTS                            
         CLC   MITQTRID,=C'  '     IS THIS A QTR HOUR RECD?                     
         BE    HDR4A               NO                                           
         CLC   MITSEQ,=C'05'       TIME PERIOD DATA ONLY                        
         BE    *+10                                                             
         CLC   MITSEQ,=C'03'                                                    
         BNE   HDR4A                                                            
         CLI   MITMIN+1,C'0'       FIRST QTR HOUR--GRAB QTRHR HUTS              
         BNE   *+10                                                             
         PACK  DUB,MIHUSHT1        1ST QTR HOUR'S HUTS                          
         CLI   MITMIN+1,C'5'                                                    
         BNE   *+10                                                             
         PACK  DUB,MIHUSHT2        2ND QTR HOUR'S HUTS                          
         B     HDR5A                                                            
HDR4A    CLC   MITSEQ,=C'04'       FOR PRG RECD, HUTS IN DIFF LOCTN             
         BE    *+14                                                             
         CLC   MITSEQ,=C'06'       FOR PRG RECD, HUTS IN DIFF LOCTN             
         BNE   HDR5A                                                            
         SR    R1,R1                                                            
         CLC   MIDUSHUT,BLANKS     DEAL WITH BLANKS                             
         BE    HDR5A1                                                           
         PACK  DUB,MIDUSHUT        HUTS FOR PRG RECD ONLY                       
HDR5A    CVB   R1,DUB                                                           
HDR5A1   ST    R1,MKTHUT           HUT FOR MKT BREAK                            
         CLC   MITMKTBR,=C'000'    IF TOTAL SAMPLE SAVE AS USHUT ALSO           
         BNE   *+8                                                              
         ST    R1,USHUT            USA HUTS                                     
*                                                                               
         PACK  DUB,MIHCAHUT        NOW SAVE THE COVERAGE AREA HUTS              
         CLC   MITQTRID,=C'  '     IS THIS A QTR HOUR RECD?                     
         BE    HDR4B               NO                                           
         CLC   MITSEQ,=C'05'       TIME PERIOD DATA ONLY                        
         BE    *+10                                                             
         ...
         CLI   MITMIN+1,C'0'       FIRST QTR HOUR--GRAB QTRHR HUTS              
         BNE   *+10                                                             
         PACK  DUB,MIHCAHT1        1ST QTR HOUR'S HUTS                          
         CLI   MITMIN+1,C'5'                                                    
         BNE   *+10                                                             
         PACK  DUB,MIHCAHT2        2ND QTR HOUR'S HUTS                          
         B     HDR5B                                                            
HDR4B    CLC   MITSEQ,=C'04'       FOR PRG RECD, HUTS IN DIFF LOCTN             
         BE    *+14                                                             
         CLC   MITSEQ,=C'06'       COMMERCIAL PROGRAM RECORDS                   
         BNE   HDR5B                                                            
         SR    R1,R1                                                            
         CLC   MIDCAHUT,BLANKS                                                  
         BE    HDR5B1                                                           
         PACK  DUB,MIDCAHUT        HUTS FOR PRG RECD ONLY                       
HDR5B    CVB   R1,DUB                                                           
HDR5B1   ST    R1,CBLHUT           CABLE HUTS BY COVERAGE AREA                  
*                                                                               
...
*                                                                               
         CLC   MITSEQ,=C'03'       SAVE US-HUTS                                 
         BNE   HDRX                                                             
         CLC   COVALPH,=C'0000'                                                 
         BNE   HDR65               USE SAVED US-HUTS                            
*                                                                               
         SR    R5,R5               SAVE US-HUTS BY FEED/MRKT/DAY/HH/QH          
         L     RE,=A(CRCOTAB)                                                   
HDR30    CLC   =X'FFFF',0(RE)                                                   
         BNE   *+6                                                              
         DC    H'0'                                                             
         CLC   MKTBRK,0(RE)                                                     
         BE    HDR35                                                            
         LA    RE,L'CRCOTAB(RE)                                                 
         AHI   R5,1                INDEX TO CORRECT MARKET BREAK                
         B     HDR30                                                            
*                                                                               
HDR35    CLC   MITQTRID,=C'  '     IF SAVING HUTS BY HALF-HOUR,                 
         BNE   HDR50                                                            
         L     RE,=A(HUTBYHH)      USE HALF-HOUR BUFFER                         
         USING HUTBYHQD,RE                                                      
HDR36    L     RF,=A(HUTBYHHX)                                                  
         CR    RE,RF                                                            
         BL    *+6                                                              
         DC    H'0'                TABLE OVERFLOW. INCREASE NFEEDS              
         CLI   HUTFEED,0                                                        
         BE    HDR37                                                            
         CLC   MITFEED,HUTFEED     POINT TO CORRECT FEED                        
         BE    HDR37                                                            
         LA    RE,L'HUTBYHH(RE)                                                 
         B     HDR36                                                            
HDR37    MVC   HUTFEED,MITFEED                                                  
         LA    RE,HUTDEMOS                                                      
         DROP  RE                                                               
         MHI   R5,DAYHHQ                                                        
         AR    RE,R5               POINT TO MARKET BREAK                        
         LA    RF,MIHDAYS                                                       
         SR    R5,R5                                                            
HDR40    CLI   0(RF),C'1'                                                       
         BE    HDR42                                                            
         ...
         B     HDR40                                                            
HDR42    MHI   R5,HHQ                                                           
         AR    RE,R5               POINT TO CORRECT DAY                         
         PACK  DUB,MITHLFID                                                     
         CVB   R1,DUB                                                           
         BCTR  R1,0                                                             
         MHI   R1,4                                                             
         AR    RE,R1               POINT TO CORRECT HH#                         
         MVC   0(4,RE),MKTHUT      STORE HUT FOR THIS HH                        
         B     HDRX                                                             
*                                                                               
HDR50    L     RE,=A(HUTBYQH)      USE QUARTER-HOUR BUFFER                      
         USING HUTBYHQD,RE                                                      
HDR56    L     RF,=A(HUTBYQHX)                                                  
         CR    RE,RF                                                            
         BL    *+6                                                              
         DC    H'0'                TABLE OVERFLOW. INCREASE NFEEDS              
         CLI   HUTFEED,0                                                        
...
HDR62    MHI   R5,QHQ                                                           
         ...
         BCTR  R1,0                                                             
         MHI   R1,4                                                             
         AR    RE,R1               POINT TO CORRECT QH#                         
         MVC   0(4,RE),MKTHUT      STORE HUT FOR THIS QH                        
         B     HDRX                                                             
*                                                                               
*                                                                               
HDR65    SR    R5,R5               USE SAVED US-HUTS BY HH AND QH               
         L     RE,=A(CRCOTAB)                                                   
HDR70    CLC   =X'FFFF',0(RE)                                                   
         BNE   *+6                                                              
         DC    H'0'                                                             
         CLC   MKTBRK,0(RE)                                                     
         BE    HDR71                                                            
         ...
         B     HDR70                                                            
*                                                                               
HDR71    CLC   MITQTRID,=C'  '     IF NEED HUTS BY HALF-HOUR,                   
         BNE   HDR90                                                            
         L     RE,=A(HUTBYHH)      USE HALF-HOUR BUFFER                         
         USING HUTBYHQD,RE                                                      
HDR76    L     RF,=A(HUTBYHHX)                                                  
         CR    RE,RF                                                            
         BL    *+6                                                              
         DC    H'0'                MISSING US-HUT FOR THIS FEED                 
         CLI   HUTFEED,0                                                        
         BNE   *+6                                                              
         DC    H'0'                MISSING US-HUT FOR THIS FEED                 
         CLC   MITFEED,HUTFEED     POINT TO CORRECT FEED                        
...
HDR82    MHI   R5,HHQ                                                           
         ...
         MHI   R1,4                                                             
         AR    RE,R1               POINT TO CORRECT HH#                         
         MVC   MKTHUT,0(RE)        STORE HUT FOR THIS HH                        
         CLC   MITMKTBR,=C'000'    IF TOTAL SAMPLE SAVE AS USHUT ALSO           
         BNE   *+10                                                             
         MVC   USHUT,MKTHUT                                                     
         B     HDRX                                                             
*                                                                               
HDR90    L     RE,=A(HUTBYQH)      USE QUARTER-HOUR BUFFER                      
         USING HUTBYHQD,RE                                                      
HDR96    L     RF,=A(HUTBYQHX)                                                  
         CR    RE,RF                                                            
         BL    *+6                                                              
         DC    H'0'                MISSING US-HUT FOR THIS FEED                 
         CLI   HUTFEED,0                                                        
...
*                      DMCB+4  - A(1ST DEMO)                                    
*                      MKTBRK  - MKT BREAK                                      
*                      USHOME  - US HOMES PROJECTION                            
*                      MKTHOME - MKT BREAK HOMES PROJECTION                     
*                      USHUT   - US HUT                                         
*                      MKTHUT  - MKT BREAK HUT                                  
*                      CBLHUT  - CABLE HUT BY COVERAGE AREA                     
* *********************************************************************         
*                                                                               
SLOTDEM  NTR1                                                                   
...
SLOT10   MH    RE,MKTDISP          DISP TO MKT BRK IN BUFFER                    
         ...
         ST    RE,DMCB             RE-SAVE                                      
         MVC   RIUSA*4(4,RE),USHOME  SLOT HOMES AND MKT BRK HOMES               
         MVC   RIHOMES*4(4,RE),MKTHOME                                          
         CLI   PUTSW,1                                                          
         BNE   *+16                                                             
         MVC   RIUSA*4(4,RE),USHUT   SLOT HUTS AND MKT BRK HUTS                 
...
SLOT19B  PACK  DUB,0(9,RF)         CONVERT DEMO TO NUMERIC                      
         CVB   R1,DUB                                                           
SLOT19C  STCM  R1,15,0(RE)         SAVE DEMO IN BUCKET                          
*                                                                               
SLOT20   L     RF,DMCB+4           INPUT DEMO ADDRESS                           
         ...
         EJECT                                                                  
* *********************************************************************         
* SLOTOTH -    SLOT DEMOS OTHER VARIOUS DEMOS                                   
*        INPUT PARMS:  DMCB    - A(OUTPUT BUFER)                                
*                      MKTBRK  - MKT BREAK                                      
*                      CBLHUT  - CABLE HUT BY COVERAGE AREA                     
* *********************************************************************         
...
SLOTO5   CLC   MKTBRK,0(R1)                                                     
         ...
         DC    H'0'                                                             
*                                                                               
SLOTO10  MH    RE,MKTDISP          DISP TO MKT BRK IN BUFFER                    
         A     RE,DMCB             ADD ADDRESS OF OUTPUT BUFFER                 
         ST    RE,DMCB             RE-SAVE                                      
*                                                                               
*SLOT OTHER VARIOUS DEMOS                                                       
         MVC   RICHUTS*4(4,RE),CBLHUT                                           
*                                                                               
         B     XIT                                                              
         EJECT                                                                  
* *********************************************************************         
...
PRGREC   DS    0H                                                               
         ...
         BE    PRG1                                                             
         CLI   MITREC,C'C'         COMMERCIAL PROGRAM RECORD                    
         BE    PRG1                                                             
         DC    H'0'                                                             
*                                                                               
...
PRG10    MVI   RELS,4                                                           
         ...
         CLI   MKTBRK,0                                                         
         BNE   *+8                                                              
         BAS   RE,PDRREC           PROCESS 'D'/'C' RECD-DESCRIPTOR INFO         
         MVI   INTRTYP,PMCODEQU    'Q' RECORD                                   
         MVI   INTMEDIA,C'C'                                                    
         CLI   MITREC,C'C'         COMMERCIAL AVERAGE DATA                      
         BNE   *+8                                                              
         MVI   INTMEDIA,X'83'      GETS MEDIA LOWER CASE 'C'                    
         MVC   SVKSRC,KEYSRC       SAVE SOURCE/VIEWING TYPE FOR QPRGKD          
         MVC   INTSTA(4),COVALPH   USAGE TO GEOGRAPHIC MKT                      
...
*                                                                               
         ...
         BAS   RE,SLOTDEM          PROCESS DEMOS-SLOT IN BUFFER                 
*                                                                               
         L     R1,=A(DEMAREA)      NOW SLOT OTHER VARIOUS DEMOS                 
         AH    R1,OTHDISP                                                       
         ST    R1,DMCB             DMCB=A(SAVE AREA)                            
         BAS   RE,SLOTOTH          PROCESS DEMOS-SLOT IN BUFFER                 
*                                                                               
...
*                                                                               
         ...
         MVC   PPREM,MIDPREM                                                    
         MVC   PSHORT,MIDSDUR                                                   
         MVC   INTMRKT,TAPEMKT     CABLE MKT=310                                
         MVC   INTSTA(4),COVALPH   SAVE NETWORK IN KEY                          
         MVI   INTSTA+4,C'C'       CABLE                                        
*                                                                               
         XC    INTPNUM,INTPNUM                                                  
...
*                                                                               
         ...
         STCM  R1,3,INTCOMSC                                                    
*                                                                               
PDR70    XC    INTCOMTC,INTCOMTC   CONTRIBUTING COMMERCIAL TELECASTS            
         CLC   MIDCNUM,BLANKS                                                   
         BE    PDR72                                                            
         PACK  DUB,MIDCNUM                                                      
         CVB   R1,DUB                                                           
         STC   R1,INTCOMTC                                                      
*                                                                               
PDR72    PACK  DUB,MIDAUPRJ        PROJECTION (XXX,XXX,XXX)                     
         CVB   R1,DUB                                                           
         SR    R0,R0                                                            
...
***      BE    PRGREL5             WHEN DONE FORMING INTERD'S FOR MKTS          
         BE    PRGTIMX             NO TIME PERIOD DATA ON THIS TAPE             
         CLC   1(1,RE),RELSLOT                                                  
         BE    *+12                                                             
         LA    RE,5(RE)                                                         
         B     PRGREL13                                                         
...
PRGKEY   NTR1                                                                   
         ...
         CLI   INTMEDIA,C'A'       FORCE COMMERCIAL DATA TO SORT                
         BNL   *+8                 SEPARATELY                                   
         MVI   QCODE,X'98'         LOWER CASE 'Q' FOR COMM DATA                 
         MVC   QKSRC,SVKSRC        VIEWING TYPE                                 
*                                                                               
...
*                                                                               
         ...
         CLI   INTMEDIA,C'A'       FORCE COMMERCIAL DATA TO SORT                
         BNL   *+8                 SEPARATELY                                   
         MVI   QCODE,X'99'         LOWER CASE 'R' FOR COMM CORRECTIONS          
         MVC   QNET,INTSTA                                                      
         XC    QDATE,QDATE                                                      
...
*                                                                               
         MVI   PRDW+2,3            FEED HIERARCHY FOR PROG NAMES:               
         CLI   INTFEED,C'D'          DUAL = 1 (HIGHEST)                         
         BNE   *+8                                                              
         MVI   PRDW+2,1                                                         
         CLI   INTFEED,C'Y'          LIVE = 2 (2ND NIGHEST)                     
         BNE   *+8                                                              
         MVI   PRDW+2,2              EVERYTHING ELSE = 3 (LOWEST)               
*                                                                               
         MVC   PRDW+3(1),INTDUR    SORT HIGHER DUR FIRST                        
         XI    PRDW+3,X'FF'                                                     
*                                                                               
         L     RE,INDEX                                                         
...
GETFILN  NTR1                                                                   
         ...
         MVC   PJBTYPE,INTBTYP     -NEEDED FOR WEIGHTED DATA                    
...
CNVWR10  CLI   INTRTYP,C'P'        TIME PERIOD RECDS ONLY                       
         ...
         CLI   PRSTAT+4,C'U'                                                    
         BNE   *+10                                                             
         MVC   INTKSRC,MITSRC      FOR UNIVERSES USE MIT SOURCE                 
...
*                                                                               
         ...
         DROP  RE                                                               
*                                                                               
CNVWR20  CLI   CNV1ST,0            MOVE IN PRG NAME FOR THIS QHR                
         BE    CNVWRX                                                           
         CLC   INTKEY(PRDW-PRKEY+1),PRVKEY SAME AS LAST KEY?                    
         BNE   CNVWRX              DIFF KEY FROM SAVED PRG NAME                 
***      MVC   INTPNAME,SVPNAME    NOT NEEDED IF NO TP DATA                     
*                                                                               
CNVWRX   LA    RE,INTKEY                                                        
         USING QPRGKD,RE                                                        
         CLI   QCODE,X'98'         RESTORE UPPER CASE IN THE SORTKEY            
         BNE   *+8                 FOR COMMERCIAL DATA                          
         MVI   QCODE,C'Q'                                                       
         CLI   QCODE,X'99'                                                      
         BNE   *+8                                                              
         MVI   QCODE,C'R'                                                       
...
HPTADD5  XR    R6,R6                                                            
         ...
         BCT   R0,HPTADD5                                                       
*                                                                               
         LA    R5,INTACCS          NOW ACCUMULATE OTHER DEMOS                   
         AHI   R5,RIOTHSQ                                                       
         LA    R0,NDEMOQ                                                        
*                                                                               
HPTADD10 XR    R6,R6                                                            
         ...
         BCT   R0,HPTADD10                                                      
*                                                                               
         CLI   HPTREC,5            ACCUMULATE TP HUTS                           
         BNE   HPTADDX                                                          
         SR    R6,R6                                                            
         OC    0(4,R1),0(R1)                                                    
         BZ    *+8                                                              
         ...
         LA    R5,INTACCS                                                       
         AHI   R5,RIPUTSQ                                                       
         ICM   RF,15,RIHOMES*4(R5)                                              
         AR    R6,RF                                                            
         STCM  R6,15,0(R9)                                                      
*                                                                               
         LA    R9,4(R9)            AND TP US-HUTS                               
         SR    R6,R6                                                            
         OC    0(4,R1),0(R1)                                                    
         BZ    *+8                                                              
         ICM   R6,15,0(R9)                                                      
         ICM   RF,15,RIUSA*4(R5)                                                
         AR    R6,RF                                                            
         STCM  R6,15,0(R9)                                                      
*                                                                               
HPTADDX  BR    RE                                                               
...
*                                                                               
         CLI   HPTREC,5            SAVE TP HUTS FOR '05'                        
         BNE   HPTR51                                                           
         LA    R1,INTACCS                                                       
         AH    R1,=Y(RIPUTSQ)                                                   
         MVC   RIHOMES*4(8,R1),NDEMS*4+NDEMOQ*4(R6)                             
*                                                                               
HPTR51   LA    R1,INTACCS          SLOT UNIVS                                   
         AH    R1,=Y(RIUNVSQ)      R1=DESTN IN INTERIM REC FOR UNIVS            
         ZIC   RE,COVSMPD          GET UNIVS FOR COVG SAMPLE                    
         MH    RE,CVGLN            BUMP TO CVG IN UNVBUFF                       
         ...
         MVC   0(0,R1),0(RE)       MOVE IN UNIVS (AFTER DEMOS)                  
*                                                                               
         LA    R1,INTACCS          COPY OTHER DEMOS                             
         AH    R1,=Y(RIOTHSQ)                                                   
         LH    RF,=Y(NDEMOQ*4)     RF=NUMBER BYTES TO MOVE                      
         BCTR  RF,0                                                             
         EXMVC RF,0(R1),NDEMS*4(R6)   SAVE DEMOS IN INTACCS                     
*                                                                               
...
HPTDIV   NTR1                                                                   
         LA    R0,NDEMS                                                         
         AHI   R0,NDEMOQ+2         +CABLE HUTS + TP HUTS FOR '05'               
...
USHOME   DS    F                   HALF HOUR US HOMES PROJ(XXX,XXX,XXX)         
MKTHOME  DS    F                   MKT BREAK HOMES  PROJ  (XXX,XXX,XXX)         
USHUT    DS    F                   TOTAL US HUT PROJ  (XXX,XXX,XXX)             
MKTHUT   DS    F                   MKT BREAK HUT PROJ  (XXX,XXX,XXX)            
CBLHUT   DS    F                   COVERAGE AREA HUT (CABLE)                    
SVAVGHI  DS    F                   SAVE PROGRAM HH                              
APACKWT  DS    A                   A(PACKWT) AREA                               
NOTAVAL  DS    C                   DATA NOT AVAILABLE                           
PUTSW    DS    X                   FLAG FOR PROCCESSING PUT DEMOS               
*                                                                               
SVSEQ    DC    CL2' '                                                           
SVNET    DC    CL6' '                                                           
SVCVG    DC    CL6' '                                                           
...
SVKSRC   DS    C                   SAVED PROG KEY SOURCE/VIEWING TYPE           
MITSRC   DS    C                   SAVED PROG KEY SOURCE FROM MIT TAPE          
PVMITSRC DC    X'00'               PREVIOUS MIT SOURCE                          
*                                                                               
...
OTHDISP  DC    AL2(NDEMS*2*4)      DISP TO THER DEMOS                           
MKTDISP  DC    AL2(NDEMS*4*2+NDEMOQ*4)  DISP TO NEXT MKT BRK                    
*AVEKEY  DS    CL(L'INTKEY)        KEY OF ACCUMULATED DEMOS                     
*        DS    0F                                                               
...
*-------------------------------------------------------------------            
         ...
         DS    0F                                                               
DEMAREA  DS    ((NDEMS*3+NDEMOQ)*NMKTS)XL4  DEMOS                               
DEMLNQ   EQU   *-DEMAREA                                                        
*-------------------------------------------------------------------            
*                                                                               
*PACKWT   DS    (NDEMS*3)PL8    (DEMOS+UNIVS+PUTS) WGHTING FACTOR PCKED         
*                                                                               
HHQ      EQU    48*4             SLOTS FOR HH'S IN A DAY                        
QHQ      EQU    96*4             SLOTS FOR QH'S IN A DAY                        
DAYHHQ   EQU    7*HHQ            SLOTS FOR DAYS/HH'S IN A MARKET                
DAYQHQ   EQU    7*QHQ            SLOTS FOR DAYS/QH'S IN A MARKET                
NFEEDS   EQU    4                MAX NUMBER OF FEEDS SUPPORTED NOW              
HUTBYHH  DS     (NFEEDS)XL(1+NMKTS*DAYHHQ) FEED+DEMOS BY MARKET/DAY/HH          
HUTBYHHX DS     0X                                                              
HUTBYQH  DS     (NFEEDS)XL(1+NMKTS*DAYQHQ) FEED+DEMOS BY MARKET/DAY/QH          
HUTBYQHX DS     0X                                                              
*                                                                               
         EJECT                                                                  
MSU      EQU   8                   BIT MASK                                     
...
*NCBLS    EQU   26                  NUMBER OF CABLE NETS                        
NCBLS    EQU   100                 NUMBER OF CABLE NETS                         
         EJECT                                                                  
**********************************************************************          
...
*                                                                               
* OTHER DEMOS                                                                   
RICHUTS  EQU   0                   CABLE HUTS (BY COVERAGE AREA)                
NDEMOQ   EQU   1                   NUMBER OF OTHER DEMOS                        
*                                                                               
...
*********************************************************************           
*CVGTBL   DC    70XL4'0000'         4 CHAR COVERAGE SAMPLE SAVED                
...
HPTMFDEM DS    CL(NDEMS*4+NDEMOQ*4+8) M-F INTACCS DEMOS+COVG HOMES              
HPTMSWGT DS    XL4                 M-SU NUMBER RECDS ACCRUED                    
HPTMSDEM DS    CL(NDEMS*4+NDEMOQ*4+8) M-SU INTACCS DEMOS+COVG HOMES             
HPTDLN   EQU   *-HPTD                                                           
*                                                                               
HUTBYHQD DSECT                                                                  
HUTFEED  DS    C                                                                
...
*        DDCOMFACS                                                              
       ...
       ++INCLUDE DDCOMFACS                                                      
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'009DECAVCI   02/28/20'                                      
         END                                                                    
