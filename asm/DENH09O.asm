*          DATA SET DENH09O    AT LEVEL 004 AS OF 09/30/15                      
*PROCESS USING(WARN(15))                                                        
*PHASE DENH09OA                                                                 
*--------------------------------------------------------------------*          
* NHTI - HISPANIC SPANISH STATIONS TAPE (SPANDOM)                               
* IPHASE: DENH09I                                                               
* OPHASE: DENH09O                                                               
*--------------------------------------------------------------------*          
         TITLE 'DEMO CONVERSION - HISPANIC TV AUDIENCE OPHASE'                  
DENH09O  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 NADWRKX-NADWRKD,DENH09O,R7                                       
...
*                                                                               
CNV2     CLI   INTRTYP,PRCODEQU    USAGE RECDS  -P- (TIME PERIOD)               
         BE    CNV4                                                             
...
QCNV5    MVI   PMCODE,PMCODEQU                                                  
         ...
         MVC   PMSRC,INTKSRC                                                    
         MVC   KEYSRC,INTKSRC      SAVE FILE TYPE FOR HUT SUMMARIES             
         MVC   PMBOOK,INTBOOK                                                   
         MVC   PMSTAT,INTSTA                                                    
         MVC   PMSTYP,INTSTYP                                                   
         MVC   PMPNUM,INTPNUM                                                   
         ...
         MVC   PMBTYP,INTBTYP                                                   
         BRAS  RE,BLDLDKY          BUILD KEY FOR LOAD INDICATOR RECD            
*                                                                               
QCNV6    OC    PREVKEY,PREVKEY                                                  
         BZ    QCNV7                                                            
         GOTO1 APUTTAPE            PUT OUT PREV RECD                            
         CLC   PREVKEY(8),=C'PWNHUT H'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PW1HUT H'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PW7HUT H'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PNNUUUUH'   SAVE HUT UNIVERSE                      
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PN1UUUUH'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PN7UUUUH'                                          
         BE    QCNV6A                                                           
         CLC   PREVKEY(8),=C'PNNHUT H'                                          
         BE    QCNV6A0                                                          
         CLC   PREVKEY(8),=C'PN1HUT H'                                          
         BE    QCNV6A0                                                          
         CLC   PREVKEY(8),=C'PN7HUT H'                                          
         BNE   QCNV7                                                            
QCNV6A0  CLI   AVGWKS,4            FOR SPECIAL TIME PERD TAPE, SAVE             
...
*                                                                               
         ...
         BL    BLDEL1                                                           
         BRAS  RE,BLDDT            BUILD DATA TYPE ELEMENT                      
         GOTO1 APUTEL,TEMP                                                      
*                                                                               
...
*                                                                               
         ...
         CLC   =C'PAX',INTSTA      ALLOW OLD CALL LETTERS FOR 'ION'             
         BE    BLDEL3                                                           
         CLC   =C'TF ',INTSTA      ALLOW OLD CALL LETTERS FOR 'UMA'             
         BE    BLDEL3                                                           
         CLC   =C'MFX',INTSTA      ALLOW OLD CALL LETTERS FOR 'MMX'             
         BE    BLDEL3                                                           
*                                  PRG 1/2HRS AVAIL ONLY FOR BROADCAST          
         ICM   RF,15,ACOMFACS      CHECK IF NET IS BROADCAST HISPANIC           
         ICM   RF,15,CDEMTABS-COMFACSD(RF)                                      
         GOTO1 (RF),DMCB,NEHBRO    GET A(HISP BROADCAST STATIONS TABLE)         
         ICM   RE,15,0(R1)         A(TABLE) RETURNED IN P1                      
         BNZ   *+6                                                              
         DC    H'0'                BAD TABLEID PASSED                           
         ICM   RF,15,4(R1)         L(TABLE) RETURNED IN P2                      
*                                                                               
         USING NEHBRNMD,RE                                                      
BLDEL2   CLI   0(RE),X'FF'         PRG 1/2HR RECS FOR NETS IN THIS TBL          
         BE    BLDEL2G             GO CHECK FOR GENERAL MARKET NETS             
         CLC   NEHBRNET,INTSTA                                                  
         BE    BLDEL3              GOOD STATION                                 
         AR    RE,RF               NEXT ENTRY IN THE TABLE OF STATIONS          
         B     BLDEL2                                                           
         DROP  RE                                                               
*                                                                               
BLDEL2G  ICM   RF,15,ACOMFACS      CHECK IF NET IS BROADCAST                    
         ICM   RF,15,CDEMTABS-COMFACSD(RF)                                      
         GOTO1 (RF),DMCB,NEHGES    GET A(GENERAL MARKET NETS TABLE)             
         ICM   RE,15,0(R1)         A(TABLE) RETURNED IN P1                      
         BNZ   *+6                                                              
         DC    H'0'                BAD TABLEID PASSED                           
         ICM   RF,15,4(R1)         L(TABLE) RETURNED IN P2                      
*                                                                               
         USING NEHGENMD,RE                                                      
BLDEL2G5 CLI   0(RE),X'FF'                                                      
         BE    BLDEL25             NOT BROADCAST.                               
         CLC   NEHGENET,INTSTA                                                  
         BE    BLDEL3              IS BROADCAST. BUILD ELEMENTS                 
         AR    RE,RF                                                            
         B     BLDEL2G5                                                         
         DROP  RE                                                               
*                                                                               
BLDEL3   DS    0H                                                               
         ...
         MVI   NTCODE,NTCODEQU                                                  
         MVI   NTLEN,NTLENEQ                                                    
         CLC   INTBOOK,=AL2(JAN_07) AFTER JAN/07 ADD EXTENDED WKS FIELD         
         BL    *+14                                                             
         MVI   NTLEN,NTLENEQ2      EXTENDED ELEMENT LENGTH                      
         MVC   NTWKS2,INTWEEKS     EXTENDED WEEKS                               
         MVC   NTSQH,INTSQH                                                     
         MVC   NTEQH,INTEQH                                                     
...
BLDEL5   CLI   0(R5),0                                                          
         ...
         GOTO1 APUTEL,NTELEM                                                    
BLDEL10  LA    R5,7(R5)            NEXT DAY IN INTVAR                           
...
SVHUT    NTR1                                                                   
         ...
         CLC   PREVKEY(8),=C'PNNUUUUH'   GET HOMES UNIV FROM RECD               
         BE    SVHUT10                                                          
         CLC   PREVKEY(8),=C'PN1UUUUH'                                          
         BE    SVHUT10                                                          
         CLC   PREVKEY(8),=C'PN7UUUUH'                                          
...
*                                                                               
         ...
         CLC   PREVKEY(8),=C'PN7UUUUH'                                          
         BNE   *+14                                                             
SVHUT11  MVC   UNIVHUT,DUB+4       SAVE AWAY UNIV HUT                           
         B     SVHUTX                                                           
*                                                                               
*CALC HUT RATING                                                                
         OC    UNIVHUT,UNIVHUT                                                  
...
*        MVC   PNMEDIA,MEDIA                                                    
         ...
         MVC   PNSTAT,INTSTA                                                    
*        MVC   PNBTYP,INTBTYP                                                   
         MVC   PNBTYP,THISBOOK     7/27/01 CHANGED FROM INTBTYP                 
         MVC   PNBSIND,INTSTYP                                                  
         MVC   PNDW,PREVDW                                                      
...
CNV30    L     R1,ACOMWRK          ADDRESS OF COMMON WORK AREA                  
         ...
         OC    PREVKEY,PREVKEY                                                  
         BZ    CNV50                                                            
         CLC   PREVKEY(8),=C'PWNHUT H'  SAVE WEEKLY HUTS IN BUFFER              
         BE    CNV45                                                            
         CLC   PREVKEY(8),=C'PW1HUT H'                                          
         BE    CNV45                                                            
         CLC   PREVKEY(8),=C'PW7HUT H'                                          
         ...
         CLC   PREVKEY(8),=C'PN7HUT H'                                          
         BNE   CNV46                                                            
CNV40    CLI   AVGWKS,4            FOR SPECIAL TIME PERD TAPE, SAVE             
         BL    CNV46               4/5 WK AVG HUT DATA IN WKLY YY00 REC         
CNV45    BAS   RE,SVHUT                                                         
CNV46    GOTO1 APUTTAPE                                                         
*                                                                               
...
*                                 CREATE EXTENT 'LIVE'                          
         ...
         CLI   KEYSRC,C'7'                                                      
         BE    GETSUM1                                                          
         DC    H'0'               INVALID SOURCE                                
GETSUM1  ICM   RE,15,DBEXTEND     OLD START OF THE LIST                         
         MVC   DBEXTEND,=A(LIVEEXT)                                             
         ICM   RF,15,DBEXTEND     NEW START OF THE LIST                         
         STCM  RE,15,DBXLIVNX     LINK THEM                                     
         DROP  R4                                                               
*                                                                               
...
STDATE   DS    CL6                                                              
*                                                                               
LIVEEXT  DC    C'NLIV',AL4(0),X'00'   LIVE EXTENT. SEE DSECT DBXLIVD            
*                                     0=LIVE, 1=LIVE+SD, 7=LIVE+7               
*                                                                               
KEYSRC   DS    C                                                                
*                                                                               
OFORMAT  DS    0CL10                                                            
...
DAYS     EQU   (*-DAYTAB)/L'DAYTAB                                              
         SPACE 2                                                                
*                                                                               
LDRTAB   DS    10XL(L'PLKMAJOR)'00'  TABLE OF LOAD INDICATOR RECD KEYS          
         DS    50XL(L'PLKMAJOR)'00'  STARTUP TABLE FOR 2006                     
LDRTABX  DS    0X                                                               
*                                                                               
* EQUATES                                                                       
*                                                                               
...
BDTX     XIT1                                                                   
         ...
         EJECT                                                                  
********************************************************************            
* BUILD KEYS FOR THE LOAD INDICATOR RECORDS IN LDRTAB              *            
* DON'T ADD DUPLICATES                                             *            
********************************************************************            
BLDLDKY  NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         CLC   =C'UUUU',INTSTA     DON'T BUILD FOR UNIVERSES                    
         BE    BLDLDKYX                                                         
         CLC   =C'ITAB',INTSTA                     INTABS                       
         BE    BLDLDKYX                                                         
         CLC   =C'HUT ',INTSTA                     HUTS                         
         BE    BLDLDKYX                                                         
         CLI   INTKEY+1,C'W'       BUILD ONLY WEEKLY INDICATORS                 
         BNE   BLDLDKYX                                                         
         LA    R2,WORK                                                          
         USING PLKEY,R2                                                         
         XC    PLKMAJOR,PLKMAJOR                                                
         MVI   PLCODE,PLCODEQU                                                  
         MVC   PLMEDIA,INTKEY+1                                                 
         MVC   PLSRC,INTKSRC                                                    
         L     RE,=A(SVKSRC)                                                    
         MVC   0(L'SVKSRC,RE),INTKSRC                                           
         MVC   PLFILE,INTSTA+4                                                  
         MVC   PLBOOK,INTBOOK                                                   
...
BLDL10   CR    RE,RF                                                            
         ...
         DC    H'0'                                                             
         CLC   PLKMAJOR,0(RE)                                                   
         BE    BLDLDKYX            KEY ALREADY IN TABLE                         
         OC    0(L'PLKMAJOR,RE),0(RE)                                           
         BZ    BLDL20                                                           
         LA    RE,L'PLKMAJOR(RE)                                                
         B     BLDL10                                                           
*                                                                               
BLDL20   MVC   0(L'PLKMAJOR,RE),PLKMAJOR                                        
         DROP  R2                                                               
*                                                                               
BLDLDKYX J     XIT                                                              
         LTORG                                                                  
         EJECT                                                                  
**********************************************************************          
* RELEASE LOAD INDICATOR RECORDS                                                
* KEYS ARE SAVED IN LDRTAB                                                      
* ********************************************************************          
RELLDR   NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         BRAS  RE,BLD2006         BUILD STARTUP KEYS FOR 2006                   
*                                                                               
         L     R2,=A(LDRTAB)                                                    
         L     R3,=A(LDRTABX)                                                   
RELL10   CR    R2,R3                                                            
         BNL   RELLDRX                                                          
         OC    0(L'LDRTAB,R2),0(R2)                                             
         BZ    RELLDRX                                                          
*                                                                               
         L     R4,=A(THISKEY)                                                   
         XC    0(L'THISKEY,R4),0(R4)                                            
         MVC   0(L'PLKMAJOR,R4),0(R2)                                           
         GOTO1 ABLDREC,DMCB,(C'P',(R4))                                         
         GOTO1 APUTTAPE            PUT LOAD INDICATOR RECORD TO TAPE            
*                                                                               
         LA    R2,L'LDRTAB(R2)                                                  
         B     RELL10                                                           
*                                                                               
RELLDRX  J     XIT                                                              
         ...
         EJECT                                                                  
********************************************************************            
* BUILD STARTUP KEYS FOR 2006                                      *            
********************************************************************            
BLD2006  NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         LHI   R1,48               WEEKS 1 THRU 48                              
BLD205   LA    R2,WORK                                                          
         USING PLKEY,R2                                                         
         XC    PLKMAJOR,PLKMAJOR                                                
         MVI   PLCODE,PLCODEQU                                                  
         MVI   PLMEDIA,C'W'                                                     
         L     RE,=A(SVKSRC)                                                    
         MVC   PLSRC,0(RE)         SAVED INTERNAL SOURCE                        
         MVI   PLFILE,C'H'                                                      
         MVI   PLBOOK,X'6A'        2006                                         
         STC   R1,PLBOOK+1                                                      
         MVI   PLORIG,PLOFINLQ     FINAL FILE                                   
*                                                                               
         L     RE,=A(LDRTAB)                                                    
...
BLD210   CR    RE,RF                                                            
         ...
         DC    H'0'                                                             
         CLC   PLKMAJOR,0(RE)                                                   
         BE    BLD230              KEY ALREADY IN TABLE                         
         OC    0(L'PLKMAJOR,RE),0(RE)                                           
         BZ    BLD220                                                           
         LA    RE,L'PLKMAJOR(RE)                                                
         B     BLD210                                                           
*                                                                               
BLD220   MVC   0(L'PLKMAJOR,RE),PLKMAJOR                                        
BLD230   BCT   R1,BLD205                                                        
         DROP  R2                                                               
*                                                                               
BLD2DKYX J     XIT                                                              
         LTORG                                                                  
...
* DEDEMTABD                                                                     
       ...
       ++INCLUDE DDMONYREQU                                                     
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'004DENH09O   09/30/15'                                      
         END                                                                    
