*          DATA SET DEMRGUXIT4 AT LEVEL 006 AS OF 10/20/15                      
...
* ************************** NOTE *********************************** *         
*                                                                     *         
* THIS EXIT *PURGES* ALL DUPLICATE MAJOR/MINOR KEYS FOUND ON THE      *         
* DEMOFILE. IT ONLY KEEPS THE FIRST OCCURRENCE FOR A GIVEN KEY. THE   *         
* EXIT IS INVOKED VIA A JOINKEYS OPERATOR, WHICH DOES A FULL OUTER    *         
* JOIN (I.E., A CARTESIAN JOIN). IF WE DON'T PURGE THE DUPLICATES     *         
* IN THIS EXIT, WE'LL END UP WITH TOO MANY RECORDS AFTER THE JOIN.    *         
*                                                                     *         
* NOTE THAT THE DISCARDED DUPLICATES ARE WRITTEN TO A DATASET         *         
* (DDNAME DUPFILE). THIS IS REALLY FOR DEBUGGING ONLY. IN PRODUCTION, *         
* DUPFILE CAN TYPICALLY BE ASSIGNED TO DUMMY.                         *         
*                                                                     *         
***********************************************************************         
DEMRGU4  CSECT                                                                  
         PRINT NOGEN                                                            
...
CHKEOFMN DS    0H                                                               
         ...
         DC    H'0'                                                             
*                                                                               
         CLC   LASTKEY,R.DMAJKEY   THIS KEY IS OUT OF SEQUENCE ON FILE?         
         BNH   *+6                                                              
         DC    H'0'                YES: CORRUPTED FILE !?!?!                    
         BNE   KEEPIT              NEW KEY IS HIGHER: THAT'S FINE               
*                                                                               
         SR    R0,R0               KEYS ARE EQUAL: THIS IS A DUPLICATE          
         ICM   R0,3,R.DRRECLEN     DEMO RECORD LENGTH...                        
         AHI   R0,L'OUT_RDW        ...PLUS SORT RECORD OVERHEAD...              
         STCM  R0,3,OUT_RDW        ...EQUALS NEW RECORD LENGTH                  
         PUT   DUPFILE,OUT_RDW                                                  
         B     SEQ                 ONLY KEEP THE FIRST DUPLICATE !!!            
*                                                                               
KEEPIT   DS    0H                                                               
         MVC   LASTKEY,R.DMAJKEY   SAVE THE CURRENT KEY                         
*                                                                               
...
DMFLIST  DC    0D                  OPEN ALL DEMO FILES AND DIRECTORIES          
         ...
         DC    C'X'                                                             
         SPACE 2                                                                
         ORG   DEMRGU4+(((*-DEMRGU4)/256)+1)*256  FOR I-CACHE PIPELINE          
         SPACE 2                                                                
         DS    0L                                                               
...
*                                                                               
         DS    0D                                                               
         DC    C'**KEYS**'         EYE-CATCHER                                  
KEY      DS    CL(DKEYLENQ)        THE CURRENT MAJOR/MINOR KEY                  
LASTKEY  DS    CL(DFULLKYQ)        THE PREVIOUS MAJOR/MINOR KEY                 
*                                                                               
...
*                                                                               
DUPFILE  DCB   DDNAME=DUPFILE,DSORG=PS,RECFM=VB,LRECL=2000,            +        
