*          DATA SET DENH09I    AT LEVEL 007 AS OF 09/30/15                      
*PROCESS USING(WARN(15))                                                        
*PHASE DENH09IA                                                                 
*--------------------------------------------------------------------*          
* NTIH HISPANIC TV AUDIENCE CONVERSION (SPANISH STATIONS)                       
* SUPPORTING PERSONAL LANGUAGE DATA STARTING FEB/2009                           
* IPHASE: DENH09I                                                               
* OPHASE: DENH09O                                                               
*--------------------------------------------------------------------*          
         TITLE 'DEMO CONVERSION - NTIH TV AUDIENCE INPUT PHASE'                 
DENH09I  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,DENH09I,RA,R3,R4                                               
...
RDTEST   GET   IN1,(RC)            GET NEXT RECORD                              
         ...
         BNE   RDTEST                                                           
*                                                                               
READ30   CLI   BYPREAD,X'FF'       1ST-TIME-THRU? (SHOULD BE RDR)               
         BNE   READ40                                                           
...
*                                                                               
         ...
         BE    READ41B                                                          
         MVC   SAVESTA,MITTYPE                                                  
         CLC   MITSEQ,=C'04'       PRINT OUT STATION CALL LETTERS               
         BNE   READ41B                                                          
         L     R9,VCPRINT                                                       
         USING DPRINT,R9                                                        
         MVC   P,SPACES                                                         
         MVC   P(L'MITTYPE),MITTYPE   STATION CALL LETTERS                      
         GOTO1 VPRINTER                                                         
*                                                                               
READ41B  CLC   SAVEPNUM,MITPRG     WHEN PRG# CHANGES-RESET VARIABLES            
         BNE   *+14                SAME                                         
...
RDR      DS    0H                                                               
         ...
         MVC   TAPEMKT,=H'512'     HPM                                          
         MVI   WGTFLAG,0                                                        
         CLC   =C'NTIH AUDIENCE ESTIMATES',RDRRPT                               
         BE    RDR5                                                             
         CLC   =C'NTIH NAD REPORT',RDRRPT                                       
         BE    RDR5                                                             
         CLC   =C'WGT NHTI AUDIENCE ESTIMAT',RDRRPT                             
         BNE   RDR2                                                             
         CLI   BOOKTYPE,C'E'                                                    
         BE    *+6                                                              
         DC    H'0'                                                             
...
*                                                                               
         CLC   TAPEBK,=AL1(109,2)  USE THIS CONVERSION FOR DATA AFTER           
         BNL   *+6                 FEB/09. USE DENH07I BEFORE FEB/09            
         DC    H'0'                                                             
*                                                                               
...
*                                                                               
         ...
         MVC   INTMRKT,TAPEMKT     SET NTIH MKT=512 OR NHTAFF=513               
         BRAS  RE,GENSTA                                                        
         BNE   *+10                                                             
         MVC   INTMRKT,=H'513'     SET MKT=HPMGMN FOR GEN MRKT STATIONS         
*                                                                               
...
D4R34A   DS    0H                                                               
         ...
         MVC   HALF+1(1),8(R1)     NETWORK WEEK                                 
         MVC   INTBOOK,HALF        BOOK SAVED AS YEAR & NETWORK WEEK            
         MVC   INTIBOOK,TAPEBK     BOOK SAVED AS YEAR & NETWORK WEEK            
         MVI   WKSFLG,C'W'         SET MEDIA IN KEY FOR WEEKLY                  
*                                                                               
...
*                                                                               
         ...
         MVC   WKSFLG,SVWKSFLG     RESTORE FROM H4 RECD                         
...
*                                                                               
         LA    RF,INTACCS                                                       
         AH    RF,SLOTLN           NEXT FREE SLOT AFTER DEMOS                   
         CLI   RELSLOT,0                                                        
         BNE   RELPRG25                                                         
         LA    RE,PUTS             POINT TO PUT BUFFER                          
         LH    R1,SLOTLN                                                        
         MOVE  ((RF),(R1)),(RE)    MOVE IN PUTS  (AFTER DEMOS)                  
*                                                                               
...
RELSUM   ZIC   RE,SUMSLOT                                                       
         ...
         MH    RE,=H'16'                                                        
         A     RE,=A(PTYPTAB)                                                   
         CLC   TAPEMKT,=H'513'     HPMGMN: NTHI AFFILIATE NETS?                 
...
*                                                                               
         LA    RF,INTACCS                                                       
         AH    RF,SLOTLN           NEXT FREE SLOT AFTER DEMOS FOR UNIVS         
         CLI   RELSLOT,0                                                        
         BNE   RELSUM6                                                          
...
H5R47    MVC   INTSTA(3),NEHGAGNT  MOVE IN ALPHA AGGREGATE CODE                 
         MVC   INTMRKT,=H'513'     HPMGMN                                       
         DROP  RE                                                               
*                                                                               
H5R50    CLC   INTMRKT,=H'512'     REG NHTI TAPE: HPM                           
         BNE   H5R52                                                            
         CLI   TAPETYP,C'A'        NHTI-AFF TAPE: HPMGMN                        
         BNE   H5R52                                                            
         MVC   INTMRKT,=H'513'                                                  
...
*        MVI   INTSTA+4,C'h'                                                    
*                                                                               
CNV05    MVI   BYPSORT,X'80'       SET NO RELEASE OF SORT REC                   
...
* SLOT OUTPUT DEMOS IN INTACCS                                                  
         BRAS  RE,OUTSLOT          SLOT OUTPUT INTO RIGHT BUCKETS               
         LA    RE,INTACCS          DESTINATION                                  
*                                                                               
         CLC   INTSTA(5),=C'UUUUH'                                              
         BE    CNV35                                                            
         LA    R1,NDEMS*4                                                       
CNV33    LA    RF,255              CHECK TO SEE IF DEMOS & PUTS                 
CNV33A   SHI   RF,1                ARE FILLED IN.                               
         EX    RF,*+8                                                           
         B     *+10                                                             
         OC    0(0,RE),0(RE)                                                    
         BNZ   CNV35                                                            
         ...
         B     CNV33A                                                           
*                                                                               
CNV34    LA    R1,INTACCS+PUTDSPQ                                               
         CR    RE,R1                                                            
         BH    CNV40                                                            
         LA    RE,INTACCS+PUTDSPQ                                               
         LA    R1,NDEMS*4                                                       
         B     CNV33                                                            
*                                                                               
...
CNV40    DS    0H                                                               
         ...
         LA    RF,NDEMS*2*4                                                     
         XCEF                                                                   
         L     RE,=A(SAVEREC)       RE PTS TO CURRENT KEY'S DEMOS               
*                                                                               
CNV70    DS    0H                                                               
...
*                                                                               
SVWKSFLG DS    X'00'               WKS FLAG ON H4 RECD                          
         EJECT                                                                  
...
*                                                                               
PROCDEM2 CLC   =H'1',DMCB+2        FOR FIRST GROUP IN THE SEQUENCE,             
         BE    *+14                                                             
         CLC   =H'501',DMCB+2                                                   
         BNE   PROCDM2                                                          
         ...
         XCEF                                                                   
PROCDM2  ST    R7,DMCB+8           SAVE START ADDR WHERE TO STORE DEMS          
         LA    R0,20                                                            
         CLI   MITPUT,C'1'         PUT?                                         
         BNE   PROCDM2A                                                         
         OC    SVPUT,ZEROS                                                      
...
PROCDX   XIT1                                                                   
         ...
         EJECT                                                                  
***********************************************************************         
* CHECK IF STATION IN INTSTA IS A GENERAL MARKET STATION                        
*        ON ENTRY, R2 -> INTREC                                                 
*        ON EXIT, CC EQ: STATION IS GENERAL MARKET                              
*                 CC NEQ: STATION ISN'T GENERAL MARKET                          
***********************************************************************         
GENSTA   NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         ICM   RF,15,ACOMFACS                                                   
         ICM   RF,15,CDEMTABS-COMFACSD(RF)                                      
         GOTO1 (RF),DMCB,NEHGES    GET A(GENERAL MARKET STATIONS TABLE)         
         ...
         ICM   RF,15,4(R1)         L'TABLE ENTRY RETURNED IN P2                 
         USING NEHGENMD,RE                                                      
GENST10  CLI   0(RE),X'FF'                                                      
         BE    GENSTAN             NOT AN ENGLISH STATION                       
         CLC   INTSTA(3),NEHGENET                                               
         BE    GENSTAY                                                          
         AR    RE,RF               TRY NEXT ENTRY IN THE STATIONS TABLE         
         B     GENST10                                                          
*                                                                               
...
* 160 BECAUSE THE DUMMY "U" MODIFIER IS BETWEEN IMPS AND RAW UNIVERSE           
PUTDSPQ  EQU   275*4               DISP TO RAW ('Z') PUTS                       
*LIMPEQ  EQU   210*4               DISP TO PERSONAL LANGUAGE DEMOS              
*LUNVEQ  EQU   270*4               DISP TO PERSONAL LANGUAGE UNIVERSE           
*LPUTEQ  EQU   330*4               DISP TO PERSONAL LANGUAGE PUTS               
         SPACE 1                                                                
...
*        DDCOMFACS                                                              
       ++INCLUDE DDCOMFACS                                                      
         ...
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'007DENH09I   09/30/15'                                      
         END                                                                    
