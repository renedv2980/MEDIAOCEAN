*          DATA SET MPRDR0AR   AT LEVEL 026 AS OF 05/01/02                      
*PHASE T5100A,*                                                                 
...
VK8D     DS    0H                                                               
         ...
         BE    *+10                                                             
         MVC   LEVLIM,CONREC       T,Q,A FROM RECORD TYPE                       
*                                                                               
...
VK10     DS    0H                                                               
         ...
         MVI   USEIO,C'Y'         DONT LET GENCON RE-READ QSPECS                
...
*                                                                               
         ...
         MVC   LISTHU(40),WORK                                                  
VKCPY7   DS    0H                                                               
         LA    R2,QCPQSOH          NEW CODE                                     
         GOTO1 ANY                                                              
*                                                                               
VKCPY8   DS    0H                                                               
         XC    NDHOOK,NDHOOK       NO HOOK                                      
         GOTO1 NODIO,DMCB,VNODBLK,(RPBA,RPCMD),WORK,(8,RPPOS),LISTHU            
*                                                                               
         CLI   NDERR,0                                                          
         BE    VKCPY10                                                          
         GOTO1 NODERRP             HANDLE NODIO ERROR                           
...
*        VALIDATE RECORD                                                        
         SPACE 3                                                                
VREC     DS    0H                                                               
         CLI   TWAOFFC,C'*'        MUST BE DDS OFFICE                           
         BE    *+16                                                             
         MVI   ERROR,ACTNSERR                                                   
         LA    R2,CONACTH                                                       
         B     TRAPERR                                                          
...
*                                                                               
VOPT43   DS    0H                                                               
         CLI   WCTDATE,0           IF  COUNT DATE SET                           
         BNE   VOPT49              OK                                           
         OC    WCTDATE,WAVDATE     ELSE SET TO WAVE DATE                        
         BNZ   VOPT49              IF THERE                                     
*                                  ELSE NEED TO SET MPQBLK                      
         MVI   OPTION2,C'V'        SET VALIDATE ONLY                            
         GOTO1 VALWAVE                                                          
*                                                                               
VOPT49   DS    0H                                                               
         CLI   MODE,VALREC         IF VALIDATING                                
         BNE   VOPT49D                                                          
         CLC   WCTDATE,WAVDATE     COUNT DATE MUST =                            
         BE    VOPT49D             WAVE DATE                                    
*                                                                               
         MVI   ERROR,COUNTERR                                                   
         B     TRAPERR                                                          
*                                                                               
...
LR5      DS    0H                                                               
         ...
         MVI   KEY,X'FF'                                                        
         CLI   LEVLIM,C' '         IF NO LEVEL LIMIT                            
         BNE   LR5C                                                             
         MVI   NDSQBACK,0          SET TO READ BACK THRU HIGHER LEVS            
         B     LR5F                (NOTE-NDSQBACK SET BY NODIO)                 
*                                                                               
LR5C     DS    0H                                                               
         CLC   SQTYP,LEVLIM        IF TYPE READ = LIMIT                         
         BE    LR5D                                                             
         CLI   LIBLST,C'Y'         OR IF LIB HEADER LISTING                     
         BNE   LR5F                                                             
LR5D     DS    0H                                                               
         ZIC   RF,NDSQBACK         BACK UP ONE EXTRA LEVEL                      
         BCTR  RF,R0                                                            
         STC   RF,NDSQBACK                                                      
*                                                                               
LR5F     DS    0H                                                               
         MVC   LCBACK,NDSQBACK     AND BACK UP CONTROL                          
*                                                                               
LR6      DS    0H                                                               
         LA    R3,=C'SEQ'                                                       
...
*                                **NODES AND DISK ADDRESS**                     
         ...
         XC    FULL,FULL                                                        
         L     R6,NDIOA                                                         
         MVI   ELCODE,X'B1'        FIND NODE ESTABLISH ELEM                     
         BAS   RE,GETEL                                                         
         BNE   *+10                                                             
         MVC   FULL,2(R6)                                                       
         GOTO1 HEXOUT,DMCB,FULL+1,WORK,3,=C'N'                                  
...
LSPEC    DS    CL29                                                             
         SPACE 3                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'026MPRDR0AR  05/01/02'                                      
         END                                                                    
