*          DATA SET REREP7402  AT LEVEL 129 AS OF 05/01/02                      
*PHASE RE7402C,+0                                                               
*INCLUDE CLPACK                                                                 
*INCLUDE MEDGET                                                                 
***********************************************************************         
...
*                                                                     *         
* FEB10/97 (DBU) --- ADD AGENCY/FLIGHT DATES WHEN REQUESTED WITH      *         
*                    DETAIL                                           *         
*                                                                   * *         
* JAN28/98 (JRD) --- 4K CONTRACTS                                     *         
*                                                                     *         
* APR03/00 (BU ) --- DON'T ABORT ON NO-FIND AGENCY CODE               *         
*                                                                     *         
*                                                                     *         
*                        ** END TOMBSTONE **                          *         
***********************************************************************         
...
*                                                                               
DET30    DS    0H                                                               
         LA    R6,RPRDREC          AGENCY/FLIGHT DATE VALIDATION ELEM           
         USING RPRDAGFL,R6                                                      
         MVI   ELCODE,4                                                         
         BAS   RE,GETEL                                                         
         BNE   DET38                                                            
*                                                                               
         MVC   REPCODE(L'REPCODE),RPRDKREP      SAVE REP CODE                   
         OC    RPRDAGAG,RPRDAGAG   ANY AGENCY?                                  
         BZ    *+10                                                             
         MVC   AGYCODE(L'AGYCODE),RPRDAGAG                                      
*                                                                               
         OC    RPRDAGAO,RPRDAGAO   ANY OFFICE?                                  
         BZ    *+10                                                             
         MVC   OFFCODE(L'OFFCODE),RPRDAGAO                                      
*                                                                               
         OC    RPRDAGDF,RPRDAGDF   START DATE?                                  
         BZ    DET33                                                            
         GOTO1 DATCON,DMCB,(3,RPRDAGDF),(11,STDATE)                             
*                                                                               
DET33    EQU   *                                                                
         OC    RPRDAGDT,RPRDAGDT   END DATE?                                    
         BZ    DET38                                                            
         GOTO1 DATCON,DMCB,(3,RPRDAGDT),(11,ENDATE)                             
*                                                                               
DET38    EQU   *                                                                
         DROP  R6                                                               
*                                                                               
         XC    RCTGKEY,RCTGKEY     GET CATEGORY CODE AND NAME                   
         MVI   RCTGKTYP,X'0F'                                                   
...
DPRINT2  DS    0H                  PRINTS RECORD IN TWO COLUMNS                 
         ...
         GOTO1 REPORT                                                           
         MVC   P+2(6),=C'AGENCY'                                                
         MVC   P+21(L'AGYCODE),AGYCODE                                          
         MVI   P+25,C'-'                                                        
         MVC   P+26(L'OFFCODE),OFFCODE                                          
         MVC   P+70(12),=C'FLIGHT DATES'                                        
         MVC   P+83(L'STDATE),STDATE                                            
         MVI   P+91,C'-'                                                        
         MVC   P+92(L'ENDATE),ENDATE                                            
*                                                                               
         OC    AGYCODE,AGYCODE                                                  
         BZ    DPRINT10                                                         
         XC    RAGYKEY,RAGYKEY     GET AGENCY NAME                              
         MVI   RAGYKEY,X'0A'                                                    
         MVC   RAGYKAGY(L'RAGYKAGY),AGYCODE                                     
         MVC   RAGYKAOF(L'RAGYKAOF),OFFCODE                                     
         OC    RAGYKAOF(L'RAGYKAOF),SPACES                                      
         MVC   RAGYKREP(L'RAGYKREP),REPCODE                                     
         MVC   KEYSAVE(L'RAGYKEY),RAGYKEY                                       
*                                                                               
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,RAGYKEY,RAGYKEY,0                     
         TM    DMCB+8,X'FD'                                                     
         BZ    *+6                                                              
         DC    H'0'                                                             
         CLC   RAGYKEY(L'RAGYKEY),KEYSAVE                                       
         BE    DPRINT6                                                          
***      DC    H'0'                                                             
         MVC   P+30(6),KEYSAVE+19                                               
         MVC   P+37(13),=C'NOT ON FILE  '                                       
         B     DPRINT08                                                         
DPRINT6  EQU   *                                                                
*                                                                               
         GOTO1 (RF),(R1),GETREC,REPFILE,RAGYKEY+28,RAGYREC,DMWORK               
         TM    DMCB+8,X'FD'                                                     
         BZ    *+6                                                              
         DC    H'0'                                                             
         LA    R6,RAGYREC          NETWORK CONTRACT ELEMENT                     
         USING RAGYELEM,R6                                                      
         MVI   ELCODE,1                                                         
         BAS   RE,GETEL                                                         
         BNE   DPRINT08                                                         
         MVC   P+30(L'RAGYNAM1),RAGYNAM1    AGENCY NAME                         
         DROP  R6                                                               
DPRINT08 EQU   *                                                                
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,KEY,KEY,0                             
DPRINT10 EQU   *                                                                
...
AGYCODE  DS    CL4                                                              
OFFCODE  DS    CL2                                                              
STDATE   DS    CL8                 START FLIGHT DATE                            
ENDATE   DS    CL8                 END FLIGHT DATE                              
...
SPESTD   DSECT                                                                  
       ++INCLUDE SPGENEST                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'129REREP7402 05/01/02'                                      
         END                                                                    
