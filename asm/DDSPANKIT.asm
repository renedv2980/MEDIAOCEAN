*          DATA SET DDSPANKIT  AT LEVEL 045 AS OF 05/15/19                      
*PHASE SPANKITA                                                                 
*INCLUDE REGSAVE                                                                
*INCLUDE CARDS                                                                  
*INCLUDE DMDMGRL                                                                
...
*INCLUDE PRTREC                                                                 
*INCLUDE QSORT                                                                  
SPANKIT  TITLE 'AGENCY RE-DISTRIBUTION PROGRAM FOR ITG FILES ONLY'              
SPANKIT  CSECT                                                                  
         PRINT NOGEN                                                            
...
SP6      MVC   P(20),=C'INVALID CONTROL CARD'                                   
         GOTOR VPRINTER                                                         
...
SP12     CLI   AGYTABD,X'FF'       TEST E-O-T                                   
         ...
         AHI   R5,AGYLEN           BUMP TO NEXT AGENCY                          
         B     SP12                                                             
...
SP52     DS    0H                                                               
**NOP    GOTOR UPDCTF              UPDATE CONTROL FILE ID RECORDS               
...
VALTYP02 CLI   SYSTABD,X'FF'       TEST E-O-T                                   
         ...
         B     VALTYP02                                                         
*                                                                               
VALTYP03 ST    R4,ASYSNTRY         SAVE A(SYSTAB ENTRY)                         
...
VALTYP22 CLI   SETABD,X'FF'        FIND CURRENT SETAB ENTRY FOR AGENCY          
         ...
         MVC   P(10),IO+15         PRINT OUT ID AND AGYOLDA                     
         MVC   P+12(2),AGYOLDA                                                  
         MVC   P+16(7),SENAME                                                   
...
VALAGY   NTR1  ,                                                                
         L     R3,=A(AGY2CPY)                                                   
         USING A2CD,R3                                                          
VALAGY10 CLI   0(R3),X'FF'    End of table                                      
         JE    *+2            Table full                                        
         CLI   AGYORG,0       Free entry?                                       
         BE    VALAGY20                                                         
         AHI   R3,A2CLNQ                                                        
         B     VALAGY10                                                         
*                                                                               
VALAGY20 MVC   AGYORG(A2CLNQ),CARD+7                                            
         MVI   VALSW,C'Y'                                                       
         B     VALPARMX                                                         
         DROP  R3                                                               
*                                                                               
VALDSERR MVC   P(37),=C'FBSA - DSPACE= CARD NEEDS TO BE FIRST'                  
         GOTO1 VPRINTER                                                         
         GOTOR VPRINT,DMCB,=C'CLOSE'                                            
         DC    H'0'                                                             
         EJECT                                                                  
...
VALOUT02 CLI   SENAME,X'FF'        FIND SETAB ENTRY FOR OUTPUT FILE             
         ...
         B     VALOUT02                                                         
*                                                                               
VALOUT03 OI    SEINDS,X'40'        SET OUTPUT FILE REQUIRED                     
...
***********************************************************************         
* Find old agency in table and set AGYNEWA (This is alpah value)                
***********************************************************************         
SETAGY   NTR1  ,                                                                
         L     R3,=A(AGY2CPY)      Agency to copy                               
         USING A2CD,R3                                                          
SETAGY10 CLI   0(R3),0             EOL (end of list)                            
         BE    SETAGYX                                                          
         CLI   0(R3),X'FF'         EOT (end of table)                           
         BE    SETAGYX                                                          
         CLC   AGYORG,AGYOLDA      Find matching alpha                          
         BE    SETAGY20                                                         
         AHI   R3,A2CLNQ                                                        
         B     SETAGY10                                                         
*                                                                               
SETAGY20 MVC   AGYNEWA,AGYCPY      Agency copy alpha                            
...
PRCREC14 CLC   OUTFILE,SEFILNUM    FIND SE TABLE ENTRY                          
         ...
         DC    H'0'                                                             
*                                                                               
         LA    RF,RECLEN(,R3)      TEST REPEATED KEY                            
         CLC   RECKEYL,RECKEYL-RECDEFD(RF)                                      
         BNE   PRCREC15                                                         
         CLC   RECKEYH,RECKEYH-RECDEFD(RF)                                      
         BNE   PRCREC15                                                         
         AHI   R3,RECLEN                                                        
         MVI   DEFINED,C'Y'                                                     
         B     PRCREC02                                                         
*                                                                               
PRCREC15 ICM   R4,7,SEADTF                                                      
         L     R6,AREC                                                          
         PUT   (R4),(R6)                                                        
...
PRCREC20 LA    RF,RECLEN(,R3)      TEST REPEATED KEY                            
         CLC   RECKEYL,RECKEYL-RECDEFD(RF)                                      
         BNE   PRCREC21                                                         
         CLC   RECKEYH,RECKEYH-RECDEFD(RF)                                      
         BNE   PRCREC21                                                         
...
PRCREC21 ICM   R4,7,SEADTF         WRITE RECORD TO FILE                         
         L     R6,AREC                                                          
         PUT   (R4),(R6)                                                        
         AP    SEOUTCNT,=P'1'      BUMP OUTPUT COUNT                            
...
CHKOPEN  NTR1                                                                   
         ...
         JE    EXIT                                                             
         MVI   OPENSW,C'Y'                                                      
         GOTOR VDATAMGR,DMCB,=C'DMOPEN',=C'CONTROL',=C'YCTFILE X'               
         J     EXIT                                                             
         LTORG                                                                  
...
* R2 Agency entry                                                               
* R3 value of DUB                                                               
* R4 key entry from KEYTAB                                                      
* R5                                                                            
***********************************************************************         
AGYCNV   NTR1  ,                                                                
         LLC   RF,DUB              Get KEY index format                         
         SHI   RF,1                                                             
         BNM   *+6                                                              
         DC    H'00'                                                            
         ...
         MHI   RF,L'KEYTAB                                                      
         LARL  R4,KEYTAB                                                        
         AR    R4,RF               Index into KEYTAB                            
                                                                                
         USING KEYTABD,R4          R4=A(KEY FORMAT TABLE ENTRY)                 
         L     R3,DUB                                                           
         LLC   R1,KEYCOMPL         R1=L'EXECUTE                                 
         LLC   RE,KEYODISP         RE=DISP TO OLD VALUE                         
         LLC   RF,KEYNDISP         RF=DISP TO NEW VALUE                         
         MVC   WORK(2),0(R3)       SAVE KEY VALUE                               
         L     R6,ASENTRY                                                       
         USING SETABD,R6           R6=A(SE TABLE ENTRY)                         
         CLI   KEYAND,0            Any key mask                                 
         BE    AGYCNV02            No                                           
         NC    0(1,R3),KEYAND      Turn off agency in record                    
         PACK  WORK+2(1),KEYAND    Flip mask around of nibble                   
         NC    WORK(1),WORK+2      TURN OFF OTHER NIBBLE IN SAVE                
*                                  X'0F' -> X'F0' and visa versa                
...
AGYCNV08 AHI   R2,AGYLEN           Next company in table                        
         B     AGYCNV06                                                         
*                                                                               
AGYCNV09 MVC   AGYLAST,0(R2)       SET LAST TIME AGENCY VALUES                  
         MVC   OLDAGY,AGYOLDA                                                   
         MVC   NEWAGY,AGYNEWA                                                   
                                                                                
AGYCNV10 LA    R5,AGYLAST(RF)                                                   
         LA    RE,AGYOC            'MOVE' OR 'OR' IN NEW KEY VALUE              
         CLI   KEYAND,0                                                         
         BNE   AGYCNV12                                                         
                                                                                
         LA    RE,AGYMVC                                                        
         CLC   AGYNEWA,SPACES      New agency?                                  
         BNH   AGYCNV12            No                                           
         CLC   0(2,R3),AGYOLDA     OLD Agency                                   
         BNE   AGYCNV12                                                         
         LA    R5,AGYNEWA          NEW Agency                                   
                                                                                
AGYCNV12 EX    R1,0(RE)                                                         
         B     AGYCONVX                                                         
                                                                                
AGYMVC   MVC   0(0,R3),0(R5)                                                    
AGYOC    OC    0(0,R3),0(R5)       Combine agency/media                         
...
PRTTOT04 EDIT  (P6,0(R3)),(8,0(R4)),FILL=0                                      
         AHI   R3,L'SEINPCNT                                                    
         LA    R4,10(R4)                                                        
         BCT   R5,PRTTOT04                                                      
         GOTOR VPRINTER                                                         
...
SPTAGY   NTR1  ,                                                                
         ...
         AHI   R4,24               Point to first element                       
...
SPTAGY06 CLC   90(2,R4),OLDAGY     OLD ALPHA                                    
         BNE   *+10                                                             
         MVC   90(2,R4),NEWAGY     FIX ALPHA                                    
                                                                                
         MVC   92(1,R4),WORK       FIX HEX AGY (AGYPROF+19)                     
         B     SPTAGY04                                                         
                                                                                
SPTAGY08 LA    RE,3(R4)            FIX AGENCY IN MEDIA ELEMENTS                 
         ST    RE,DUB                                                           
         MVI   DUB,AGYL                                                         
         GOTOR AGYCNV                                                           
         B     SPTAGY04                                                         
...
***********************************************************************         
* SPECIAL CODE FOR NET UNIT RECORDS                                             
***********************************************************************         
NETUNIT  NTR1  ,                                                                
                                                                                
         L     R3,AAGYTAB          R5=A(AGENCY TABLE)                           
         USING AGYTABD,R3                                                       
NUN01    CLI   AGYTABD,X'FF'       TEST E-O-T                                   
         BE    NUNX                                                             
         CLC   AGYOLDA,OLDAGY                                                   
         BE    NUN02                                                            
         AHI   R3,AGYLEN           BUMP TO NEXT AGENCY                          
         B     NUN01                                                            
*                                  OPEN ALL OUTPUT FILES & SET A(DTF)           
         USING NURECD,R4                                                        
NUN02    MVC   DUB(1),NUKAM                                                     
         NI    DUB,X'F0'                                                        
         CLC   DUB(1),AGYOLDL                                                   
         BNE   NUNX                                                             
*                                                                               
         LA    R5,NUDATA                                                        
NUN03    CLI   0(R5),0             TEST E-O-R                                   
         BE    NUNX                                                             
         CLI   0(R5),NUMAINEQ      MAIN DATA ELEMENT                            
         BE    NUN06                                                            
NUN04    LLC   R0,1(R5)            BUMP TO NEXT ELEMENT                         
         AR    R5,R0                                                            
         B     NUN03                                                            
*                                                                               
         USING NUMAINEL,R5                                                      
NUN06    CLC   NUALPHA,AGYOLDA     FIX AGENCY ALPHA CODE                        
         BNE   NUN04                                                            
         MVC   NUALPHA,AGYNEWA                                                  
         B     NUN04                                                            
*                                                                               
...
OUTXAI   EQU   X'80'               . EXTRA ALPHA ID (WITH ZERO CODE)            
*                                                                               
AAGYTAB  DC    A(AGYTAB)                                                        
ASETAB   DC    A(SETAB)                                                         
ASENTRY  DC    A(0)                                                             
...
ALPHATAB DC    C'ABCDEFGHIJKLMNOPQRMTUVWXYZ0123456789',X'FF'                    
...
PARMTAB  DS    0XL20                                                            
         ...
         DC    AL1(8),C'OUTPUTIT',AL3(VALOUT),AL1(37),7X'00'                    
         DC    AL1(8),C'OUTPUTI1',AL3(VALOUT),AL1(40),7X'00'                    
         DC    AL1(8),C'OUTPUTI2',AL3(VALOUT),AL1(41),7X'00'                    
         DC    AL1(8),C'OUTPUTI3',AL3(VALOUT),AL1(42),7X'00'                    
         ...
         DC    AL1(8),C'OUTPUTO1',AL3(VALOUT),AL1(43),7X'00'                    
         DC    AL1(8),C'OUTPUTOV',AL3(VALOUT),AL1(49),7X'00'                    
         DC    AL1(7),C'OUTPUTP ',AL3(VALOUT),AL1(25),7X'00'                    
         DC    AL1(7),C'OUTPUTQ ',AL3(VALOUT),AL1(26),7X'00'                    
         DC    AL1(7),C'OUTPUTR ',AL3(VALOUT),AL1(27),7X'00'                    
         DC    AL1(7),C'OUTPUTS ',AL3(VALOUT),AL1(28),7X'00'                    
         DC    AL1(8),C'OUTPUTTT',AL3(VALOUT),AL1(38),7X'00'                    
         DC    AL1(8),C'OUTPUTTU',AL3(VALOUT),AL1(39),7X'00'                    
         DC    AL1(7),C'OUTPUTT ',AL3(VALOUT),AL1(29),7X'00'                    
         DC    AL1(8),C'OUTPUTU1',AL3(VALOUT),AL1(44),7X'00'                    
         DC    AL1(8),C'OUTPUTU2',AL3(VALOUT),AL1(48),7X'00'                    
         DC    AL1(7),C'OUTPUTU ',AL3(VALOUT),AL1(30),7X'00'                    
         DC    AL1(7),C'OUTPUTV ',AL3(VALOUT),AL1(31),7X'00'                    
         DC    AL1(7),C'OUTPUTW ',AL3(VALOUT),AL1(32),7X'00'                    
         DC    AL1(7),C'OUTPUTY ',AL3(VALOUT),AL1(34),7X'00'                    
         DC    AL1(7),C'OUTPUTZ ',AL3(VALOUT),AL1(35),7X'00'                    
         DC    AL1(7),C'OUTPUT0 ',AL3(VALOUT),AL1(36),7X'00'                    
         DC    AL1(7),C'CONTROL ',AL3(VALUPT),8X'00'                            
         DC    AL1(5),C'DDSIO   ',AL3(VALDDS),8X'00'                            
         DC    AL1(6),C'DSPACE  ',AL3(VALDSP),8X'00'                            
         DC    AL1(6),C'NEWAGY  ',AL3(VALAGY),8X'00'                            
...
SYSTAB   DS    0XL19               SYSTEM TABLE                                 
         ...
         DC    AL2(17,00,00),X'01'                                              
         DC    C'XSPFILE ',X'02',AL3(XSPDEFN)                                   
         DC    AL2(32,32,42),X'01'                                              
         DC    C'NETFILE ',X'03',AL3(NETDEFN)                                   
         DC    AL2(20,20,27),X'01'                                              
...
KEYTAB   DS    0XL4                KEY FORMAT TABLE                             
         ...
         DC    AL1(L'AGYOLDA-1,AGYOLDA-AGYTABD,AGYNEWA-AGYTABD),X'00'           
...
DTFTAB   DS    0A                  OUTPUT FILE TABLE                            
         ...
         DS    0D                                                               
         DC    CL8'*AGYTAB*'                                                    
AGYTAB   DC    X'FF',800XL(AGYLEN)'00'                                          
AGYTABX  EQU   *                                                                
                                                                                
         DS    0D                                                               
         DC    CL8'*AGY2CP*'                                                    
AGY2CPY  DC    (MAXCPY)XL(A2CLNQ)'00'                                           
         DC    X'FF'                                                            
MAXCPY   EQU   32                                                               
...
***********************************************************************         
* NOTE THAT SUCCESSIVE ENTRIES IN THE RECORD DEFINITIONS TABLE WITH             
* EXACT MATCHING KEYS (RECKEYL AND RECKEYH) WILL RESULT IN THE SAME             
* RECORD BEING PROCESSED AGAIN                                                  
***********************************************************************         
***********************************************************************         
* ACCOUNT FILE RECORD DEFINITIONS (LAST RECTYPE# WAS 56)                        
***********************************************************************         
ACCDEFN  DS    0X                                                               
...
*nop     DC    AL1(2,AGYR)                                                      
         DC    X'1000FEFF',AL1(00,00),AL1(22),X'00',AL3(0)  <- KEY IS           
         DC    AL1(0,AGYL)                                     THE SAME         
         DC    X'1000FEFF',AL1(00,00),AL1(22),X'00',AL3(0)  <- PROCESS          
         DC    AL1(20,AGYA)                                    AGAIN            
...
XSPDEFN  DS    0X                                                               
         ...
         DC    AL1(2,AGYL)                                                      
         DC    X'0D0E0D0E',AL1(00,00),AL1(15),X'00',AL3(0)                      
         DC    AL1(4,AGYL)                                                      
...
NETDEFN  DS    0X                                                               
         ...
         DC    AL1(11,AGYL)                                                     
         DC    X'040004FF',AL1(00,00),AL1(03),X'00',AL3(NETUNIT)                
         DC    AL1(01,AGYL)                                                     
...
***********************************************************************         
* RECORDS UPDATED 08/2017 BY MBLU                                               
*                                                                               
REPDEFN  DS    0X                                                               
         DC    X'00000000',AL1(00,00),AL1(01),X'40',AL3(0)                      
         DC    AL1(0,0)                                                         
...
TOUTOV   DCB   DDNAME=TOUTOV,DSORG=PS,MACRF=(PM),                      *        
               RECFM=VB,BLKSIZE=32760,LRECL=6200,BUFNO=2                        
...
AGYNEWA  DS    CL2                 OLD AGENCY ALPHA                             
...
***********************************************************************         
* DSECT to cover list of alphas to copy from to to                              
***********************************************************************         
A2CD     DSECT                                                                  
AGYORG   DS    CL2                 Orginal alpha                                
AGYCPY   DS    CL2                 New alpha to copy to                         
A2CLNQ   EQU   *-A2CD                                                           
...
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'045DDSPANKIT 05/15/19'                                      
         END                                                                    
