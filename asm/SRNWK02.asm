*          DATA SET SRNWK02    AT LEVEL 059 AS OF 10/01/19                      
*PHASE T13502A                                                                  
         TITLE '$NWK - FILE FUNCTIONS'                                          
         PRINT NOGEN                                                            
WKRPRT   CSECT                                                                  
         NMOD1 000,**$NW2**,RA,R9,R8,RR=R4                                      
         LR    RC,R1                                                            
         USING NWKWKD,RC           RC=A(ROOTS WORKING STORAGE)                  
...
DATV0    CLC   22(5,R6),SR@TODAY   CHECK FOR TODAYS DATE                        
         BNE   DATV1                                                            
         MVC   0(2,R7),DATE30      USE NEW COMP DATES                           
         B     DATVX                                                            
...
DATV3    GOTO1 ADATCON,DMCB,(0,DUB),(30,(R7))                                   
DATVX    B     P4VX                                                             
         EJECT                                                                  
...
*                                                                               
         GOTO1 AWKSCAN,DMCB,(R0),ACTREC,AFILTAB,ACXREC,ACOMFACS,LOGONAG         
         CLI   8(R1),0                                                          
         BE    *+6                                                              
...
COMP010  XC    CISAVE,CISAVE       CLEAR ENTRY                                  
         MVC   CISWRKF,WSEWRKF                                                  
         MVC   CISADR(2),WSECIAD   SAVE CIADR                                   
         TM    DDS,DDSTOTL                                                      
         BNO   *+10                                                             
...
UNCO010  XC    WSEDATA,WSEDATA                                                  
         MVC   WSEWRKF,CISWRKF                                                  
         MVC   WSECIAD(2),CISADR RESTORE CIADR                                  
         TM    DDS,DDSTOTL                                                      
         BO    UNCO016                                                          
...
*----------------------------------------------------------------------         
         XR    R1,R1                                                            
         ICM   R1,3,CICITOT        TOTAL PART1 (HALF)                           
         BAS   RE,EDIT1                                                         
         MVC   SZTOT1,SWORK                                                     
*                                                                               
         SR    R1,R1                                                            
...
*                                                                               
         CLC   CISADR(2),CIPASS    COMPARE TRACK                                
         BE    FLST028                                                          
         AHI   R7,L'CISAVE                                                      
         B     FLST023                                                          
*                                                                               
...
*                                                                               
         ...
         SR    RE,RE                                                            
         D     RE,=F'8'                                                         
         A     RF,ATIA             THIS IS WHERE TWA11 IS                       
         AH    RF,=Y(NWKBITTB-SRSD)                                             
         LA    R1,X'80'                                                         
         SRL   R1,0(RE)                                                         
         STC   R1,BYTE                                                          
         OC    0(1,RF),BYTE                                                     
...
*                                                                               
FLST031  GOTO1 ADATCON,DMCB,(X'4E',HALF),(7,WKLCDATE)                           
         MVC   DUB(2),W_TIMEL                                                   
         BAS   RE,TIMEOUT                                                       
...
*                                                                               
         ...
         BO    FLST041                                                          
         GOTO1 ADATCON,DMCB,(2,HALF),(30,HALF)                                  
FLST041  GOTO1 ADATCON,DMCB,(X'4E',HALF),(7,WKLSDATE)                           
         MVC   DUB(2),W_TIMED                                                   
         BAS   RE,TIMEOUT                                                       
...
*                                                                               
         ...
         MVC   SRVSTAT,SWORK                                                    
*                                                                               
         MVC   SRVDATC,SPACES      SET PERMANENT BY DEFAULT                     
         MVC   SRVDATC(3),=C'PERM'                                              
         MVC   SRVDATD,SPACES                                                   
         MVC   SRVDATD(3),=C'PERM'                                              
*                                                                               
         CLC   W_AGELD,PERMDATE    IS THIS PERMANENT?                           
         BNL   DISF022             YES: SKIP DATCON                             
         MVC   HALF,W_AGELD                                                     
         TM    W_ATTB,W_ATNCD                                                   
         BO    DISF020                                                          
         GOTO1 ADATCON,DMCB,(2,HALF),(30,HALF)                                  
DISF020  GOTO1 ADATCON,DMCB,(X'4E',HALF),(8,SRVDATC)                            
*                                                                               
DISF022  CLC   W_AGERD,PERMDATE    IS THIS PERMANENT?                           
         BNL   DISF026             YES: SKIP DATCON                             
         MVC   HALF,W_AGERD                                                     
         TM    W_ATTB,W_ATNCD                                                   
         BO    DISF024                                                          
         GOTO1 ADATCON,DMCB,(2,HALF),(30,HALF)                                  
DISF024  GOTO1 ADATCON,DMCB,(X'4E',HALF),(8,SRVDATD)                            
*                                                                               
DISF026  SR    R0,R0               RETAIN TIME                                  
         SR    R1,R1                                                            
         IC    R1,W_AGERT                                                       
...
*                                                                               
         ...
         MVC   SRVWKDA+09(5),=C'WRKF#'                                          
         MVC   SRVWKDA+14(1),WRKFCHR                                            
*                                                                               
         GOTO1 AHEXOUT,DMCB,W_CINEXT,SRVNEXT,2                                  
*                                                                               
         CLI   W_SEQ,2             IS THIS PART 1                               
         BNL   DISF990                                                          
*                                                                               
         CLI   CIADDR+2,1          IS IT BLOCK 1                                
         BNE   DISF990                                                          
...
***********************************************************************         
* INITIALISE & READ RECORD CIPASS=TTTTBB00                                      
***********************************************************************         
READREC  NTR1                                                                   
...
*                                                                               
         ...
         BO    ERR8                                                             
*                                                                               
ACTV010  MVC   STAACT,ACTACT       SET TO ACTV                                  
         GOTO1 AWKUPDT                                                          
*                                                                               
ACTVFILX B     XIT1                                                             
...
*                                                                               
         ...
         BNE   RETA010                                                          
         MVC   NXINFO(2),=X'FFFF'                                               
         NI    NXFLAG,X'FF'-UKFLHRS PASSING PERM DATE, NOT HRS                  
*                                                                               
RETA010  MVC   STAACT,ACTRET       SET TO RETAIN                                
         GOTO1 AWKUPDT                                                          
*                                                                               
RETAFILX B     XIT1                                                             
         EJECT                                                                  
...
*        BNE   CLRERR                                                           
*                                                                               
*LR4     OC    USERID,USERID       CAN BE ALL FOR DDS                           
*        BZ    *+14                                                             
...
*                                                                               
*LR6     BAS   RE,GETCAD           PASS 2 ZAP CIREC INDEX                       
*        L     R6,ACIREC                                                        
...
*                                                                               
*LRA     MVC   SRVMSG(17),=C'ALL FILES CLEARED'                                 
*        OI    SRVMSGH+6,X'80'                                                  
*        OI    SRVP1H+6,X'40'                                                   
*        B     EXIT                                                             
*                                                                               
...
*                                                                               
         ...
         BNO   HOUROLD                                                          
*                                                                               
HOURNEW  CLC   HALF,PERMDATE       TEST FOR PERMANENT                           
         BNL   HOUR045                                                          
         MVC   DUB(2),HALF         GET RETAIN DATE                              
         CLC   DUB(2),DATE30       CHECK WITH TODAY (NCD)                       
         BL    HOUR042                                                          
         GOTO1 ADATCON,DMCB,(14,DUB),(0,DUB1)                                   
         B     HOUR002                                                          
*                                                                               
HOUROLD  CLC   HALF,PERMDATE       TEST FOR PERMANENT                           
         BNL   HOUR045                                                          
HOUR001  MVC   DUB(2),HALF         GET RETAIN DATE                              
         CLC   DUB(2),DATE2        CHECK WITH TODAY                             
         BL    HOUR042                                                          
         GOTO1 ADATCON,DMCB,(2,DUB),(0,DUB1)                                    
*                                                                               
HOUR002  GOTO1 APERVERT,DMCB,DATE,DUB1                                          
...
HOUR045  MVC   FULL,=C'PER'        PERMANENT                                    
         B     XIT1                                                             
*                                                                               
PERMDATE DC    X'FF9F'             CMPRSD PERMANENT => FF9F                     
...
*                                                                               
         ...
         TM    DDS,DDSTRM                                                       
         BZ    CNTDH                                                            
         CLI   WRKFCHR,C'I'         TEST DDS TERM AND MULTIPLE WRKFS            
         BE    CNTDSPX                                                          
         MVC   SRVINF(6),=C'WRKF#N'                                             
         MVC   SRVINF+5(1),WRKFCHR                                              
         B     CNTDSPX                                                          
*                                                                               
...
***********************************************************************         
* EXPAND BINARY TIME IN DUB(2)                                                  
***********************************************************************         
TIMEOUT  XC    DUB+2(6),DUB+2                                                   
         CLI   DUB,23                                                           
         BH    TIMEOUTX                                                         
         CLI   DUB+1,59                                                         
         BH    TIMEOUTX                                                         
...
***********************************************************************         
* SHARED MEMORY INDEX AND WORKER FILE ROUTINES                                  
***********************************************************************         
       ++INCLUDE DMWRKFR                                                        
       ++INCLUDE SHFIR                                                          
         EJECT                                                                  
...
ATTBTBLX DC    X'00'                                                            
         SPACE 2                                                                
ATTINDS  DC    C'Obj/ncd/Err/Pwd/AT5/Xtn/AT7/Sht'                               
...
*DMWRKFK                                                                        
       ...
       ++INCLUDE SRNWKTWAB                                                      
         IHAASCB LIST=YES                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'059SRNWK02   10/01/19'                                      
         END                                                                    
