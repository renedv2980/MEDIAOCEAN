*          DATA SET RERMP10    AT LEVEL 060 AS OF 04/29/10                      
*PHASE T81010C                                                                  
*INCLUDE INVDAY                                                                 
*INCLUDE REBKLST                                                                
...
*                                                                  *            
*                                                                  *            
* MAR20/01 (FD ) --- MULTI HUT FEATURE.  ALLOWS ADDITIONAL DAYS    *            
*                    AND TIMES TO BE ADDED FOR CALCULATION OF HUTS *            
*                    AND PUTS.  THE ADDITIONAL DAY TIMES ARE KEPT  *            
*                    IN THE X'09' ELEMENT.  WE DIDN'T JUST CREATE  *            
*                    ADDITIONAL X'02'S BECAUSE THERE WAS SOME PRE- *            
*                    EXISTING CODE THAT WOULD HAVE RECOGNIZED THEM *            
*                    AND WE WERE NOT SURE IF THERE WAS ANY         *            
*                    SIMILAR, BUT HARD CODED LOGIC THAT MAY HAVE   *            
*                    CAUSED SOME BAD MOJO.                         *            
*                                                                  *            
*            !!!!--->TAKE NOTE, THE X'02' IS TRANSLATED INTO A     *            
*                    X'09' VS A X'CE' ON TRACKS WHEN THERE ARE     *            
*                    MULTIPLE DAY/TIMES(DONT' ASK). THIS HAPPENS   *            
*                    IN SINV & MINV                                *            
*                                                                  *            
* AUG12/08 (KUI) --- DIE IF WE ARE NOT PROCESSING X'12' KEYS       *            
*                                                                  *            
* MAR17/09 (KUI) --- SUPPORT NEW INVENTORY KEY X'62'               *            
*                    ***  END TOMBSTONE  ***                       *            
********************************************************************            
T81010   CSECT                                                                  
         PRINT NOGEN                                                            
...
*                                                                               
         ...
         BNE   ERREND                                                           
*                                  MUST BE HEADER                               
         OC    KEY+RINVKRTP-RINVKEY(3),KEY+RINVKRTP-RINVKEY                     
         BNZ   *+10                                                             
         MVC   TEMPSVDA,KEY+28                                                  
*                                                                               
RST100   CLC   KEY(RINVKRTP-RINVKEY),KEYSAVE  CHECK UP TO RECORD TYPE           
         BNE   RST200                                                           
*                                  MUST BE HEADER                               
         OC    KEY+RINVKRTP-RINVKEY(3),KEY+RINVKRTP-RINVKEY                     
         BNZ   *+10                                                             
...
VR140    L     RE,AIO1                                                          
         MVC   RINVKSTD,RINVKSTD-RINVKEY(RE)                                    
         MVC   RINVPEFF,RINVPEFF-RINVKEY(RE)                                    
*                                                                               
         LA    R2,INVEFFDH                                                      
         CLI   5(R2),0                                                          
         BE    VR180                                                            
...
VR240    LA    R5,1(R5)                                                         
         ...
         SPACE 3                                                                
*                                                                               
*  COPY DYS/TIMS TO AVAIL DYS/TIMS                                              
*                                                                               
VR250    MVI   ERROR,INVALID                                                    
         LA    R2,INVCDYH                                                       
*                                                                               
         CLI   5(R2),0                       ANY INPUT?                         
         BNE   VR253                                                            
         MVI   8(R2),C'N'                     NO, DEFAULT IS "NO"               
         OI    6(R2),X'80'                        XMIT IT                       
         B     VR257                              NOW, MARK FLAGS               
*                                                                               
VR253    DS    0H                             YES, VALIDATE IT                  
         CLI   8(R2),C'Y'                    USER CHOSE YES?                    
         BNE   VR255                                                            
         OI    RINVFLG1,RINVCMAQ             YES, TURN REC  FLAG ON             
         OI    GENFLG1,CPYMHAVQ                   TURN PROG FLAG ON             
         B     VR260                                                            
VR255    CLI   8(R2),C'N'                    NO, USER CHOSE 'NO'?               
         BNE   ERREND                            NO, ERROR                      
VR257    NI    RINVFLG1,X'FF'-RINVCMAQ           YES,TURN REC  FLAG OFF         
         NI    GENFLG1,X'FF'-CPYMHAVQ                TURN PROG FLAG OFF         
*                                                                               
*  TRANSFER DEFAULTS                                                            
...
*                                                                               
*      FIRST, CHECK MANDATORY DAY/TIME (INCLUDED IN KEY)                        
*                                                                               
VR460    MVI   ERROR,MISSING                                                    
         BAS   RE,CHKDAYTM         CHECK DAY TIME IN SYNC                       
         CLI   DAYINP,C'Y'                                                      
         BNE   ERREND                                                           
*                                                                               
VR480    GOTO1 =A(OVFLRTN),DMCB,(5,DUB),(RC),RR=RELO    (VALDAY)                
         BAS   RE,NEXTFLD                                                       
*                                                                               
VR485    BAS   RE,VALTIME          TIME VALIDATION                              
         MVI   FIRSTSW,C'N'                                                     
         LA    RF,INVSDAYH         SET ONE-TIME SWITCH FOR POSSIBLE             
*                                     X'09' GEN OF FIRST D/T STRING             
         ST    RF,FRSTDTIM         SAVE A(FIRST DAY/TIME STRING)                
*                                                                               
*    NEXT VALIDATE MULTI HUT DAYS/TIMES                                         
*                                                                               
         LHI   R5,7                INIT COUNTER                                 
         LA    R2,INVMDY1H                                                      
*                                                                               
VR500    BAS   RE,CHKDAYTM         DAY AND TIME FIELD IN SYNCH?                 
         CLI   DAYINP,C'Y'         IS THERE A MULTI-HUT ENTRY?                  
         BNE   VR530               NO, SKIP TO NEXT                             
         OC    FRSTDTIM,FRSTDTIM   YES - WAS X'09' GENERATED FOR                
*                                     FIRST X'02' ENTRY?                        
         BZ    VR520               YES                                          
         ST    R2,SAVADTIM         NO  - SAVE A(CURRENT MULTI-HUT)              
         L     R2,FRSTDTIM         LOAD A(X'02' SCREEN LINE)                    
         BRAS  RE,VALMULDT         GENERATE AN X'09' FOR IT                     
         L     R2,SAVADTIM         RELOAD A(CURRENT MULTI-HUT)                  
         XC    FRSTDTIM,FRSTDTIM   CLEAR FIRST-TIME SWITCH                      
VR520    EQU   *                                                                
         BRAS  RE,VALMULDT         VALIDATE MULTI-HUT                           
*                                                                               
VR530    LA    R2,TONXDQ(R2)                                                    
         BCT   R5,VR500            LOOP BACK FOR ALL MHUT DY/TM                 
*                                                                               
*     VALIDATE REMAINING PROGRAM FIELDS                                         
*                                                                               
         MVI   ERROR,MISSING                                                    
         LHI   R5,8                INIT COUNTER                                 
         LA    R2,INVPRG1H         1ST PROGRAM FIELD                            
         CLI   5(R2),0             ANYTHING ENTERED?                            
         BE    ERREND                NO, ERROR (1ST REQUIRED)                   
VR540    GOTO1 =A(OVFLRTN),DMCB,(6,DUB),(RC),RR=RELO    (VALPROG)               
         LA    R2,TOPRGQ(R2)         NEXT PROGRAM FIELD                         
         BCT   R5,VR540                                                         
*                                                                               
*     NOW DECIDE HOW TO TREAT AVAIL DAY/TIME FIELDS                             
*                                                                               
VR550    TM    GENFLG1,CPYMHAVQ    COPY DY/TMS OPTION ON?                       
         BZ    VR560               NO, SKIP COPY (JUST VALIDATE AS IS)          
         BRAS  RE,COPYDYTM         YES, COPY AVAIL DAYS/TMS                     
*                                                                               
*                                  VALIDATE AND BUILD ELEMS                     
VR560    LHI   R5,8                INIT COUNTER                                 
         LA    R2,INVADY1H                                                      
VR570    GOTO1 =A(OVFLRTN),DMCB,(7,DUB),(RC),RR=RELO    (VALADTM)               
         LA    R2,TOAVDQ(R2)                                                    
         BCT   R5,VR570                                                         
         B     VREX                                                             
         DROP  R6                                                               
...
*  IF DAY INPUTTED TIME MUST ALSO BE INPUTTED                                   
         ...
         CLI   5(R3),0                                                          
         BNE   CHKDY100                                                         
         LR    R2,R3               CURSOR POS FOR GENCON                        
         B     ERREND                                                           
CHKDY100 MVI   DAYINP,C'Y'         DAY INPUTTED                                 
         B     CHKDYEX                                                          
...
**********************************************************************          
*                                                                               
*  VALTIME VALIDATE THE TIME FIELD (R2 POINTS TO TIME FIELD)                    
*                                                                               
**********************************************************************          
VALTIME  NTR1                                                                   
         USING RIDTELEM,R6                                                      
...
VREX20   L     RF,AMENUTAB                                                      
         ...
         B     VR05                                                             
*                                                                               
VREXX    GOTO1 =A(OVFLRTN2),DMCB,(3,DUB),(RC),RR=RELO    (DREC)                 
         OI    MYFLAG,FRMVREC      CAME FROM VREC                               
         B     DKEY                                                             
*                                                                               
         DROP  R6                                                               
         SPACE 1                                                                
...
*                                                                               
         MVI   ERROR,OVERLAP                                                    
         CLC   0(RINVKSTD-RINVKEY,R4),0(R6) DOES RETURNED RECRD QUALIFY         
         BNE   MNT40               NO EXIT                                      
...
*                                                                               
MNT300   LA    RE,RINVKSTD-RINVKEY EXECUTE LENGTH FOR CHGD EFD                  
*                                                                               
*  WAS EFFECTIVE DATE CHANGED                                                   
*                                                                               
         CLC   RINVKSTD-RINVKEY(L'RINVKSTD,R4),RINVKSTD-RINVKEY(R6)             
         BE    *+8                                                              
         LA    RE,RINVKSPR-RINVKEY EXECUTE ELNGTH FOR UNCHANGED EFD             
*                                                                               
         STC   RE,CHNGLEN                                                       
*                                                                               
...
*                                                                               
         ...
         GOTO1 HIGH                                                             
         SPACE 1                                                                
ENDOLDA  CLC   KEYSAVE(RINVKSTD-RINVKEY),0(R4)                                  
...
*                                                                               
         L     R5,12(R1)           POINT TO CE ELEMENT                          
         MVC   DUB(3),RINVZBK-RINVZCOD(R5) SAVE SOURCE & BOOK                   
*                                                                               
...
*                                                                               
         LA    RF,WORK                                                          
         USING RINVZCOD,RF         ESTABLISH ELEMENT                            
*                                                                               
         MVI   RINVZCOD,X'CE'      ELEMENT ID                                   
         MVI   RINVZLEN,RINVZLNQ   ELEMENT LENGTH                               
         MVC   RINVZDAY,2(R5)      DAY FROM X'02' ELEMENT                       
         MVC   RINVZTIM,3(R5)      TIME                                         
         MVC   RINVZBK,DUB         FROM BOOK                                    
*                                                                               
         DROP  RF                                                               
*                                                                               
...
*                                                                               
         ...
         XC    INVKEY,INVKEY                                                    
*                                                                               
LR50     OC    KEY(RINVKINV-RINVKEY),KEY                                        
         BNZ   LR100                                                            
*                                                                               
         SPACE                                                                  
         LA    R6,KEY                                                           
...
LR100    DS    0H                                                               
         MVI   RDUPDATE,C'N'       NOT FILE MARKER                              
         GOTO1 HIGH                                                             
...
*                                  SAME INVENTORY NUMBER?                       
         CLC   RINVKINV,TEMPKEY+RINVKINV-RINVKEY                                
         BNE   *+14                                                             
         MVC   SAVEKEY,KEY                                                      
         B     LR200                                                            
*                                                                               
...
*  FILTERS                                                                      
         ...
         OI    INVSFLTH+6,X'80'                                                 
*                                                                               
*  COPY MULTI-HUT DYS/TIMS  TO AVAIL DYS/TIMS                                   
*                                                                               
DREC70   LA    R2,INVCDYH                                                       
         TM    RINVFLG1,RINVCMAQ       FLAG ON?                                 
         BZ    *+12                                                             
         MVI   8(R2),C'Y'                YES                                    
         B     *+8                                                              
         MVI   8(R2),C'N'                NO                                     
...
*  AVAIL TIME BECAUSE THEY ARE UNPROTECTED AND 1 CHARACTER APART.               
*                                                                               
...
DREC220  OI    6(R2),X'80'         TRANSMIT                                     
         DROP  R4                                                               
*                                                                               
*  MULTI HUT DAYS/TIMES                                                         
DREC230  BRAS  RE,DISMHDT          DISPLAY MULTI HUT DAYS/TIMES                 
*                                                                               
*  PROGRAM                                                                      
DREC240  LA    R2,INVPRG1H                                                      
         USING RIPGELEM,R4                                                      
         PRINT GEN                                                              
...
*                                                                               
         ...
         MVC   BSVDA,KEY+28     SAVE DISK ADDRESS                               
*                                                                               
OVDEL10  CLC   KEY(RINVKSPR-RINVKEY),KEYSAVE     CHECK UP TO REC TYPE           
         BNE   OVDELX                                                           
         OI    MYFLAG,DOTRANS                                                   
...
*                                                                               
OVDELX   DS    0H                                                               
* TAKE OUT CODE TO READ INVENTORY KEY  WHICH SHOULD HAVE ALREADY                
* BEEN DELETED.  WE SEE NO NEED TO TAKE THIS EXTREME PRECAUTION AS              
* THIS POSES A PROBLEM WITH USERS TRYING TO DELETE RECORDS SIGNING              
* ON IN READ ONLY MODE.                                                         
*                                                                               
*&&DO                                                                           
         XC    KEY,KEY                                                          
         MVC   KEY(RINVKSPR-RINVKEY),KEYSAVE                                    
*                                                                               
         MVC   KEYSAVE(27),KEY                                                  
         ...
         DC    H'00'                                                            
*                                                                               
         CLC   KEYSAVE(RINVKSPR-RINVKEY),KEY     SHOULD NOT FIND MATCH!!        
         BNE   *+6                                                              
         DC    H'00'               !!!!!!!!1                                    
*&&                                                                             
         B     OV2EXIT                                                          
...
*                                                                               
         GOTO1 GETINV                                                           
*                                                                               
*                                  FOUND MATCH UP TO EFF DATE?                  
         CLC   KEY(RINVKSPR-RINVKEY),SAVEKEY                                    
         BNE   OV2ERRND                                                         
*                                  IS THIS A HEADER?                            
         OC    KEY+RINVKRTP-RINVKEY(3),KEY+RINVKRTP-RINVKEY                     
         BNZ   OV2ERRND                                                         
...
********************************************************************            
*                                                                               
*   VALMULDT ROUTINE  - VALIDATE MULTI HUT DAY/TIME FIELDS                      
*                                                                               
********************************************************************            
VALMULDT NTR1  BASE=*,LABEL=*                                                   
         MVI   ERROR,INVALID                                                    
         XC    WORK2,WORK2                                                      
         LA    R6,WORK2                                                         
         USING RIMHELEM,R6                                                      
         MVC   RIMHCODE(2),=XL2'0907'                                           
*                                                                               
         XR    R5,R5                                                            
         IC    R5,5(R2)            LENGTH OF EXPRESSION                         
         GOTO1 DAYVAL,DMCB,((R5),8(R2)),RIMHDAY,WORK                            
         CLI   RIMHDAY,0                                                        
         JE    ERREND                                                           
*                                                                               
*     NOW VALIDATE TIME                                                         
*                                                                               
         LA    R2,TOMTMQ(R2)       POINT R2 AT TIME FIELD                       
         CLI   8(R2),C'N'          NONE                                         
         JE    ERREND                                                           
         CLI   8(R2),C'V'          VARIOUS ARE NOT VALID                        
         JE    ERREND                                                           
         ...
         XR    R5,R5                                                            
         IC    R5,5(R2)            LENGTH OF EXPRESSION                         
         LA    R3,6(R5,R2)         POINT R3 2CHARS BEFORE END                   
*        CLC   0(2,R3),=C',B'      ENDS WITH ,B?                                
*        BNE   *+8                  NO                                          
*        SHI   R5,2                 YES,REDUCE INPUT LENGTH                     
         GOTO1 TIMVAL,DMCB,((R5),8(R2)),RIMHTIME                                
         CLI   DMCB,X'FF'                                                       
         JE    ERREND                                                           
         OC    RIMHTIME+2(2),RIMHTIME+2  END TIME ENTERED?                      
         JZ    ERREND                                                           
...
********************************************************************            
*                                                                               
*      COPYDYTM ROUTINE - COPIES DAY TIME FIELDS TO                             
*                         AVAIL DAY TIME FIELDS                                 
*                                                                               
********************************************************************            
COPYDYTM NTR1  BASE=*,LABEL=*                                                   
                                                                                
         LHI   R6,8                 INIT COUNTER                                
                                                                                
*     INIT POINTERS                                                             
         LA    R2,INVSDAYH          1ST DAY FIELD                               
         LA    R3,INVADY1H          1ST AVAIL DAY FIELD                         
         LA    R4,INVSTMEH          1ST TIME FIELD                              
         LA    R5,INVATM1H          1ST AVAIL TIME FIELD                        
*     COPY DAY FIELD                                                            
CPYD10   MVC   5(1,R3),5(R2)         COPY LENGTH                                
         MVC   8(L'INVADY1,R3),8(R2) COPY DATA                                  
         OI    6(R3),X'80'           TRANSMIT CHANGE                            
*                                                                               
*     COPY TIME FIELD                                                           
         MVC   5(1,R5),5(R4)         COPY LENGTH                                
         MVC   8(L'INVATM1,R5),8(R4) COPY DATA                                  
         OI    6(R5),X'80'           TRANSMIT CHANGE                            
*                                                                               
*     BUMP POINTERS                                                             
         LA    R2,TONXDQ(R2)         NEXT DAY                                   
         LA    R3,TONXTQ(R3)         NEXT TIME                                  
         LA    R4,TOAVDQ(R4)         NEXT AVAIL DAY                             
         LA    R5,TOAVTQ(R5)         NEXT AVAIL TIME                            
         BCT   R6,CPYD10                                                        
                                                                                
         XIT1                                                                   
********************************************************************            
*                                                                               
*   DISMHDT ROUTINE - DISPLAY MULTI HUT DAYS/TIMES                              
*                                                                               
********************************************************************            
DISMHDT  NTR1  BASE=*,LABEL=*                                                   
*                                                                               
         L     R5,AIO                                                           
         LA    R2,INVMDY1H      POINT TO FIRST MULTI-HUT DAY FLD                
         LA    R3,INVMDYLH      POINT TO LAST "                 "               
*                                                                               
         MVC   DATADISP,=H'34'                                                  
         MVI   ELCODE,X'09'                                                     
         BRAS  RE,GETEL                                                         
         BNE   DISMHX                                                           
         BRAS  RE,NEXTEL           RETRIEVE SECOND X'09'                        
*                                  DON'T DISPLAY FIRST X'09' BECAUSE            
*                                     FIRST IS GENERATED FROM 1ST X'02'         
*                                     AND WOULD BE A DUPLICATE DISPLAY          
         BNE   DISMHX                                                           
         USING RIDTELEM,R5                                                      
*                                                                               
DISM100  XC    WORK,WORK                                                        
         GOTO1 UNDAY,DMCB,RIDTDAY,WORK              DAY                         
         MVC   8(11,R2),WORK                                                    
         OI    6(R2),X'80'         TRANSMIT                                     
         ZICM  RF,0(R2)                                                         
...
*-- NO END TIME MOVE ,B AFTER START TIME                                        
         LA    RE,8(R2)                                                         
         LA    RF,11                                                            
DISM120  CLI   0(RE),X'40'                                                      
         BNH   DISM200                                                          
         LA    RE,1(RE)                                                         
         BCT   RF,DISM120                                                       
         DC    H'0'                                                             
DISM200  MVC   0(2,RE),=CL2',B'                                                 
*                                                                               
DISM220  OI    6(R2),X'80'         TRANSMIT                                     
*                                                                               
DISM240  OI    6(R2),X'80'         TRANSMIT                                     
         BRAS  RE,NEXTEL           ANOTHER?                                     
         BNE   DISMHX               NO                                          
         LA    R2,MT2MDQ(R2)        YES, BUMP TO NEXT MULTI DAY FIELD           
         CR    R2,R3                     CHECK IF SPACE TO DISPLAY IT           
         BNH   DISM100                    YES, LOOP BACK                        
         DC    H'0'                       NO, SHOULD NEVER, EVER HAPPEN         
         DROP  R5                                                               
DISMHX   XIT1                                                                   
********************************************************************            
*                                                                               
...
*                                                                               
GENFLG1  DS    XL1                 GENERAL FLAG                                 
CPYMHAVQ EQU   X'80'               COPY M-HUT DYS/TMS TO AV DYS/TMS             
*                                                                               
...
INVSRC   DS    CL(RINVKBK-RINVKRTP)   SOURCE                                    
...
FRSTDTIM DS    A                   A(FIRST DAYTIME STRING)                      
SAVADTIM DS    A                   A(CURRENT DAYTIME STRING)                    
...
*                                                                               
TONXDQ   EQU   INVMDY1H-INVSDAYH   DISTANCE BETWEEN DAY FIELDS                  
TONXTQ   EQU   INVMTM1H-INVSTMEH   DISTANCE BETWEEN TIME FIELDS                 
TOMTMQ   EQU   INVMTM1H-INVMDY1H   DISTANCE BETWEEN MULT-DAY/MULT-TIME          
TOPRGQ   EQU   INVPRG2H-INVPRG1H   DISTANCE BETWEEN PROGRAM FIELDS              
TOAVDQ   EQU   INVADY2H-INVADY1H   DISTANCE BETWEEN AVAIL DAY FIELDS            
TOAVTQ   EQU   INVATM2H-INVATM1H   DISTANCE BETWEEN AVAIL TIME FIELDS           
MT2MDQ   EQU   INVMDY2H-INVMTM1H   DISTANCE FROM M-TIM TO NXT M-DAY             
         EJECT                                                                  
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'060RERMP10   04/29/10'                                      
         END                                                                    
**PAN#1  CSECT                                                                  
         END                                                                    
