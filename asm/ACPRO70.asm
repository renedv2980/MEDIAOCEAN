*          DATA SET ACPRO70    AT LEVEL 007 AS OF 10/27/11                      
*PHASE T60B70A                                                                  
*INCLUDE TWABLD                                                                 
*INCLUDE HEXIN                                                                  
         TITLE 'T60B70 - FALINK CONTROLLER'                                     
*                                                                               
* DPEA 002 08APR03 DEAL WITH ERRORS RETURNED BY GENCON                          
* MWHE 003 26AUG04 PHASE CARD CHANGE FOR PANAPT                                 
* TKLU 004 11MAR08 <LO01-7431> ENLARGE LINKL LOCAL STORAGE BY 2K                
* TKLU 005 27OCT08 <LO01-8119> ENLARGE LINKL LOCAL STORAGE BY 1K                
* AHYD 007 17OCT11 <US-MR038880> CONVERT TO USE GETFACT FOR FALINK              
*                                                                               
T60B70   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,*T60B70*,R8,RR=R4                                              
         SRA   RF,32-8                                                          
         BNZ   MAIN02                                                           
         L     RC,0(R1)                                                         
         USING GEND,RC                                                          
...
EXITERR  DS    0H                  RETURN ERROR TO FALINK                       
         MVI   FAMSGNO,0                                                        
         MVC   FAMSGNO+1(1),ERROR     COPY ERROR DETAILS                        
         MVI   FAMSGTYP,C'E'                                                    
         MVI   FAMSGSYS,X'FF'                                                   
         CLI   ERROR,61            ERROR CODE BELOW 61 ARE GENERAL              
         BL    *+8                                                              
         MVI   FAMSGSYS,16         OTHERWISE PRODUCTION                         
         OC    FAMSGNO,FAMSGNO     TEST MESSAGE SET                             
         BNZ   *+10                                                             
         MVC   FAMSGNO,=AL2(INVALID) NO - SET TO INVALID INPUT FIELD            
*                                                                               
EXITN    LTR   RB,RB                                                            
         B     EXIT                                                             
*                                                                               
...
EXIT     XIT1  ,                                                                
         EJECT                                                                  
***********************************************************************         
* INITIALISATION CALL TO FALINK                                       *         
***********************************************************************         
         SPACE 1                                                                
INIT     DS    0H                                                               
         L     R7,4(RD)            EXTEND W/S FOR LINKD                         
         LA    R7,72(R7)                                                        
         ST    R7,ALINK                                                         
         LHI   R3,LINKL                                                         
         L     RF,4(RD)                                                         
...
*        ST    RF,FVADDR                                                        
         ...
         MVC   FALASWCH,SWITCH                                                  
         MVC   FALASVE,AFASAVE                                                  
         OI    CONSERVH+FHATD,FHATMO NEEDED FOR COMPRESSION TO WORK             
*                                                                               
         GOTO1 LOADOVR                                                          
*                                                                               
         GOTO1 CALLOV,DMCB,0,X'D9000AE7'   GET A(FALINK)                        
         CLI   4(R1),X'FF'                                                      
         BNE   *+6                                                              
         DC    H'0'                                                             
         L     RF,DMCB                                                          
         GOTO1 (RF),DMCB,FALINKD           CALL FALINK                          
         MVI   ERROR,SUPPLIED                                                   
         OI    CONTAGH+FHOID,FHOICU        POSITION CURSOR                      
         B     EXITY                                                            
         EJECT                                                                  
***********************************************************************         
...
RCV02    DS    0H                                                               
         ...
         GOTO1 AGETDATA,DMCB,FALINKD,FARETBK                                    
...
*                                                                               
         XC    ORCVHDRF,ORCVHDRF   SET A(ROUTINES)                              
...
*                                                                               
RCV06    DS    0H                                                               
         XC    FAELEM,FAELEM       CLEAR ELEMENT DETAILS                        
         XC    AMDEL,AMDEL                                                      
         TM    MHINDS,MHIRECEL     TEST RECORD ELEMENT                          
         BZ    RCV08               YES - SET ELEMENT CODE/LENGTH                
         MVC   FAELEM(L'MHELCOD),MHCODE+1                                       
         CLI   MHELCOD,0                                                        
         BE    *+10                                                             
         MVC   FAELEM(L'MHELCOD),MHELCOD                                        
         MVC   FAELEM+L'MHELCOD(L'MHELLEN),MHELLEN                              
*                                                                               
RCV08    DS    0H                                                               
...
RCV20    DS    0H                                                               
         ...
         ST    RF,ORCVDATA                                                      
*                                                                               
         TM    MDINDS,MDIELFLD     TEST PUT DATA IN FAELEM                      
         BZ    RCV22                                                            
         XR    RE,RE                                                            
         IC    RE,MDDISP                                                        
         LA    R1,FAELEM(RE)                                                    
         ...
         CLM   RE,1,FAELEM+1                                                    
         BNH   *+8                                                              
         STC   RE,FAELEM+1         RESET ELEMENT LENGTH                         
*                                                                               
RCV22    DS    0H                                                               
...
SENDEND  DS    0H                                                               
         ...
         BZ    EXITY                                                            
         GOTO1 AADDDATA,DMCB,FALINKD,0,0,0    END OF SEND                       
         B     EXITY                                                            
         EJECT                                                                  
***********************************************************************         
* LOAD OVERLAY AND TEST WHETHER TO TURN COMPRESSION OFF               *         
*                                                                     *         
* EXIT: ONTRYA = A(OVERLAY)                                           *         
***********************************************************************         
LOADOVR  NTR1  ,                                                                
         LA    R2,CONTAGH                                                       
         USING FALSCRD,R2                                                       
         ...
         BNE   LOVR10                                                           
*                                                                               
         GOTO1 GETFACT,DMCB,(X'80',0),F#FAL1ST                                  
         L     R3,0(,R1)           GET RETURN BLOCK                             
         CLC   =C'B=',0(R3)                                                     
         BNE   LOADOVRN                                                         
         XC    SVEL,SVEL                                                        
         AHI   R3,2                                                             
         GOTO1 =V(HEXIN),DMCB,(R3),SVEL+1,2,RR=RELO                             
         CLI   DMCB+15,1                                                        
         BNE   LOADOVRN                                                         
*                                                                               
         LA    R4,OVRTAB                                                        
...
LOVR10   CLI   SVOVR,0             TEST NOTHING DONE YET                        
         BE    LOADOVRX                                                         
         GOTO1 CALLOV,DMCB,(SVOVR,SYSDUMMY),0                                   
         CLI   4(R1),X'FF'                                                      
         BNE   *+6                                                              
         DC    H'0'                                                             
         XR    RF,RF                                                            
...
SHDR04   DS    0H                  LOCATE A(ENTRY)                              
         GOTO1 ASETMHEL,DMCB,(R2)                                               
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R3,AMHEL                                                         
*                                                                               
SHDR06   DS    0H                  SEND ELEMENT CODE                            
         GOTO1 ASETELEM,DMCB,FALINKD,MHELD,0                                    
...
SDATA04  DS    0H                  LOCATE A(ENTRY)                              
         GOTO1 ASETMDEL,DMCB,(R2)                                               
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R3,AMDEL                                                         
*                                                                               
SDATA06  DS    0H                                                               
*        CLC   MDVSN,CSFXBVSN      TEST DATA OKAY FOR THIS VERSION              
*        BH    SNDDATAX                                                         
*                                                                               
         LTR   R4,R4                                                            
         BNZ   *+8                                                              
         IC    R4,MDDLEN                                                        
*                                                                               
*        CLI   MDTYPE,MDTCHQ       TEST STRING                                  
*        BNE   SDATA10                                                          
*        LTR   RE,R4                                                            
*        BZ    SDATA10                                                          
*        LR    RF,R5                                                            
*DATA08  CLI   0(RF),0             CONVERT ZEROS TO SPACES                      
*        BNE   *+8                                                              
*        MVI   0(RF),C' '                                                       
*        CLI   0(RF),C'?'          CONVERT ? TO . (DUE TO FALINK BUG)           
*        BNE   *+8                                                              
*        MVI   0(RF),C'.'                                                       
*        LA    RF,1(RF)                                                         
*        BCT   RE,SDATA08                                                       
...
SDATA10  DS    0H                                                               
         ...
         CP    PZERO,0(0,R5)                                                    
*                                                                               
SDATA12  CLI   MDTYPE,MDTCHQ       TEST STRING FOR SPACES (THEN ZEROS)          
         BNE   SDATA14                                                          
         EX    RE,*+8                                                           
         BE    SNDDATAX                                                         
         CLC   0(0,R5),MYSPACES                                                 
*                                                                               
SDATA14  EX    RE,*+8              TEST OTHER TYPES FOR 0                       
         BZ    SNDDATAX                                                         
         OC    0(0,R5),0(R5)                                                    
*                                                                               
SDATA20  DS    0H                                                               
         GOTO1 AADDDATA,DMCB,FALINKD,MDELD,(R5),(R4)                            
         OI    LINKINDS,LINKISNT                                                
*                                                                               
SNDDATAX B     EXITY                                                            
         DROP  R3                                                               
...
SNDFLD   DS    0H                                                               
         XR    RF,RF                                                            
         ICM   RF,7,5(R1)          COPY ELEMENT TO FAELEM                       
         LA    RE,FAELEM                                                        
         CR    RE,RF                                                            
         BE    *+10                                                             
         MVC   FAELEM,0(RF)                                                     
*                                                                               
         MVC   BYTE,4(R1)          SAVE INDICATORS                              
*                                                                               
         L     R3,0(R1)                                                         
         USING MDELD,R3            R3 = A(MAPPING DATA ELEMENT)                 
         LH    R2,2(R1)            OR R2 = MAP CODE                             
...
SFLD06   DS    0H                                                               
         ...
         BZ    SFLD10                                                           
         CLC   MDDISP,FAELEM+1     TEST FIELD IS PART OF ELEMENT                
         BNL   SNDFLDX                                                          
         XR    R2,R2                                                            
         ...
         BNZ   SFLD08                                                           
         IC    R4,FAELEM+1         IF VARIABLE LENGTH                           
         SR    R4,R2               ASSUME LENGTH = L'ELEMENT - DISP.            
SFLD08   DS    0H                                                               
         LA    R2,FAELEM(R2)       R2=A(DATA)                                   
...
SFLD10   DS    0H                                                               
         ...
         A     RF,ONTRYA                                                        
         ST    RF,OSNDFLD                                                       
         TM    BYTE,X'80'          TEST IGNORE ROUTINE                          
         BO    SFLD12                                                           
...
SFLD12   DS    0H                                                               
         ...
         BZ    SNDFLDX                                                          
         LH    RF,DATALEN                                                       
         GOTO1 ASNDDATA,DMCB,MDELD,((RF),DATA)                                  
*                                                                               
SNDFLDX  DS    0H                                                               
         B     EXITY                                                            
         DROP  R3                                                               
...
SELEM06  DS    0H                                                               
         ...
         BZ    SELEM10                                                          
         A     RF,ONTRYA           YES - GO TO OVERLAY INSTEAD                  
         ST    RF,OSNDEL                                                        
         TM    BYTE,X'80'          TEST IGNORE ROUTINE                          
         BO    SELEM10                                                          
         MVI   LINKMODE,MSNDEL                                                  
...
SELEM12  CLI   MDLEN,MDLENX        TEST END OF TABLE                            
         BE    SNDELEMX                                                         
*                                                                               
         GOTO1 ASNDFLD,DMCB,MDELD,FAELEM                                        
*                                                                               
         XR    RF,RF                                                            
         IC    RF,MDLEN                                                         
         BXH   R3,RF,SELEM12                                                    
...
SREC02   CLI   0(R3),0             TEST END OF RECORD                           
         BE    SNDRECX                                                          
         XC    FAELEM,FAELEM       COPY ELEMENT TO FAELEM                       
         IC    RF,1(R3)                                                         
         BCTR  RF,0                                                             
         EX    RF,*+8                                                           
         B     *+10                                                             
         MVC   FAELEM(0),0(R3)                                                  
         GOTO1 ASNDELEM,DMCB,0,FAELEM                                           
         XR    RF,RF                                                            
         IC    RF,1(R3)                                                         
         BXH   R3,RF,SREC02                                                     
*                                                                               
...
CALLOVR  NTR1  ,                                                                
         L     RF,8(RD)                                                         
         ST    RF,SYSRD            MAKE SURE GENCON RETURNS HERE                
         GOTO1 ONTRYA                                                           
         IPM   RE                  SAVE CONDITION CODE                          
         CLI   ERROUT,0            TEST FOR GENCON ERROR                        
         BNE   *+6                 YES - KEEP CC NOT EQUAL                      
...
T60B70   CSECT                                                                  
OVRTAB   DS    0X                                                               
         DC    AL1(X'71'),AL2(MH#JOB)     JOB MAINTENANCE                       
OVRTABX  DC    AL1(EOT)                                                         
         EJECT                                                                  
* ACPROWORK                                                                     
*        PRINT OFF                                                              
...
SVEL     DS    XL2                 ELEMENT CODE (FIRST SENT FOR ACTION)         
...
ONTRYA   DS    A                   OVERLAY ENTRY POINT                          
*                                                                               
ERROUTH  DS    XL8                 DUMMY AREA FOR ERROR OUTPUT                  
ERROUT   DS    CL72                                                             
*                                                                               
         DS    (L'CONTWS-(*-CONTWS))X                                           
         SPACE 1                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'007ACPRO70   10/27/11'                                      
         END                                                                    
