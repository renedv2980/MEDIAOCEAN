*          DATA SET ACLDXBFM   AT LEVEL 053 AS OF 07/02/01                      
*PHASE ACLDXBFM,*                                                               
*INCLUDE ACRECTYP                                                               
*INCLUDE SORTER                                                                 
*INCLUDE HELEN                                                                  
*INCLUDE HEXOUT                                                                 
         TITLE 'ACLDXBFM EXTENAL'                                               
         SPACE 2                                                                
***********************************************************************         
...
DMXINIT  ZAP   LINE,MAXLINE                                                     
         ...
         L     R1,APARAMC      PARAM=CFFF                                       
         MVC   CPY,0(R1)       COMPANY CODE                                     
         PACK  DUB,1(3,R1)     FORMAT NUMBER (NNN)                              
         CVB   RF,DUB                                                           
         STC   RF,FMT                                                           
         B     DMXIT                                                            
*                                                                               
...
*        BE    BEDR                                                             
         USING BEDRECD,R2                                                       
         CLI   BEDKTYP,BEDKTYPQ                                                 
         BNE   DMXPURGE                                                         
         CLI   BEDKSUB,BEDKSUBQ                                                 
         BNE   DMXPURGE                                                         
         B     BEDR                                                             
*                                                                               
         USING BFMRECD,R2                                                       
BFMR     CLC   BFMKCPY,CPY                                                      
         BNE   DMXPURGE                                                         
         CLC   BFMKFMT,FMT                                                      
         BNE   DMXPURGE                                                         
         BAS   RE,PRNT                                                          
...
BEDR     CLC   BEDKCPY,CPY                                                      
         ...
         CLC   BLHFORM,FMT                                                      
         BNE   DMXPURGE                                                         
         MVC   EDKEY,BEDKEY        SAVE KEY OF EDIT RECORD                      
         B     BEDR5                                                            
*                                                                               
BEDR3    CLC   EDKEY,BEDKEY                                                     
         BNE   DMXPURGE                                                         
BEDR5    BAS   RE,PRNT                                                          
         B     DMXKEEP                                                          
...
PRNT     NTR1  ,                                                                
         L     R4,ARECTAB          POINT TO RECORD TABLE                        
         USING RECTABD,R4                                                       
DMXREC02 CLI   RECTREC,RECTEOTQ    END OF TABLE?                                
         ...
         B     DMXREC02                                                         
DMXREC04 GOTO1 HEXPRT,DMCB,RECTNAM,KEYLEN,AREC                                  
         GOTO1 VPRINTER                                                         
         L     R3,AREC                                                          
         AHI   R3,BFMRFST-BFMRECD  R3=A(FIRST ELEMENT ON RECORD)                
         USING ELD,R3                                                           
         SR    R0,R0                                                            
DMXREC06 CLI   ELCOD,0             END OF RECORD?                               
         BE    DMXREC12            EXIT FOR NEXT RECORD                         
         L     R4,AELETAB          POINT TO START OF ELEMENT TABLE              
         USING ELETABD,R4                                                       
...
*                                                                               
         ...
         EJECT                                                                  
SORTCARD DC    CL80'SORT FIELDS=(8,1,A,4,42,A),FORMAT=BI,WORK=1'                
...
RECTAB   DS    0X                  RECORD TYPE LOOK UP TABLE                    
         ...
         DC    AL1(ACRTBFM),CL8'ACRTBFM'                                        
         DC    AL1(ACRTBED),CL8'ACRTBED'                                        
*&&UK                                                                           
...
*        DC    AL1(RSFELQ),CL8'RSFEL'                                           
         DC    AL1(BFSELQ),CL8'BFSEL'       X'F4'                               
*        DC    AL1(SPYELQ),CL8'SPYEL'                                           
         DC    AL1(BFPELQ),CL8'BFPEL'       X'F5'                               
         DC    AL1(FWTELQ),CL8'FWTEL'                                           
         DC    AL1(BFMELQ),CL8'BFMEL'       X'F7'                               
         DC    AL1(BFXELQ),CL8'BFXEL'       X'F8'                               
         DC    AL1(RACELQ),CL8'RACEL'       X'F9'                               
         DC    AL1(PTRELQ),CL8'PTREL'                                           
         DC    AL1(DTSELQ),CL8'DTSEL'                                           
         DC    AL1(AGRELQ),CL8'AGREL'                                           
         DC    AL1(BTTELQ),CL8'BTTEL'       X'FD'                               
...
* DDDPRINT                                                                      
       ...
       ++INCLUDE DDDPRINT                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'053ACLDXBFM  07/02/01'                                      
         END                                                                    
