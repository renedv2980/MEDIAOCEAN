*          DATA SET SPEZF16    AT LEVEL 028 AS OF 05/01/02                      
*PHASE T23016A                                                                  
*INCLUDE SORTER                                                                 
*INCLUDE QSORT                                                                  
*                                                                               
***********************************************************************         
*                                                                     *         
*  TITLE: T23016 - EZ EXPRESS BILLING - READS FILE FROM EZLOAD        *         
*                                                                     *         
...
***********************************************************************         
*                                                                     *         
*  LEV  3    DEC12/94 CHANGE EDITS FOR # INVOICES & DOLLARS           *         
*  LEV  4    DEC13/94 CHANGE INVOICE TO MM- 5, NOT 6 DIGITS           *         
*  LEV  5    DEC13/94 TEMP FOR FOULED UP OCT07 LOAD OF AUG/SEP        *         
*  LEV  6    APR05/95 MAKE EDIT TO PCHRG 9, NOT 8                     *         
*  LEV  7    SEP19/95 SPECIAL BILLING FOR YTD NOTICE                  *         
*  LEV  8    OCT01/95 ALLOW AGENCY OR STATION BILLING ONLY            *         
*  LEV  9    OCT09/95 LARGER EDITS FOR DOLLARS                        *         
*  LEV 10    DEC08/95 MAKE MORE ROOM FOR REGISTER GROSS DOLLARS       *         
*  LEV 11    FEB06/96 FIX LINKED AGENCY PRINT OUT IN HEADING          *         
*  LEV 12    FEB09/96 READ IN RADIO/NET RADIO AS TV                   *         
*  LEV 13    MAR07/96 FIX EZBMEDIA                                    *         
*  LEV 14    MAR25/96 MOVE DATE CHECK BEFORE MEDIA CHECK              *         
*  LEV 15    APR17/96 TEMP FIX FOR MED R, PLUS PRINT DUPS             *         
*  LEV 16    APR25/96 ADD NEW CTRS, CHGE REGISTER PRINTOUT            *         
*  LEV 17    AUG09/96 READ MOVE RECS (ZM) CHANGE AGY NAME ACCORDINGLY *         
*  LEV 18    AUG30/96 ENLARGE MOVTAB - FROM 51,000 TO 102,000         *         
*  LEV 19    SEP11/96 SORT MOVTAB IN SEQ, AND CT ENTRIES AND USAGE    *         
*  LEV 20    OCT17/96 CHANGE . IN STATION CALL LETTERS TO BLANK       *         
*                     FIX INVOICE MOVE CHECKING                       *         
*  LEV 21    DEC02/97 BYPASS CERTAIN CABLE NETWORKS                   *         
*  LEV 22    JUL23/98 FIX CLEAR MOVE TABLE                            *         
*  LEV 23    NOV18/98 SHOW UNBILLED AGENCIES, SORT LINKED AGENCIES    *         
*  LEV 24    FEB02/99 ALLOW STATION BILLING, BIGGER TABLE             *         
*  LEV 25    MAR26/99 EXCLUDE WIM SOURCE FROM BILLING                 *         
*                     CHANGE SORT OF MOVED BATCHES                    *         
*  LEV 26    AUG11/00 ?                                               *         
*  LEV 27    MAY29/01 FIX DUMMY                                       *         
*                                                                     *         
***********************************************************************         
         TITLE 'T23016 - EZ EXPRESS BILLING'                                    
T23016   CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,T23016**,R7,RR=R2                                              
         L     RC,0(R1)                                                         
         USING GEND,RC                                                          
...
***********************************************************************         
         SPACE                                                                  
VKEY     CLI   ACTNUM,16          THIS HAD BETTER BE BILLING                    
         BE    *+12                                                             
         CLI   ACTNUM,ACTSEL                                                    
         BNE   INVAL                                                            
         SPACE                                                                  
...
VKXIT    XC    KEY,KEY                                                          
         ...
         EJECT                                                                  
***********************************************************************         
*   LIST - OPEN EZI.BILL FILE, SORT INVOICES FOR AGENCIES/STATIONS    *         
*          PRINT BILLS                                                *         
***********************************************************************         
         SPACE                                                                  
LIST     DS    0H                                                               
...
INIT010  ZAP   0(5,R1),=P'0'                                                    
         ...
         DC    H'0'                                                             
         SPACE                                                                  
* PUT OUT ONE HEADER FOR ENTIRE FILE *                                          
         SPACE                                                                  
         L     R1,=A(EZSOLM)                                                    
         L     R0,=A(HEADER)                                                    
         PUT   (1),(0)                                                          
...
BILTABLN EQU   (20000/L'BILENT)                                                 
         ...
         XCEF                                                                   
MOVTABLN EQU   (110000/L'MOVENT)    2000 ENTRIES                                
         SPACE                                                                  
...
LIST035  GOTO1 =V(QSORT),DMCB,AMOVTAB,(R5),L'MOVENT,10,1                        
         ...
         DC    H'0'                                                             
         SPACE                                                                  
* READ BILLING RECORDS FROM GENDIR/FIL, BUILD TABLE OF LINKED                   
         SPACE                                                                  
         XC    KEY,KEY                                                          
         LA    R4,KEY                                                           
         USING EZBKEY,R4                                                        
...
LIST040  CLC   KEY(2),KEYSAVE                                                   
         ...
         DC    H'0'                                                             
         SPACE                                                                  
LIST050  GOTO1 DATAMGR,DMCB,=C'DMRSEQ',=C'GENDIR',KEYSAVE,KEY                   
         B     LIST040                                                          
         SPACE                                                                  
...
LIST164  AP    CABYPASS,=P'1'                                                   
         ...
         SPACE                                                                  
LIST166  DS    0H                                                               
         CLC   =C'WIM',EZBSRC      BYPASS THIS SOURCE                           
         BNE   LIST168                                                          
         SPACE                                                                  
         AP    WIMBYPAS,=P'1'                                                   
         B     LIST100                                                          
...
CABNOBCT EQU   (*-CABNOBIL)/8                                                   
         SPACE                                                                  
         DS   0D                                                                
STATABLE DC 6000XL8'000000000000000F'                                           
STATABLN EQU   6000                                                             
         SPACE                                                                  
         DS   0D                                                                
...
*ORTCARD DC    CL80'SORT FIELDS=(1,17,A,22,6,A,32,4,A,42,25,A,92,10,A),C        
               FORMAT=BI,WORK=1 '                                               
SORTCARD DC    CL80'SORT FIELDS=(1,17,A,21,14,A,91,10,A),FORMAT=BI,WORKC        
               =1 '                                                             
...
* PRINT OUT BYPASSED TOTALS *                                                   
         ...
         FREEPOOL (R2)                                                          
         SPACE                                                                  
         MVC   P+5(21),=C'BYPASSED WIM INVOICES'                                
         EDIT  WIMBYPAS,(7,P+28),COMMAS=YES                                     
         SPACE                                                                  
         MVI   SPACING,2                                                        
         GOTO1 SPOOL,DMCB,(R8)                                                  
         SPACE                                                                  
* NOW PR NT INVOICE REGISTER                                                    
         SPACE                                                                  
         LM    R0,R1,=A(HEADINGR,HDHKR) NEW HEADINGS                            
         ST    R0,SPECS                                                         
         ST    R1,HEADHOOK                                                      
         SPACE                                                                  
...
DUPCTB   DS    PL5                 DUPLICATE BILLING RECORDS FOR BILL           
         SPACE                                                                  
EZSOLMCT DS    PL5                 MASTER                                       
EZSOLTCT DS    PL5                 TRANS                                        
NCTRS    EQU   (*-CTRS)/5                                                       
         SPACE                                                                  
FIRSTSW  DS    CL1                                                              
...
PREGDUP  DS    CL3                                                              
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'028SPEZF16   05/01/02'                                      
         END                                                                    
