*          DATA SET DECNSOR3   AT LEVEL 081 AS OF 11/22/13                      
...
*INCLUDE PRINTER                                                                
*INCLUDE SORTER                                                                 
         TITLE 'SORT CABLE MOVIE GOER TAPE BEFORE RUNNING CONVERSION'           
DECABSRT CSECT                                                                  
         PRINT NOGEN                                                            
         NBASE 0,DECABSRT,=V(REGSAVE),R8                                        
         USING MITMGD,R2             R2 = MIT DSECT                             
         L     R9,=V(CPRINT)                                                    
         USING DPRINT,R9                                                        
...
INP03    CLC   MITSEQ,=C'03'       TVU                                          
         ...
         MVC   MITPRG-MITMGD+4(6,R5),MITCOVG                                    
...
*        BNE   INPPURG                                                          
*        CLI   MITPUT,C'1'         NOT PROVIDED ON MOVIE GOER                   
*        BE    INPPURG                                                          
...
INP05    CLC   MITSEQ,=C'05'       TVU                                          
         BNE   INP99                                                            
         CLC   MITAVG,=C'05'       GET RID OF M-F,M-SU AVGS ON 03,05'S          
         BE    INPPURG             NONE ON MOVIE GOER TAPE                      
         CLC   MITAVG,=C'07'                                                    
         BE    INPPURG                                                          
         CLC   MITHLFID,SPACES     IS HH NUMBER PROVIDED?                       
         BNE   *+8                 YES                                          
...
INP99    CLC   MITSEQ,=C'99'       AUTHORIZATION RECDS                          
         BE    INPPURG                                                          
*                                                                               
INPXX    MVC   WORK(L'MITKEY),MITKEY                                            
...
MRG20    DS    0H                  DONE PROCESSING SORTED TAPE RECDS            
         ...
         BNE   MRGX                                                             
         L     R3,AREC                                                          
         CLI   MITORIG-MITMGD(R3),C'X' DELETE THIS RECD?                        
         BE    MRG30                                                            
*                                                                               
MRG25    DS    0H                                                               
         CLC   MITCORDT-MITMGD(7,R3),BLANKS <--REPLACE AFTER TESTING            
         BE    *+14                                                             
         CLC   MITCORDT-MITMGD(7,R3),=C'1000731'                                
         BL    MRG30               DELETE CORR RECDS < 7/31/00                  
*                                                                               
         AP    OUTCNT,=P'1'                                                     
         PUT   FILOUT,(R3)         PUT RECD TO OUTPUT TAPE                      
         CLC   MITCORDT-MITMGD(7,R3),BLANKS                                     
         BE    MRG30                                                            
         AP    OUTCNT2,=P'1'                                                    
...
*        GOTO1 =V(PRINTER)                                                      
         ...
         CLC   MITKEY(MITHLFID-MITMGD),0(RF)  CMP TILL RECTYP FLD               
         BE    CMB_P                                                            
...
CMB30    CLI   SRTORIG,C'0'                                                     
         ...
         BE    *+12                                                             
         MVC   WORK(L'SRTKEY),SRTKEY   HELP FOR DUMP LOOKUP                     
...
CMB50    CLI   MITREC,C'D'                                                      
         ...
         CLI   MITREC,C'P'                                                      
         BE    *+12                                                             
         MVC   WORK(L'MITKEY),MITKEY    HELP FOR DUMP LOOKUP                    
         DC    H'0'                UNEXPECTED RECD TYPE                         
         CLC   AK.MITKEY(MITREC-MITMGD),0(R2)   SAME AS RECD IN BUFFER?         
         BE    CMB_P                                                            
...
CMB70    DS    0H                                                               
         ...
         BNE   CMB72                                                            
         CLC   MITCORDT-MITMGD(7,R4),BLANKS                                     
         BE    *+14                                                             
         CLC   MITCORDT-MITMGD(7,R4),=C'1000731'                                
...
* MITPUT IS BLANK ON MOVIE GOER MIT                                             
...
*        AR    RF,R1               YES- PT TO PUT AREA IN AREC                  
         ...
         CLC   MITDEMG,=C'061'     DEMOS GROUP 21-40? (MOVIE GOER)              
         BE    CMB_P10                                                          
         CLC   MITDEMG,=C'081'                                                  
         BE    *+12                                                             
         MVC   WORK(L'MITKEY),MITKEY                                            
...
SRTREP   DS    CL1                 MIDUSRAB (REPORTABILITY INDICATOR)           
...
*                                                                               
       ...
       ++INCLUDE DEMITCMD                                                       
         ...
         SPACE 3                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'081DECNSOR3  11/22/13'                                      
         END                                                                    
