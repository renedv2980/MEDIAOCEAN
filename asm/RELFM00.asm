*          DATA SET RELFM00    AT LEVEL 234 AS OF 09/21/10                      
*PHASE T80400A                                                                  
*INCLUDE INVDAY                                                                 
*INCLUDE INVLST                                                                 
*INCLUDE DEMTIME                                                                
...
*                                                               *               
*  MAR15/03 (HQ ) --- ALLOW ALPHA NUMERIC COMBINATION IN        *               
*                     STATION CALL LETTER                       *               
*                                                               *               
*  JUN24/05 (BU ) --- ADD 'STAOFFC' AS RECORD TYPE. DDS         *               
*                     TERMINALS ONLY.                           *               
*                                                               *               
*  JUL21/05 (BU ) --- ADD 'STAPROP' AS RECORD TYPE. DDS         *               
*                     TERMINALS ONLY.                           *               
*                                                               *               
*  OCT17/05 (BU ) --- PROTECT MQUEUE FIELDS FOR STAD SCREEN     *               
*                     IF NOT DDS TERMINAL                       *               
*                                                               *               
*  FEB01/07 (BU ) --- ADD RECLIST ENTRY 'REPURL / SCREEN D4'    *               
*                                                               *               
*  MAY25/07 (SKU) --- ALLOW NON DDS TERM TO USE STAPRO          *               
*                                                               *               
*  JUL25/07 (BU ) --- CHANGE STAPRO PROCESSING TO INCLUDE       *               
*                     A CUTOFF DATE FOR MEDIA OCEAN NEEDS       *               
*                 --- SCREEN FOR STAPRO CHANGED TO D8 FROM D3   *               
*                                                               *               
*  SEP06/07 (BU ) --- ADD STAPLUG FACILITY                      *               
*                     SCREEN FOR STAPLUG IS D9                  *               
*                                                               *               
*  SEP10/07 (BU ) --- STAPLUG -> STAPALL                        *               
*                                                               *               
*  SEP20/07 (BU ) --- PROTECT SEND TO DDS FOR STAPAL SCREEN     *               
*                     IF NOT DDS TERMINAL                       *               
*                                                               *               
*  OCT23/09 (SMY) --- ADD "LOCKED" (Y/N) AGY SCREEN FIELD       *               
*                     FOR DDS TERMINALS ONLY                    *               
*                                                               *               
*  AUG04/10 (SMY) --- ADD RECLIST ENTRY 'STAALIAS / SCREEN CA'  *               
*                                                               *               
*                                                               *               
*                     ***  END TOMBSTONE  ***                   *               
*****************************************************************               
...
*                                                                               
         ...
         LR    R4,RA                                                            
*                                                                               
*   RECORD TYPE 'STAOFFC' IS TO BE USED FOR THE MIGRATION OF A REPPAK           
*        CLIENT TO MEDIAOCEAN.  AS SUCH, IT IS ISOLATED TO DDS                  
*        TERMINALS ONLY.  THIS CAN BE CHANGED BY COMMENTING                     
*        OUT THE FOLLOWIING SECTION OF CODE.                                    
*                                                                               
*                                                                               
*   RECORD TYPE 'STAPROP' IS TO BE USED FOR ASSIGNMENT OF PROPOSAL              
*        HEADERS TO EITHER DDS OR MO.  AS SUCH, IT IS ISOLATED TO DDS           
*        TERMINALS ONLY.  THIS CAN BE CHANGED BY COMMENTING                     
*        OUT THE FOLLOWIING SECTION OF CODE.                                    
*                                                                               
         CLI   TWAOFFC,C'*'        DDS TERMINAL?                                
         BE    MAIN0400            YES - PERMIT 'STAOFFC'/'STAPROP'             
*                                        AND 'STAPALL'                          
         L     R5,=A(STAOFFC)      NO                                           
         A     R5,BASERELO         ADD RELOCATION FACTOR                        
         MVC   0(2,R5),=X'0000'    INACTIVATE TABLE ENTRY                       
*        L     R5,=A(STAPROP)                                                   
*        A     R5,BASERELO         ADD RELOCATION FACTOR                        
*        MVC   0(2,R5),=X'0000'    INACTIVATE TABLE ENTRY                       
*        L     R5,=A(STAPALL)                                                   
...
KVAL0460 CLC   BSVSCRN,BSCRN       TEST HAVE RIGHT SCREEN                       
         ...
         SPACE 1                                                                
*                                                                               
         GOTO1 =A(OPNSPC01),DMCB,(RC),RR=BASERELO                               
...
*                                 THE PROFILE BIT IS "ON"                       
         ...
         NI    1(R2),X'FF'-X'08'-X'04'                                          
*                                  SET NORM INTENSITY                           
         FOUT  (R2)                                                             
         LA    R2,LF9MONMH                                                      
         NI    1(R2),X'FF'-X'20'-X'08'-X'04'                                    
...
*SMY*    BE    KVAL0730                                                         
         BNE   KVAL0730                                                         
         LA    R2,LAGHLOKH                                                      
         NI    1(R2),X'FF'-X'08'-X'04'   SET NORM INTENSITY                     
*SMY*    OI    1(R2),X'0C'        LOW INTENSITY                                 
         LA    R2,LAGDLOKH                                                      
*SMY*    OI    1(R2),X'20'        DATA FIELD PROTECTED                          
         NI    1(R2),X'FF'-X'20'         SET UNPROTECTED                        
*                                                                               
KVAL0730 DS   0H                                                                
         TM    SVPGPBIT+1,X'10'                                                 
...
KVAL0780 DS   0H                                                                
         ...
         SPACE 1                                                                
*                                                                               
         CLC   =C'STAPALL',LFMREC  PROPOSER STATION/OFFICE MAINT?               
         BNE   KVAL0790            NO                                           
         CLC   =C'DIS',LFMACT      PROPOSER DISPLAY REQUEST?                    
         BE    KVAL0790            YES                                          
         B     EXXMOD              NO  - EXIT MODULE                            
KVAL0790 EQU   *                                                                
         CLI   ERRAREA,0                                                        
         BNE   EXMODALT                                                         
         SPACE 1                                                                
...
*                                                                               
       ...
       ++INCLUDE FLDIND                                                         
         EJECT                                                                  
*********************************************************************           
*   OPNSPC01:  MOVED FOR ADDRESSABILITY                                         
*********************************************************************           
OPNSPC01 NMOD1 0,*OPSP01*                                                       
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         CLI   BSCRN,X'F7'         ADVERTISER SCREEN?                           
         BNE   KVAL0465            NO                                           
         TM    SVPGPBIT+1,X'40'    AGENCY PROFILE BIT/CONVERSION                
         BO    KVAL0463            YES                                          
         TM    SVPGPBIT+1,X'10'    AGENCY PROFILE BIT/CONVERSION                
         BO    KVAL0463            YES                                          
         OI    LADHAVQH+1,X'0C'    NO  - SET LOW INTENSITY                      
         FOUT  LADHAVQH                                                         
         OI    LADAVEQH+1,X'0C'    SET LOW INTENSITY                            
         FOUT  LADAVEQH                                                         
         B     KVAL0465                                                         
KVAL0463 EQU   *                                                                
...
*                                                                               
KVAL0467 EQU   *                                                                
         CLI   BSCRN,X'D0'         TEST FOR STAD SCREEN                         
         BNE   KVALX470                                                         
         USING TWAD,R4                                                          
         LR    R4,RA                                                            
         CLI   TWAOFFC,C'*'        DDS TERMINAL?                                
         BE    KVALX470            YES - PERMIT 'MQUEUE FIELDS'                 
         OI    SDTMQUAH+1,X'0C'    SET LOW INTENSITY                            
         FOUT  SDTMQUAH                                                         
         OI    SDTMQUBH+1,X'0C'    SET LOW INTENSITY                            
         FOUT  SDTMQUBH                                                         
         OI    SDTMQUEH+1,X'20'    PROTECT MQUEUE FIELDS                        
         FOUT  SDTMQUEH                                                         
         OI    SDTMQSPH+1,X'20'                                                 
         FOUT  SDTMQSPH                                                         
         DROP  R4                                                               
KVALX470 EQU   *                                                                
         CLI   BSCRN,X'D9'         TEST FOR STAPAL SCREEN                       
         BNE   KVALX480                                                         
         CLI   BACT,C'C'           FIRST TIME: CHANGE ACTION?                   
         BNE   KVALX475            NO                                           
         MVI   BACT,C'D'           YES - CHANGE TO 'DISPLAY'                    
         MVC   LFMACT,=C'DIS       '                                            
*                                  CLEAR OUT ACTION                             
         MVI   LFMACTH+5,3         SET LENGTH TO 3                              
         FOUT  LFMACTH                                                          
*                                                                               
KVALX475 EQU   *                                                                
         USING TWAD,R4                                                          
         LR    R4,RA                                                            
         CLI   TWAOFFC,C'*'        DDS TERMINAL?                                
         BE    KVALX480            YES - PERMIT 'STAPAL FIELD'                  
         OI    PLGSHDRH+1,X'0C'    SET LOW INTENSITY                            
         FOUT  PLGSHDRH                                                         
         OI    PLGSDDSH+1,X'20'    PROTECT STAPAL FIELD                         
         FOUT  PLGSDDSH                                                         
         DROP  R4                                                               
KVALX480 EQU   *                                                                
         XIT1                                                                   
...
KVAL0960 EQU   *                                                                
         MVC   LFMMSG(32),=C'*1 REQUESTED RECORD DISPLAYED **'                  
...
KVAL1060 CLI   BACT,C'L'                                                        
         BNE   KVAL1080                                                         
         MVC   LFMMSG(32),=C'*2 REQUESTED RECORD DISPLAYED **'                  
         LA    R2,LFMACTH                                                       
...
VSTAK012 DS    0H                                                               
         ...
         LA    R5,4                                                             
         LA    R1,BKEY+22                                                       
         BAS   RE,TESTA2           TEST AND MOVE FIELD                          
         TM    BYTE,X'02'          TEST INVALID CHARACTER                       
         BO    VSTAKEXH                                                         
         TM    BYTE,X'08'          TEST ALPHA CHARACTERS                        
         BZ    VSTAKEXH                                                         
*                                                                               
...
VSTAKEX  DS    0H                                                               
         ...
         LTORG                                                                  
         EJECT                                                                  
TESTA2   MVI   BYTE,0              CLEAR VALID A (X'04') AND N (X'08')          
         LA    R0,1(R5)            GET MAX LEN+1                                
TESTA21  CLI   0(R4),C' '                                                       
         BE    TESTA2X                                                          
         CLI   0(R4),0                                                          
         ...
         CLI   0(R4),C'Z'                                                       
         BH    TESTA22                                                          
         OI    BYTE,X'08'          FIELD CONTAINS ALPHA                         
         B     TESTA26                                                          
TESTA22  DS    0H                                                               
         CLI   0(R4),C'0'                                                       
         BL    TESTA24                                                          
         CLI   0(R4),C'9'                                                       
         BH    TESTA24                                                          
         OI    BYTE,X'04'          FIELD CONTAINS NUMERIC                       
         B     TESTA26                                                          
TESTA24  DS    0H                                                               
         OI    BYTE,X'02'          NOT ALPHA, NOT NUMERIC                       
TESTA26  MVC   0(1,R1),0(R4)                                                    
         LA    R1,1(R1)                                                         
         LA    R4,1(R4)                                                         
         BCT   R0,TESTA21                                                       
         B     ERRORALT                                                         
TESTA2X  BCTR  R0,0                ADJUST COUNT                                 
         SR    R5,R0               GIVES CHARACTER COUNT                        
         BR    RE                                                               
*                                                                               
         DS    0D                                                               
...
RECLIST  DC    H'14'               <-- ENTRY LENGTH                             
         ...
         DC    CL8'URLREP  ',X'17',X'E0',X'08',X'D4',AL1(NDXREP),X'C0'          
*                                                                               
...
*MN                                                                             
         DC    CL8'STAEMAIL',X'02',X'60',X'14',X'D7',AL1(NDXSTA),X'C0'          
*MN                                                                             
         DC    CL8'STAALIAS',X'02',X'60',X'14',X'CA',AL1(NDXSTA),X'C0'          
STAOFFC  DC    CL8'STAOFFC ',X'02',X'60',X'14',X'D3',AL1(NDXSTA),X'C0'          
STAPROP  DC    CL8'STAPROP ',X'02',X'60',X'14',X'D8',AL1(NDXSTA),X'C0'          
STAPALL  DC    CL8'STAPALL ',X'02',X'60',X'14',X'D9',AL1(NDXSTA),X'C0'          
*                                                                               
...
RAGENCY  DSECT                                                                  
       ++INCLUDE REGENAGY                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'234RELFM00   09/21/10'                                      
         END                                                                    
