*          DATA SET SPOTBOOK   AT LEVEL 113 AS OF 11/04/20                      
*PHASE T00A49B                                                                  
*INCLUDE GETBROAD                                                               
*INCLUDE ACTBKG                                                                 
*INCLUDE NSIWEEK                                                                
...
***********************************************************************         
*================================================================*              
* 01DEC09  AKAT LINK IN NEW VERSION OF SPCNCBLTAB                *              
* 08SEP08  AKAT SBQBOOK NOW 24 BYTES (6 BOOKS - 4 BYTES EACH)    *              
* 31MAY06  AKAT DO NOT HONOR OUT OF WEEK ROTATOR FOR AFFIDS      *              
* 16DEC05  AKAT CANADIAN SOFT DEMO OVERRIDES                     *              
* 07DEC05  AKAT RE-LINK TO PICK UP NEW SPACTBKG                  *              
* 06DEC05  AKAT FIX FOR OVERRIDING 00A PROFILE TO N/F/L/0        *              
* 14SEP05  AKAT WRITER SUPPORT TO OVERRIDE 00A PROF TO N/F/L/0   *              
* 25MAY05  AKAT FUSION SUPPORT                                   *              
* 08DEC04  AKAT LENGTH OF BOOK LIST ENTRY NOW 9 (LPM POSTING)    *              
* 26NOV03  AKAT CANADIAN SOFT DEMOS AS PER BZEH                  *              
* 22OCT03  AKAT CALL ACTBKG TWICE TO GET MORE ENTRIES IN         *              
*               SBABKLST FOR UP TO TWO YEARS IF THE DATE RANGE   *              
*               (STDATE-ENDATE) IS GREATER THAN 13 MONTHS (13    *              
*               BECAUSE ACTBKG CALLS MOBILE TO GET 13 MONTHS)    *              
...
BOOK2    LM    R2,R3,SBADATE       R2=A(REQUEST DATE LIST),R3=N'ENTRIES         
         ...
         MVC   OPTDEMO,SBEDEMTY    DEMO TYPE                                    
*                                                                               
...
*                                                                               
         ...
         BE    REG1                                                             
         TM    SBEFLAG9,SBE9CMBK   SET BOOK FORCE BOOK FOR COMSCORE?            
         BZ    *+12                NO - CLEAR LOOKBOOK                          
         OI    SBEFLAG9,SBE9CERR   FLAG FORCED BOOK ERROR                       
         B     REG1                LEAVE LOOKBOOK INTACT                        
         XC    LOOKBOOK,LOOKBOOK   BOOK IS NOT ON FILE                          
*                                                                               
REG1     L     R4,SBABKLST         R4=A(OUTPUT)                                 
...
ACT01    DS    0H                                                               
         ...
         MVC   ABKEND,ENDATE                                                    
         MVC   ABKBOOK,SBQBOOK                                                  
         XC    ABKBOOK+4(2),ABKBOOK+4  ONLY MOVE 1ST 4 BYTES OF SBQBOOK         
         ...
         MVI   ABKBOOK+3,C' '                                                   
*                                                                               
         MVC   ABKRRTYP,SBEDEMTY                                                
         CLI   ABKRRTYP,C'A'                                                    
         BNE   *+8                                                              
         MVI   ABKRRTYP,C'I'                                                    
         TM    SBINDS,SBIRSOVR     TEST RATING SERVICE OVERRIDE                 
         BO    *+10                                                             
         MVC   ABKMKT,SBBMKT     NO-PASS AGENCY MARKET NUMBER                   
*                                                                               
         CLI   MEDIA,C'R'          FIX FOR RADIO RE-RATE                        
         BNE   *+10                                                             
         MVC   ABKSPMKT,SBMKNUM                                                 
*                                                                               
         OC    SBEMKT,SBEMKT       TEST FOR REQUESTED MARKET                    
         BZ    ACT02               NO                                           
         CLC   SBEMKT,SBBMKT       TEST FOR SPILL LOOKUP                        
         BE    ACT02               NO                                           
         MVC   ABKSPMKT,SBBSPL     YES-SET SPILL MKT RTG SRV NUMBER             
*                                  NO-PASS SPILL AGENCY MARKET NUMBER           
         TM    SBINDS,SBIRSOVR     TEST RATING SERVICE OVERRIDE                 
         BO    ACT02                                                            
         MVC   ABKMKT,SBEMKT                                                    
ACT02    DS    0H                                                               
         MVC   ABKSTAT,SBSTA                                                    
         MVC   ABKS0PRF,SBSPPROF                                                
         CLI   SBEDEMTY,C'A'       DOING AFFIDS?                                
         BNE   *+8                 NO                                           
         MVI   ABKS0PRF+8,0        YES - DONT HONOR OUT OF WEEK ROTATOR         
         MVC   ABK1WPRF,SB1WPROF                                                
         MVC   ABKD0PRF,SBD0PROF                                                
         MVC   ABKCMFAC,SBCOMFAC                                                
         MVC   ABKGTBRD,VGETBRD                                                 
         MVC   ABKADDAY,VADDAY                                                  
         MVC   ABKGTDAY,VGETDAY                                                 
         MVC   ABKDATCN,VDATCON                                                 
         MVC   ABKMOBIL,VMOBILE                                                 
         MVC   ABKDEMND,CDEMAND                                                 
         MVC   ABKOPT1,SBD0PROF+2                                               
         L     RE,=V(NSIWEEK)                                                   
         A     RE,RELO                                                          
         ST    RE,ABKNSIWK                                                      
         BRAS  RE,GETAMKT                                                       
         MVC   ABKALFM,ALPHAMKT                                                 
                                                                                
         TM    SBEFLAG2,SBEWTP          WEEKLY WTP OPTION?                      
         BZ    *+8                                                              
         MVI   ABKD0PRF+15,C'W'         PASS THIS TO ACTBKG                     
                                                                                
*&&DO                                                                           
***                                                                             
* TEST CANADIAN SOFT DEMO LOOKUPS                                               
***                                                                             
         CLI   ABKCTRY,C'C'         CANADA?                                     
         BNE   ACT02AA              NO                                          
*                                                                               
         CLI   ABKS0PRF+15,C'Y'     TEST SOFT DEMO LOOKUPS                      
         BNE   ACT02AA              NO                                          
*                                                                               
         ICM   RE,7,SBASTANT        STA BUFFER EMPTY?                           
         BZ    ACT02AA              YES                                         
         USING STABUFFD,RE                                                      
         MVC   ABKSTAT,STBCALL1     NSI CALL LETTERS                            
         CLI   SBCPROF+3,C'0'       TEST NSI CLIENT                             
         BE    *+10                                                             
         MVC   ABKSTAT,STBCALL2     ELSE USE BBM/ARB CALL LETTERS               
         MVI   ABKSTAT+4,C'T'                                                   
         CLI   ABKSTAT,C' '                                                     
         BH    *+10                                                             
         MVC   ABKSTAT,SBSTA        SAVE CURRENT CALL LETTERS                   
         DROP  RE                                                               
*                                                                               
         OC    DBSELMK,DBSELMK      ANY SPILL MARKET NUMBER?                    
         BNZ   ACT02AA              YES                                         
         LA    RE,SBMKTREC                                                      
         USING MKTREC,RE                                                        
         OC    MKTALST(3),MKTALST   ANY MARKET ALPHA CODES?                     
         BZ    ACT02AA              NO                                          
         MVC   ABKALFM,MKTALST                                                  
         DROP  RE                                                               
*&&                                                                             
ACT02AA  CLI   MEDIA,C'R'           RADIO?                                      
         BNE   ACT02AB              NO                                          
         LA    RE,SBMKTREC                                                      
         USING MKTREC,RE                                                        
         OC    MKTALST(3),MKTALST   ANY MARKET ALPHA CODES?                     
         BZ    ACT02AB              NO                                          
         MVC   ABKALFM,MKTALST      PASS ALPHA MARKET                           
         XC    ABKSPMKT,ABKSPMKT    AND NOT THE SPILL MARKET                    
         DROP  RE                                                               
*                                                                               
ACT02AB  BAS   RE,FUSION             CABLE?                                     
         BNE   ACT02AC               NO                                         
*                                                                               
         CLI   WORK+16,C'N'          NSI?                                       
         BNE   *+8                   NO                                         
         MVI   ABKBTYP,C'W'          NEEDS TO BE SET FOR CABLE NSI              
         MVC   ABKSRC,WORK+16                                                   
         MVC   ABKALFM,WORK+17                                                  
***                                                                             
* CALL PERVERT TO SEE IF WE HAVE A DATE RANGE OF > 1 YEAR                       
***                                                                             
ACT02AC  XR    R2,R2                R2 HOLDS DISPLACEMENT INTO SBABKLST         
         MVC   SVSTART,STDATE                                                   
         GOTO1 CPERVERT,DMCB,STDATE,ENDATE                                      
         LA    R0,0                                                             
         ICM   R0,3,14(R1)          GET MONTHS                                  
         CHI   R0,13                MORE THAN 13 MONTHS?                        
         BNH   ACT02A               NO                                          
***                                                                             
* CALL SPACTBKG TWICE                                                           
***                                                                             
         LA    R0,364                                                           
         GOTO1 CADDAY,DMCB,SVSTART,SVSTART,(R0)       END = START+364           
         GOTO1 CDATCON,DMCB,SVSTART,(X'02',ABKBEN)    2-BYTE END DATE           
         GOTO1 CDATCON,DMCB,(X'02',ABKBEN),ABKEND     6-BYTE END DATE           
         LA    R0,1                                                             
         GOTO1 CADDAY,DMCB,SVSTART,SVSTART,(R0)       2ND START DATE            
*                                                                               
ACT02A   GOTO1 =V(ACTBKG),DMCB,ABKAREA,RR=RELO                                  
         LA    R1,ABKOLIST         POINT TO BOOK LIST                           
         L     RE,SBABKLST         POINT TO OUTPUT LIST                         
         AR    RE,R2               MIGHT BE SECOND TIME THROUGH                 
         SR    R0,R0                                                            
BLDOLST  MVC   0(LNBKENT,RE),0(R1) MOVE THEM OUT                                
*                                                                               
         TM    SBEFLAG9,SBE9CMBK   ALWAYS SET BOOK FOR COMSCORE?                
         BZ    BLDOLST1            NO - LEAVE BOOK AS IS                        
         OC    4(2,RE),4(RE)       DO WE HAVE A BOOK?                           
         BNZ   BLDOLST1            YES                                          
         MVC   4(2,RE),=X'FFFF'    NO - SET TO X'FFFF' FOR COMSCORE             
*                                                                               
BLDOLST1 BAS   R6,OLYF02           CHECK FOR OLYMPICS                           
         LA    RE,LNBKENT(RE)                                                   
         LA    R2,LNBKENT(R2)      R2 SAVES DISPLACEMENT FROM SBABKLST          
         LA    R1,LNBKENT(R1)                                                   
         OC    0(2,R1),0(R1)                                                    
         BZ    BLDOLST2                                                         
         BCT   R0,BLDOLST          COUNT THEM                                   
*                                                                               
BLDOLST2 LPR   R0,R0                                                            
         AHI   R0,1                                                             
         L     RE,SBNBKS                                                        
         AR    R0,RE                                                            
         ST    R0,SBNBKS           SAVE COUNT                                   
***                                                                             
* IF THE END DATE = ORIGINAL END DATE THEN WE ARE DONE HERE, OTHERWISE          
* CALL SPACTBKG 1 MORE TIME WITH 2ND YEAR AND CONTINUE FILLING IN TABLE         
***                                                                             
         CLC   SVSTART,STDATE      FINISHED FILLING TABLE?                      
         BE    BOOKX               YES                                          
         GOTO1 CDATCON,DMCB,SVSTART,(X'02',ABKBST)   2-BYTE START DATE          
         GOTO1 CDATCON,DMCB,(X'02',ABKBST),ABKSTART  6-BYTE START DATE          
         MVC   ABKBEN,OPTEND                                                    
         MVC   ABKEND,ENDATE                                                    
         MVC   SVSTART,STDATE      SO WE CALL SPACTBKG 1 MORE TIME              
         B     ACT02A                                                           
*                                                                               
         DS    H'0'                                                             
*                                                                               
OLYF02   CLI   SBD0PROF+7,C'Y'    TEST TO USE OLY EXCL DATA AT ALL              
         JE    *+10                                                             
         CLI   SBD0PROF+7,C'F'                                                  
         BNER  R6                                                               
         ...
         BER   R6                                                               
* BOOK IS FEB/2002                                                              
OLYF02C  CLC   4(2,RE),=X'6602'                                                 
         BNER  R6                                                               
* TEST IF DATES ARE OUTSIDE OF B'CAST FEB/02                                    
OLYF02X  CLC   0(2,RE),=X'CC58'    TEST STARTS AFTER 020224                     
         JH    OLYF02X2             YES                                         
         CLC   2(2,RE),=X'CC3C'    TEST ENDS BEFORE 020128                      
         JL    OLYF02X2             YES                                         
         BR    R6                                                               
*                                                                               
OLYF02X2 OI    5(RE),X'80'         SET BOOK TO X'82'                            
         BR    R6                                                               
         EJECT                                                                  
*&&DO                                                                           
*                                                                               
...
*                                                                               
         XC    DBLOCK,DBLOCK                                                    
         OI    DBVOPT,X'40'        SPECIFY TO DEMAND SPOT POSTING               
*                                  CALLER.  NEEDED FOR LIVE ONLY                
         MVC   DBFILE,=C'TP '                                                   
         ST    R9,DBCOMFCS                                                      
         LA    R1,IOAREA2                                                       
         ST    R1,DBAREC                                                        
...
VALSTA2  CLI   DBSELMED,C'T'       REDIRECT ARB TO NSI AFTER 1993               
         ...
         MVI   DBSELSRC,C'N'                                                    
*                                                                               
VALSTA3  CLI   DBSELMED,C'C'       USE BBM/CSI WEEKLY IF AVAIL                  
         BNE   VALSTA4                                                          
         MVI   DBBEST,C'M'         USE WEEKLY FOR MONTH                         
         MVI   DBUSEBBM,C'Y'       USE BBM WEEKLY                               
*                                                                               
VALSTA4  MVC   DBSELSYC,SBSYSCD                                                 
*                                                                               
         BAS   RE,FUSION                                                        
         BNE   VALSTA4A                                                         
*                                                                               
         MVC   DBSELSRC,WORK+16                                                 
         MVC   DBSELALF,WORK+17                                                 
*                                                                               
VALSTA4A LA    RE,SBAGYREC                                                      
         USING AGYHDR,RE                                                        
         CLI   AGYPROF+7,C'C'      CANADIAN AGENCY?                             
         BNE   VALSTA4B            NO                                           
         DROP  RE                                                               
*                                                                               
         OC    SBCANALF,SBCANALF   HAVE ALPHA OVERRIDE?                         
         BZ    VALSTA4B            NO, CONTINUE                                 
         MVC   DBSELALF,SBCANALF                                                
*                                                                               
VALSTA4B GOTO1 CDEMAND,DMCB,DBLOCK,0                                            
         MVC   DBSELSRC,SOURCE     RESTORE SOURCE                               
         CLI   DBSELSRC,C'N'       NO NSI JUN09 BK-DIGITAL TRANSITION           
         BNE   *+8                 ALWAYS ALLOW IT TO PASS THROUGH              
         CLI   DBSELMED,C'T'       SO WE CAN SWITCH LOOKUPS IN GETDEMO          
         BNE   *+14                                                             
         CLC   DBSELBK,=X'6D06'                                                 
         BE    VALSTA5                                                          
*                                                                               
...
*                                                                               
         XC    WORK,WORK                                                        
         OC    SBSYSCD,SBSYSCD     CABLE?                                       
         BZ    FUSNEQ              NO, SUPRESS LOOKUP                           
*                                                                               
         MVC   WORK(4),=C'S00A'                                                 
         NI    WORK,X'BF'          MAKE 'S' LOWERCASE                           
         MVC   WORK+4(2),SBAGY                                                  
*                                                                               
         GOTO1  CGETPROF,DMCB,(X'90',WORK),WORK+16,CDATAMGR                     
         MVC   ABK0APRF,WORK+16    PASS 00A PROFILE                             
*                                                                               
         LA    RE,SBMKTREC                                                      
         USING MKTREC,RE                                                        
*                                                                               
         ...
         BZ    FUS05               NO                                           
         MVI   WORK+16,C'F'        YES - THIS OVERRIDES PROF & MKTCDEM          
         B     FUSEQU                                                           
*                                                                               
FUS05    TM    CBLFLAG,CFNLS       OVERRIDE TO NIELSON                          
         BZ    FUS10               NO                                           
         MVI   WORK+16,C'N'        YES - THIS OVERRIDES PROF & MKTCDEM          
         B     FUSEQU                                                           
*                                                                               
FUS10    TM    CBLFLAG,CFLPM       OVERRIDE TO NIELSON FOR LPM?                 
         BZ    FUS15               NO                                           
         MVI   WORK+16,C'N'        YES - THIS OVERRIDES PROF & MKTCDEM          
         B     FUSEQU                                                           
*                                                                               
FUS15    TM    CBLFLAG,CFNONE      OVERRIDE TO NO CABLE                         
         BNZ   FUSNEQ              YES - WE DON'T WANT CABLE                    
*                                                                               
FUS20    CLI   WORK+16,C'N'        NSI DEFAULT?                                 
         BE    *+12                YES                                          
         CLI   WORK+16,C'F'        FUSION?                                      
         BNE   FUSNEQ              NO, SUPRESS LOOKUP                           
*                                                                               
         CLI   MKTCDEM,C'N'        NSI?                                         
         BE    *+12                YES, OVERRIDE 00A PROFILE                    
         CLI   MKTCDEM,C'F'        FUSION?                                      
         BNE   FUSEQU              NO, 00A IS DEFAULT                           
*                                                                               
         MVC   WORK+16(1),MKTCDEM  SAVE SOURCE FROM THE MARKET RECORD           
*                                                                               
FUSEQU   MVC   WORK+17(3),MKTALST                                               
         DROP  RE                                                               
         CR    RB,RB               SET CC EQ                                    
         B     EXIT                                                             
*                                                                               
FUSNEQ   LTR   RB,RB               SET CC NEQ                                   
         B     EXIT                                                             
*&&DO                                                                           
*===================================================================*           
...
*                                                                               
         OC    SBCANALF,SBCANALF   HAVE ALPHA OVERRIDE?                         
         BZ    GAMK05              NO, CONTINUE                                 
         MVC   ALPHAMKT,SBCANALF                                                
         B     GAMKX                                                            
*                                                                               
         USING BUYREC,R1                                                        
GAMK05   L     R1,SBAIO1                                                        
...
OPTQBOOK DS    CL4                 REQUESTED BOOK                               
...
SWEEP2ND DS    XL2                                                              
*                                                                               
LNBKENT  EQU   9                   LENGTH OF ACTBKG OUTPUT ENTRY                
...
* SPGENBUY                                                                      
       ...
       ++INCLUDE SPGENBUY                                                       
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'113SPOTBOOK  11/04/20'                                      
         END                                                                    
