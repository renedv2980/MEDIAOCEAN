*          DATA SET RERES08    AT LEVEL 027 AS OF 05/19/08                      
*PHASE T81908A                                                                  
*INCLUDE CLPACK                                                                 
         TITLE 'T81908 - RERES08 - MARKET PROFILE REPORT'                       
*                                                                               
*******************************************************************             
*                                                                 *             
*        RESFM0C (T81908) --- MARKET PROFILE REPORT               *             
*                                                                 *             
* --------------------------------------------------------------- *             
* UPDATE HISTORY:                                                 *             
*                                                                 *             
...
*                    HARD.                                        *             
*                                                                 *             
* NOV09/94 (BU ) --- MAKE SPOT SYSTEM NUMBER 'SOFT' RATHER THAN   *             
*                    HARD.                                        *             
*                                                                 *             
* NOV16/94 (BU ) --- FOOL SPOT ASSIGNMENT                         *             
*                                                                 *             
...
****************************************************************                
         SPACE 1                                                                
VKEY     DS    0H                                                               
         LA    RE,COMWORK           CLEAR OUT COMMON SAVE AREA                  
         LA    RF,SYSSPARE                                                      
         SR    RF,RE                                                            
         XCEF                                                                   
         SPACE 1                                                                
...
VK40     LA    R2,MPRBKSH                                                       
         ...
         B     MYEND                                                            
         SPACE 1                                                                
VK42     LA    R3,BOOKS            HISTORICAL BOOKS ONLY -                      
         ZIC   R1,ACTUAL                                                        
...
VK70     MVI   ERROR,NOTFOUND                                                   
         ...
         MVI   ELCODE,X'01'                                                     
         BAS   RE,GETEL                                                         
         MVC   NUMDEMS,62(R6)      SAVE NUMBER OF DEMOS                         
         MVI   ELCODE,X'02'                                                     
         LA    R6,IO                                                            
         BAS   RE,GETEL                                                         
         ...
         EX    RE,*+8                                                           
         B     *+10                                                             
         MVC   DEMOS(0),2(R6)         SAVE DEMOS                                
         SPACE 1                                                                
         CLI   COP2,C'Y'                                                        
         BNE   VK75                                                             
         CLI   NUMDEMS,18          MAXIMUM OF 18 DEMOS FOR WIDE                 
         BNH   VK110                                                            
         MVI   NUMDEMS,18                                                       
         LA    R1,19                                                            
         B     VK77                                                             
         SPACE 1                                                                
VK75     CLI   NUMDEMS,13          MAXIMUM OF 13 DEMOS                          
         BNH   VK110                                                            
         MVI   NUMDEMS,13                                                       
         LA    R1,13                                                            
VK77     MH    R1,=H'3'                                                         
         ...
         B     VK110                                                            
         SPACE 1                                                                
VK80     MVI   NFLDS,4             INDICATE 4 LINES OF DEMOS ON SCREEN          
         MVI   MAX,20              FUDGE SO I CAN DO ERROR MESSAGE              
         GOTO1 ANY                                                              
         GOTO1 VVALDEM                                                          
...
VK110    LA    R2,MPRNDEXH                                                      
         CLI   5(R2),0                                                          
         BE    VK150                                                            
         MVC   BLOCK1(8),BOOKS       SAVE 1ST AND 2ND BOOK                      
         ...
         B     MYEND                                                            
         MVC   CINDEX,BOOKS        SAVE INDEX BOOK                              
         MVC   BOOKS(8),BLOCK1     RESTORE 1ST AND 2ND BOOK                     
         SPACE 1                                                                
****************************************************************                
...
****************************************************************                
*    VALIDATE OPTIONS FIELD  (OPTIONAL)                        *                
*   OPTION 2 - Y=WIDE PRINT  (DEFAULT=N)                       *                
****************************************************************                
         SPACE 1                                                                
VK170    LA    R2,MPROP2H                                                       
         MVI   COP2,C'N'                                                        
         ...
         B     VK190                                                            
****************************************************************                
*    VALIDATE DEMO CALCULATION BASIS (DEFAULT = R)             *                
*   OPTION 3 - DMA=R(ATINGS)*, DMA=I(MPRESSIONS)               *                
****************************************************************                
         SPACE 1                                                                
VK190    LA    R2,MPROP3H                                                       
         MVI   COP3,C'R'           OPTION IS 'RATINGS'                          
         CLI   5(R2),0             ANY VALUE IN FIELD?                          
         BE    VK900               NO                                           
         ZIC   RE,5(R2)            LENGTH OF FIELD                              
         BCTR  RE,0                SUBTRACT 1                                   
         LTR   RE,RE               ANY LENGTH LEFT?                             
         BNZ   VK300               YES - > 1 CHAR = ERROR                       
         CLI   8(R2),C'R'          IS IT 'RATINGS'?                             
         BE    VK900               YES                                          
         MVI   COP3,C'I'           NO  - 'IMPRESSIONS'?                         
         CLI   8(R2),C'I'                                                       
         BE    VK900               YES                                          
VK300    EQU   *                   UNRECOGNIZED VALUE                           
         MVI   ERROR,INVALID                                                    
         B     ERREND                                                           
         SPACE 2                                                                
...
VUP30    MVC   CONHEAD+10(L'BADOBOOK),BADOBOOK                                  
         ...
         STC   RE,ELEM                                                          
         SPACE 1                                                                
         GOTO1 BOOKVAL,DMCB,(C'N',ELEM),(1,DUB),(C'B',SCANNER),        X        
...
TWADCOND DSECT                                                                  
       ...
       ++INCLUDE DDTWADCONS                                                     
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'027RERES08   05/19/08'                                      
         END                                                                    
