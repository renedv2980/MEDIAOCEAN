*          DATA SET RECNT41    AT LEVEL 088 AS OF 05/01/02                      
*PHASE T80241A,+0                                                               
         TITLE 'T80241 - RECNT41 - SPL DISPLAY/EDIT'                            
**********************************************************************          
*                                                                    *          
*        RECNT41 --- SPL/COMPETITIVE DISPLAY/EDIT                    *          
*                                                                    *          
* ------------------------------------------------------------------ *          
* UPDATE HISTORY:                                                    *          
*                                                                    *          
* MAR03/89 (MRR) --- ADD PERCENTAGE TO DOLLAR DISPLAY, SHOW          *          
*                     'N/A' IF ENTERED                               *          
*                                                                    *          
* CNT HISTORY:                                                       *          
*                                                                    *          
* 03AUG90  (EFJ) --- COMBINE WITH CNT42                              *          
*                                                                    *          
* 09JAN92  (SKU) --- STORED COMMENT SUPPORT                          *          
*                    *** THIS CHANGE NOT RELEASED YET ***            *          
*                                                                    *          
* OCT05/92 (BU ) --- STORE PERCENTS ONLY.  INCORPORATE REP'D STATION *          
*                    DOLLAR OVERRIDE.                                *          
*     NOTE:  !!!!  ++INCLUDE RECNTFCAD MUST BE CHANGED TO RECNTFCD!!!*          
*                                                                    *          
* FEB25/93 (BU ) --- FIX DIVIDE-BY-ZERO ERROR IN CALCPERC ROUTINE    *          
*                                                                    *          
* MAR03/93 (BU ) --- SET TRUE ACTIVITY DATE DISPLAY FROM X'08' ELT   *          
*                                                                    *          
* JUL01/93 (BU ) --- FIX CALCPERC ROUTINE FOR HUGE NUMBERS           *          
*                                                                    *          
* MAY09/94 (SKU) --- ADD FORECAST$ AND BUDGET$                       *          
*                                                                    *          
* DEC08/94 (BU ) --- EXPAND COMPETITIVE STATIONS FROM FIVE TO 11.    *          
*                                                                    *          
* OCT09/95 (SKU) --- 2K CONTRACT SUPPORT                             *          
*                                                                    *          
* OCT03/96 (SKU) --- SUPPORT LOW POWER STATION                       *          
*                                                                    *          
* JUL23/97 (SKU) --- 4K CONTRACT SUPPORT                             *          
*                    **  END TOMBSTONE  **                           *          
**********************************************************************          
T80241   CSECT                                                                  
         PRINT NOGEN                                                            
         ...
         LR    RF,RA                                                            
         AH    RF,=Y(TWAWORKQ)     4K                                           
         ST    RF,ATWAWORK         SAVE A(TWAWORK)                              
         ST    RF,SAVUSING         SAVE A(TWAWORK)                              
         L     R2,4(R1)                                                         
         CLC   =C'DISP',0(R2)                                                   
         BE    DISP                                                             
         CLC   =C'EDIT',0(R2)                                                   
...
DISP     DS    0H                                                               
         ...
         BO    ERROR                                                            
         SPACE 1                                                                
         CLI   RCONTYPE,C'B'       IS CONTRACT TYPE 'B'?                        
         BNE   DISP0010            NO                                           
         LA    R2,CONBACTH         YES - ERROR                                  
         LA    R3,191              NO SPL/EPL ON TYPE B K'S                     
         B     ERROR                                                            
         SPACE 1                                                                
DISP0010 DC    0H'0'                                                            
         GOTO1 VFOUTBLK,DMCB,SPLFCASH,SPLLAST,0                                 
         GOTO1 =A(TRANOUT),DMCB,(RC),SPLFCASH,SPLLAST,RR=Y                      
         EJECT                                                                  
*                                                                               
* DISPLAY FORECAST $ AND BUDGET $                                               
*                                                                               
         OI    SPLFCAHH+1,X'0C'    LOW INTENSITY                                
         OI    SPLFCASH+1,X'0C'+X'20' PROTECTED                                 
         OI    SPLBGEHH+1,X'0C'    LOW INTENSITY                                
         OI    SPLBGETH+1,X'0C'+X'20' PROTECTED                                 
                                                                                
         L     RF,ATWAWORK                                                      
         USING TWAWORK,RF                                                       
         TM    TWATIME,X'20'       REP OPTION TO USE ALTERNATE SPL SCRN         
         BZ    DSPL0000                                                         
         DROP  RF                                                               
                                                                                
         NI    SPLFCAHH+1,X'FF'-X'0C'    NORMAL INTENSITY                       
         NI    SPLFCASH+1,X'FF'-X'0C'-X'20' UNPROTECTED                         
         NI    SPLBGEHH+1,X'FF'-X'0C'    NORMAL INTENSITY                       
         NI    SPLBGETH+1,X'FF'-X'0C'-X'20' UNPROTECTED                         
...
DISP0011 DS    0H                                                               
         EDIT  RCONSXFD,(8,SPLFCAS),ALIGN=LEFT,ZERO=NOBLANK                     
...
DISP0014 DS    0H                                                               
         EDIT  RCONSXBD,(8,SPLBGET),ALIGN=LEFT,ZERO=NOBLANK                     
                                                                                
DISP0015 DS    0H                                                               
         OI    SPLFCASH+4,X'20'    VALIDATED PREVIOUSLY                         
         OI    SPLBGETH+4,X'20'    VALIDATED PREVIOUSLY                         
*                                                                               
*   BEGIN DISPLAY OF SPL OF SCREEN...                                           
*                                                                               
DSPL0000 DS    0H                                                               
...
DSPL0120 EQU   *                                                                
         ...
         SR    R3,R3                                                            
*                                                                               
*   FIND THE X'08' ELEMENT FOR TRUE ACTIVITY DATE                               
*                                                                               
DSPL0130 CLI   0(R2),0             END OF RECORD?                               
         BE    DSPL0150            YES - LOOK FOR X'08' ELEMENT                 
         CLI   0(R2),8             NO  - AUTOCLOSE/TRUE ACT DATE ELT?           
         BE    DSPL0140            YES                                          
...
*                                                                               
         ...
         LA    R2,SPLSCM5H                                                      
         SPACE 1                                                                
DSPL0220 MVC   WORK3(80),MYSPACES                                               
...
*                                                                               
         GOTO1 =A(PREVALID),DMCB,(RC),SPLFCAHH,SPLLAST,0,0,RR=Y                 
*                                                                               
         BAS   RE,TESTDISP         DISPLAY CONTROL INFORMATION                  
         MVC   CONCACT,MYSPACES    TURN OFF DISPLAY ACTION                      
...
EAMT0010 EQU   *                                                                
         ...
         B     EAMT0030                                                         
*                                                                               
EAMT0020 EDIT  (R3),(7,0(R8)),ALIGN=LEFT                                        
EAMT0030 EQU   *                                                                
         XIT1                                                                   
*                                                                               
EDITPRCT NTR1                                                                   
...
*                                                                               
         GOTO1 =A(PREVALID),DMCB,(RC),SPLSMTHH,SPLSCM8H,1,0,RR=Y                
*                                  IGNORE RETURN CODE                           
*                                                                               
ESPL0010 DS    0H                                                               
...
*                                                                               
ESPL0440 EQU   *                                                                
         LA    R2,SPLSCM5H         CHECK FOR MANDATORY NEW COMMENT              
         CLI   5(R2),0                                                          
...
ESPL0490 DS    0H                                                               
         ...
         EJECT                                                                  
ESPL0500 LA    R2,SPLSCM5H         COMMENTS                                     
         CLI   5(R2),0          COMMENT REQUIRED IF REP STATION IS $0           
         BNE   ESPL0510                                                         
...
ESPL0520 TM    4(R2),X'20'         PREVIOUSLY VALID?                            
         ...
         BCT   R3,ESPL0520         CHECK EACH HEADER                            
         B     ESPL0580                                                         
         SPACE 1                                                                
ESPL0530 DS    0H                                                               
         L     R8,AIO4                                                          
...
LPROT    EQU   *-PROTFLDS                                                       
         ...
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'088RECNT41   05/01/02'                                      
         END                                                                    
