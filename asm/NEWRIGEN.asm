*          DATA SET NEWRIGEN   AT LEVEL 238 AS OF 06/10/20                      
*                                                                               
*PHASE T00A4AE,+0                                                               
*INCLUDE DDUCOM                                                                 
...
* AKAT  DSNTK-62      04/11/16  SUPPORT UCOM=E5-E8                    *         
*                                                                     *         
* JUN6/06      PXZ    UCOMDEF GETS FLAG SET IN DRARGSI+4              *         
* JUN3/03      PXZ    SET NBINDS2 WITH NBBILLRD IF BH KEYWORDS        *         
* JUN2/03      PXZ    SET NBINDS2 WITH NBBILLRD IF BILLING KEYWORDS   *         
*                     DRATTRIB = N = BILLING KEYWORD                  *         
...
*              ROUTINE TO VALIDATE OPTIONS                                      
         SPACE 3                                                                
VVALOPTS GOTO1 =A(OVERFLOW),DMCB,(0,(RC)),RR=NDRELO                             
         DS    0H                                                               
         XIT1                                                                   
VVALOTH  GOTO1 =A(OVERFLOW),DMCB,(1,(RC)),RR=NDRELO                             
         DS    0H                                                               
         XIT1                                                                   
*                                                                               
VVALUDEF NTR1                                                                   
         GOTO1 =A(OVERFLOW),DMCB,(3,(RC)),RR=NDRELO                             
         XIT1                                                                   
VVALUCOM NTR1                                                                   
         GOTO1 =A(OVERFLOW),DMCB,(4,(RC)),RR=NDRELO                             
         XIT1                                                                   
         EJECT                                                                  
...
VVRT2    CLI   5(R2),0                                                          
         ...
         CLI   NDWIDOPT,C'Y'                                                    
         BNE   *+8                                                              
         MVI   NDMYPOSO+2,162      (COLUMN 162 FOR WIDE)                        
         BAS   RE,VALROW                                                        
         CLC   =C'HIGHCOM4',8(R2)                                               
         BNE   VVRT3                                                            
         LA    R4,3(R4)                                                         
...
*                                                                               
         ...
         BE    BADROW                                                           
*                                                                               
         CLC   =C'INVCHGDT',12(R4) INVCHGDT KEYWORD?                            
         BE    *+14                YES                                          
         CLC   =C'INVPID',12(R4)   INVPID KEYWORD?                              
         BNE   *+8                 NO                                           
         OI    NDINDS2,ND2ILAST    YES - SET FLAG                               
*                                                                               
         ...
         BNE   CMRTN                                                            
         CLI   OFFLINE,C'Y'        IF OFFLINE SKIP BECAUSE OF RLP               
         BE    *+14                NEWRI20N WILL TURN IT OFF                    
         CLC   =C'POL',NBSELPRD    ...IF POL                                    
         BE    BADROW              ...SET ERROR                                 
         OI    NBINDS3,NBI3BPRD    SET BILLED PROD MODE                         
         B     VROWNXT                                                          
*                                                                               
CMRTN    CLC   12(6,R4),=C'COMRTN' COMMERCIAL ROTATION PROD KEYWRD              
         BNE   *+12                                                             
         OI    NBINDS3,NBI3CMRT    SET COMMERCIAL ROTATION MODE                 
         B     VROWNXT                                                          
         CLC   12(6,R4),=C'TARGET'    TARGET KEYWORD?                           
***      BNE   *+12                                                             
***      OI    NBSBKEND,X'08'         YES TARGET KEYWORD                        
***      B     VROWNXT                                                          
         BNE   *+8                                                              
         OI    NBSBKEND,X'08'         YES TARGET KEYWORD                        
...
VROW2AA  CLC   12(4,R4),=C'UCOM'   USER DEFINITION RECORDS                      
         ...
         BNE   BADROW                                                           
         SPACE 1                                                                
VROW2AB  CLC   12(6,R4),=C'EFUCOM'   EFFECTIVE DATE UCOM                        
         BNE   VROW2A                                                           
         BAS   RE,VVALUCOM                                                      
         BNE   BADROW                                                           
         MVI   DRARGSI+2,X'FF'     SET EFFECTIVE DATE FILTER                    
         MVC   DRARGSO,DRARGSI     O/P ARGS = I/P ARGS                          
         SPACE 1                                                                
VROW2A   DS    0H                                                               
...
VROW3    CLC   12(4,R4),=C'VEND'   VENDOR NAME/CODE                             
         ...
         B     VROWNXT                                                          
*                                                                               
VROW3C   CLC   12(4,R4),=C'$TYP'   $TYP KEYWORD?                                
         BNE   VROW4                                                            
...
INSROW   NTR1                                                                   
         ...
         CLC   8(6,R2),=C'PRDEST'  SPECIAL FOR ACCENT                           
         BNE   *+8                                                              
         BAS   RE,ESTBINS          INSERT ESTBOTH                               
INSRX    B     XIT                                                              
*                                                                               
DPTINS   NTR1                                                                   
         XC    P,P                                                              
...
*              CONTROL COLUMN VALIDATION                                        
         SPACE 3                                                                
VVALCOLS DS    0H                                                               
* SET MERGE FLAG HERE - NEWRI20 VALIDATES OPTIONS BEFORE                        
* ROWS/COLS AND THUS CLEARS NDDWNL2                                             
         CLI   NSMRGSW,C'Y'       MERGING?                                      
         BNE   *+8                                                              
         OI    NDDWNL2,GLDADDCH   FOR DOWNLOAD                                  
*                                                                               
         LA    R0,12               (MAX 12 COLUMNS)                             
         BAS   RE,DELINS           (CHECK FOR INSERTS/DELETES)                  
         CLI   5(R2),0                                                          
...
VVALCOL2 BAS   RE,VALSET                                                        
         ...
         CLI   NDWIDOPT,C'Y'                                                    
         BNE   VVALCBIG                                                         
         CLI   NDTOTWID,198        UP TO 165 ALLOWED WITH WIDE OPTION           
         BNH   XIT                                                              
         SPACE 1                                                                
         USING T320FFD,R4                                                       
...
VALSET2  XC    NDMYPOSO,NDMYPOSO                                                
         CLI   OFFLINE,C'Y'        PROTECTION EXCEPTION                         
         BNE   *+10                                                             
...
VALMAC   NTR1                                                                   
         ...
         BE    VALMAC2                                                          
         CLC   8(4,R2),=C'ASS*'    ALMOST ALL MACROS START WITH ...             
         BE    VALMAC2                                                          
         CLC   8(2,R2),=C'BH'      BILL HEADER INVOICE READ                     
         BNE   VALMAC01                                                         
         OI    NDRDBCEL,X'01'                                                   
         OI    NBINDS2,NBBILLRD                                                 
VALMAC01 DS    0H                  NOT MACRO BUT CHECK HERE FOR STABRD          
         CLC   8(6,R2),=C'MANNET'  MANNET ?                                     
         BE    *+14                                                             
         CLC   8(6,R2),=C'MANGRS'  MANGRS ?                                     
         BNE   VALMAC02                                                         
         OI    NDINDS1,ND1STAB     READ MANUAL BILLS (STAB RECS)                
*                                                                               
VALMAC02 CLC   8(2,R2),=C'CS'      READ CASH APPLIED DATA                       
         BNE   *+8                                                              
         OI    NDRDBCEL,X'21'                                                   
         ...
         OI    NDRDBCEL,X'02'                                                   
*                                                                               
         CLC   8(6,R2),=C'CHQINV'  CHECK INVOICE REQUESTED                      
         BNE   *+8                                                              
         OI    NBINDS8,NBI8CLIV                                                 
*                                                                               
...
*                                                                               
         ...
         BO    EXPANDX             YES-DONT SET EXPDEM FLG                      
...
EXPANDX  DS    0H                                                               
*                                                                               
         CLC   8(6,R2),=C'TARGET'    TARGET KEYWORD?                            
         BNE   *+8                                                              
         OI    NBSBKEND,X'08'         YES TARGET KEYWORD                        
*                                                                               
         BAS   RE,VALCOL                                                        
...
*                                                                               
         ...
         BE    VCOL4AA                                                          
         CLC   12(6,R4),=C'EFUCOM'   USER DEFINITION RECORDS                    
         BNE   VCOL4A                                                           
         BAS   RE,VVALUCOM                                                      
         MVI   DRARGSI+2,X'FF'                                                  
         MVC   DRARGSO,DRARGSI                                                  
         B     VCOL4A                                                           
VCOL4AA  BAS   RE,VVALUCOM                                                      
         BNE   BADCOL                                                           
...
***      CLI   DRERROR,0                                                        
***      BNE   VCOLC                                                            
         CLI   DRERROR,0           ,,THESE INSTRUCTIONS REPLACE THE             
         BE    PREFIXOK            ,,TWO ABOVE SO THAT COMPUTE COLS             
         MVI   NDPREFIX,0          ,,WITH C'A' DON'T TRIGGER                    
         B     VCOLC               ,,ACTUAL DEMO REQUEST                        
PREFIXOK BAS   RE,ESTACPOP         POP IN EST OR ACT                            
         LA    R4,BLOCK+42                                                      
         B     VCOLNXT                                                          
         SPACE 1                                                                
...
*                                   8 R=RAW E=EQUIV                             
*                                   9 DAY CODE                                  
*                                  10 PRODUCT NUMBER - CHANGED TO ->            
*                                     PROD -> DRARGSI+14 GLARG XTNSION          
...
VCOLFDA2 CLI   0(R1),X'FF'         (EOL?)                                       
         ...
         B     VCOLFDA2                                                         
         SPACE 1                                                                
* PR FILTER MOVED TO GLXRTN FILTERS                                             
...
VCOLFDP  CLC   12(3,R4),=C'DP '    DAYPART FILTER =ARG 15                       
         BNE   VCOLPRD                                                          
         LA    R1,1                DAYPART LEN = 2                              
         BAS   RE,GLXRTN                                                        
         B     VCOLNXT                                                          
                                                                                
VCOLPRD  CLC   12(3,R4),=C'PR '    PRODUCT FILTER =ARG 15                       
         BNE   VCOLSTA                                                          
         LA    R1,2                PRODUCT LEN = 3                              
         BAS   RE,GLXRTN                                                        
         B     VCOLNXT                                                          
...
VCOLZRO  CLC   12(1,R4),=C'Z'      ZERO FILTER                                  
         BNE   VCOLSTS                                                          
         LA    R1,ZEROTBL                                                       
VCOLZRO5 CLC   0(2,R1),22(R4)                                                   
         BE    VCOLZRO9                                                         
         LA    R1,2(R1)                                                         
         CLI   0(R1),0                                                          
         BE    BADCOL                                                           
         B     VCOLZRO5                                                         
VCOLZRO9 LA    R1,1                ZERO LENGTH = 2                              
         BAS   RE,GLXRTN                                                        
         B     VCOLNXT                                                          
...
STSTBL   DC    C'PREMISADUMKGPFBEST'                                            
         DC    X'00'                                                            
ZEROTBL  DC    C'A0A G0G '                                                      
         DC    X'00'                                                            
*                                                                               
VCOLVTY  CLC   12(2,R4),=C'VT'    VIEWING TYPE                                  
         BNE   VCOLFPE                                                          
*                                                                               
         MVC   HALF,22(R4)                                                      
*                                                                               
         CLI   22(R4),C'O'         OOH VIEWING TYPE?                            
         JNE   VCOLTY1Z                                                         
         MVI   HALF+1,0                                                         
         OI    NBVTYPS3,NBVT3OOH                                                
*                                                                               
         CLC   =C'OL',22(R4)                                                    
         JNE   *+12                                                             
         MVI   HALF,SRCLOLQ        OOH LIVE                                     
         J     VCOLTY1Z                                                         
         CLC   =C'OS',22(R4)                                                    
         JNE   *+12                                                             
         MVI   HALF,SRCLOSQ        OOH LIVE+SD                                  
         J     VCOLTY1Z                                                         
         CLC   =C'O3',22(R4)                                                    
         JNE   *+12                                                             
         MVI   HALF,SRCLO3Q        OOH LIVE+3                                   
         J     VCOLTY1Z                                                         
         CLC   =C'O7',22(R4)                                                    
         JNE   *+12                                                             
         MVI   HALF,SRCLO7Q        OOH LIVE+7                                   
         J     VCOLTY1Z                                                         
         CLC   =C'OCL',22(R4)                                                   
         JNE   *+12                                                             
         MVI   HALF,SRCLOCLQ       OOHC LIVE                                    
         J     VCOLTY1Z                                                         
         CLC   =C'OCS',22(R4)                                                   
         JNE   *+12                                                             
         MVI   HALF,SRCLOCSQ       OOHC LIVE+SD                                 
         J     VCOLTY1Z                                                         
         CLC   =C'OC3',22(R4)                                                   
         JNE   *+12                                                             
         MVI   HALF,SRCLOC3Q       OOHC LIVE+3                                  
         J     VCOLTY1Z                                                         
         CLC   =C'OC7',22(R4)                                                   
         JNE   BADCOL                                                           
         MVI   HALF,SRCLOC7Q       OOHC LIVE+7                                  
*                                                                               
VCOLTY1Z LA    R1,VTYPTBL                                                       
VCOLTY2  CLC   0(2,R1),HALF                                                     
         BE    VCOLTY4                                                          
         LA    R1,3(R1)                                                         
         CLI   0(R1),0                                                          
         BNE   VCOLTY2                                                          
         B     BADCOL                                                           
VCOLTY4  CLI   22(R4),C'M'         MINUTE BY MINUTE?                            
         BNE   VCOLTY6                                                          
         CLI   24(R4),C'P'         POD?                                         
         BNE   VCOLTY6                                                          
         NI    22(R4),X'FF'-X'40'  MAKE IT LOWER CASE TO INDICAT POD            
VCOLTY6  LA    R1,1                VTYPE LENGTH = 2                             
         BAS   RE,GLXRTN           SPECIAL FOR VT                               
*************************                                                       
* ADD APPROPRIATE TEXT FOR VTYPE FLAVOR IN H4                                   
         LA    R1,DRHEAD4                                                       
         XC    0(64,R1),0(R1)                                                   
         OI    0(R1),X'80'                                                      
         MVI   27(R1),6            LENGTH OF HEAD TEXT                          
*                                                                               
         MVC   BYTE,23(R4)                                                      
         CLC   =C'OC',22(R4)       OOHC VTYPE?                                  
         JNE   *+10                                                             
         MVC   BYTE,24(R4)                                                      
*                                                                               
         LA    RE,VTHEAD                                                        
...
*!!!     CLC   0(1,RE),23(R4)                                                   
         ...
         MVC   28(1,R1),22(R4)                                                  
*                                                                               
         CLI   HALF,SRCLOLQ        OOH LIVE                                     
         JE    VCOLTY8A                                                         
         CLI   HALF,SRCLOSQ        OOH LIVE+SD                                  
         JE    VCOLTY8A                                                         
         CLI   HALF,SRCLO3Q        OOH LIVE+3                                   
         JE    VCOLTY8A                                                         
         CLI   HALF,SRCLO7Q        OOH LIVE+7                                   
         JNE   VCOLTY8B                                                         
VCOLTY8A MVI   28(R1),C' '                                                      
         MVC   29(1,R1),22(R4)                                                  
         MVC   30(R4,R1),1(RE)                                                  
         B     VCOLHD10                                                         
*                                                                               
VCOLTY8B CLI   HALF,SRCLOCLQ       OOHC LIVE                                    
         JE    VCOLTY8C                                                         
         CLI   HALF,SRCLOCSQ       OOHC LIVE+SD                                 
         JE    VCOLTY8C                                                         
         CLI   HALF,SRCLOC3Q       OOHC LIVE+3                                  
         JE    VCOLTY8C                                                         
         CLI   HALF,SRCLOC7Q       OOHC LIVE+7                                  
         JNE   VCOLTY8D                                                         
VCOLTY8C MVI   29(R1),C'C'                                                      
         MVC   30(4,R1),1(RE)      SET LITERAL FROM TABLE                       
         B     VCOLHD10                                                         
*                                                                               
VCOLTY8D TM    28(R1),X'40'        IS IT LOWER CASE (POD INDICATOR)             
         BO    VCOLTY9             NO                                           
         OI    28(R1),X'40'        YES/SET UPPER CASE FOR PRINT LINE            
         MVI   33(R1),C'P'         SET AS POD                                   
VCOLTY9  MVC   29(4,R1),1(RE)      SET LITERAL FROM TABLE                       
         B     VCOLHD10                                                         
*                                                                               
VTHEAD   DC    C'LLIVE'                                                         
         DC    C'SSAME'                                                         
...
***************************************************************                 
*                                                                               
VCOLHD10 LA    R1,NDVTYPTB         SET DEMO BLOCK INDICATOR                     
         LA    RE,3                MAX 3 VTYPES                                 
*                                                                               
VCOLHD12 CLC   0(2,R1),HALF       ALREADY SET                                   
         BE    VCOLHD15            YES                                          
         LA    R1,2(R1)            NO                                           
         BCT   RE,VCOLHD12                                                      
         LA    R1,NDVTYPTB         NO MATCH - SET INTO TABLE                    
         LA    RE,3                                                             
VCOLHD13 CLI   0(R1),0                                                          
         BNE   VCOLHD14                                                         
         MVC   0(2,R1),HALF       SET IT HERE                                   
         B     VCOLHD15                                                         
VCOLHD14 LA    R1,2(R1)            BUMP TO NEXT POSITION                        
         BCT   RE,VCOLHD13                                                      
         B     BADCOL                                                           
VCOLHD15 EQU   *                                                                
         MVI   NBVTYPS3,X'01'      SET OVERRIDE FLAG                            
*************************                                                       
         B     VCOLNXT                                                          
*                                                                               
**     2 BYTE TYPE                                                              
VTYPTBL  DC    C'PL PS P1 P2 P3 P7 '  PROGRAM AVERAGE                           
         DC    C'N3 '                                                           
         DC    C'CL CS C1 C2 C3 C7 '  AVERAGE COMMERCAIL MINUTE                 
         DC    C'AL AS A7 '           AVERAGE COMMERCAIL MINUTE                 
         DC    C'ML MS M1 M2 M3 M7 '  MXM MINUTE BY MINUTE                      
         DC    C'MLPMSPM1PM2PM3PM7P'  MXM MINUTE BY MINUTE POD                  
         DC    X'C00040C10040C20040C30040' OOH VTYPES                           
         DC    X'C40040C50040C60040C70040' OOHC VTYPES                          
         DC    X'00'                                                            
*                                                                               
GLXRTN   NTR1                                                                   
...
* SPECIAL FOR VT TYPES SINCE THEY NEED TO BE 'TRANSLATED'                       
GLXRTNVT NTR1                                                                   
         MVC   DRARGSI+14(1),NDGLRGCT   PASS INDEX NUMBER                       
         ZIC   R5,NDGLRGCT                                                      
         MH    R5,=Y(NDGLRGXE)     MULTIPLY BY LENGTH OF FILTER                 
         ...
         LA    RF,NDGLRXNM         NUMBER OF FILTERS IN EXTENSION               
GLX05VT  CLI   0(RE),0             ROOM HERE?                                   
         BE    GLX10VT                                                          
         LA    RE,NDGLRGXE(RE)     LENGTH OF EACH FILTER AREA                   
         BCT   RF,GLX05VT                                                       
         B     GLXXVT              NO MORE ROOM /EXIT FOR NOW                   
GLX10VT  MVC   0(1,RE),12(R4)      SET FIRST CHARACTER AS INDICATOR             
*                                                                               
         LA    R1,VTYPTBL          TRANSLATE TO ONE CHARACTER                   
GLXT12VT CLI   0(R1),0                                                          
         BNE   *+6                                                              
         DC    H'0'                                                             
         CLC   0(2,R1),22(R4)     SET FILTER FIELD                              
         BE    *+12                                                             
         LA    R1,3(R1)                                                         
         B     GLXT12VT                                                         
         MVC   1(1,RE),2(R1)      SET TRANSLATION                               
GLXXVT   XIT1                                                                   
...
*****    LA    RF,10           MAX NUMBER OF PRD GROUP COLUMN FILTER            
         LA    RF,3            MAX NUMBER OF PRD GROUP COLUMN FILTER            
VCPG10   CLI   0(R5),0                                                          
         BE    VCPG20                                                           
         LA    R5,3(R5)                                                         
         LA    R1,1(R1)                                                         
...
****     MH    R1,=H'256'          BUMP TO PRDGRP AREA                          
         MH    R1,=H'500'          BUMP TO PRDGRP AREA                          
         L     R5,NBAPLIST                                                      
         AR    R5,R1                                                            
***      LA    R1,256                                                           
         LA    R1,500                                                           
***VCPG36AA CLC   0(1,R5),3(RE)       ALREADY THERE?                            
VCPG36AA CLC   0(3,R5),KEY+8       ALREADY THERE?                               
         BE    VCPG37              YES                                          
         CLI   0(R5),0                                                          
         BE    VCPG36A                                                          
***      LA    R5,1(R5)                                                         
...
VCPG36A  MVC   0(3,R5),KEY+8       SET 3 CHAR PROD NUMBER                       
*                                                                               
...
VCOLRTAL CLC   12(5,R4),=C'RIGHT'  RIGHT ALIGN OUTPUT                           
         ...
         B     VCOLNXT                                                          
         SPACE 1                                                                
VCOLMAT  LA    R1,MATTBL                                                        
VCOLM05  CLI   0(R1),0                                                          
         BE    VCOLFEND                                                         
VCOLM10  CLC   12(3,R4),0(R1)                                                   
         BNE   *+14                                                             
         OC    DRARGSI+12(1),3(R1)                                              
         B     VCOLNXT                                                          
         LA    R1,4(R1)                                                         
         B     VCOLM05                                                          
*8       BNE   *+12                                                             
...
***                                                                             
         CLI   DRATTRIB,C'N'       BILLING KEYWORD?                             
         BNE   *+8                                                              
         OI    NBINDS2,NBBILLRD    YES/SET INDICATOR                            
*                                                                               
         CLI   NDPREFIX,C'A'       IF REQUESTING ACTUAL DEMO                    
         BNE   *+8                                                              
         MVI   NBACTOPT,C'Y'                                                    
         CLI   DRATTRIB,C'D'       DEMO?                                        
         BNE   ENDCHK                                                           
*                                                                               
         TM    NBINDS8,NBI8RNTK    DEMO+RENTRAK ADD 2 TO OUTLEN                 
         BNO   ENDRNTK                                                          
         ZIC   R1,DRLENO                                                        
         LA    R1,2(R1)                                                         
         STC   R1,DRLENO                                                        
ENDRNTK  DS    0H                                                               
         CLC   =C'EAI',8(R2)       KEYWORD?                                     
         BE    ENDC01                                                           
         CLC   =C'EAR',8(R2)       KEYWORD?                                     
         BE    ENDC01                                                           
         CLC   =C'EAX',8(R2)       KEYWORD? EAXI/EAXR/EAXT                      
         BE    ENDC01                                                           
         CLC   =C'GA',8(R2)        KEYWORD? GAT/GADT                            
         BE    ENDC01                                                           
         CLC   =C'PL',8(R2)        KEYWORD? PLANGOAL                            
         BNE   ENDCHK                                                           
ENDC01   MVI   NBACTOPT,C'Y'       GET ACTUAL DEMOS                             
ENDCHK   DS    0H                                                               
***                                                                             
         SPACE 1                                                                
...
ANYMOCOL NTR1                                                                   
         ...
         LA    R4,BLOCK                                                         
         GOTO1 VCOLDRON                                                         
         MVC   DRARGSI(1),NDCOLRNK      PASS ARGUMENT                           
         MVI   DRNARGSI,1                                                       
         MVI   NDCOLRNK,0                                                       
...
ENTARG2  CLC   12(5,R4),=C'TEXT '  TEXT= NEEDS (SURPRISE) TEXT                  
         BE    ENTARG2B                                                         
         CLC   =C'TEXTA',12(R4)    FORCE AS ALPHA/DIES IN HEADLINE!!            
         BNE   ENTARG2A                                                         
         MVI   DRTYPEI,C'C'                                                     
         B     ENTARG2B                                                         
ENTARG2A CLC   =C'TEXTX',12(R4)    KEEP BLANK                                   
         BE    *+12                                                             
         B     ENTARG4                                                          
ENTARG2B OI    NDDWNL2,GLD2FIX     TO REMOVE EXTRA BLANK COLS                   
         ZIC   R1,1(R4)            (L'TEXT)                                     
         LTR   R1,R1                                                            
         BNZ   *+8                                                              
         LA    R1,1                                                             
...
ENTARG4  CLC   DRRTNI,=C'NISTACC ' COLUMN STACK NEEDS ARGS                      
         BNE   ENTARG6                                                          
*        BAS   RE,VALACC                                                        
         GOTO1 =A(OVERFLOW),DMCB,(2,(RC)),RR=NDRELO                             
         DS    0H                                                               
         XIT1                                                                   
         B     XIT                                                              
         SPACE 1                                                                
...
HEADPOP  NTR1                                                                   
         ...
         BNO   XIT                                                              
         CLI   NDUSERRC,C'Y'        USER WANTS TO OVERRIDE HEADS                
         BE    HEADPOP8            DON'T INSERT HEDPOPS                         
...
VGENHB2  LR    R3,R2                                                            
         ...
         CLI   NBESTOPT,0          IF DOING DEMOS                               
         BNE   VGENHB2A            PREPARE FOR NIELSON COPYRIGHT                
         CLI   NBACTOPT,0          IF DOING DEMOS                               
         BNE   VGENHB2A            PREPARE FOR NIELSON COPYRIGHT                
...
VGENHB5  BAS   RE,GETLONG                                                       
         ...
         BAS   RE,SHUFFLE                                                       
****     LA    R3,6                                                             
         LA    R3,10        CHANGED 8/03 TO MATCH LABELAREA DSECT               
         BAS   RE,GETLONG   IN NEWRIDRIVE                                       
***      LA    R3,7(R2)                                                         
         LA    R3,11(R2)    CHANGED 8/03 TO MATCH LABELAREA DSECT IN            
         A     R2,FULL      NEWRIDRIVE                                          
         LA    R2,2(R2)                                                         
         BAS   RE,SHUFFLE                                                       
         SPACE 1                                                                
VGENHBX  L     R2,NDAH4            RESTORE PERIOD ETC. TO H4                    
         MVC   50(80,R2),BLOCK                                                  
         CLI   NBESTOPT,0          NIELSEN COPYRIGHT?                           
         BNE   VGENLSN                                                          
         CLI   NBACTOPT,0          NIELSEN COPYRIGHT?                           
         BNE   VGENLSN                                                          
         ...
         B     XIT                                                              
VGENLSN  ICM   R3,15,DUB           YES/GET SAVED ADDRESS OF PRINT LINE          
         MVC   0(30,R3),=C'RTG/IMP-(C) 2019 NIELSEN MEDIA'                      
         TM    NBINDS8,NBI8RNTK    RENTRAK?                                     
         BNO   *+10                                                             
         MVC   0(30,R3),=C'RTG/IMP-(C) 2019 RENTRAK MEDIA'                      
*                                                                               
...
*                                                                               
         ...
         JNO   *+12                                                             
         USING XDDEMBLK,RF                                                      
         LA    RF,XDDEMOS          A(INPUT DEMO CATEGORIES)                     
         DROP  RF                                                               
*                                                                               
VGENLSN0 CLI   0(RF),X'FF'         ANY MORE DEMOS?                              
         JE    VGENLSN2                                                         
         CLI   2(RF),0             COMSCORE DEMO?                               
         JNE   *+12                                                             
         OI    BYTE,X'01'          MARK COMSCORE DEMO EXISTS                    
         J     *+8                                                              
         OI    BYTE,X'02'          MARK NIELSEN DEMO EXISTS                     
         AHI   RF,3                CHECK NEXT DEMO                              
         J     VGENLSN0                                                         
*                                                                               
VGENLSN2 TM    BYTE,X'01'+X'02'    BOTH ARE PRESENT?                            
         JNO   VGENLSN4                                                         
         MVC   0(11,R3),=C'COPYRIGHT @'                                         
         MVC   30(15,R3),=C', COMSCORE INC.'                                    
         J     VGENLSNX                                                         
*                                                                               
VGENLSN4 TM    BYTE,X'02'          ONLY NEILSEN?                                
         JO    VGENLSNX                                                         
         MVC   0(30,R3),=C'COPYRIGHT @ 2019 COMSCORE INC.'                      
*                                                                               
VGENLSNX B     XIT                                                              
         EJECT                                                                  
...
VGETBM   GOTO1 DATCON,DMCB,(2,0(R2)),(X'20',WORK)                               
         CLI   NDPEROPT,C'S'     ,,IF SPECIAL, ADD 7 TO START DATE              
         BNE   VGETBM1                                                          
         CLI   NDPEROPT,C'S'     ,,IF NTI, ADD 7 TO START DATE                  
         BNE   VGETBM1                                                          
         SR    R5,R5              ,,TO PRINT PROPER MONTH/YEAR                  
         CLI   BYTE,C'U'           CALLING THIS FOR UCOMS?                      
         BNE   VGETBM5             (SPEC-5901)                                  
         AH    R5,=H'4'                                                         
         B     VGETBM5A                                                         
*                                                                               
VGETBM1  CLI   NDPEROPT,C'C'       IF CALENDAR, SKIP BROADCAST BIT              
         BE    VGETBM6                                                          
...
VGETBM4  GOTO1 GETDAY,DMCB,WORK,WORK+6                                          
         ...
         BP    *+8                                                              
VGETBM5  AH    R5,=H'7'                                                         
VGETBM5A GOTO1 ADDAY,DMCB,WORK,(X'20',WORK+6),(R5)                              
         MVC   WORK(6),WORK+6                                                   
         SPACE 1                                                                
VGETBM6  PACK  WORK+8(8),WORK+2(2) YYMM NOW IN WORK                             
         CVB   R1,WORK+8                                                        
         STC   R1,WORK+4           RETURN MONTH NUMBER WORK+4                   
         PACK  WORK+8(8),WORK(2)   GET YEAR                                     
         CVB   R1,WORK+8                                                        
         STC   R1,WORK+5           RETURN YEAR IN WORK+5                        
         B     XIT                                                              
         EJECT                                                                  
*              DRONE UTILITIES                                                  
         SPACE 3                                                                
...
ALLVAL   XC    WORK,WORK           GENERATE A PSEUDO TWA HEADER                 
         ...
         BNE   ALLV10                                                           
         MVC   HALF,=H'685'        KEYWORD SECURITY LOCKOUT                     
         B     ERR2                                                             
ALLV10   BAS   RE,SETREAD                                                       
         B     XIT                                                              
         SPACE 1                                                                
...
*        OI    BYTE,X'40'          TURN ON GRP EQUIV                            
         ...
         MVI   NSOPTSO2,0                                                       
         OI    NBINDS9,NBI9GDM     DEFAULT - REQUEST W/ GUARANTEES              
         MVI   NDOPTS1,0                                                        
*                                                                               
         EJECT                                                                  
...
VOTHERS  DS     0H                                                              
         CLI   5(R2),0                                                          
         BE    XIT2                                                             
         GOTO1 SCANNER,DMCB,(40,(R2)),(X'94',BLOCK),0                           
         LA    R3,BLOCK                                                         
         MVI   FIELDERR,1                                                       
         ZIC   R4,DMCB+4                                                        
         LTR   R4,R4                                                            
...
OPT12    CLC   12(3,R3),=C'PER'    PERIOD OPTION                                
         BNE   OPT16                                                            
         MVC   NDPEROPT,22(R3)                                                  
         CLI   1(R3),1             ONE CHARACTER INPUT                          
         BNE   OPT12A                                                           
...
OPT30    CLC   12(5,R3),=C'ROUND'  ROUNDING OPTION - TOTALS ONLY                
         BNE   OPT31                                                            
         OI    NDDRINDS,X'01'      ROUND                                        
         OI    NDDRIND3,X'10'      DONT REDUCE OUTPUT LEN                       
         B     OPTNEXT                                                          
         SPACE 1                                                                
OPT31    CLC   12(5,R3),=C'ALLROUND'  ROUNDING TOTALS AND DETAILS               
         BNE   OPT32                                                            
         OI    NDDRIND3,X'01'      ROUND TOT AND DET                            
         OI    NDDRIND3,X'10'      DONT REDUCE OUTPUT LEN                       
         B     OPTNEXT                                                          
         SPACE 1                                                                
...
OPT56    CLC   12(4,R3),=C'ZERO'   SAME AS ABOVE                                
         ...
         B     OPTNEXT                                                          
         SPACE 1                                                                
OPT57    CLC   12(6,R3),=C'ACCMON'  FLAG SET IN NEWRI20                         
         BE    OPTNEXT                                                          
         SPACE 1                                                                
...
*                                                                               
OPT131   DS    0H                                                               
         CLC   =C'BHRDATE',22(R3)     BHRDATE=BHRDATE?                          
         BE    OPT370                                                           
*                                                                               
...
OPT140   DS    0H                                                               
         ...
         CLC   =C'CLEARED',22(R3)                                               
         BNE   OPT145                                                           
         OI    NBACCFLT,X'20'      CLEARED ONLY                                 
         B     OPTNEXT                                                          
OPT145   CLC   =C'ORDERED',22(R3)                                               
         BNE   OPTERR                                                           
         OI    NBACCFLT,X'40'      ORDERED ONLY                                 
         B     OPTNEXT                                                          
         SPACE 1                                                                
OPT150   DS    0H                                                               
...
OPT152   CLI   12(R3),C'S'         SPACING                                      
         BNE   OPT160                                                           
         CLI   0(R3),1             MAKE SURE IT'S ONLY S                        
         BNE   OPT160                                                           
         MVC   NDSPAOPT,11(R3)                                                  
         CLI   NDSPAOPT,0                                                       
         BE    OPTERR                                                           
...
*                                                                               
         ...
         BNE   OPTNEXT                                                          
         MVC   NDCMLFLT,22(R3)                                                  
         CLI   1(R3),8             ISCI COMMERCIAL?                             
         BE    OPTNEXT             YES                                          
         XC    DMCB(24),DMCB       NO-PACK AD-ID TYPE CMML                      
         MVC   DMCB+4(4),=X'D9000AFE'    TRPAK                                  
         GOTO1 CALLOV,DMCB                                                      
         L     RF,DMCB                                                          
         GOTO1 (RF),DMCB,(C'P',22(R3)),NDCMLFLT                                 
         B     OPTNEXT                                                          
*                                                                               
OPT295   DS    0H                                                               
...
*                                                                               
OPT350   DS    0H                                                               
         CLC   =C'EASI-',12(R3)   SKIP EASI INVOICES?                           
         BNE   *+12                                                             
         OI    NBACCFLT,X'08'          NO EASI INVOICES                         
         B     OPTNEXT                                                          
         CLC   =C'EASI',12(R3)    ONLY EASI INVOICES?                           
         BNE   OPT360                                                           
         OI    NBACCFLT,X'10'       TURN ON OPTION                              
         B     OPTNEXT                                                          
*                                                                               
OPT360   DS    0H                                                               
         CLC   =C'RESTD',12(R3) RESTORED STARCOM UNITS?                         
         BNE   OPT361                                                           
         OI    NBACCFLT,X'20'                                                   
         B     OPTNEXT                                                          
*                                                                               
OPT361   DS    0H                                                               
         CLC   =C'CONTINUE',12(R3)        CONTINUATION REPORT                   
         BNE   OPT365                                                           
         BAS   RE,VALRP2                                                        
         BNE   OPTERR                                                           
         OI    NDDOWNL,GLDLACTV           FORCE DOWNLOAD                        
         B     OPTNEXT                                                          
*                                                                               
OPT365   DS    0H                                                               
         CLC   =C'CBU',12(R3)      REPORT CLEARED BUT UNDISBURSED               
         BNE   OPT370                                                           
         OI    NBINDS8,NBI8CBU            SET INDICATOR                         
         B     OPTNEXT                                                          
*                                                                               
OPT370   DS    0H                                                               
...
*                                                                               
         ZIC   RF,8(R3)            GET DISPLACEMENT OF RFP KEYWORD              
         ZIC   RE,1(R3)            GET LENGTH OF RFP KEYWORD                    
         AR    RF,RE                                                            
...
*                                                                               
OPTBHD10 ZIC   RF,5(R2)            RF=INPUT LENGTH                              
         ZIC   RE,1(R3)            L'RFP KEYWORD INPUT                          
         SR    RF,RE               RF=L'INPUT - RFP KEYWORD                     
*                                                                               
         ZIC   RE,0(R2)            RE=MAX INPUT LENGTH                          
         AHI   RE,-8                                                            
...
OPTBHD20 DS    0H                                                               
         SR    RE,RE                                                            
         LHI   RE,17               MAX EXPANSION                                
         ZIC   RF,1(R3)            L'RFP KEYWORD INPUT                          
...
*                                                                               
         ...
         MVC   8(L'QRFPESC,RF),22(R3)   MOVE ESC SEQ TO SCREEN AFTER =          
         MVI   11(RF),17           SET L'EXPANSION OF ESC SEQ                   
*                                                                               
         CLC   23(2,R3),=Y(NE#BHRDT) ONLY BHRDATE IS VALID HERE                 
         BNE   OPTDERR                                                          
***      B     OPT600                                                           
         B     OPTNEXT                                                          
*                                                                               
OPT600   DS    0H                                                               
         CLC   =C'ACOF',12(R3)     ACC OFFICE FILTER                            
         BNE   OPT610                                                           
         MVC   NBACCOFF,22(R3)      SET ACC OFFICE FILTER                       
*                                                                               
OPT610   DS    0H                                                               
         CLC   =C'POD',12(R3)      POD FILTER?                                  
         BNE   OPT612                                                           
         CLI   1(R3),3             LENGTH?                                      
         BNE   OPTERR                                                           
         MVC   NBPOD,22(R3)                                                     
...
*                                                                               
OPT612   DS    0H                                                               
         CLC   =C'DOWNFIX',12(R3) FIX BLANK AFTER TEXT                          
         BNE   OPT614                                                           
         OI    NDDWNL2,GLD2FIX                                                  
         B     OPTNEXT                                                          
*                                                                               
*                                                                               
OPT614   DS    0H                                                               
         CLC   =C'ETYPE',12(R3)    STEWARDHIP EST FILTER                        
         BNE   OPT616                                                           
         CLI   22(R3),C'S'         STEWARD ONLY                                 
         BNE   *+12                                                             
         OI    NBINDS4,NBI4STWO                                                 
         B     OPTNEXT                                                          
         CLI   22(R3),C'*'         INCLUDE STEWARD                              
         BNE   OPTERR                                                           
         OI    NBINDS4,NBI4STWY                                                 
         B     OPTNEXT                                                          
OPT616   DS    0H                                                               
         CLC   =C'GMT',12(R3)      GOALS BY MEDIA TYPE                          
         BNE   OPT618                                                           
         CLI   22(R3),C'N'         NETWORK?                                     
         BNE   *+12                                                             
         OI    NBINDS5,NBI5MTN                                                  
         B     OPTNEXT                                                          
         CLI   22(R3),C'S'         SYNDICATION?                                 
         BNE   *+12                                                             
         OI    NBINDS5,NBI5MTS                                                  
         B     OPTNEXT                                                          
         CLI   22(R3),C'C'         CABLE?                                       
...
*                                                                               
OPT618   DS    0H                                                               
         CLC   =C'SCOMM',12(R3)      COMMENTS BY SUB-MEDIA                      
         BNE   OPT620                                                           
         OI    NDINDS1,ND1SCOM                                                  
         B     OPTNEXT                                                          
*                                                                               
OPT620   DS    0H                                                               
         CLC   =C'VTYP',12(R3)       VIEWING TYPES                              
         BNE   OPT630                                                           
         MVC   HALF,22(R3)                                                      
         MVI   BYTE,1              SWITCH                                       
         CLI   HALF,C'O'           OOH?                                         
         ...
         MVC   WORK(3),22(R3)                                                   
*                                                                               
OPT621   LA    RE,CSVTBL           COMSCORE VIEWING TYPES                       
OPT622   CLI   0(RE),0             END OF TABLE?                                
         JE    OPT622A                                                          
         CLC   HALF,0(RE)                                                       
         JE    *+12                                                             
         AHI   RE,3                                                             
         ...
         CLC   =C'RL',0(RE)                                                     
         JNE   *+8                                                              
         MVI   NBCVTYP,C'L'        LIVE                                         
         CLC   =C'RC',0(RE)                                                     
         JNE   *+8                                                              
         MVI   NBCVTYP,C'C'        LIVE COMMERCIAL                              
         CLC   =C'R3',0(RE)                                                     
         JNE   *+8                                                              
         MVI   NBCVTYP,C'3'        LIVE COMMERCIAL + 3                          
         CLC   =C'R7',0(RE)                                                     
         JNE   *+8                                                              
         MVI   NBCVTYP,C'7'        LIVE COMMERCIAL + 7                          
         J     OPT629                                                           
*                                                                               
OPT622A  CLI   HALF,C'O'           OOH VIEWING TYPE?                            
         JNE   OPT623                                                           
         OI    NBVTYPS3,NBVT3OOH                                                
         CLC   =C'OL',WORK                                                      
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLOLQ     OOH LIVE                                     
         J     OPT628                                                           
         CLC   =C'OS',WORK                                                      
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLOSQ     OOH LIVE+SD                                  
         J     OPT628                                                           
         CLC   =C'O3',WORK                                                      
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLO3Q     OOH LIVE+3                                   
         J     OPT628                                                           
         CLC   =C'O7',WORK                                                      
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLO7Q     OOH LIVE+7                                   
         J     OPT628                                                           
         CLC   =C'OCL',WORK                                                     
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLOCLQ    OOHC LIVE                                    
         J     OPT628                                                           
         CLC   =C'OCS',WORK                                                     
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLOCSQ    OOHC LIVE+SD                                 
         J     OPT628                                                           
         CLC   =C'OC3',WORK                                                     
         JNE   *+12                                                             
         MVI   NBVTYPS,SRCLOC3Q    OOHC LIVE+3                                  
         J     OPT628                                                           
         CLC   =C'OC7',WORK                                                     
         JNE   OPTERR                                                           
         MVI   NBVTYPS,SRCLOC7Q    OOHC LIVE+7                                  
         J     OPT628                                                           
*                                                                               
OPT623   LA    RE,VTBL                                                          
         LA    RF,NBVTYPS          PROGRAM FLAG                                 
OPT626   CLC   HALF,0(RE)                                                       
         BE    OPT627                                                           
         LA    RE,3(RE)                                                         
         CLI   0(RE),0                                                          
         BE    OPTERR                                                           
         B     OPT626                                                           
*                                                                               
OPT627   MVC   NBVTYPS(2),HALF                                                  
         CLI   NBVTYPS,C'M'        MXM TYPE ?                                   
         BNE   OPT628                                                           
         CLI   24(R3),C'P'         POD REQUEST ?                                
         BNE   OPT628                                                           
****     NI    NBVTYPS,X'FF'-X'40' MAKE IT LOWER CASE FOR POD                   
****                               SIMPLER TO OI NBVTYPS3 WITH POD              
         OI    NBVTYPS3,X'08'      SET POD FLAG                                 
OPT628   OI    NBVTYPS3,X'01'      SET OVERRIDE FLAG                            
*                                                                               
OPT629   CLI   BYTE,2              PROCESSED BOTH?                              
         JE    OPTNEXT                                                          
         LA    RF,24(R3)                                                        
*                                                                               
         CLC   =C'OCL',22(R3)                                                   
         ...
         AHI   RF,1                                                             
*                                                                               
         CLI   0(RF),0             ANY MORE VTYPES?                             
         JE    OPTNEXT                                                          
         CLI   0(RF),C' '                                                       
         JE    OPTNEXT                                                          
         CLI   0(RF),C'/'                                                       
         JNE   OPTERR                                                           
         MVC   HALF,1(RF)          VALIDATE THIS ONE                            
         MVI   BYTE,2                                                           
         CLI   HALF,C'O'           OOH?                                         
         JNE   *+10                                                             
         MVC   WORK(3),1(RF)                                                    
...
*                                                                               
* NBVTYPS                                                                       
CSVTBL   DC    C'RL RC R3 R7 '                                                  
         DC    X'00'                                                            
*                                                                               
VTBL     DC    C'PL PS P1 P2 P3 P7 '                                            
         DC    C'CL CS C1 C2 C3 C7 '                                            
         DC    C'ML MS M1 M2 M3 M7 '                                            
         DC    C'MLPMSPM1PM2PM3PM7P'                                            
         DC    X'00'                                                            
OPT630   DS    0H                                                               
         CLC   =C'CHQDFLT',12(R3)   USE DEFAULT DATE FOR VENDOR CHECKS          
         BNE   OPT640                                                           
         OI    NBINDS4,NBI4DISB     SET INDICATOR                               
         B     OPTNEXT                                                          
OPT640   DS    0H                                                               
         CLC   =C'IEC',12(R3)        INCLUDE EARNED COST ?                      
         BNE   OPT642                                                           
         OI    NBINDS4,NBI4ECOP     INCLUDE EC CHANGES                          
         B     OPTNEXT                                                          
OPT642   DS    0H                                                               
         CLC   =C'GOALXF',12(R3)       READ NEW GOAL RECORDS                    
         BNE   OPT644                                                           
         OI    NBINDS7,NBI7XFL         SET FLAG                                 
         B     OPTNEXT                                                          
*                                                                               
OPT644   DS    0H                                                               
         CLC   =C'FILE',12(R3)         WRITE TO DATASET (WB)                    
         BNE   OPT645                                                           
         OI    NDINDS1,ND1WB           SET FLAG                                 
         B     OPTNEXT                                                          
*                                                                               
OPT645   DS    0H                                                               
         CLC   =C'WBMED',12(R3)      FILTER WB MEDIA TYPE                       
         BNE   OPT646                                                           
         CLC   =C'OV',22(R3)                                                    
         BNE   *+12                                                             
         OI    NBWBFLT1,NBWBFOV     FLIGHT OVERRIDES ONLY                       
         B     OPTNEXT                                                          
         CLC   =C'NOV',22(R3)                                                   
         BNE   *+12                                                             
         OI    NBWBFLT1,NBWBFNOV     SKIP FLIGHT OVERRIDES                      
         B     OPTNEXT                                                          
         LA    RE,22(R3)           USE RE->SCANNER 2ND FIELD                    
         CLI   1(R3),1             LENGTH OF FLD2                               
         BE    OPT645A                                                          
         CLI   0(RE),C'C'         ...IF CC,CT,CV                                
         BNE   OPTERR                                                           
         LA    RE,1(RE)            ...BUMP SCAN POINTER BY 1                    
         LA    R1,WBMEDC           ...TABLE->2ND CHARACTER                      
         B     *+8                                                              
OPT645A  LA    R1,WBMEDIA                                                       
OPT645D  CLI   0(R1),0                                                          
         BE    OPTERR                                                           
         CLC   0(1,R1),0(RE)                                                    
         BE    *+12                                                             
         LA    R1,1(R1)                                                         
         B     OPT645D                                                          
         OC    NBWBMEDF,1(R1)            SET FLAG                               
         B     OPTNEXT                                                          
                                                                                
*                                                                               
OPT646   DS    0H                    WB FLIGHT ID FILTER                        
         CLC   =C'FLTID',12(R3)                                                 
         BNE   OPT647                                                           
...
*                                  ..VALID NUMERIC                              
         ...
         BNE   OPTNEXT             ..SET TO DSECT                               
*                                                                               
         L     R1,NBWBFLT            GET PASSED WORK AREA                       
         LTR   R1,R1                                                            
         BNZ   *+6                                                              
         DCHO                                                                   
         USING BINDSCT,R1                                                       
         CLI   1(R3),10            ,,IF LENGTH OF FIELD=10                      
         BNE   OPT646B                                                          
         MVC   BWBFLSTR,22(R3)     ,,JUST MOVE IT IN                            
         B     OPTNEXT                                                          
*                                                                               
OPT646B  CLI   1(R3),9             ,,IF LENGTH OF FIELD=9                       
         BNE   OPT646C                                                          
         MVC   BWBFLSTR+1(9),22(R3)     ,,JUST MOVE IT IN                       
         MVI   BWBFLSTR,C'0'                                                    
         B     OPTNEXT                                                          
*                                                                               
OPT646C  SR    RE,RE                ELSE                                        
         ICM   RE,7,9(R3)           BINARY VALUE OF NUMERIC FIELD               
         CVD   RE,DUB                CHANGE TO PACKED FORMAT                    
         UNPK  BWBFLSTR,DUB           SET INTO DSECT                            
         OI    BWBFLSTR+9,X'F0'                                                 
         B     OPTNEXT                                                          
*                                                                               
*                                                                               
...
*                                  ..VALID NUMERIC                              
         ...
         USING BINDSCT,R1                                                       
*                                                                               
         CLI   1(R3),10            FLT= 10 CHAR?                                
         BNE   OPT47B                                                           
         MVC   BWBFLEND,22(R3)     JUST MOVE IT IN                              
         B     OPTNEXT                                                          
*                                                                               
OPT47B   CLI   1(R3),9             FLT= 9 CHAR?                                 
         BNE   OPT47C                                                           
         MVC   BWBFLEND+1(9),22(R3)     JUST MOVE IT IN                         
         MVI   BWBFLEND,C'0'                                                    
         B     OPTNEXT                                                          
*                                                                               
OPT47C   SR    RE,RE                                                            
         ICM   RE,7,9(R3)            BINARY VALUE OF FIELD                      
         CVD   RE,DUB                                                           
         UNPK  BWBFLEND,DUB                                                     
         OI    BWBFLEND+9,X'F0'                                                 
         B     OPTNEXT                                                          
...
OPT648   DS    0H                                                               
         CLC   =C'AIROVR',12(R3)                                                
         BNE   OPT649                                                           
         CLC   =C'NO',22(R3)       IGNORE VENDOR AIRED STATION ?                
         BNE   OPTERR                                                           
         OI    NBINDS7,NBI7AIRD    IGNORE AIRED VENDOR STATION                  
         B     OPTNEXT                                                          
*                                                                               
OPT649   DS    0H                                                               
         CLC   =C'DEAL',12(R3)                                                  
         BNE   *+12                                                             
         OI    NBINDS7,NBI7DEAL    DEAL ONLY UNITS                              
         B     OPTNEXT                                                          
         CLC   =C'NODEAL',12(R3)                                                
         BNE   OPT650                                                           
         OI    NBINDS8,NBI8NODL    NO DEAL UNITS PASSED                         
         B     OPTNEXT                                                          
*                                                                               
OPT650   DS    0H                                                               
         CLC   =C'CFSUM',12(R3)                                                 
         BNE   OPT651                                                           
         OI    NBINDS8,NBI8CFSM   CFSUM OPTION FOR CASHFLOW                     
         B     OPTNEXT                                                          
*                                                                               
OPT651   DS    0H                                                               
         CLC   =C'INGMIR',12(R3)   INVOICE GHOST MIRROR                         
         BNE   OPT652                                                           
         CLC   =C'NO',22(R3)                                                    
         BNE   OPTNEXT                                                          
         OI    NBINDS8,NBI8IMIR    SET INVOICE UNIT MIRROR FLAG                 
         B     OPTNEXT                                                          
*                                                                               
OPT652   CLC   =C'NOASTSK',12(R3)     DON'T SET ASTERISK IN CMLA/CMLAB          
         BNE   OPT654                                                           
         OI    NBINDS8,NBI8AST0                                                 
         B     OPTNEXT                                                          
*                                                                               
OPT654   CLC   =C'RENTRAK',12(R3)     RENTRAK OPTION                            
         BNE   OPT656                                                           
         OI    NBINDS8,NBI8RNTK                                                 
         MVC   NBVTYPS(2),=C'PL'                                                
         OI    NBVTYPS3,X'01'      OVERRIDE UNIT VTYPS                          
         B     OPTNEXT                                                          
*                                                                               
OPT656   CLC   =C'DIGINET',12(R3)  DIGINET?                                     
         BNE   OPT658                                                           
         OI    NDOPTS1,NO1DIGIQ                                                 
         B     OPTNEXT                                                          
*                                                                               
...
WBMEDIA  DS    0H                                                               
         DC    C'N',X'01'          NETWORK                                      
         DC    C'S',X'02'          SYNDICATION                                  
         DC    C'O',X'04'          OTHER                                        
         DC    X'00'                                                            
WBMEDC   DC    C'C',X'08'          CC-CABLE/CABLE                               
         DC    C'T',X'10'          CT-CABLE/TIVO                                
         DC    C'V',X'20'          CV-CABLE/VOD                                 
         DC    X'00'                                                            
*                                                                               
...
*                                                                               
XIT2     XIT1                                                                   
*          DATA SET SPWRI0C    AT LEVEL 011 AS OF 02/05/03                      
*********************************************************************           
* VALRP2: VALIDATE REPORT2/CONTINUE OPTIONS                         *           
*  PASS R3 = A(SCAN BLOCK ENTRY)                                    *           
*                                                                   *           
*********************************************************************           
         SPACE                                                                  
VALRP2   NTR1                                                                   
         SR    RE,RE                                                            
         ICM   RE,1,1(R3)          GET L'REPORT NAME                            
         BZ    XITNEQ                                                           
...
*                                                                               
         ...
         MVI   CT01SYS,3           SET FOR NET SYSTEM                           
         MVI   CT01PRG,X'20'       SET FOR NET WRITER                           
         MVI   CT01PHAS,X'20'      SET FOR PHASE 1  (T32020)                    
         MVC   CT01NAME,22(R3)     SECOND REPORT NAME                           
*                                                                               
         MVI   RDUPDATE,C'N'                                                    
...
*                                                                               
         ...
         BNE   XITNEQ                                                           
         MVC   NDRPT2ID,CT01NAME     SAVE SECOND REPORT ID                      
         B     XITEQU                                                           
XITEQU   SR    RE,RE                                                            
XITNEQ   LTR   RE,RE                                                            
         XIT1                                                                   
...
VSTK4    LA    RF,5                5 BYTE EXPRESSIONS                           
         ...
         BE    VSTK5                                                            
         LA    R1,37                                                            
         CLC   0(4,R3),=C'PCPP'    PLAN GOAL CPP =37                            
         BE    VSTK5                                                            
         LA    R1,21                                                            
...
SKIPNET  LA    R1,8                                                             
         ...
         BE    VSTK5                                                            
         LA    R1,37                                                            
         CLC   0(2,R3),=C'PC'      PLAN GOAL CPP/CPM =37                        
         BE    VSTK5                                                            
         LA    R1,32                                                            
...
*              ROUTINE TO VALIDATE ACCOUNTING STACK                             
         SPACE 3                                                                
VALAST   NTR1                                                                   
         OI    NBINDS2,NBBILLRD     SET READ BILL FLAG                          
         ZIC   R1,1(R3)            LENGTH OF EXPRESSIONS                        
         LTR   R1,R1                                                            
         BZ    BADCOLX                                                          
         LA    R1,22(R1,R3)                                                     
...
VALUCOM  DS    0H                                                               
         ...
         CLI   OFFLINE,C'Y'        IF OFFLINE                                   
         BNE   VUCOM0                                                           
         CLC   22(2,R4),=C'E5'     UCOM=E5?                                     
         BE    *+10                YES                                          
         CLC   22(2,R4),=C'E6'     UCOM=E6?                                     
         BE    *+10                YES                                          
         CLC   22(2,R4),=C'E7'     UCOM=E7?                                     
         BE    *+10                YES                                          
         CLC   22(2,R4),=C'E8'     UCOM=E8?                                     
         BNE   *+8                 NO                                           
         OI    NDINDS1,ND8EST      SET FLAG                                     
...
*                                  GET CLT LEVEL UCOM                           
         ...
         MVC   UCSAM,NBACTAM                                                    
         MVC   UCSCLT,NBACTCLI                                                  
         OI    UCOPT,UCOTTL        GET TITLES ONLY                              
         TM    NDINDS1,ND8EST      GETTING EST UCOMMS 5-8?                      
         BZ    *+8                 NO                                           
         OI    UCOPT,UCO8EST       YES - GET 8 ESTIMATE UCOMS                   
*                                                                               
DOUCOM   GOTO1 =V(DDUCOM),ELEM                                                  
         LA    RF,ELEM                                                          
         CLI   UCERROR,0                                                        
         BE    *+6                                                              
         DC    H'0'                                                             
...
*                                                                               
         LA    R0,32                                                            
***      CLI   0(R1),X'FF'         EFFECTIVE DATE UCOM?                         
***      BNE   VUCOM3B                                                          
***      MVI   2(R1),0             YES-CLEAR DATA TYPE                          
***      B     VUCOM4                  GO SET MAX LENGTH                        
VUCOM3B  CLC   NBSELCLI,=C'ALL'    TEST ALL CLIENT REQUEST                      
         BE    VUCOM4                                                           
...
UCOMTAB  DS    0CL7                                                             
         ...
         DC    CL2'E5',AL2(UCDP1LEN-UCOMD),X'01',AL2(UCDP1TYP-UCOMD)            
         DC    CL2'E6',AL2(UCDP2LEN-UCOMD),X'02',AL2(UCDP2TYP-UCOMD)            
         DC    CL2'E7',AL2(UCDP3LEN-UCOMD),X'03',AL2(UCDP3TYP-UCOMD)            
         DC    CL2'E8',AL2(UCDP4LEN-UCOMD),X'04',AL2(UCDP4TYP-UCOMD)            
         DC    CL2'EF',AL2(32),X'FF',AL2(UCDP4TYP-UCOMD)                        
         DC    X'00'                                                            
         EJECT                                                                  
*                                                                               
...
MACENTRY DS    CL20                                                             
         ...
         SPACE 1                                                                
         DC    CL8'ASS*    ',A(ASGN),A(FRZN),X'00000000'                        
         SPACE 1                                                                
         DC    X'FF'                                                            
         SPACE 1                                                                
MCERH    DC    CL4'ERH '                                                        
...
VALRFP   NTR1  BASE=*,LABEL=*                                                   
         ...
         EX    R1,*+8                                                           
         B     *+10                                                             
         MVC   QRFPWORK(0),22(R3)  PASS SYMBOLIC NAME                           
         OC    QRFPWORK,=C'          '                                          
         GOTO1 RFP,DMCB,(R2)                                                    
         OC    QRFPWORK,QRFPWORK   ERROR                                        
         BZ    VRFPNEQ                                                          
         MVC   22(L'QRFPESC,R3),QRFPWORK                                        
*                                                                               
VRFPEQ   CR    RB,RB                                                            
         B     XIT3                                                             
VRFPNEQ  LTR   RB,RB                                                            
...
* RFP INCLUDES                                                                  
       ...
       ++INCLUDE DEDEMTABD                                                      
         SPACE 1                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'238NEWRIGEN  06/10/20'                                      
         END                                                                    
