*          DATA SET ACLFM17    AT LEVEL 027 AS OF 02/24/03                      
*PHASE T60317A,*                                                                
*INCLUDE SCANNER                                                                
*INCLUDE UNSCAN                                                                 
*INCLUDE ACPUTEL                                                                
...
BLD11    STC   R1,SCRN             SAVE THE SCREEN NUMBER                       
         ...
         EJECT                                                                  
* *********************************************************************         
* DSP000 - DISPLAY ELEMENTS                                                     
* *********************************************************************         
*                                                                               
DSP000   CLI   MODE,DSPLYREC                                                    
         BNE   CHA00                                                            
         MVC   LDGERKEY,KEY        SAVE KEY TO LEDGER RECD                      
         LA    R8,LOGIDH           R8 TO FIRST IS ENTRY                         
         USING OFD,R8                                                           
         LA    R0,MXIDS                                                         
DSP010   NI    OFIDH+1,X'FF'-X'20' UNPROTECT ID FIELDS                          
...
*                                                                               
         ...
         BNE   DSP020              NEW CHK RECD WAS NOT FOUND                   
         MVC   CHKEY,CHIO          READ CHK RECD INTO CHKIO                     
         BAS   RE,IOREAD                                                        
         BE    DSP020                                                           
         DC    H'0'                                                             
*                                                                               
DSP020   AH    R4,DATADISP                                                      
*                                                                               
*  CHECK TO SEE IF RECORD IS TOO BIG FOR =FILE                                  
*                                                                               
         ST    R4,SVREG            SAVE R4                                      
         MVI   ERRFLG,C'N'         INIT ERRFLG TO NO                            
         SR    R6,R6               ELEMENT COUNTER                              
         B     DSP040                                                           
         USING OCNELD,R4                                                        
DSP030   ZIC   R0,OCNLN                                                         
         AR    R4,R0                                                            
DSP040   CLI   0(R4),0                                                          
         BE    DSP050                                                           
         CLI   OCNEL,OCNELQ        X'54' ELEMENT                                
         BNE   DSP030                                                           
         AHI   R6,1                INCREMENT COUNTER                            
         CHI   R6,24               CAN ONLY HANDLE 24 ELEMENTS                  
         BNH   DSP030                                                           
*                                                                               
...
*                                                                               
         ...
         MVI   ERROR,X'FE'                                                      
         B     EXIT                                                             
DSP050   L     R4,SVREG            RESTORE R4                                   
*                                                                               
         B     DSP070                                                           
*                                                                               
         USING OCNELD,R4                                                        
DSP060   ZIC   R0,OCNLN            SKIP THE FIRST N (R1-1) ELEMENTS             
         AR    R4,R0                                                            
DSP070   CLI   0(R4),0             CHECK FOR END OF RECD CONDITION              
         BE    DSPX                                                             
...
*                                                                               
         ...
         USING OFD,R8                                                           
*                                                                               
DSP080   MVI   CTBYTE,0            READ FOR BINARY ID NUMBER                    
...
*                                                                               
DSP090   CLI   0(RF),X'FF'         NO SORT OPTION                               
         BE    DSP100                                                           
         CLC   OCNSORT,0(RF)                                                    
         BE    *+12                                                             
         LA    RF,L'SORTS(RF)                                                   
...
*                                                                               
DSP100   MVC   OFPOFF,OCNPOFF      POSTING OFFICE                               
         OC    OFPOFF,SPACES                                                    
...
*                                                                               
DSP110   OC    OCNDPLR,OCNDPLR     TEST LOCAL REGISTER PENDING                  
         BZ    DSP120                                                           
         LA    R6,=C'LOCAL'                                                     
         LA    R1,OCNDPLR                                                       
         BAS   RE,PDTE             DISPLAY THE PENDING DATE                     
*                                                                               
DSP120   OC    OCNSEQN,OCNSEQN     STACK SEQUENCE NUMBER                        
         BZ    DSP130                                                           
...
*                                                                               
DSP130   LTR   R5,R5               UNSCAN INTO THE STATUS FIELD                 
         BZ    DSP140                                                           
         LA    RF,OFSTAH                                                        
...
CHA27    LA    R2,OFSTAH           VALIDATE STATUS LINE                         
         ...
         BO    EXIT                                                             
         TM    OCNSTAT2,OCNSMICR+OCNS820  CAN'T HAVE MICR & 820                 
         BO    EXIT                                                             
         TM    OCNSTAT2,OCNSLBLT+OCNS820  CAN'T HAVE FTP & 820                  
         BO    EXIT                                                             
*                                                                               
         CLI   OCNLASR,OCNWSP                                                   
         BNE   CHA27A                                                           
...
*                                                                               
CHA28    EQU   *                                                                
         CLI   OCNLASR,OCNWSP      WSP CAN HAVE SOON, LOCAL                     
         BE    CHA31                                                            
*                                                                               
         CLI   OCNLASR,0           CAN'T HAVE LASER(XXX)                        
         BNE   EXIT                                                             
         TM    OCNSTAT2,OCNS820    CAN'T HAVE 820                               
         BO    EXIT                                                             
...
*MN                                                                             
         ...
         MVI   ERROR,INVALID                                                    
         LA    R2,OFBNKH                                                        
         CLC   OCNBANK-OCNEL(L'OCNBANK,R4),OCNBANK-OCNEL(RF)                    
         BNE   EXIT                                                             
         LA    R2,OFFLTH                                                        
         CLC   OCNFILT-OCNEL(L'OCNFILT,R4),OCNFILT-OCNEL(RF)                    
         BNE   EXIT                                                             
         LA    R2,OFPOFFH                                                       
         CLC   OCNPOFF-OCNEL(L'OCNPOFF,R4),OCNPOFF-OCNEL(RF)                    
         BNE   EXIT                                                             
         LA    R2,OFSTAH                                                        
         CLC   OCNLASR-OCNEL(L'OCNLASR,R4),OCNLASR-OCNEL(RF)                    
         BNE   EXIT                                                             
         CLC   OCNSTAT-OCNEL(L'OCNSTAT,R4),OCNSTAT-OCNEL(RF)                    
         BNE   EXIT                                                             
         CLC   OCNSTAT2-OCNEL(L'OCNSTAT2,R4),OCNSTAT2-OCNEL(RF)                 
         BNE   EXIT                                                             
         MVI   ERROR,X'FF'                                                      
CHA45    EQU   *                                                                
*MN                                                                             
...
*        DDLFDIND                                                               
       ...
       ++INCLUDE ACMSGEQUS                                                      
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'027ACLFM17   02/24/03'                                      
         END                                                                    
