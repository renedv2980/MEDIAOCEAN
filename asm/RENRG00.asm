*          DATA SET RENRG00    AT LEVEL 070 AS OF 03/17/04                      
*PHASE T81500B,+0                                                               
*INCLUDE GETBROAD                                                               
*INCLUDE KHDUMMY                                                                
...
*                                                                     *         
* SEP14/99 (BG )  60 FIX READ SETS FOR SALESPERSON                    *         
*                                                                     *         
* JAN20/00 (BG )  61 CLEAR MKT NAME IF ON MASTER REP                  *         
*                                                                     *         
* FEB16/00 (BG )  62 ADD MARKET SET                                   *         
*                                                                     *         
* MAR14/00 (BG )  63 FIX EXCLUSION SETS                               *         
*                                                                     *         
* APR26/00 (BG )  64 FOR SET OF SET KEYS TO SPACES, NOT NULLS         *         
*                                                                     *         
* JUN05/00 (BG )  65 ADD GROUP U                                      *         
*                                                                     *         
* NOV15/00 (BU )  66 FIX CLEARCHANNEL SET USAGE                       *         
*                                                                     *         
* JUN25/01 (BU )     SET GROUP 'F' FOR FOX STATION SALES              *         
*                                                                     *         
* OCT01/01 (BU )     ADD GROUP W                                      *         
*                                                                     *         
* MAR17/04 (HQ )     SUPPRESS ALPHA CHECK ON STATION CALL LETTER      *         
*                                                                     *         
*                                                                     *         
*                    ***  END TOMBSTONE  ***                          *         
***********************************************************************         
...
* CHECK REP REC FOR MASTER STATUS/SET PROFILE *                                 
         ...
         DC    H'0'                                                             
*                                                                               
*   CLEARCHANNEL (POWER CODE = NU) IS A STANDALONE REP, BUT MUST BE             
*        TREATED AS A SUBSIDIARY OF KRG (K3).                                   
*                                                                               
         L     RF,AIO                                                           
         USING RREPREC,RF                                                       
         CLC   =C'NU',RREPKREP     CLEARCHANNEL?                                
         BNE   VUSER30                                                          
*                                                                               
         DROP  RF                                                               
*                                                                               
         MVI   CMBOREP,C'Y'        YES - TREAT AS COMBO REP                     
         MVC   CMBOMSTR,=C'K3'     SATELLITE ON KRG                             
         B     VUSER50                                                          
VUSER30  EQU   *                                                                
         USING RREPELEM,R6                                                      
         CLC   RREPMAST,SPACES                                                  
...
*                                                                               
         CLI   RSTACODE,X'51'      IF MASTER REP, CLEAR ALL BUT 1ST 2           
         BNE   *+10                                                             
         XC    EXMKTNAM+2(L'EXMKTNAM-2),EXMKTNAM+2                              
*                                                                               
         LA    R6,RSTAELEM                                                      
         MVI   ELCODE,X'08'                                                     
         BAS   RE,NEXTEL                                                        
...
VGRP35   EQU   *                                                                
         ...
         BE    VGRP40                                                           
         CLI   QGROUP,C'U'         PMCNY ?                                      
         BE    VGRP40                                                           
         CLI   QGROUP,C'W'         WSMNY ?                                      
         BE    VGRP40                                                           
...
VMKT     DS   0H                                                                
         ...
         SPACE                                                                  
         MVC   FULL(2),=C'MK'                                                   
         MVC   FULL+2(2),=AL1(QLMKT,L'QMKT)                                     
         BAS   RE,VSET             GO VALIDATE SET                              
         MVI   QMKT,C'*'                                                        
         MVI   EXMK2NAM,C'*'                                                    
         MVC   EXMK2NAM+1(L'EXMK2NAM-1),WORK                                    
         B     EXIT                                                             
         SPACE                                                                  
VMKT20   DS   0H                                                                
         CLI   5(R2),1                                                          
...
* FOR SETS FOR AG, AD, DT, GS,    NEED TO CHECK MASTER REP                      
         ...
         BE    VSET020                                                          
         CLC   =C'SP',FULL         SALESPERSON                                  
         BE    VSET020                                                          
         CLC   =C'MK',FULL         MARKET                                       
         BE    VSET020                                                          
         DC    H'0'                                                             
...
VSET060  EX    RE,SETMVC                                                        
         SPACE                                                                  
         CLC   =C'ST',FULL         STATION                                      
         BE    VSET068             ALLOW BLANKS FOR STATION                     
         SPACE                                                                  
         ZIC   RE,1(R3)            GET ENTRY LENGTH                             
         BCTR  RE,0                                                             
         SPACE                                                                  
VSET062  DS    0H                                                               
         CLI   0(RF),0             IF NULL, END OF ENTRIES                      
         BE    VSET068                                                          
         LA    R1,0(RE,RF)         POINT TO END OF ENTRY                        
VSET064  DS    0H                                                               
         CLI   0(R1),C' '          IF NON-BLANK, GET OUT                        
         BH    VSET066                                                          
         MVI   0(R1),0             NULL BLANKS                                  
         BCTR  R1,0                                                             
         B     VSET064                                                          
VSET066  DS    0H                                                               
         LA    RF,1(RE,RF)         NEXT ENTRY                                   
         B     VSET062                                                          
         SPACE                                                                  
VSET068  DS    0H                                                               
...
VSET071  CLI   FULL+2,QLADV        IF ADV                                       
         ...
         SPACE                                                                  
VSET072  DS    0H                                                               
         CLI   FULL+2,QLMKT        THIS A MARKET SET                            
         BNE   VSET073                                                          
         CLI   3(RF),C' '          CHANGE BLANKS TO NULLS                       
         BH    VSET073                                                          
         MVI   3(RF),0                                                          
         SPACE                                                                  
         CLI   2(RF),C' '                                                       
...
RSETRECD DSECT                                                                  
       ...
       ++INCLUDE REGENSET                                                       
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'070RENRG00   03/17/04'                                      
         END                                                                    
