*          DATA SET REREPSW05  AT LEVEL 110 AS OF 02/03/04                      
*          DATA SET REREPSW03  AT LEVEL 035 AS OF 07/18/96                      
*PHASE RESW02B,*                                                                
*INCLUDE PRINT                                                                  
*INCLUDE PRNTBL                                                                 
*INCLUDE HEXIN                                                                  
*INCLUDE QSORT                                                                  
         TITLE 'REREPSW05  (RESW05) --- COMPANY MERGER: C1/C2/C3'               
*                                       UTS INTO FOX MERGER                     
*                                                                               
********************************************************************            
*                                                                  *            
*        REREPSW05  -- REP FILE COMPANY MERGER.                    *            
*                       CREATE A SINGLE FILE FROM C1/C2/C3 DATA,   *            
*                       AS ORIGINAL COMPANY 'C1'                   *            
*                                                                  *            
*  NOTE:  SEE CORRECTION WARNING IN COVERSHEET ROUTINE !!!         *            
*                                                                  *            
********************************************************************            
*                                                                  *            
*                                                                  *            
...
*                                                                  *            
*                                                                  *            
* DEC23/03 (BU ) --- ADJUST G/SG 'RS' PROCESSING                   *            
*                                                                  *            
* DEC01/03 (BU ) --- UPGRADE FOR ALLIED INTO MCGAVERN MERGE        *            
*                                                                  *            
* AUG09/01 (BU ) --- UPGRADE FOR UTS INTO FOX MERGE                *            
*                                                                  *            
* NOV27/00 (BU ) --- UPGRADE FOR INTEREP MCGAVERN/CABALLERO MERGE  *            
*                                                                  *            
* JUN05/96 (BU ) --- ORIGINAL ENTRY, BASED ON PRIOR MERGER         *            
*                    REREPSW02B (REP1/REP2)                        *            
*                                                                  *            
* FEB24/96 (PXZ) --- ORIGINAL ENTRY, BASED ON REREPSW03            *            
*                                                                  *            
*                    **  END TOMBSTONE  **                         *            
********************************************************************            
...
*                 =  D  =  DARE RECORD                             *            
*     QUESTOR+9   =  STATION DISPLAY:  ONLY MERGE!                 *            
...
*                                                                               
         GOTO1 =A(BUYPROC),DMCB        PROCESS BUY RECORDS                      
*                                                                               
         GOTO1 =A(COVPROC),DMCB        PROCESS COVERSHEETS                      
*                                                                               
***      GOTO1 =A(REPRECRD),DMCB       STRIP REP RECORD                         
*                                                                               
         GOTO1 =A(DAREPROC),DMCB       STRIP DARE RECORDS                       
*                                                                               
MAINEXIT EQU   *                                                                
...
STAT0040 EQU   *                                                                
         ...
         BNE   STAT0120            NO  - GO PROCESS REP2                        
         ...
         LA    R1,RSTAELEM                                                      
         LA    RF,STADS1CM         SET A(COMBO STATIONS)                        
STAT0060 EQU   *                                                                
         CLI   0(R1),0             END OF RECORD?                               
         BE    STAT0100            YES                                          
         CLI   0(R1),X'0A'         COMBINED STATION ELEMENT?                    
         BNE   STAT0080            YES                                          
         CLI   RSTACPRF-RSTACSEL(R1),C'-'                                       
*                                  NON-PARTICIPANT?                             
         BE    STAT0080            YES - SKIP STATION                           
         MVC   0(5,RF),RSTACS-RSTACSEL(R1)                                      
*                                  NO  - INSERT STATION INTO TABLE              
         LA    RF,5(RF)            BUMP TO NEXT SLOT W/IN STATION               
STAT0080 EQU   *                                                                
         ZIC   RE,1(R1)            GET ELEMENT LENGTH                           
         AR    R1,RE               BUMP TO NEXT ELEMENT                         
         B     STAT0060            GO BACK FOR NEXT ELEMENT                     
STAT0100 EQU   *                                                                
...
*      FOR SEARCH.                                                              
*                                                                               
         XC    KEY,KEY             SET UP STATION KEY FOR REP2                  
         MVI   KEY,2                                                            
         MVC   KEY+20(2),REP2                                                   
         GOTO1 HIGHDIR             RETRIEVE FIRST KEY                           
         B     STAT0160                                                         
...
STAT0160 EQU   *                                                                
         CLC   KEY(22),KEYSAVE     STATION RECORD/SAME COMPANY?                 
         BNE   STAT0240            NO  - GO GENERATE RECORDS                    
         GOTO1 GETRECRD            YES - RETRIEVE THE RECORD                    
         BAS   RE,FINDSTAT         LOOK FOR STATION IN TABLE                    
         BZ    STAT0140            FOUND - DATA INSERTED                        
*                                     GO BACK FOR NEXT                          
*                                  NOT FOUND - INSERT INFO IN NEXT SLOT         
         ...
         LA    R1,RSTAELEM                                                      
         LA    RF,STADS2CM         SET A(COMBO STATIONS)                        
STAT0180 EQU   *                                                                
         CLI   0(R1),0             END OF RECORD?                               
         BE    STAT0220            YES                                          
         CLI   0(R1),X'0A'         COMBINED STATION ELEMENT?                    
...
STAT0200 EQU   *                                                                
         ZIC   RE,1(R1)            GET ELEMENT LENGTH                           
         AR    R1,RE               BUMP TO NEXT ELEMENT                         
         B     STAT0180            GO BACK FOR NEXT ELEMENT                     
STAT0220 EQU   *                                                                
         LA    R7,STADLEN(R7)      BUMP TO NEXT TABLE ENTRY                     
         ST    R7,NEXTAREA         SAVE A(NEXT OPEN SLOT)                       
         B     STAT0140            GO BACK FOR NEXT                             
STAT0240 EQU   *                                                                
*                                                                               
...
STAT0280 EQU   *                                                                
         CLC   KEY(22),KEYSAVE     STATION RECORD/SAME COMPANY?                 
         BNE   STAT0360            NO  - GO GENERATE RECORDS                    
         GOTO1 GETRECRD            YES - RETRIEVE THE RECORD                    
         BAS   RE,FINDSTA2         LOOK FOR STATION IN TABLE                    
         BZ    STAT0260            FOUND - DATA INSERTED                        
*                                     GO BACK FOR NEXT                          
*                                  NOT FOUND - INSERT INFO IN NEXT SLOT         
         L     R7,NEXTAREA         SET A(NEXT OPEN SLOT)                        
         MVC   STADCALL,RSTAKSTA   INSERT STATION CALLS                         
         ...
         LA    R1,RSTAELEM                                                      
         LA    RF,STADS3CM         SET A(COMBO STATIONS)                        
STAT0300 EQU   *                                                                
         CLI   0(R1),0             END OF RECORD?                               
         BE    STAT0340            YES                                          
         CLI   0(R1),X'0A'         COMBINED STATION ELEMENT?                    
...
STAT0320 EQU   *                                                                
         ZIC   RE,1(R1)            GET ELEMENT LENGTH                           
         AR    R1,RE               BUMP TO NEXT ELEMENT                         
         B     STAT0300            GO BACK FOR NEXT ELEMENT                     
STAT0340 EQU   *                                                                
         LA    R7,STADLEN(R7)      BUMP TO NEXT TABLE ENTRY                     
         ST    R7,NEXTAREA         SAVE A(NEXT OPEN SLOT)                       
         B     STAT0260            GO BACK FOR NEXT                             
STAT0360 EQU   *                                                                
*                                                                               
*   TEST                                                                        
*        DC    H'0'                                                             
...
STAT0380 EQU   *                                                                
         ...
         CLI   QUESTOR+0,C'0'      0TH FLAG SET?                                
         BE    STAT0400            NO                                           
         CLI   QUESTOR+9,C'9'      9TH FLAG SET?                                
         BE    STAT0400            NO                                           
         MVI   P+1,C'*'            YES - SET DIVIDER                            
         MVC   P+2(80),P+1                                                      
         GOTO1 REPORT                                                           
         GOTO1 REPORT                                                           
STAT0400 EQU   *                                                                
         CLC   =C'DISPTABL',QUESTOR                                             
         BNE   STAT0420            NO SPECIAL DISPLAY                           
         MVC   P+20(STADLEN),0(R7)  DISPLAY TABLE ENTRY                         
         MVC   P+1(12),=C'TABLE ENTRY:'                                         
         GOTO1 REPORT                                                           
STAT0420 EQU   *                                                                
         OC    STADCALL,STADCALL   ANY ENTRY?                                   
         BZ    STAT0580            NO  - STATION PROCESSING COMPLETE            
*                                     SHOW GROUP TABLE                          
         OC    STADS1DA,STADS1DA   REP1 STATION?                                
         BZ    STAT0460            NO                                           
         OC    STADS2DA,STADS2DA   YES - REP2 STATION ALSO?                     
         BNZ   STAT0540            YES - COMPARE DATES                          
         OC    STADS3DA,STADS3DA   NO  - REP3 STATION ALSO?                     
         BNZ   STAT0540            YES - COMPARE DATES                          
*                                  NO  - GEN REC FROM REP1                      
*                                                                               
*   A STATION THAT EXISTS ON REP1 ONLY DOES NOT HAVE TO BE OUTPUT               
*        TO THE TAPE.  IT REMAINS UNCHANGED ON THE REP1 FILE.                   
*                                                                               
...
***      MVC   RSTAKREP,NEWREP     INSERT NEW REP CODE                          
         ...
         BNE   STAT0440            NOT SPECIAL REQUEST                          
         MVC   P+1(33),=C'STATION WXXXA EXISTS ON REP1 ONLY'                    
         MVC   P+09(5),STADCALL                                                 
         GOTO1 REPORT                                                           
***>>>   GOTO1 DISPPUT,DMCB,(RC)                                                
STAT0440 EQU   *                                                                
         L     RF,STA1CTR          BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,STA1CTR          SAVE IT BACK                                 
***      BAS   RE,PUTRECS          GENERATE NEW OUTPUT                          
***      GOTO1 DISPSTAT,DMCB,(RC),(R7),1                                        
*                                  DISPLAY STATION INFORMATION                  
         LA    R7,STADLEN(R7)      BUMP TO NEXT TABLE ENTRY                     
         B     STAT0380            GO BACK FOR NEXT                             
STAT0460 EQU   *                                                                
         OC    STADS2DA,STADS2DA   REP2 STATION?                                
         BZ    STAT0500            NO  - ONLY REP3 LEFT                         
         OC    STADS3DA,STADS3DA   YES - REP3 STATION ALSO?                     
         BNZ   STAT0540            YES - COMPARE AND GEN                        
*                                  NO  - GEN REC FROM REP2                      
         ...
         MVC   RSTAKREP,NEWREP     INSERT NEW REP CODE                          
*                                                                               
*   ALL OUTPUT RECORDS WILL BE FOR THE GROUP/SUBGROUP OF THE NEWREP.            
*                                                                               
         MVC   RSTAGRUP,=C'RM'     FORCE 'RM' INTO GROUP/SUBGRP                 
*                                                                               
         CLI   QUESTOR+0,C'Y'      DISPLAY STATION DATA?                        
         BNE   STAT0480            NOT SPECIAL REQUEST                          
         MVC   P+1(33),=C'STATION WXXXA EXISTS ON REP2 ONLY'                    
         MVC   P+09(5),STADCALL                                                 
         GOTO1 REPORT                                                           
         GOTO1 DISPPUT,DMCB,(RC)                                                
STAT0480 EQU   *                                                                
         L     RF,STA2CTR          BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,STA2CTR          SAVE IT BACK                                 
         BAS   RE,PUTRECS          GENERATE NEW OUTPUT                          
...
*                                                                               
         CLI   QUESTOR+0,C'Y'      DISPLAY STATION DATA?                        
         BNE   STAT0520            NOT SPECIAL REQUEST                          
         MVC   P+1(33),=C'STATION WXXXA EXISTS ON REP3 ONLY'                    
         MVC   P+09(5),STADCALL                                                 
         GOTO1 REPORT                                                           
         GOTO1 DISPPUT,DMCB,(RC)                                                
STAT0520 EQU   *                                                                
         L     RF,STA3CTR          BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,STA3CTR          SAVE IT BACK                                 
         BAS   RE,PUTRECS          GENERATE NEW OUTPUT                          
...
STAT0580 EQU   *                                                                
         ...
         L     R3,AGRPAREA         DISPLAY GROUP/SUBGROUP AREA                  
STAT0590 EQU   *                                                                
         MVC   P+1(13),=C'GROUP-SUBGRP:'                                        
         MVC   P+20(GRPDLEN),0(R3)                                              
         GOTO1 REPORT                                                           
         LA    R3,GRPDLEN(R3)      BUMP TO NEXT SLOT                            
         OC    0(9,R3),0(R3)       ANYTHING IN SLOT?                            
         BNZ   STAT0590            YES - GO BACK AND DISPLAY                    
         L     R3,ACOMAREA         DISPLAY COMM/BUDGET AREA                     
STAT0600 EQU   *                                                                
         OC    0(9,R3),0(R3)       ANYTHING IN SLOT?                            
         BZ    STAT0900            NO  - FINISHED                               
         MVC   P+1(12),=C'COMM-BUDGET:'                                         
         MVC   P+16(9),0(R3)                                                    
         GOTO1 REPORT                                                           
         LA    R3,9(R3)            BUMP TO NEXT SEGMENT                         
         B     STAT0600                                                         
STAT0900 EQU   *                                                                
         XIT1                                                                   
         EJECT                                                                  
...
FIND0020 EQU   *                                                                
         ...
         BE    FIND0060            FOUND - INSERT REP2 DATA INTO SLOT           
...
FIND0080 EQU   *                                                                
         ZIC   RE,1(R1)            GET ELEMENT LENGTH                           
         AR    R1,RE               BUMP TO NEXT ELEMENT                         
         B     FIND0070            GO BACK FOR NEXT ELEMENT                     
FIND0090 EQU   *                                                                
...
*&&DO                                                                           
*   TEST                                                                        
         MVC   P+1(10),=C'STA2COMBO:'                                           
         MVC   P+20(20),STADS2CM                                                
         GOTO1 REPORT                                                           
*   TEST END                                                                    
*&&                                                                             
...
FIN20020 EQU   *                                                                
         CLC   STADCALL,RSTAKSTA   TABLE VS STATION RECORD                      
         BE    FIN20060            FOUND - INSERT REP2 DATA INTO SLOT           
         LA    R7,STADLEN(R7)                                                   
         L     RF,NEXTAREA         CHECK FOR END OF TABLE                       
         CR    R7,RF               END REACHED?                                 
         BNE   FIN20020            NO                                           
...
FIN20080 EQU   *                                                                
         ZIC   RE,1(R1)            GET ELEMENT LENGTH                           
         AR    R1,RE               BUMP TO NEXT ELEMENT                         
         B     FIN20070            GO BACK FOR NEXT ELEMENT                     
FIN20100 EQU   *                                                                
         CLI   QUESTOR+2,C'Y'      TABLE MATCH DISPLAY?                         
         BNE   FIN20120            NO                                           
         MVC   P+1(12),=C'TABLE MATCH:'                                         
...
*                                                                               
         ...
         OC    STADS2JN,STADS2JN   REP2 JOIN DATE PRESENT?                      
         BZ    STDA0144            NO                                           
         CLC   JOINDATE,STADS2JN   HELD JD EARLIER THAN THIS ONE?               
         BNH   STDA0144            YES                                          
         MVC   JOINDATE,STADS2JN   NO  - THIS DATE BECOMES EARLIEST             
STDA0144 EQU   *                                                                
         OC    STADS3JN,STADS3JN   REP3 JOIN DATE PRESENT?                      
         BZ    STDA0148            NO                                           
         CLC   JOINDATE,STADS3JN   HELD JD EARLIER THAN THIS ONE?               
         BNH   STDA0148            YES                                          
         MVC   JOINDATE,STADS3JN   NO  - THIS DATE BECOMES EARLIEST             
STDA0148 EQU   *                                                                
*                                                                               
*   NOW, JOINDATE SHOULD HOLD THE EARLIEST JOIN DATE FROM THE RECORDS           
...
*                                                                               
         CLC   STADS1CL,RSTACLDT   REP1 DATE VS STA REC DATE                    
         BNH   STDA0330            REP1 LESS OR EQUAL                           
         MVC   RSTACLDT,STADS1CL   REP1 DATE LATER: USE IT                      
STDA0330 EQU   *                                                                
         CLC   STADS2CL,RSTACLDT   REP2 DATE VS STA REC DATE                    
         BNH   STDA0332            REP2 LESS OR EQUAL                           
         MVC   RSTACLDT,STADS2CL   REP2 DATE LATER: USE IT                      
STDA0332 EQU   *                                                                
         CLC   STADS3CL,RSTACLDT   REP3 DATE VS STA REC DATE                    
         BNH   STDA0334            REP3 LESS OR EQUAL                           
         MVC   RSTACLDT,STADS3CL   REP3 DATE LATER: USE IT                      
STDA0334 EQU   *                                                                
         L     RF,STAMCTR          BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,STAMCTR          SAVE IT BACK                                 
*                                                                               
*   ALL OUTPUT RECORDS WILL BE FOR THE GROUP/SUBGROUP OF THE NEWREP.            
*                                                                               
         MVC   RSTAGRUP,=C'RM'     FORCE 'RM' INTO GROUP/SUBGRP                 
*                                                                               
         CLC   STADS1GP,SPACES     NEWREP G/SG PRESENT?                         
         BNH   STDA0335            NO                                           
         MVC   RSTAGRUP,STADS1GP   YES - FORCE NEWREP GROUP/SUBGRP              
STDA0335 EQU   *                                                                
...
*                                  DISPLAY STATION INFORMATION                  
         MVC   KEY(27),RSTAREC     SET REP1 RECORD KEY                          
         MVC   KEY+20(2),NEWREP    INSERT REP1 INTO KEY                         
         GOTO1 HIGHDIR             RETRIEVE REP1 STATION RECORD                 
         CLC   KEY(27),KEYSAVE     KEY FOUND?                                   
         BE    *+6                                                              
         DC    H'0'                NO                                           
         GOTO1 GETRECRD            RETRIEVE RECORD                              
*                                                                               
         OI    RSTACNTL,X'80'      MARK STATION RECORD DELETED                  
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   STDA0360            NO  - SKIP REWRITE                           
         GOTO1 PREC                REWRITE RECORD AS DELETED                    
STDA0360 EQU   *                                                                
*                                                                               
         OI    KEY+27,X'80'        MARK RECORD KEY DELETED                      
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   STDA0380            NO  - SKIP REWRITE                           
         GOTO1 WRT                 REWRITE KEY AS DELETED                       
STDA0380 EQU   *                                                                
         L     RF,STA1MKD          INCREASE DROPPED STATION COUNT               
         LA    RF,1(RF)                                                         
         ST    RF,STA1MKD          REPLACE                                      
         CLI   QUESTOR+0,C'X'      DISPLAY STATION DROPS?                       
         BE    STDA0400            NOT SPECIAL REQUEST                          
         CLI   QUESTOR+0,C'Y'      DISPLAY STATION DATA?                        
         BNE   STDA0420            NOT SPECIAL REQUEST                          
STDA0400 EQU   *                                                                
         MVC   P+1(16),=C'STATION DROPPED:'                                     
         MVC   P+20(34),KEY                                                     
         GOTO1 REPORT                                                           
         GOTO1 DISPPUT,DMCB,(RC)                                                
STDA0420 EQU   *                                                                
...
DIAT0080 EQU   *                                                                
         CLI   QUESTOR+9,C'Y'      ONLY MERGE LINES DISPLAYED?                  
         BNE   DIAT0100            NO  - DISPLAY ALL                            
         CLC   =C'MERGE',P+23      YES - MERGE LINE?                            
         BE    DIAT0100            YES - DISPLAY IT                             
         MVC   P,SPACES            NO  - CLEAR PRINT LINE                       
         B     DIAT0120                                                         
DIAT0100 EQU   *                                                                
         GOTO1 REPORT                                                           
         CLI   DBLSPACE,C'N'       FROM REP1+REP2?                              
         BE    DIAT0120            YES - DON'T DOUBLE SPACE                     
         GOTO1 REPORT                                                           
DIAT0120 EQU   *                                                                
         XIT1                                                                   
*                                                                               
...
*     TAKEN FROM BOTH REP1 AND REP2 RECORDS.                     *              
*                                                                *              
******************************************************************              
*                                                                               
XTRADISP NTR1                                                                   
...
*        BZ    XTRA0020            NO                                           
*        MVC   P+1(10),=C'REP1 JOIN:  '                                         
*        GOTO1 DATCON,DMCB,(3,STADS1JN),(5,P+11)                                
*        MVC   P+20(10),=C'REP2 JOIN:  '                                        
*        GOTO1 DATCON,DMCB,(3,STADS2JN),(5,P+30)                                
*        GOTO1 REPORT                                                           
*        B     XTRA0150            EXIT                                         
XTRA0020 EQU   *                                                                
*        OC    REP2OPEN,REP2OPEN   REP2 CURRENT?                                
*        BZ    XTRA0040            NO                                           
*                                  YES - REP2 USED TO OUTPUT RECORD             
*        MVC   P+1(10),=C'REP1 JOIN:  '                                         
*        GOTO1 DATCON,DMCB,(3,STADS1JN),(5,P+11)                                
*        MVC   P+20(10),=C'REP1 LEFT:  '                                        
*        GOTO1 DATCON,DMCB,(3,STADS1LV),(5,P+30)                                
*        MVC   P+40(10),=C'REP2 JOIN:  '                                        
*        GOTO1 DATCON,DMCB,(3,STADS2JN),(5,P+50)                                
*        MVC   P+70(02),REP2                                                    
*        CLI   REP2OPEN,C'H'       REP2 START DATE USED?                        
*        BE    XTRA0030                                                         
*        MVC   P+70(02),REP1                                                    
XTRA0030 EQU   *                                                                
...
*        B     XTRA0150            EXIT                                         
XTRA0040 EQU   *                                                                
*        OC    REP1OPEN,REP1OPEN   REP1 CURRENT?                                
*        BZ    XTRA0100            NO                                           
*                                  YES - REP1 USED TO OUTPUT RECORD             
*        MVC   P+1(10),=C'REP2 JOIN:  '                                         
*        GOTO1 DATCON,DMCB,(3,STADS2JN),(5,P+11)                                
*        MVC   P+20(10),=C'REP2 LEFT:  '                                        
*        GOTO1 DATCON,DMCB,(3,STADS2LV),(5,P+30)                                
*        MVC   P+40(10),=C'REP1 JOIN:  '                                        
*        GOTO1 DATCON,DMCB,(3,STADS1JN),(5,P+50)                                
*        MVC   P+70(02),REP1                                                    
*        CLI   REP1OPEN,C'D'       REP1 START DATE USED?                        
*        BE    XTRA0060                                                         
*        MVC   P+70(02),REP2                                                    
XTRA0060 EQU   *                                                                
*        MVC   P+73(15),=C'START DATE USED'                                     
...
*   TEST                                                                        
         ...
         CLC   =C'RL',SAVEGRP                                                   
         BE    TEST0020                                                         
         MVC   P+10(28),=C'STRANGE GROUP/SUBGROUP      '                        
         MVC   P+40(2),SAVEGRP                                                  
         GOTO1 REPORT                                                           
TEST0020 EQU   *                                                                
*   TEST END                                                                    
*&&                                                                             
         CLI   STADCALL+4,C'C'     COMBO STATION?                               
         BNE   XTRA0176                                                         
         MVC   P+1(06),=C'COMBO:'                                               
         MVC   P+10(08),=C'MATCHED '                                            
         CLC   STADS1CM,STADS2CM                                                
         BE    XTRA0172                                                         
         MVC   P+10(11),=C'MIS-MATCHED'                                         
XTRA0172 EQU   *                                                                
         MVC   P+30(5),=C'REP1:'                                                
         MVC   P+40(20),STADS1CM                                                
         MVC   P+70(5),=C'REP2:'                                                
         MVC   P+80(20),STADS2CM                                                
         GOTO1 REPORT                                                           
XTRA0176 EQU   *                                                                
*                                                                               
...
*        THESE ARE ONLY STATIONS THAT OVERLAP IN BOTH REPS.      *              
******************************************************************              
*                                                                               
TABLSTAT NTR1                                                                   
         L     RC,0(R1)            RESET A(WORKSPACE)                           
...
*        BNE   TSTA0020            NO                                           
*****>   MVC   COMDREPB,REP2       YES - FORCE 'REP2' FOR BUDGET                
*        MVC   COMDREPC,REP2       YES - FORCE 'REP2' FOR COMMISSION            
TSTA0020 EQU   *                                                                
...
BUDGPROC NTR1                                                                   
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         GOTO1 BUDGCOMP,DMCB,(RC)  ROLL UP COMPANY BUDGETS                      
         LA    R3,REC                                                           
         USING RBUDREC,R3                                                       
...
*                                                                               
         CLC   KEY+20(4),=C'C*MP'  COMPANY BUDGET?                              
         BE    BUDG0020            YES - SKIP: ALREADY PROCESSED                
*                                                                               
         GOTO1 GETRECRD            ALWAYS RETRIEVE RECORD                       
         GOTO1 BUDGALT,DMCB,(RC)                                                
*                                  CHECK IF STATION/REP IS TO BE                
*                                     USED:  IF NOT, SKIP BEFORE READ           
         BZ    BUDG0045            CC = ZERO: IN TABLE, CORRECT REP             
*                                     KEEP IT                                   
         B     BUDG0060            CC = NOT ZERO: SKIP THIS RECORD              
*                                                                               
*   RETURN CONDITIONS FROM BUDGALT:                                             
*        CC        HALF                                                         
*        --        ----                                                         
*        NZ         0       NOT IN TABLE: LEAVE ON FILE AS IS                   
*                                  REP1 RECORDS REMAIN ON FILE                  
*                                  REP2 RECORDS BECOME REP1 RECORDS             
*        NZ         1       IN TABLE, BUT NOT USED:  DROP IT                    
*         Z        N/A      KEEP THIS ONE, SET CORRECT REP CODE                 
*                                                                               
BUDG0045 EQU   *                                                                
         MVC   REC-4(2),RBUDLEN    INSERT LENGTH FOR PUT                        
         L     RF,BUDCTR           BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,BUDCTR           SAVE IT BACK                                 
         CLC   RBUDKREP,NEWREP     BUDGET FOR REP1?                             
         BE    BUDG0050            YES                                          
BUDG0047 EQU   *                                                                
         ...
         MVC   P+25(11),RBUDKREP   INSERT BUDGET KEY                            
         MVI   P+36,C'/'                                                        
         MVC   P+37(2),KEY+16                                                   
         GOTO1 REPORT                                                           
****>    GOTO1 DISPPUT,DMCB,(RC)                                                
...
BUDG0050 EQU   *                                                                
         ...
         B     BUDG0020            GO BACK FOR NEXT                             
BUDG0060 EQU   *                                                                
         CLI   HALF,1              BUDGET OVERLAP AND DROP?                     
         BE    BUDG0080            YES                                          
         CLC   RBUDKREP,REP2       BUDGET RECORD FOR REP2?                      
         BE    BUDG0047            YES - MOVE AS A REP1 RECORDS                 
         ...
         B     BUDG0020            GO BACK FOR NEXT                             
BUDG0080 EQU   *                                                                
         CLC   RBUDKREP,REP1       BUDGET RECORD FOR REP1?                      
         BNE   BUDG0020            NO  - ONLY DROP REP1'S RECORDS               
*                                     REP2 RECS ARE IGNORED                     
         OI    RBUDCNTL,X'80'      MARK BUDGET RECORD DELETED                   
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   BUDG0085            NO  - SKIP REWRITE                           
         GOTO1 PREC                REWRITE RECORD AS DELETED                    
BUDG0085 EQU   *                                                                
*                                                                               
         OI    KEY+27,X'80'        MARK RECORD KEY DELETED                      
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   BUDG0090            NO  - SKIP REWRITE                           
         GOTO1 WRT                 REWRITE KEY AS DELETED                       
BUDG0090 EQU   *                                                                
         L     RF,BUDMKD           INCREASE DROPPED BUDGET  COUNT               
         LA    RF,1(RF)                                                         
         ST    RF,BUDMKD           REPLACE                                      
         MVC   P+1(22),=C'BUDGET RECORD DROPPED:'                               
         MVC   P+25(34),KEY                                                     
         MVI   P+59,C'/'                                                        
         MVC   P+60(36),RBUDREC                                                 
         GOTO1 REPORT                                                           
...
*                                                                               
******************************************************************              
*  BUDGCOMP:  SWEEP REP1 BUDGET RECORDS.  FOR ALL C*MP BUDGETS,  *              
*        SEE IF THERE IS A CORRESPONDING REP2 BUDGET.            *              
*        IF PRESENT, ADD REP2 FIGURES TO REP1, REWRITE RECORD    *              
******************************************************************              
*                                                                               
BUDGCOMP NTR1                                                                   
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         LA    R3,REC2                                                          
         USING RBUDREC,R3                                                       
         MVC   REPCODE,REP1        READ ALL REP1 BUDGETS                        
*                                                                               
BCOM0020 EQU   *                                                                
         XC    KEY,KEY                                                          
         MVI   KEY,X'13'           SET UP BUDGET KEY                            
...
BCOM0060 EQU   *                                                                
         CLC   KEY(18),KEYSAVE     BUDGET RECORD/SAME COMPANY?                  
         BNE   BCOM0200            NO  - FINISHED                               
*                                                                               
         CLC   KEY+20(4),=C'C*MP'  COMPANY BUDGET?                              
         BNE   BCOM0040            NO  - SKIP                                   
         MVC   BDGTKEY,KEY         SAVE REP1 BUDGET KEY FOR RESTART             
         MVC   KEY+16(2),REP2      LOOK FOR MATCHING REP2 C*MP BUDGET           
         GOTO1 HIGHDIR             READ REP2 KEY                                
         CLC   KEY+18(9),BDGTKEY+18  SAME KEY SOUTH OF REP CODE?                
         BE    BCOM0080            YES - PROCESS RECORDS TOGETHER               
         MVC   KEY(27),BDGTKEY     NO  - RESTART REP1 BUDGETS                   
         GOTO1 HIGHDIR                                                          
         CLC   KEY(27),BDGTKEY     KEY FOUND?                                   
         BE    BCOM0040            YES - CONTINUE TO PROCESS                    
         DC    H'0'                THIS SHOULDN'T HAVE HAPPENED                 
BCOM0080 EQU   *                                                                
         MVC   P+1(09),=C'REP1 BUD='                                            
         MVC   P+10(27),BDGTKEY                                                 
         GOTO1 REPORT                                                           
         MVC   P+1(09),=C'REP2 BUD='                                            
         MVC   P+10(27),KEY                                                     
         GOTO1 REPORT                                                           
         GOTO1 GETBDGT             READ REP2 BUDGET REC INTO REC2               
*                                                                               
         MVC   KEY(27),BDGTKEY     GET REP1 BUDGET KEY AGAIN                    
         GOTO1 HIGHDIR                                                          
         CLC   KEY(27),BDGTKEY     KEY FOUND?                                   
         BE    BCOM0100            YES - CONTINUE TO PROCESS                    
         DC    H'0'                SHOULDN'T HAPPEN                             
BCOM0100 EQU   *                                                                
         GOTO1 GETRECRD            GET REP1 BUDGET INTO REC                     
         MVC   P+1(10),=C'REC  AREA:'                                           
         GOTO1 REPORT                                                           
         LA    RF,200                                                           
         LA    R4,REC                                                           
         GOTO1 =V(PRNTBL),DMCB,(0,(R4)),(R4),C'DUMP',(RF),=C'1D'                
         GOTO1 REPORT                                                           
         MVC   P+1(10),=C'REC2 AREA:'                                           
         GOTO1 REPORT                                                           
         LA    RF,200                                                           
         LA    R4,REC2                                                          
         GOTO1 =V(PRNTBL),DMCB,(0,(R4)),(R4),C'DUMP',(RF),=C'1D'                
         ...
         GOTO1 REPORT                                                           
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   BCOM0120            NO  - SKIP REWRITE                           
         GOTO1 PREC                REWRITE RECORD AS DELETED                    
BCOM0120 EQU   *                                                                
         B     BCOM0040            GO BACK FOR NEXT REP1 REC                    
BCOM0200 EQU   *                                                                
         XIT1                                                                   
BDGTKEY  DS    CL27                SAVE KEY FOR RESTART                         
         DROP  R3                                                               
         EJECT                                                                  
*                                                                               
*                                                                               
******************************************************************              
*  BUDGADD :  ACCUMULATE BUDGET MONTHLY FIGURES IN REC AND REC2  *              
*             FINAL FIGURES ARE PLACED IN REC, AND THIS IS       *              
...
BUDGADD  NTR1                                                                   
         USING RBUDREC,R1                                                       
         LA    R1,REC                                                           
         LA    R2,RBUDSTA          SET A(1ST BUCKET IN REP1 BUDGET)             
         LA    R1,REC2                                                          
         LA    R3,RBUDSTA          SET A(1ST BUCKET IN REP2 BUDGET)             
*                                                                               
         DROP  R1                                                               
*                                                                               
         LA    R1,25               SET LOOP                                     
BADD0020 EQU   *                                                                
         ZICM  RE,0(R2),4          EXTRACT REP1 MONTH BUDGET                    
         ZICM  RF,0(R3),4          EXTRACT REP2 MONTH BUDGET                    
         AR    RE,RF               ADD BUDGETS TOGETHER                         
         STCM  RE,15,0(R2)         REPLACE REP1 MONTHLY BUDGET                  
         LA    R2,4(R2)            BUMP TO NEXT REP1 MONTH BUDGET               
         LA    R3,4(R3)            BUMP TO NEXT REP2 MONTH BUDGET               
         BCT   R1,BADD0020         GO BACK FOR NEXT BUCKET                      
         XIT1  FINISHED                                                         
         EJECT                                                                  
*                                                                               
******************************************************************              
...
BUDGALT  NTR1                                                                   
         ...
         USING COMDSECT,R2                                                      
         MVI   HALF,0              SET 'STATION NOT OVERLAPPING'                
BALT0020 EQU   *                                                                
         OC    COMDCALL,COMDCALL   ANY TABLE ENTRY?                             
         BZ    BALT0100            NO  - EXIT CC NOT ZERO                       
         CLC   COMDCALL,KEY+20     STATION MATCH IN TABLE?                      
         BNE   BALT0040            NO  - CHECK NEXT                             
         CLC   COMDREPB,KEY+16     YES - SAME REP?                              
         BE    BALT0120            YES - KEEP IT                                
BALT0030 EQU   *                                                                
         MVI   HALF,1              SET 'STA OVERLAP: DROP IT'                   
...
*                                  CHECK IF STATION/REP IS TO BE                
*                                     USED:  IF NOT, SKIP BEFORE READ           
         BZ    COMM0045            CC = ZERO: IN TABLE, CORRECT REP             
*                                     KEEP IT                                   
         B     COMM0060            CC = NOT ZERO: SKIP THIS RECORD              
*                                                                               
*   RETURN CONDITIONS FROM COMMALT:                                             
*        CC        HALF                                                         
*        --        ----                                                         
*        NZ         0       NOT IN TABLE: LEAVE ON FILE AS IS                   
*                                  REP1 RECORDS REMAIN ON FILE                  
...
COMM0045 EQU   *                                                                
         ...
         L     RF,COMMCTR          BUMP COUNTER                                 
         LA    RF,1(RF)                                                         
         ST    RF,COMMCTR          SAVE IT BACK                                 
         CLC   RCOMKREP,NEWREP     COMMISSION FOR REP1?                         
         BE    COMM0050            YES                                          
COMM0047 EQU   *                                                                
...
COMM0050 EQU   *                                                                
         CLI   QUESTOR+6,C'Y'      DISPLAY COMM   RECORD?                       
         BNE   COMM0020            NOT SPECIAL REQUEST                          
         MVC   P+1(17),=C'REP1 COMMIS KEPT:'                                    
         MVC   P+20(14),RCOMKREP   INSERT COMM   KEY                            
         MVI   P+34,C'/'                                                        
         MVC   P+35(2),KEY+16                                                   
         GOTO1 REPORT                                                           
         B     COMM0020            GO BACK FOR NEXT                             
COMM0060 EQU   *                                                                
         CLI   HALF,1              COMM   OVERLAP AND DROP?                     
         BE    COMM0080            YES                                          
         CLC   RCOMKREP,REP2       COMM   RECORD FOR REP2?                      
         BE    COMM0047            YES - MOVE AS A REP1 RECORDS                 
         ...
         B     COMM0020            GO BACK FOR NEXT                             
COMM0080 EQU   *                                                                
         CLC   RCOMKREP,REP1       COMM   RECORD FOR REP1?                      
         BNE   COMM0020            NO  - ONLY DROP REP1'S RECORDS               
*                                     REP2 RECS ARE IGNORED                     
*                                                                               
         OI    RCOMCNTL,X'80'      MARK COMM   RECORD DELETED                   
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   COMM0085            NO  - SKIP REWRITE                           
         GOTO1 PREC                REWRITE RECORD AS DELETED                    
COMM0085 EQU   *                                                                
*                                                                               
         OI    KEY+27,X'80'        MARK RECORD KEY DELETED                      
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   COMM0090            NO  - SKIP REWRITE                           
         GOTO1 WRT                 REWRITE KEY AS DELETED                       
COMM0090 EQU   *                                                                
         L     RF,COMMMKD          INCREASE DROPPED COMM    COUNT               
         LA    RF,1(RF)                                                         
         ST    RF,COMMMKD          REPLACE                                      
         MVC   P+1(22),=C'COMMIS RECORD DROPPED:'                               
         MVC   P+25(34),KEY                                                     
         MVI   P+59,C'/'                                                        
         MVC   P+60(36),RCOMREC                                                 
         GOTO1 REPORT                                                           
...
CALT0020 EQU   *                                                                
         ...
         BE    CALT0120            YES - KEEP IT                                
         MVI   HALF,1              SET 'STA OVERLAP: DROP IT'                   
         B     CALT0100            NO  - EXIT CC ZERO                           
CALT0040 EQU   *                                                                
         LA    R2,COMDLEN(R2)      BUMP TO NEXT SLOT                            
...
CONTPROC NTR1                                                                   
         ...
         USING RCONREC,R3                                                       
*                                                                               
*   ONLY PROCESS REP2 CONTRACTS, BUYLINES, AND COVERSHEETS!!!!                  
*        MAJOR NOTE:  THERE IS A HOLE IN THIS LOGIC!!  IF A STATION             
*        IS PRESENT ON MORE THAN ONE REP, AND THERE ARE CONTRACTS ON            
*        BOTH REPS, ONLY THE GROUP/SUBGROUP OF REP2 ARE BEING CHANGED           
*        TO THE G/SG OF THE STATION RECORD!!  ORDERS FROM                       
*        REP1 ARE REMAINING AS IS, AND, IF THE G/SG IS TAKEN FROM THE           
*        REP2 STATION RECORD, THEY WILL NOT CONTAIN THE CORRECT GROUP           
*        AND SUBGROUP.  THIS CODING MUST BE CORRECTED IN FUTURE!!               
*        BILL/INTEREP MCGAVERN+CABALLERO MERGE.  12/00                          
*                                                                               
*   DEC/03 - THIS VERSION FORCES ALL GROUP/SUBGROUPS TO THE BASIC RM            
*        USED IN MCGAVREN.  AS A RESULT, NO REWRITING MUST BE DONE,             
*        AND THE MCGAVREN CONTRACTS AND BUYS WILL NOT BE PROCESSED.             
*        THIS IS POSSIBLE BECAUSE THERE ARE NO CHANGES BEING MADE               
*        TO THESE RECORDS, SUCH AS S/P SWITCHES.                                
*                                                                               
****>>>  MVC   REPCODE,REP1        SET CODE FOR REP1                            
         MVC   REPCODE,REP2        SET CODE FOR REP2                            
*                                  ONLY PROCESS REP2  !!!!                      
CPRO0020 EQU   *                                                                
         XC    KEY,KEY                                                          
...
***>>>   MVC   RCONKGRP,=C'RM'     FORCE 'RM' INTO ALL CONTRACTS                
*                                                                               
*   TABLE ENTRIES WILL NOT BE USED TO SET GROUP/SUBGROUP IN CONTRACTS.          
*        ALL ORDERS WILL GO TO GROUP/SUBGRP 'RM'.                               
*                                                                               
*   CLEAR OUT SENDING INFO ID                                                   
*                                                                               
         MVI   ELCODE,X'20'        FIND SENDING INFO ID                         
         LA    R6,RCONREC                                                       
         BAS   RE,GETEL                                                         
         BNE   CPRO0085            ELEMENT NOT FOUND                            
...
*        WHEN EMPTY.                                                            
*                                                                               
***      CLC   REPCODE,REP1        REP1 BEING PROCESSED?                        
***      BNE   CPRO0090            NO  -                                        
***      GOTO1 PUTRECRD            YES - REWRITE RECORD WITH                    
CPRO0090 EQU   *                                                                
...
*                                                                               
*   TEST                                                                        
         CLC   =C'KCFXF',RCONKSTA  SPECIAL STATION?                             
         BNE   CPRO0120            NO                                           
         MVC   P+1(19),=C'CONTRACT: G-SGRP:  '                                  
         MVC   P+20(27),RCONREC                                                 
         GOTO1 REPORT                                                           
         GOTO1 REPORT                                                           
...
*                                  CONVERT CON# TO EBCDIC                       
         ...
         A     R0,=F'1000000'      ADD 1,000,000 TO CONTRACT NUMBER             
...
GRPD0020 EQU   *                                                                
         ...
         BZ    GRPD0050            NO  - NOT IN TABLE: FORCE                    
...
*                                                                               
         CLC   RCONKGRP,=C'RC'     ANY EX-CABALLERO STATIONS?                   
         BNE   GRPD0030                                                         
         MVC   P+1(10),=C'CABALLERO:'                                           
         GOTO1 REPORT                                                           
*                                                                               
GRPD0030 EQU   *                                                                
         CLC   RCONKGRP,=C'RL'     ANY DATA GOING OUT AS 'RL'?                  
         BNE   GRPD0035                                                         
         MVC   P+1(14),=C'ERROR: RL DATA'                                       
         MVC   P+20(27),RCONREC                                                 
         GOTO1 REPORT                                                           
*                                                                               
GRPD0035 EQU   *                                                                
...
GRPD0050 EQU   *                                                                
         MVC   RCONKGRP,=C'RM'     FORCE 'RM' WHEN NOT IN TABLE                 
*                                                                               
GRPD0060 EQU   *                                                                
         XIT1                                                                   
*                                                                               
...
OTHERCTR DS    F                                                                
STA1CTR  DS    F                                                                
STA1MKD  DS    F                   REP 1 STATIONS MARKED FOR DELETE             
STA2CTR  DS    F                                                                
STA3CTR  DS    F                                                                
STAMCTR  DS    F                                                                
SPCRTCTR DS    F                                                                
...
NEWREP   DC    CL2'MG'             NEW REP CODE (MCGAVERN)                      
REP1     DC    CL2'MG'             1ST REP      (MCGAVERN)                      
REP2     DC    CL2'AQ'             2ND REP      (ALLIED)                        
REP3     DC    2X'FF'              3RD REP -    (NO 3D REP)                     
...
REC2     DS    CL500               AREA FOR RECORD2                             
         SPACE 2                                                                
         DS    F                   LENGTH OF RECORD                             
REC      DS    CL4200              AREA FOR RECORD                              
         SPACE 2                                                                
...
*                                     8 CHARS * 2000 SLOTS                      
         ...
         A     RF,=F'7200'         LEAVE ROOM FOR 800 ENTRIES                   
*                                     9 CHARS * 800 SLOTS + SPARE               
         ST    RF,ASTNAREA         A(STATION TABLE)                             
         MVI   FORCEHED,C'Y'       FORCE FIRST PAGE BREAK                       
         CLI   QOPTION1,C'T'       TEST RUN?                                    
         BNE   INIT0100            NO  - EXIT                                   
         MVC   NEWREP,=C'VZ'       SET NPRANYT (VZ)                             
         MVC   REP1,NEWREP                                                      
         MVC   REP2,=C'QB'         SET NPRANYT2(QB)                             
*                                                                               
         XC    KEY,KEY                                                          
         MVI   KEY,1                                                            
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,KEY,KEY,0                             
         CLC   KEY+25(2),=C'QB'    FIRST REP = QB?                              
         BE    *+6                 YES - THIS IS REPFIL9                        
         DC    H'0'                NO  - STOP JOB RIGHT HERE!                   
INIT0100 EQU   *                                                                
         MVC   P+1(21),=C'REPS BEING PROCESSED:'                                
         GOTO1 REPORT                                                           
         MVC   P+1(13),=C'NEW REP CODE:'                                        
         MVC   P+15(2),NEWREP                                                   
         GOTO1 REPORT                                                           
         MVC   P+1(13),=C'REP CODE1 IN:'                                        
         MVC   P+15(2),REP1                                                     
         GOTO1 REPORT                                                           
         MVC   P+1(13),=C'REP CODE2 IN:'                                        
         MVC   P+15(2),REP2                                                     
         GOTO1 REPORT                                                           
         GOTO1 REPORT                                                           
         XIT1                                                                   
...
BPRO0100 EQU   *                                                                
         ...
         GOTO1 REPORT                                                           
         B     BPRO0040            GO BACK FOR NEXT CONTRACT                    
BPRO0600 EQU   *                                                                
         CLC   REPCODE,REP1        REP1 PROCESSED?                              
         BNE   BPRO0604            YES -                                        
         MVC   REPCODE,REP2        NO  - PROCESS IT                             
...
*                                  CONVERT CON# (9'S COMP) TO EBCDIC            
         ...
         PACK  DUB,CNUMAREA        PACK NUMBER FOR ADDITION                     
         CVB   R0,DUB              CONVERT IT TO BINARY                         
         S     R0,=F'1000000'      SUBTRACT 1,000,000 FROM CON#                 
         EDIT  (R0),(8,CNUMAREA),FILL=0                                         
*                                  PUT THE NUMBER BACK OUT                      
...
*                                                                               
******************************************************************              
*  COVPROC:   RETRIEVE ALL COVERSHEETS.                          *              
*                                                                *              
*  NOTE:  THERE IS A CONTRACT # REFERENCE IN THE COVERSHEET      *              
*         ELEMENT IN THE CONTRACT HEADER WHICH IS NOT BEING      *              
*         SET TO THE NEWLY ADJUSTED CONTRACT NUMBER!             *              
*         THIS MUST BE CORRECTED.                                *              
*                                                                *              
*                                                                *              
******************************************************************              
*                                                                               
COVPROC  NTR1  BASE=*,LABEL=*                                                   
         LA    R3,REC                                                           
         USING RCOVREC,R3                                                       
         XC    KEY,KEY                                                          
         MVI   KEY,X'49'           SET UP REP RECD KEY                          
         MVC   KEY+16(2),REP2      INSERT REP2 CODE                             
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,KEY,KEY,0                             
         B     COVP0040                                                         
COVP0020 EQU   *                                                                
         GOTO1 DATAMGR,DMCB,DMRSEQ,REPDIR,KEY,KEY,0                             
COVP0040 EQU   *                                                                
         CLC   KEY(18),KEYSAVE     STILL COVERSHEET+REP?                        
         BNE   COVP0800            FINISHED                                     
*        GOTO1 GETRECR2            YES - RETRIEVE RECORD                        
         LA    R6,GETREC                                                        
         GOTO1 DATAMGR,DMCB,(DMINBTS,(R6)),REPFILE,KEY+28,             X        
               REC,(0,DMWORK)                                                   
         LA    R3,REC              POINT TO RECORD                              
         MVC   REC-4(2),RCOVLEN    INSERT LENGTH FOR PUT                        
         MVC   RCOVKREP,NEWREP     INSERT NEW REP CODE                          
         CLI   RCOVKNAM,X'FF'      CONTRACT NUMBER OR NAME?                     
         BNE   COVP0060            NAME                                         
         GOTO1 NUMDELTA,DMCB,RCOVKNAM+4  CHANGE CONTRACT NUMBER                 
COVP0060 EQU   *                                                                
**       BAS   RE,PUTREC2                                                       
         LH    RF,REC-4            ADD LENGTH OF 'LENGTH WORD'                  
*                                     TO RECORD CONTROL                         
...
*                                                                               
         ...
         ST    RF,NUMCOVS                                                       
*                                                                               
         CLC   NUMCOVS,=F'50'      DISPLAY FIRST 50 COV RECDS ONLY              
         BH    COVP0020            GO BACK FOR NEXT COV RECORD                  
         MVC   P+1(16),=C'     COV RECORD:'                                     
         MVC   P+20(27),RCOVREC                                                 
         GOTO1 REPORT                                                           
...
*                                                                               
******************************************************************              
*  REPRECRD   PUTS REP RECORD FOR REP1 OUT TO TAPE                              
*                                                                               
******************************************************************              
*                                                                               
REPRECRD NMOD1 0,*RRCD*                                                         
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         LA    R3,KEY                                                           
         USING RREPREC,R3                                                       
         XC    KEY,KEY                                                          
         MVI   RREPKTYP,X'01'      SET UP REP RECD KEY                          
         MVC   RREPKREP,REP1       INSERT REP CODE                              
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,KEY,KEY,0                             
         CLC   KEY(27),KEYSAVE                                                  
         BE    *+6                                                              
...
***DARE PROCESSING                                                              
******************************************************************              
*  DAREPROC   PUTS DARE RECORDS FOR REP2 OUT TO TAPE                            
*                                                                               
******************************************************************              
*                                                                               
DAREPROC NMOD1 0,*DARE*                                                         
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         LA    R3,KEY                                                           
         USING RDARREC,R3                                                       
*                                                                               
*   'USING' SET TO COVER KEY HERE                                               
*                                                                               
         MVI   KEYTYPE,X'41'       SET X'41' DARE RECORDS FIRST                 
DPRO0020 EQU   *                                                                
         XC    KEY,KEY                                                          
         MVC   RDARKTYP,KEYTYPE    SET UP DARE RECORD KEY                       
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,DMRDHI,REPDIR,KEY,KEY,0                             
*                                  READ HIGH                                    
         B     DPRO0060                                                         
...
DPRO0060 EQU   *                                                                
         CLC   KEY(1),KEYTYPE                                                   
         BNE   DPRO0200            RECORD TYPE FINISHED                         
         CLC   RDARKREP,REP2       DARE REC FOR REP2?                           
         BNE   DPRO0040            READ THROUGH ALL X'41' BECAUSE               
*                                     THERE ARE X'4101' RECORDS ON FILE         
*                                  YES - RETRIEVE RECORD                        
         CLI   KEYTYPE,X'41'       '41' RECORDS?                                
         BNE   DPRO0080            NO  -                                        
         L     RF,DAR41CTR                                                      
         LA    RF,1(RF)                                                         
         ST    RF,DAR41CTR                                                      
...
DPRO0100 EQU   *                                                                
         ...
         LA    R3,REC              POINT TO RECORD                              
*                                                                               
*   'USING' NOW SET TO COVER RECORD HERE                                        
*                                                                               
         MVC   REC-4(2),RDARLEN    INSERT LENGTH FOR PUT                        
         MVC   RDARKREP,NEWREP     INSERT NEW REP CODE                          
         CLI   RDARKRT,X'10'       AGENCY HEADER RECORD?                        
         BNE   DPRO0120            NO  -                                        
         OC    RDARREP#,RDARREP#   LINKED? (CONTRACT # PRESENT?)                
         BZ    DPRO0110            NO  - DON'T CHANGE CONTRACT NUMBER           
         GOTO1 NUMDELTA,DMCB,RDARREP#    CHANGE CONTRACT NUMBER                 
DPRO0110 EQU   *                                                                
*                                                                               
*   FIELD RDARPPRP IN X'0A' ELEMENT COULD BE UPDATED HERE.  THIS                
*        FIELD IS ONLY REFERENCED IN THE TAKEOVER, AND IS OTHER                 
*        WISE UNUSED, SO IT IS NOT BEING CHANGED HERE.                          
*                                                                               
*&&DO                                                                           
         LA    R6,RDARREC                                                       
         ...
         USING RDARPPEL,R6                                                      
*                                                                               
         MVC   RDARPPRP,RDARKREP   INSERT REP OF ORDER                          
*                                                                               
         DROP  R6                                                               
*&&                                                                             
*                                                                               
*   AT THIS POINT, THE ID OF REC'VR MUST BE ADJUSTED.  THIS IS A                
*???AT THIS POINT, THE ID OF SENDER MUST BE ADJUSTED.  THIS IS A                
*        HARD-CODED CHECK.                                                      
*                                                                               
         CLC   RDARRCVR(3),=C'ALL' ALLIED DARE ORDER?                           
         BE    DPRO0112            YES                                          
         MVC   P+1(16),=C'NOT ALLIED DARE:'                                     
         MVC   P+17(10),RDARSNDR                                                
         GOTO1 REPORT                                                           
         L     RC,0(R1)            RESET A(WORKSPACE)                           
         LA    R4,REC-4            A(RECORD LENGTH FIELD)                       
         SR    RF,RF                                                            
         ...
         B     DPRO0120                                                         
DPRO0112 EQU   *                                                                
         MVC   RDARRCVR(2),=C'MG'  INSERT MCGAVREN POWER CODE                   
         MVC   RDARRCVR+2(2),RDARRCVR+3                                         
*                                  MOVE OFFICE CODE                             
         MVC   RDARRCVR+4(6),SPACES                                             
...
*                                     TO RECORD CONTROL                         
         ...
         PUT   FILOUTB,(R0)        PUT RECORD TO BACKUP OUTPUT                  
*                                                                               
         CLI   QUESTOR+8,C'D'      DISPLAY DARE RECORDS?                        
         BNE   DPRO0180            NO                                           
         CLI   KEYTYPE,X'41'       '41' RECORDS IN PROCESS?                     
         BNE   DPRO0140            NO                                           
         CLC   DAR41CTR,=F'100'    DISPLAY FIRST 'N' RECS                       
         BH    DPRO0180                                                         
         B     DPRO0160                                                         
DPRO0140 EQU   *                                                                
         CLC   DAR51CTR,=F'100'    DISPLAY FIRST 'N' RECS                       
         BH    DPRO0180                                                         
DPRO0160 EQU   *                                                                
         MVC   P+1(16),=C'    DARE RECORD:'                                     
         MVC   P+18(27),RDARREC                                                 
         GOTO1 REPORT                                                           
...
DPRO0180 EQU   *                                                                
*                                                                               
*   ALL DARE RECORDS FOR REP2 WILL BE DELETED.  THESE WOULD OTHERWISE           
*        GENERATE DUPLICATE IN-BOX KEYS FOR THE MASTER REP.                     
*                                                                               
         OI    KEY+27,X'80'        MARK RECORD KEY DELETED                      
         CLI   QOPTION2,C'U'       UPDATE RUN?                                  
         BNE   DPRO0190            NO  - SKIP REWRITE                           
         GOTO1 WRT3                REWRITE KEY AS DELETED                       
DPRO0190 EQU   *                                                                
         L     RF,DARMKD           INCREASE DROPPED DARE    COUNT               
         LA    RF,1(RF)                                                         
         ST    RF,DARMKD           REPLACE                                      
         B     DPRO0040            GO BACK FOR NEXT DARE REC                    
DPRO0200 EQU   *                                                                
         CLI   KEYTYPE,X'51'       ALL 51'S PROCESSED?                          
         BE    DPRO0800            YES - FINISHED                               
         MVI   KEYTYPE,X'51'       NO  - DO THEM                                
         B     DPRO0020            GO BACK FOR FIRST 51                         
*                                                                               
DPRO0800 EQU   *                                                                
         XIT1                                                                   
*                                                                               
...
DIRCTRY3 NTR1                                                                   
         IC    R4,DMINBTS                                                       
         MVC   KEYSAVE,KEY                                                      
         GOTO1 DATAMGR,DMCB,((R4),COMMAND3),=C'REPDIR',KEYSAVE,KEY              
         OC    DMCB+8(1),DMCB+8                                                 
         XIT1                                                                   
COMMAND3 DS    CL8                                                              
         SPACE 3                                                                
***DARE PROCESSING                                                              
******************************************************************              
*  DISPTOTS:  SCAN REP1 AND REP2 STATION RECORDS. CONSTRUCT TABLE *             
*     BASED ON DSECT STADSECT.  THEN RESCAN TABLE, SELECTING     *              
*     APPROPRIATE RECORDS FOR OUTPUT.                            *              
******************************************************************              
*                                                                               
DISPTOTS NMOD1 0,*TOTS*                                                         
         ...
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'REP1  STATIONS RELEASED:'                             
         EDIT  STA1CTR,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'REP1  STATIONS MARKED  :'                             
         EDIT  STA1MKD,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'REP2  STATIONS RELEASED:'                             
         EDIT  STA2CTR,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'REP3  STATIONS RELEASED:'                             
         EDIT  STA3CTR,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'MERGE STATIONS RELEASED:'                             
         EDIT  STAMCTR,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'COVERSHEETS   PROCESSED:'                             
         EDIT  NUMCOVS,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'BUYS          PROCESSED:'                             
         EDIT  BUYCTR,(12,P+30),COMMAS=YES                                      
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'DARE 41S      PROCESSED:'                             
         EDIT  DAR41CTR,(12,P+30),COMMAS=YES                                    
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'DARE 51S      PROCESSED:'                             
         EDIT  DAR51CTR,(12,P+30),COMMAS=YES                                    
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'DARE41/51 RECS MARKED  :'                             
         EDIT  DARMKD,(12,P+30),COMMAS=YES                                      
         GOTO1 REPORT                                                           
         ...
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'BUDGET RECS   MARKED   :'                             
         EDIT  BUDMKD,(12,P+30),COMMAS=YES                                      
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'COMMISSION RECS READ   :'                             
         EDIT  COMMCTR,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
         MVC   P+1(24),=C'COMMISSION RECS MARKED :'                             
         EDIT  COMMMKD,(12,P+30),COMMAS=YES                                     
         GOTO1 REPORT                                                           
...
STADS1CM DS    CL20                S1 COMBO STATIONS                            
STADS2CM DS    CL20                S2 COMBO STATIONS                            
STADS3CM DS    CL20                S3 COMBO STATIONS                            
*                                                                               
...
RECORD   DS    CL1008                                                           
       ...
       ++INCLUDE REREPMODES                                                     
         ...
         SPACE 4                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'110REREPSW05 02/03/04'                                      
         END                                                                    
