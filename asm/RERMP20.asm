*          DATA SET RERMP20    AT LEVEL 137 AS OF 12/21/10                      
*PHASE T81020C                                                                  
*INCLUDE UPOUT                                                                  
*INCLUDE PRINT                                                                  
*INCLUDE PRNTBL                                                                 
...
*                     A HARD-CODED TABLE                              *         
*  OCT31/03 (BU ) --- HFS PROCESSING: RETRIEVE STATION RECORD, CHECK  *         
*                     FLAG                                            *         
*                                                                     *         
*  NOV24/03 (BU ) --- SET VENDOR PER STATION RECORD                   *         
*                     ALL OUTPUT IS UNIX FORMAT                       *         
*                                                                     *         
*  APR29/04 (BU ) --- SET STATION MEDIA TO SPACE TO ACCESS REP STA    *         
*                     RECORD PROPERLY                                 *         
*                                                                     *         
*  OCT21/04 (BU ) --- BYPASS INVALID TEXT RECORDS                     *         
*                                                                     *         
*  JAN07/05 (BU ) --- ALL REQUESTS VIA HFS                            *         
*                                                                     *         
*  MAR12/07 (BU ) --- MODIFY START / END DATE TESTING, PER L. HENDY   *         
*                                                                     *         
*  MAR28/07 (BU ) --- FIX RANGE TESTING:  HIGH END TEST WAS WRONG     *         
*                                                                     *         
*   APR/09  (SMY) --- NEW INVENTORY KEY                               *         
*                                                                     *         
*  JUN01/09 (KUI) --- FIX DATE OUTPUT TO NOT USE DDS INTERNAL         *         
*                                                                     *         
*  JAN21/10 (KUI) --- FIX DATE OUTPUT FOR H1                          *         
***********************************************************************         
...
EDICT05  DS    0H                                                               
         ...
         LA    R6,KEY                                                           
         USING CTIREC,R6                                                        
         MVC   P+15(8),VENDORID    SET VENDOR ID                                
         TM    HFSFLAG,X'80'       OUTPUT IS UNIX FILE?                         
         BNO   EDICT10             NO                                           
...
SETHFS   NTR1  BASE=*,LABEL=*                                                   
         ...
         L     R5,AIO              SAVE A(IO AREA IN USE)                       
         L     R2,AIO3             SET A(IO AREA3 AS INPUT)                     
         ST    R2,AIO                                                           
         LA    R2,KEY              USING COVERS KEY                             
         USING RSTAREC,R2                                                       
*                                                                               
         XC    KEY,KEY                                                          
         MVI   KEY,2               SET STATION RECORD TYPE                      
         MVC   RSTAKREP,AGENCY     PLACE CURRENT ALPHA CODE                     
         MVC   RSTAKSTA,CSTAT      PLACE REQUEST STATION                        
*                                                                               
*   ALL ETRANS IS DONE FOR TV.  THE MEDIA MUST BE SET TO SPACE TO               
*        ACCESS THE CORRESPONDING REP STATION RECORD                            
*                                                                               
**       CLI   RSTAKSTA+4,C'T'     MEDIA = T?                                   
**       BNE   SETH0060            NO                                           
         MVI   RSTAKSTA+4,C' '     YES - REPLACE WITH SPACE                     
SETH0060 EQU   *                                                                
         GOTO1 HIGH                                                             
         ...
         L     R2,AIO              SET A(IO AREA)                               
*                                     USING NOW COVERS RECORD                   
         LA    R1,RSTAELEM                                                      
SETH0080 EQU   *                                                                
         CLI   0(R1),0             END OF RECORD?                               
         BE    SETH0120            YES                                          
         CLI   0(R1),X'08'         08 ELEMENT?                                  
         BE    SETH0100            YES - TEST BIT                               
         ZIC   RF,1(R1)            NO  - BUMP TO NEXT ELEMENT                   
         AR    R1,RF                                                            
         B     SETH0080            GO BACK FOR NEXT ELEMENT                     
SETH0100 EQU   *                                                                
         USING RSTAXXEL,R1                                                      
         TM    RSTAOPTB,X'01'      HFS UNIX FILE?                               
         BNO   SETH0120            NO                                           
         ...
         XC    ALET,ALET                                                        
SETH0120 EQU   *                                                                
****>>>  MVC   VENDORID,=C'*TVSCAN '                                            
         MVC   VENDORID,=C'*TVSCANB'  - TVSCAN VIA BDE                          
*                                  SET DEFAULT VENDOR ID                        
         LA    R1,RSTAELEM                                                      
SETH0140 EQU   *                                                                
         CLI   0(R1),0             END OF RECORD?                               
         BE    SETH0180            YES                                          
         CLI   0(R1),X'2C'         2C ETRANS VENDOR ELEMENT?                    
         BE    SETH0160            YES - TEST BIT                               
         ZIC   RF,1(R1)            NO  - BUMP TO NEXT ELEMENT                   
         AR    R1,RF                                                            
         B     SETH0140            GO BACK FOR NEXT ELEMENT                     
SETH0160 EQU   *                                                                
         USING RSTAETEL,R1                                                      
         MVC   VENDORID(8),RSTAETVN                                             
*                                  INSERT VENDORID FROM STA REC                 
SETH0180 EQU   *                                                                
         CLC   VENDORID,=C'*TVSCAN '                                            
*                                  TVSCAN O/P?  (BINARY FILE)                   
*                                     (NOT TVSCANB - IMPORTANT!)                
*                                                                               
*   EVERY REQUEST WILL GENERATE AN HFS FILE:  FOLLOWING                         
*        BRANCH IS COMMENTED OUT.                                               
*                                                                               
****>>   BE    SETH0200            YES                                          
         OI    HFSFLAG,X'80'       NO  - ALL OTHER VENDORS WILL                 
*                                     RECEIVE A UNIX FILE                       
         LA    RF,200              SET FIELD VALUES                             
         ST    RF,SIZE                                                          
         XC    ALET,ALET                                                        
...
VREND1   GOTO1 DATCON,DMCB,(0,WORK),(3,ENDOPT) SAVE END DATE                    
         GOTO1 DATCON,DMCB,(0,WORK),(2,ENDOPTC) SAVE END DATE (COMP)            
...
RPM60    CLI   INVKEYD.RINVKRTP,0                                               
         BNE   RPM100                                                           
         BAS   RE,BLDINVH          INV HEADER RECORD                            
         B     RPM40                                                            
*                                                                               
RPM100   CLI   INVKEYD.RINVKRTP,X'FF'                                           
         BNE   RPM200                                                           
*                                                                               
         BRAS   RE,VALCOMM         CHECK FOR VALID RATIONALE.                   
         BNZ   RPM40               ERROR - SKIP IT                              
*                                                                               
         BRAS  RE,BLDCOMM          COMMENT RECORD                               
         B     RPM40                                                            
*                                                                               
RPM200   CLI   INVKEYD.RINVKRTP,C'M'                                            
         BE    RPM40                                                            
...
GTINV    NTR1  BASE=*,LABEL=*                                                   
         ...
         MVI   RINVKTYP,RINVKTYQ   INVENTORY TYPE                               
...
*                                  CHECK UP TO STATION                          
         ...
         BE    GTINV440            CHECK 1ST SHOT FOR HEADER                    
****>>>  BE    GTINV300            ORIGINAL CODE WAS WRONG                      
...
*SMY*    CLI   RINVKSRC,0          IS RECORD A HEADER                           
*SMY*    BE    GTINV300             YES, DO HEADER LOGIC                        
         OC    RINVKRTP,RINVKRTP   IS RECORD A HEADER                           
         BZ    GTINV300             YES, DO HEADER LOGIC                        
...
GTINV300 BAS   RE,CHKFILT          CHK EFF START DATE FILTERS                   
         BNE   GTINV400            CC NE = EXCLUDE                              
*                                                                               
*   'CHKFILT' ONLY TESTS FOR KEY'S EFFECTIVE START DATE.  THE                   
*        EFFECTIVE END DATE IS IN THE RECORD. ANOTHER CHECK MUST                
*        BE DONE AFTER THE RECORD IS RETRIEVED.                                 
                                                                                
         GOTO1 GETREC              INCLUDE (SO FAR)  GET RECORD                 
*                                                                               
         BAS   RE,CHKFILT2         CHK EFF END DATE FILTERS                     
         BNE   GTINV400            CC NE = EXCLUDE                              
*                                                                               
         B     GTINVX              EXIT CC EQ = INCLUDE                         
...
INVKEYD  USING RINVKEY,KEY                                                      
*                                  CLEAR REST OF KEY                            
         XC    INVKEYD.RINVKRTP(RINVLEN-RINVKRTP),INVKEYD.RINVKRTP              
                                                                                
         GOTO1 HIGH                FIND NEXT PROSPECT                           
                                                                                
*                                  MATCH UP TO STATION?                         
GTINV440 CLC   KEY(RINVKINV-RINVKEY),KEYSAVE                                    
         BE    GTINV450            YES, CONTINUE                                
         MVI   KEY,X'FF'           NO, END OF LOOKUP                            
         B     GTINVX                                                           
                                                                                
GTINV450 DS    0H                                                               
*SMY*    CLI   RINVKSRC,X'00'      IS HEADER? (SHOULD BE)                       
*SMY*    BE    GTINV300            YES, DO HEADER LOGIC                         
*SMY*    B     GTINV425            NO,  TRY AGAIN                               
         OC    RINVKRTP,RINVKRTP   IS HEADER? (SHOULD BE)                       
         BZ    GTINV300            YES, DO HEADER LOGIC                         
...
**********************************************************************          
*                                                                               
*        CHKFILT ROUTINE -- CHECK CURRENT INV HEADER EFFECTIVE START            
*                           DATE AGAINST FILTERS                                
*                           (IF PRESENT) AND RETURN CC SET                      
*        LOGIC NOW SET TO ONLY CHECK EFFECTIVE START DATE AGAINST               
*          REQUEST END DATE.  IF START IS LATER, INVENTORY IS                   
*          BYPASSED.                                                            
*                                                                               
**********************************************************************          
...
CHKFLT40 CLI     0(R3),X'FF'             END OF TABLE?                          
         ...
         BNH     FILTINC                 IF NOT HI IT'S WITHIN RANGE            
...
**********************************************************************          
*                                                                               
*        CHKFILT2 ROUTINE -- CHECK CURRENT INV HEADER END DATE, IN              
*                           BODY OF RECORD, AGAINST FILTERS                     
*                           (IF PRESENT) AND RETURN CC SET                      
*                                                                               
**********************************************************************          
                                                                                
CHKFILT2 NTR1  BASE=*,LABEL=*                                                   
         L     R5,AIO              RECORD IS IN IO AREA                         
         USING REINVREC,R5                                                      
*                                                                               
         CLC   ENDOPT,=X'FFFFFF'   REQUEST END DATE ENTERED?                    
         BE    FILTINC2            NO  - ACCEPT ANY DATE                        
*                                                                               
         OC    RINVPEFF+2(2),RINVPEFF+2                                         
*                                  ANY EFFECTIVE END DATE?                      
         BZ    FILTINC2            NO  - ACCEPT RECORD                          
*                                                                               
         CLC   RINVPEFF+2(2),ENDOPTC                                            
*                                  YES - CHECK END VS REQUEST                   
         BNH   FILTEXC2            BNH: =< REQUEST END:  EXCLUDE                
         B     FILTINC2            BH: EFF END AFTER REQ END                    
*                                                                               
FILTEXC2 LA      R1,1                    SET CC != (FOR EXCLUDE)                
         B       *+6                                                            
FILTINC2 SR      R1,R1                   SET CC  = (FOR INCLUDE)                
         LTR     R1,R1                                                          
         XIT1                                                                   
         DROP    R5                                                             
         LTORG                                                                  
...
****     MVC   HDVER,=CL2'02'     VERSION 02 AS OF 11/03/00                     
         ...
         MVC   HDREP,AGENCY                                                     
         GOTO1 DATCON,DMCB,(5,DUB),(X'20',HDSVDAT)                              
         TIME  DEC                                                              
         ST    R0,FULL                                                          
         GOTO1 HEXOUT,DMCB,FULL,DUB,4,=C'TOG'                                   
         MVC   HDSVTIM(2),DUB                                                   
...
*             GET EFFECTIVE START DATE FROM KEY                                 
         ...
         GOTO1 DATCON,DMCB,(3,RINVKSTD),(X'20',EXCEFFS)                         
         DROP  R3                                                               
*                                                                               
...
*                                    SAME ITEM/EFF DATE?                        
         ...
         CLI   RINVKRTP,C'M'         IGNORE MARKET FACT                         
         BE    BLDEX020                                                         
         CLI   RINVKRTP,C'S'         IGNORE STATION FACT                        
         BE    BLDEX020                                                         
         CLI   RINVKRTP,X'FF'        IGNORE "RATIONAL" RECORDS                  
         BE    BLDEX020                                                         
...
BLDEX060 MVC   EXCSRVY,RINVKBK        BOOK YY/MM                                
         ...
         MVC   EXCBKTYP,RINVKBTP      BOOK TYPE                                 
...
*                                                                               
BLDSR060 MVC   SRVSRC,RINVKRSR     SERVICE                                      
         MVC   SRVLOOK,WORK+11     LOOKUP TYPE                                  
         MVC   SRVBTYP,RINVKBTP    BOOK TYPE                                    
         CLI   SRVBTYP,0                                                        
         BNE   *+8                                                              
         MVI   SRVBTYP,X'40'                                                    
*                                                                               
         MVC   SRVPROG,SAVPROG     GET DEFAULT PROGRAM NAME                     
*SMY*    CLI   RINVKSRC,C'O'       IF PROJECTED                                 
         TM    RINVKQLF,X'04'      IF PROJECTED                                 
         BNZ   BLDSR100                                                         
*                                                                               
         MVC   SRVPROG,SPACES                                                   
...
*                                                                               
         ...
         MVC   OVRINV,RINVKINV                                                  
         PRINT GEN                                                              
         GOTO1 DATCON,DMCB,(3,RINVKSTD),(X'20',OVRSEFF)                         
         PRINT NOGEN                                                            
*                                                                               
...
**********************************************************************          
*                                                                               
*        VALCOMM ROUTINE --LOOK FOR INVALID RATIONALE RECORDS TO SKIP           
*                                                                               
**********************************************************************          
                                                                                
VALCOMM  NTR1  BASE=*,LABEL=*                                                   
         L     R5,AIO                                                           
         USING REINVREC,R5                                                      
...
*                                                                               
         ...
         MVI   CMSTA+4,X'40'                                                    
         MVC   CMINVN,RINVKINV                                                  
         GOTO1 DATCON,DMCB,(3,RINVKSTD),(X'20',CMEFFS)                          
...
ENDOPTC  DS    CL2                 END DATE COMPRESSED                          
...
KEYSKEL  DS    XL(RINVKRTP-RINVKEY) TEMP KEY SKELETON HOLDER                    
...
*                                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'137RERMP20   12/21/10'                                      
         END                                                                    
