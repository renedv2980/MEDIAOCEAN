*          DATA SET REREPXA02  AT LEVEL 186 AS OF 02/27/03                      
*PHASE REXA02A,*                                                                
*INCLUDE HEXIN                                                                  
*INCLUDE HEXOUT                                                                 
*INCLUDE BINSRCH2                                                               
...
*INCLUDE PERVERT                                                                
*INCLUDE XSORT                                                                  
         TITLE 'REREPXA02  (REXA02A) --- REP BUY DATA: RTS AUDIT'               
*                                                                               
********************************************************************            
*                                                                  *            
*                                                                  *            
*        REREPXA02  -- EXTRACT REP RTS BUY DATA FOR COMPARISON     *            
*                      WITH SPOT BUY DATA                          *            
*                                                                  *            
* ---------------------------------------------------------------- *            
* UPDATE HISTORY:                                                  *            
...
MAIN0180 EQU   *                                                                
         CLC   KEY+10(2),STRTDATE  END DATE < STARTDATE?                        
...
*   TEST BUY RECORD                                                             
*                                                                               
         XC    EFFDATE,EFFDATE     CLEAR LAST EFFECTIVE DATE                    
         MVI   EFF#WK,0            CLEAR LAST EFF # SPOTS/WK                    
         LA    R2,RBUYELEM         FIND X'08' ELT                               
MAIN0260 EQU   *                                                                
         CLI   0(R2),0             END OF RECORD REACHED?                       
         BE    MAIN0220            YES - NO X'08' - READ NEXT                   
         CLI   0(R2),3             EFFECTIVE DATE ELEMENT FOUND?                
         BNE   MAIN0270            NO  -                                        
         MVC   EFFDATE,RBUYDTED-RBUYDTEL(R2)                                    
         CLI   RBUYDTNW-RBUYDTEL(R2),0                                          
*                                  ANY EFFECTIVE DATE SPOTS?                    
         BE    MAIN0270            NO  - DON'T SET                              
         MVI   EFF#WK,1            YES - SET FLAG 'ON'                          
MAIN0270 EQU   *                                                                
...
MAIN0280 EQU   *                                                                
         CLC   EFFDATE,STRTDAT3    BUY'S LAST EFF DATE PRIOR                    
*                                     TO AUDIT START DATE?                      
         BL    MAIN0220            YES - READ NEXT RECORD                       
         XC    SORTREC,SORTREC     CLEAR SORT RECORD                            
         CLI   RBUYCHGI,C'C'       BUY CANCELLED?                               
         BNE   MAIN0282                                                         
         MVI   SCANCEL,C'C'        YES - SET FLAG                               
MAIN0282 EQU   *                                                                
         CLI   RBUYCHGI+1,C'C'     BUY CANCELLED?                               
         BNE   MAIN0284                                                         
         MVI   SCANCEL,C'C'        YES - SET FLAG                               
MAIN0284 EQU   *                                                                
         MVC   SSPOTSWK,RBUYNW     SET NUMBER SPOTS/WEEK                        
         CLI   EFF#WK,1            EFFECTIVE DATES CONTAIN SPOTS?               
         BE    MAIN0290            YES                                          
         MVI   SSPOTSWK,0          NO  - SET TO ZERO                            
MAIN0290 EQU   *                                                                
         USING RBUYSPEL,R2                                                      
         ...
         MVC   SFLTSTRT,STARTKEY+8 INSERT FLIGHT START DATE                     
         MVC   SFLTEND,STARTKEY+10 INSERT FLIGHT END DATE                       
*                                                                               
***      CLI   SBUYLN#,0           BUYLINE PREVIOUSLY TRANSFERRED?              
***      BZ    MAIN0220            NO  - DON'T RELEASE IT TO SORT               
*                                                                               
...
*                                                                               
         ...
         BH    INIT0040            NO  - JUST ADJUST MONTH                      
...
INIT0040 EQU   *                                                                
         ...
         GOTO1 DATCON,DMCB,(3,WORK+6),(2,STRTDATE)                              
         MVC   STRTDAT3,WORK+6     SAVE BINARY START DATE                       
         MVC   P+1(20),=C'AUDIT START DATE IS:'                                 
         GOTO1 DATCON,DMCB,(2,STRTDATE),(5,P+25)                                
         GOTO1 DATCON,DMCB,(3,STRTDAT3),(5,P+40)                                
         GOTO1 REPORT                                                           
INIT0120 EQU   *                                                                
         XC    KEY,KEY                                                          
...
*                                                                               
         ...
         CLC   =C'K3',QREP         KATZ?                                        
         BNE   INIT0170            NO                                           
         LA    RF,1(RF)            YES - INCREASE COUNT BY 1                    
*                                                                               
*   RF USED FOR COUNT IN XSORT LATER - DON'T DESTROY                            
*                                                                               
         MVC   0(2,R2),=C'NU'                                                   
         LA    R2,2(R2)            SKIP OVER NEW ENTRY                          
INIT0170 EQU   *                                                                
...
*                                                                               
         LA    R2,REPCODES         SET A(REPCODES TABLE)                        
         GOTO1 XSORT,DMCB,(0,0(R2)),(RF),2,2,0                                  
*                                                                               
**GET SUBREPS                                                                   
INIT0220 DS    0H                                                               
         MVC   P+1(14),=C'REPCODE TABLE:'                                       
         MVC   P+16(40),REPCODES                                                
         GOTO1 REPORT                                                           
*                                                                               
         XIT1                                                                   
*                                                                               
...
RUNID    DS    CL5                                                              
USERID   DS    CL4                                                              
STRTDATE DS    XL2                 START CUTOFF DATE                            
STRTDAT3 DS    XL3                 START CUTOFF DATE - BINARY                   
EFFDATE  DS    XL3                 BUY'S LAST EFFECTIVE DATE                    
EFF#WK   DS    XL3                 EFFECTIVE DATE # SPOTS/WK                    
...
SCANCEL  DS    CL1                 CANCEL FLAG                                  
SSPOTSWK DS    CL1                 SPOTS PER WEEK                               
SFLTSTRT DS    CL2                 FLIGHT START DATE COMPRESSED                 
SFLTEND  DS    CL2                 FLIGHT END   DATE COMPRESSED                 
         DS    CL2                 SPARE                                        
*                                                                               
         DS    0H                                                               
*                                                                               
...
*********************************************************************           
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'186REREPXA02 02/27/03'                                      
         END                                                                    
