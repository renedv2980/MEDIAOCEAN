*          DATA SET ACREC04    AT LEVEL 053 AS OF 07/18/19                      
*PHASE T60804A                                                                  
*INCLUDE CATCALL                                                                
ACREC04  TITLE '- CANADA OVERLAY TO HANDLE HEADER SCREEN AND POSTINGS'          
***********************************************************************         
*  ID  LVL   DATE    TICKET            COMMENTS                       *         
* ---- --- ------- ------------ --------------------------------------*         
* JSAY 052 13MAR18 <SPEC-21208> RELINK FOR NEW TSAR FIELD TSARBTYC    *         
* JSAY 053 19JUL19 <SPEC-36076> RELINK FOR NEW TSAR FIELD TSARPOEL    *         
***********************************************************************         
ACREC04  CSECT                                                                  
         PRINT NOGEN                                                            
         NMOD1 0,**REC4**,RA,R9,RR=RE                                           
         LR    RC,R1                                                            
         USING WORKD,RC                                                         
...
SETLST   DS    0H                                                               
         ...
         MVI   TOTALSX,EOT                                                      
         MVC   BANKLIST,=Y(BANKULS-ACREC04)                                     
         MVC   DISCLIST,=Y(DISCULS-ACREC04)                                     
         MVC   RECVLIST,=Y(RECVULS-ACREC04)                                     
         MVC   GSTLIST,=Y(RECVULS-ACREC04)                                      
         MVC   PSTLIST,=Y(RECVULS-ACREC04)                                      
         MVC   WOFFLIST,=Y(WOFFULS-ACREC04)                                     
...
*                                                                               
         ...
         OI    RECGSTH+(FVATRB-FVIHDR),FVAPROT                                  
*        OI    RECGSTNH+(FVATRB-FVIHDR),FVAPROT                                 
*                                                                               
         OC    PST,PST             PROT PST IF PRESENT                          
         BZ    RESHED3                                                          
         OI    RECPSTH+(FVATRB-FVIHDR),FVAPROT                                  
*        OI    RECPSTNH+(FVATRB-FVIHDR),FVAPROT                                 
*                                                                               
RESHED3  DS    0H                                                               
RESHEDX  B     EXIT                                                             
         EJECT                                                                  
...
RELAST   DS    0H                                                               
         ...
         NI    RECGSTH+(FVATRB-FVIHDR),X'FF'-FVAPROT                            
*        NI    RECGSTNH+(FVATRB-FVIHDR),X'FF'-FVAPROT                           
         NI    RECPSTH+(FVATRB-FVIHDR),X'FF'-FVAPROT                            
*        NI    RECPSTNH+(FVATRB-FVIHDR),X'FF'-FVAPROT                           
RELASTX  B     EXIT                                                             
         EJECT                                                                  
         LTORG                                                                  
         EJECT                                                                  
...
***********************************************************************         
         SPACE 1                                                                
VALGST   TM    RECGSTH+(FVATRB-FVIHDR),FVAPROT                                  
         BO    VALGSTX                                                          
         CLI   RECWACH+(FVILEN-FVIHDR),0                                        
         BZ    *+8                                                              
         MVI   FVMINL,1            REQUIRED FIELD ONLY IF W/O ACC USED          
         GOTO1 AVALGST,RECGSTH                                                  
         BH    VALHEDXX                                                         
         BL    VALGSTX                                                          
         ...
         BNE   VALHEDE             EXIT WITH CC SET NEQ                         
VALGST2  OC    WOFF,WOFF                                                        
         BNZ   VALGST4                                                          
         LA    R1,RECWACH          SET CURSOR TO WRITE-OFF ACCOUNT              
         ST    R1,FVADDR                                                        
         MVC   FVMSGNO,=AL2(EGIFMISS)                                           
         B     VALHEDE             ERROR EXIT SETS CC NEQ                       
VALGST4  GOTO1 VACSRCHC,DMCB,FVADDR,TWAD,(C'*',GSTUL),                 X        
               (X'C0',RECNDSP),AIO1,(L'GSTNAME,GSTNAME)                         
VALGSTX  DS    0H                                                               
         EJECT                                                                  
***********************************************************************         
* VALIDATE PST ACCOUNT                                                *         
***********************************************************************         
VALPST   TM    RECPSTH+(FVATRB-FVIHDR),FVAPROT                                  
         BO    VALPSTX                                                          
         GOTO1 AVALPST,RECPSTH                                                  
         BH    VALHEDXX                                                         
         BL    VALPSTX                                                          
         TM    COMPSTAT,CPYSOROE   TEST COMPANY ON OFFICES                      
         BZ    VALPST2                                                          
         ...
         CLC   RECOFFC,RECVOFF     MUST BE SAME OFFICE                          
         BNE   VALHEDE             EXIT WITH CC SET NEQ                         
VALPST2  OC    WOFF,WOFF                                                        
         BNZ   VALPST4                                                          
         LA    R1,RECWACH          SET CURSOR TO WRITE-OFF ACCOUNT              
         ST    R1,FVADDR                                                        
         MVC   FVMSGNO,=AL2(EGIFMISS)                                           
         B     VALHEDE             ERROR EXIT SETS CC NEQ                       
VALPST4  GOTO1 VACSRCHC,DMCB,FVADDR,TWAD,(C'*',PSTUL),                 X        
               (X'C0',RECNDSP),AIO1,(L'PSTNAME,PSTNAME)                         
VALPSTX  DS    0H                                                               
         EJECT                                                                  
***********************************************************************         
...
*&&                                                                             
*                                                                               
* CANDAIAN  7 DAYS INTO THE FUTURE AND 1 YEAR IN THE PAST.  EG IF TODAY         
* RULE:     IS 8/31/99 THE EARLIEST DATE YOU CAN ENTER IS 8/31/98               
...
*&&UK                                                                           
         TM    COMPSTA4,CPYSOV12   TEST ALLOW OVER 12 MONTHS BACK DATE          
         BNZ   VALODT2             YES - SKIP TEST FOR BACKDATE                 
         SR    RE,RE                                                            
         ICM   RE,1,TODAYB                                                      
         BNZ   *+8                                                              
         LA    RE,100              YEAR 2000 IS 100 RELATIVE                    
...
*&&                                                                             
*                                                                               
* CANADIAN  7 DAYS INTO THE FUTURE AND 1 YEAR IN THE PAST. EG IF TODAY          
* RULE      IS 8/31/99, THE EARLIEST DTE YOU CAN ENTER IS 8/31/98               
*           (CAN OVERRIDE THE 1 YEAR BACK DATE WITH THE CONTROL REC)            
*                                                                               
         MVC   FVMSGNO,=AL2(EADTFUTR)   DATE TOO FAR IN THE FUTURE              
         XC    TEMP,TEMP                                                        
...
UPDRCF22 GOTO1 AGETACC,0           GET 1P COSTING ACCOUNT                       
         ...
         NI    OVRBYTE,255-NOCWOF  SET WRITE-OFF COSTING OK                     
*                                                                               
UPDRCF24 TM    WOFFINDS,WOFFIYCP   TEST NEW COSTING POSTINGS                    
         BNZ   UPDRCF26                                                         
         TM    WOFFINDS,WOFFINCP   TEST NO NEW COSTING POSTINGS                 
         BNZ   UPDRCF40                                                         
         OC    DISCANAL,DISCANAL   TEST DISCOUNT ANALYSIS A/C                   
...
*                                                                               
         ZAP   OVRWOGST,=P'0'      CLEAR WRITE-OFF GST ADJUSTMENT               
         ZAP   OVRWOPST,=P'0'      CLEAR WRITE-OFF PST ADJUSTMENT               
         OC    GST,GST                                                          
         BZ    UPDP04X                                                          
*                                                                               
         ZAP   OVRPKWK,TSARPOST    PERCENTAGE OF ORIGINAL BILL =                
         SRP   OVRPKWK,5,0         WRITE-OFF * 100000                           
         ZAP   DUB,TSARDR          / ORIGINAL BILL AMOUNT                       
         CP    DUB,=P'0'           CAN'T DIVIDE BY 0                            
         BNE   UPDP04A                                                          
         ZAP   PERCENT,=P'0'       MAKE PERCENTAGE ZERO                         
         B     UPDP04B                                                          
*                                                                               
UPDP04A  DP    OVRPKWK,DUB                                                      
         SRP   OVRPKWK(L'OVRPKWK-L'DUB),64-1,5      / 10 AND ROUND              
         ZAP   PERCENT,OVRPKWK(L'OVRPKWK-L'DUB)                                 
*                                                                               
UPDP04B  ZAP   OVRPKWK,TSARGST     NEW GST                                      
         MP    OVRPKWK,PERCENT                                                  
         SRP   OVRPKWK,64-4,5                                                   
         ZAP   OVRWOGST,OVRPKWK                                                 
         ZAP   OVRPKWK,TSARGSTB    NEW GST BASIS                                
         MP    OVRPKWK,PERCENT                                                  
         SRP   OVRPKWK,64-4,5                                                   
         ZAP   GSTBASIS,OVRPKWK                                                 
         ZAP   OVRPKWK,TSARPST     NEW PST                                      
         MP    OVRPKWK,PERCENT                                                  
         SRP   OVRPKWK,64-4,5                                                   
         ZAP   OVRWOPST,OVRPKWK                                                 
         ZAP   OVRPKWK,TSARPSTB    NEW PST BASIS                                
         MP    OVRPKWK,PERCENT                                                  
         SRP   OVRPKWK,64-4,5                                                   
         ZAP   PSTBASIS,OVRPKWK                                                 
*                                                                               
UPDP04X  BRAS  RE,UPMYPARM                                                      
...
UPDP12   GOTO1 ABLDTRN,MYPARM                                                   
         XC    OVERXTRA,OVERXTRA   CLEAR EXTRA ELEMENTS TO AVOID DUPS           
         AP    RCVITEM,=P'1'                                                    
...
*                                                                               
         ...
         OC    GST,GST             GST ELEM?                                    
         BZ    UPDP15                                                           
         BAS   RE,EXTRAGST         BUILD SCIEL FOR GST                          
         LA    R1,SCILN3Q(R1)                                                   
*                                                                               
UPDP15   OC    PST,PST             PST ELEM?                                    
         BZ    UPDP15X             FORGET ABOUT IT                              
         BAS   RE,EXTRAPST         BUILD SCIEL FOR PST                          
*                                                                               
UPDP15X  TM    TSARINDS,TSARILAT   TEST LATE (SEPARATE POSTING)                 
         BO    UPDP16                                                           
...
*                                                                               
         ...
         ZAP   POSTAMNT,TSARPOST                                                
*                                                                               
         SP    POSTAMNT,OVRWOGST   TAKE IT FROM WRITE-OFF POSTING               
         SP    JRCVWOF,OVRWOGST    TAKE FROM JOURNAL WRITE-OFF TOTALS           
         SP    JTOTWOF,OVRWOGST                                                 
         AP    JRCVWGST,OVRWOGST   ADD TO JOURNAL WRITE-OFF GST TOTALS          
         AP    JTOTWGST,OVRWOGST                                                
         SP    POSTAMNT,OVRWOPST   TAKE IT FROM WRITE-OFF POSTING               
         SP    JRCVWOF,OVRWOPST    TAKE FROM JOURNAL WRITE-OFF TOTALS           
         SP    JTOTWOF,OVRWOPST                                                 
         AP    JRCVWPST,OVRWOPST   ADD TO JOURNAL WRITE-OFF GST TOTALS          
         AP    JTOTWPST,OVRWOPST                                                
*                                                                               
UPDP17   TM    WOFFINDS,WOFFISI    TEST SI WRITE-OFF                            
         BO    UPDP26                                                           
         TM    WOFFINDS,WOFFISE    TEST SE WRITE-OFF                            
         BZ    UPDP28                                                           
         TM    WOFFINDS,WOFFIYCP                                                
...
*                                                                               
         ...
         AP    RCVITEM,=P'1'                                                    
*                                                                               
UPDP36   MVC   POSTDATE,TSARDAT    RESET TRANSACTION DATE                       
         MVC   POSTREF,TSARREF     RESET TRANSACTION REFERENCE                  
         OC    GST,GST             TEST GST A/C PRESENT                         
         BZ    UPDP36B                                                          
         CP    OVRWOGST,=P'0'      IF NON-ZERO AMOUNT                           
         BNE   UPDP36A                                                          
         CLI   TSARGSTT,C'Z'       ZERO RATED                                   
         BE    *+8                                                              
         CLI   TSARGSTT,C'X'       EXEMPT                                       
         BNE   UPDP36B                                                          
UPDP36A  MVC   POSTACT,GST         BUILD GST/RECEIVABLE POSTING                 
         MVC   POSTACTN,GSTNAME                                                 
         MVC   POSTCAC,RECVACT                                                  
         MVC   POSTCACN,RECVACTN                                                
         MVI   POSTSTAT,0          CREDIT                                       
         ZAP   POSTAMNT,OVRWOGST                                                
...
*                                                                               
         ...
         AP    RCVITEM,=P'1'                                                    
*                                                                               
UPDP36B  OC    PST,PST             TEST PST A/C PRESENT                         
         BZ    UPDP37                                                           
         CP    OVRWOPST,=P'0'      IF NON-ZERO AMOUNT                           
         BNE   UPDP36C                                                          
         CLI   TSARPSTT,C'Z'       ZERO RATED                                   
         BE    *+8                                                              
         CLI   TSARPSTT,C'X'       EXEMPT                                       
         BNE   UPDP37                                                           
UPDP36C  MVC   POSTACT,PST         BUILD PST/RECEIVABLE POSTING                 
         MVC   POSTACTN,PSTNAME                                                 
         MVC   POSTCAC,RECVACT                                                  
         MVC   POSTCACN,RECVACTN                                                
         MVI   POSTSTAT,0          CREDIT                                       
         ZAP   POSTAMNT,OVRWOPST                                                
         MP    POSTAMNT,=P'-1'     MINUS                                        
*                                                                               
         LA    R1,ELEXTRA          R1=A(EXTRA ELEMENT AREA)                     
         BAS   RE,EXTRAPST                                                      
         GOTO1 ABLDTRN,MYPARM                                                   
         MVI   ELEXTRA,0           CLEAR EXTRA ELEMENTS                         
         AP    RCVITEM,=P'1'                                                    
*                                                                               
UPDP37   MVC   POSTREF,WOFREF      FIX UP REFERENCE                             
         MVC   POSTAMNT,TSARPOST   FIX UP AMOUNT                                
         OC    GST,GST                                                          
         BZ    *+10                                                             
         SP    POSTAMNT,OVRWOGST   TAKE IT FROM WRITE-OFF POSTING               
         OC    PST,PST                                                          
         BZ    *+10                                                             
         SP    POSTAMNT,OVRWOPST   TAKE IT FROM WRITE-OFF POSTING               
         MVC   POSTDATE,WOFDATE    FIX UP DATE                                  
*                                                                               
         TM    WOFFINDS,WOFFISE    TEST SE WRITE-OFF                            
         BZ    UPDP44                                                           
         TM    WOFFSTAT,RSTSEADD   TEST DEPARTMENT ANALYSIS                     
...
*                                                                               
         ...
         AP    JRCVCRS,JRCVWOF                                                  
         MVC   JRNDATE(42),=42C'-'                                              
         MVC   JRNDATE+14(L'JRNWOFSR),JRNWOFSR                                  
...
UPDRCL6  LA    R2,L'REPP1(R2)      DISCOUNT - SR                                
         MVC   JRNDATE(42),=42C'-'                                              
         MVC   JRNDATE+14(L'JRNDSCSR),JRNDSCSR                                  
         AP    JRCVDRS,JRCVDSC                                                  
...
UPDLASTX B     UPDATEXX                                                         
         ...
         USING SCIELD,R1                                                        
*---------------------------------------------------------------------          
* EXTRAGST - BUILD SCIEL FOR GST, R1 POINTS TO EXTRA AREA                       
*---------------------------------------------------------------------          
EXTRAGST DS    0H                                                               
         MVI   SCIEL,SCIELQ                                                     
         MVI   SCILN,SCILN3Q                                                    
         MVI   SCITYPE,SCITTAXP    TYPE T TAX GST PAID                          
         ZAP   SCIAMNT,OVRWOGST    GST AMOUNT                                   
         ZAP   SCIBASE,GSTBASIS    GST BASIS                                    
         MVC   SCISUBPR,SPACES     GST HAS NO PROVINCE                          
         MVC   SCISUBPT,TSARGSTT   GST TAX TYPE                                 
         MVI   SCIEL+SCILN3Q,0     ZERO TERMINATING                             
         BR    RE                                                               
*                                                                               
*---------------------------------------------------------------------          
* EXTRAPST - BUILD SCIEL FOR PST, R1 POINTS TO EXTRA AREA                       
*---------------------------------------------------------------------          
EXTRAPST DS    0H                                                               
         MVI   SCIEL,SCIELQ                                                     
         MVI   SCILN,SCILN3Q                                                    
         MVI   SCITYPE,SCITTQST    TYPE Q TAX PST PAID                          
         ZAP   SCIAMNT,OVRWOPST    PST AMOUNT                                   
         ZAP   SCIBASE,PSTBASIS    PST BASIS                                    
         MVC   SCISUBPR,TSARPSTC   PST PROVINCE                                 
         MVC   SCISUBPT,TSARPSTT   PST TAX TYPE                                 
         MVI   SCIEL+SCILN3Q,0     ZERO TERMINATING                             
         BR    RE                                                               
         DROP  R1                                                               
         EJECT                                                                  
...
*                                                                               
         ...
         MVI   0(R1),X'FF'                                                      
*                                                                               
UPMY34   OC    GST,GST             TEST GST PRESENT                             
         BZ    UPMY35                                                           
         LA    RE,GST              GST                                          
         ST    RE,0(R1)                                                         
         MVI   0(R1),0                                                          
         ...
         MVI   0(R1),X'FF'                                                      
*                                                                               
UPMY35   OC    PST,PST             TEST PST PRESENT                             
         BZ    UPMY36                                                           
         LA    RE,PST              PST                                          
         ST    RE,0(R1)                                                         
         MVI   0(R1),0                                                          
...
OVRWOGST DS    PL6                 WRITE-OFF GST ADJUSTMENT                     
OVRWOPST DS    PL6                 WRITE-OFF PST ADJUSTMENT                     
PERCENT  DS    PL6                 CALCULATED PERCENTAGE                        
GSTBASIS DS    PL6                 GST BASIS                                    
PSTBASIS DS    PL6                 PST BASIS                                    
...
JRCVWGST DS    PL6                 WRITE-OFF GST ADJUSTMENT AMOUNT              
JRCVWPST DS    PL6                 WRITE-OFF PST ADJUSTMENT AMOUNT              
...
JTOTWGST DS    PL6                 WRITE-OFF GST ADJUSTMENT AMOUNT              
JTOTWPST DS    PL6                 WRITE-OFF PST ADJUSTMENT AMOUNT              
...
* ACCATCALLD                                                                    
       ...
       ++INCLUDE ACCATCALLD                                                     
         ...
         SPACE 1                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'053ACREC04   07/18/19'                                      
         END                                                                    
