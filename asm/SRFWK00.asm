*          DATA SET SRFWK00    AT LEVEL 047 AS OF 03/23/15                      
*PHASE T13D00A                                                                  
*INCLUDE SQUASHER                                                               
*INCLUDE TIMBER                                                                 
*INCLUDE WRSCAN                                                                 
...
*INCLUDE DECODE                                                                 
         ...
         PRINT NOGEN                                                            
WRKF000  CSECT                                                                  
         NMODL FWKWKX-FWKWKD,**=WK0**,RA,R9,RR=R4,CLEAR=YES                     
         USING FWKWKD,RC                                                        
         ST    RB,ABASE                                                         
...
WRKF060  LA    RF,GETUSER          COMMON SUBROUTINES                           
         ...
         ST    RF,CIENQDEQ                                                      
*                                                                               
WRKF070  L     RF,=A(CTREC-FWKWKD) SAVE OUT OF RANGE WORK                       
         LA    RF,FWKWKD(RF)                                                    
         ST    RF,ACTREC                                                        
         L     RF,=A(CXREC-FWKWKD)                                              
         LA    RF,FWKWKD(RF)                                                    
         ...
         ST    RF,AFILTAB                                                       
         EJECT                                                                  
WRKF080  MVI   LOCKT,0             SET INITIAL VALUES                           
         MVI   DDS,0                                                            
         MVI   INTFLAG,0                                                        
         XC    STATCH,STATCH                                                    
         MVI   PAGFLAG,FF                                                       
         MVC   HELPKEY,HELPID                                                   
*                                                                               
         MVC   WRKFID,=CL8'FACWRK'                                              
         CLC   SRVID+1(2),=C'WK'                                                
         BNE   *+10                                                             
         MVC   WRKFID,=CL8'WKFILE'                                              
         CLC   SRVID+1(8),=C'FACWK,WK'                                          
         BNE   *+10                                                             
         MVC   WRKFID,=CL8'WKFILE'                                              
*                                                                               
WRKF090  L     RF,VSSB             SAVE SSB DATA                                
         ...
         MVC   SYSID,SSBSYSID      SET FACPAK SYSID                             
         DROP  RF                                                               
*                                                                               
...
WRKF192  DS    0H                                                               
*                                                                               
         CLC   WKSID,=C'T13D'      ID MUST BE GOOD ELSE INIT                    
         BE    *+8                                                              
...
*                                                                               
         MVC   WKSID,=C'T13D'      SET OR RESET WF ID                           
         EJECT                                                                  
*************************************************************                   
...
VALSEL1  SR    RE,RE               POINT R7 TO CISAVED AREA                     
         IC    RE,WKSPAGE                                                       
         MH    RE,=Y(16*6)         RE=INDEX TO CI ADDRS                         
         LA    R7,WKSCIADS                                                      
         AR    R7,RE               R7=CI ADDRS                                  
         USING CISAVED,R7                                                       
         OC    CISADR,CISADR       TEST WE HAVE A CIADDR                        
         BZ    VALSELX                                                          
*                                                                               
...
ACT002   MVC   WKACT,CISACTN       COPY ACTION AND NUMBER                       
         MVC   WKACT2,CISNUM                                                    
         MVC   WKPASS,CISWRKF                                                   
         MVC   CIPASS(2),CISADR    READ RECORD INTO INDEX AREA                  
...
*                                                                               
         ...
         DROP  RF                                                               
*                                                                               
EXIT1A   CLI   LOCKT,0             UNLOCK WRKF                                  
         BE    EXIT2                                                            
         MVI   LOCKT,0                                                          
         CLC   WRKFID,=CL8'WKFILE'                                              
         BE    EXIT1B                                                           
         GOTO1 CIENQDEQ,DMCB,(C'D',WRKFID)                                      
         B     EXIT2                                                            
EXIT1B   GOTO1 CIENQDEQ,DMCB,(C'D',=C'WRKR')                                    
*                                                                               
EXIT2    L     R1,ATCB             DOUBLE CHECK WK UNLOCKED                     
         TM    TCBFLAG2-TCBD(R1),TCBENQFW                                       
         BNO   EXITX                                                            
         DC    H'0'                                                             
*                                                                               
EXITX    XMOD1                                                                  
...
*        GET FILTER                                         *                   
*************************************************************                   
GETFILT  NTR1  ,                   PUT CIADDR INTO HALF                         
         L     R6,ACXREC                                                        
         USING WKRECD,R6                                                        
         OC    HALF,HALF                                                        
         BNZ   *+6                                                              
...
RIDX020  OC    WKSYSPG(4),WKSYSPG  TEST FOR ZEROS                               
         ...
         CLI   WKSYSPG+3,C' '                                                   
         BH    *+8                                                              
         MVI   3(R4),C'*'          PLACEHOLDER FOR SQUASHER                     
         LA    R4,4(R4)                                                         
         B     RIDX030                                                          
*                                                                               
RIDX024  MVC   0(7,R4),SR8ALL      MOVE IN ALL                                  
         LA    R4,9(R4)                                                         
*                                                                               
RIDX030  CLI   WRKDAY,0            TEST FOR ZERO DAY                            
         BE    RIDX037                                                          
         CLI   WRKDAY,X'5C'        TEST FOR "*"                                 
         BE    RIDX038                                                          
         CLI   WRKDAY,X'C1'        TEST FOR A-9                                 
         BNL   RIDX038                                                          
         IC    R1,WRKDAY           MUST BE PACKED DAY                           
         MVC   1(1,R4),WRKDAY                                                   
         OC    1(1,R4),=X'F0'                                                   
         SRL   R1,4                                                             
         STC   R1,0(R4)                                                         
         ...
         B     RIDX040                                                          
*                                                                               
RIDX037  OC    WKFILEN,WKFILEN     WAS SEQ# INPUT                               
         BNZ   *+12                IF SO MUST BE DAY 00                         
         CLI   WRKDAY0,C'Y'        DAY 00 WAS INPUT                             
         BNE   RIDX039                                                          
         MVC   0(2,R4),=C'00'                                                   
         LA    R4,2(R4)                                                         
         B     RIDX040                                                          
*                                                                               
RIDX038  MVC   0(1,R4),WRKDAY                                                   
RIDX039  LA    R4,1(R4)                                                         
*                                                                               
RIDX040  CLI   WRKCLASS,0          ANY CLASS ?                                  
         BE    RIDX044                                                          
         MVC   0(1,R4),WRKCLASS    MOVE IN CLASS VALUE                          
         TM    WRKCLASS,X'40'                                                   
         BO    RIDX042                                                          
...
RIDX042  LA    R4,1(R4)                                                         
*                                                                               
RIDX044  CLI   WRKEXTRA,0          ANY EXTRA ?                                  
         BE    RIDX044X                                                         
         MVC   0(1,R4),WRKEXTRA                                                 
         LA    R4,1(,R4)                                                        
RIDX044X LA    R4,1(,R4)                                                        
...
P2V010   CLI   1(R6),0             TEST FOR X=Y                                 
         ...
         MVC   WKDDSFN,12(R6)                                                   
*                                                                               
         CLC   22(2,R6),=C'X'''    CHECK FOR HEX NUMBER U=X'12'                 
         BNE   P2V012                                                           
*                                                                               
         CLI   1(R6),3                                                          
         BL    ERR6                MUST BE AT LEAST 1                           
         BH    *+12                                                             
         CLI   24(R6),C''''        MAKE SURE THIS DINK'S LINED UP               
         BE    ERR6                                                             
*                                                                               
         CLI   1(R6),7                                                          
         BH    ERR6                NO MORE THAN 4                               
         BL    *+12                                                             
         CLI   28(R6),C''''        MAKE SURE THIS DINK'S LINED UP               
         BNE   ERR6                                                             
*                                                                               
         LLC   R1,1(R6)            FIND LENGTH OF HEX NUMBER                    
         AHI   R1,-1                                                            
         AR    R1,R6                                                            
         CLI   22(R1),C''''        MAKE SURE THIS DINK'S LINED UP               
         BNE   *+8                                                              
         AHI   R1,-1                                                            
         AHI   R1,-1                                                            
         SR    R1,R6               R1=L'HEX NUMBER                              
*                                                                               
         MVC   FULL,=C'0000'       PUT WHAT'S THERE IN FULL                     
         LA    RF,FULL+L'FULL                                                   
         SR    RF,R1                                                            
         AHI   R1,-1                                                            
         EX    R1,*+8                                                           
         ...
         LA    RF,1(R1)                                                         
*                                                                               
         GOTO1 AHEXIN,DMCB,FULL,WKUSER,L'FULL                                   
         OC    DMCB+12(4),DMCB+12                                               
         BZ    ERR6                NOT A VALID HEX NUMBER                       
         B     P2V01X                                                           
*                                                                               
P2V012   TM    3(R6),X'80'         TEST U=NNNNNN                                
         BZ    P2V014                                                           
         CLI   1(R6),6                                                          
         ...
         B     P2V01X                                                           
*                                                                               
P2V014   CLC   22(8,R6),SR8ALL     TEST U=ALL                                   
         BE    P2V01X                                                           
*                                                                               
...
*                                                                               
         ...
         BNO   P2V022                                                           
         MVO   17(1,R6),16(1,R6)                                                
         MVC   WRKDAY,17(R6)       MOVE PACKED DAY                              
         CLI   WRKDAY,0                                                         
         BNE   *+8                                                              
         MVI   WRKDAY0,C'Y'                                                     
         CLI   0(R6),6                                                          
         BE    P2V02X              WAS P2V026X                                  
         MVC   WRKCLASS,18(R6)     CLASS AS WELL PERHAPS                        
         CLI   0(R6),7                                                          
         BE    P2V02X              WAS SPPUDDCX                                 
         MVC   WRKEXTRA,19(R6)     CLASS AS WELL PERHAPS                        
         B     P2V02X                                                           
*                                                                               
P2V022   MVC   WRKDAY,16(R6)       MOVE NON NUMERIC DAY                         
         CLI   0(R6),5                                                          
         BE    P2V02X              WAS P2V026X                                  
         MVC   WRKCLASS,17(R6)     CLASS AS WELL PERHAPS                        
         CLI   0(R6),6                                                          
         BE    P2V02X              WAS P2V026X                                  
         MVC   WRKEXTRA,18(R6)     CLASS AS WELL PERHAPS                        
         B     P2V02X                                                           
*                                                                               
...
P2V026   TM    12(R6),X'F0'        TEST NUMERIC DAY                             
         ...
         BNO   P2V027                                                           
         MVO   13(1,R6),12(1,R6)                                                
         MVC   WRKDAY,13(R6)       MOVE PACKED DAY                              
         CLI   WRKDAY,0                                                         
         BNE   *+8                                                              
         MVI   WRKDAY0,C'Y'                                                     
         CLI   0(R6),3                                                          
         BL    P2V026X                                                          
         MVC   WRKCLASS,14(R6)     CLASS AS WELL PERHAPS                        
         CLI   0(R6),4                                                          
         BL    P2V026X                                                          
         MVC   WRKEXTRA,15(R6)     EXTRA AS WELL                                
         B     P2V02X                                                           
*                                                                               
...
P2V027   CLI   0(R6),1                                                          
         BNE   P2V028                                                           
         MVC   WRKCLASS,12(R6)     SINGLE CHR IS POSITIVE CLASS FILTER          
         CLI   WRKCLASS,C'*'                                                    
         BE    P2V02X                                                           
         CLI   WRKCLASS,C'A'                                                    
         BL    ERR5                                                             
...
P2V028   CLI   0(R6),2                                                          
         ...
         CLI   WRKCLASS,C'A'                                                    
         BL    ERR5                                                             
         NI    WRKCLASS,X'BF'      NEGATIVE FILTERS ARE LOWER CASE              
*                                                                               
...
SETBUFF  ST    RE,SAVERE                                                        
         ...
         L     R5,ACIREC                                                        
         GOTO1 VDATAMGR,WKDMCB,BUFFER,WRKFID,NDX,(X'FA',ACTREC),(R5)            
         CLI   8(R1),0                                                          
         BE    *+6                                                              
         DC    H'0'                                                             
         L     R5,ACIREC                                                        
...
WKUPDT   NTR1  BASE=ABASE                                                       
         LM    R9,RB,ABASES                                                     
         L     R8,ASYSFAC                                                       
         GOTO1 ADATAMGR,WKDMCB,(LOCKT,STAACT),WRKFID                            
         CLI   8(R1),0                                                          
         BE    WKUPDTX             OK EXIT                                      
         BAS   RE,WKUNLK                                                        
         MVI   LOCKT,0             UNLOCK WK                                    
         DC    H'0'                                                             
WKUPDTX  LH    R1,STATCH           BUMP NUMBER OF STATUS CHANGES                
         ...
         EJECT                                                                  
*************************************************************                   
*        LOCK WRKF IF NOT LOCKED ALREADY                    *                   
*************************************************************                   
WKLOCKR  NTR1  BASE=ABASE                                                       
         LM    R9,RB,ABASES                                                     
         L     R8,ASYSFAC                                                       
         TM    LOCKT,X'40'         TEST ALREADY LOCKED                          
         BO    WKLOCKX                                                          
*                                                                               
         CLC   WRKFID,=CL8'WKFILE'                                              
         BE    WKLOCK2                                                          
*                                                                               
WKLOCK1  BAS   RE,FWLOCK           ENQ FACWK                                    
         B     WKLOCK9                                                          
WKLOCK2  BAS   RE,WKLOCK           ENQ WRKR                                     
WKLOCK9  TM    4(R1),X'40'                                                      
         BNO   *+6                                                              
         DC    H'0'                                                             
*                                                                               
         OI    LOCKT,X'40'         SET LOCKT                                    
WKLOCKX  B     XIT1                                                             
         EJECT                                                                  
*************************************************************                   
...
SETUSR   NTR1                                                                   
         ...
         CLI   WKQUEUE,C'U'                                                     
         BE    SETUSR2                                                          
*        MVC   WRKFID,=CL8'WKFILE' SET SPECIFIC QUEUE NAME                      
...
HELPTAB  DS    0CL3                    INDEX/FLAGS/PANEL                        
         ...
         DC    X'00'                                                            
*                                                                               
HELPID   DC    XL10'013DFF00000000000000'                                       
         EJECT                                                                  
         DS    0H                                                               
...
*************************************************************                   
       ++INCLUDE DMWRKRR           WORKER ROUTINES                              
         LTORG                                                                  
       ++INCLUDE SRFWKDD           DICTIONARY REFS                              
         DS    0H                                                               
         EJECT                                                                  
       ++INCLUDE SRFWKWK                                                        
         EJECT                                                                  
...
*DDGLOBEQUS                                                                     
       ...
       ++INCLUDE DDGLOBEQUS                                                     
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'047SRFWK00   03/23/15'                                      
         END                                                                    
