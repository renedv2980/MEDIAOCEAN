*          DATA SET SPLDEXTSDD AT LEVEL 041 AS OF 01/18/05                      
*PHASE SP@XTSDA SP@XTSDD                                                        
*INCLUDE PRINT                                                                  
*INCLUDE PRINTER                                                                
*INCLUDE PRNTBL                                                                 
*INCLUDE HEXOUT                                                                 
...
*INCLUDE CLUNPK                                                                 
*INCLUDE DATCON                                                                 
         TITLE 'DMLDEXT - OVERNIGHT DATA BUILD FOR TMNY ON SPOT FILE'           
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
* THIS IS AN EXTERN USED FOR DUMPING THE SPOT0 FILE.  IT IS STEP 1              
*  IN CREATING A NEW BASE TAPE OF THE SPOT0 FILE.  SINCE OTHER PROGRAMS         
*  ARE USING SPOT0 AND WOULD LIKE THEIR DATA RESET DAILY, WE ARE USING          
*  THIS EXTERN TO PEEL THE RECORDS THAT WE DO WANT TO ADD OR CHANGE.            
*                                                                               
...
*                                                                               
* FIRST A TABLE IS CREATED OF CLIENTS WHOSE CLIENT RECORD FIELD                 
* 'CCLTTYPE' MATCHES PRGRUNG.  THIS WILL BE THE LIST OF CLIENTS WHOSE           
* RECORDS WILL BE PEELED OFF.                                                   
*                                                                               
* THERE ARE THREE OTHER TABLES IN THIS PROGRAM THAT REPRESENT THE               
* BUYERS, SUPERVISORS, AND  NWS BUYERS WHOSE RECORDS WILL ALSO BE               
* PEELED.  THE FIRST CHARACTER IN EACH ENTRY OF THESE TABLES SPECIFIES          
* WHICH PROGRAM THE BUYER, SUPERVISOR, OR NWS BUYER IS USED FOR.  THIS          
* CHARACTER WILL BE CHECKED TO SEE IF IT MATCHES PRGRUNG.                       
*                                                                               
* A LIST OF ALL THE CLIENTS, BUYERS, SUPERVISORS, AND NWS BUYERS ARE            
* WRITTEN OUT TO SDCLT, WHICH WILL BE USED FOR THE LOAD PROGRAM.                
*                                                                               
* AS EACH RECORD IS PROCESSED THE CLIENT, BUYER, SUPERVISOR, OR NWS             
* BUYER CODE WILL BE CHECKED TO SEE IF THE RECORD SHOULD BE PEELED              
* AND WRITTEN OUT TO TAPEOUT WHICH IS USED FOR THE LOAD.                        
*                                                                               
* STEP 2 IS THE LOAD, WHERE THE EXTERN SPLDEXTSDL IS USED.  IT WILL USE         
* AS INPUT THE CURRENT BASE FILE, WHICH WAS CREATED FROM THE PREVIOUS           
* DAY, TAPEOUT, AND SDCLT.  AS IT PROCESSES EACH RECORD ON THE CURRENT          
* FILE, IT WILL THROW OUT ANY RECORDS WHOSE CLIENTS, BUYERS,                    
* SUPERVISORS, OR NWS BUYERS MATCHES THE LIST FOUND IN SDCLT AND KEEP           
* ALL OTHER RECORDS AND CONCATENATE THIS WITH TAPEOUT.                          
*                                                                               
* STEP 3 DUMP THIS FILE AND CREATE A NEW BASE FILE.                             
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *         
* PARAMETER LIST                                                                
*                                                                               
...
*                                                                               
         L     R2,APARAMC                                                       
         CLC   0(2,R2),SPACES2     IS AGENCY VALID?                             
         BH    *+6                                                              
         DCHO                                                                   
...
*                                                                               
DMXREC   L     R3,AREC                                                          
         TM    15(R3),X'80'              IGNORE RECS MARKED FOR DELETE          
         BO    DMXPURGE                                                         
         LA    R4,CLTTAB                                                        
         ZICM  RE,COUNTER,2                                                     
         MHI   RE,6                                                             
...
RECTYPD  MVC   BAMCLT,1(R3)        XX A/M CLT CLT                               
         ...
         USING CLTTABD,R4                                                       
         USING CLTHDR,R3                                                        
         CLC   CCLTTYPE,PRGRUNG    CLIENT FOR PROGRAM RUNNING FOR?              
         BNE   RECTYPDX                                                         
...
RECTYPK  CLC   2(2,R3),AGY         XX XX AGY AGY 4XX'S CLT CLT                  
         ...
         MVC   BCLT,8(R3)                                                       
         B     DMXKX10             ASSUME TV SINCE CANADIAN NETWORK             
*                                                                               
...
RECTYPM  DS    0H                                                               
         ...
         BNE   RECM10                                                           
         CLI   1(R3),X'3D'         KEEP ALL 0D3D'S                              
         BE    DMXKEEP             XX XX A/M 3X CLT CLT                         
RECM10   MVC   BAGYMD,2(R3)                                                     
         MVC   BCLT,6(R3)                                                       
         CLI   6(R3),0             NO CLIENT IN SOME 0D31'S                     
         BNE   DMXKX20                                                          
         B     DMXPURGE                                                         
*                                                                               
...
RECTYPS  MVC   BAGYMD,2(R3)        XX XX A/M CLT CLT CLT                        
         ...
         B     DMXKX30                                                          
*                                                                               
RECTYPTS LA    R5,SPVTAB           SUPERVISOR RECORDS                           
         BAS   RE,SPVBUY                                                        
         BNE   DMXPURGE                                                         
         B     DMXKX30                                                          
*                                                                               
RECTYPTB LA    R5,BUYTAB           BUYER RECORDS                                
         BAS   RE,SPVBUY                                                        
         BNE   DMXPURGE                                                         
         B     DMXKX30                                                          
*                                                                               
...
RECTYPX  DS    0H                  0D XX A/M                                    
         CLI   KEEPAGY,C'Y'        KEEP AGENCY RECORD?                          
         BNE   DMXPURGE            THEN KEEP THESE RECORDS ALSO IF              
         MVC   BYTE,2(R3)          AGENCY MATCHES BAM                           
         NI    BYTE,X'F0'          CLEAR MEDIA                                  
         MVC   BAGYMD,BAM                                                       
         NI    BAGYMD,X'F0'        CLEAR MEDIA                                  
         CLC   BYTE,BAGYMD                                                      
         BNE   DMXPURGE                                                         
...
*                                                                               
         MVC   BYTE,BAGYMD                                                      
         NI    BYTE,X'0F'          CLEAR AGENCY AND                             
         CLI   BYTE,0              CHECK MEDIA IS SET!                          
         BNE   *+6                                                              
         DC    H'0'                                                             
*                                                                               
DMXKX20  BAS   RE,CHKBCLT          IS CLIENT IN CLIENT LIST?                    
         BNE   DMXPURGE                                                         
DMXKX30  DS    0H                                                               
***      MVI   P,C'K'                                                           
...
CBCLT10  CLI   0(R5),X'FF'                                                      
         ...
         CLC   CLTAGMD,BAGYMD      CORRECT AGENCY/MEDIA?                        
         BNE   CBCLT20                                                          
         OC    BCLT,BCLT           HAVE A BINARY CLIENT?                        
         BZ    CBCLTX              NO                                           
...
UPDTBY10 CLI   0(R2),X'FF'         END OF TABLE?                                
         BE    NO                                                               
         CLC   PRGRUNG,BYRPROG     CHECK IF THIS IS THE PROGRAM WE WANT         
         BNE   UPDTBY20                                                         
...
SPVBUY10 CLI   0(R5),X'FF'                                                      
         BE    NO                                                               
         CLC   PRGRUNG,SVBYPROG    CHECK IF THIS IS THE PROGRAM WE WANT         
         BNE   SPVBUY20                                                         
...
*                                                                               
***********************************************************************         
* PUTNBY  STORE TABLES INTO DATASET                                   *         
***********************************************************************         
*                                                                               
PUTNBY   NTR1                                                                   
         USING BYRD,R5                                                          
...
*                                                                               
         ...
         DC    X'0614161819203233'                                              
         DC    X'363B3C464750525A5B5D5E'                                        
         DC    X'60E2E5'                                                        
...
TYPTAB   DC    AL4(RECTYPD)                XX A/M CLT CLT                       
         DC    X'000237B7FF'                                                    
         DC    AL4(RECTYPH),X'09FF'        XX AGY AGY MED CLT CLT               
...
*                                                                               
         ...
         EJECT                                                                  
*                                                                               
* PROGRAM, BINARY A/M(1), OFFICE CODE(2), SUPERVISOR CODE(4)                    
SPVTAB   DC    C'S',X'10',CL2'CH',CL4'FRED'  SUPERDESK SUPERVISORS              
         DC    C'S',X'10',CL2'CH',CL4'ETHE'                                     
         DC    C'S',X'10',CL2'CH',CL4'LUCY'                                     
         DC    C'S',X'10',CL2'NY',CL4'BUFF'                                     
         DC    C'S',X'10',CL2'NY',CL4'ANGE'                                     
         DC    C'S',X'10',CL2'NY',CL4'RICK'                                     
         DC    C'S',X'10',CL2'SD',CL4'BSUP'                                     
         DC    C'S',X'10',CL2'SD',CL4'CSUP'                                     
         DC    C'S',X'10',CL2'NY',CL4'ROSS' *******DELETE*******                
         DC    C'S',X'10',CL2'CH',CL4'ROSS' *******DELETE*******                
*                                                                               
         DC    X'FF'                                                            
*                                                                               
* PROGRAM, BINARY A/M(1), OFFICE CODE(2), BUYER CODE(4)                         
BUYTAB   DC    C'S',X'10',CL2'NY',CL4'ROSS'  SUPERDESK BUYERS                   
         DC    C'S',X'10',CL2'NY',CL4'CHAN'                                     
         DC    C'S',X'10',CL2'NY',CL4'PHOE'                                     
         DC    C'S',X'10',CL2'NY',CL4'RACH'                                     
         DC    C'S',X'10',CL2'NY',CL4'JOEY'                                     
         DC    C'S',X'10',CL2'NY',CL4'MONI'                                     
         DC    C'S',X'10',CL2'CH',CL4'RED '                                     
         DC    C'S',X'10',CL2'CH',CL4'GREE'                                     
         DC    C'S',X'10',CL2'CH',CL4'BLUE'                                     
         DC    C'S',X'10',CL2'CH',CL4'ORAN'                                     
         DC    C'S',X'10',CL2'SD',CL4'JOHN'                                     
         DC    C'S',X'10',CL2'SD',CL4'JANE'                                     
         DC    C'S',X'10',CL2'NY',CL4'RICK' *******DELETE*******                
         DC    C'S',X'10',CL2'CH',CL4'ROSS' *******DELETE*******                
*                                                                               
         DC    X'FF'                                                            
*                                                                               
* NWS BUYERS                                                                    
* PROG CODE, BINARY A/M(1), EBCDIC BUYER CODE(3), BINARY BUYER CODE(1)          
BYRTABLE DC    C'S',X'11',C'ROS',X'00'   SUPERDESK NWS BUYERS                   
         DC    C'S',X'11',C'CHA',X'00'                                          
         DC    C'S',X'11',C'PHO',X'00'                                          
         DC    C'S',X'11',C'JOE',X'00'                                          
         DC    C'S',X'11',C'MON',X'00'                                          
         DC    C'S',X'11',C'RAC',X'00'                                          
*                                                                               
         DC    C'T',X'11',C'CHM',X'00'   TAM NWS BUYERS                         
         DC    C'T',X'11',C'M6 ',X'00'                                          
         DC    C'T',X'11',C'NA1',X'00'                                          
         DC    C'T',X'11',C'ND ',X'00'                                          
         DC    C'T',X'11',C'PB ',X'00'                                          
*                                                                               
         DC    C'P',X'21',C'CCS',X'00'   CANTEST STEWARD NWS BUYERS             
         DC    C'P',X'21',C'FHS',X'00'                                          
         DC    C'P',X'21',C'HMS',X'00'                                          
         DC    C'P',X'21',C'KLP',X'00'                                          
         DC    C'P',X'21',C'LAB',X'00'                                          
         DC    C'P',X'21',C'MAS',X'00'                                          
         DC    C'P',X'21',C'MMS',X'00'                                          
         DC    C'P',X'21',C'NNS',X'00'                                          
         DC    C'P',X'21',C'ONS',X'00'                                          
         DC    C'P',X'21',C'PYS',X'00'                                          
         DC    C'P',X'21',C'UDS',X'00'                                          
         DC    C'P',X'21',C'VSS',X'00'                                          
         DC    C'P',X'21',C'WRS',X'00'                                          
         DC    X'FF'                                                            
*                                                                               
* THESE FIELDS MAY HAVE TO BE CHANGED                                           
***KEEPAGY  DC    C'N'               KEEP CHANGES TO AGENCY RECORD              
*                                                                               
* VALUES FOR NET STEWARD                                                        
***AGY      DC    C'HA'              2 BYTE AGENCY                              
***BAM      DC    X'21'              BINARY A/M CODE                            
***PRGRUNG  DC    C'P'               PROGRAM RUNNING (STEWARD)                  
*                                                                               
* VALUES FOR TMNY PROGRAMS                                                      
***AGY      DC    C'TM'              2 BYTE AGENCY CODE                         
***BAM      DC    X'11'              A/M CODE                                   
***PRGRUNG  DC    C'D'               POGRAM RUNNING SPOT BUY DESKTOP            
***PRGRUNG  DC    C'R'               PROGRAM RUNNING SPOT OM DESKTOP            
...
*                                                                               
BYRD     DSECT                                                                  
BYRPROG  DS    C                   PROGRAM THIS IS FOR                          
BYRAGYMD DS    X                   BINARY AGENCY MEDIA                          
BYREB    DS    CL3                 EBCDIC BUYER CODE                            
BYRBIN   DS    X                   BINARY BUYER CODE                            
BYRLNQ   EQU   *-BYRD                                                           
*                                                                               
SVBYTABD DSECT                     SUPERVISOR/BUYER                             
SVBYPROG DS    C                   PROGRAM THIS IS FOR                          
SVBYMED  DS    X                   A/M                                          
...
*                                                                               
       ...
       ++INCLUDE SPGENDRORD                                                     
         EJECT                                                                  
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'041SPLDEXTSDD01/18/05'                                      
         END                                                                    
