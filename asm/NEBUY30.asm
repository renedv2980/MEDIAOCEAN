*          DATA SET NEBUY30    AT LEVEL 041 AS OF 11/08/18                      
*PHASE T31130B,+0                                                               
...
DISPRDX  BR    R4                                                               
         SPACE 2                                                                
*                                                                               
* DISPLAY PRODUCT FROM ALPHA FORMAT 19 ELEMENT                                  
*                                                                               
DISPALPH XC    DUB,DUB             ALPHA PRODUCT HOLD AREA                      
         MVI   ELCODE,X'19'                                                     
         BAS   R8,GETEL                                                         
         BE    *+6                                                              
         DC    H'0'                                                             
         ...
         USING NUPDED,RE                                                        
         MVC   DUB(3),NUPDEPR                                                   
         CLI   NUPDELEN,11         CHECK IF TWO PRODUCTS                        
         BL    DISPA20                                                          
         MVC   DUB+3(3),NUPDEPR+7                                               
*                                                                               
         CLI   ACTION,DM           TEST FOR DISPLAY MULTIPLE                    
         BE    DISPA20             NO-DO NOT FORCE OUT ZERO                     
         CLI   ACTION,CM           TEST FOR FORCED DIS MULTIPLE                 
         BE    DISPA20                                                          
*                                                                               
         TM    NUPDEIND,X'C0'      COPY-SPLIT,TRIGGYPBACK SET                   
         BZ    DISPA20                                                          
         MVC   FLD(8),=8C'*'                                                    
         BR    R4                                                               
*                                                                               
DISPA20  OC    DUB,DUB             TEST IF ANY PRODUCT ALLOCATED                
         BZ    DISPADX                                                          
         LA    R2,FLD              R2 POINTS TO OUTPUT                          
         TM    NBUNST2,X'20'       TEST FOR FROZEN PROD. ALLOCATION             
         BZ    *+12                NO                                           
...
DISTIME  ST    RE,FULL                                                          
         ...
         BR    RE                                                               
         SPACE 2                                                                
* DISPLAY GAP AFFIDAVIT TIME                                                    
*                                                                               
DISGAFFD LR    R0,RE               SAVE RETURN POINT                            
         GOTO1 VHELLO,DMCB2,(C'G',UNTFILE),(X'18',NBAIO),0                      
         CLI   12(R1),0            TEST IF 18 ELEMENT                           
         BNE   DISGAFEX            NO                                           
         L     R3,12(R1)                                                        
         USING NUDTAD,R3                                                        
         OC    NUDTGAFD,NUDTGAFD                                                
         BZ    DISGAFEX                                                         
         XC    FULL,FULL                                                        
         MVC   FULL(2),NUDTGAFD                                                 
...
DISASS3  GOTO1 VHELLO,DMCB2,(C'G',UNTFILE),(X'02',NBAIO),0                      
         CLI   12(R1),0            TEST IF PKG GUARANTEE FOUND                  
         BNE   DISASSEX            NO                                           
         L     R3,12(R1)                                                        
         USING NUSDRD,R3                                                        
         TM    NUSDST3,X'80'       CHECK FOR FROZEN ASSIGNED COST               
         BZ    DISASSEX                                                         
         LA    R8,FLD                                                           
         MVC   DUB(8),0(R8)                                                     
         MVI   0(R8),C'*'        SET FROZEN STATUS                              
         LA    R8,1(R8)                                                         
         MVC   0(8,R8),DUB                                                      
DISASSEX BR    R4                                                               
         SPACE 2                                                                
...
*                                                                               
DISINT   L     RF,APACKREC                                                      
         USING NPRECD,RF                                                        
*                                                                               
DISI050  ICM   R2,15,NBINTEG                                                    
         BZ    DISI100                                                          
         BAS   R8,EDTMIN                                                        
         BR    RE                                                               
         SPACE                                                                  
...
*                                                                               
         ...
         L     R3,12(R1)                                                        
*                                                                               
         CLI   NUSTCAR,0           ANY TCAR FOR THIS UNIT?                      
         BE    DISTCARX                                                         
         MVC   FLD(1),NUSTCAR                                                   
*                                                                               
DISTCARX LR    RE,R2                                                            
...
DISBB    LR    R2,RE                                                            
         ...
         BZ    DISBBX                                                           
*******  BNZ   DISBB50                                                          
*******  CLI   NUCMLBSL-NUCMLEL(RE),0  OR PRESENT                               
*******  BE    DISBBX                                                           
*******  CLI   NUCMLBSL-NUCMLEL(RE),X'FF'                                       
*******  BE    DISBBX                                                           
DISBB50  MVC   FLD(2),=C'BB'                                                    
DISBBX   BR    R2                                                               
         SPACE 2                                                                
...
FC110    ZIC   RF,1(RE)                                                         
         ...
         CLI   0(RE),0                                                          
         BE    DISFCDX                                                          
         CLI   0(RE),X'19'         FIND MEDIA 21 ELEM (NOT DELETED)             
         BNE   *+8                                                              
         MVI   BYTE,X'FF'                                                       
         CLI   0(RE),X'21'         FIND MEDIA 21 ELEM (NOT DELETED)             
         BL    FC110                                                            
         BH    FC120                                                            
         CLI   BYTE,X'FF'           CHECK FOR ALPH ELEMENT                      
         BNE   FC112                CHECK 1 BYTE PRODS                          
         CLC   NUCMPROD-NUCMLEL(3,RE),FULL                                      
         BNE   FC120               INPUTTED PROD TO ELEMENT PRODUCT             
         CLI   NUCMPPOS-NUCMLEL(RE),0                                           
         BE    FC115               NO POSITION NUMBER                           
         CLC   NUCMPPOS-NUCMLEL(1,RE),FULL+3                                    
         BNE   FC120               COMPARE POSITION NUMBER                      
         B     FC115                                                            
FC112    CLC   NUCMLPRD-NUCMLEL(1,RE),FULL                                      
         BNE   FC120               INPUTTED PROD TO ELEMENT PRODUCT             
         CLI   NUCMPPOS-NUCMLEL(RE),0                                           
         BE    FC115               NO POSITION NUMBER                           
...
FC120    ZIC   RF,1(RE)                                                         
         ...
         LR    R3,R2                                                            
         B     FC135                                                            
FC130    CLI   BYTE,X'FF'           CHECK FOR ALPH ELEMENT                      
         BNE   FC132                CHECK 1 BYTE PRODS                          
         CLC   NUFDPROD-NUFDCEL(3,RE),FULL                                      
         BNE   FC120               INPUTTED PROD TO ELEMENT PRODUCT             
         CLI   NUFDPPOS-NUFDCEL(RE),0                                           
         BE    FC135               NO POSITION NUMBER                           
         CLC   NUFDPPOS-NUFDCEL(1,RE),FULL+3                                    
         BNE   FC120               CHECK POSITION MATCH                         
         B     FC135                                                            
FC132    CLC   NUFDCPRD-NUFDCEL(1,RE),FULL                                      
         BNE   FC120               INPUTTED PROD TO ELEMENT PRODUCT             
         CLI   NUFDPPOS-NUFDCEL(RE),0                                           
         BE    FC135               NO POSITION NUMBER                           
...
DISRAT   SR    R2,R2                                                            
         ...
         BNER  RE                                                               
         SPACE                                                                  
DISRAT2  TM    NBINDS9,NBI9DOV     SET DEMO OVERRIDES?                          
         JZ    DISRAT4                                                          
         STH   R2,HALF                                                          
         NI    HALF,X'FF'-X'80'                                                 
         LH    R2,HALF                                                          
DISRAT4  LR    R3,RE               SAVE RETURN POINT                            
*                                                                               
*--CHECK PRECISION FACTOR FOR RATING                                            
         TM    DEMPREC+1,X'82'                                                  
         BO    DISRAT25                                                         
...
*                                                                               
DISTIER  LR    R2,RE               SAVE RETURN POINT                            
         MVI   ELCODE,X'18'                                                     
         BAS   R8,GETEL                                                         
         BNE   DISTIERX            NO PACKAGE FILTER ELEMENT                    
         L     R4,12(R1)                                                        
         USING NUDTAD,R4                                                        
         MVC   FLD(1),NUDTATIR                                                  
DISTIERX LR    RE,R2                                                            
         ...
         SPACE 2                                                                
*                                                                               
* DISPLAY OLD/NEW PROGRAM                                                       
*                                                                               
DISNEW   LR    R2,RE               SAVE RETURN POINT                            
         MVI   ELCODE,X'18'                                                     
         BAS   R8,GETEL                                                         
         BNE   DISNEWX                                                          
         L     R4,12(R1)                                                        
...
DISNEWX  LR    RE,R2                                                            
         ...
         DROP  R4                                                               
*                                                                               
* DISPLAY CPR INFO (CROSS PLATFORM PROGRAM)                                     
*                                                                               
DISRCP   MVI   FLD,C'N'                                                         
         TM    NBUNST4,X'10'                                                    
         BZ    DISRCP10                                                         
...
DISRCP10 BR    RE                                                               
         SPACE 2                                                                
*                                                                               
* DISPLAY DEFAULT FLIGHT INFO                                                   
*                                                                               
DISFLT   LR    R2,RE               SAVE RETURN POINT                            
         GOTO1 VGETFLT,DMCB2,(0,NBAIO),(0,FLD)                                  
DISPFLTX LR    RE,R2                                                            
         BR    RE                                                               
         SPACE 2                                                                
*                                                                               
...
*                                                                               
DISPTYPE LR    R2,RE               SAVE RETURN POINT                            
         MVI   ELCODE,X'18'                                                     
         BAS   R8,GETEL            SUBSIDIARY 01 ELEMENT                        
         BNE   DISPTYPX                                                         
         L     R4,12(R1)                                                        
         USING NUDTAD,R4                                                        
         MVC   FLD(2),NUDTATYP                                                  
         MVC   FLD+2(4),NUDTASPT                                                
DISPTYPX LR    RE,R2                                                            
         BR    RE                                                               
         DROP  R4                                                               
         SPACE 2                                                                
*                                                                               
* DISPLAY PROGRAM RATING                                                        
*                                                                               
DISCONT  LR    R2,RE               SAVE RETURN POINT                            
         MVI   ELCODE,X'18'                                                     
         BAS   R8,GETEL            SUBSIDIARY 01 ELEMENT                        
         BNE   DISCONTX                                                         
         L     R4,12(R1)                                                        
...
DISBUY10 TM    NBUNST3,X'10'       CHECK FOR PUP CREATED BUY                    
         BZ    *+8                                                              
         MVI   FLD+8,C'P'                                                       
         TM    NBUNST4,X'08'       CHECK FOR FRONTRUNNER CREATED BUY            
         BZ    *+8                                                              
         MVI   FLD+8,C'F'                                                       
...
DISOT050 CLI   0(R8),X'FF'                                                      
         ...
         BE    DISOT070                                                         
         B     DISOT060                                                         
DISOT055 ZIC   RF,9(R8)                                                         
         BCTR  RF,0                                                             
         EX    RF,COMOTHTB                                                      
         BE    DISOT070            SINGLE ELEMENT REQUESTED                     
DISOT060 LA    R8,10(R8)                                                        
         B     DISOT050                                                         
COMOTHTB CLC   0(0,R3),1(R8)       IF MULTIPLE DISPLAY ONLY                     
*                                                                               
...
*--SPECIAL SETUP FOR (NEW) DAYPART MOVE                                         
DISOT071 CLI   NUOTTYP,C'U'                                                     
         BNE   DISOT072                                                         
*                                                                               
         LR    RE,R4                                                            
         MVC   0(2,RE),NUOTHER                                                  
...
*                                                                               
         ...
         SH    RF,=H'6'                                                         
         EX    RF,*+4                                                           
         MVC   0(0,RE),NUOTHER+2                                                
* SET TOTAL OUTPUT LENGTH                                                       
         ZIC   RF,NUOTLEN                                                       
         LA    RE,4                                                             
         CLI   NUOTHER+1,X'40'      CHECK 2 BYTE DAYPART CODE                   
         BE    *+6                                                              
         BCTR  RE,0                 INCREASE OUTPUT LENGTH BY 1                 
         SR    RF,RE                                                            
         B     DISOT180                                                         
*                                                                               
...
DISOT074 CLI   0(R8),C'L'                                                       
         ...
         B     DISOT180                                                         
*                                                                               
*--SPECIAL SETUP FOR IMPRESSION BILLED PRODUCT                                  
DISOT075 CLI   0(R8),C'N'                                                       
         BNE   DISOT076                                                         
         MVC   0(3,R4),NUOTHER                                                  
         LA    RF,3                                                             
         B     DISOT180                                                         
*                                                                               
*--SPECIAL SETUP AIRING VENDOR                                                  
DISOT076 CLI   0(R8),C'V'                                                       
         BNE   DISOT080                                                         
         MVC   0(4,R4),NUOTHER                                                  
         LA    RF,2                                                             
         CLI   NUOTHER+3,C' '                                                   
         ...
         B     DISOT180                                                         
*                                                                               
*--SPECIAL SETUP HP PERCENT                                                     
DISOT080 CLI   0(R8),C'0'                                                       
         BNE   DISOT170                                                         
         LR    RE,R4                                                            
         LA    RF,1                                                             
         MVC   0(3,R4),NUOTHER                                                  
         ...
         LA    RF,1(RF)                                                         
         LA    RE,1(RE)                                                         
DISOT082 LA    RF,3(RF)             ADD STAR ADD PCT TO FIELD LENGTH            
         MVI   0(RE),STAR                                                       
         LR    R8,R0                                                            
         EDIT  (B2,NUOTHER+3),(2,1(RE)),ALIGN=LEFT,ZERO=NOBLANK                 
         LR    R0,R8                                                            
         B     DISOT180                                                         
*                                                                               
DISOT170 ZIC   RF,NUOTLEN          MOVE DATA OUT                                
         LA    RE,4                                                             
         SR    RF,RE                                                            
         EX    RF,*+8                                                           
         B     *+10                                                             
         MVC   0(0,R4),NUOTHER                                                  
DISOT180 CLI   ACTION,DM           IF MULTIPLE DISPLAY DONT                     
         BE    DISOTHEX            LOOK FOR ANY MORE ELEMENTS                   
         LA    RF,1(RF)                                                         
         LA    R4,0(RF,R4)                                                      
*                                                                               
DISOT200 ZIC   RF,NUOTLEN                                                       
         AR    R2,RF                                                            
         CLI   0(R2),X'60'                                                      
         BNE   DISOTHEX                                                         
...
TYPTAB   DS    0XL4                                                             
         ...
         DC    AL1(UGAFFID),AL3(DISGAFFD)                                       
         DC    AL1(UASS),AL3(DISASS)                                            
         DC    AL1(UACT),AL3(DISACT)                                            
         DC    AL1(UTCAR),AL3(DISTCAR)                                          
         DC    AL1(UTCAR1),AL3(DISTCAR)                                         
         ...
         DC    AL1(UTIER),AL3(DISTIER)                                          
         DC    AL1(UNEW),AL3(DISNEW)                                            
         DC    AL1(UPRTYPE),AL3(DISPTYPE)                                       
         DC    AL1(UCONT),AL3(DISCONT)                                          
         DC    AL1(URCP),AL3(DISRCP)                                            
         DC    AL1(UFLT),AL3(DISFLT)                                            
...
INT      EQU   C'I'                INTEGRATION DOLLARS                          
         SPACE 2                                                                
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'041NEBUY30   11/08/18'                                      
         END                                                                    
