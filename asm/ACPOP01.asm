*          DATA SET ACPOP01    AT LEVEL 010 AS OF 07/23/13                      
*PHASE T60A01C                                                                  
*INCLUDE POPTXT                                                                 
...
INIT02   IC    R2,0(R1)            'OR' IN SPACES                               
         TM    1(R1),X'20'         IS FIELD PROTECTED?                          
         BO    INIT04              YES, LEAVE IT ALONE                          
         CLI   5(R1),0             ANY DATA?                                    
         BE    INIT04              NO, LEAVE IT ALONE                           
         SH    R2,=H'9'                                                         
         EX    R2,*+8                                                           
         B     *+10                                                             
         OC    8(0,R1),SPACES                                                   
...
BLDNEW   MVI   ORDKTYP,ORDKTYPQ    R5(ORDER RECORD)                             
         ...
         CLI   TYPE,ANALZED        THIS EXPENSE ANALYZED?                       
         BNE   *+8                 NO                                           
         OI    ORDSTAT,ORDSANL     YES, INDICATE IT ON ORDEL                    
         GOTOR ADDELEM,AIOAREA3    ADD THE ELEMENT TO AIOAREA3                  
                                                                                
         TM    TYPE,EXPENSE+ANALZED EXPENSE OR ANALYZED?                        
         BZ    BLDN02              NO                                           
                                                                                
         GOTOR VALAMT              VALIDATE AMOUNT                              
         BNE   ERXIT                AND ADD OAMELQ                              
         CLI   TYPE,ANALZED        THIS EXPENSE ANALYZED?                       
         BNE   BLDN04              NO                                           
         GOTO1 BLDEXO              YES, BUILD EXOELQ ELEMENT                    
         GOTOR ADDELEM,AIOAREA3      AND ADD IT                                 
         B     BLDN04                                                           
                                                                                
BLDN02   GOTOR VALWC               VALIDATE WORKCODE, STATUS & AMOUNTS          
...
CHGE16   TM    TYPE,EXPENSE+ANALZED   THIS AN EXPENSE ORDER?                    
         BZ    CHGE18              NO                                           
         GOTOR VALAMT              VALIDATE AMOUNT                              
         BNE   ERXIT                AND ADD OAMELQ                              
         B     CHGE20                                                           
                                                                                
CHGE18   GOTOR VALWC               VALIDATE WORKCODE, STATUS & AMOUNT           
         BNE   ERXIT                                                            
...
VALCLI   NTR1                                                                   
         ...
         BNE   VALCER                                                           
         LA    R1,APODEPH                                                       
         CLC   LDEP(L'APODEP),8(R1)                                             
         BNE   VALCER                                                           
         LA    R1,APOPERH                                                       
         CLC   LPER(L'APOPER),8(R1)                                             
         BNE   VALCER                                                           
         LA    R1,APOSUPH                                                       
         CLC   LSUPP,8(R1)                                                      
         BE    VALCX                                                            
...
VALTAX   NTR1                                                                   
         ...
         TM    TYPE,EXPENSE+ANALZED EXPENSE OR ANALYZED?                        
         BZ    VALT00              NO                                           
         MVI   FERN,EXPTAX         TAX IS INVALID FOR EXPENSE                   
         CLI   5(R1),0                                                          
         BNE   VALTER                                                           
         B     VALTX                                                            
                                                                                
VALT00   CLI   5(R1),0             DID THEY ANSWER TAX QUESTION ?               
...
VALAMT   NTR1                                                                   
         ...
         TM    TYPE,ANALZED        IF ANALYZED, MUST USE TYPE 10                
         BZ    *+8                                                              
         OI    VSTAT,ORDSTY10                                                   
         ...
         LA    RE,OAMTELS          GET PREVIOUS ENTRY                           
...
VALA01   MVC   NAMTELS(L'ELEMENT),ELEMENT                                       
         GOTOR ADDELEM,AIOAREA3                                                 
         LHI   R6,20               MAXIMUM NUMBER OF SCREEN ENTRIES             
         B     VALA04                                                           
                                                                                
VALA02   BAS   RE,NEXTWORK         GET TO THE NEXT LINE                         
         CLI   SCRWRKH+5,0         ALL OTHER ENTRIES MUST BE BLANK              
         BNE   VALAWER                                                          
...
VALA04   BCT   R6,VALA02                                                        
                                                                                
VALAMX   B     OKXIT                                                            
                                                                                
VALAWER  MVI   FERN,EXPWRK                                                      
         LA    R2,SCRWRKH                                                       
         B     VALAERR                                                          
                                                                                
VALACER  MVI   FERN,EXPCOM                                                      
         LA    R2,SCRCOMH                                                       
         B     VALAERR                                                          
                                                                                
VALAAER  MVI   FERN,INVALID                                                     
         ST    R2,FADR                                                          
...
VALWC    NTR1                                                                   
         ...
         GOTOR AFVAL,APOWRKH       NO, MUST HAVE AT LEAST 1 WORKCODE            
         BZ    VALWER                                                           
...
VALW04   CLI   SCRWRKH+5,0         ANYTHING ENTERED?                            
         ...
         MVI   FERN,NOINPUT        NO, PREPARE ERROR MESSAGE                    
         CLI   SCRCOMH+5,0                                                      
...
CHKEST   NTR1                                                                   
         ...
         USING JBLOCKD,RE                                                       
         L     R1,JBACOLTB                                                      
         CLI   JBNEWEST,JBMCSQ     IS THIS BRANDO?                              
         BE    CHKE06              YES, HANDLE DIFFERENTLY                      
*                                                                               
         USING JBCOLD,R1                                                        
         LH    R0,JBNROWS                                                       
...
ADDSX    B     OKXIT                                                            
         ...
         EJECT                                                                  
***********************************************************************         
*        BUILD EXPENSE ANALYSIS ELEMENT                               *         
***********************************************************************         
                                                                                
         USING EXOELD,R3                                                        
BLDEXO   NTR1                                                                   
         LA    R3,ELEMENT                                                       
         MVC   EXOEL(EXOLNQ),SPACES                                             
         MVI   EXOEL,EXOELQ                                                     
         MVI   EXOLN,EXOLNQ                                                     
         MVC   EXOCLI(L'LCLI),LCLI   SAVE CLIENT                                
         MVC   EXOPRO(L'LPRO),LPRO        PRODUCT                               
         MVC   EXODOF,LDOF                DEBIT OFFICE                          
         MVC   EXOCOF,LCOF                CREDIT OFFICE                         
         MVC   EXOAOF,LAOF                ANALYSIS OFFICE                       
         MVC   EXODEP,LDEP                DEPARTMENT CODE                       
         MVC   EXOPER,LPER                PERSON CODE                           
...
ADDR10   AHI   R7,OAMLN2Q                                                       
         ...
         BNE   ADDR12              NO                                           
         GOTO1 BLDEXO              YES, BUILD EXOELQ ELEMENT                    
         GOTOR ADDELEM,AIOAREA1      AND ADD IT                                 
                                                                                
ADDR12   MVC   KEY,TRNKEY          MOVE FROM AIO1 INTO KEY                      
         GOTOR AREADL,AIOAREA2     AND READ INTO AIO2                           
         CLI   FERN,NOTFOUND                                                    
         BE    ADDR14              IF NOT FOUND, ADD IT                         
         GOTOR AWRITE,AIOAREA1                                                  
         B     ADDR16                                                           
                                                                                
ADDR14   GOTOR AADD,AIOAREA1       ADD SJ RECORD FROM AIO1                      
...
MANU02   LA    R2,KEY                                                           
         ...
         MVC   ONAKCPY,COMPANY                                                  
         GOTO1 ARDHI,AIOAREA4                                                   
         L     R2,AIOAREA4                                                      
*                                                                               
MANU04   CLC   ONAKEY(ONAKAPP-ONAKEY),KEYSAVE                                   
         BNE   MANU12              NOTHING OUT THERE                            
*                                                                               
         USING ONCELD,R3                                                        
         L     R3,AIOAREA4                                                      
         AH    R3,DATADISP                                                      
*                                                                               
MANU06   CLI   0(R3),0             END OF RECORD?                               
         BE    MANU08              YES, SEE IF ANY MORE RECORDS                 
         CLI   0(R3),ONCELQ        ORDER NUMBER CONTROL ELEMENT?                
         BE    MANU10              YES, CHECK RANGE                             
*                                                                               
         SR    R0,R0                                                            
         IC    R0,1(R3)                                                         
         AR    R3,R0                                                            
         B     MANU06                                                           
*                                                                               
MANU08   GOTO1 ASEQ,AIOAREA4                                                    
         B     MANU04                                                           
*                                                                               
MANU10   CLC   ORDKORD,ONCEND#                                                  
         BH    MANU08              NEW ORDER HIGHER THAN LAST ONE               
         CLC   ORDKORD,ONCSTA#                                                  
         BL    MANU08              OR LOWER THAN START                          
         MVI   FERN,ORDAFM                                                      
         B     MANUER                                                           
*                                                                               
         USING ORDELD,R6                                                        
MANU12   MVI   ELCODE,ORDELQ       GET ORDELQ FROM AIOAREA3                     
         BAS   RE,GETELIO                                                       
         BE    *+6                                                              
         DC    H'0'                HAVE TO HAVE IT                              
         ...
         TM    TYPE,EXPENSE+ANALZED   DON'T NEED THIS FOR EXPENSE               
         BNZ   MANU14                                                           
...
MANU14   MVC   KEY,ORDKEY          READ FOR RECORD BEFORE ADDING                
         GOTOR AREADL,AIOAREA4                                                  
         CLI   DMCB+8,0                                                         
         BNE   MANU18              NOT THERE, GO ADD IT                         
                                                                                
         L     R5,AIOAREA4                                                      
         TM    ORDRSTA,X'A0'       ONLY DELETED ORDERS CAN HAVE                 
         BNZ   MANU16              THEIR NUMBER REUSED                          
         MVI   FERN,ORDUSED                                                     
         B     MANUER                                                           
                                                                                
MANU16   GOTOR AWRITE,AIOAREA3                                                  
         BE    MANU20                                                           
         MVI   FERN,NOADD                                                       
         B     MANUER                                                           
                                                                                
MANU18   GOTOR AADD,AIOAREA3                                                    
         CLI   DMCB+8,0                                                         
         BE    MANU20                                                           
         DC    H'0'                                                             
                                                                                
MANU20   MVC   BIGKEY,ORDRECD      READ THE NEW RECORD                          
         GOTOR ARDHID,0                                                         
...
AUTO14   CLC   NEXTNUM,ONCEND#                                                  
         ...
         B     AUTO18                                                           
*                                                                               
AUTO16   L     R5,AIOAREA3         GET ORDER RECORD                             
         USING ORDRECD,R5                                                       
         ...
         BE    AUTO18              IF ORDER EXISTS, GET NEXT NUMBER             
...
SCRDES   DS    CL15                                                             
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'010ACPOP01   07/23/13'                                      
         END                                                                    
