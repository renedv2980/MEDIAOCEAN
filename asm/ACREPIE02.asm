*          DATA SET ACREPIE02  AT LEVEL 040 AS OF 08/17/00                      
*PHASE ACIE02A                                                                  
...
*                                  ALWAYS OPEN INCASE WRITING TO DASD           
         ...
         CLC   ALPHAID,=C'*B'       CHECK IF DDSB TEST                          
         BNE   *+10                                                             
         MVC   DSPARM+13(2),=C'BD'  IF SO-FUDGE BBDO                            
...
*                                                                               
         ...
         BAS   RE,BLD40            BUILD 40 RECORD AND CALL GETTXT              
         BAS   RE,BLD80                                                         
*                                                                               
...
BLD1040  MVC   IE10CI,UFCI         FILL IN USER FIELD DATA                      
         MVC   IE10PI,UFPI                                                      
         CLC   ALPHAID,=C'BD'      ARE WE DOING BBDO?                           
         BE    *+14                                                             
         CLC   ALPHAID,=C'JW'      OR JWT?                                      
         BNE   *+10                                                             
         MVC   IE10PIBD,UFPI       SPECIAL PI FIELD FOR BD/JW                   
         MVC   IE10JI,UFJI                                                      
         MVC   IE10CN,UFCN                                                      
         MVC   IE10TY,UFTY                                                      
         MVC   IE10PA,UFPA                                                      
...
*                                                                               
         ...
         LA    R3,IE10TEXT             POINTER TO CURRENT TEXT POSITION         
         CLC   ALPHAID,=C'BD'          ARE WE DOING BBDO?                       
         BE    *+14                                                             
         CLC   ALPHAID,=C'JW'          OR JWT?                                  
         BNE   JBTXT10                                                          
         LA    R6,IE10TXBD+L'IE10TXBD  DIFFERENT TEXT FIELDS FOR BD/JW          
         LA    R3,IE10TXBD                                                      
         DROP  R5                                                               
*                                                                               
         USING TFDELD,R4                                                        
...
*                                                                               
         LA    R7,0(R1,R3)         SEE IF THIS WILL FIT                         
...
JBTXT20  BCTR  R1,0                                                             
         ...
         MVC   0(0,R3),TFDTEXT                                                  
*                                                                               
         LA    R3,2(R1,R3)         BUMP TO NEXT OUTPUT POSITION                 
*                                                                               
...
*                                                                               
         LA    R5,TPREC                                                         
         CLC   ALPHAID,=C'BD'      ARE WE DOING BBDO OR JWT?                    
         BE    PRJT10                                                           
         CLC   ALPHAID,=C'JW'      SPECIAL FOR BOTH                             
         BE    PRJT10                                                           
         LA    R5,IE10TEXT                                                      
         ...
         B     EXIT                                                             
*                                                                               
PRJT10   LA    R5,IE10TXBD                                                      
         LA    R4,P                PRINT JOB TEXT IN 3 BLOCKS OF 100            
         MVC   PTEXT,0(R5)                                                      
         LA    R4,PSECOND                                                       
         MVC   PTEXT,L'PTEXT(R5)                                                
         LA    R4,PTHIRD                                                        
         MVC   PTEXT,((2*L'PTEXT)-1)(R5)                                        
         GOTO1 ACREPORT                                                         
         B     EXIT                                                             
         DROP  R4                                                               
         EJECT                                                                  
...
*                                  RETURN COMMISSION AMOUNT IN WCCOMM           
         BAS   RE,PRWC                                                          
         MVC   WCDESC,SPACES                                                    
         CLC   ALPHAID,=C'BD'      DONT ADD NAME UNLESS BBDO                    
         BE    *+14                                                             
         CLC   ALPHAID,=C'JW'      SPECIAL FOR BOTH                             
         BNE   *+8                                                              
...
*                                                                               
         ...
         MVC   IE40COM,SPACES                                                   
*                                                                               
         L     R4,ADACC            READ THE COMMENT ELM OF ACCT REC             
         MVI   ELCODE,SCMELQ       X'3E' - STANDARD COMMENT ELEMENT             
         BAS   RE,GETEL                                                         
         BNE   BLD4010                                                          
*                                                                               
         USING SCMELD,R4                                                        
         ...
         SHI   R1,1                                                             
         EX    R1,*+4                                                           
         MVC   IE40COM(0),SCMNARR     STANDARD COMMENT NARRATIVE                
*                                                                               
BLD4010  BAS   RE,PUTTAPE                                                       
         B     EXIT                                                             
         EJECT                                                                  
**********************************************************************          
* BUILD 80                                                           *          
...
**********************************************************************          
         SPACE 1                                                                
GETWCNM  NTR1                                                                   
         MVC   SVKEY,0(R2)         SAVE OFF KEY TO RE-EST SEQ                   
         L     RE,AIO3             CLEAR AIO3                                   
         LA    RF,2000                                                          
         SR    R1,R1                                                            
         MVCL  RE,R0                                                            
*                                                                               
         ...
         MVC   WCOKUNT(2),=C'SJ'                                                
         MVC   WCOKWRK,WC          WORK CODE                                    
*                                                                               
         L     R2,AIO3                                                          
         BAS   RE,READDM                                                        
         CLC   0(WCOKEND,R2),MYKEY SAME KEY/                                    
         BNE   GETWCNX                                                          
*                                                                               
         MVI   ELCODE,NAMELQ                                                    
         LR    R4,R2               PREP FOR GETEL                               
         ...
         SH    R1,=Y(NAMLN1Q+1)                                                 
         EX    R1,*+4                                                           
         MVC   WCDESC(0),NAMEREC                                                
         B     EXIT                                                             
*                                                                               
GETWCNX  DS    0H                                                               
         MVC   MYKEY,SVKEY                                                      
         L     R2,AIO3             DONT DISTROY AIO2                            
         BAS   RE,READDM           REESTABLISH SEQ                              
         B     EXIT                                                             
         EJECT                                                                  
**********************************************************************          
...
*        BATES REMOVED 6/00 - PER HILLARY                                       
VENTAB   DS    0CL15                                                            
...
SVKEY    DS    CL42                SAVE AREA FOR ESTIMATE KEY                   
...
COMRATE  DS    PL16                                                             
*                                                                               
ACIEDLN  EQU   *-ACIED             MUST BE LAST                                 
         EJECT                                                                  
**********************************************************************          
...
IE10TEXT DS    CL300               JOB LEVEL FOOTER COMMENT                     
         ORG   IE10PI                                                           
IE10PIBD DS    CL3                 PAYOR CODE-BBDO ONLY                         
IE10TXBD DS    CL299               JOB LEVEL FOOTER COMMENT-BBDO ONLY           
*                                                                               
...
IE30WC   DS    CL2                                                              
IE30TEXT DS    CL300                                                            
IE30WCNM DS    CL40                WORK CODE DESC-BBDO ONLY                     
         ORG   IEREC                                                            
IE40     DS    CL2                 BBDO ONLY - APR/00                           
IE40COM  DS    CL79                                                             
         ORG   IEREC                                                            
IE80     DS    CL2                                                              
IE80NET  DS    CL11                                                             
...
JBLOCKD  DSECT                                                                  
       ...
       ++INCLUDE ACJOBBLOCK                                                     
         PRINT ON                                                               
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'040ACREPIE02 08/17/00'                                      
         END                                                                    
