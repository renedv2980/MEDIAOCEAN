*          DATA SET NEBUY14    AT LEVEL 013 AS OF 02/26/20                      
*          DATA SET NEBUY14    AT LEVEL 095 AS OF 01/26/00                      
*PHASE T31114B,+0                                                               
***********************************************************************         
*  ID  LVL   DATE    TICKET            COMMENTS                       *         
* ---- --- ------- ------------ --------------------------------------*         
* JSAY 013 05DEC19 <SPEC-39994> ALLOW UNITS TO USE THE COS2 FACTOR    *         
*                               FROM THE PACKAGE                      *         
***********************************************************************         
         TITLE 'NETPAK BUY PROGRAM - MULTIPLE BUY/CALENDARIZE - T31114'         
T31114   CSECT                                                                  
         PRINT NOGEN                                                            
...
MULT     BAS   RE,ACTED                                                         
         ...
         GOTO1 DISLIST,0           DISPLAY GENERATED RECORDS                    
         B     MULT4                                                            
         SPACE                                                                  
MULT2    BAS   RE,LIST             BUILD SCHEDULE LIST                          
         CLI   ACTION,CAL          TEST FOR CALENDARIZE                         
         BE    MULT3               YES                                          
         GOTO1 VBLDRQST            GENERATE TURN AROUND REQUEST                 
         BAS   RE,BUILD            FOR BM, CONSTRUCT UNITS AND ADD              
MULT3    GOTO1 DISLIST,0                                                        
         SPACE                                                                  
MULT4    LA    R2,BUYACTH                                                       
         ST    R2,FADDR                                                         
         CLI   ACTION,CAL                                                       
         BNE   MULT6                                                            
         MVC   BUYMSG(18),=C'SCHEDULE DISPLAYED'                                
         MVI   BUYMSG+19,DASH                                                   
         MVC   BUYMSG+21(17),=C'ENTER NEXT ACTION'                              
         B     MULTX                                                            
         SPACE                                                                  
MULT6    BAS   RE,UPDPKG           SET SWITCH UNITS UNDER PACKAGE               
         TM    MODE,FIRST                                                       
         BZ    MULT8                                                            
         CLI   INPNTRY,0                                                        
         BNE   MULT8                                                            
         MVC   BUYMSG(22),=C'ENTER SCHEDULE DETAILS'                            
         B     MULTX                                                            
         SPACE                                                                  
...
*                                                                               
         ...
         SPACE                                                                  
         CLI   LISTDAY,0           TEST IF USER OVERRODE CALENDARIZING          
         BE    *+10                NO                                           
         MVC   UNODATE,LISTDATE                                                 
...
*                                                                               
         ...
         BAS   RE,CHKCOS2           CHECK COST2 SETTINGS                        
*  CHECK IF UNIT SPLIT BETWEEN CASH AND TRADE                                   
         GOTO1 VCALCASH,DMCB,AIOAREA1,ACOMFACS,(1,CLICPRD)                      
...
*  TURN ON ZERO ASSIGNED COST INPUT AS DEFAULT                                  
*                                                                               
         TM    CLIOPT4,COP4MIDS     TEST MIDAS CLIENT                           
         BZ    BUILD10              NO                                          
         TM    NETFLAG1,SMIDAS      TEST MIDAS STATION                          
         BNZ   BUILD10              YES                                         
         OI    NUUNITST,NUUNIGCI    SET ASSIGNED COST TO ZERO INPUT             
         OI    NUSDST3,X'80'        SET ASSIGNED COST TO OVERRIDE               
*                                                                               
         DROP  R3                                                               
*                                                                               
*  GET DEMO INFO                                                                
*                                                                               
BUILD10  GOTO1 VEDIT,DMCB,(C'D',(R6))                                           
         CLI   UNERROR,0                                                        
         BE    *+14                                                             
         MVC   FERN,UNERROR                                                     
         B     BUILDR                                                           
         GOTO1 (RF),(R1),(C'F',(R6))                                            
         SPACE                                                                  
         CLC   NUKDATE,SVOLDDTE    SEE IF EDIT CHANGE DATE                      
         BE    *+8                                                              
...
SUBPRTX  B     EXXMOD                                                           
         DROP  R4                                                               
         EJECT                                                                  
* SUB-ROUTINE TO DISPLAY SCHEDULE LIST                                          
*                                                                               
* AT ENTRY, R1=ZERO TO DISPLAY ALL UNITS OR A COUNTER VALUE                     
*                                                                               
DISLIST  NTR1                                                                   
         ZIC   R6,UNITS            DEFAULT COUNTER                              
         LTR   R1,R1                                                            
         BZ    *+6                                                              
         LR    R6,R1                                                            
         GOTO1 VCLEARF,DMCB,MULUNITH,MULLAST                                    
         LA    R4,SCHEDULE         R4 POINTS TO SCHEDULE ENTRY                  
         USING LISTD,R4                                                         
         LA    R2,MULUNITH                                                      
         SPACE                                                                  
DISL2    LA    R3,SCOUT            OUTPUT POINTER                               
         MVC   SCOUT,SPACES        PRE-CLEAR SCREEN OUTPUT AREA                 
         CLI   ACTION,BM           TEST FOR BUY MULTIPLE                        
         BNE   *+12                                                             
         MVI   0(R3),STAR          YES-PLUG IN A STAR                           
         LA    R3,1(R3)                                                         
         GOTO1 VDATCON,DMCB,(2,LISTDATE),(4,(R3))                               
         CLI   ACTION,CAL          TEST FOR ACTION CALENDARIZE                  
         BNE   DISL3                                                            
*                                                                               
         GOTO1 VGETPROG,DMCB,LISTDATE                                           
         CLI   FERN,0                                                           
         BE    DISL3                                                            
         ST    R2,FADDR                                                         
         B     ERROR                                                            
         SPACE                                                                  
DISL3    LA    R3,5(R3)            BUMP PAST UNIT DATE                          
         CLI   LISTSUB,0                                                        
         BE    DISL4                                                            
         CLI   LISTSUB,1                                                        
         BE    DISL4                                                            
*                                                                               
         ...
         AR    R3,R0               BUMP POINTER                                 
         SPACE                                                                  
DISL4    CLI   LISTDAY,0           TEST FOR DAY OVERRIDE                        
         BE    DISL5                                                            
         MVI   0(R3),COMMA         PUT A COMMA BEFORE OUTPUT                    
         LA    R3,1(R3)                                                         
         LA    R0,DAYS                                                          
         LA    RE,DAYTAB                                                        
         CLC   LISTDAY,0(RE)                                                    
...
DISL6    ZIC   R1,0(R2)                                                         
         SH    R1,=H'9'                                                         
         EX    R1,DISLMOVE         MOVE OUTPUT TO SCREEN                        
         LA    R4,LISTNTRL(R4)                                                  
         ZIC   R0,0(R2)                                                         
         AR    R2,R0               NEXT FIELD HEADER                            
         BCT   R6,DISL2                                                         
         B     EXXMOD                                                           
         DROP  R4                                                               
         SPACE 2                                                                
DISLMOVE MVC   8(0,R2),SCOUT                                                    
         EJECT                                                                  
* SUB-ROUTINE TO SET PACKAGE FOR UNITS                                          
*                                                                               
UPDPKG   NTR1                                                                   
         L     RE,APACKREC                                                      
...
STARECD  DSECT                                                                  
       ++INCLUDE SPGENSTA                                                       
**PAN#1  CSECT                                                                  
**PAN#1  DC    CL21'013NEBUY14   02/26/20'                                      
         END                                                                    
